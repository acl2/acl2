...
(defthmd bitn-shift-down
  (implies (and (natp i)
           ...
           (equal (bitn (fl (/ x (expt 2 k))) i)
                  (bitn x (+ i k))))
  :hints (("goal" :in-theory (e/d (bits-shift-down-1 bitn) (BITS-FL)))))
...
(defthmd bitn-rec-0
  (implies (integerp x)
           (equal (bitn x 0)
                  (mod x 2)))
  :hints (("goal" :use ((:instance bitn-def (n 0))))))
...
(defthm bit-expo-c
    (implies (and (<= (- (expt 2 n) (expt 2 k)) x)
	     ...
	     (equal (bitn x k) 1))
  :rule-classes ()
  :hints (("goal" :use ((:instance bitn-def (n k))
...
(defthmd bitn-neg
  (implies (and (< n 0)
                (integerp x))
           (equal (bitn x n) 0))
  :hints (("Goal" :in-theory (enable bitn bits-neg))))

; Start proof of sum-bitn.
...
(defthm sum-b-bounds
  (implies (all-bits-p b k)
           (and (<= 0 (sum-b b k))
                (< (sum-b b k) (expt 2 k))))
  :hints (("Goal" :expand ((expt 2 k))))
  :rule-classes :linear)

(defun sum-b-alt (b lower upper)
  (if (or (zp upper) ; for termination
          (equal upper lower))
      0
    (+ (* (expt 2 (1- upper)) (nth (1- upper) b))
       (sum-b-alt b lower (1- upper)))))

(defthmd sum-b-alt-split
  (implies (and (<= lower k)
                (< k upper)
                (integerp upper)
                (natp k))
           (equal (sum-b-alt b lower upper)
                  (+ (sum-b-alt b lower k)
                     (* (expt 2 k) (nth k b))
                     (sum-b-alt b (1+ k) upper)))))

(defthm nth-nthcdr
  (implies (and (natp i)
                (natp j))
           (equal (nth i (nthcdr j x))
                  (nth (+ i j) x))))

(defthm sum-b-alt-is-sum-b
  (implies (and (natp lower)
           ...
           (equal (sum-b-alt b lower upper)
                  (* (expt 2 lower)
                     (sum-b (nthcdr lower b) (- upper lower))))))

(defthm all-bits-p-goes-down
  ...
  (implies (and (< k upper)
           ...
           (equal (sum-b b upper)
                  (+ (sum-b b k)
                     (* (expt 2 k) (nth k b))
                     (* (expt 2 (1+ k))
                        (sum-b (nthcdr (1+ k) b) (1- (- upper k)))))))
  :hints (("Goal" :use ((:instance sum-b-alt-split (lower 0))))))

(defthm natp-sum-b
  (implies (and (force (all-bits-p b k))
                (force (natp k)))
           (natp (sum-b b k)))
  :rule-classes :type-prescription)

(defthm all-bits-p-nthcdr-cdr
  (implies (and (natp k)
                (natp n)
                (all-bits-p x n)
                (< k n))
           (all-bits-p (nthcdr k (cdr x)) (+ -1 n (* -1 k)))))
...
(defthm sum-bitn-1-1
  (implies (and (natp n)
                ...
                (natp k)
                (< k n))
           (equal (bitn (+ (sum-b b k)
                           (* (expt 2 k) (nth k b))
                           (* (expt 2 (1+ k))
                              (sum-b (nthcdr (1+ k) b) (1- (- n k)))))
                        k)
                  (bitn (+ (sum-b b k)
                           (* (expt 2 k) (nth k b)))
                        k)))
...
(defthmd sum-bitn-1
  (implies (and (natp n)
           ...
           (equal (bitn (+ (sum-b b k)
                        ...
                        k)
                  (nth k b)))
  :hints (("Goal" :use (sum-bitn-1-1 sum-bitn-1-2))))
  ...
  (implies (and (natp n)
		...
		(< k n))
           (equal (bitn (sum-b b n) k)
	          (nth k b)))
  :hints (("Goal" :use sum-bitn-1
           :in-theory (enable sum-b-split))))
