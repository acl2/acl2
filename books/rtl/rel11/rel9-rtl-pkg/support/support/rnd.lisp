...
(local (include-book "land0")) ; for roundup
(local (include-book "lior0")) ; for roundup
...
(defthm ieee-mode-p-implies-common-rounding-mode-p
...
(defthm RND-CONST-THM
  ...
  :rule-classes ()
  :hints (("Goal"  :in-theory (enable inf minf rnd common-rounding-mode-p ieee-mode-p)
...
;; (i-am-here) ;; Thu Oct 12 11:57:41 2006
   ...
   (implies (and (common-rounding-mode-p mode)
            ...
            (= (rnd x mode n)
               (if (roundup x mode n)
                   (fp+ (trunc x n) n)
                 (trunc x n))))
   :hints (("Goal"  :in-theory (enable inf minf rnd common-rounding-mode-p
                                       ieee-mode-p)
            :use (trunc-away trunc-exactp-b away-exactp-b)))))
...
; n) n), by fp+1 and trunc-exact-b.  We need that fact in order to apply
 ...
 (defthm roundup-thm-2-4-3
   (implies (and (integerp n)
            ...
            (equal (trunc (+ (fp+ (trunc x n) n)
                          ...
                          n)
                   (fp+ (trunc x n) n)))
   :hints (("Goal" :use ((:instance fp+1 (x (trunc x n)))
                         (:instance plus-trunc-corollary
                                    (x (fp+ (trunc x n) n))
                                    (y (trunc-rem x (1+ n))))
                         (:instance trunc-rem-small (n (1+ n)))
                         (:instance fp+1-2 (x (trunc x n))))
...
(defthm rnd-pos
  (implies (and (< 0 x)
           ...
           (> (rnd x mode n) 0))
  :rule-classes (:type-prescription)
  :hints (("goal" :in-theory (enable common-rounding-mode-p ieee-mode-p near rnd inf minf)
           :use ())))
...
(defthmd rnd-exactp-b
  (implies (and (rationalp x)
           ...
           (equal (equal x (rnd x mode n))
                  (exactp x n)))
  :hints (("Goal" :in-theory (enable ieee-mode-p common-rounding-mode-p rnd near+ minf inf)
           :use (near-exactp-b
                 trunc-exactp-b
                 away-exactp-b))))
...
(defthmd rnd-exactp-d
    (implies (and (rationalp x)
		  ...
		  (<= a x))
	     (<= a (rnd x mode n)))
    :hints (("Goal" :in-theory (enable away-minus inf minf
                                       common-rounding-mode-p
                                       ieee-mode-p flip rnd)
             :use ((:instance trunc-exactp-c)
                   (:instance away-exactp-c
...
;; 		  (<= a x))
;; 	     (<= a (rnd x mode n)))
;;   :hints (("Goal" :in-theory (enable ieee-mode-p common-rounding-mode-p rnd minf inf)
;; 		  :use (near-exactp-c
;; 			trunc-exactp-c
;; 			away-lower-pos))))
...
(defthm exactp-rnd
  (implies (and (rationalp x)
                ...
                (> n 0))
           (exactp (rnd x mode n) n))
  :hints (("Goal" :in-theory (enable ieee-mode-p common-rounding-mode-p rnd inf minf))))
...
(defthm rnd-choice
  ...
  :hints (("Goal" :in-theory (set-difference-theories
                              (enable rnd near+ inf minf near common-rounding-mode-p ieee-mode-p)
                              '(re evenp))))
  :rule-classes nil)
...
(defthm rnd-rarely-zero
  (implies (and (rationalp x)
                (integerp k)
                (case-split (< 0 k))
                (case-split (common-rounding-mode-p mode)))
           (equal (equal (rnd x mode k) 0)
                  (equal x 0)
           ))
  :hints (("Goal" :in-theory (enable rnd near+ minf inf near COMMON-ROUNDING-MODE-P ieee-mode-p))))
...
(defthm rnd-diff
  ...
  :hints (("Goal" :in-theory (enable rnd near near+ inf minf ieee-mode-p common-rounding-mode-p)
