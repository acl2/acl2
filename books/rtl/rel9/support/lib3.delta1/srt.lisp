  ...
  (implies (and (natp k)
           ...
           (and (<= (- (/ (expt 2 (* k (rho$)))))
                ...
                (< (- (/ (x$) (d$)) (q$ k))
                   (/ (expt 2 (* k (rho$)))))))
  :rule-classes ()
  :hints (("Goal" :use (lemma-2-1-corollary))))
...
(defthm admissible-div-table-criterion-converse
  ...
  :rule-classes ()
  :hints (("Goal" :use (lemma-2-2-converse))))
  ...
  (implies (natp k)
           (and (<= (- (/ (expt 2 (* k (rho%)))))
                ...
                (< (- (/ (x%) (d%)) (q% k))
                   (/ (expt 2 (* k (rho%)))))))
  :rule-classes ()
  :hints (("Goal" :use (theorem-1-a))))

(defthm srt-div-theorem-b
  (< (abs (p% k)) 2)
  :rule-classes ()
  :hints (("Goal" :use (theorem-1-b))))

(defthm srt-div-theorem-c
  (implies (not (zp k))
           (div-accessible-p (i% k) (j%) (m%) (n%)))
  :rule-classes ()
  :hints (("Goal" :use (theorem-1-c))))


;;**********************************************************************************
;; Formula for Square RooPArtial Remainder
;;**********************************************************************************
...
(defthmd sqrt-remainder-formula
  (implies (natp k)
           (equal (p$$ k)
                  (* (expt 2 (* k (rho$$)))
                     (- (x$$) (* (q$$ k) (q$$ k))))))
  :hints (("Goal" :use (lemma-3-1))))

;;**********************************************************************************
;; Equivalemt Bounds on Partial Remainder
;;**********************************************************************************
             ...
             (and (natp i)
                  ...
                  (or (<= h (- (expt 2 rho)))
                      ...
                      (> p (* (/ (1+ h) (expt 2 rho))
                              (+ d (* (1+ h) (expt 2 (- (* k rho)))))))))))
  :rule-classes ()
  :hints (("Goal" :use (lemma-3-4-converse))))
