...
(defthm mod012
  (implies (integerp x)
           (or (equal (mod x 2) 0)
               (equal (mod x 2) 1)))
  :rule-classes ()
  :hints (("Goal" :use ((:instance mod-bnd-1 (m x) (n 2))))))
...
(defthm mod-mod-2-not-equal
   ...
   :rule-classes ()
   :hints (("Goal" :use ((:instance quot-mod (m x) (n 2))
                         (:instance quot-mod (m (1+ x)) (n 2))))))
...
(defthm mod-must-be-n
  ...
  :hints (("goal" :in-theory (disable  MOD-does-nothing)
           :use ((:instance mod-does-nothing (x m) (y n))
                 (:instance mod-does-nothing (y n) (x (- m n)))
  ...
  (implies (and; (syntaxp (and (quotep k) (quotep y)))
           ...
           (equal (mod (+ k x) y)
                  (if (< (+ k (mod x y)) y)
                      (+ k (mod x y))
                    (+ (- k y) (mod x y)))))
  :hints (("Goal" :use((:instance mod-does-nothing (x (+ k (mod x y))) (y y))
                       (:instance mod-does-nothing (x (+ (- k y) (mod x y))) (y y)))
...
;                (<= 0 k)
           ...
           (equal (mod (+ k x) y)
                  (if (< (+ (mod k y) (mod x y)) y)
                    ...
                    (+ (mod k y) (mod x y) (* -1 y)))))
  :otf-flg t
  :hints (("Goal" :use((:instance mod-does-nothing (x (+ (mod k y) (mod x y))) (y y))
