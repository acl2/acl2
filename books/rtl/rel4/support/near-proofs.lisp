 ...
 (defthm near-near-3
    (implies (and (rationalp x)
	     ...
	     (and (<= x (near-witness x y n))
		  (<= (near-witness x y n) y)))
  :rule-classes ()
  :hints (("Goal" :in-theory (disable near near-monotone near-witness)
 ...
 (defthm near-est-1
   (implies (and (integerp n) (> n 0)
            ...
            (< (trunc x n)
               (- x (expt 2 (- (expo x) n)))))
   :rule-classes ()
   :hints (("goal" :in-theory (disable TRUNC-EXACTP-B REARRANGE-NEGATIVE-COEFS-<)
...
(local
 (defthm near-trunc-case-3
    (implies (and (rationalp x) (> x 0)
		...
		(trunc (+ x (expt 2 (- (expo x) n))) (1- n))))
  :rule-classes ()
  :hints (("goal" :in-theory (disable near-exactp-b)
		  :use ((:instance near-trunc-21)
			(:instance near-exactp-b)
