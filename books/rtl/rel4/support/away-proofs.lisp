...
(defthm hack-87
    (implies (and (rationalp x)
	     ...
	     (equal (+ x (* -1 (expt 2 (+ (expo x) (* -1 n))))
		    ...
		    (+ x (expt 2 (+ (expo x) (* -1 n))))))
  :rule-classes ()
  :hints (("goal" :use ((:instance expt-split (r 2) (i (- (expo x) n)) (j 1)))))))
...
(defthm trunc-away-15
    (implies (and (integerp n) (> n 0)
		 ...
		 (+ (- x (expt 2 (- (expo x) n)))
		    (expt 2 (- (1+ (expo x)) n)))))
  :rule-classes ()
  :hints (("goal" :in-theory (disable away-exactp-b EXPT-COMPARE EXPO-COMPARISON-REWRITE-TO-BOUND)
		  :use ((:instance trunc-away-8)
			(:instance trunc-away-3)
			(:instance expo-lower-bound)
			(:instance expt-strong-monotone (n (- (expo x) n)) (m (expo x)))
...
(local (defthm away-imp-8
    (implies (and (rationalp x)
		...
		x))
  :rule-classes ()
  :hints (("goal" :in-theory (disable trunc-exactp-c trunc-exactp-b)
...
(local (defthm away-imp-9
    (implies (and (rationalp x)
		...
		(away x n)))
  :rule-classes ()
  :hints (("goal"  :in-theory (disable trunc-exactp-b away-exactp-c)
           :use (away-imp-8
    ...
    (implies (and (rationalp x)
		...
		(trunc (+ x
			  ...
			  (- (expt 2 (- (1+ (expo x)) m))))
		       n)))
  :rule-classes ()
  :hints (("goal" :in-theory (disable AWAY-EXACTP-C) ;consider disabling this globally...
