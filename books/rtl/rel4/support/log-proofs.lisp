...
;			(:instance and-dist-a)
			(:instance mod-does-nothing (x (logand x y)) (y (expt 2 n)))))))
...
(defthm bitn-logand
  ...
  :hints (("goal" :induct (op-dist-induct x y n))
	  ...
	  ("subgoal *1/2" :use ((:instance bitn-rec-pos (k n))
				(:instance bitn-rec-pos (k n) (x y))
				(:instance bitn-rec-pos (k n) (x (logand x y)))
;				(:instance logand-fl)
                                ))))
...
(defthm bitn-logior
  ...
  :hints (("goal" :induct (op-dist-induct x y n))
	  ...
	  ("subgoal *1/2" :use ((:instance bitn-rec-pos (k n))
				(:instance bitn-rec-pos (k n) (x y))
				(:instance bitn-rec-pos (k n) (x (logior x y)))
				;(:instance logior-fl)
                                ))))
 ...
 (defthm logand-slice-<-pos-4
   (implies (and (integerp x) (>= x 0)
            ...
            (= (mod (fl (* 1/2 x)) (expt 2 (1- n)))
               (fl (* 1/2 x))))
   :rule-classes ()
   :hints (("Goal" :use ((:instance mod-does-nothing (x (fl (/ x 2))) (y (expt 2 (1- n))))
...
(defthm logand-slice-<-pos-7
  ...
  :rule-classes ()
  :hints (("Goal" :in-theory (enable bits)
           :use ((:instance mod-does-nothing (x x) (y (expt 2 n))))))))
...
(encapsulate
 ...
 (local (defthm logxor-rewrite-1
          ...
          :hints (("Goal" :in-theory (e/d (bvecp) ( lnot))
                   :use ((:instance lnot-fl (k 1))
                         (:instance lnot-fl (k 1) (x y)))))))
...
(defthm bitn-logxor
  ...
  :hints (("Goal" :induct (op-dist-induct x y n))
	  ...
	  ("Subgoal *1/2" :use ((:instance bitn-rec-pos (k n))
				(:instance bitn-rec-pos (k n) (x y))
				(:instance bitn-rec-pos (k n) (x (logxor x y)))
                                ))))
...
(encapsulate
 ...
 (local (defthm bits-logxor-2
               ...
               :hints (("Goal" :in-theory (set-difference-theories
                                           ...
                                           '(lnot-bvecp lnot))
                        :use (bits-logxor-1
                              (:instance bits-lnot (m n))
                              (:instance bits-lnot (x y) (m n)))))))
