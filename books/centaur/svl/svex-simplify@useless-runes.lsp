(SVL::RP-STATEP-UPDATE-NOT-SIMPLIFIED-ACTION
     (501 3
          (:REWRITE RP::RP-TERM-LISTP-IS-TRUE-LISTP))
     (484 1 (:DEFINITION TRUE-LISTP))
     (372 2 (:DEFINITION RP::RP-TERM-LISTP))
     (344 4
          (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (335 4 (:DEFINITION QUOTEP))
     (328 1
          (:LINEAR APPLY$-BADGEP-PROPERTIES . 1))
     (326 1 (:DEFINITION APPLY$-BADGEP))
     (275 4 (:DEFINITION SUBSETP-EQUAL))
     (202 30 (:DEFINITION MEMBER-EQUAL))
     (170 10 (:REWRITE SUBSETP-REFLEXIVE-LEMMA))
     (127 4
          (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (120 16
          (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-1))
     (116 1 (:DEFINITION RP::RP-TERMP))
     (60 60 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (58 16 (:REWRITE O-P-O-INFP-CAR))
     (52 52 (:REWRITE SUBSETP-IMPLIES-MEMBER))
     (52 1 (:DEFINITION RP::FALIST-CONSISTENT))
     (48 48
         (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-3))
     (45 1
         (:DEFINITION RP::FALIST-CONSISTENT-AUX))
     (38 38 (:TYPE-PRESCRIPTION RP::RP-TERMP))
     (32 32
         (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-2))
     (28 28 (:TYPE-PRESCRIPTION SUBSETP-EQUAL))
     (25 25 (:REWRITE NTH-WHEN-PREFIXP))
     (17 17 (:DEFINITION NTH))
     (15 5 (:REWRITE RP::RP-TERMP-IS-IF-LEMMA))
     (14 14 (:REWRITE O-P-DEF-O-FINP-1))
     (13 13
         (:TYPE-PRESCRIPTION RP::RP-TERM-LISTP))
     (12 5 (:REWRITE RP::IS-IF-RP-TERMP))
     (10 10
         (:TYPE-PRESCRIPTION RP::IS-IF$INLINE))
     (10 10
         (:REWRITE TRANSITIVITY-OF-SUBSETP-EQUAL))
     (9 9
        (:TYPE-PRESCRIPTION RP::FALIST-CONSISTENT-AUX))
     (9 9
        (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (8 8 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (8 3 (:REWRITE RP::RP-TERMP-CADR))
     (7 3
        (:REWRITE RP::ATOM-RP-TERMP-IS-SYMBOLP))
     (6 3 (:REWRITE DEFAULT-+-2))
     (6 3
        (:REWRITE APPLY$-BADGEP-PROPERTIES . 3))
     (5 5 (:TYPE-PRESCRIPTION ALL-NILS))
     (4 4 (:TYPE-PRESCRIPTION QUOTEP))
     (4 4 (:TYPE-PRESCRIPTION APPLY$-BADGEP))
     (4 4 (:REWRITE LEN-MEMBER-EQUAL-LOOP$-AS))
     (4 4 (:REWRITE DEFAULT-<-2))
     (4 4 (:REWRITE DEFAULT-<-1))
     (4 2 (:REWRITE RP::RP-TERMP-CADDR))
     (4 2 (:REWRITE RP::IS-RP-PSEUDO-TERMP))
     (4 1
        (:REWRITE RP::VALID-RP-STATE-SYNTAXP-IMPLIES-RP-STATEP))
     (4 1 (:DEFINITION ALL-NILS))
     (3 3
        (:REWRITE TRUE-LIST-LISTP-IMPLIES-TRUE-LISTP))
     (3 3
        (:REWRITE SYMBOL-LISTP-IMPLIES-SYMBOLP))
     (3 3 (:REWRITE DEFAULT-+-1))
     (2 2
        (:TYPE-PRESCRIPTION RP::VALID-RP-STATE-SYNTAXP))
     (2 2 (:TYPE-PRESCRIPTION NATP))
     (2 2 (:TYPE-PRESCRIPTION RP::IS-RP$INLINE))
     (2 2
        (:REWRITE INTEGER-LISTP-IMPLIES-INTEGERP))
     (2 2 (:LINEAR LEN-WHEN-PREFIXP))
     (2 1
        (:REWRITE RP::RP-TERMP-IMPLIES-SYMBOL-CAR-TERM))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-RP-STATEP))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP))
     (1 1 (:LINEAR BOUNDS-POSITION-EQUAL-AC))
     (1 1 (:LINEAR BOUNDS-POSITION-EQUAL))
     (1 1 (:DEFINITION WEAK-APPLY$-BADGE-P))
     (1 1 (:DEFINITION UPDATE-NTH)))
(SVL::RP-STATEP-RP-STATE-NEW-RUN
     (501 3
          (:REWRITE RP::RP-TERM-LISTP-IS-TRUE-LISTP))
     (484 1 (:DEFINITION TRUE-LISTP))
     (372 2 (:DEFINITION RP::RP-TERM-LISTP))
     (344 4
          (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (335 4 (:DEFINITION QUOTEP))
     (328 1
          (:LINEAR APPLY$-BADGEP-PROPERTIES . 1))
     (326 1 (:DEFINITION APPLY$-BADGEP))
     (275 4 (:DEFINITION SUBSETP-EQUAL))
     (202 30 (:DEFINITION MEMBER-EQUAL))
     (170 10 (:REWRITE SUBSETP-REFLEXIVE-LEMMA))
     (127 4
          (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (120 16
          (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-1))
     (116 1 (:DEFINITION RP::RP-TERMP))
     (60 60 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (52 52 (:REWRITE SUBSETP-IMPLIES-MEMBER))
     (52 1 (:DEFINITION RP::FALIST-CONSISTENT))
     (50 14 (:REWRITE O-P-O-INFP-CAR))
     (48 48
         (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-3))
     (45 1
         (:DEFINITION RP::FALIST-CONSISTENT-AUX))
     (41 41 (:REWRITE NTH-WHEN-PREFIXP))
     (38 38 (:TYPE-PRESCRIPTION RP::RP-TERMP))
     (32 32
         (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-2))
     (28 28 (:TYPE-PRESCRIPTION SUBSETP-EQUAL))
     (15 15 (:DEFINITION NTH))
     (15 5 (:REWRITE RP::RP-TERMP-IS-IF-LEMMA))
     (13 13
         (:TYPE-PRESCRIPTION RP::RP-TERM-LISTP))
     (12 12 (:REWRITE O-P-DEF-O-FINP-1))
     (12 5 (:REWRITE RP::IS-IF-RP-TERMP))
     (10 10
         (:TYPE-PRESCRIPTION RP::IS-IF$INLINE))
     (10 10
         (:REWRITE TRANSITIVITY-OF-SUBSETP-EQUAL))
     (10 2 (:DEFINITION ALISTP))
     (9 9
        (:TYPE-PRESCRIPTION RP::FALIST-CONSISTENT-AUX))
     (9 9
        (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (8 8 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (8 3 (:REWRITE RP::RP-TERMP-CADR))
     (7 7 (:REWRITE LEN-MEMBER-EQUAL-LOOP$-AS))
     (6 3 (:REWRITE DEFAULT-+-2))
     (6 3
        (:REWRITE APPLY$-BADGEP-PROPERTIES . 3))
     (6 2
        (:REWRITE RP::ATOM-RP-TERMP-IS-SYMBOLP))
     (5 5 (:TYPE-PRESCRIPTION ALL-NILS))
     (4 4 (:TYPE-PRESCRIPTION QUOTEP))
     (4 4 (:TYPE-PRESCRIPTION APPLY$-BADGEP))
     (4 4 (:REWRITE DEFAULT-<-2))
     (4 4 (:REWRITE DEFAULT-<-1))
     (4 4 (:DEFINITION UPDATE-NTH))
     (4 2 (:REWRITE RP::RP-TERMP-CADDR))
     (4 2 (:REWRITE RP::IS-RP-PSEUDO-TERMP))
     (4 1
        (:REWRITE RP::VALID-RP-STATE-SYNTAXP-IMPLIES-RP-STATEP))
     (4 1 (:DEFINITION ALL-NILS))
     (3 3
        (:REWRITE TRUE-LIST-LISTP-IMPLIES-TRUE-LISTP))
     (3 3 (:REWRITE DEFAULT-+-1))
     (2 2
        (:TYPE-PRESCRIPTION RP::VALID-RP-STATE-SYNTAXP))
     (2 2 (:TYPE-PRESCRIPTION NATP))
     (2 2 (:TYPE-PRESCRIPTION RP::IS-RP$INLINE))
     (2 2
        (:REWRITE SYMBOL-LISTP-IMPLIES-SYMBOLP))
     (2 2
        (:REWRITE INTEGER-LISTP-IMPLIES-INTEGERP))
     (2 2 (:LINEAR LEN-WHEN-PREFIXP))
     (2 1
        (:REWRITE RP::RP-TERMP-IMPLIES-SYMBOL-CAR-TERM))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-RP-STATEP))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP))
     (1 1 (:LINEAR BOUNDS-POSITION-EQUAL-AC))
     (1 1 (:LINEAR BOUNDS-POSITION-EQUAL))
     (1 1 (:DEFINITION WEAK-APPLY$-BADGE-P)))
(SVL::SYMBOLP-NOT-SIMPLIFIED-ACTION)
(SVL::TO-SVEX-FNC)
(SVL::SVEX-P-OF-TO-SVEX-FNC.RES
     (722446 10962 (:DEFINITION RP::RP-TERMP))
     (661896 7480 (:DEFINITION RP::RP-TERM-LISTP))
     (555442 6526
             (:REWRITE RP::ATOM-RP-TERMP-IS-SYMBOLP))
     (453068 30832 (:REWRITE RP::IS-IF-RP-TERMP))
     (218800 4416 (:REWRITE RP::RP-TERMP-CADDDR))
     (178832 10968 (:REWRITE RP::IS-RP-PSEUDO-TERMP))
     (178128 10968 (:REWRITE RP::RP-TERMP-CADDR))
     (177764 6520
             (:REWRITE RP::RP-TERMP-IMPLIES-SYMBOL-CAR-TERM))
     (176008 15448 (:REWRITE RP::RP-TERMP-CADR))
     (171692 24284
             (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (136196 5440 (:DEFINITION APPLY$-BADGEP))
     (101816 23920 (:DEFINITION QUOTEP))
     (88932 3892
            (:LINEAR APPLY$-BADGEP-PROPERTIES . 1))
     (80672 2256
            (:DEFINITION RP::FALIST-CONSISTENT))
     (77032 21610 (:REWRITE O-P-O-INFP-CAR))
     (73640 30832
            (:REWRITE RP::RP-TERMP-IS-IF-LEMMA))
     (69336 1568
            (:DEFINITION RP::FALIST-CONSISTENT-AUX))
     (61220 1012 (:DEFINITION SUBSETP-EQUAL))
     (58388 1260 (:REWRITE RP::RP-TERMP-CADDDDR))
     (57088 24284
            (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (55980 252
            (:REWRITE RP::RP-TERM-LISTP-IS-TRUE-LISTP))
     (51524 7704 (:DEFINITION MEMBER-EQUAL))
     (49456 1916
            (:LINEAR APPLY$-BADGEP-PROPERTIES . 2))
     (42808 42808
            (:TYPE-PRESCRIPTION RP::IS-IF$INLINE))
     (41864 41864
            (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (35044 1016 (:REWRITE SUBSETP-REFLEXIVE-LEMMA))
     (33860 33860
            (:TYPE-PRESCRIPTION RP::RP-TERM-LISTP))
     (30392 4040
            (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-1))
     (24572 24572 (:TYPE-PRESCRIPTION QUOTEP))
     (18474 18474 (:REWRITE O-P-DEF-O-FINP-1))
     (15400 15400 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (13788 13788
            (:TYPE-PRESCRIPTION RP::FALIST-CONSISTENT-AUX))
     (13349 13349 (:REWRITE SUBSETP-IMPLIES-MEMBER))
     (12108 12108
            (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-3))
     (11422 11422 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (9546 200
           (:REWRITE SVL::INTEGERP-IMPLIES-4VECP))
     (8076 8076
           (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-2))
     (6840 6840 (:TYPE-PRESCRIPTION APPLY$-BADGEP))
     (6526 6526
           (:REWRITE SYMBOL-LISTP-IMPLIES-SYMBOLP))
     (6448 6448
           (:TYPE-PRESCRIPTION RP::IS-RP$INLINE))
     (5440 5440 (:DEFINITION WEAK-APPLY$-BADGE-P))
     (4680 48
           (:REWRITE APPLY$-BADGEP-PROPERTIES . 1))
     (4616 32
           (:REWRITE APPLY$-BADGEP-PROPERTIES . 2))
     (2664 144
           (:REWRITE SV::SVAR-P-OF-CAR-WHEN-SVARLIST-P))
     (1800 300
           (:REWRITE SV::SVARLIST-P-OF-CDR-WHEN-SVARLIST-P))
     (1716 1716 (:TYPE-PRESCRIPTION NATP))
     (1586 196
           (:REWRITE SV::4VEC-P-OF-CAR-WHEN-4VECLIST-P))
     (1451 862
           (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
     (1298 200
           (:REWRITE SV::4VEC-P-WHEN-MAYBE-4VEC-P))
     (1063 1063
           (:REWRITE TRANSITIVITY-OF-SUBSETP-EQUAL))
     (1008 504
           (:REWRITE APPLY$-BADGEP-PROPERTIES . 3))
     (998 100
          (:REWRITE SV::MAYBE-4VEC-P-WHEN-4VEC-P))
     (900 150
          (:REWRITE SV::4VECLIST-P-OF-CDR-WHEN-4VECLIST-P))
     (888 888
          (:REWRITE SV::SVARLIST-P-WHEN-SUBSETP-EQUAL))
     (663 663
          (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP))
     (496 496
          (:REWRITE SV::4VECLIST-P-WHEN-SUBSETP-EQUAL))
     (444 444
          (:REWRITE SV::SVARLIST-P-WHEN-NOT-CONSP))
     (400 400
          (:REWRITE SV::4VEC-P-WHEN-MEMBER-EQUAL-OF-4VECLIST-P))
     (300 300
          (:TYPE-PRESCRIPTION SV::MAYBE-4VEC-P))
     (300 300
          (:REWRITE SV::SVAR-P-WHEN-MEMBER-EQUAL-OF-SVARLIST-P))
     (300 200 (:REWRITE SVL::BITP-IMPLIES-4VECP))
     (274 274 (:TYPE-PRESCRIPTION SV::4VEC-P))
     (252 252
          (:REWRITE TRUE-LIST-LISTP-IMPLIES-TRUE-LISTP))
     (252 252
          (:REWRITE LEN-MEMBER-EQUAL-LOOP$-AS))
     (252 252 (:DEFINITION TRUE-LISTP))
     (252 252 (:DEFINITION LEN))
     (252 252 (:DEFINITION ALL-NILS))
     (248 248
          (:REWRITE SV::4VECLIST-P-WHEN-NOT-CONSP))
     (111 111 (:TYPE-PRESCRIPTION SV::SVAR-P))
     (100 100 (:TYPE-PRESCRIPTION BITP))
     (80 8 (:DEFINITION LOOP$-AS))
     (50 50
         (:REWRITE INTEGER-LISTP-IMPLIES-INTEGERP))
     (37 37 (:TYPE-PRESCRIPTION SV::FNSYM-P))
     (24 8 (:DEFINITION EMPTY-LOOP$-AS-TUPLEP))
     (24 8 (:DEFINITION CDR-LOOP$-AS-TUPLE))
     (24 8 (:DEFINITION CAR-LOOP$-AS-TUPLE))
     (16 16 (:TYPE-PRESCRIPTION LOOP$-AS)))
(SVL::SVEXL-NODE-P-OF-TO-SVEX-FNC.RES
     (361223 5481 (:DEFINITION RP::RP-TERMP))
     (330948 3740 (:DEFINITION RP::RP-TERM-LISTP))
     (277721 3263
             (:REWRITE RP::ATOM-RP-TERMP-IS-SYMBOLP))
     (226534 15416 (:REWRITE RP::IS-IF-RP-TERMP))
     (109400 2208 (:REWRITE RP::RP-TERMP-CADDDR))
     (89416 5484 (:REWRITE RP::IS-RP-PSEUDO-TERMP))
     (89064 5484 (:REWRITE RP::RP-TERMP-CADDR))
     (88882 3260
            (:REWRITE RP::RP-TERMP-IMPLIES-SYMBOL-CAR-TERM))
     (88004 7724 (:REWRITE RP::RP-TERMP-CADR))
     (85846 12142
            (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (68098 2720 (:DEFINITION APPLY$-BADGEP))
     (50908 11960 (:DEFINITION QUOTEP))
     (44466 1946
            (:LINEAR APPLY$-BADGEP-PROPERTIES . 1))
     (40336 1128
            (:DEFINITION RP::FALIST-CONSISTENT))
     (38516 10805 (:REWRITE O-P-O-INFP-CAR))
     (36820 15416
            (:REWRITE RP::RP-TERMP-IS-IF-LEMMA))
     (34668 784
            (:DEFINITION RP::FALIST-CONSISTENT-AUX))
     (30492 504 (:DEFINITION SUBSETP-EQUAL))
     (29194 630 (:REWRITE RP::RP-TERMP-CADDDDR))
     (28544 12142
            (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (27990 126
            (:REWRITE RP::RP-TERM-LISTP-IS-TRUE-LISTP))
     (25452 3780 (:DEFINITION MEMBER-EQUAL))
     (24728 958
            (:LINEAR APPLY$-BADGEP-PROPERTIES . 2))
     (21404 21404
            (:TYPE-PRESCRIPTION RP::IS-IF$INLINE))
     (20932 20932
            (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (17514 504 (:REWRITE SUBSETP-REFLEXIVE-LEMMA))
     (16930 16930
            (:TYPE-PRESCRIPTION RP::RP-TERM-LISTP))
     (15120 2016
            (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-1))
     (12286 12286 (:TYPE-PRESCRIPTION QUOTEP))
     (9237 9237 (:REWRITE O-P-DEF-O-FINP-1))
     (7560 7560 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (6894 6894
           (:TYPE-PRESCRIPTION RP::FALIST-CONSISTENT-AUX))
     (6552 6552 (:REWRITE SUBSETP-IMPLIES-MEMBER))
     (6048 6048
           (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-3))
     (5711 5711 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (4773 100
           (:REWRITE SVL::INTEGERP-IMPLIES-4VECP))
     (4032 4032
           (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-2))
     (3420 3420 (:TYPE-PRESCRIPTION APPLY$-BADGEP))
     (3263 3263
           (:REWRITE SYMBOL-LISTP-IMPLIES-SYMBOLP))
     (3224 3224
           (:TYPE-PRESCRIPTION RP::IS-RP$INLINE))
     (2720 2720 (:DEFINITION WEAK-APPLY$-BADGE-P))
     (2340 24
           (:REWRITE APPLY$-BADGEP-PROPERTIES . 1))
     (2308 16
           (:REWRITE APPLY$-BADGEP-PROPERTIES . 2))
     (1332 72
           (:REWRITE SV::SVAR-P-OF-CAR-WHEN-SVARLIST-P))
     (900 150
          (:REWRITE SV::SVARLIST-P-OF-CDR-WHEN-SVARLIST-P))
     (870 870 (:TYPE-PRESCRIPTION NATP))
     (793 98
          (:REWRITE SV::4VEC-P-OF-CAR-WHEN-4VECLIST-P))
     (782 782
          (:REWRITE SVL::SVEXL-NODELIST-P-WHEN-NOT-CONSP))
     (649 100
          (:REWRITE SV::4VEC-P-WHEN-MAYBE-4VEC-P))
     (504 504 (:TYPE-PRESCRIPTION SUBSETP-EQUAL))
     (504 504
          (:REWRITE TRANSITIVITY-OF-SUBSETP-EQUAL))
     (504 252
          (:REWRITE APPLY$-BADGEP-PROPERTIES . 3))
     (499 50
          (:REWRITE SV::MAYBE-4VEC-P-WHEN-4VEC-P))
     (450 75
          (:REWRITE SV::4VECLIST-P-OF-CDR-WHEN-4VECLIST-P))
     (444 444
          (:REWRITE SV::SVARLIST-P-WHEN-SUBSETP-EQUAL))
     (248 248
          (:REWRITE SV::4VECLIST-P-WHEN-SUBSETP-EQUAL))
     (222 222
          (:REWRITE SV::SVARLIST-P-WHEN-NOT-CONSP))
     (200 200
          (:REWRITE SV::4VEC-P-WHEN-MEMBER-EQUAL-OF-4VECLIST-P))
     (150 150
          (:TYPE-PRESCRIPTION SV::MAYBE-4VEC-P))
     (150 150
          (:REWRITE SV::SVAR-P-WHEN-MEMBER-EQUAL-OF-SVARLIST-P))
     (150 100 (:REWRITE SVL::BITP-IMPLIES-4VECP))
     (126 126
          (:REWRITE TRUE-LIST-LISTP-IMPLIES-TRUE-LISTP))
     (126 126
          (:REWRITE LEN-MEMBER-EQUAL-LOOP$-AS))
     (126 126 (:DEFINITION TRUE-LISTP))
     (126 126 (:DEFINITION LEN))
     (126 126 (:DEFINITION ALL-NILS))
     (124 124 (:TYPE-PRESCRIPTION SV::4VEC-P))
     (124 124
          (:REWRITE SV::4VECLIST-P-WHEN-NOT-CONSP))
     (50 50 (:TYPE-PRESCRIPTION BITP))
     (36 36 (:TYPE-PRESCRIPTION SV::SVAR-P))
     (36 36 (:TYPE-PRESCRIPTION SV::FNSYM-P))
     (25 25
         (:REWRITE INTEGER-LISTP-IMPLIES-INTEGERP)))
(SVL::EX-FROM-RP-IS-NONNIL
     (60 15 (:REWRITE O-P-O-INFP-CAR))
     (30 30 (:TYPE-PRESCRIPTION O-P))
     (28 28
         (:TYPE-PRESCRIPTION RP::INCLUDE-FNC-SUBTERMS))
     (15 15 (:REWRITE O-P-DEF-O-FINP-1))
     (15 15 (:DEFINITION QUOTEP))
     (14 14 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (10 2
         (:REWRITE RP::ATOM-RP-TERMP-IS-SYMBOLP))
     (6 2 (:REWRITE RP::RP-TERMP-IS-IF-LEMMA))
     (6 2
        (:REWRITE RP::RP-TERMP-IMPLIES-SYMBOL-CAR-TERM))
     (4 4 (:TYPE-PRESCRIPTION RP::IS-IF$INLINE))
     (3 3
        (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (2 2
        (:REWRITE SYMBOL-LISTP-IMPLIES-SYMBOLP)))
(SVL::SVEX-P-TO-SVEX-FNC-QUOTE)
(SVL::4VEC-TO-SVEX (1419 133 (:REWRITE O<=-O-FINP-DEF))
                   (514 116 (:REWRITE O-P-O-INFP-CAR))
                   (389 145 (:REWRITE AC-<))
                   (226 133 (:REWRITE O-INFP-O-FINP-O<=))
                   (202 115 (:REWRITE DEFAULT-<-2))
                   (196 8
                        (:DEFINITION NONNEGATIVE-INTEGER-QUOTIENT))
                   (174 48 (:REWRITE O-FIRST-EXPT-<))
                   (166 116 (:REWRITE O-P-DEF-O-FINP-1))
                   (133 115
                        (:REWRITE RP::EQUALITY-MEASURE-LEMMA3))
                   (133 115
                        (:REWRITE RP::EQUALITY-MEASURE-LEMMA1))
                   (115 115 (:REWRITE DEFAULT-<-1))
                   (104 6
                        (:REWRITE RP::ATOM-RP-TERMP-IS-SYMBOLP))
                   (96 48 (:REWRITE O-FIRST-COEFF-<))
                   (83 67
                       (:REWRITE RP::EQUALITY-MEASURE-LEMMA2))
                   (80 51 (:REWRITE DEFAULT-+-2))
                   (79 79 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
                   (78 39 (:REWRITE O-FIRST-EXPT-DEF-O-FINP))
                   (68 8 (:DEFINITION NFIX))
                   (62 51 (:REWRITE DEFAULT-+-1))
                   (54 6 (:REWRITE RP::IS-IF-RP-TERMP))
                   (50 50 (:TYPE-PRESCRIPTION O-FINP))
                   (44 44
                       (:TYPE-PRESCRIPTION RP::IS-IF$INLINE))
                   (28 20 (:REWRITE DEFAULT-UNARY-MINUS))
                   (28 6 (:REWRITE RP::RP-TERMP-CADR))
                   (27 9 (:REWRITE RP::M-MEASURE-LEMMA6))
                   (27 9 (:REWRITE RP::IS-IF-CONS-COUNT))
                   (18 14 (:REWRITE DEFAULT-*-2))
                   (18 14 (:REWRITE DEFAULT-*-1))
                   (18 6
                       (:REWRITE RP::RP-TERMP-IMPLIES-SYMBOL-CAR-TERM))
                   (14 10 (:REWRITE O-INFP->NEQ-0))
                   (14 6 (:REWRITE RP::RP-TERMP-IS-IF-LEMMA))
                   (12 12
                       (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
                   (10 10
                       (:REWRITE INTEGER-LISTP-IMPLIES-INTEGERP))
                   (9 9 (:REWRITE RP::MEASURE-LEMMA7-2))
                   (9 9 (:REWRITE RP::MEASURE-LEMMA7))
                   (9 9 (:REWRITE RP::MEASURE-LEMMA6-5))
                   (9 9 (:REWRITE RP::M-MEASURE-LEMMA11))
                   (8 4
                      (:REWRITE RP::RP-TERMP-EXTRACT-FROM-RP))
                   (8 4 (:REWRITE RP::RP-TERMP-EX-FROM-RP))
                   (8 4 (:REWRITE INTEGERP==>NUMERATOR-=-X))
                   (8 4 (:REWRITE INTEGERP==>DENOMINATOR-=-1))
                   (8 4
                      (:REWRITE RP::EXTRACT-FROM-RP-PSEUDO-TERM-LISTP))
                   (6 6
                      (:REWRITE SYMBOL-LISTP-IMPLIES-SYMBOLP))
                   (6 6 (:REWRITE FOLD-CONSTS-IN-+))
                   (4 4 (:REWRITE DEFAULT-NUMERATOR))
                   (4 4 (:REWRITE DEFAULT-DENOMINATOR))
                   (3 1
                      (:REWRITE RP::QUOTEP-TERM-WITH-EX-FROM-RP)))
(SVL::4VEC-TO-SVEX-FLAG
     (1308 121 (:REWRITE O<=-O-FINP-DEF))
     (392 16
          (:DEFINITION NONNEGATIVE-INTEGER-QUOTIENT))
     (353 133 (:REWRITE AC-<))
     (282 50 (:REWRITE O-P-O-INFP-CAR))
     (234 145 (:REWRITE DEFAULT-<-2))
     (208 12
          (:REWRITE RP::ATOM-RP-TERMP-IS-SYMBOLP))
     (202 121 (:REWRITE O-INFP-O-FINP-O<=))
     (178 49 (:REWRITE O-FIRST-EXPT-<))
     (163 145
          (:REWRITE RP::EQUALITY-MEASURE-LEMMA3))
     (163 145
          (:REWRITE RP::EQUALITY-MEASURE-LEMMA1))
     (145 145 (:REWRITE DEFAULT-<-1))
     (136 16 (:DEFINITION NFIX))
     (133 89 (:REWRITE DEFAULT-+-2))
     (132 50 (:REWRITE O-P-DEF-O-FINP-1))
     (128 96
          (:REWRITE RP::EQUALITY-MEASURE-LEMMA2))
     (108 12 (:REWRITE RP::IS-IF-RP-TERMP))
     (103 89 (:REWRITE DEFAULT-+-1))
     (98 49 (:REWRITE O-FIRST-COEFF-<))
     (82 82 (:TYPE-PRESCRIPTION O-FINP))
     (80 40 (:REWRITE O-FIRST-EXPT-DEF-O-FINP))
     (56 40 (:REWRITE DEFAULT-UNARY-MINUS))
     (56 12 (:REWRITE RP::RP-TERMP-CADR))
     (54 18
         (:REWRITE RP::QUOTEP-TERM-WITH-EX-FROM-RP))
     (52 52
         (:TYPE-PRESCRIPTION RP::IS-IF$INLINE))
     (36 28 (:REWRITE DEFAULT-*-2))
     (36 28 (:REWRITE DEFAULT-*-1))
     (36 12
         (:REWRITE RP::RP-TERMP-IMPLIES-SYMBOL-CAR-TERM))
     (28 20 (:REWRITE O-INFP->NEQ-0))
     (28 12 (:REWRITE RP::RP-TERMP-IS-IF-LEMMA))
     (27 9 (:REWRITE RP::M-MEASURE-LEMMA6))
     (27 9 (:REWRITE RP::IS-IF-CONS-COUNT))
     (24 24
         (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (20 20
         (:REWRITE INTEGER-LISTP-IMPLIES-INTEGERP))
     (16 16 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (16 8
         (:REWRITE RP::RP-TERMP-EXTRACT-FROM-RP))
     (16 8 (:REWRITE RP::RP-TERMP-EX-FROM-RP))
     (16 8 (:REWRITE INTEGERP==>NUMERATOR-=-X))
     (16 8 (:REWRITE INTEGERP==>DENOMINATOR-=-1))
     (16 8
         (:REWRITE RP::EXTRACT-FROM-RP-PSEUDO-TERM-LISTP))
     (12 12
         (:REWRITE SYMBOL-LISTP-IMPLIES-SYMBOLP))
     (9 9 (:REWRITE RP::MEASURE-LEMMA7-2))
     (9 9 (:REWRITE RP::MEASURE-LEMMA7))
     (9 9 (:REWRITE RP::MEASURE-LEMMA6-5))
     (9 9 (:REWRITE RP::M-MEASURE-LEMMA11))
     (8 8 (:REWRITE DEFAULT-NUMERATOR))
     (8 8 (:REWRITE DEFAULT-DENOMINATOR))
     (6 6 (:REWRITE FOLD-CONSTS-IN-+)))
(FLAG::FLAG-EQUIV-LEMMA)
(SVL::4VEC-TO-SVEX-FLAG-EQUIVALENCES)
(SVL::4VEC-TO-SVEX-MEMOIZE-CONDITION)
(SVL::4VEC-TO-SVEX-MEMOIZE-CONDITION)
(SVL::FLAG-LEMMA-FOR-SVEXL-NODE-P-OF-4VEC-TO-SVEX
     (22224 1044
            (:DEFINITION NONNEGATIVE-INTEGER-QUOTIENT))
     (8520 5672 (:REWRITE DEFAULT-+-2))
     (6564 1044 (:DEFINITION NFIX))
     (6398 5672 (:REWRITE DEFAULT-+-1))
     (6144 4882 (:REWRITE DEFAULT-*-2))
     (5464 4882 (:REWRITE DEFAULT-*-1))
     (4308 3128 (:REWRITE DEFAULT-UNARY-MINUS))
     (3830 3542 (:REWRITE DEFAULT-<-1))
     (3542 3542
           (:REWRITE RP::EQUALITY-MEASURE-LEMMA1))
     (3384 3384
           (:REWRITE INTEGER-LISTP-IMPLIES-INTEGERP))
     (1782 1254 (:REWRITE DEFAULT-NUMERATOR))
     (1758 1278 (:REWRITE DEFAULT-DENOMINATOR))
     (928 928
          (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP))
     (912 304
          (:REWRITE RP::QUOTEP-TERM-WITH-EX-FROM-RP))
     (840 840
          (:REWRITE RATIONAL-LISTP-IMPLIES-RATIONALP))
     (780 780
          (:REWRITE SV::SVAR-P-WHEN-MEMBER-EQUAL-OF-SVARLIST-P))
     (714 444 (:REWRITE O-INFP->NEQ-0))
     (640 368 (:REWRITE RATIONALP-*))
     (527 134
          (:REWRITE SV::SVEX-P-OF-CAR-WHEN-SVEXLIST-P))
     (450 450 (:REWRITE FOLD-CONSTS-IN-+))
     (440 240
          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (374 362
          (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
     (273 273 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (240 240
          (:REWRITE ACL2-NUMBER-LISTP-IMPLIES-ACL2-NUMBERP))
     (236 236
          (:REWRITE SVL::LOGHEAD-NON-POS-SIZE))
     (210 210
          (:REWRITE SV::4VEC-P-WHEN-MEMBER-EQUAL-OF-4VECLIST-P))
     (206 206
          (:REWRITE SYMBOL-LISTP-IMPLIES-SYMBOLP))
     (186 186
          (:REWRITE RP::RP-TERMP-IMPLIES-SYMBOL-CAR-TERM))
     (105 105 (:REWRITE SVL::BITP-IMPLIES-4VECP))
     (40 5
         (:REWRITE SVL::SVEXL-NODE-P-OF-CAR-WHEN-SVEXL-NODELIST-P))
     (8 8 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (4 4 (:REWRITE SUBSETP-IMPLIES-MEMBER))
     (2 2 (:REWRITE RP::SYMBOLP-EX-FROM-RP)))
(SVL::SVEXL-NODE-P-OF-4VEC-TO-SVEX)
(SVL::SVEXL-NODELIST-P-OF-4VEC-TO-SVEX-LST)
(SVL::4VEC-TO-SVEX-TERMLIST-QUOTE)
(SVL::SVEXL-NODELIST-P-OF-4VEC-TO-SVEX-TERMLIST-QUOTE.RES
     (5490 90 (:DEFINITION RP::RP-TERMP))
     (5310 180
           (:REWRITE RP::RP-TERMP-IMPLIES-SYMBOL-CAR-TERM))
     (2340 45 (:DEFINITION RP::FALIST-CONSISTENT))
     (2117 599 (:REWRITE O-P-O-INFP-CAR))
     (2025 45
           (:DEFINITION RP::FALIST-CONSISTENT-AUX))
     (1151 137
           (:REWRITE SVL::SVEXLIST-P-IMPLIES-SVEXL-NODELIST-P))
     (939 194
          (:REWRITE SV::4VEC-P-OF-CAR-WHEN-4VECLIST-P))
     (810 135
          (:REWRITE SV::SVAR-P-OF-CAR-WHEN-SVARLIST-P))
     (752 98
          (:REWRITE SV::INTEGERP-OF-CDAR-WHEN-SVAR-BOOLMASKS-P))
     (640 640
          (:REWRITE SV::SVEXLIST-P-WHEN-SUBSETP-EQUAL))
     (630 180
          (:REWRITE RP::ATOM-RP-TERMP-IS-SYMBOLP))
     (540 180 (:REWRITE RP::RP-TERMP-IS-IF-LEMMA))
     (540 90
          (:REWRITE SV::SVEXLIST-P-OF-CDR-WHEN-SVEXLIST-P))
     (506 506 (:REWRITE O-P-DEF-O-FINP-1))
     (442 442
          (:REWRITE INTEGER-LISTP-IMPLIES-INTEGERP))
     (405 405
          (:TYPE-PRESCRIPTION RP::FALIST-CONSISTENT-AUX))
     (388 388
          (:REWRITE SV::4VEC-P-WHEN-MEMBER-EQUAL-OF-4VECLIST-P))
     (360 360
          (:TYPE-PRESCRIPTION RP::IS-IF$INLINE))
     (360 180 (:REWRITE RP::IS-IF-RP-TERMP))
     (360 90
          (:REWRITE SV::SVAR-BOOLMASKS-P-OF-CDR-WHEN-SVAR-BOOLMASKS-P))
     (320 320
          (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP))
     (315 315
          (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (286 51
          (:REWRITE SV::SVEX-P-WHEN-MAYBE-SVEX-P))
     (284 194 (:REWRITE SVL::BITP-IMPLIES-4VECP))
     (280 280
          (:REWRITE SV::4VECLIST-P-WHEN-SUBSETP-EQUAL))
     (277 47
          (:REWRITE SV::SVEX-P-OF-CAR-WHEN-SVEXLIST-P))
     (270 270
          (:REWRITE SV::SVARLIST-P-WHEN-SUBSETP-EQUAL))
     (270 270
          (:REWRITE SV::SVAR-P-WHEN-MEMBER-EQUAL-OF-SVARLIST-P))
     (270 270 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (270 45
          (:REWRITE SV::4VECLIST-P-OF-CDR-WHEN-4VECLIST-P))
     (188 188
          (:REWRITE SV::SVAR-BOOLMASKS-P-WHEN-NOT-CONSP))
     (180 180
          (:REWRITE SYMBOL-LISTP-IMPLIES-SYMBOLP))
     (180 90 (:REWRITE RP::RP-TERMP-CADR))
     (180 90 (:REWRITE RP::RP-TERMP-CADDR))
     (180 90 (:REWRITE RP::IS-RP-PSEUDO-TERMP))
     (180 45
          (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (141 49
          (:REWRITE SV::MAYBE-SVEX-P-WHEN-SVEX-P))
     (140 140
          (:REWRITE SV::4VECLIST-P-WHEN-NOT-CONSP))
     (137 137
          (:REWRITE SVL::SVEXL-NODELIST-P-WHEN-NOT-CONSP))
     (135 135 (:TYPE-PRESCRIPTION SV::SVAR-P))
     (135 135
          (:REWRITE SV::SVARLIST-P-WHEN-NOT-CONSP))
     (98 98
         (:TYPE-PRESCRIPTION SV::MAYBE-SVEX-P))
     (98 98
         (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
     (90 90
         (:TYPE-PRESCRIPTION RP::RP-TERM-LISTP))
     (90 90
         (:TYPE-PRESCRIPTION RP::IS-RP$INLINE))
     (90 90 (:TYPE-PRESCRIPTION BITP))
     (90 45
         (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (61 2 (:REWRITE SV::SVEXLIST-P-OF-CONS))
     (45 45 (:TYPE-PRESCRIPTION QUOTEP))
     (45 45 (:TYPE-PRESCRIPTION SV::FNSYM-P))
     (45 45 (:DEFINITION QUOTEP)))
(SVL::SVEXLIST-P-OF-4VEC-TO-SVEX-TERMLIST-QUOTE
     (197 4 (:DEFINITION SUBSETP-EQUAL))
     (149 10 (:DEFINITION MEMBER-EQUAL))
     (146 10
          (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-1))
     (137 1
          (:REWRITE SV::SVEX-P-OF-CAR-WHEN-SVEXLIST-P))
     (80 8 (:DEFINITION LOOP$-AS))
     (70 20
         (:REWRITE SV::4VEC-P-OF-CAR-WHEN-4VECLIST-P))
     (67 67 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (62 4
         (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
     (43 16 (:REWRITE O-P-O-INFP-CAR))
     (40 40
         (:REWRITE SV::4VEC-P-WHEN-MEMBER-EQUAL-OF-4VECLIST-P))
     (40 10
         (:REWRITE SV::INTEGERP-OF-CDAR-WHEN-SVAR-BOOLMASKS-P))
     (31 31
         (:REWRITE INTEGER-LISTP-IMPLIES-INTEGERP))
     (25 25 (:REWRITE SUBSETP-IMPLIES-MEMBER))
     (24 8 (:DEFINITION EMPTY-LOOP$-AS-TUPLEP))
     (24 8 (:DEFINITION CDR-LOOP$-AS-TUPLE))
     (24 8 (:DEFINITION CAR-LOOP$-AS-TUPLE))
     (20 20 (:REWRITE SVL::BITP-IMPLIES-4VECP))
     (20 20
         (:REWRITE SV::4VECLIST-P-WHEN-SUBSETP-EQUAL))
     (13 13
         (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP))
     (12 12 (:TYPE-PRESCRIPTION LOOP$-AS))
     (12 12
         (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-3))
     (12 12
         (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-2))
     (12 6 (:REWRITE SUBSETP-REFLEXIVE-LEMMA))
     (10 10
         (:REWRITE TRANSITIVITY-OF-SUBSETP-EQUAL))
     (10 10
         (:REWRITE SV::SVAR-BOOLMASKS-P-WHEN-NOT-CONSP))
     (10 10
         (:REWRITE SV::4VECLIST-P-WHEN-NOT-CONSP))
     (9 9 (:REWRITE O-P-DEF-O-FINP-1)))
(SVL::4VEC-TO-SVEX-TERMLIST
     (3368 3 (:REWRITE O<=-O-FINP-DEF))
     (1639 562 (:REWRITE DEFAULT-+-2))
     (956 562 (:REWRITE DEFAULT-+-1))
     (430 43 (:DEFINITION LENGTH))
     (344 86 (:DEFINITION INTEGER-ABS))
     (258 43 (:DEFINITION LEN))
     (200 50 (:REWRITE O-P-O-INFP-CAR))
     (172 43
          (:REWRITE APPLY$-BADGEP-PROPERTIES . 1))
     (162 81 (:DEFINITION APPLY$-BADGEP))
     (152 38
          (:REWRITE APPLY$-BADGEP-PROPERTIES . 2))
     (124 124 (:REWRITE FOLD-CONSTS-IN-+))
     (101 91 (:REWRITE DEFAULT-<-2))
     (101 91 (:REWRITE DEFAULT-<-1))
     (86 86 (:REWRITE DEFAULT-UNARY-MINUS))
     (86 43
         (:REWRITE STR::COERCE-TO-LIST-REMOVAL))
     (81 81 (:TYPE-PRESCRIPTION APPLY$-BADGEP))
     (81 81 (:DEFINITION WEAK-APPLY$-BADGE-P))
     (66 66
         (:LINEAR MEMBER-EQUAL-ACL2-COUNT-SMALLER-3))
     (50 50 (:REWRITE O-P-DEF-O-FINP-1))
     (45 45
         (:LINEAR MEMBER-EQUAL-ACL2-COUNT-SMALLER-2))
     (43 43
         (:TYPE-PRESCRIPTION STR::TRUE-LISTP-OF-EXPLODE))
     (43 43 (:TYPE-PRESCRIPTION LEN))
     (43 43
         (:REWRITE RATIONAL-LISTP-IMPLIES-RATIONALP))
     (43 43 (:REWRITE LEN-MEMBER-EQUAL-LOOP$-AS))
     (43 43 (:REWRITE INTEGERP==>NUMERATOR-=-X))
     (43 43
         (:REWRITE INTEGERP==>DENOMINATOR-=-1))
     (43 43
         (:REWRITE INTEGER-LISTP-IMPLIES-INTEGERP))
     (43 43
         (:REWRITE STR::EXPLODE-WHEN-NOT-STRINGP))
     (43 43 (:REWRITE DEFAULT-REALPART))
     (43 43 (:REWRITE DEFAULT-NUMERATOR))
     (43 43 (:REWRITE DEFAULT-IMAGPART))
     (43 43 (:REWRITE DEFAULT-DENOMINATOR))
     (43 43 (:REWRITE STR::CONSP-OF-EXPLODE))
     (30 30
         (:LINEAR MEMBER-EQUAL-ACL2-COUNT-SMALLER-0))
     (23 23
         (:LINEAR MEMBER-EQUAL-ACL2-COUNT-SMALLER-1))
     (16 2 (:REWRITE O-FIRST-EXPT-<))
     (15 3 (:REWRITE AC-<))
     (12 4 (:REWRITE O-FIRST-EXPT-DEF-O-FINP))
     (9 3 (:REWRITE O-INFP-O-FINP-O<=))
     (6 2 (:REWRITE O-FIRST-COEFF-<))
     (3 3
        (:REWRITE |a < b & b < c  =>  a < c|)))
(SVL::SVEXL-NODELIST-P-OF-4VEC-TO-SVEX-TERMLIST.RES
     (332 52
          (:REWRITE SVL::SVEXLIST-P-IMPLIES-SVEXL-NODELIST-P))
     (106 106
          (:REWRITE SV::SVEXLIST-P-WHEN-SUBSETP-EQUAL))
     (53 53
         (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP))
     (52 52
         (:REWRITE SVL::SVEXL-NODELIST-P-WHEN-NOT-CONSP))
     (23 23 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (21 1 (:REWRITE SV::SVEXLIST-P-OF-CONS))
     (11 2
         (:REWRITE SV::SVEX-P-WHEN-MAYBE-SVEX-P))
     (8 1
        (:REWRITE SV::MAYBE-SVEX-P-WHEN-SVEX-P))
     (4 4 (:TYPE-PRESCRIPTION SV::SVEX-P))
     (4 4
        (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
     (3 3
        (:TYPE-PRESCRIPTION SV::MAYBE-SVEX-P)))
(SVL::SVEXLIST-P-OF-4VEC-TO-SVEX-TERMLIST
     (71 2 (:DEFINITION SUBSETP-EQUAL))
     (38 2
         (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-1))
     (36 3 (:DEFINITION MEMBER-EQUAL))
     (20 2 (:DEFINITION LOOP$-AS))
     (19 19 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (17 17 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (8 8
        (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP))
     (8 2 (:REWRITE O-P-O-INFP-CAR))
     (7 7 (:REWRITE SUBSETP-IMPLIES-MEMBER))
     (7 7 (:REWRITE CDR-CONS))
     (7 7 (:REWRITE CAR-CONS))
     (6 3 (:REWRITE SUBSETP-REFLEXIVE-LEMMA))
     (6 2 (:DEFINITION EMPTY-LOOP$-AS-TUPLEP))
     (6 2 (:DEFINITION CDR-LOOP$-AS-TUPLE))
     (6 2 (:DEFINITION CAR-LOOP$-AS-TUPLE))
     (5 5
        (:REWRITE TRANSITIVITY-OF-SUBSETP-EQUAL))
     (4 4 (:TYPE-PRESCRIPTION LOOP$-AS))
     (2 2 (:REWRITE O-P-DEF-O-FINP-1))
     (2 2
        (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-3))
     (2 2
        (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-2)))
(SVL::NATP-RP-RW-STEP-LIMIT)
(SVL::UNSIGNED-BYTE-P-RW-STEP-LIMIT)
(SVL::SVEXL-NODE-SIMPLIFY-FN
     (61 57
         (:TYPE-PRESCRIPTION SVL::NATP-RP-RW-STEP-LIMIT))
     (10 1
         (:REWRITE SVL::SVEX-P-IMPLIES-SVEXL-NODE-P))
     (8 2
        (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (8 2
        (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (6 2
        (:REWRITE STATE-P-IMPLIES-AND-FORWARD-TO-STATE-P1))
     (5 1
        (:REWRITE SV::SVEX-P-WHEN-MAYBE-SVEX-P))
     (4 2 (:DEFINITION QUOTEP))
     (3 3 (:TYPE-PRESCRIPTION SV::SVEX-P))
     (2 2 (:TYPE-PRESCRIPTION QUOTEP))
     (2 2 (:TYPE-PRESCRIPTION SV::MAYBE-SVEX-P))
     (2 2
        (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
     (2 2
        (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (2 2 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (2 1
        (:REWRITE SV::MAYBE-SVEX-P-WHEN-SVEX-P)))
(SVL::SVEXL-NODE-P-OF-SVEXL-NODE-SIMPLIFY.NODE-NEW)
(SVL::VALID-RP-STATE-SYNTAXP-OF-SVEXL-NODE-SIMPLIFY.RP-STATE-RES
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP)))
(SVL::SVEXL-NODELIST-SIMPLIFY-FN
     (61 57
         (:TYPE-PRESCRIPTION SVL::NATP-RP-RW-STEP-LIMIT))
     (8 2
        (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (8 2
        (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (6 2
        (:REWRITE STATE-P-IMPLIES-AND-FORWARD-TO-STATE-P1))
     (6 1
        (:REWRITE SVL::SVEXLIST-P-IMPLIES-SVEXL-NODELIST-P))
     (4 2 (:DEFINITION QUOTEP))
     (2 2 (:TYPE-PRESCRIPTION SV::SVEXLIST-P))
     (2 2 (:TYPE-PRESCRIPTION QUOTEP))
     (2 2
        (:REWRITE SV::SVEXLIST-P-WHEN-SUBSETP-EQUAL))
     (2 2
        (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (2 2 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (1 1
        (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP))
     (1 1
        (:REWRITE SVL::SVEXL-NODELIST-P-WHEN-NOT-CONSP)))
(SVL::SVEXL-NODELIST-P-OF-SVEXL-NODELIST-SIMPLIFY.NODE-NEW)
(SVL::VALID-RP-STATE-SYNTAXP-OF-SVEXL-NODELIST-SIMPLIFY.RP-STATE-RES
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP)))
(SVL::SVEX-SIMPLIFY-LINEARIZE-AUX-FN)
(SVL::SVEXL-NODE-ARRAY-P-OF-SVEX-SIMPLIFY-LINEARIZE-AUX.SVEXL-NEW)
(SVL::VALID-RP-STATE-SYNTAXP-OF-SVEX-SIMPLIFY-LINEARIZE-AUX.RP-STATE-RES)
(SVL::LEMMA1)
(SVL::SVEX-SIMPLIFY-LINEARIZE-AUX-FN
     (28 7
         (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (28 7
         (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (24 8
         (:REWRITE STATE-P-IMPLIES-AND-FORWARD-TO-STATE-P1))
     (20 5 (:REWRITE O-P-O-INFP-CAR))
     (16 1
         (:REWRITE SVL::SVEX-P-IMPLIES-SVEXL-NODE-P))
     (14 7 (:DEFINITION QUOTEP))
     (7 7 (:TYPE-PRESCRIPTION QUOTEP))
     (7 7
        (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (7 7 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (6 6
        (:REWRITE SVL::SVEXL-NODE-ARRAY-P-WHEN-NOT-CONSP))
     (6 1
        (:REWRITE SV::SVEX-P-OF-CDAR-WHEN-SVEX-ALIST-P))
     (5 5 (:REWRITE O-P-DEF-O-FINP-1))
     (5 1
        (:REWRITE SV::SVEX-P-WHEN-MAYBE-SVEX-P))
     (4 1
        (:REWRITE SVL::SVEXL-NODE-P-OF-CDAR-WHEN-SVEXL-NODE-ALIST-P))
     (3 3 (:TYPE-PRESCRIPTION SV::SVEX-P))
     (2 2 (:TYPE-PRESCRIPTION SV::MAYBE-SVEX-P))
     (2 2
        (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
     (2 2
        (:REWRITE SV::SVEX-ALIST-P-WHEN-SUBSETP-EQUAL))
     (2 1
        (:REWRITE SV::MAYBE-SVEX-P-WHEN-SVEX-P))
     (2 1
        (:DEFINITION SVL::SVEX-SIMPLIFY-LINEARIZE-AUX-FN))
     (1 1
        (:REWRITE SVL::SVEXL-NODE-ALIST-P-WHEN-NOT-CONSP))
     (1 1
        (:REWRITE SV::SVEX-ALIST-P-WHEN-NOT-CONSP))
     (1 1 (:DEFINITION ACONS)))
(SVL::SVEX-SIMPLIFY-LINEARIZE-FN)
(SVL::SVEXL-P-OF-SVEX-SIMPLIFY-LINEARIZE.SVEXL-NEW
 (10 2
     (:REWRITE SV::SVEX-P-WHEN-MAYBE-SVEX-P))
 (7 1
    (:REWRITE SV::MAYBE-SVEX-P-WHEN-SVEX-P))
 (4 4
    (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
 (3 3 (:TYPE-PRESCRIPTION SV::MAYBE-SVEX-P))
 (1 1
    (:REWRITE SVL::SVEXL-OF-SVEXL-NODE-FIX-TOP-NODE-NORMALIZE-CONST))
 (1 1
    (:REWRITE SVL::SVEXL-OF-SVEXL-NODE-ARRAY-FIX-NODE-ARRAY-NORMALIZE-CONST))
 (1 1
    (:REWRITE SVL::SVEXL->TOP-NODE$INLINE-OF-SVEXL-FIX-X-NORMALIZE-CONST))
 (1 1
    (:REWRITE SVL::SVEXL->NODE-ARRAY$INLINE-OF-SVEXL-FIX-X-NORMALIZE-CONST)))
(SVL::VALID-RP-STATE-SYNTAXP-OF-SVEX-SIMPLIFY-LINEARIZE.RP-STATE-RES
 (1 1
    (:REWRITE SVL::SVEXL-OF-SVEXL-NODE-FIX-TOP-NODE-NORMALIZE-CONST))
 (1 1
    (:REWRITE SVL::SVEXL-OF-SVEXL-NODE-ARRAY-FIX-NODE-ARRAY-NORMALIZE-CONST))
 (1 1
    (:REWRITE SVL::SVEXL->TOP-NODE$INLINE-OF-SVEXL-FIX-X-NORMALIZE-CONST))
 (1 1
    (:REWRITE SVL::SVEXL->NODE-ARRAY$INLINE-OF-SVEXL-FIX-X-NORMALIZE-CONST))
 (1 1
    (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP)))
(SVL::SVEXLIST-SIMPLIFY-LINEARIZE-FN)
(SVL::SVEXLLIST-P-OF-SVEXLIST-SIMPLIFY-LINEARIZE.SVEXLLIST-NEW
 (2 2
    (:REWRITE SV::SVEXLIST-P-WHEN-SUBSETP-EQUAL))
 (1 1
    (:REWRITE
         SVL::SVEXLLIST-OF-SVEXL-NODELIST-FIX-TOP-NODELIST-NORMALIZE-CONST))
 (1 1
    (:REWRITE
         SVL::SVEXLLIST-OF-SVEXL-NODE-ARRAY-FIX-NODE-ARRAY-NORMALIZE-CONST))
 (1
  1
  (:REWRITE
     SVL::SVEXLLIST->TOP-NODELIST$INLINE-OF-SVEXLLIST-FIX-X-NORMALIZE-CONST))
 (1
  1
  (:REWRITE
       SVL::SVEXLLIST->NODE-ARRAY$INLINE-OF-SVEXLLIST-FIX-X-NORMALIZE-CONST))
 (1 1
    (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP)))
(SVL::VALID-RP-STATE-SYNTAXP-OF-SVEXLIST-SIMPLIFY-LINEARIZE.RP-STATE-RES
 (1 1
    (:REWRITE
         SVL::SVEXLLIST-OF-SVEXL-NODELIST-FIX-TOP-NODELIST-NORMALIZE-CONST))
 (1 1
    (:REWRITE
         SVL::SVEXLLIST-OF-SVEXL-NODE-ARRAY-FIX-NODE-ARRAY-NORMALIZE-CONST))
 (1
  1
  (:REWRITE
     SVL::SVEXLLIST->TOP-NODELIST$INLINE-OF-SVEXLLIST-FIX-X-NORMALIZE-CONST))
 (1
  1
  (:REWRITE
       SVL::SVEXLLIST->NODE-ARRAY$INLINE-OF-SVEXLLIST-FIX-X-NORMALIZE-CONST))
 (1 1
    (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP)))
(SVL::CONS-COUNT-COMPARE)
(RP::RP-STATEP-OF-UPDATE-NOT-SIMPLIFIED-ACTION
     (501 3
          (:REWRITE RP::RP-TERM-LISTP-IS-TRUE-LISTP))
     (484 1 (:DEFINITION TRUE-LISTP))
     (372 2 (:DEFINITION RP::RP-TERM-LISTP))
     (344 4
          (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (335 4 (:DEFINITION QUOTEP))
     (328 1
          (:LINEAR APPLY$-BADGEP-PROPERTIES . 1))
     (326 1 (:DEFINITION APPLY$-BADGEP))
     (275 4 (:DEFINITION SUBSETP-EQUAL))
     (202 30 (:DEFINITION MEMBER-EQUAL))
     (170 10 (:REWRITE SUBSETP-REFLEXIVE-LEMMA))
     (127 4
          (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (120 16
          (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-1))
     (116 1 (:DEFINITION RP::RP-TERMP))
     (60 60 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (58 16 (:REWRITE O-P-O-INFP-CAR))
     (52 52 (:REWRITE SUBSETP-IMPLIES-MEMBER))
     (52 1 (:DEFINITION RP::FALIST-CONSISTENT))
     (48 48
         (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-3))
     (45 1
         (:DEFINITION RP::FALIST-CONSISTENT-AUX))
     (38 38 (:TYPE-PRESCRIPTION RP::RP-TERMP))
     (32 32
         (:REWRITE MEMBER-EQUAL-NEWVAR-COMPONENTS-2))
     (28 28 (:TYPE-PRESCRIPTION SUBSETP-EQUAL))
     (25 25 (:REWRITE NTH-WHEN-PREFIXP))
     (17 17 (:DEFINITION NTH))
     (15 5 (:REWRITE RP::RP-TERMP-IS-IF-LEMMA))
     (14 14 (:REWRITE O-P-DEF-O-FINP-1))
     (13 13
         (:TYPE-PRESCRIPTION RP::RP-TERM-LISTP))
     (12 5 (:REWRITE RP::IS-IF-RP-TERMP))
     (10 10
         (:TYPE-PRESCRIPTION RP::IS-IF$INLINE))
     (10 10
         (:REWRITE TRANSITIVITY-OF-SUBSETP-EQUAL))
     (9 9
        (:TYPE-PRESCRIPTION RP::FALIST-CONSISTENT-AUX))
     (9 9
        (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (8 8 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (8 3 (:REWRITE RP::RP-TERMP-CADR))
     (7 3
        (:REWRITE RP::ATOM-RP-TERMP-IS-SYMBOLP))
     (6 3 (:REWRITE DEFAULT-+-2))
     (6 3
        (:REWRITE APPLY$-BADGEP-PROPERTIES . 3))
     (5 5 (:TYPE-PRESCRIPTION ALL-NILS))
     (4 4 (:TYPE-PRESCRIPTION QUOTEP))
     (4 4 (:TYPE-PRESCRIPTION APPLY$-BADGEP))
     (4 4 (:REWRITE LEN-MEMBER-EQUAL-LOOP$-AS))
     (4 4 (:REWRITE DEFAULT-<-2))
     (4 4 (:REWRITE DEFAULT-<-1))
     (4 2 (:REWRITE RP::RP-TERMP-CADDR))
     (4 2 (:REWRITE RP::IS-RP-PSEUDO-TERMP))
     (4 1
        (:REWRITE RP::VALID-RP-STATE-SYNTAXP-IMPLIES-RP-STATEP))
     (4 1 (:DEFINITION ALL-NILS))
     (3 3
        (:REWRITE TRUE-LIST-LISTP-IMPLIES-TRUE-LISTP))
     (3 3
        (:REWRITE SYMBOL-LISTP-IMPLIES-SYMBOLP))
     (3 3 (:REWRITE DEFAULT-+-1))
     (2 2
        (:TYPE-PRESCRIPTION RP::VALID-RP-STATE-SYNTAXP))
     (2 2 (:TYPE-PRESCRIPTION NATP))
     (2 2 (:TYPE-PRESCRIPTION RP::IS-RP$INLINE))
     (2 2
        (:REWRITE INTEGER-LISTP-IMPLIES-INTEGERP))
     (2 2 (:LINEAR LEN-WHEN-PREFIXP))
     (2 1
        (:REWRITE RP::RP-TERMP-IMPLIES-SYMBOL-CAR-TERM))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-RP-STATEP))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP))
     (1 1 (:LINEAR BOUNDS-POSITION-EQUAL-AC))
     (1 1 (:LINEAR BOUNDS-POSITION-EQUAL))
     (1 1 (:DEFINITION WEAK-APPLY$-BADGE-P))
     (1 1 (:DEFINITION UPDATE-NTH)))
(RP::VALID-RP-STATE-SYNTAXP-OF-UPDATE-NOT-SIMPLIFIED-ACTION
     (152 38 (:REWRITE O-P-O-INFP-CAR))
     (118 59
          (:TYPE-PRESCRIPTION TRUE-LISTP-UPDATE-NTH))
     (76 76 (:TYPE-PRESCRIPTION O-P))
     (64 64 (:REWRITE NTH-WHEN-PREFIXP))
     (59 59 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (38 38 (:REWRITE O-P-DEF-O-FINP-1))
     (30 30 (:DEFINITION HONS-EQUAL))
     (25 25
         (:REWRITE SYMBOL-LISTP-IMPLIES-SYMBOLP))
     (25 25
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP))
     (25 25
         (:REWRITE RP::ATOM-RP-TERMP-IS-SYMBOLP))
     (15 15
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP))
     (14 14
         (:REWRITE RP::VALID-RP-STATE-SYNTAXP-AUX-NECC))
     (11 11 (:DEFINITION UPDATE-NTH)))
(SVL::SVEX-SIMPLIFY-TO-4VEC-FN
     (731 157
          (:REWRITE SV::SVEX-P-WHEN-MAYBE-SVEX-P))
     (482 92
          (:REWRITE SV::MAYBE-SVEX-P-WHEN-SVEX-P))
     (368 92
          (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (368 92
          (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (314 314
          (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
     (222 222
          (:TYPE-PRESCRIPTION SV::MAYBE-SVEX-P))
     (184 92 (:DEFINITION QUOTEP))
     (92 92 (:TYPE-PRESCRIPTION QUOTEP))
     (92 92
         (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (92 92 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (65 65
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP))
     (51 31 (:REWRITE DEFAULT-<-2))
     (33 31 (:REWRITE DEFAULT-<-1))
     (22 22
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-RP-STATEP))
     (22 22
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP))
     (18 9 (:REWRITE O-INFP->NEQ-0)))
(SVL::VALID-RP-STATE-SYNTAXP-OF-SVEX-SIMPLIFY-TO-4VEC.RP-STATE-RES
     (2754 204 (:DEFINITION RP::RP-TERM-LISTP))
     (1938 102
           (:REWRITE RP::RP-TERMP-RP-RW-SUBTERMS))
     (1020 1020 (:TYPE-PRESCRIPTION RP::RP-TERMP))
     (816 204
          (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (816 204
          (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (408 204 (:DEFINITION QUOTEP))
     (204 204 (:TYPE-PRESCRIPTION QUOTEP))
     (204 204
          (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (204 204 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (192 148
          (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP))
     (76 44 (:REWRITE DEFAULT-<-2))
     (52 52
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-RP-STATEP))
     (52 52
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP))
     (44 44
         (:TYPE-PRESCRIPTION RP::RP-STATE-PRESERVEDP))
     (44 44 (:REWRITE DEFAULT-<-1))
     (24 12 (:REWRITE O-INFP->NEQ-0)))
(SVL::SVEXLIST-SIMPLIFY-TO-4VEC-FN
     (404 101
          (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (404 101
          (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (202 101 (:DEFINITION QUOTEP))
     (101 101 (:TYPE-PRESCRIPTION QUOTEP))
     (101 101
          (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (101 101 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (74 74
         (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP))
     (74 74
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP))
     (57 34 (:REWRITE DEFAULT-<-2))
     (36 34 (:REWRITE DEFAULT-<-1))
     (27 27
         (:REWRITE TRANSITIVITY-OF-SUBSETP-EQUAL))
     (22 22
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-RP-STATEP))
     (22 22
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP))
     (18 9 (:REWRITE O-INFP->NEQ-0)))
(SVL::VALID-RP-STATE-SYNTAXP-OF-SVEXLIST-SIMPLIFY-TO-4VEC.RP-STATE-RES
     (2754 204 (:DEFINITION RP::RP-TERM-LISTP))
     (1938 102
           (:REWRITE RP::RP-TERMP-RP-RW-SUBTERMS))
     (1020 1020 (:TYPE-PRESCRIPTION RP::RP-TERMP))
     (816 204
          (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (816 204
          (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (408 204 (:DEFINITION QUOTEP))
     (204 204 (:TYPE-PRESCRIPTION QUOTEP))
     (204 204
          (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (204 204 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (192 148
          (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP))
     (76 44 (:REWRITE DEFAULT-<-2))
     (52 52
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-RP-STATEP))
     (52 52
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP))
     (44 44
         (:TYPE-PRESCRIPTION RP::RP-STATE-PRESERVEDP))
     (44 44 (:REWRITE DEFAULT-<-1))
     (24 12 (:REWRITE O-INFP->NEQ-0)))
(SVL::LOCALLY-SIMPLIFIED-TO-SVEX)
(SVL::SVEX-P-OF-LOCALLY-SIMPLIFIED-TO-SVEX.RES
     (10 2
         (:REWRITE SV::SVEX-P-WHEN-MAYBE-SVEX-P))
     (7 1
        (:REWRITE SV::MAYBE-SVEX-P-WHEN-SVEX-P))
     (4 4
        (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
     (4 1 (:REWRITE O-P-O-INFP-CAR))
     (3 3 (:TYPE-PRESCRIPTION SV::MAYBE-SVEX-P))
     (2 2 (:TYPE-PRESCRIPTION O-P))
     (1 1 (:REWRITE O-P-DEF-O-FINP-1))
     (1 1 (:REWRITE FN-CHECK-DEF-NOT-QUOTE)))
(SVL::LOCALLY-SIMPLIFIED-TO-SVEXLIST)
(SVL::SVEXLIST-P-OF-LOCALLY-SIMPLIFIED-TO-SVEXLIST.RES
     (4 1 (:REWRITE O-P-O-INFP-CAR))
     (2 2 (:TYPE-PRESCRIPTION O-P))
     (2 2
        (:REWRITE SV::SVEXLIST-P-WHEN-SUBSETP-EQUAL))
     (1 1
        (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP))
     (1 1 (:REWRITE O-P-DEF-O-FINP-1))
     (1 1 (:REWRITE FN-CHECK-DEF-NOT-QUOTE)))
(SVL::SVEX-SIMPLIFY-FN
     (26 6
         (:REWRITE SV::SVEX-P-WHEN-MAYBE-SVEX-P))
     (16 4
         (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (16 4
         (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (16 4
         (:REWRITE SV::MAYBE-SVEX-P-WHEN-SVEX-P))
     (12 12
         (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
     (12 4
         (:REWRITE STATE-P-IMPLIES-AND-FORWARD-TO-STATE-P1))
     (8 8 (:TYPE-PRESCRIPTION SV::MAYBE-SVEX-P))
     (8 4 (:DEFINITION QUOTEP))
     (4 4 (:TYPE-PRESCRIPTION QUOTEP))
     (4 4
        (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (4 4 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (4 2 (:REWRITE DEFAULT-<-1))
     (2 2 (:REWRITE DEFAULT-<-2))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-RP-STATEP))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP)))
(SVL::SVEX-P-OF-SVEX-SIMPLIFY.RES
     (30 6
         (:REWRITE SV::SVEX-P-WHEN-MAYBE-SVEX-P))
     (21 3
         (:REWRITE SV::MAYBE-SVEX-P-WHEN-SVEX-P))
     (12 12
         (:REWRITE SV::SVEX-P-WHEN-MEMBER-EQUAL-OF-SVEXLIST-P))
     (9 9 (:TYPE-PRESCRIPTION SV::MAYBE-SVEX-P))
     (8 4 (:REWRITE DEFAULT-<-2))
     (4 4 (:REWRITE DEFAULT-<-1)))
(SVL::VALID-RP-STATE-SYNTAXP-OF-SVEX-SIMPLIFY.RP-STATE-RES
     (12 3
         (:REWRITE RP::VALID-RP-STATE-SYNTAXP-IMPLIES-RP-STATEP))
     (10 8
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP))
     (8 4 (:REWRITE DEFAULT-<-2))
     (4 4 (:REWRITE DEFAULT-<-1))
     (3 3
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-RP-STATEP))
     (3 3
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP))
     (2 2
        (:TYPE-PRESCRIPTION RP::RP-STATE-PRESERVEDP)))
(SVL::SVEXLIST-SIMPLIFY-FN
     (16 4
         (:REWRITE RP::RP-TERMP-IMPLIES-SUBTERMS))
     (16 4
         (:REWRITE RP::RP-TERMP-IMPLIES-CDR-LISTP))
     (12 4
         (:REWRITE STATE-P-IMPLIES-AND-FORWARD-TO-STATE-P1))
     (8 4 (:DEFINITION QUOTEP))
     (4 4 (:TYPE-PRESCRIPTION QUOTEP))
     (4 4
        (:REWRITE RP::RP-TERMP-SHOULD-TERM-BE-IN-CONS-LHS))
     (4 4 (:REWRITE FN-CHECK-DEF-NOT-QUOTE))
     (4 2 (:REWRITE DEFAULT-<-1))
     (2 2
        (:REWRITE TRANSITIVITY-OF-SUBSETP-EQUAL))
     (2 2
        (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP))
     (2 2 (:REWRITE DEFAULT-<-2))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-RP-STATEP))
     (1 1
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP)))
(SVL::SVEXLIST-P-OF-SVEXLIST-SIMPLIFY.RES
     (8 4 (:REWRITE DEFAULT-<-2))
     (6 6
        (:REWRITE SV::SVEXLIST-P-WHEN-SUBSETP-EQUAL))
     (4 4 (:REWRITE DEFAULT-<-1))
     (3 3
        (:REWRITE SV::SVEXLIST-P-WHEN-NOT-CONSP)))
(SVL::VALID-RP-STATE-SYNTAXP-OF-SVEXLIST-SIMPLIFY.RP-STATE-RES
     (12 3
         (:REWRITE RP::VALID-RP-STATE-SYNTAXP-IMPLIES-RP-STATEP))
     (10 8
         (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-VALID-RP-STATE-SYNTAXP))
     (8 4 (:REWRITE DEFAULT-<-2))
     (4 4 (:REWRITE DEFAULT-<-1))
     (3 3
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-RP-STATEP))
     (3 3
        (:REWRITE RP::RP-STATE-PRESERVEDP-IMPLIES-ALISTP))
     (2 2
        (:TYPE-PRESCRIPTION RP::RP-STATE-PRESERVEDP)))
