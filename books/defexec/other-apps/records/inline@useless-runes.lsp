(SYMBOL-BINARY-APPEND)
(PREFIX-MATCH (5 5
                 (:TYPE-PRESCRIPTION STRINGP-SUBSEQ-TYPE-PRESCRIPTION)))
(STOBJ-ACCESS-FN)
(EXEC-CALL)
(EXEC-FNNAME)
(IS-STOBJ-NAME)
(DROP-STOBJS)
(EXEC-TRM (79 38 (:REWRITE DEFAULT-+-2))
          (53 38 (:REWRITE DEFAULT-+-1))
          (32 8 (:REWRITE COMMUTATIVITY-OF-+))
          (32 8 (:DEFINITION INTEGER-ABS))
          (32 4 (:DEFINITION LENGTH))
          (20 4 (:DEFINITION LEN))
          (13 13 (:REWRITE DEFAULT-CDR))
          (13 10 (:REWRITE DEFAULT-<-2))
          (12 10 (:REWRITE DEFAULT-<-1))
          (11 11 (:REWRITE FOLD-CONSTS-IN-+))
          (9 9 (:REWRITE DEFAULT-CAR))
          (8 8 (:REWRITE DEFAULT-UNARY-MINUS))
          (4 4 (:TYPE-PRESCRIPTION LEN))
          (4 4 (:REWRITE DEFAULT-REALPART))
          (4 4 (:REWRITE DEFAULT-NUMERATOR))
          (4 4 (:REWRITE DEFAULT-IMAGPART))
          (4 4 (:REWRITE DEFAULT-DENOMINATOR))
          (4 4 (:REWRITE DEFAULT-COERCE-2))
          (4 4 (:REWRITE DEFAULT-COERCE-1)))
(IS-EXEC-DECL)
(PARSE-EXEC-DECLS)
(COLLECT-EXEC-DECLS)
(DECR-FAST (5 5 (:REWRITE DEFAULT-<-2))
           (5 5 (:REWRITE DEFAULT-<-1))
           (1 1 (:REWRITE DEFAULT-+-2))
           (1 1 (:REWRITE DEFAULT-+-1)))
(ZP-FAST)
(LOOP-FAST (25 25 (:REWRITE DEFAULT-<-2))
           (25 25 (:REWRITE DEFAULT-<-1))
           (16 2 (:DEFINITION LOOP-FAST))
           (6 6 (:REWRITE DEFAULT-+-2))
           (6 6 (:REWRITE DEFAULT-+-1))
           (6 2 (:REWRITE FOLD-CONSTS-IN-+)))
(DECR-SLOW (5 5 (:REWRITE DEFAULT-<-2))
           (5 5 (:REWRITE DEFAULT-<-1))
           (1 1 (:REWRITE DEFAULT-+-2))
           (1 1 (:REWRITE DEFAULT-+-1)))
(ZP-SLOW)
(LOOP-SLOW (6 6 (:REWRITE DEFAULT-<-2))
           (6 6 (:REWRITE DEFAULT-<-1))
           (1 1 (:TYPE-PRESCRIPTION LOOP-SLOW))
           (1 1 (:REWRITE DEFAULT-+-2))
           (1 1 (:REWRITE DEFAULT-+-1)))
