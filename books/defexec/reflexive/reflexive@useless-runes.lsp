(WEIRD-IDENTITY (51 27 (:REWRITE DEFAULT-+-2))
                (36 27 (:REWRITE DEFAULT-+-1))
                (24 6 (:REWRITE COMMUTATIVITY-OF-+))
                (24 3 (:DEFINITION LENGTH))
                (16 13 (:REWRITE DEFAULT-<-1))
                (15 3 (:DEFINITION LEN))
                (13 13 (:REWRITE DEFAULT-<-2))
                (6 6 (:REWRITE FOLD-CONSTS-IN-+))
                (6 6 (:REWRITE DEFAULT-UNARY-MINUS))
                (6 6 (:REWRITE DEFAULT-CDR))
                (3 3 (:TYPE-PRESCRIPTION LEN))
                (3 3 (:REWRITE DEFAULT-REALPART))
                (3 3 (:REWRITE DEFAULT-NUMERATOR))
                (3 3 (:REWRITE DEFAULT-IMAGPART))
                (3 3 (:REWRITE DEFAULT-DENOMINATOR))
                (3 3 (:REWRITE DEFAULT-COERCE-2))
                (3 3 (:REWRITE DEFAULT-COERCE-1))
                (3 3 (:REWRITE DEFAULT-CAR)))
(WEIRD-IDENTITY-LOGIC (6 6 (:REWRITE DEFAULT-<-2))
                      (6 6 (:REWRITE DEFAULT-<-1))
                      (4 4 (:REWRITE DEFAULT-+-2))
                      (4 4 (:REWRITE DEFAULT-+-1)))
(WEIRD-IDENTITY-LEMMA (20 20 (:REWRITE DEFAULT-<-2))
                      (20 20 (:REWRITE DEFAULT-<-1))
                      (20 6 (:REWRITE DEFAULT-+-2))
                      (8 2
                         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                      (6 6 (:REWRITE DEFAULT-+-1)))
(WEIRD-IDENTITY (6 6 (:REWRITE DEFAULT-<-2))
                (6 6 (:REWRITE DEFAULT-<-1))
                (4 4 (:REWRITE DEFAULT-+-2))
                (4 4 (:REWRITE DEFAULT-+-1)))
