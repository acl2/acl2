(H)
(MAP-H)
(MAP-H-APPEND (26 22 (:REWRITE DEFAULT-CAR))
              (23 19 (:REWRITE DEFAULT-CDR))
              (11 11 (:TYPE-PRESCRIPTION TRUE-LISTP)))
(HP)
(FILTER-MAP-H)
(FILTER-MAP-H-APPEND (61 51 (:REWRITE DEFAULT-CAR))
                     (49 42 (:REWRITE DEFAULT-CDR))
                     (19 19 (:TYPE-PRESCRIPTION TRUE-LISTP)))
(BUMPER1)
(BUMPER2)
(BUMPER3)
(BUMPER4)
(GENERIC-RUN)
(M5-RUN)
(GENERIC-EXISTS)
(G)
(GENERIC-LIST-ITERATOR)
(GET-INTEGERS)
(GET-BIG-INTEGERS)
(GENERIC-RUN-SUM (22 13 (:REWRITE ZP-OPEN))
                 (18 18 (:REWRITE DEFAULT-+-2))
                 (18 18 (:REWRITE DEFAULT-+-1))
                 (10 10 (:REWRITE DEFAULT-<-2))
                 (10 10 (:REWRITE DEFAULT-<-1))
                 (8 6 (:REWRITE FOLD-CONSTS-IN-+)))
(GENERIC-LIST-ITERATOR-APPEND (20 17 (:REWRITE DEFAULT-CAR))
                              (18 15 (:REWRITE DEFAULT-CDR))
                              (18 9
                                  (:TYPE-PRESCRIPTION TRUE-LISTP-APPEND))
                              (9 9 (:TYPE-PRESCRIPTION TRUE-LISTP))
                              (9 9 (:TYPE-PRESCRIPTION BINARY-APPEND)))
(ASSOC-APPEND (1526 613
                    (:TYPE-PRESCRIPTION TRUE-LISTP-APPEND))
              (815 613 (:TYPE-PRESCRIPTION BINARY-APPEND))
              (613 613 (:TYPE-PRESCRIPTION TRUE-LISTP))
              (23 20 (:REWRITE DEFAULT-CAR))
              (19 16 (:REWRITE DEFAULT-CDR)))
(APP)
(REV)
(ASSOC-OF-APP (248 248 (:TYPE-PRESCRIPTION APP))
              (21 20 (:REWRITE DEFAULT-CAR))
              (17 16 (:REWRITE DEFAULT-CDR)))
(REV-REV (103 92 (:REWRITE DEFAULT-CDR))
         (92 81 (:REWRITE DEFAULT-CAR)))
(TRUE-LISTP-REV (52 45 (:REWRITE DEFAULT-CDR))
                (30 30 (:REWRITE DEFAULT-CAR)))
(TRIPLE-REV (26 3 (:DEFINITION REV))
            (15 3 (:DEFINITION APP))
            (7 7 (:REWRITE DEFAULT-CDR))
            (6 6 (:REWRITE DEFAULT-CAR))
            (5 1 (:DEFINITION TRUE-LISTP)))
(FFF)
(FFF-CONSTRAINT (5 5 (:TYPE-PRESCRIPTION FFF)))
(BASE)
(OP)
(OP-IS-ASSOCIATIVE)
(OP-IS-COMMUTATIVE)
(OP-IS-COMMUTATIVE2)
(MAP-OP)
(MAP-OP-REV (77 68 (:REWRITE DEFAULT-CDR))
            (77 68 (:REWRITE DEFAULT-CAR)))
(SUM-LIST)
(SUM-LIST-REV (10 5 (:REWRITE DEFAULT-+-2))
              (10 5 (:REWRITE DEFAULT-+-1))
              (5 5
                 (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
              (5 5 (:REWRITE DEFAULT-CDR))
              (5 5 (:REWRITE DEFAULT-CAR)))
(PROPERTYP)
(FF)
(CONSTRAINT-THM)
(GG)
(HH)
