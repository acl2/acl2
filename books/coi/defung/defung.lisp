...
(def::und construct-a-binding (var fapp dom default funmap defmap term)
  (declare (xargs :signature ((symbolp wf-fapp symbolp symbolp symbol-map lambda-map pseudo-termp) pseudo-termp)
  ...
  (if (and (symbolp (car fapp))
	...
	(if (and fx dx (equal (len (cadr (cdr dx))) (len (cdr fapp))))
	    `(mv-let (,dom ,var) (,(cdr fx) ,dom ,default ,@(cdr fapp))
...
(def::un construct-bindings-rec (blist dom default funmap defmap term)
  ...
  (if (endp blist) term
    (let ((term (construct-a-binding (caar blist) (cdar blist) dom default funmap defmap term)))
      (construct-bindings-rec (cdr blist) dom default funmap defmap term))))

(def::und construct-bindings (blist dom default funmap defmap term)
  (declare (xargs :signature ((binding-listp symbolp symbolp symbol-map lambda-map pseudo-termp) pseudo-termp)))
  (construct-bindings-rec blist dom default funmap defmap `(mv ,dom ,term)))

(mutual-recursion

 (defun monadic-surgery-args-list (fmap dom default defmap list blist vars res hit)
   (declare (xargs :measure (acl2-count list)
	    ...
	    (type (satisfies pseudo-term-listp) res))
    ...
    ((equal (car term) 'if)
     (let ((args (cdr term)))
       (let ((test (nth 0 args))
	     (then (nth 1 args))
	     (else (nth 2 args)))
	 (acl2::met ((test blist vars hit) (monadic-surgery-expr fmap dom default defmap test blist vars hit))
	   (acl2::met ((then then-blist vars hit) (monadic-surgery-expr fmap dom default defmap then nil vars hit))
	     (acl2::met ((else else-blist vars hit) (monadic-surgery-expr fmap dom default defmap else nil vars hit))
	       (if (or then-blist else-blist)
		   (met ((fvar vars) (gensym+ 'var vars))
		     (mv fvar (acons fvar `(if ,test ,(construct-bindings then-blist dom default fmap defmap then)
					     ,(construct-bindings else-blist dom default fmap defmap else)) blist) vars hit))
		 (mv `(if ,test ,then ,else) blist vars hit))))))))
    (t
     (acl2::met ((args blist vars hit) (monadic-surgery-args-list fmap dom default defmap (cdr term) blist vars nil hit))
       (cond
	((assoc-eq (car term) fmap)
	 (met ((fvar vars) (gensym+ (car term) vars))
	   (mv fvar (acons fvar `(,(car term) ,@args) blist) vars t)))
	((consp (car term))
	 (let ((formals (cadr (car term)))
	       (body    (caddr (car term))))
	   (acl2::met ((body bblist vars hit) (monadic-surgery-expr fmap dom default defmap body nil vars hit))
...
(def::und monadic-surgery (fmap dom default defmap term vars)
  (declare (xargs :measure (acl2-count term)
   ...
   ((equal (car term) 'if)
    (let ((args (cdr term)))
      (let ((test (nth 0 args))
	    (then (nth 1 args))
	    (else (nth 2 args)))
	(acl2::met ((test blist vars hit) (monadic-surgery-expr fmap dom default defmap test nil vars nil))
	  (let ((then (monadic-surgery fmap dom default defmap then vars))
		(else (monadic-surgery fmap dom default defmap else vars)))
	    (let ((term `(if ,test ,then ,else)))
	      (let ((term (construct-bindings-rec blist dom default fmap defmap term)))
		(if (not hit) term
		  `(mv-let (dom val) ,term
			   (if dom (mv t val) (mv nil ,default)))))))))))
   (t
    (acl2::met ((args blist vars hit) (monadic-surgery-args-list fmap dom default defmap (cdr term) nil vars nil nil))
      (let ((term (cond
		   ((assoc-eq (car term) fmap)
		    (construct-bindings-rec blist dom default fmap defmap `(,(cdr (assoc-eq (car term) fmap)) ,dom ,default ,@args)))
		   ((consp (car term))
		    (let ((formals (cadr (car term)))
			  (body    (caddr (car term))))
		      (let ((body (monadic-surgery fmap dom default defmap body vars)))
			(construct-bindings-rec blist dom default fmap defmap `((lambda ,formals ,body) ,@args)))))
		   (t (construct-bindings blist dom default fmap defmap (cons (car term) args))))))
	(if (not hit) term
	  `(mv-let (dom val) ,term
		   (if dom (mv t val) (mv nil ,default)))))))))
...
(defun mk-monadic (fnx fn fn-definition fn0 fn0-definition
		  ...
		  vars args arg-type-info ftest fbase default-value tbody xbody ec-call
		  )
  ...
  (let* ((omit               `(,fnx ,@ec-call ,@(ec-call-omit)))
	 ...
	 (fnx-tbody           (monadic-surgery (acons fn fnx nil) dom default defmap fnx-tbody vars))
    ...
    `(
      ...
      ;; value.
      (defun ,fnx (,dom ,default ,@args)
	,@arg-type-info
	(declare (type (satisfies booleanp) ,dom))
	(declare (xargs :verify-guards nil))
	(mbe :logic (met ((dom1 val) (mv (,fn0-domain ,@args) (,fn0 ,@args)))
		  ...
		  replace-domain-index-by-fn0-measure)
	       ...
	       ,@(and (not nx) 
		      (mk-monadic
		       ...
		       fn-induction fn-induction-rule fn-induction-is-fn-domain
		       open-fn-domain fn-domain-true
		       vars args arg-type-info ftest fbase default-value tbody sbody ec-call
		       ))
