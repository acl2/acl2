(ITH (5 5 (:REWRITE DEFAULT-<-2))
     (5 5 (:REWRITE DEFAULT-<-1))
     (1 1 (:REWRITE DEFAULT-+-2))
     (1 1 (:REWRITE DEFAULT-+-1)))
(OPEN-ITH-NOT-ZP (10 4 (:REWRITE ZP-OPEN))
                 (6 6 (:REWRITE DEFAULT-+-2))
                 (6 6 (:REWRITE DEFAULT-+-1))
                 (6 2 (:REWRITE FOLD-CONSTS-IN-+))
                 (5 5 (:REWRITE DEFAULT-<-2))
                 (5 5 (:REWRITE DEFAULT-<-1))
                 (2 2 (:REWRITE DEFAULT-CAR)))
(OPEN-ITH-CONSP (20 3 (:REWRITE OPEN-ITH-NOT-ZP))
                (12 12 (:REWRITE DEFAULT-<-2))
                (12 12 (:REWRITE DEFAULT-<-1))
                (8 5 (:REWRITE DEFAULT-+-2))
                (7 2 (:REWRITE FOLD-CONSTS-IN-+))
                (5 5 (:REWRITE DEFAULT-CDR))
                (5 5 (:REWRITE DEFAULT-CAR))
                (5 5 (:REWRITE DEFAULT-+-1))
                (4 4 (:TYPE-PRESCRIPTION ZP))
                (3 3
                   (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(ITH-NON-INCREASING (164 82 (:REWRITE DEFAULT-+-2))
                    (112 82 (:REWRITE DEFAULT-+-1))
                    (50 10 (:DEFINITION LEN))
                    (46 35 (:REWRITE DEFAULT-<-1))
                    (42 35 (:REWRITE DEFAULT-<-2))
                    (28 7 (:REWRITE OPEN-ITH-NOT-ZP))
                    (21 21 (:REWRITE DEFAULT-CDR))
                    (20 20 (:REWRITE FOLD-CONSTS-IN-+))
                    (20 20 (:REWRITE DEFAULT-UNARY-MINUS))
                    (16 10 (:REWRITE ZP-OPEN))
                    (14 14 (:TYPE-PRESCRIPTION ZP))
                    (12 12 (:REWRITE DEFAULT-CAR))
                    (10 10 (:REWRITE DEFAULT-NUMERATOR))
                    (10 10 (:REWRITE DEFAULT-DENOMINATOR))
                    (10 10 (:REWRITE DEFAULT-COERCE-2))
                    (10 10 (:REWRITE DEFAULT-COERCE-1))
                    (9 9 (:REWRITE DEFAULT-REALPART))
                    (9 9 (:REWRITE DEFAULT-IMAGPART))
                    (6 2 (:DEFINITION NFIX))
                    (2 2
                       (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(ITH-TO-NTH (15 9 (:REWRITE ZP-OPEN))
            (12 3 (:REWRITE OPEN-ITH-NOT-ZP))
            (6 6 (:REWRITE DEFAULT-CAR))
            (6 4 (:REWRITE DEFAULT-+-2))
            (6 2 (:DEFINITION NFIX))
            (4 4 (:REWRITE DEFAULT-CDR))
            (4 4 (:REWRITE DEFAULT-<-2))
            (4 4 (:REWRITE DEFAULT-<-1))
            (4 4 (:REWRITE DEFAULT-+-1))
            (2 2
               (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
