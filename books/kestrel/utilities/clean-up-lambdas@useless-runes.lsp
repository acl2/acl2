(PSEUDO-TERM-LISTP-OF-REVAPPEND-2
     (78 1 (:DEFINITION PSEUDO-TERMP))
     (53 53 (:REWRITE DEFAULT-CDR))
     (47 43 (:REWRITE DEFAULT-CAR))
     (40 8 (:DEFINITION LEN))
     (40 4 (:DEFINITION TAKE))
     (28 28
         (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (28 20 (:REWRITE DEFAULT-+-2))
     (25 3 (:DEFINITION LENGTH))
     (20 20 (:REWRITE DEFAULT-+-1))
     (16 4 (:REWRITE ZP-OPEN))
     (14 14
         (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (12 8 (:REWRITE DEFAULT-<-2))
     (12 4 (:REWRITE FOLD-CONSTS-IN-+))
     (12 4 (:DEFINITION LAST))
     (9 1 (:DEFINITION SYMBOL-LISTP))
     (8 8 (:TYPE-PRESCRIPTION TRUE-LISTP-TAKE))
     (8 8 (:REWRITE DEFAULT-<-1))
     (7 7 (:TYPE-PRESCRIPTION SYMBOL-LISTP))
     (6 3
        (:REWRITE SYMBOLP-OF-CAR-WHEN-SYMBOL-LISTP-CHEAP))
     (4 4 (:TYPE-PRESCRIPTION LAST))
     (4 2 (:DEFINITION TRUE-LISTP))
     (4 1 (:REWRITE SYMBOL-LISTP-OF-CDR))
     (2 2 (:REWRITE CONSP-OF-REVAPPEND))
     (1 1 (:REWRITE DEFAULT-COERCE-2))
     (1 1 (:REWRITE DEFAULT-COERCE-1)))
(PSEUDO-TERM-LISTP-OF-REVERSE
     (10 1 (:DEFINITION PSEUDO-TERM-LISTP))
     (2 2
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (2 1
        (:REWRITE PSEUDO-TERMP-OF-CAR-WHEN-PSEUDO-TERM-LISTP-CHEAP))
     (2 1
        (:REWRITE PSEUDO-TERM-LISTP-OF-CDR-WHEN-PSEUDO-TERM-LISTP-CHEAP))
     (1 1 (:TYPE-PRESCRIPTION PSEUDO-TERMP))
     (1 1 (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (1 1 (:REWRITE DEFAULT-CDR))
     (1 1 (:REWRITE DEFAULT-CAR)))
(DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS)
(SYMBOL-LISTP-OF-MV-NTH-0-OF-DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS
     (94 60 (:REWRITE DEFAULT-CAR))
     (93 59 (:REWRITE DEFAULT-CDR))
     (36 36 (:REWRITE CONSP-OF-REVERSE))
     (8 1 (:DEFINITION TRUE-LIST-FIX))
     (8 1 (:DEFINITION BINARY-APPEND)))
(PSEUDO-TERM-LISTP-OF-MV-NTH-1-OF-DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS
     (78 1 (:DEFINITION PSEUDO-TERMP))
     (69 63 (:REWRITE DEFAULT-CDR))
     (67 61 (:REWRITE DEFAULT-CAR))
     (41 41
         (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (25 3 (:DEFINITION LENGTH))
     (24 2 (:REWRITE REVERSE-OF-CONS))
     (22 2 (:DEFINITION BINARY-APPEND))
     (20 20
         (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (20 4 (:DEFINITION LEN))
     (10 10 (:TYPE-PRESCRIPTION REVERSE))
     (9 9 (:TYPE-PRESCRIPTION LEN))
     (9 1 (:DEFINITION SYMBOL-LISTP))
     (8 4 (:REWRITE DEFAULT-+-2))
     (7 7 (:TYPE-PRESCRIPTION SYMBOL-LISTP))
     (6 6 (:REWRITE CONSP-OF-REVERSE))
     (6 3
        (:REWRITE SYMBOLP-OF-CAR-WHEN-SYMBOL-LISTP-CHEAP))
     (4 4 (:REWRITE DEFAULT-+-1))
     (4 2 (:DEFINITION TRUE-LISTP))
     (4 1 (:REWRITE SYMBOL-LISTP-OF-CDR))
     (1 1 (:REWRITE DEFAULT-COERCE-2))
     (1 1 (:REWRITE DEFAULT-COERCE-1)))
(TRUE-LISTP-OF-MV-NTH-0-OF-DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS
     (25 25 (:REWRITE DEFAULT-CDR))
     (22 11 (:DEFINITION TRUE-LISTP))
     (14 14 (:REWRITE DEFAULT-CAR)))
(TRUE-LISTP-OF-MV-NTH-1-OF-DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS
     (25 25 (:REWRITE DEFAULT-CDR))
     (22 11 (:DEFINITION TRUE-LISTP))
     (14 14 (:REWRITE DEFAULT-CAR)))
(DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS
     (42 14 (:DEFINITION MEMBER-EQUAL))
     (29 29 (:REWRITE DEFAULT-CDR))
     (27 27 (:REWRITE DEFAULT-CAR))
     (6 6
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (2 2 (:TYPE-PRESCRIPTION PSEUDO-TERMP))
     (2 2
        (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP)))
(LEN-OF-MV-NTH-1-OF-DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS
 (68 34 (:REWRITE DEFAULT-+-2))
 (59 59 (:REWRITE DEFAULT-CDR))
 (34 34 (:REWRITE DEFAULT-+-1))
 (28 28 (:REWRITE DEFAULT-CAR))
 (24 12
     (:TYPE-PRESCRIPTION
          TRUE-LISTP-OF-MV-NTH-0-OF-DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS)))
(DROP-UNUSED-LAMBDA-BINDINGS (405 175 (:REWRITE DEFAULT-+-2))
                             (245 175 (:REWRITE DEFAULT-+-1))
                             (128 32 (:DEFINITION INTEGER-ABS))
                             (128 16 (:DEFINITION LENGTH))
                             (80 16 (:DEFINITION LEN))
                             (53 38 (:REWRITE DEFAULT-<-2))
                             (42 38 (:REWRITE DEFAULT-<-1))
                             (32 32 (:REWRITE DEFAULT-UNARY-MINUS))
                             (16 16 (:TYPE-PRESCRIPTION LEN))
                             (16 16 (:REWRITE DEFAULT-REALPART))
                             (16 16 (:REWRITE DEFAULT-NUMERATOR))
                             (16 16 (:REWRITE DEFAULT-IMAGPART))
                             (16 16 (:REWRITE DEFAULT-DENOMINATOR))
                             (16 16 (:REWRITE DEFAULT-COERCE-2))
                             (16 16 (:REWRITE DEFAULT-COERCE-1)))
(FLAG-DROP-UNUSED-LAMBDA-BINDINGS
     (737 335 (:REWRITE DEFAULT-+-2))
     (468 335 (:REWRITE DEFAULT-+-1))
     (264 66 (:DEFINITION INTEGER-ABS))
     (264 33 (:DEFINITION LENGTH))
     (165 33 (:DEFINITION LEN))
     (108 81 (:REWRITE DEFAULT-<-2))
     (94 81 (:REWRITE DEFAULT-<-1))
     (66 66 (:REWRITE DEFAULT-UNARY-MINUS))
     (33 33 (:TYPE-PRESCRIPTION LEN))
     (33 33 (:REWRITE DEFAULT-REALPART))
     (33 33 (:REWRITE DEFAULT-NUMERATOR))
     (33 33 (:REWRITE DEFAULT-IMAGPART))
     (33 33 (:REWRITE DEFAULT-DENOMINATOR))
     (33 33 (:REWRITE DEFAULT-COERCE-2))
     (33 33 (:REWRITE DEFAULT-COERCE-1))
     (12 2 (:TYPE-PRESCRIPTION RETURN-LAST))
     (2 2
        (:TYPE-PRESCRIPTION THROW-NONEXEC-ERROR)))
(FLAG::FLAG-EQUIV-LEMMA)
(FLAG-DROP-UNUSED-LAMBDA-BINDINGS-EQUIVALENCES)
(PSEUDO-TERM-LISTP-WHEN-SYMBOL-LISTP
     (12 6
         (:REWRITE PSEUDO-TERMP-OF-CAR-WHEN-PSEUDO-TERM-LISTP-CHEAP))
     (9 9 (:REWRITE DEFAULT-CAR))
     (9 5
        (:REWRITE PSEUDO-TERM-LISTP-OF-CDR-WHEN-PSEUDO-TERM-LISTP-CHEAP))
     (8 8
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (8 2 (:REWRITE SYMBOL-LISTP-OF-CDR))
     (7 7 (:REWRITE DEFAULT-CDR))
     (6 6
        (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP)))
(FLAG-LEMMA-FOR-PSEUDO-TERMP-OF-DROP-UNUSED-LAMBDA-BINDINGS
     (1413 55
           (:REWRITE PSEUDO-TERM-LISTP-WHEN-SYMBOL-LISTP))
     (1303 134 (:DEFINITION SYMBOL-LISTP))
     (1000 831 (:REWRITE DEFAULT-CDR))
     (864 184 (:REWRITE SYMBOL-LISTP-OF-CDR))
     (781 674 (:REWRITE DEFAULT-CAR))
     (594 27
          (:DEFINITION DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS))
     (312 156
          (:REWRITE SYMBOLP-OF-CAR-WHEN-SYMBOL-LISTP-CHEAP))
     (196 98 (:REWRITE DEFAULT-+-2))
     (135 27 (:DEFINITION MEMBER-EQUAL))
     (98 98 (:REWRITE DEFAULT-+-1))
     (61 61
         (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (51 51
         (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (27 27 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (18 18 (:REWRITE DEFAULT-COERCE-2))
     (18 18 (:REWRITE DEFAULT-COERCE-1))
     (16 1 (:REWRITE SYMBOL-LISTP-OF-CONS)))
(PSEUDO-TERMP-OF-DROP-UNUSED-LAMBDA-BINDINGS)
(PSEUDO-TERMP-OF-DROP-UNUSED-LAMBDA-BINDINGS-LST)
(DROP-UNUSED-LAMBDA-BINDINGS
  (146 134 (:REWRITE DEFAULT-CDR))
  (100 20 (:DEFINITION LEN))
  (98 92 (:REWRITE DEFAULT-CAR))
  (84 2
      (:DEFINITION DROP-UNUSED-LAMBDA-BINDINGS))
  (44 2
      (:DEFINITION DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS))
  (40 20 (:REWRITE DEFAULT-+-2))
  (20 20 (:REWRITE DEFAULT-+-1))
  (20 10 (:DEFINITION TRUE-LISTP))
  (12 4
      (:TYPE-PRESCRIPTION
           TRUE-LISTP-OF-MV-NTH-1-OF-DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS))
  (12 4
      (:TYPE-PRESCRIPTION
           TRUE-LISTP-OF-MV-NTH-0-OF-DROP-UNUSED-LAMBDA-FORMALS-AND-ACTUALS))
  (10 2 (:DEFINITION MEMBER-EQUAL))
  (9 9 (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
  (9 9
     (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
  (9 3
     (:DEFINITION DROP-UNUSED-LAMBDA-BINDINGS-LST))
  (5 5 (:REWRITE DEFAULT-COERCE-2))
  (5 5 (:REWRITE DEFAULT-COERCE-1))
  (2 2 (:TYPE-PRESCRIPTION MEMBER-EQUAL)))
