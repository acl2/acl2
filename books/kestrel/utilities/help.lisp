...
(defun try-enabling-functions (body-fns body state)
  (declare (xargs :guard (symbol-listp body-fns)
                  :mode :program
                  :stobjs state))
  (if (endp body-fns)
...
(defun try-to-find-hints-for-claim (body state)
  (declare (xargs :mode :program
                  :stobjs state))
  (b* ((wrld (w state))
       (translated-body (translate-term body 'try-to-find-hints-for-claim wrld))
       (body-fns (all-fnnames translated-body))
       ((mv erp provedp hints state) (try-enabling-functions body-fns body state))
       ((when erp) (mv erp nil nil state))
       ((when provedp)
