...
;; $KESTREL_ACL2/utilities/), passing it arguments SCRIPT-ARGS.  Returns (mv
;; erp result state), where ERP is nil or an exit status from the script and
;; RESULT is the output generated by the script.
...
(defund call-script (script-name script-args state)
  ...
  (mv-let (erp kestrel-acl2 state)
    ...
    (if (or erp ;always nil in practice
            (not (stringp kestrel-acl2)))
        (prog2$
         (er hard? 'call-script "Could not read environment variable KESTREL_ACL2")
         (mv t ;error
             nil state))
      (sys-call+
       (concatenate 'string kestrel-acl2 "/utilities/" script-name)
       script-args state))))
