...
(defxdoc composite-constructors
  ...
  :long
  "<p>These correspond to multiple
      ...
      XML tags, preprocessor directives, and concatenations.</p>")
...
(defxdoc generic-composite-constructors
  ...
  :long
  "<p>These are composite XDOC constructors
      that should be generally useful.
      Other composite XDOC constructors are more specific.</p>")
...
(defsection topstring
  ...
  :long
  "<p>A @(':long') documentation string often consists of
      a series of one or more XDOC trees
      for paragraphs, (un)ordered lists, code blocks, etc.
      This function can be used to wrap them into a top-level ``pseudo-tag''.</p>
   @(def topstring)"
...
(defsection topstring-p
  ...
  :long
  "<p>Sometimes a @(':long') documentation string consists of a single paragraph.
      This macro wraps the arguments (often just one string)
      into a paragraph tree and then calls @(tsee topstring).</p>
   @(def topstring-p)"
...
(defsection topstring-@def
  ...
  :long
  "<p>Sometimes a @(':long') documentation string
      consists of a single definition preprocessor directive.
      This macro wraps the arguments (often just one string)
      into a definition directive tree and then calls @(tsee topstring).</p>
   @(def topstring-@def)"
...
(defsection desc
  ...
  "<p>This consists of
      ...
      followed by a quoted block of a sequence of trees that describe the thing(s).</p>
   <p>The first argument is either a string for a single thing being described
      or a list of strings for a multiple things being described.
      In the latter case, the strings are separated by line breaks
      in the generated paragraph.</p>
   <p>An alternative to generating a paragrah followed by a quoted block,
      ...
      so it seems preferable to use a paragraph and a quoted block.</p>
   @(def desc)"
...
(defsection codeblock
  ...
  :long
  "<p>The arguments must evaluate to strings that are the lines of the code block,
      ...
      to ensure the proper formatting in the final XDOC string.</p>
   <p>Since the XDOC tree is constructed from a sequence of subtrees,
      the aforementioned newlines are not
      directly concatenated with the input strings.
      Instead, each input string is turned into two strings.</p>
   <p>This XDOC constructor provides a possibly more convenient way
      to enter the lines that form the code block,
      compared to the bare @(tsee @{}) XDOC constructor.</p>
   @(def codeblock)"
