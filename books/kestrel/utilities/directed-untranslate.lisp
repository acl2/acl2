...
; simply associate var with sterm.  Otherwise we have a heuristic decision to
; make.  Our intention is to perform lambda abstraction using alist, in
; essence, creating a (translated) LET expression whose bindings are generated
; by alist.  It might be best to bind a variable to the largest possible
; expression, intuitively to get the most abstraction.  But it might be best to
; bind a variable to the smallest possible expression, to get the most sharing.
; If there are two candidates and neither occurs in the other, we could even
; consider trying to bind to their largest common subexpression.  Consider the
; following example.
