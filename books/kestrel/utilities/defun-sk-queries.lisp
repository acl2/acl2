...
(defxdoc defun-sk-queries
  ...
  "<p>
   ...
   In this case, the function could have been introduced via @(tsee defun-sk),
...
(std::defaggregate defun-sk-info
  :short "@(tsee defun-sk)-specific constituents
...
(define defun-sk-check-signature-result (sig-result)
  ...
  :short "Check the result information of the @(see signature)
...
(define defun-sk-check-witness-def (witness-def
  ...
  :returns (mv (yes/no booleanp
               ...
               witness-body
               (strengthen booleanp))
  :short "Check the local definition of the witness function
...
(define defun-sk-check-function-def (function-def
  ...
  :verify-guards nil
  :short "Check the (non-local) definition of the @(tsee defun-sk) function
...
(define defun-sk-check ((fn (function-namep fn wrld))
  ...
  (b* ((args (formals fn wrld))
       ...
       ((unless (and (or (tuplep 7 event)
        ...
        nil)
       (constrained (let ((def (nth 4 event)))
                      (and (consp def)
                           (eq (car def) 'local)
                           (let ((thm (nth 5 event)))
                             (case-match thm
                               ...
                               (& nil))))))
       ((when constrained)
        (er hard 'defun-sk-check
            "The :constrained keyword of defun-sk is not yet supported by ~
             defun-sk-check."))
       ((unless (equal (nth (- (len event) 2) event)
                       `(in-theory (disable (,fn)))))
        nil)
       (signatures (nth 1 event))
       (witness-def (nth 3 event))
       (function-def (nth (- (len event) 3) event))
       (rewrite (nth (- (len event) 1) event))
