; Documentation for the R1CS formalization
;
...
(acl2::defxdoc r1cs
  ...
  :long
  (xdoc::topstring
   (xdoc::p "An R1CS is a conjunction of constraints, each of the form:")

   (xdoc::p "@({(a DOT x) * (b DOT x) = (c DOT x)})")

   (xdoc::p"where a, b, and c are vectors of coefficients (elements of the prime
...
(xdoc::p "For background on R1CSes, see Vitalik's Medium post below.")

(xdoc::p "We provide a formalization of R1CSes in sparse form (where only
...
(xdoc::p "We provide several verified R1CS \"gadgets\", that is, ways to represent
certain computations, such as XORing two bits, using only the
operators available in R1CSes.  See gadgets.lisp.")

(xdoc::p "We also provide a \"lifter\" which turns an R1CS into a (potentially
...
(xdoc::p "The Axe Prover can be used to establish that, whenever the R1CS holds,
...
(xdoc::p "The Axe Prover uses rewriting, with each simplification rule having
...
<tt>(equal intermediate-var defining-expression)</tt>.  See
<tt>../../axe/prover-basic.lisp</tt>.")

(xdoc::p "References:
<a href=\"https://medium.com/@VitalikButerin/quadratic-arithmetic-programs-from-zero-to-hero-f6d558cea649\">Quadratic Arithmetic Programs: from Zero to Hero, Vitalik Buterin</a>"))))
