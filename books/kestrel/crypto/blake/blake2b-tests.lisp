...
(defund run-parsed-blake2b-test (test)
            ...
            (let ((in (acl2::lookup-equal "in" alist))
              ...
              (b* (((when (not (and (stringp in)
                   ...
                   ((when (< *max-hash-bytes* nn))
                    ...
                    :error)
                   (test-result (blake2b in-bytes key-bytes nn)))
...
(defund run-parsed-blake2b-tests (tests)
  (declare (xargs :guard t))
  (if (atom tests)
      :pass
    (let ((result (run-parsed-blake2b-test (first tests))))
      (if (eq :error result)
          :error
        (if (eq :fail result)
            :fail
          (run-parsed-blake2b-tests (rest tests)))))))

;; Returns (mv result acl2::state) where result is :pass, :fail, or :error
(defun run-blake2b-tests (acl2::state)
  ...
  (b* (((mv erp parsed-tests acl2::state)
       ...
       ((when (not (and (acl2::call-of :array parsed-tests)
        ...
        (mv :error acl2::state))
       (result (run-parsed-blake2b-tests (acl2::farg1 parsed-tests))))
    (mv result acl2::state)))

(acl2::assert!-stobj (mv-let (res acl2::state)
                       (run-blake2b-tests acl2::state)
