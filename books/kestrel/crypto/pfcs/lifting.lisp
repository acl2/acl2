...
(defsection lifter-rules
  ...
  (defruled lifter-rule-omap-in-to-in-of-keys
    (iff (omap::in key map)
         (set::in key (omap::keys map)))
    :by omap::in-to-in-of-keys)

  (defruled lifter-rule-omap-consp-of-in-iff-in
    (iff (consp (omap::in key map))
         (omap::in key map))
    :by omap::consp-of-in-iff-in)

  (defruled lifter-rule-natp-of-mod
    (implies (and (natp a)
                  (posp b))
             (natp (mod a b))))

  (defruled lifter-rule-nfix-when-natp
    (implies (natp x)
             (equal (nfix x) x))))
...
(define sesem-definition-thm ((def definitionp) (prime symbolp))
    ...
    `(defruled ,thm-name
       ...
       ((defruled only-if-direction
          ...
          :in-theory '(definition-satp
                        ...
                        acl2::nat-listp-of-cons
                        acl2::not-reserrp-when-nat-listp
                        lifter-rule-nfix-when-natp
                        (:t mod)
                        (:t reserr)
                        fty::reserrp-of-reserr
                        lifter-rule-omap-consp-of-in-iff-in
                        (:e set::in)
  ...
  ((define sesem-definition-thm-aux1 ((free symbol-listp) (witness "A term."))
   ...
   (define sesem-definition-thm-aux2 ((vars symbol-listp) (witness "A term."))
     ...
     (cond ((endp vars) nil)
           (t (cons `(:instance lifter-rule-omap-in-to-in-of-keys
                                (key ',(car vars))
                                (map ,witness))
