...
(define pprint-ext-declon ((ext ext-declonp) (options pprint-options-p))
  ...
  (ext-declon-case ext
                   ...
                   :fun-declon (pprint-fun-declon ext.get 0)
...
(define pprint-trans-unit ((tunit trans-unitp)
                           (include maybe-stringp)
                           (options pprint-options-p))
  :returns (lines msg-listp)
  ...
  (xdoc::topstring
   (xdoc::p
    "We start with a line comment saying that
     the translation unit is generated by ATC.")
   (xdoc::p
    "The @('include') parameter of this pretty-printing ACL2 function
     says whether we should generate (when non-@('nil'),
     a @('#include') directive with double quotes
     and the file name specified by the string.
     Note that our abstract syntax
     does not yet have a representation of @('#include') directives;
     instead, since for now we only generate very simple such directives,
     we generate them in this ad hoc fashion for now."))
  (b* (((trans-unit tunit) tunit))
    (append (list "// This file is generated by ATC.~%")
            (and include
                 (list (str::cat "~%#include \"" include "\"~%")))
...
(define pprint-trans-unit-to-filesystem ((tunit trans-unitp)
                                         (include maybe-stringp)
                                         (options pprint-options-p)
                                         (filepath stringp)
                                         state)
  :returns (mv erp val state)
  :mode :program
  :short "Pretty-print a translation unit to the file system."
  :long
  (xdoc::topstring
   (xdoc::p
    "We pretty-print the lines and then we write them to the file path."))
  (pprinted-lines-to-file (pprint-trans-unit tunit include options)
                          filepath
                          state))
...
(define pprint-trans-ensemble ((tunits trans-ensemblep)
  ...
  (b* (((trans-ensemble tunits) tunits)
    ...
    (if tunits.dot-h
        (b* (((er &) (pprint-trans-unit-to-filesystem tunits.dot-h
                                                      nil
                                                      options
                                                      path-dot-h
                                                      state)))
          (pprint-trans-unit-to-filesystem tunits.dot-c
                                           (str::cat file-name ".h")
                                           options
                                           path-dot-c
                                           state))
