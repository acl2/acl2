...
(define run-fun ((fun identp)
  ...
  (xdoc::topstring
   (xdoc::p
    "We initialize the function environment from the translation unit.
     We create an initial computation state, with the given heap.
     We call the function.
     We discard the resulting computation state,
     because currently the heap is read-only
  ...
  (b* ((fenv (init-fun-env tunit))
       ...
       (compst (make-compustate :frames nil :heap heap))
       ((mv result  &) (exec-fun fun args compst fenv 1000000000))) ; 10^9
    result)
  :hooks (:fix))
