...
(defval *atc-operations-1-conv-definition-rules*
  :short "List of definition rules for operations
          that involve one C integer type
          and that involve some conversion."
  :long
  (xdoc::topstring
   (xdoc::p
  ...
  (b* ((types (list (type-schar)
                    ...
                    (type-sshort)
                    (type-ushort))))
    (atc-integer-ops-1-names-loop-ops '(plus minus bitnot shl shr) types))

  :prepwork

  ((define atc-integer-ops-1-names-loop-types ((op symbolp) (types type-listp))
     ...
     (cond ((endp types) nil)
           (t (b* ((type (car types))
                   ...
                   (names (if (and (eq op 'minus)
                            ...
                            (list (pack op '- fixtype))))
                   (more-names
                    (atc-integer-ops-1-names-loop-types op (cdr types))))
                (append names more-names)))))

   (define atc-integer-ops-1-names-loop-ops ((ops symbol-listp)
                                             (types type-listp))
     :guard (and (subsetp-eq ops '(plus minus bitnot shl shr))
                 (type-integer-listp types))
     :returns (names symbol-listp)
     (cond ((endp ops) nil)
           (t (append (atc-integer-ops-1-names-loop-types (car ops) types)
                      (atc-integer-ops-1-names-loop-ops (cdr ops) types)))))))
...
(defval *atc-definition-rules*
   ...
   *atc-operations-1-conv-definition-rules*))
