...
(defruled exec-stmt-when-compound
  :short "Opener rule for compound statements."
...
(defruled exec-stmt-when-if-and-true
  :short "Opener non-splitting rule for @('if') statements with true tests."
  (implies (and (syntaxp (quotep s))
                (equal (stmt-kind s) :if)
                (expr-purep (stmt-if->test s))
                (integerp limit)
...
(defruled exec-stmt-when-if-and-false
  :short "Opener nno-splitting rule for @('if') statements with false tests."
  (implies (and (syntaxp (quotep s))
                (equal (stmt-kind s) :if)
                (expr-purep (stmt-if->test s))
                (integerp limit)
...
(defruled exec-stmt-when-ifelse-and-true
  :short "Opener splitting rule for
          @('if')-@('else') statements with true tests."
  (implies (and (syntaxp (quotep s))
                (equal (stmt-kind s) :ifelse)
                (expr-purep (stmt-ifelse->test s))
                (integerp limit)
...
(defruled exec-stmt-when-ifelse-and-false
  :short "Opener splitting rule for
          @('if')-@('else') statements with false tests."
  (implies (and (syntaxp (quotep s))
                (equal (stmt-kind s) :ifelse)
                (expr-purep (stmt-ifelse->test s))
                (integerp limit)
