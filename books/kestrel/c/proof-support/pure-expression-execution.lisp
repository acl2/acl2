...
(defxdoc+ pure-expression-execution
  ...
  :short "Execution of pure expressions."
  :long
  (xdoc::topstring
   (xdoc::p
   ...
   (xdoc::p
    "The functions @(tsee exec-expr-pure) and @(tsee exec-expr-pure-list)
     ...
     but there are cases in which it is useful
     to use these simpler execution functions for pure expressions;
     for example, the proofs currently generated by "
    (xdoc::seetopic "atc" "ATC")
    " are based on these functions for pure expression execution."))
  :order-subtopics t
  :default-parent t)
