...
"
 :cond (b* ((transunit (omap::head-val (transunit-ensemble->units ast)))
            ...
            (item (car items))
            (declon (block-item-decl->decl item))
            (ideclors (declon-declon->init declon))
            (ideclor (car ideclors))
            (initer (init-declor->initer? ideclor))
...
"
  :cond (b* ((filepath-transunit-map (transunit-ensemble->units ast))
             ...
             (foo-init1 (first (declon-declon->init (extdecl-decl->decl (first edecls1)))))
             ...
             (foo-init2 (first (declon-declon->init (block-item-decl->decl bar-body-decl1))))
             ...
             (foo-init3 (first (declon-declon->init (block-item-decl->decl bar-body-decl2))))
             ...
             (bar-init (first (declon-declon->init (extdecl-decl->decl (first edecls2)))))
