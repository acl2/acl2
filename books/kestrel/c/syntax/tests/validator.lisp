...
"
 :cond (b* ((transunit (omap::head-val (transunit-ensemble->units ast)))
            ...
            (item (car items))
            (declon (block-item-declon->declon item))
...
"
  :cond (b* ((filepath-transunit-map (transunit-ensemble->units ast))
             ...
             (foo-init1 (first (declon-declon->declors (ext-declon-decl->decl (first edecls1)))))
             ...
             (bar-init (first (declon-declon->declors (ext-declon-decl->decl (first edecls2)))))
