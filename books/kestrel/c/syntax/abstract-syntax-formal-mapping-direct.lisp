  ...
  (xdoc::topstring
   ...
   (xdoc::p
    "This function will always result in a @(tsee c::fun-adeclor)
     of the @(':base') kind;
     the @(':pointer') kind is generated by @(tsee ldm-absdeclor-fun).")
   (xdoc::p
    "This function is called when we expect an abstract function declarator,
     not an object declarator, for which we have a separate function."))
...
(define ldm-absdeclor-fun ((absdeclor absdeclorp))
  :guard (absdeclor-unambp absdeclor)
  :returns (mv erp (funadeclor c::fun-adeclorp))
  :short "Map an abstract declarator to
          an abstract function declarator in the language definition."
  ...
  (xdoc::topstring
   (xdoc::p
    "We map the direct abstract declarator to an abstract function declarator,
     and then we recursively add pointer layers
     based on the pointer part of the abstract declarator.")
   (xdoc::p
    "The direct abstract declarator must be present.")
   (xdoc::p
    "This function is called when we expect an abstract function declarator,
     not an object declarator, for which we have a separate function."))
  (b* (((reterr) (c::fun-adeclor-base nil))
       ((absdeclor absdeclor) absdeclor)
       ((when (dirabsdeclor-option-case absdeclor.direct? :none))
        (reterr (msg "Unsupported absent direct abstract declarator ~
                      in abstract declarator ~x0 for function."
                     (absdeclor-fix absdeclor))))
       (dirabsdeclor
        (dirabsdeclor-option-some->val absdeclor.direct?))
       ((erp adeclor1) (ldm-dirabsdeclor-fun dirabsdeclor)))
    (ldm-absdeclor-fun-loop adeclor1 absdeclor.pointers))

  :prepwork
  ((define ldm-absdeclor-fun-loop ((adeclor1 c::fun-adeclorp)
                                   (pointers typequal/attribspec-list-listp))
     :returns (mv erp (adeclor2 c::fun-adeclorp))
     :parents nil
     (b* (((reterr) (c::fun-adeclor-base nil))
          ((when (endp pointers)) (retok (c::fun-adeclor-fix adeclor1)))
          (qualspecs (car pointers))
          ((unless (endp qualspecs))
           (reterr (msg "Unsupported type qualifiers ~
                         or attribute specifiers ~
                         ~x0 in pointer."
                        (typequal/attribspec-list-fix qualspecs))))
          ((erp adeclor2) (ldm-absdeclor-fun-loop adeclor1 (cdr pointers))))
       (retok (c::fun-adeclor-pointer adeclor2)))
     :verify-guards :after-returns

     ///

     (defret ldm-absdeclor-fun-loop-ok-when-pointers-formalp
       (not erp)
       :hyp (pointers-formalp pointers)
       :hints (("Goal" :induct t :in-theory (enable pointers-formalp)))))))
