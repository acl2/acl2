  ...
  (b* (((reterr) (irr-type))
    ...
    (case (unop-kind op)
      ...
      (:indir (b* (((when (or (type-case type-arg :unknown)          ; could be
                              (type-case type-arg :unknown-scalar))) ; pointer
                    (retok (type-unknown)))
...
(define valid-binary ((expr exprp)
  ...
  (b* (((reterr) (irr-type))
    ...
    (case (binop-kind op)
      ((:mul :div :rem)
       (b* (((when (and (or (type-case type-arg1 :unknown)
                            (type-case type-arg1 :unknown-scalar))
                        (or (type-case type-arg2 :unknown)
                            (type-case type-arg2 :unknown-scalar))))
             (retok (type-unknown-arithmetic)))
      ...
      (:add (b* ((type1 (type-apconvert type-arg1))
                 (type2 (type-apconvert type-arg2)))
              (cond
               ((or (type-case type1 :unknown)         ; could be pointer
                    (type-case type1 :unknown-scalar)) ; or arithmetic
                (cond ((or (type-case type2 :unknown)
                           (type-case type2 :unknown-scalar)
                           (type-case type2 :unknown-arithmetic)
                           (type-integerp type2))
                       (retok (type-unknown-scalar)))
                      ((type-arithmeticp type2) ; not integer, checked above
                       ;; type1 cannot be pointer, in valid code:
                       (retok (type-unknown-arithmetic)))
                      ((type-case type2 :pointer)
                       ;; type1 must be integer, in valid code:
                       (retok type2))
                      (t (reterr msg))))
               ((type-integerp type1)
                (cond ((or (type-case type2 :unknown)         ; could be pointer
                           (type-case type2 :unknown-scalar)) ; or arithmetic
                       (retok (type-unknown-scalar)))
                      ((type-case type2 :pointer)
                       (retok type2))
                      ...
                      (t (reterr msg))))
               ((type-arithmeticp type1) ; not integer, checked above
                (cond ((type-arithmeticp type2)
                       (retok (type-uaconvert type1 type2 ienv)))
                      ((or (type-case type2 :unknown)
                           (type-case type2 :unknown-scalar))
                       ;; type2 cannot ba pointer, in valid code:
                       (retok (type-unknown-arithmetic)))
                      (t (reterr msg))))
      ...
      (:sub (b* ((type1 (type-apconvert type-arg1))
              ...
              (cond
               ((or (type-case type1 :unknown)         ; could be pointer
                ...
                (cond ((or (type-case type2 :unknown)
                      ...
                      ((type-arithmeticp type2) ; not integer, checked above
                       ...
                       (retok (type-unknown-arithmetic)))
                      ((type-case type2 :pointer)
                       ;; type1 could be pointer too:
                       (retok (type-unknown-scalar)))
                      (t (reterr msg))))
               ((type-integerp type1)
                (cond ((or (type-case type2 :unknown)
                           (type-case type2 :unknown-scalar))
                       ;; type2 cannot be pointer, in valid code:
                       (retok (type-unknown-arithmetic)))
                      ((type-arithmeticp type2)
                       (retok (type-uaconvert type1 type2 ienv)))
                      (t (reterr msg))))
               ...
               ((type-case type1 :pointer)
                (cond ((or (type-case type2 :unknown)
                           (type-case type2 :unknown-scalar))
                       ;; type2 could be pointer too:
                       (retok (type-unknown-scalar)))
                      ((type-case type2 :unknown-arithmetic)
                       ;; type2 must be integer, in valid code:
                       (retok type1))
                      ((type-integerp type2)
                       (retok type1))
                      ((type-case type2 :pointer)
                      ...
                      (t (reterr msg))))
               (t (reterr msg)))))
      ((:shl :shr) (b* (((when (or (type-case type-arg1 :unknown)
                                   (type-case type-arg1 :unknown-scalar)
                                   (type-case type-arg1 :unknown-arithmetic)))
                         (if (or (type-case type-arg1 :unknown)
                                 (type-case type-arg1 :unknown-scalar)
                                 (type-case type-arg1 :unknown-arithmetic)
                                 (type-integerp type-arg2))
                             (retok (type-unknown-arithmetic))
                           (reterr msg)))
                        ((when (and (type-integerp type-arg1)
                                    (or (type-case type-arg2 :unknown)
                                        (type-case type-arg2 :unknown-scalar)
                                        (type-case type-arg2 :unknown-arithmetic)
                                        (type-integerp type-arg2))))
      ...
      ((:lt :gt :le :ge)
       (b* ((type1 (type-apconvert type-arg1))
            (type2 (type-apconvert type-arg2))
            ((when (or (type-case type1 :unknown)
                       (type-case type1 :unknown-scalar)
                       (type-case type1 :unknown-arithmetic)
                       (type-case type2 :unknown)
                       (type-case type2 :unknown-scalar)
      ...
      ((:bitand :bitxor :bitior)
       (b* (((when (or (type-case type-arg1 :unknown)
                       (type-case type-arg1 :unknown-scalar)
                       (type-case type-arg1 :unknown-arithmetic)))
             (if (or (type-case type-arg2 :unknown)
                     (type-case type-arg2 :unknown-scalar)
                     (type-case type-arg2 :unknown-arithmetic)
                     (type-integerp type-arg2))
                 (retok (type-unknown-arithmetic))
               (reterr msg)))
            ((when (or (type-case type-arg2 :unknown)
                       (type-case type-arg2 :unknown-scalar)
                       (type-case type-arg2 :unknown-arithmetic)
                       (type-integerp type-arg2)))
             (if (type-integerp type-arg1)
                 (retok (type-unknown-arithmetic))
               (reterr msg)))
  ...
  (define valid-init-declor ((initdeclor init-declorp)
    ...
    (b* (((reterr) (irr-init-declor) nil (irr-vstate))
         ...
         ((unless (or (equal type info.type)
                      ...
                      (equal type (type-unknown-arithmetic))
                      (equal info.type (type-unknown))
                      (equal info.type (type-unknown-scalar))
                      (equal info.type (type-unknown-arithmetic))))
          (retmsg$ "The identifier ~x0 ~
