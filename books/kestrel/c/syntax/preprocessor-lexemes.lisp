  ...
  (xdoc::topstring
   ...
   (xdoc::p
    "A preprocessing lexeme for an identifier also includes
     ...
     This provenance list is empty for identifiers that occur in the file,
     but it is non-empty for identifiers generated by macro expansion.
     The order of the list is unimportant, and the list may have duplicates;
     it is treated as a set.
     We use a list with possible repetitions, instead of an oset,
     because the only operations we perform on it are
     adding elements (via @(tsee cons) or @(tsee append))
     and checking membership (via @(tsee member-equal)).")
   (xdoc::p
