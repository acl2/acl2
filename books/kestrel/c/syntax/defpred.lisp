     ...
     (b* (((reterr) nil)
          ...
          ((erp key val)
           (if sump
             ...
             (b* (((reterr) nil nil)
                  ((unless (eq typemacro 'fty::defprod))
                   (reterr (msg "The type ~x0 in the :OVERRIDE list ~
                                 is not accompanied by a kind, ~
                                 but that type is not a product type."
                                type))))
               (retok type term))))
          ((erp alist)
...
(define defpred-gen-sum-cases ((type symbolp)
  ...
  (b* (((when (endp prods)) nil)
       ...
       (term
        (b* ((term (cdr (assoc-equal (cons type kind) overrides)))
             ((when term) term)
             (fields (fty::flexprod->fields prod))
...
(define defpred-gen-prod-pred ((sum fty::flexsum-p)
  ...
  (b* ((type (fty::flexsum->name sum))
       ...
       ((mv body ignorable)
        (b* ((term (cdr (assoc-equal type overrides)))
             ((when term) (mv term t))
