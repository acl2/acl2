(*-OF-/-SAME (12 4 (:LINEAR <-OF-*-SAME-LINEAR-SPECIAL))
             (8 8 (:LINEAR <=-OF-*-AND-*-SAME-LINEAR))
             (8 8
                (:LINEAR <=-OF-*-AND-*-SAME-ALT-LINEAR))
             (8 8 (:LINEAR <-OF-*-AND-*))
             (6 6
                (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
             (5 2 (:REWRITE DEFAULT-UNARY-/))
             (4 4 (:REWRITE DEFAULT-<-2))
             (4 4 (:REWRITE DEFAULT-<-1))
             (4 4 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-4))
             (4 4 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-3))
             (4 4 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-2))
             (4 4 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-1))
             (3 2 (:REWRITE DEFAULT-*-1))
             (2 2 (:REWRITE DEFAULT-*-2))
             (1 1 (:REWRITE EQUAL-OF-*-AND-CONSTANT)))
(*-OF-*-OF-/-SAME (56 9 (:LINEAR <-OF-*-SAME-LINEAR-SPECIAL))
                  (18 18 (:LINEAR <=-OF-*-AND-*-SAME-LINEAR))
                  (18 18
                      (:LINEAR <=-OF-*-AND-*-SAME-ALT-LINEAR))
                  (18 18 (:LINEAR <-OF-*-AND-*))
                  (15 13 (:REWRITE DEFAULT-*-2))
                  (12 12
                      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                  (10 7 (:REWRITE DEFAULT-<-2))
                  (9 9 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-4))
                  (9 9 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-3))
                  (9 9 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-2))
                  (9 9 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-1))
                  (7 7 (:REWRITE DEFAULT-<-1))
                  (3 3 (:REWRITE EQUAL-OF-*-AND-CONSTANT))
                  (3 3 (:REWRITE *-OF-*-COMBINE-CONSTANTS)))
(EQUAL-OF-*-OF-/ (69 11 (:LINEAR <-OF-*-SAME-LINEAR-SPECIAL))
                 (22 22 (:LINEAR <=-OF-*-AND-*-SAME-LINEAR))
                 (22 22
                     (:LINEAR <=-OF-*-AND-*-SAME-ALT-LINEAR))
                 (22 22 (:LINEAR <-OF-*-AND-*))
                 (11 11
                     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                 (11 11 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-4))
                 (11 11 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-3))
                 (11 11 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-2))
                 (11 11 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-1))
                 (9 9 (:REWRITE DEFAULT-<-2))
                 (9 9 (:REWRITE DEFAULT-<-1))
                 (7 4 (:REWRITE DEFAULT-UNARY-/))
                 (6 5 (:REWRITE DEFAULT-*-1))
                 (2 2 (:REWRITE EQUAL-OF-*-AND-CONSTANT))
                 (1 1 (:REWRITE *-OF-*-COMBINE-CONSTANTS)))
(EQUAL-OF-/ (7 7
               (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
            (7 5 (:REWRITE DEFAULT-*-1))
            (6 5 (:REWRITE DEFAULT-*-2))
            (6 3 (:REWRITE DEFAULT-UNARY-/))
            (1 1 (:REWRITE EQUAL-OF-*-AND-CONSTANT)))
(/-OF-* (37 3 (:REWRITE DEFAULT-UNARY-/))
        (9 9
           (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
        (8 6 (:REWRITE DEFAULT-*-1))
        (6 6 (:REWRITE DEFAULT-*-2))
        (6 2 (:LINEAR <-OF-*-SAME-LINEAR-SPECIAL))
        (4 4 (:LINEAR <=-OF-*-AND-*-SAME-LINEAR))
        (4 4
           (:LINEAR <=-OF-*-AND-*-SAME-ALT-LINEAR))
        (4 4 (:LINEAR <-OF-*-AND-*))
        (2 2 (:REWRITE EQUAL-OF-*-AND-CONSTANT))
        (2 2 (:REWRITE DEFAULT-<-2))
        (2 2 (:REWRITE DEFAULT-<-1))
        (2 2 (:REWRITE *-OF-*-COMBINE-CONSTANTS))
        (2 2 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-4))
        (2 2 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-3))
        (2 2 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-2))
        (2 2 (:LINEAR <-OF-*-AND-*-SAME-LINEAR-1)))
(<-OF-*-OF-/-ARG2-ARG1 (6 2 (:LINEAR <-OF-*-SAME-LINEAR-SPECIAL))
                       (4 4 (:REWRITE DEFAULT-*-2))
                       (4 4 (:REWRITE DEFAULT-*-1))
                       (4 4 (:LINEAR <-OF-*-AND-*))
                       (2 2 (:REWRITE DEFAULT-<-2))
                       (2 2 (:REWRITE DEFAULT-<-1))
                       (2 2 (:REWRITE *-OF-*-COMBINE-CONSTANTS))
                       (2 2 (:LINEAR <=-OF-*-AND-*-SAME-LINEAR)))
(<-OF-*-OF-/-ARG2-ARG2 (6 6 (:REWRITE DEFAULT-<-2))
                       (6 6 (:REWRITE DEFAULT-<-1))
                       (5 5 (:REWRITE DEFAULT-*-2))
                       (5 5 (:REWRITE DEFAULT-*-1))
                       (2 2 (:REWRITE DEFAULT-UNARY-/)))
(<-OF-*-OF-/-ARG1-ARG1 (6 2 (:LINEAR <-OF-*-SAME-LINEAR-SPECIAL))
                       (4 4 (:LINEAR <-OF-*-AND-*))
                       (2 2 (:REWRITE DEFAULT-<-2))
                       (2 2 (:REWRITE DEFAULT-<-1))
                       (2 2 (:REWRITE DEFAULT-*-2))
                       (2 2 (:REWRITE DEFAULT-*-1))
                       (2 2 (:LINEAR <=-OF-*-AND-*-SAME-LINEAR)))
(<-OF-*-OF-/-ARG1-ARG2 (6 6 (:REWRITE DEFAULT-<-2))
                       (6 6 (:REWRITE DEFAULT-<-1))
                       (5 5 (:REWRITE DEFAULT-*-2))
                       (5 5 (:REWRITE DEFAULT-*-1))
                       (2 2 (:REWRITE DEFAULT-UNARY-/)))
