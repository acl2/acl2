...
(defun make-memory-region-machinery-fn (num-address-bits pkg)
  ...
  (let* ((in-regionp-name (acl2::pack-in-package pkg 'in-region num-address-bits 'p))
         ...
         (disjoint-regionsp-name (acl2::pack-in-package pkg 'disjoint-regions num-address-bits 'p))
         (disjoint-regionsp-spec-name (acl2::pack-in-package pkg disjoint-regionsp-name '-spec)))
    `(progn
       ...
       (encapsulate ()
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-same)
         ...
         (defthm ,(acl2::pack-in-package pkg 'not- in-regionp-name '-too-far)
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-of-bvplus-tighten-arg1)
           (implies (and (< ,num-address-bits n)
                         (integerp n))
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-of-logext-arg1)
           (implies (and (<= ,num-address-bits n)
                         (integerp n))
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-of-bvsx-arg1)
           (implies (and (<= ,num-address-bits n)
                         (<= n high)
                         (integerp n)
                         (integerp high))
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-of-bvplus-tighten-arg3)
           (implies (and (< ,num-address-bits n)
                         (integerp n))
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-of-logext-arg3)
           (implies (and (<= ,num-address-bits n)
                         (integerp n))
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-of-bvsx-arg3)
           (implies (and (<= ,num-address-bits n)
                         (<= n high)
                         (integerp n)
                         (integerp high))
         ...
         (theory-invariant (incompatible (:rewrite ,(acl2::pack-in-package pkg in-regionp-name '-of-+-arg1)) (:definition bvplus)))

         (defthmd ,(acl2::pack-in-package pkg in-regionp-name '-of-+-arg3)
           (implies (and (integerp x)
                         (integerp y))
         ...
         (theory-invariant (incompatible (:rewrite ,(acl2::pack-in-package pkg in-regionp-name '-of-+-arg3)) (:definition bvplus)))
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-cancel-1-2)
           (equal (,in-regionp-name x len2 (bvplus ,num-address-bits y x)) ; todo: why len2, here and below?
                  (,in-regionp-name 0 len2 y))
           :hints (("Goal" :in-theory (enable ,in-regionp-name))))

         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-cancel-2-1)
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-cancel-2-2)
           (equal (,in-regionp-name (bvplus ,num-address-bits y x) len2 (bvplus ,num-address-bits z x))
                  (,in-regionp-name y len2 z))
           :hints (("Goal" :in-theory (enable ,in-regionp-name))))

         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-cancel-3-1)
         ...
         ;; todo: hints
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-cancel-1-3)
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-when-non-negative-and-negative-range)
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-subst-constant-arg1)
         ...
         (defthmd ,(acl2::pack-in-package pkg in-regionp-name '-opener)
...
;       (unsigned-byte-p ,num-address-bits ad)
         ...
         (defthmd ,(acl2::pack-in-package pkg in-regionp-name '-alt-def)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
         ...
         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-name '-when-not-posp)
           (implies (not (posp len1))
                    (,disjoint-regionsp-name len1 ad1 len2 ad2))
           :hints (("Goal" :in-theory (enable ,disjoint-regionsp-name))))

         ;; (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-name '-when-zp)
         ;;   (implies (and (zp len1)
         ...
         (defthmd ,(acl2::pack-in-package pkg disjoint-regionsp-name '-symmetric)
         ...
         (defthm ,(acl2::pack-in-package pkg 'not-equal-of-bvchop-and-bvchop-when- disjoint-regionsp-name)
         ...
         (defthmd ,(acl2::pack-in-package pkg disjoint-regionsp-name '-of-+-arg2)
         ...
         (theory-invariant (incompatible (:rewrite ,(acl2::pack-in-package pkg disjoint-regionsp-name '-of-+-arg2)) (:definition bvplus)))

         (defthmd ,(acl2::pack-in-package pkg disjoint-regionsp-name '-of-+-arg4)
         ...
         (theory-invariant (incompatible (:rewrite ,(acl2::pack-in-package pkg disjoint-regionsp-name '-of-+-arg4)) (:definition bvplus)))

         ;; todo: more, or make a general cancellation rule (add the negation of the term to be cancelled):

         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-name '-cancel-1-1+)
         ...
         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-name '-cancel-1-2)
           (equal (,disjoint-regionsp-name len1 x len2 (bvplus ,num-address-bits y x))
                  (,disjoint-regionsp-name len1 0 len2 y))
           :hints (("Goal" :in-theory (enable ,disjoint-regionsp-name))))

         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-name '-cancel-2-1)
         ...
         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-name '-of-bvchop-arg2)
         ...
         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-name '-of-bvchop-arg4)
           (implies (and (<= ,num-address-bits size)
                         (integerp size))
         ...
         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-necc-better)
           (implies (and (,in-regionp-name ad len1 ad1)
                         (,in-regionp-name ad len2 ad2))
                    (not (,disjoint-regionsp-spec-name len1 ad1 len2 ad2)))
           :hints (("Goal" :use ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-necc)
                    :in-theory (theory 'minimal-theory))))

         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-necc-better-alt)
         ...
         (encapsulate ()
           (local (defthmd ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-of-bvchop-arg2-fw)
           ...
           (local (defthmd ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-of-bvchop-arg2-bk)
           ...
           (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-of-bvchop-arg2)
             (equal (,disjoint-regionsp-spec-name len1 (bvchop ,num-address-bits ad1) len2 ad2)
                    (,disjoint-regionsp-spec-name len1 ad1 len2 ad2))
             :hints (("Goal" :use (,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-of-bvchop-arg2-fw)
                                   ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-of-bvchop-arg2-bk))))))

         (encapsulate ()
           (local (defthmd ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-of-bvchop-arg4-fw)
                    (implies (,disjoint-regionsp-spec-name len1 ad1 len2 (bvchop ,num-address-bits ad2))
                             (,disjoint-regionsp-spec-name len1 ad1 len2 ad2))
                    :hints (("Goal" :expand (,disjoint-regionsp-spec-name len1 ad1 len2 ad2)))))

           (local (defthmd ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-of-bvchop-arg4-bk)
                    (implies (,disjoint-regionsp-spec-name len1 ad1 len2 ad2)
           ...
           (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-of-bvchop-arg4)
             (equal (,disjoint-regionsp-spec-name len1 ad1 len2 (bvchop ,num-address-bits ad2))
                    (,disjoint-regionsp-spec-name len1 ad1 len2 ad2))
             :hints (("Goal" :use (,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-of-bvchop-arg4-fw)
                                   ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-of-bvchop-arg4-bk))))))

         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-of-one-smaller)
           ...
           :hints (("Goal" :expand ((,disjoint-regionsp-spec-name (+ -1 len1) (bvplus ,num-address-bits 1 ad1) len2 ad2))
                    :use (:instance ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-necc-better)
                                    (ad (,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-witness) (+ -1 len1) (bvplus ,num-address-bits 1 ad1) len2 ad2)))
                    :in-theory (e/d (,(acl2::pack-in-package pkg in-regionp-name '-opener)
                                     ,(acl2::pack-in-package pkg in-regionp-name '-of-+-arg3))
                                    (,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-necc-better)
                                     ,(acl2::pack-in-package pkg disjoint-regionsp-spec-name '-necc-better-alt))))))

         (local (defun indf (n ad) (if (zp n) (list n ad) (indf (+ -1 n) (bvplus ,num-address-bits 1 ad)))))

         (defthmd ,(acl2::pack-in-package pkg disjoint-regionsp-name '-opener)
         ...
         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-name '-correct-backward)
           (implies (and (,disjoint-regionsp-spec-name len1 ad1 len2 ad2)
                         (unsigned-byte-p ,num-address-bits ad1)
                         (unsigned-byte-p ,num-address-bits ad2))
                    (,disjoint-regionsp-name len1 ad1 len2 ad2))
           :hints (("Goal" :induct (indf len1 ad1)
                    :in-theory (enable ,(acl2::pack-in-package pkg disjoint-regionsp-name '-opener)
                                       ,(acl2::pack-in-package pkg disjoint-regionsp-name '-of-+-arg2)
                                       zp natp))))

         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-name '-correct-forward)
         ...
         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-name '-correct)
           (equal (,disjoint-regionsp-spec-name len1 ad1 len2 ad2)
                  (,disjoint-regionsp-name len1 ad1 len2 ad2))
           :hints (("Goal" :use ((:instance ,(acl2::pack-in-package pkg disjoint-regionsp-name '-correct-forward) (ad1 (bvchop ,num-address-bits ad1)) (ad2 (bvchop ,num-address-bits ad2)))
                                 (:instance ,(acl2::pack-in-package pkg disjoint-regionsp-name '-correct-backward) (ad1 (bvchop ,num-address-bits ad1)) (ad2 (bvchop ,num-address-bits ad2))))
                    :in-theory (disable ,(acl2::pack-in-package pkg disjoint-regionsp-name '-correct-forward)
                                        ,(acl2::pack-in-package pkg disjoint-regionsp-name '-correct-backward)))))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
         ...
         (defthm ,(acl2::pack-in-package pkg subregionp-name '-reflexive)
         ...
         (defthm ,(acl2::pack-in-package pkg subregionp-name '-when-zp-arg1)
           (implies (zp len1)
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-when- in-regionp-name '-and- subregionp-name)
         ...
         (defthm ,(acl2::pack-in-package pkg 'not- subregionp-name '-when- in-regionp-name '-and-not- in-regionp-name)
           (implies (and (,in-regionp-name ad len1 ad1)
         ...
         (defthm ,(acl2::pack-in-package pkg subregionp-name '-cancel-1-1)
         ...
         (defthm ,(acl2::pack-in-package pkg subregionp-name '-cancel-2-1)
         ...
         (defthm ,(acl2::pack-in-package pkg subregionp-name '-cancel-1-2)
         ...
         (defthm ,(acl2::pack-in-package pkg subregionp-name '-cancel-2-2)
         ...
         (defthmd ,(acl2::pack-in-package pkg subregionp-name '-of-+-arg2)
           (implies (and (integerp x)
                         (integerp y))
                    (equal (,subregionp-name len1 (+ x y) len2 ad2)
                           (,subregionp-name len1 (bvplus ,num-address-bits x y) len2 ad2)))
           :hints (("Goal" :in-theory (enable ,subregionp-name
                                              ,(acl2::pack-in-package pkg in-regionp-name '-of-+-arg1)
                                              acl2::bvminus-of-+-arg2))))
         (theory-invariant (incompatible (:rewrite ,(acl2::pack-in-package pkg subregionp-name '-of-+-arg2)) (:definition bvplus)))

         (defthmd ,(acl2::pack-in-package pkg subregionp-name '-of-+-arg4)
           (implies (and (integerp x)
                         (integerp y))
           ...
           :hints (("Goal" :in-theory (enable ,subregionp-name
                                              acl2::bvminus-of-+-arg3
                                              ,(acl2::pack-in-package pkg in-regionp-name '-of-+-arg3)))))
         (theory-invariant (incompatible (:rewrite ,(acl2::pack-in-package pkg subregionp-name '-of-+-arg4)) (:definition bvplus)))

         (defthm ,(acl2::pack-in-package pkg subregionp-name '-of-bvchop-arg2)
           (implies (and (<= ,num-address-bits size)
                         (integerp size))
         ...
         (defthm ,(acl2::pack-in-package pkg subregionp-name '-of-bvchop-arg4)
           (implies (and (<= ,num-address-bits size)
                         (integerp size))
         ...
         ;; ;; removes the negative part of the range
         (defthm ,(acl2::pack-in-package pkg subregionp-name '-when-non-negative-and-negative-range)
         ...
         (defthm ,(acl2::pack-in-package pkg subregionp-name '-reduce-sizes)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
         ...
         (defthm ,(acl2::pack-in-package pkg in-regionp-name '-when- in-regionp-name '-and- subregionp-spec-name)
           (implies (and (,subregionp-spec-name len1 ad1 len2 ad2)
                         (,in-regionp-name ad len1 ad1))
                    (,in-regionp-name ad len2 ad2))
           :hints (("Goal" :use ,(acl2::pack-in-package pkg subregionp-spec-name '-necc))))

         (defthm ,(acl2::pack-in-package pkg 'not- subregionp-spec-name '-when- in-regionp-name '-and-not- in-regionp-name)
           (implies (and (,in-regionp-name ad len1 ad1) ; ad is a free var
                         (not (,in-regionp-name ad len2 ad2)))
                    (not (,subregionp-spec-name len1 ad1 len2 ad2)))
           :hints (("Goal" :use ,(acl2::pack-in-package pkg in-regionp-name '-when- in-regionp-name '-and- subregionp-spec-name)
                    :in-theory (disable ,(acl2::pack-in-package pkg in-regionp-name '-when- in-regionp-name '-and- subregionp-spec-name)))))

         (encapsulate ()
           (local
             (defthmd ,(acl2::pack-in-package pkg subregionp-spec-name '-of-bvchop-arg2-fw)
           ...
           (local
             (defthmd ,(acl2::pack-in-package pkg subregionp-spec-name '-of-bvchop-arg2-bk)
           ...
           (defthm ,(acl2::pack-in-package pkg subregionp-spec-name '-of-bvchop-arg2)
             (equal (,subregionp-spec-name len1 (bvchop ,num-address-bits ad1) len2 ad2)
                    (,subregionp-spec-name len1 ad1 len2 ad2))
             :hints (("Goal" :use (,(acl2::pack-in-package pkg subregionp-spec-name '-of-bvchop-arg2-fw)
                                   ,(acl2::pack-in-package pkg subregionp-spec-name '-of-bvchop-arg2-bk))))))

         (encapsulate ()
           (local
             (defthmd ,(acl2::pack-in-package pkg subregionp-spec-name '-of-bvchop-arg4-fw)
               (implies (,subregionp-spec-name len1 ad1 len2 (bvchop ,num-address-bits ad2))
                        (,subregionp-spec-name len1 ad1 len2 ad2))
               :hints (("Goal" :expand (,subregionp-spec-name len1 ad1 len2 ad2)))))

           (local
             (defthmd ,(acl2::pack-in-package pkg subregionp-spec-name '-of-bvchop-arg4-bk)
               (implies (,subregionp-spec-name len1 ad1 len2 ad2)
           ...
           (defthm ,(acl2::pack-in-package pkg subregionp-spec-name '-of-bvchop-arg4)
             (equal (,subregionp-spec-name len1 ad1 len2 (bvchop ,num-address-bits ad2))
                    (,subregionp-spec-name len1 ad1 len2 ad2))
             :hints (("Goal" :use (,(acl2::pack-in-package pkg subregionp-spec-name '-of-bvchop-arg4-fw)
                                   ,(acl2::pack-in-package pkg subregionp-spec-name '-of-bvchop-arg4-bk))))))

         (local
           (defthm ,(acl2::pack-in-package pkg subregionp-name '-correct-forward)
             (implies (,subregionp-name len1 ad1 len2 ad2)
                      (,subregionp-spec-name len1 ad1 len2 ad2))
             :hints (("Goal"
                      :use (:instance ,(acl2::pack-in-package pkg in-regionp-name '-when- in-regionp-name '-and- subregionp-name)
                                      (ad (,(acl2::pack-in-package pkg subregionp-spec-name '-witness) len1 ad1 len2 ad2)))
                      :in-theory (e/d (,subregionp-spec-name)
                                      (,(acl2::pack-in-package pkg in-regionp-name '-when- in-regionp-name '-and- subregionp-name)))))))

         (local
           (defthm ,(acl2::pack-in-package pkg subregionp-spec-name '-same-ads-forward)
             (implies (and; (unsigned-byte-p ,num-address-bits len1)
                      ...
                      (implies (,subregionp-spec-name len1 ad len2 ad)
                               (<= len1 len2)))
             :hints (("Goal" :use (:instance ,(acl2::pack-in-package pkg 'not- subregionp-spec-name '-when- in-regionp-name '-and-not- in-regionp-name)
                                             (ad (bvplus ,num-address-bits ad len2))
                                             (ad1 ad)
                                             (ad2 ad))
                      :in-theory (e/d (,in-regionp-name bvlt)
                                      (,(acl2::pack-in-package pkg 'not- subregionp-spec-name '-when- in-regionp-name '-and-not- in-regionp-name)))))))

         (local
           (defthm ,(acl2::pack-in-package pkg subregionp-spec-name '-same-ads-backward)
             (implies (and (unsigned-byte-p ,num-address-bits len1)
                           (unsigned-byte-p ,num-address-bits len2)
                           )
                      (implies (<= len1 len2)
         ...
         (defthm ,(acl2::pack-in-package pkg subregionp-spec-name '-same-ads)
           (implies (and (unsigned-byte-p ,num-address-bits len1)
                         (unsigned-byte-p ,num-address-bits len2))
...
;todo: move some of this up:
         ...
         (defthmd ,(acl2::pack-in-package pkg subregionp-name '-opener)
           (implies (and ;(unsigned-byte-p ,num-address-bits len1)
                      ;(unsigned-byte-p ,num-address-bits len2)
                      (integerp ad1)
                      (not (zp len1)))
         ...
         (local
           (defthm ,(acl2::pack-in-package pkg 'unsigned-byte-p-of-bad-guy)
         ...
         (local
           (defthm ,(acl2::pack-in-package pkg in-regionp-name '-of-bad-guy)
             (implies (and (bad-guy len1 ad1 len2 ad2)
                           ;(integerp ad1)
                           (unsigned-byte-p ,num-address-bits ad1)
                           (unsigned-byte-p ,num-address-bits ad2)
             ...
             :hints (("Goal" :in-theory (enable ,(acl2::pack-in-package pkg in-regionp-name '-opener)
                                                bad-guy
                                                ,(acl2::pack-in-package pkg in-regionp-name '-of-+-arg3))))))

         (local
           (defthm ,(acl2::pack-in-package pkg subregionp-name '-when- in-regionp-name '-of-bad-guy)
             (implies (and (,in-regionp-name (bad-guy len1 ad1 len2 ad2) len2 ad2)
                           ...
                           (integerp ad1))
                      (,subregionp-name len1 ad1 len2 ad2))
             :hints (("Goal" :in-theory (enable ,(acl2::pack-in-package pkg subregionp-name '-opener)
                                                bad-guy
                                                ,(acl2::pack-in-package pkg subregionp-name '-of-+-arg2))))))

         (local
           (defthm ,(acl2::pack-in-package pkg subregionp-name '-when-not-bad-guy)
         ...
         ;;drop some hyps?
         (defthmd ,(acl2::pack-in-package pkg subregionp-name '-correct-back)
           ...
           :hints (("Goal"
                    ...
                    :in-theory (enable ,(acl2::pack-in-package pkg subregionp-name '-opener)))))

         ;; Our subregionp function matches the spec
         (defthm ,(acl2::pack-in-package pkg subregionp-name '-correct)
           (equal (,subregionp-spec-name len1 ad1 len2 ad2)
                  (,subregionp-name len1 ad1 len2 ad2))
           :hints (("Goal" :use (:instance ,(acl2::pack-in-package pkg subregionp-name '-correct-back)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
         ...
         (defthm ,(acl2::pack-in-package pkg disjoint-regionsp-name '-when- disjoint-regionsp-name '-and- subregionp-name '-and- subregionp-name '-alt)
           (implies (and (,disjoint-regionsp-name len2 ad2 len1 ad1)
                         (,subregionp-name len3 ad3 len1 ad1) ; expand to bv ops?
                         (,subregionp-name len4 ad4 len2 ad2) ; expand to bv ops?
                         )
                    (,disjoint-regionsp-name len3 ad3 len4 ad4))
           :hints (("Goal" :in-theory (enable ,(acl2::pack-in-package pkg disjoint-regionsp-name '-symmetric)))))))))

(defmacro make-memory-region-machinery (num-address-bits pkg)
  (make-memory-region-machinery-fn num-address-bits pkg))
