(CONSP-OF-STRIP-CARS
 (6 6 (:REWRITE DEFAULT-CAR))
 (3 3 (:REWRITE DEFAULT-CDR))
 )
(LEN-OF-STRIP-CARS
 (18 8 (:REWRITE DEFAULT-CDR))
 (14 7 (:REWRITE DEFAULT-+-2))
 (7 7 (:REWRITE DEFAULT-+-1))
 (6 6 (:REWRITE DEFAULT-CAR))
 (6 6 (:REWRITE CONSP-OF-STRIP-CARS))
 )
(STRIP-CARS-OF-CONS
 (8 8 (:REWRITE DEFAULT-CAR))
 (3 3 (:REWRITE DEFAULT-CDR))
 )
(STRIP-CARS-OF-ACONS)
(CAR-OF-STRIP-CARS
 (2 2 (:REWRITE DEFAULT-CDR))
 (2 2 (:REWRITE CONSP-OF-STRIP-CARS))
 )
(CADR-OF-STRIP-CARS
 (12 9 (:REWRITE DEFAULT-CAR))
 (8 5 (:REWRITE DEFAULT-CDR))
 )
(NTH-OF-STRIP-CARS
 (24 23 (:REWRITE DEFAULT-CAR))
 (14 7 (:REWRITE DEFAULT-CDR))
 (6 6 (:REWRITE ZP-OPEN))
 (6 2 (:REWRITE CAR-OF-STRIP-CARS))
 (5 5 (:REWRITE DEFAULT-+-2))
 (5 5 (:REWRITE DEFAULT-+-1))
 )
(STRIP-CARS-OF-APPEND
 (38 37 (:REWRITE DEFAULT-CAR))
 (28 18 (:REWRITE DEFAULT-CDR))
 (9 3 (:REWRITE CAR-OF-STRIP-CARS))
 (8 8 (:TYPE-PRESCRIPTION TRUE-LISTP))
 (6 6 (:REWRITE CONSP-OF-STRIP-CARS))
 )
(STRIP-CARS-WHEN-NOT-CONSP-CHEAP)
(STRIP-CARS-OF-CDR
 (9 6 (:REWRITE DEFAULT-CDR))
 (8 8 (:REWRITE DEFAULT-CAR))
 (6 6 (:REWRITE STRIP-CARS-WHEN-NOT-CONSP-CHEAP))
 )
(MEMBER-EQUAL-OF-STRIP-CARS-IFF
 (128 120 (:REWRITE DEFAULT-CAR))
 (59 32 (:REWRITE DEFAULT-CDR))
 (21 7 (:REWRITE CAR-OF-STRIP-CARS))
 (14 14 (:REWRITE CONSP-OF-STRIP-CARS))
 )
(STRIP-CARS-OF-PAIRLIS$
 (18 18 (:REWRITE DEFAULT-CAR))
 (14 9 (:REWRITE STRIP-CARS-WHEN-NOT-CONSP-CHEAP))
 (10 10 (:REWRITE DEFAULT-CDR))
 )
(STRIP-CARS-OF-REVAPPEND
 (158 4 (:REWRITE CDR-OF-REVAPPEND))
 (112 4 (:DEFINITION BUTLAST))
 (73 57 (:REWRITE DEFAULT-CAR))
 (56 41 (:REWRITE DEFAULT-CDR))
 (48 4 (:DEFINITION TAKE))
 (40 40 (:TYPE-PRESCRIPTION LEN))
 (30 4 (:REWRITE CAR-OF-REVAPPEND))
 (24 20 (:REWRITE DEFAULT-+-2))
 (20 20 (:REWRITE DEFAULT-+-1))
 (20 4 (:DEFINITION LEN))
 (16 16 (:TYPE-PRESCRIPTION LAST))
 (16 4 (:REWRITE ZP-OPEN))
 (12 12 (:TYPE-PRESCRIPTION TRUE-LISTP))
 (12 12 (:TYPE-PRESCRIPTION TAKE))
 (12 8 (:REWRITE DEFAULT-<-2))
 (12 4 (:REWRITE FOLD-CONSTS-IN-+))
 (12 4 (:REWRITE COMMUTATIVITY-OF-+))
 (12 4 (:DEFINITION LAST))
 (8 8 (:REWRITE DEFAULT-<-1))
 (2 2 (:REWRITE CONSP-OF-REVAPPEND))
 )
(STRIP-CARS-OF-REVERSE
 (12 2 (:REWRITE DEFAULT-COERCE-3))
 (4 4 (:REWRITE DEFAULT-COERCE-2))
 (2 2 (:REWRITE DEFAULT-COERCE-1))
 (2 2 (:REWRITE CONSP-OF-REVAPPEND))
 )
(STRIP-CARS-IFF
 (10 10 (:REWRITE DEFAULT-CAR))
 (8 8 (:REWRITE STRIP-CARS-WHEN-NOT-CONSP-CHEAP))
 (4 4 (:REWRITE DEFAULT-CDR))
 )
(<=-OF-ACL2-COUNT-OF-STRIP-CARS-LINEAR
 (916 432 (:REWRITE DEFAULT-+-2))
 (606 432 (:REWRITE DEFAULT-+-1))
 (210 42 (:DEFINITION LEN))
 (185 135 (:REWRITE DEFAULT-CDR))
 (136 107 (:REWRITE DEFAULT-<-1))
 (128 107 (:REWRITE DEFAULT-<-2))
 (84 84 (:REWRITE DEFAULT-UNARY-MINUS))
 (48 16 (:REWRITE CAR-OF-STRIP-CARS))
 (42 42 (:REWRITE DEFAULT-NUMERATOR))
 (42 42 (:REWRITE DEFAULT-DENOMINATOR))
 (42 42 (:REWRITE DEFAULT-COERCE-2))
 (42 42 (:REWRITE DEFAULT-COERCE-1))
 (41 41 (:REWRITE DEFAULT-REALPART))
 (41 41 (:REWRITE DEFAULT-IMAGPART))
 )
