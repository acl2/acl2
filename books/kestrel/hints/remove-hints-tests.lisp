...
(assert! (mv-let (breakage-type res)
           (break-hint-keyword-value-list-in-nth-way 0 '(:in-theory (enable foo bar)))
           (and (equal breakage-type '(:remove-enable-item foo))
                (equal res '(:in-theory (enable bar))))))

(assert! (mv-let (breakage-type res)
           (break-hint-keyword-value-list-in-nth-way 1 '(:in-theory (enable foo bar)))
           (and (equal breakage-type '(:remove-enable-item bar))
                (equal res '(:in-theory (enable foo))))))

(assert! (mv-let (breakage-type res)
           (break-hint-keyword-value-list-in-nth-way 0 '(:in-theory (e/d (foo) (bar))))
           (and (equal breakage-type '(:remove-enable-item foo))
                (equal res '(:in-theory (disable bar))))))

(assert! (mv-let (breakage-type res)
           (break-hint-keyword-value-list-in-nth-way 1 '(:in-theory (e/d (foo) (bar))))
           (and (equal breakage-type '(:remove-disable-item bar))
                (equal res '(:in-theory (enable foo))))))
