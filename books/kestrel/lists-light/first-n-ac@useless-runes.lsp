(CONSP-OF-FIRST-N-AC (2 2 (:REWRITE ZP-OPEN))
                     (2 2 (:REWRITE DEFAULT-+-2))
                     (2 2 (:REWRITE DEFAULT-+-1))
                     (1 1 (:REWRITE DEFAULT-CAR))
                     (1 1 (:REWRITE DEFAULT-<-2))
                     (1 1 (:REWRITE DEFAULT-<-1)))
(FIRST-N-AC-OF-CONS (10 4 (:REWRITE ZP-OPEN))
                    (10 2 (:REWRITE REVAPPEND-OF-CONS))
                    (6 2 (:REWRITE FOLD-CONSTS-IN-+))
                    (6 2
                       (:REWRITE APPEND-WHEN-NOT-CONSP-ARG1-CHEAP))
                    (5 5 (:REWRITE DEFAULT-+-2))
                    (5 5 (:REWRITE DEFAULT-+-1))
                    (3 3 (:REWRITE DEFAULT-CDR))
                    (3 3 (:REWRITE DEFAULT-CAR))
                    (3 3 (:REWRITE DEFAULT-<-2))
                    (3 3 (:REWRITE DEFAULT-<-1))
                    (2 2 (:REWRITE APPEND-OF-CONS-ARG1)))
(FIRST-N-AC-OF-APPEND-ARG3 (350 2 (:REWRITE REVAPPEND-OF-CONS))
                           (222 2 (:REWRITE APPEND-ASSOCIATIVE))
                           (180 32 (:DEFINITION REVAPPEND))
                           (140 14 (:DEFINITION TAKE))
                           (138 40
                                (:REWRITE APPEND-WHEN-NOT-CONSP-ARG1-CHEAP))
                           (103 103 (:REWRITE DEFAULT-CDR))
                           (89 75 (:REWRITE DEFAULT-CAR))
                           (79 65 (:REWRITE DEFAULT-+-2))
                           (70 22 (:REWRITE ZP-OPEN))
                           (70 14 (:DEFINITION LEN))
                           (65 65 (:REWRITE DEFAULT-+-1))
                           (48 16 (:REWRITE FOLD-CONSTS-IN-+))
                           (44 30 (:REWRITE DEFAULT-<-2))
                           (42 42 (:TYPE-PRESCRIPTION TRUE-LISTP-TAKE))
                           (42 14 (:DEFINITION LAST))
                           (30 30 (:REWRITE DEFAULT-<-1))
                           (14 14 (:TYPE-PRESCRIPTION LAST)))
(FIRST-N-AC-OF-CONS-ARG3)
(FIRST-N-AC-NORMALIZE-AC (16 16 (:REWRITE DEFAULT-CAR))
                         (15 15 (:REWRITE REVAPPEND-NORMALIZE-ACC))
                         (15 15 (:REWRITE DEFAULT-CDR))
                         (15 15 (:REWRITE DEFAULT-+-2))
                         (15 15 (:REWRITE DEFAULT-+-1))
                         (5 5 (:REWRITE DEFAULT-<-2))
                         (5 5 (:REWRITE DEFAULT-<-1)))
(LEN-OF-FIRST-N-AC
     (104 59 (:REWRITE DEFAULT-+-2))
     (73 59 (:REWRITE DEFAULT-+-1))
     (50 31 (:REWRITE DEFAULT-CDR))
     (40 12 (:REWRITE CONSP-OF-FIRST-N-AC))
     (18 17 (:REWRITE DEFAULT-<-1))
     (18 6 (:DEFINITION POSP))
     (17 17 (:REWRITE DEFAULT-<-2))
     (6 6 (:REWRITE DEFAULT-CAR))
     (5 5 (:REWRITE ZP-OPEN))
     (1 1
        (:TYPE-PRESCRIPTION TRUE-LISTP-REVAPPEND-TYPE-PRESCRIPTION))
     (1 1 (:TYPE-PRESCRIPTION REVAPPEND))
     (1 1
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(TRUE-LIST-FIX-OF-FIRST-N-AC (62 41 (:REWRITE DEFAULT-CDR))
                             (61 38 (:REWRITE DEFAULT-CAR))
                             (43 13 (:REWRITE ZP-OPEN))
                             (30 10 (:REWRITE FOLD-CONSTS-IN-+))
                             (28 26 (:REWRITE DEFAULT-+-2))
                             (26 26 (:REWRITE DEFAULT-+-1))
                             (20 6 (:REWRITE CONSP-OF-FIRST-N-AC))
                             (20 2 (:DEFINITION TAKE))
                             (19 17 (:REWRITE DEFAULT-<-2))
                             (17 17 (:REWRITE DEFAULT-<-1))
                             (12 4 (:DEFINITION POSP))
                             (10 2 (:DEFINITION LEN))
                             (8 8 (:TYPE-PRESCRIPTION TRUE-LISTP))
                             (6 2 (:DEFINITION LAST))
                             (2 2 (:TYPE-PRESCRIPTION LAST)))
(FIRST-N-AC-OF-0
     (22 11
         (:TYPE-PRESCRIPTION TRUE-LISTP-FIRST-N-AC-TYPE-PRESCRIPTION))
     (11 11 (:TYPE-PRESCRIPTION TRUE-LISTP)))
(FIRST-N-AC-IFF (7 7 (:TYPE-PRESCRIPTION TRUE-LISTP))
                (6 6 (:REWRITE DEFAULT-CDR))
                (6 6 (:REWRITE DEFAULT-CAR))
                (5 5 (:REWRITE DEFAULT-+-2))
                (5 5 (:REWRITE DEFAULT-+-1))
                (4 4 (:REWRITE DEFAULT-<-2))
                (4 4 (:REWRITE DEFAULT-<-1))
                (1 1 (:REWRITE ZP-OPEN)))
(CAR-OF-FIRST-N-AC (27 27 (:REWRITE DEFAULT-CDR))
                   (17 11 (:REWRITE ZP-OPEN))
                   (12 6 (:REWRITE CONSP-OF-FIRST-N-AC))
                   (11 11 (:REWRITE DEFAULT-+-2))
                   (11 11 (:REWRITE DEFAULT-+-1))
                   (6 2 (:REWRITE FOLD-CONSTS-IN-+))
                   (6 2 (:DEFINITION POSP))
                   (4 4 (:REWRITE DEFAULT-<-2))
                   (4 4 (:REWRITE DEFAULT-<-1)))
(FIRST-N-AC-WHEN-NOT-INTEGERP-CHEAP
     (38 19
         (:TYPE-PRESCRIPTION TRUE-LISTP-FIRST-N-AC-TYPE-PRESCRIPTION))
     (19 19 (:TYPE-PRESCRIPTION TRUE-LISTP)))
