  ...
  (if (endp alist)
    ...
    (b* ((entry (first alist))
      ...
      (if (or (string-starts-withp path "META-INF/") ; I've seen module-info.class under META-INF/
          ...
          (events-for-classes-from-alist (rest alist) acc count)
        (b* ((bytes (cdr entry))
...
(defthm true-listp-of-mv-nth-1-of-events-for-classes-from-alist
  (implies (and (true-listp acc)
                (byte-list-listp (strip-cdrs alist)))
           (true-listp (mv-nth 1 (events-for-classes-from-alist alist acc count)))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; Returns (mv erp event byte-array-stobj state).
(defund read-jar-fn (jar-path dir class-names verbosep whole-form state byte-array-stobj)
  (declare (xargs :guard (and (stringp jar-path)
                  ...
                  :stobjs (state byte-array-stobj)))
  (b* (;; Check for redundancy:
       ((when (command-is-redundantp whole-form state))
        (mv (erp-nil) '(value-triple :invisible) state byte-array-stobj))
       ...
       ((when (not (stringp directory)))
        ...
        (mv `(:bad-include-book-dir ,directory) nil state byte-array-stobj))
       ...
       ((when erp) (mv erp nil state byte-array-stobj))
       ;; Parse each of the class files and create events to register it:
       ((mv erp events count)
        (events-for-classes-from-alist path-to-decompressed-bytes-alist nil 0))
       ((when erp) (mv erp nil state byte-array-stobj))
...
(defmacro read-jar (&whole whole-form
  ...
  `(make-event-quiet (read-jar-fn ,jar-path ,dir ',classes ,verbosep ',whole-form state byte-array-stobj)))
