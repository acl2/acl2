...
(defconst *make-prover-simple-rules*
  '((:COMPOUND-RECOGNIZER AXE-TREEP-COMPOUND-RECOGNIZER)
    ...
    (:REWRITE all-dargp-less-than-of-strip-cdrs-of-match-hyp-with-nodenum-to-assume-false)
                   ...
                   (if (atom expr) ;must be a variable - just see if its node needs to be replaced
                     ...
                     (let ((fn (ffn-symb expr)))
                       (if (eq 'quote fn) ;; it's a quoted constant, so it rewrites to itself:
                         ...
                         ;; TODO: Consider adding special handling for if and related operators.
                         ;; TODO: Consider trying to rewrite the IF-test using an equiv of IFF.
                         (let* ((args (dargs expr))
                           ...
                           (if extended-worklist-or-nil
                               ...
                               ;; When the current node is processed again, some analysis will be redone but the outcome will be different.
                               (,rewrite-nodes-name extended-worklist-or-nil result-array-name result-array
                                                    dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist
                                                    nodenums-to-assume-false assumption-array assumption-array-num-valid-nodes
                                                    rule-alist equiv-alist interpreted-function-alist print info tries monitored-symbols
                                                    case-designator prover-depth options (+ -1 count))
                             ;; All args are simplified (but only under an equiv of 'equal):
                             (b* ((simplified-args (lookup-args-in-result-array args result-array-name result-array)) ;combine this with the get-args-not-done somehow?
                                  ...
                                  ((mv erp new-nodenum-or-quotep dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist info tries)
                                   ;; TODO: Calling ,simplify-tree-name here is overkill.  Instead, we should use the fact that we know it's a function call with simplified args: First, replace nodenum args if possible using the assumption-array (if the equiv is better than the 'equal that was used when we processed it).  Then, check for resolvable if/boolif/etc.  Then, check for evaluatable ground term.  Then call ,simplify-fun-call-name (or make a wrapper that handles ground terms first).
                                   (,simplify-tree-name expr
                                  ...
                                  ;; ;; See whether the whole simplified node should be replaced using an assumption:
                                  ;; (new-nodenum-or-quotep
                                  ;;  (if (consp new-nodenum-or-quotep) ;check for quotep
                                  ;;      new-nodenum-or-quotep
                                  ;;    ;; it's a nodenum, so check the assumptions:
                                  ;;    (maybe-replace-nodenum-using-assumption-array new-nodenum-or-quotep 'equal assumption-array assumption-array-num-valid-nodes print)))
