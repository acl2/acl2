...
;; TODO: Rename since more general
...
(test-simp-form
 (simplify-terms-repeatedly '((if (equal x '3) y z)  (equal x '3)) nil nil t state)
 '(y (equal x '3)))

(test-simp-form
 (simplify-terms-repeatedly '((equal (car (cons x y)) '3) (equal x '4)) (make-rule-alist! '(car-cons) (w state)) nil t state)
 '('NIL (EQUAL X '4)))
...
(deftest
  ...
  (test-simp-form
   ...
   (simplify-terms-repeatedly '((foo2 (if (equal x '3) y 'nil)) (if (equal x '3) w v)) (make-rule-alist! '(foo2) (w state)) nil t state)
   '(y (equal x '3) w)))
