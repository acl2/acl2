...
# NOTE: The book tester.lisp should be certified before
# this script is run.
...
BASE_NAME="acl2-with-tester" # Also the name of the wrapper script that gets created

# We could test the actual image, but the file extension depends on Lisp+OS, so we test the wrapper script:
if [[ ( ! -f ${BASE_NAME} ) || ( ${BASE_NAME} -ot tester.cert ) || ( ${BASE_NAME} -ot tester.lisp ) ]] ;then
    rm -f "${BASE_NAME}"
    ...
    (echo '(include-book "kestrel/utilities/exit-if-function-not-defined" :dir :system) (include-book "kestrel/jvm/portcullis" :dir :system) (include-book "kestrel/axe/jvm/tester" :dir :system :ttags :all) (exit-if-function-not-defined test-file-fn) :q (save-exec "'${BASE_NAME}'" "This copy of ACL2 has the Formal Unit Tester built in.")' | ${ACL2})
    ls -l acl2-with-tester*
    echo "Done saving image.)"
else
    echo "Not saving an image for Formal Unit Tester (things seem to be up-to-date)."
fi
