; An unrolling lifter for ARM code
;
...
;; 10 seconds
(acl2::make-repeatedly-run-function repeatedly-run acl2::simplify-dag-arm)
...
(defun unroll-arm-code-core (target
  ...
  (b* ((- (cw "(Lifting ~s0.~%" target)) ;todo: print the executable name, also handle non-string targets better
       ...
       ((when (not (eq :elf-32 executable-type)))
        (er hard? 'unroll-arm-code-core "The executable type must be :elf-32, but it is ~x0." executable-type)
        (mv :bad-executable-type nil state))
       ;; Make sure it's a ARM executable:
       (- (ensure-arm parsed-executable))
       ...
       ((when erp)
        (er hard? 'unroll-arm-code-core "Error generating assumptions: ~x0." erp)
       ...
       ((when (quotep dag-to-simulate))
        (er hard? 'unroll-arm-code-core "Unexpected quotep: ~x0." dag-to-simulate)
...
;;                                  (remove-assumption-rules 'nil)
  ...
  `(,(if (print-level-at-least-tp print) 'make-event 'make-event-quiet)
    (acl2-unwind-protect ; enable cleanup on errors/interrupts
      ...
      (maybe-remove-temp-dir ; ,keep-temp-dir
        state)))
  :parents (acl2::axe-arm acl2::axe-lifters)
  :short "A tool to lift ARM binary code into logic, unrolling loops as needed."
  :args ((lifted-name "A symbol, the name to use for the generated function.  The name of the generated constant is created by adding stars to the front and back of this symbol.")
         (executable "The ARM binary executable that contains the target function.  Usually this is a string representing the file name/path of the executable.  However, it can instead be a parsed executable (satisfying @('parsed-executablep')).")
