...
;; WARNING: Keep in sync with the analogous tests for x86 and risc-v

;; Stops once it finds a non-write.
(defund write-with-addr-and-size-presentp-axe (size-darg addr-darg
                                                         stat-darg ; called "arm"
                                                         dag-array)
  ...
  (if (consp stat-darg) ; tests for quotep
    ...
    (and (mbt (natp stat-darg)) ; for termination
         (let ((expr (aref1 'dag-array dag-array stat-darg)))
           (if (and (consp expr)
                    (eq 'write (car expr)) ; (write n addr val arm)
