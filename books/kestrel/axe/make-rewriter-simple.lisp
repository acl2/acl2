...
(defun make-rewriter-simple-fn (suffix ;; gets added to generated names
  ...
  (let* ((eval-axe-syntaxp-expr-fn (pack$ 'eval-axe-syntaxp-expr- syntaxp-evaluator-suffix)) ; keep in sync with make-axe-syntaxp-evaluator.lisp
         ...
         (def-simplified-dag-core-name (pack$ 'def-simplified-dag-core- suffix))
...
;maybe-dargp
         ...
         ;; Core function of def-simplified-dag-xxx.  Unlike its wrapper, this function is in :logic mode.
         ;; Returns (mv erp event state).
         ;; TODO: Perhaps add an option to take a rule-alist.
         ;; TODO: Allow passing a term instead of a DAG.
         (defund ,def-simplified-dag-core-name (defconst-name ; the name of the constant to create
                                                dag
                                                assumptions
                                                rules
                                                interpreted-function-alist
         ...
         (defund ,def-simplified-dag-fn-name (defconst-name ; the name of the constant to create
           ...
           (b* (((when (command-is-redundantp whole-form state)) ; will check the table named (pack$ def-simplified-dag-name '-table)
                ...
                (assumptions (translate-terms assumptions ',def-simplified-dag-fn-name (w state))))
             (,def-simplified-dag-core-name defconst-name dag assumptions rules interpreted-function-alist normalize-xors limits memoizep count-hits print monitored-symbols fns-to-elide whole-form state)))
