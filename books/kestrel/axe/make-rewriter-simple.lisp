...
(defun make-rewriter-simple-fn (suffix ;; gets added to generated names
  ...
  (let* (;; functions in the main mutual recursion:
              ...
              (b* ((hyp (first hyps))
                ...
                (if (eq 'axe-syntaxp fn)
                  ...
                  (if (eq 'axe-bind-free fn)
                    ...
                    ;; First, we substitute in for all the vars in HYP that are bound in ALIST (this also evaluates what it can):
                    (mv-let
                      ...
                      (if (fquotep instantiated-hyp) ;; we know the instantiated-hyp is a cons, because hyp is
                        ...
                        (if free-vars-flg ;can't be a work-hard since there are free vars
                            ;; If some free vars remain in the instantiated-hyp, we search the REFINED-ASSUMPTIONS for matches to bind them:
                            ...
                            (,relieve-free-var-hyp-and-all-others-name (lookup-eq fn refined-assumption-alist)
                  ...
                  (b* ((- (and (eq print ':verbose2)
                    ...
                    (if hyps-relievedp
                        ...
                        (prog2$ (and (eq print ':verbose2)
                                ...
                                (mv (erp-nil)
                                    ;; TODO: This bakes in the basic evaluator:
                                    (my-sublis-var-and-eval-basic alist (stored-rule-rhs stored-rule) interpreted-function-alist)
                            ...
                            (b* ((args (fargs tree))
                              ...
                              (if (consp fn) ;;tests for lambda
                                  ...
                                  (let* ((formals (second fn))
                                         (body (third fn))
                                         ;;BOZO could optimize this pattern: (my-sublis-var (pairlis$-fast formals args) body)
                                         ;; TODO: This bakes in the basic evaluator:
                                         (new-expr (my-sublis-var-and-eval-basic (pairlis$-fast formals args) body interpreted-function-alist)))
