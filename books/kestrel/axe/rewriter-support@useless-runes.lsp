(LEN-OF-CADAR-WHEN-PSEUDO-TERMP
     (147 3 (:DEFINITION SYMBOL-LISTP))
     (99 55 (:REWRITE DEFAULT-CAR))
     (96 5 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (85 52 (:REWRITE DEFAULT-CDR))
     (78 78 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (62 31
         (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (62 6
         (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (60 21
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (56 10 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (44 1 (:DEFINITION PSEUDO-TERM-LISTP))
     (36 36
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (34 5 (:REWRITE ALL-CONSP-OF-CDR))
     (31 16 (:REWRITE DEFAULT-+-2))
     (26 14 (:REWRITE DEFAULT-<-2))
     (20 20 (:TYPE-PRESCRIPTION ALL-CONSP))
     (16 16 (:REWRITE DEFAULT-+-1))
     (15 15 (:REWRITE USE-ALL-CONSP-2))
     (15 15 (:REWRITE USE-ALL-CONSP))
     (15 15 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (14 14 (:REWRITE DEFAULT-<-1))
     (14 14 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
     (12 5
         (:REWRITE SYMBOLP-OF-CAR-WHEN-AXE-TREEP-CHEAP))
     (10 10
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (10 5
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (8 4
        (:REWRITE PSEUDO-TERM-LISTP-WHEN-SYMBOL-LISTP-CHEAP))
     (7 7
        (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (7 1 (:LINEAR LEN-OF-CDR-LINEAR-STRONG))
     (6 6
        (:REWRITE EQUAL-OF-+-COMBINE-CONSTANTS))
     (6 6
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (5 5 (:TYPE-PRESCRIPTION AXE-TREEP))
     (5 5 (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (5 5
        (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
     (4 4
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (4 4
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (4 4
        (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (3 3 (:REWRITE DEFAULT-COERCE-2))
     (3 3 (:REWRITE DEFAULT-COERCE-1)))
(LEN-OF-CADAR-WHEN-AXE-TREEP
    (11890 6 (:DEFINITION ALL-AXE-TREEP))
    (10914 35 (:DEFINITION PSEUDO-TERMP))
    (10229 21 (:REWRITE ALL-AXE-TREEP-OF-CDR-2))
    (9247 21 (:REWRITE ALL-AXE-TREEP-OF-CDR))
    (5643 19
          (:REWRITE AXE-TREEP-WHEN-PSEUDO-TERMP))
    (2447 45 (:DEFINITION SYMBOL-LISTP))
    (2440 27
          (:REWRITE ALL-AXE-TREEP-WHEN-PSEUDO-TERM-LISTP))
    (2314 27 (:DEFINITION PSEUDO-TERM-LISTP))
    (1625 9 (:REWRITE AXE-TREEP-OF-CAR))
    (1496 98 (:REWRITE USE-ALL-CONSP-FOR-CAR))
    (1400 700
          (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
    (1192 735 (:REWRITE DEFAULT-CAR))
    (1142 357
          (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
    (1086 1032
          (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
    (1085 745 (:REWRITE DEFAULT-CDR))
    (1031 1031
          (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
    (1005 19 (:REWRITE AXE-TREEP-WHEN-DARGP))
    (801 173 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
    (800 800
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
    (764 105 (:DEFINITION LENGTH))
    (753 27
         (:REWRITE ALL-AXE-TREEP-WHEN-ALL-DARGP))
    (666 74
         (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
    (635 17 (:DEFINITION MYQUOTEP))
    (484 51 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
    (483 6 (:DEFINITION DARGP))
    (482 78 (:REWRITE ALL-CONSP-OF-CDR))
    (374 202 (:REWRITE DEFAULT-+-2))
    (372 372 (:TYPE-PRESCRIPTION TRUE-LISTP))
    (346 346 (:TYPE-PRESCRIPTION ALL-CONSP))
    (346 193 (:REWRITE DEFAULT-<-2))
    (282 17 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
    (246 90
         (:REWRITE SYMBOLP-OF-CAR-WHEN-AXE-TREEP-CHEAP))
    (226 17 (:DEFINITION QUOTEP))
    (218 218
         (:TYPE-PRESCRIPTION PSEUDO-TERM-LISTP))
    (205 205 (:REWRITE USE-ALL-CONSP-2))
    (205 205 (:REWRITE USE-ALL-CONSP))
    (205 205 (:REWRITE CONSP-WHEN-LEN-GREATER))
    (202 202 (:REWRITE DEFAULT-+-1))
    (196 98
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
    (193 193 (:REWRITE DEFAULT-<-1))
    (193 193 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
    (178 89
         (:REWRITE PSEUDO-TERM-LISTP-WHEN-SYMBOL-LISTP-CHEAP))
    (173 173
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
    (172 25 (:REWRITE ALL-DARGP-OF-CDR))
    (102 102 (:TYPE-PRESCRIPTION ALL-DARGP))
    (102 51
         (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
    (101 101
         (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
    (98 9 (:REWRITE USE-ALL-DARGP-FOR-CAR))
    (97 97
        (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
    (93 93
        (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
    (90 90
        (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
    (89 89
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
    (82 82
        (:REWRITE EQUAL-OF-+-COMBINE-CONSTANTS))
    (69 69
        (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
    (67 67
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
    (52 14 (:REWRITE +-COMBINE-CONSTANTS))
    (51 51 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
    (51 51
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
    (51 51
        (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
    (35 35 (:REWRITE DEFAULT-COERCE-2))
    (35 35 (:REWRITE DEFAULT-COERCE-1))
    (33 19
        (:REWRITE AXE-TREEP-WHEN-EQUAL-OF-CAR-AND-QUOTE-CHEAP))
    (31 17
        (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
    (19 19
        (:REWRITE AXE-TREEP-WHEN-NOT-CONSP-AND-NOT-SYMBOLP-CHEAP))
    (17 17 (:TYPE-PRESCRIPTION DARGP))
    (17 17 (:REWRITE USE-ALL-DARGP-2))
    (17 17 (:REWRITE USE-ALL-DARGP))
    (17 17
        (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
    (17 17
        (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
    (17 17 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
    (17 17
        (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
    (12 6 (:REWRITE DARGP-WHEN-NATP-CHEAP))
    (12 6 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP))
    (8 8
       (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
    (8 8
       (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
    (7 1 (:LINEAR LEN-OF-CDR-LINEAR-STRONG))
    (6 6 (:TYPE-PRESCRIPTION NATP))
    (6 6 (:TYPE-PRESCRIPTION MYQUOTEP))
    (5 5
       (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
    (5 5
       (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
    (3 1
       (:REWRITE PSEUDO-TERMP-OF-CDR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP)))
(PSEUDO-TERMP-OF-CADDDR-WHEN-AXE-TREEP
     (931 2 (:REWRITE ALL-AXE-TREEP-OF-CDR))
     (705 59 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (454 2
          (:REWRITE AXE-TREEP-WHEN-PSEUDO-TERMP))
     (314 6 (:DEFINITION SYMBOL-LISTP))
     (192 10 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (152 92 (:REWRITE DEFAULT-CAR))
     (139 97 (:REWRITE DEFAULT-CDR))
     (137 133 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (133 133 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (112 56
          (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (112 20 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (105 13
          (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (91 29
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (68 10 (:REWRITE ALL-CONSP-OF-CDR))
     (66 66
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (62 3
         (:REWRITE ALL-AXE-TREEP-WHEN-ALL-DARGP))
     (54 36 (:REWRITE DEFAULT-<-2))
     (52 2 (:REWRITE AXE-TREEP-WHEN-DARGP))
     (50 50 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (44 3
         (:REWRITE ALL-AXE-TREEP-WHEN-PSEUDO-TERM-LISTP))
     (40 40 (:TYPE-PRESCRIPTION ALL-CONSP))
     (40 29 (:REWRITE DEFAULT-+-2))
     (40 3 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (36 36 (:REWRITE DEFAULT-<-1))
     (36 36 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
     (32 2 (:REWRITE ALL-AXE-TREEP-OF-CDR-2))
     (31 31 (:REWRITE USE-ALL-CONSP-2))
     (31 31 (:REWRITE USE-ALL-CONSP))
     (31 31 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (31 2 (:DEFINITION PSEUDO-TERM-LISTP))
     (29 29 (:REWRITE DEFAULT-+-1))
     (28 1 (:DEFINITION DARGP))
     (24 10
         (:REWRITE SYMBOLP-OF-CAR-WHEN-AXE-TREEP-CHEAP))
     (21 2 (:DEFINITION MYQUOTEP))
     (20 20
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (20 10
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (19 2 (:DEFINITION QUOTEP))
     (18 18
         (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (16 8
         (:REWRITE PSEUDO-TERM-LISTP-WHEN-SYMBOL-LISTP-CHEAP))
     (15 15
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (12 12
         (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (12 12
         (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (12 4 (:REWRITE +-COMBINE-CONSTANTS))
     (10 10
         (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (10 10
         (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
     (8 8
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (8 2 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
     (8 1 (:REWRITE ALL-DARGP-OF-CDR))
     (6 6 (:TYPE-PRESCRIPTION ALL-DARGP))
     (6 3
        (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (4 4
        (:REWRITE EQUAL-OF-+-COMBINE-CONSTANTS))
     (3 3 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (3 3
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (3 3
        (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
     (2 2 (:TYPE-PRESCRIPTION DARGP))
     (2 2 (:REWRITE USE-ALL-DARGP-2))
     (2 2 (:REWRITE USE-ALL-DARGP))
     (2 2
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
     (2 2 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
     (2 2
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (2 2 (:REWRITE DEFAULT-COERCE-2))
     (2 2 (:REWRITE DEFAULT-COERCE-1))
     (2 2
        (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
     (2 2 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE AXE-TREEP-WHEN-NOT-CONSP-AND-NOT-SYMBOLP-CHEAP))
     (2 2
        (:REWRITE AXE-TREEP-WHEN-EQUAL-OF-CAR-AND-QUOTE-CHEAP))
     (2 1 (:REWRITE DARGP-WHEN-NATP-CHEAP))
     (2 1 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP))
     (1 1 (:TYPE-PRESCRIPTION NATP))
     (1 1 (:TYPE-PRESCRIPTION MYQUOTEP)))
(ALL-DARGP-OF-IF)
(ALL-DARGP-LESS-THAN-OF-IF)
(ALL-AXE-TREEP-WHEN-ALL-DARGP-LESS-THAN
     (167 1 (:DEFINITION ALL-DARGP-LESS-THAN))
     (89 5 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (87 1 (:DEFINITION DARGP-LESS-THAN))
     (68 1 (:DEFINITION MYQUOTEP))
     (56 4
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (36 36 (:TYPE-PRESCRIPTION LEN))
     (26 2
         (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (26 1 (:DEFINITION QUOTEP))
     (22 6
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (20 2 (:REWRITE LEN-OF-CDR))
     (11 6 (:REWRITE DEFAULT-<-2))
     (10 10 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (10 10 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (10 5
         (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (9 1
        (:REWRITE USE-ALL-DARGP-LESS-THAN-FOR-CAR))
     (9 1 (:REWRITE ALL-DARGP-LESS-THAN-OF-CDR))
     (8 4
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (8 1 (:DEFINITION NATP))
     (6 6
        (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (6 6 (:REWRITE DEFAULT-<-1))
     (6 6 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
     (5 5 (:REWRITE USE-ALL-CONSP-2))
     (5 5 (:REWRITE USE-ALL-CONSP))
     (5 5 (:REWRITE DEFAULT-CDR))
     (5 5 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (5 1 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (4 4 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (4 4
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (4 4
        (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
     (4 2 (:REWRITE DEFAULT-+-2))
     (3 1
        (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-CAR-AND-QUOTE))
     (2 2 (:TYPE-PRESCRIPTION ALL-CONSP))
     (2 2
        (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (2 2 (:REWRITE EQUAL-OF-LEN-AND-0))
     (2 2 (:REWRITE DEFAULT-CAR))
     (2 2 (:REWRITE DEFAULT-+-1))
     (2 2
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (2 1
        (:REWRITE DARGP-LESS-THAN-WHEN-NATP-CHEAP))
     (2 1
        (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (1 1 (:TYPE-PRESCRIPTION NATP))
     (1 1 (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
     (1 1 (:REWRITE USE-ALL-DARGP-LESS-THAN))
     (1 1
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (1 1
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (1 1
        (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
     (1 1
        (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
     (1 1 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
     (1 1
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (1 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (1 1
        (:REWRITE DARGP-LESS-THAN-WHEN-QUOTEP-CHEAP))
     (1 1
        (:REWRITE DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (1 1
        (:REWRITE DARGP-LESS-THAN-WHEN-CONSP-CHEAP))
     (1 1 (:REWRITE DARGP-LESS-THAN-MONO))
     (1 1
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (1 1 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP)))
(ALL-AXE-TREEP-OF-CONS
     (3540 12 (:DEFINITION PSEUDO-TERMP))
     (2718 9
           (:REWRITE AXE-TREEP-WHEN-PSEUDO-TERMP))
     (1810 2 (:DEFINITION AXE-TREEP))
     (1751 148 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (1319 1319 (:TYPE-PRESCRIPTION LEN))
     (1258 4 (:REWRITE ALL-AXE-TREEP-OF-CDR))
     (718 14 (:DEFINITION SYMBOL-LISTP))
     (640 88 (:REWRITE LEN-OF-CDR))
     (639 14
          (:REWRITE ALL-AXE-TREEP-WHEN-PSEUDO-TERM-LISTP))
     (610 44
          (:REWRITE EQUAL-OF-+-WHEN-NEGATIVE-CONSTANT))
     (578 9 (:REWRITE AXE-TREEP-WHEN-DARGP))
     (556 15 (:DEFINITION PSEUDO-TERM-LISTP))
     (548 30 (:DEFINITION TRUE-LISTP))
     (499 147
          (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (462 10 (:DEFINITION MYQUOTEP))
     (440 6 (:DEFINITION DARGP))
     (434 14
          (:REWRITE ALL-AXE-TREEP-WHEN-ALL-DARGP))
     (428 36 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (363 363 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (363 363 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (360 220 (:REWRITE DEFAULT-CAR))
     (346 234 (:REWRITE DEFAULT-CDR))
     (312 24
          (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (256 128
          (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (252 36 (:DEFINITION LENGTH))
     (229 21 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (218 52 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (164 164
          (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (154 81 (:REWRITE DEFAULT-<-2))
     (136 10 (:DEFINITION QUOTEP))
     (128 4 (:REWRITE ALL-AXE-TREEP-OF-CDR-2))
     (120 54 (:REWRITE DEFAULT-+-2))
     (112 10 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
     (109 109 (:TYPE-PRESCRIPTION PSEUDO-TERMP))
     (108 18 (:REWRITE ALL-CONSP-OF-CDR))
     (104 104
          (:TYPE-PRESCRIPTION PSEUDO-TERM-LISTP))
     (104 104 (:TYPE-PRESCRIPTION ALL-CONSP))
     (98 1 (:REWRITE ALL-DARGP-OF-CONS))
     (96 96 (:TYPE-PRESCRIPTION SYMBOL-LISTP))
     (84 84 (:REWRITE USE-ALL-CONSP-2))
     (84 84 (:REWRITE USE-ALL-CONSP))
     (84 84 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (84 42
         (:REWRITE PSEUDO-TERM-LISTP-WHEN-SYMBOL-LISTP-CHEAP))
     (81 81 (:REWRITE DEFAULT-<-1))
     (81 81 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
     (72 72 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (72 36
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (64 32
         (:REWRITE SYMBOLP-OF-CAR-WHEN-AXE-TREEP-CHEAP))
     (64 2 (:REWRITE AXE-TREEP-OF-CAR))
     (54 54 (:REWRITE DEFAULT-+-1))
     (52 52
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (52 8 (:DEFINITION NATP))
     (44 44 (:TYPE-PRESCRIPTION AXE-TREEP))
     (42 42
         (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (42 21
         (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (40 40 (:TYPE-PRESCRIPTION ALL-DARGP))
     (40 40 (:REWRITE EQUAL-OF-LEN-AND-0))
     (39 39
         (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (38 38
         (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (32 32
         (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
     (32 16 (:DEFINITION FIX))
     (32 4 (:REWRITE ALL-DARGP-OF-CDR))
     (28 28
         (:REWRITE EQUAL-OF-+-COMBINE-CONSTANTS))
     (28 28
         (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (21 21 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (21 21
         (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (21 21
         (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
     (18 18
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (16 16
         (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (16 2 (:REWRITE USE-ALL-DARGP-FOR-CAR))
     (14 14
         (:REWRITE ALL-AXE-TREEP-WHEN-ALL-DARGP-LESS-THAN))
     (14 10
         (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
     (13 9
         (:REWRITE AXE-TREEP-WHEN-EQUAL-OF-CAR-AND-QUOTE-CHEAP))
     (12 12 (:REWRITE DEFAULT-COERCE-2))
     (12 12 (:REWRITE DEFAULT-COERCE-1))
     (12 6 (:REWRITE DARGP-WHEN-NATP-CHEAP))
     (12 6 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP))
     (10 10 (:REWRITE USE-ALL-DARGP-2))
     (10 10 (:REWRITE USE-ALL-DARGP))
     (10 10
         (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
     (10 10
         (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
     (10 10 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
     (10 10
         (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (9 9 (:TYPE-PRESCRIPTION DARGP))
     (9 9
        (:REWRITE AXE-TREEP-WHEN-NOT-CONSP-AND-NOT-SYMBOLP-CHEAP))
     (8 8
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (8 8
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (6 6 (:TYPE-PRESCRIPTION NATP))
     (6 6 (:TYPE-PRESCRIPTION MYQUOTEP))
     (2 2
        (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (2 2
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN)))
(DARGP-LESS-THAN-WHEN-NOT-QUOTEP-CHEAP
     (24 2 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (16 10 (:REWRITE DEFAULT-<-2))
     (10 10 (:TYPE-PRESCRIPTION LEN))
     (10 10 (:REWRITE DEFAULT-<-1))
     (10 10 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
     (4 4
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (4 4
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (4 4
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (4 2
        (:REWRITE DARGP-LESS-THAN-WHEN-NATP-CHEAP))
     (2 2 (:TYPE-PRESCRIPTION NATP))
     (2 2 (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
     (2 2 (:REWRITE USE-ALL-DARGP-LESS-THAN))
     (2 2 (:REWRITE USE-ALL-CONSP-2))
     (2 2 (:REWRITE USE-ALL-CONSP))
     (2 2 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (2 2 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (2 2 (:REWRITE DEFAULT-CAR))
     (2 2
        (:REWRITE DARGP-LESS-THAN-WHEN-QUOTEP-CHEAP))
     (2 2
        (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-CAR-AND-QUOTE))
     (2 2 (:REWRITE DARGP-LESS-THAN-MONO))
     (2 2 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (2 2
        (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (1 1
        (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
     (1 1
        (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
     (1 1 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
     (1 1
        (:REWRITE DARGP-LESS-THAN-WHEN-CONSP-CHEAP)))
(SYMBOL-ALISTP-WHEN-ALISTP
     (240 20 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (44 23 (:REWRITE DEFAULT-CAR))
     (36 18 (:REWRITE DEFAULT-<-2))
     (33 20
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (30 6 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (27 24 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (24 24 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (19 19
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (18 18 (:REWRITE USE-ALL-CONSP-2))
     (18 18 (:REWRITE USE-ALL-CONSP))
     (18 18 (:REWRITE DEFAULT-<-1))
     (18 18 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (18 18 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
     (17 7
         (:REWRITE SYMBOLP-OF-CAR-WHEN-AXE-TREEP-CHEAP))
     (16 8
         (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (12 12 (:TYPE-PRESCRIPTION ALL-CONSP))
     (12 6
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (11 10 (:REWRITE DEFAULT-CDR))
     (10 5
         (:REWRITE SYMBOL-ALISTP-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (7 7 (:TYPE-PRESCRIPTION AXE-TREEP))
     (7 7
        (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (7 7
        (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
     (6 6
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (6 6 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP)))
(LEN-WHEN-AXE-TREEP-AND-CONSP-OF-CAR
     (101 6 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (78 2
         (:REWRITE AXE-TREEP-WHEN-PSEUDO-TERMP))
     (72 2 (:DEFINITION PSEUDO-TERMP))
     (61 2
         (:REWRITE ALL-AXE-TREEP-WHEN-ALL-DARGP))
     (60 6 (:DEFINITION TRUE-LISTP))
     (52 2 (:REWRITE AXE-TREEP-WHEN-DARGP))
     (50 1 (:REWRITE ALL-AXE-TREEP-OF-CDR))
     (43 2
         (:REWRITE ALL-AXE-TREEP-WHEN-PSEUDO-TERM-LISTP))
     (40 3 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (32 1 (:REWRITE ALL-AXE-TREEP-OF-CDR-2))
     (31 2 (:DEFINITION PSEUDO-TERM-LISTP))
     (28 1 (:DEFINITION DARGP))
     (26 8
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (26 2
         (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (21 2 (:DEFINITION MYQUOTEP))
     (20 2 (:REWRITE LEN-OF-CDR))
     (19 2 (:DEFINITION QUOTEP))
     (16 16 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (16 16
         (:TYPE-PRESCRIPTION PSEUDO-TERM-LISTP))
     (15 15 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (15 15 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (13 13 (:REWRITE DEFAULT-CDR))
     (13 13 (:REWRITE DEFAULT-CAR))
     (12 7 (:REWRITE DEFAULT-<-2))
     (12 6
         (:REWRITE PSEUDO-TERM-LISTP-WHEN-SYMBOL-LISTP-CHEAP))
     (8 8 (:TYPE-PRESCRIPTION PSEUDO-TERMP))
     (8 2 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
     (8 1 (:REWRITE ALL-DARGP-OF-CDR))
     (7 7 (:REWRITE DEFAULT-<-1))
     (7 7 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
     (6 6 (:TYPE-PRESCRIPTION SYMBOL-LISTP))
     (6 6 (:TYPE-PRESCRIPTION ALL-DARGP))
     (6 6 (:REWRITE USE-ALL-CONSP-2))
     (6 6 (:REWRITE USE-ALL-CONSP))
     (6 6
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (6 6 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (6 3
        (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (6 2 (:DEFINITION LENGTH))
     (4 4 (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (4 2 (:REWRITE DEFAULT-+-2))
     (3 3 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (3 3
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (3 3
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (3 3
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (3 3
        (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
     (2 2 (:TYPE-PRESCRIPTION DARGP))
     (2 2 (:REWRITE USE-ALL-DARGP-2))
     (2 2 (:REWRITE USE-ALL-DARGP))
     (2 2
        (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (2 2
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
     (2 2 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
     (2 2
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (2 2 (:REWRITE EQUAL-OF-LEN-AND-0))
     (2 2 (:REWRITE DEFAULT-+-1))
     (2 2
        (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
     (2 2 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE AXE-TREEP-WHEN-NOT-CONSP-AND-NOT-SYMBOLP-CHEAP))
     (2 2
        (:REWRITE AXE-TREEP-WHEN-EQUAL-OF-CAR-AND-QUOTE-CHEAP))
     (2 2
        (:REWRITE ALL-AXE-TREEP-WHEN-ALL-DARGP-LESS-THAN))
     (2 1 (:REWRITE DARGP-WHEN-NATP-CHEAP))
     (2 1 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP))
     (1 1 (:TYPE-PRESCRIPTION NATP))
     (1 1 (:TYPE-PRESCRIPTION MYQUOTEP)))
(INTEGERP-WHEN-DARGP (44 2 (:REWRITE CONSP-FROM-LEN-CHEAP))
                     (18 18 (:TYPE-PRESCRIPTION LEN))
                     (13 1
                         (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
                     (11 3
                         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
                     (10 1 (:REWRITE LEN-OF-CDR))
                     (5 5 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
                     (5 5 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
                     (5 3 (:REWRITE DEFAULT-<-2))
                     (3 3 (:REWRITE DEFAULT-CDR))
                     (3 3 (:REWRITE DEFAULT-<-1))
                     (3 3 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
                     (2 2 (:REWRITE USE-ALL-CONSP-2))
                     (2 2 (:REWRITE USE-ALL-CONSP))
                     (2 2 (:REWRITE CONSP-WHEN-LEN-GREATER))
                     (2 1 (:REWRITE DEFAULT-+-2))
                     (2 1 (:REWRITE DARGP-WHEN-NATP-CHEAP))
                     (2 1 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP))
                     (1 1 (:TYPE-PRESCRIPTION NATP))
                     (1 1 (:TYPE-PRESCRIPTION MYQUOTEP))
                     (1 1 (:REWRITE USE-ALL-DARGP-2))
                     (1 1 (:REWRITE USE-ALL-DARGP))
                     (1 1
                        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
                     (1 1
                        (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
                     (1 1
                        (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
                     (1 1 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
                     (1 1
                        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
                     (1 1 (:REWRITE EQUAL-OF-LEN-AND-0))
                     (1 1 (:REWRITE DEFAULT-CAR))
                     (1 1 (:REWRITE DEFAULT-+-1))
                     (1 1
                        (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
                     (1 1 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
                     (1 1 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
                     (1 1
                        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN)))
(NATP-WHEN-DARGP (44 2 (:REWRITE CONSP-FROM-LEN-CHEAP))
                 (18 18 (:TYPE-PRESCRIPTION LEN))
                 (13 1
                     (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
                 (11 3
                     (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
                 (10 1 (:REWRITE LEN-OF-CDR))
                 (5 5 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
                 (5 5 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
                 (5 3 (:REWRITE DEFAULT-<-2))
                 (3 3 (:REWRITE DEFAULT-CDR))
                 (3 3 (:REWRITE DEFAULT-<-1))
                 (3 3 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
                 (2 2 (:REWRITE USE-ALL-CONSP-2))
                 (2 2 (:REWRITE USE-ALL-CONSP))
                 (2 2 (:REWRITE CONSP-WHEN-LEN-GREATER))
                 (2 1 (:REWRITE DEFAULT-+-2))
                 (2 1 (:REWRITE DARGP-WHEN-NATP-CHEAP))
                 (2 1 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP))
                 (1 1 (:TYPE-PRESCRIPTION MYQUOTEP))
                 (1 1 (:REWRITE USE-ALL-DARGP-2))
                 (1 1 (:REWRITE USE-ALL-DARGP))
                 (1 1
                    (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
                 (1 1
                    (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
                 (1 1
                    (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
                 (1 1 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
                 (1 1
                    (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
                 (1 1 (:REWRITE EQUAL-OF-LEN-AND-0))
                 (1 1 (:REWRITE DEFAULT-CAR))
                 (1 1 (:REWRITE DEFAULT-+-1))
                 (1 1
                    (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
                 (1 1 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
                 (1 1 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
                 (1 1
                    (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN)))
(QUOTEP-WHEN-DARGP (44 2 (:REWRITE CONSP-FROM-LEN-CHEAP))
                   (18 18 (:TYPE-PRESCRIPTION LEN))
                   (13 1
                       (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
                   (11 3
                       (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
                   (10 1 (:REWRITE LEN-OF-CDR))
                   (5 5 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
                   (5 5 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
                   (5 3 (:REWRITE DEFAULT-<-2))
                   (3 3 (:REWRITE DEFAULT-CDR))
                   (3 3 (:REWRITE DEFAULT-<-1))
                   (3 3 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
                   (2 2 (:REWRITE USE-ALL-CONSP-2))
                   (2 2 (:REWRITE USE-ALL-CONSP))
                   (2 2 (:REWRITE DEFAULT-CAR))
                   (2 2 (:REWRITE CONSP-WHEN-LEN-GREATER))
                   (2 1 (:REWRITE DEFAULT-+-2))
                   (2 1 (:REWRITE DARGP-WHEN-NATP-CHEAP))
                   (2 1 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP))
                   (1 1 (:TYPE-PRESCRIPTION NATP))
                   (1 1 (:TYPE-PRESCRIPTION MYQUOTEP))
                   (1 1 (:REWRITE USE-ALL-DARGP-2))
                   (1 1 (:REWRITE USE-ALL-DARGP))
                   (1 1
                      (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
                   (1 1
                      (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
                   (1 1
                      (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
                   (1 1 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
                   (1 1
                      (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
                   (1 1 (:REWRITE EQUAL-OF-LEN-AND-0))
                   (1 1 (:REWRITE DEFAULT-+-1))
                   (1 1
                      (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
                   (1 1 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
                   (1 1 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
                   (1 1
                      (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN)))
(<=-OF-0-WHEN-DARGP (2 1 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP))
                    (1 1 (:TYPE-PRESCRIPTION MYQUOTEP))
                    (1 1 (:REWRITE USE-ALL-DARGP-2))
                    (1 1 (:REWRITE USE-ALL-DARGP))
                    (1 1
                       (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
                    (1 1
                       (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
                    (1 1 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
                    (1 1 (:REWRITE DARGP-WHEN-NATP-CHEAP))
                    (1 1
                       (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
                    (1 1 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
                    (1 1 (:REWRITE DARGP-WHEN-CONSP-CHEAP)))
(<-WHEN-DARGP-LESS-THAN)
(LEN-WHEN-DARGP (44 2 (:REWRITE CONSP-FROM-LEN-CHEAP))
                (26 26 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
                (26 2 (:LINEAR LEN-OF-CDR-LINEAR-STRONG))
                (22 2 (:LINEAR LEN-OF-CDR-LINEAR))
                (13 1
                    (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
                (11 10 (:REWRITE DEFAULT-+-2))
                (10 10 (:REWRITE DEFAULT-+-1))
                (8 8 (:REWRITE DEFAULT-CDR))
                (5 3 (:REWRITE DEFAULT-<-2))
                (3 3 (:REWRITE DEFAULT-<-1))
                (3 3 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
                (2 2 (:REWRITE USE-ALL-CONSP-2))
                (2 2 (:REWRITE USE-ALL-CONSP))
                (2 2 (:REWRITE CONSP-WHEN-LEN-GREATER))
                (2 1 (:REWRITE DARGP-WHEN-NATP-CHEAP))
                (2 1 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP))
                (1 1 (:TYPE-PRESCRIPTION NATP))
                (1 1 (:TYPE-PRESCRIPTION MYQUOTEP))
                (1 1 (:REWRITE USE-ALL-DARGP-2))
                (1 1 (:REWRITE USE-ALL-DARGP))
                (1 1
                   (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
                (1 1
                   (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
                (1 1
                   (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
                (1 1 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
                (1 1
                   (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
                (1 1 (:REWRITE EQUAL-OF-LEN-AND-0))
                (1 1 (:REWRITE DEFAULT-CAR))
                (1 1
                   (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
                (1 1 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
                (1 1 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
                (1 1
                   (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN)))
(<-OF--1-WHEN-DARGP)
(<-OF--0-WHEN-DARGP)
(INTEGERP-OF-IF)
(SYMBOL-LISTP-OF-CONS (24 2 (:REWRITE CONSP-FROM-LEN-CHEAP))
                      (10 10 (:TYPE-PRESCRIPTION LEN))
                      (5 5
                         (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
                      (4 2
                         (:REWRITE SYMBOLP-OF-CAR-WHEN-AXE-TREEP-CHEAP))
                      (4 2 (:REWRITE DEFAULT-<-2))
                      (3 3 (:REWRITE DEFAULT-CDR))
                      (3 3 (:REWRITE DEFAULT-CAR))
                      (2 2 (:TYPE-PRESCRIPTION AXE-TREEP))
                      (2 2 (:REWRITE USE-ALL-CONSP-2))
                      (2 2 (:REWRITE USE-ALL-CONSP))
                      (2 2
                         (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
                      (2 2 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
                      (2 2 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
                      (2 2 (:REWRITE DEFAULT-<-1))
                      (2 2 (:REWRITE CONSP-WHEN-LEN-GREATER))
                      (2 2 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
                      (2 2
                         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP)))
(SYMBOL-ALISTP-WHEN-ALISTP-AND-EQUAL-OF-STRIP-CARS
     (406 32 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (76 34 (:REWRITE DEFAULT-CAR))
     (58 31
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (56 28 (:REWRITE DEFAULT-<-2))
     (46 10 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (43 37 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (37 37 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (30 15
         (:REWRITE LEN-WHEN-AXE-TREEP-AND-CONSP-OF-CAR))
     (29 29
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (29 29 (:REWRITE USE-ALL-CONSP-2))
     (29 29 (:REWRITE USE-ALL-CONSP))
     (29 29 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (28 28 (:REWRITE DEFAULT-<-1))
     (28 28 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
     (28 11
         (:REWRITE SYMBOLP-OF-CAR-WHEN-AXE-TREEP-CHEAP))
     (26 26 (:TYPE-PRESCRIPTION AXE-TREEP))
     (24 12
         (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (20 10
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (18 18 (:TYPE-PRESCRIPTION ALL-CONSP))
     (15 15 (:REWRITE DEFAULT-CDR))
     (14 7
         (:REWRITE SYMBOL-ALISTP-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (11 11
         (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (11 11
         (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
     (9 9
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (9 9 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP)))
(<-TRANS)
(EQUAL-OF-QUOTE-AND-NTH-1-OF-ASSOC-EQUAL-WHEN-ALL-DARGP-OF-STRIP-CDRS
     (1674 63 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (1065 225 (:REWRITE DEFAULT-CDR))
     (1010 249 (:REWRITE DEFAULT-CAR))
     (865 395
          (:REWRITE LEN-WHEN-AXE-TREEP-AND-CONSP-OF-CAR))
     (665 650 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (636 636 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (578 289
          (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (546 43
          (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (404 214 (:REWRITE <-TRANS))
     (395 395 (:TYPE-PRESCRIPTION AXE-TREEP))
     (342 342
          (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (310 171 (:REWRITE DEFAULT-<-2))
     (249 135 (:REWRITE DEFAULT-+-2))
     (200 200 (:REWRITE USE-ALL-CONSP-2))
     (200 200 (:REWRITE USE-ALL-CONSP))
     (200 200 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (171 171 (:REWRITE DEFAULT-<-1))
     (171 171 (:REWRITE <-WHEN-BOUNDED-AXE-TREEP))
     (140 41 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (135 135 (:REWRITE DEFAULT-+-1))
     (127 15 (:REWRITE USE-ALL-DARGP-FOR-CAR))
     (127 15 (:REWRITE ALL-DARGP-OF-CDR))
     (126 63
          (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (102 5 (:DEFINITION TRUE-LISTP))
     (83 63
         (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (82 41
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (63 63 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (63 63
         (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
     (57 57 (:TYPE-PRESCRIPTION ALL-CONSP))
     (57 57
         (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (50 20
         (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
     (40 20 (:REWRITE DARGP-WHEN-NATP-CHEAP))
     (40 20 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP))
     (24 12
         (:REWRITE IFF-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (22 20 (:REWRITE USE-ALL-DARGP))
     (21 21 (:REWRITE NTH-WHEN-ZP-CHEAP))
     (21 21 (:REWRITE NTH-WHEN-NOT-CONSP-CHEAP))
     (21 21
         (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (21 21
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (21 21 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (20 20 (:TYPE-PRESCRIPTION NATP))
     (20 20 (:TYPE-PRESCRIPTION MYQUOTEP))
     (20 20 (:REWRITE USE-ALL-DARGP-2))
     (20 20
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (20 20
         (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
     (20 20
         (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
     (20 20 (:REWRITE MYQUOTEP-WHEN-AXE-TREEP))
     (20 20
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (20 20
         (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (20 20 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
     (15 15
         (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (15 15
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (2 2 (:TYPE-PRESCRIPTION MEMBERP)))
