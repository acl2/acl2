...
(defun repeatedly-run (steps-done step-limit step-increment dag rule-alist pruning-rule-alist assumptions 64-bitp rules-to-monitor use-internal-contextsp prune-precise normalize-xors count-hits print print-base untranslatep memoizep state)
      ...
      (if run-completedp
        ...
        (b* ((steps-done (+ steps-for-this-iteration steps-done))
          ...
          (repeatedly-run steps-done step-limit
                          step-increment
                          dag rule-alist pruning-rule-alist assumptions 64-bitp rules-to-monitor use-internal-contextsp prune-precise normalize-xors count-hits print print-base untranslatep memoizep
                          state))))))
...
(defun unroll-x86-code-core (target
  ...
  (b* ((- (cw "(Lifting ~s0.~%" target)) ;todo: print the executable name, also handle non-string targets better
       ...
       ((mv erp result-dag-or-quotep state)
        (repeatedly-run 0 step-limit step-increment dag-to-simulate lifter-rule-alist pruning-rule-alist assumptions 64-bitp rules-to-monitor use-internal-contextsp prune-precise normalize-xors count-hits print print-base untranslatep memoizep state))
       ((when erp) (mv erp nil nil nil nil nil state))
  ...
  (b* (;; Check whether this call to the lifter has already been made:
       ...
       ((mv erp result-dag assumptions assumption-vars lifter-rules-used assumption-rules-used state)
        (unroll-x86-code-core target parsed-executable
          ...
          inputs output use-internal-contextsp prune-precise extra-rules remove-rules extra-assumption-rules remove-assumption-rules
