...
__m128i por_sse_register(__m128i x, __m128i y) {
    __asm__ volatile (
        "por %1, %0"          // destination &= source_register
...
bool test_por_8bit(__m128i x, __m128i y) {
    uint8_t x_vals[16], y_vals[16], result_vals[16];

    __m128i result = por_sse_register(x, y);
...
//**************************************************************************
 // POR xmm1, m128 8bit            (memory source form)


 __m128i por_sse_memory(__m128i x, const __m128i* val) {
    __asm__ volatile (
        "por %1, %0"          // destination &= source_memory
        : "+x"(x)              // destination: input/output XMM register
...
bool test_por_memory_8bit(__m128i x, __m128i y) {
    uint8_t x_vals[16], y_vals[16], result_vals[16];

     static __m128i y_aligned __attribute__((aligned(16)));
    y_aligned = y;                           

    __m128i result = por_sse_memory(x, &y_aligned);
