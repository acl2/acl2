; Proofs about a 1-instruction binary that shifts a register (64-bit) right by CL (logical)
;
...
;; Lifts the functionality of shr_rax_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shr_rax_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function shr_rax_cl, which
;; represents the effect of the program on the x86 state.
;; SHR RAX, CL is encoded as 48 D3 E8 (3 bytes), so stop PC = #x401003.
(def-unrolled shr_rax_cl
  :executable "shr_rax_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
;; logical right shift).
(defthm shr_rax_cl-rax
  (equal (rax (shr_rax_cl x86))
         (bvshr 64 (rax x86) (bvchop 6 (cl x86))))
  :hints (("Goal" :in-theory (enable bvshr))))

;; The RIP is advanced by 3 (SHR RAX, CL is encoded as 48 D3 E8 (3 bytes), so stop PC = #x401003.)
(defthm shr_rax_cl-rip
  (equal (rip (shr_rax_cl x86))
         (+ 3 #x401000)))

;; Registers other than RAX are unchanged.
(defthm shr_rax_cl-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (shr_rax_cl x86))
                  (rgfi reg x86)))
...
(defthm shr_rax_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 6 (cl x86)) 0)
           (equal (get-flag flag (shr_rax_cl x86))
                  (get-flag flag x86))))
...
;; 1-bit shifts).
(defthm shr_rax_cl-of
  (implies (equal (bvchop 6 (cl x86)) 1)
           (equal (get-flag :of (shr_rax_cl x86))
                  (getbit 63 (rax x86)))))

;; For any nonzero masked count, the zero flag is 1 iff the result is 0.
(defthm shr_rax_cl-zf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :zf (shr_rax_cl x86))
                  (if (equal 0 (bvshr 64 (rax x86) (bvchop 6 (cl x86))))
                      1
                    0)))
...
(defthm shr_rax_cl-sf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :sf (shr_rax_cl x86))
                  (getbit 63 (bvshr 64 (rax x86) (bvchop 6 (cl x86))))))
  :hints (("Goal" :in-theory (enable bvshr))))
...
;; of 1s.
(defthm shr_rax_cl-pf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :pf (shr_rax_cl x86))
                  (if (evenp (bvcount 8 (bvshr 64 (rax x86) (bvchop 6 (cl x86)))))
                      1
                    0)))
  :hints (("Goal" :in-theory (enable bvshr pf-spec64-alt-def))))

;; All memory addresses are unchanged.
(defthm shr_rax_cl-memory-unchanged
  (equal (memi address (shr_rax_cl x86))
         (memi address x86)))
