; Proofs about a 1-instruction binary that shifts a register (8-bit) right by CL (arithmetic)
;
...
;; Lifts the functionality of sar_al_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sar_al_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sar_al_cl, which
;; represents the effect of the program on the x86 state.
;; SAR AL, CL is encoded as D2 F8 (2 bytes), so stop PC = #x401002.
(def-unrolled sar_al_cl
  :executable "sar_al_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401002))
...
;; AL after the operation: DEST is shifted right by the masked count in CL,
...
(defthm sar_al_cl-al
  (implies (bvlt 5 (bvchop 5 (cl x86)) 8)
           (equal (al (sar_al_cl x86))
                  (bvashr 8 (al x86) (bvchop 5 (cl x86)))))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))

;; The RIP is advanced by 2 (SAR AL, CL is encoded as D2 F8 (2 bytes), so stop PC = #x401002.)
(defthm sar_al_cl-rip
  (equal (rip (sar_al_cl x86))
         (+ 2 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sar_al_cl-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sar_al_cl x86))
                  (rgfi reg x86)))
...
(defthm sar_al_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (sar_al_cl x86))
                  (get-flag flag x86))))
...
(defthm sar_al_cl-cf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
           ...
           (equal (get-flag :cf (sar_al_cl x86))
                  (getbit (+ (bvchop 5 (cl x86)) -1) (al x86)))))
...
(defthm sar_al_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (sar_al_cl x86))
                  0)))
...
(defthm sar_al_cl-zf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
                (bvlt 5 (bvchop 5 (cl x86)) 8))
           (equal (get-flag :zf (sar_al_cl x86))
                  (if (equal 0 (bvashr 8 (al x86) (bvchop 5 (cl x86))))
                      1
                    0)))
...
(defthm sar_al_cl-sf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
                (bvlt 5 (bvchop 5 (cl x86)) 8))
           (equal (get-flag :sf (sar_al_cl x86))
                  (getbit 7 (bvashr 8 (al x86) (bvchop 5 (cl x86))))))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))
...
(defthm sar_al_cl-pf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
                (bvlt 5 (bvchop 5 (cl x86)) 8))
           (equal (get-flag :pf (sar_al_cl x86))
                  (if (evenp (bvcount 8 (bvashr 8 (al x86) (bvchop 5 (cl x86)))))
                      1
                    0)))
...
(defthm sar_al_cl-memory-unchanged
  (equal (memi address (sar_al_cl x86))
         (memi address x86)))
