; Proofs about a 1-instruction binary that compares AX to a memory word (16-bit)
;
...
;; Lifts the functionality of cmp_ax_mem16.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "cmp_ax_mem16.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function cmp_ax_mem16, which
;; represents the effect of the program on the x86 state.
;; CMP AX, [RBX] is encoded as 66 3B 03 (3 bytes), so stop PC = 0x401003.
;; Both the base address and the adjacent byte must be canonical for the 16-bit read.
(def-unrolled cmp_ax_mem16
  :executable "cmp_ax_mem16.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; The RIP is advanced by 3 (CMP AX, [RBX] is 3 bytes: 66 3B 03)
(defthm cmp_ax_mem16-rip
  (equal (rip (cmp_ax_mem16 x86))
         (+ 3 #x401000)))

;; CMP computes DEST - SRC only to set flags; DEST is not updated, so all
;; registers are unchanged (Intel SDM Vol 2A: CMP entry).
(defthm cmp_ax_mem16-registers
  (equal (rgfi reg (cmp_ax_mem16 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff mem[RBX] > AX unsigned (borrow):
(defthm cmp_ax_mem16-cf
  (equal (get-flag :cf (cmp_ax_mem16 x86))
         (if (bvlt 16 (ax x86) (read 2 (rbx x86) x86)) 1 0)))

;; The zero flag is 1 iff the (uncommitted) difference is zero:
(defthm cmp_ax_mem16-zf
  (equal (get-flag :zf (cmp_ax_mem16 x86))
...
(defthm cmp_ax_mem16-sf
  (equal (get-flag :sf (cmp_ax_mem16 x86))
...
(defthm cmp_ax_mem16-af
  (equal (get-flag :af (cmp_ax_mem16 x86))
         (if (< (bvchop 4 (ax x86))
...
(defthm cmp_ax_mem16-of
  (equal (get-flag :of (cmp_ax_mem16 x86))
...
(defthm cmp_ax_mem16-pf
  (equal (get-flag :pf (cmp_ax_mem16 x86))
...
(defthm cmp_ax_mem16-memory-unchanged
  (equal (memi address (cmp_ax_mem16 x86))
         (memi address x86)))

(defthm cmp_ax_mem16-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (cmp_ax_mem16 x86))
                  (get-flag flag x86)))
