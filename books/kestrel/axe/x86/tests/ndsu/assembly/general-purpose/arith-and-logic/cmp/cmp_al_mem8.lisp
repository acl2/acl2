; Proofs about a 1-instruction binary that compares AL to a memory byte (8-bit)
;
...
;; Lifts the functionality of cmp_al_mem8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "cmp_al_mem8.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function cmp_al_mem8, which
;; represents the effect of the program on the x86 state.
;; CMP AL, [RBX] is encoded as 3A 03 (2 bytes), so stop PC = 0x401002.
...
(def-unrolled cmp_al_mem8
  :executable "cmp_al_mem8.elf64"
  :target #x401000
  :stop-pcs '(#x401002)
...
;; The RIP is advanced by 2 (CMP AL, [RBX] is 2 bytes: 3A 03)
(defthm cmp_al_mem8-rip
  (equal (rip (cmp_al_mem8 x86))
         (+ 2 #x401000)))

;; CMP computes DEST - SRC only to set flags; DEST is not updated, so all
;; registers are unchanged (Intel SDM Vol 2A: CMP entry).
(defthm cmp_al_mem8-registers
  (equal (rgfi reg (cmp_al_mem8 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff mem[RBX] > AL unsigned (borrow):
(defthm cmp_al_mem8-cf
  (equal (get-flag :cf (cmp_al_mem8 x86))
         (if (bvlt 8 (al x86) (read 1 (rbx x86) x86)) 1 0)))

;; The zero flag is 1 iff the (uncommitted) difference is zero:
(defthm cmp_al_mem8-zf
  (equal (get-flag :zf (cmp_al_mem8 x86))
...
(defthm cmp_al_mem8-sf
  (equal (get-flag :sf (cmp_al_mem8 x86))
...
(defthm cmp_al_mem8-af
  (equal (get-flag :af (cmp_al_mem8 x86))
         (if (< (bvchop 4 (al x86))
...
(defthm cmp_al_mem8-of
  (equal (get-flag :of (cmp_al_mem8 x86))
...
(defthm cmp_al_mem8-pf
  (equal (get-flag :pf (cmp_al_mem8 x86))
...
(defthm cmp_al_mem8-memory-unchanged
  (equal (memi address (cmp_al_mem8 x86))
         (memi address x86)))

(defthm cmp_al_mem8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (cmp_al_mem8 x86))
                  (get-flag flag x86)))
