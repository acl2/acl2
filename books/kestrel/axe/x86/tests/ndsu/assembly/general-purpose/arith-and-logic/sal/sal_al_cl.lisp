...
;; Lifts the functionality of sal_al_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_al_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_al_cl, which
;; represents the effect of the program on the x86 state.
;; SAL AL, CL is encoded as D2 E0 (2 bytes), so stop PC = 0x401002.
(def-unrolled sal_al_cl
  :executable "sal_al_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401002))
...
(defthm sal_al_cl-al
  (equal (al (sal_al_cl x86))
         (bvshl 8 (al x86) (bvchop 5 (cl x86))))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 2 (SAL AL, CL is encoded as D2 E0 (2 bytes), so stop PC = 0x401002.)
(defthm sal_al_cl-rip
  (equal (rip (sal_al_cl x86))
         (+ 2 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sal_al_cl-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sal_al_cl x86))
                  (rgfi reg x86)))
...
(defthm sal_al_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (sal_al_cl x86))
                  (get-flag flag x86))))
...
(defthm sal_al_cl-cf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
           ...
           (equal (get-flag :cf (sal_al_cl x86))
...
(defthm sal_al_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (sal_al_cl x86))
...
(defthm sal_al_cl-zf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :zf (sal_al_cl x86))
...
(defthm sal_al_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (sal_al_cl x86))
...
(defthm sal_al_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (sal_al_cl x86))
...
(defthm sal_al_cl-memory-unchanged
  (equal (memi address (sal_al_cl x86))
         (memi address x86)))
