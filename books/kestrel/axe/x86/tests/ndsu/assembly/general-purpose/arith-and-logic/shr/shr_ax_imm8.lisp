; Proofs about a 1-instruction binary that shifts a register (16-bit) right by 3 (logical)
;
...
;; Lifts the functionality of shr_ax_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shr_ax_imm8.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function shr_ax_imm8, which
;; represents the effect of the program on the x86 state.
;; SHR AX, 3 is encoded as 66 C1 E8 03 (4 bytes), so stop PC = #x401004.
(def-unrolled shr_ax_imm8
  :executable "shr_ax_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401004))
...
(defthm shr_ax_imm8-rax
  (equal (rax (shr_ax_imm8 x86))
         (bvcat 48 (slice 63 16 (rax x86)) 16 (bvshr 16 (ax x86) 3)))
  :hints (("Goal" :in-theory (enable bvshr))))

;; AX after the operation: DEST is shifted right by 3, filling with 0 on
;; the left (Intel SDM Vol 1 3.4.3.1 and Vol 2B SHR entry: logical right shift).
(defthm shr_ax_imm8-ax
  (equal (ax (shr_ax_imm8 x86))
         (bvshr 16 (ax x86) 3))
  :hints (("Goal" :in-theory (enable bvshr))))

;; The RIP is advanced by 4 (SHR AX, 3 is encoded as 66 C1 E8 03 (4 bytes), so stop PC = #x401004.)
(defthm shr_ax_imm8-rip
  (equal (rip (shr_ax_imm8 x86))
         (+ 4 #x401000)))

;; Registers other than RAX are unchanged.
(defthm shr_ax_imm8-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (shr_ax_imm8 x86))
                  (rgfi reg x86)))
...
(defthm shr_ax_imm8-cf
  (equal (get-flag :cf (shr_ax_imm8 x86))
         (getbit 2 (ax x86))))
...
(defthm shr_ax_imm8-zf
  (equal (get-flag :zf (shr_ax_imm8 x86))
         (if (equal 0 (bvshr 16 (ax x86) 3))
             1
           0))
  :hints (("Goal" :in-theory (enable bvshr zf-spec))))

;; The sign flag is the sign bit (bit 15) of the 16-bit result.
(defthm shr_ax_imm8-sf
  (equal (get-flag :sf (shr_ax_imm8 x86))
         (getbit 15 (bvshr 16 (ax x86) 3)))
  :hints (("Goal" :in-theory (enable bvshr))))
...
(defthm shr_ax_imm8-pf
  (equal (get-flag :pf (shr_ax_imm8 x86))
         (if (evenp (bvcount 8 (bvshr 16 (ax x86) 3)))
             1
           0))
  :hints (("Goal" :in-theory (enable bvshr pf-spec16-alt-def))))

;; All memory addresses are unchanged.
(defthm shr_ax_imm8-memory-unchanged
  (equal (memi address (shr_ax_imm8 x86))
         (memi address x86)))
...
(defthm shr_ax_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (shr_ax_imm8 x86))
                  (get-flag flag x86)))
