...
; writing the result to a 64-bit destination register.
;
...
;; Lifts the functionality of crc32_rax_bl.elf64 into logic using the
;; Axe-based x86 lifter and proves various properties.

;; (depends-on "crc32_rax_bl.elf64")
;; cert_param: (uses-stp)
...
(def-unrolled crc32_rax_bl
  :executable "crc32_rax_bl.elf64"
  :target #x401000
  :stop-pcs '(#x401006)
...
(defthm crc32_rax_bl-rax
  (equal (rax (crc32_rax_bl x86))
...
(defthm crc32_rax_bl-upper-32-bits-zero
  (equal (slice 63 32 (rax (crc32_rax_bl x86)))
         0)
  :hints (("Goal" :use (crc32_rax_bl-rax
                        (:instance unsigned-byte-p-32-of-spec-crc32
                                   (crc (eax x86)) (data (bl x86)) (data-width 8)))
           :in-theory (e/d (slice) (crc32_rax_bl-rax spec-crc32)))))

;; The RIP is advanced by 6 (CRC32 RAX, BL is 6 bytes: F2 48 0F 38 F0 C3)
(defthm crc32_rax_bl-rip
  (equal (rip (crc32_rax_bl x86))
         (+ 6 #x401000)))

;; Registers other than RAX (the destination) are unchanged; in particular,
;; RBX (which holds the source, BL) is unchanged, since CRC32 does not
;; modify its source.
(defthm crc32_rax_bl-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (crc32_rax_bl x86))
                  (rgfi reg x86)))
  :hints (("Goal" :in-theory (enable set-rax))))

;; No flags are affected by CRC32 (Intel SDM Vol 2A: CRC32).
(defthm crc32_rax_bl-flags
  (implies (member-equal flag *standard-flags*)
           (equal (get-flag flag (crc32_rax_bl x86))
                  (get-flag flag x86))))

;; All memory addresses are unchanged (CRC32 RAX, BL has no memory operand).
(defthm crc32_rax_bl-memory-unchanged
  (equal (memi address (crc32_rax_bl x86))
         (memi address x86)))
