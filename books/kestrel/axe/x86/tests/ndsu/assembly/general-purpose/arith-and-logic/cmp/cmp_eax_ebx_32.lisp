; Proofs about a 1-instruction binary that compares 2 numbers (32-bit)
;
...
;; Lifts the functionality of cmp_eax_ebx_32.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "cmp_eax_ebx_32.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function cmp_eax_ebx_32, which
;; represents the effect of the program on the x86 state.
;; CMP EAX, EBX is encoded as 39 D8 (2 bytes), so stop PC = 0x401002.
(def-unrolled cmp_eax_ebx_32
  :executable "cmp_eax_ebx_32.elf64"
  :target #x401000
  :stop-pcs '(#x401002))
...
;; The RIP is advanced by 2 (CMP EAX,EBX is 2 bytes: 39 D8)
(defthm cmp_eax_ebx_32-rip
  (equal (rip (cmp_eax_ebx_32 x86))
         (+ 2 #x401000)))

;; CMP computes DEST - SRC only to set flags; DEST is not updated, so all
;; registers are unchanged (Intel SDM Vol 2A: CMP entry).
(defthm cmp_eax_ebx_32-registers
  (equal (rgfi reg (cmp_eax_ebx_32 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff the 32-bit subtraction borrows (unsigned src > unsigned dst):
(defthm cmp_eax_ebx_32-cf
  (equal (get-flag :cf (cmp_eax_ebx_32 x86))
         (if (bvlt 32 (eax x86) (ebx x86)) 1 0)))

;; The zero flag is 1 iff the (uncommitted) difference is zero:
(defthm cmp_eax_ebx_32-zf
  (equal (get-flag :zf (cmp_eax_ebx_32 x86))
...
(defthm cmp_eax_ebx_32-sf
  (equal (get-flag :sf (cmp_eax_ebx_32 x86))
...
(defthm cmp_eax_ebx_32-af
  (equal (get-flag :af (cmp_eax_ebx_32 x86))
         (if (< (bvchop 4 (eax x86))
                (bvchop 4 (ebx x86)))
             1
           0))
...
(defthm cmp_eax_ebx_32-of
  (equal (get-flag :of (cmp_eax_ebx_32 x86))
...
(defthm cmp_eax_ebx_32-pf
  (equal (get-flag :pf (cmp_eax_ebx_32 x86))
...
(defthm cmp_eax_ebx_32-memory-unchanged
  (equal (memi address (cmp_eax_ebx_32 x86))
         (memi address x86)))

(defthm cmp_eax_ebx_32-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (cmp_eax_ebx_32 x86))
                  (get-flag flag x86)))
