...
;; Lifts the functionality of sal_al_1.elf64 into logic using the Axe-based x86
...
;; SAL AL, 1 is encoded as D0 E0 (2 bytes), so stop PC = 0x401002.
(def-unrolled sal_al_1
  :executable "sal_al_1.elf64"
  :target #x401000
  :stop-pcs '(#x401002))
...
(defthm sal_al_1-al
  (equal (al (sal_al_1 x86))
         (bvshl 8 (al x86) 1))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 2 (SAL AL, 1 is encoded as D0 E0 (2 bytes), so stop PC = 0x401002.)
(defthm sal_al_1-rip
  (equal (rip (sal_al_1 x86))
         (+ 2 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sal_al_1-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sal_al_1 x86))
                  (rgfi reg x86)))
...
(defthm sal_al_1-cf
  (equal (get-flag :cf (sal_al_1 x86))
         (getbit 7 (al x86))))
...
(defthm sal_al_1-sf
  (equal (get-flag :sf (sal_al_1 x86))
         (getbit 7 (bvshl 8 (al x86) 1)))
  :hints (("Goal" :in-theory (enable bvshl))))
...
(defthm sal_al_1-pf
  (equal (get-flag :pf (sal_al_1 x86))
...
(defthm sal_al_1-memory-unchanged
  (equal (memi address (sal_al_1 x86))
         (memi address x86)))
