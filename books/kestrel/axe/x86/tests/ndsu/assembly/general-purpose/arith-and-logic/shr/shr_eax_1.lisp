; Proofs about a 1-instruction binary that shifts a register (32-bit) right by 1 (logical)
;
...
;; Lifts the functionality of shr_eax_1.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shr_eax_1.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function shr_eax_1, which
;; represents the effect of the program on the x86 state.
;; SHR EAX, 1 is encoded as D1 E8 (2 bytes), so stop PC = #x401002.
(def-unrolled shr_eax_1
  :executable "shr_eax_1.elf64"
  :target #x401000
  :stop-pcs '(#x401002))
...
(defthm shr_eax_1-rax
  (equal (rax (shr_eax_1 x86))
         (bvshr 32 (eax x86) 1))
  :hints (("Goal" :in-theory (enable bvshr))))

;; EAX after the operation: DEST is shifted right by 1, filling with 0 on
;; the left (Intel SDM Vol 1 3.4.3.1 and Vol 2B SHR entry: logical right shift).
(defthm shr_eax_1-eax
  (equal (eax (shr_eax_1 x86))
         (bvshr 32 (eax x86) 1))
  :hints (("Goal" :in-theory (enable bvshr))))

;; The RIP is advanced by 2 (SHR EAX, 1 is encoded as D1 E8 (2 bytes), so stop PC = #x401002.)
(defthm shr_eax_1-rip
  (equal (rip (shr_eax_1 x86))
         (+ 2 #x401000)))

;; Registers other than RAX are unchanged.
(defthm shr_eax_1-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (shr_eax_1 x86))
                  (rgfi reg x86)))
...
(defthm shr_eax_1-cf
  (equal (get-flag :cf (shr_eax_1 x86))
         (getbit 0 (eax x86))))
...
(defthm shr_eax_1-of
  (equal (get-flag :of (shr_eax_1 x86))
         (getbit 31 (eax x86))))

;; The zero flag is 1 iff the result is 0.
(defthm shr_eax_1-zf
  (equal (get-flag :zf (shr_eax_1 x86))
         (if (equal 0 (bvshr 32 (eax x86) 1))
             1
           0))
...
(defthm shr_eax_1-sf
  (equal (get-flag :sf (shr_eax_1 x86))
         (getbit 31 (bvshr 32 (eax x86) 1)))
  :hints (("Goal" :in-theory (enable bvshr))))
...
(defthm shr_eax_1-pf
  (equal (get-flag :pf (shr_eax_1 x86))
         (if (evenp (bvcount 8 (bvshr 32 (eax x86) 1)))
             1
           0))
  :hints (("Goal" :in-theory (enable bvshr pf-spec32-alt-def))))

;; All memory addresses are unchanged.
(defthm shr_eax_1-memory-unchanged
  (equal (memi address (shr_eax_1 x86))
         (memi address x86)))
...
(defthm shr_eax_1-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (shr_eax_1 x86))
                  (get-flag flag x86)))
