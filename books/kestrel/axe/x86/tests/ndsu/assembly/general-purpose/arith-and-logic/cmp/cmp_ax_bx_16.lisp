; Proofs about a 1-instruction binary that compares 2 numbers (16-bit)
;
...
;; Lifts the functionality of cmp_ax_bx_16.elf64 into logic using the Axe-based x86
...
;; CMP AX, BX is encoded as 66 39 D8 (3 bytes), so stop PC = 0x401003.
(def-unrolled cmp_ax_bx_16
  :executable "cmp_ax_bx_16.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
(defthm cmp_ax_bx_16-sf
  (equal (get-flag :sf (cmp_ax_bx_16 x86))
...
(defthm cmp_ax_bx_16-af
  (equal (get-flag :af (cmp_ax_bx_16 x86))
         (if (< (bvchop 4 (ax x86))
                (bvchop 4 (bx x86)))
             1
           0))
...
(defthm cmp_ax_bx_16-of
  (equal (get-flag :of (cmp_ax_bx_16 x86))
...
(defthm cmp_ax_bx_16-pf
  (equal (get-flag :pf (cmp_ax_bx_16 x86))
...
(defthm cmp_ax_bx_16-memory-unchanged
  (equal (memi address (cmp_ax_bx_16 x86))
         (memi address x86)))

(defthm cmp_ax_bx_16-other-flags
