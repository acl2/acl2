; Proofs about a 1-instruction binary that shifts a memory qword left by CL
;
...
;; Lifts the functionality of shl_mem64_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shl_mem64_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function shl_mem64_cl, which
;; represents the effect of the program on the x86 state.
;; SHL qword [RBX], CL is encoded as 48 D3 23 (3 bytes), so stop PC = 0x401003.
...
(def-unrolled shl_mem64_cl
  :executable "shl_mem64_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; qword shifted left by the masked count in CL, filling with 0 on the
;; right (Intel SDM Vol 1 3.4.3.1 and Vol 2B SHL/SAL entry: logical left
;; shift, size = qword).
(defthm shl_mem64_cl-memory-at-rbx
  (equal (read 8 (rbx x86) (shl_mem64_cl x86))
         (bvshl 64 (read 8 (rbx x86) x86) (bvchop 6 (cl x86))))
...
(defthm shl_mem64_cl-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 8))
           (equal (read 1 address (shl_mem64_cl x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SHL qword [RBX], CL is encoded as 48 D3 23 (3 bytes), so stop PC = 0x401003.)
(defthm shl_mem64_cl-rip
  (equal (rip (shl_mem64_cl x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm shl_mem64_cl-registers
  (equal (rgfi reg (shl_mem64_cl x86))
         (rgfi reg x86)))
...
(defthm shl_mem64_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 6 (cl x86)) 0)
           (equal (get-flag flag (shl_mem64_cl x86))
                  (get-flag flag x86))))
...
(defthm shl_mem64_cl-cf
  (implies (and (not (equal (bvchop 6 (cl x86)) 0))
                (bvlt 6 (bvchop 6 (cl x86)) 64))
           (equal (get-flag :cf (shl_mem64_cl x86))
                  (getbit (+ 64 (- (bvchop 6 (cl x86)))) (read 8 (rbx x86) x86)))))
...
(defthm shl_mem64_cl-of
  (implies (equal (bvchop 6 (cl x86)) 1)
           (equal (get-flag :of (shl_mem64_cl x86))
...
(defthm shl_mem64_cl-zf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :zf (shl_mem64_cl x86))
                  (if (equal 0 (bvshl 64 (read 8 (rbx x86) x86) (bvchop 6 (cl x86))))
                      1
                    0)))
...
(defthm shl_mem64_cl-sf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :sf (shl_mem64_cl x86))
                  (getbit 63 (bvshl 64 (read 8 (rbx x86) x86) (bvchop 6 (cl x86))))))
  :hints (("Goal" :in-theory (enable bvshl))))
...
;; of 1s.
(defthm shl_mem64_cl-pf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :pf (shl_mem64_cl x86))
                  (if (evenp (bvcount 8 (bvshl 64 (read 8 (rbx x86) x86) (bvchop 6 (cl x86)))))
                      1
                    0)))
...
(defthm shl_mem64_cl-other-flags
