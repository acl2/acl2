...
;; Lifts the functionality of sal_mem32_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_mem32_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_mem32_cl, which
;; represents the effect of the program on the x86 state.
;; SAL dword [RBX], CL is encoded as D3 23 (2 bytes), so stop PC = 0x401002.
...
(def-unrolled sal_mem32_cl
  :executable "sal_mem32_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401002)
...
;; shift, size = dword).
(defthm sal_mem32_cl-memory-at-rbx
  (equal (read 4 (rbx x86) (sal_mem32_cl x86))
...
(defthm sal_mem32_cl-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 4))
           (equal (read 1 address (sal_mem32_cl x86))
                  (read 1 address x86))))

;; The RIP is advanced by 2 (SAL dword [RBX], CL is encoded as D3 23 (2 bytes), so stop PC = 0x401002.)
(defthm sal_mem32_cl-rip
  (equal (rip (sal_mem32_cl x86))
         (+ 2 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sal_mem32_cl-registers
  (equal (rgfi reg (sal_mem32_cl x86))
         (rgfi reg x86)))
...
(defthm sal_mem32_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (sal_mem32_cl x86))
                  (get-flag flag x86))))
...
(defthm sal_mem32_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (sal_mem32_cl x86))
...
(defthm sal_mem32_cl-zf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :zf (sal_mem32_cl x86))
...
(defthm sal_mem32_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (sal_mem32_cl x86))
...
;; of 1s.
(defthm sal_mem32_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (sal_mem32_cl x86))
...
(defthm sal_mem32_cl-other-flags
