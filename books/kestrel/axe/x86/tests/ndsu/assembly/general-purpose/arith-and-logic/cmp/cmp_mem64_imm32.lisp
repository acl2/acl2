; Proofs about a 1-instruction binary that compares a memory qword to a sign-extended 32-bit immediate
;
...
;; Lifts the functionality of cmp_mem64_imm32.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "cmp_mem64_imm32.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function cmp_mem64_imm32, which
;; represents the effect of the program on the x86 state.
;; CMP [RBX], 1000 is encoded as 48 81 3B E8 03 00 00 (7 bytes), so stop PC = 0x401007.
...
(def-unrolled cmp_mem64_imm32
  :executable "cmp_mem64_imm32.elf64"
  :target #x401000
  :stop-pcs '(#x401007)
...
;; The RIP is advanced by 7 (CMP [RBX], imm32 is 7 bytes: REX.W 81 3B E8 03 00 00)
(defthm cmp_mem64_imm32-rip
  (equal (rip (cmp_mem64_imm32 x86))
         (+ 7 #x401000)))

;; CMP computes DEST - SRC only to set flags; DEST (memory) is not updated, so
;; all memory is unchanged (Intel SDM Vol 2A: CMP entry).
(defthm cmp_mem64_imm32-memory-unchanged
  (equal (memi address (cmp_mem64_imm32 x86))
         (memi address x86)))

;; All registers are unchanged (CMP does not write to any register either).
(defthm cmp_mem64_imm32-registers
  (equal (rgfi reg (cmp_mem64_imm32 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff 1000 > mem[RBX] unsigned (borrow):
(defthm cmp_mem64_imm32-cf
  (equal (get-flag :cf (cmp_mem64_imm32 x86))
         (if (bvlt 64 (read 8 (rbx x86) x86) 1000) 1 0)))

;; The zero flag is 1 iff the (uncommitted) difference is zero:
(defthm cmp_mem64_imm32-zf
  (equal (get-flag :zf (cmp_mem64_imm32 x86))
...
(defthm cmp_mem64_imm32-sf
  (equal (get-flag :sf (cmp_mem64_imm32 x86))
...
(defthm cmp_mem64_imm32-af
  (equal (get-flag :af (cmp_mem64_imm32 x86))
...
(defthm cmp_mem64_imm32-of
  (equal (get-flag :of (cmp_mem64_imm32 x86))
...
(defthm cmp_mem64_imm32-pf
  (equal (get-flag :pf (cmp_mem64_imm32 x86))
...
(defthm cmp_mem64_imm32-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (cmp_mem64_imm32 x86))
                  (get-flag flag x86)))
