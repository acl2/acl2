...
;; Lifts the functionality of sal_ax_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_ax_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_ax_cl, which
;; represents the effect of the program on the x86 state.
;; SAL AX, CL is encoded as 66 D3 E0 (3 bytes), so stop PC = 0x401003.
(def-unrolled sal_ax_cl
  :executable "sal_ax_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
(defthm sal_ax_cl-rax
  (equal (rax (sal_ax_cl x86))
...
(defthm sal_ax_cl-ax
  (equal (ax (sal_ax_cl x86))
         (bvshl 16 (ax x86) (bvchop 5 (cl x86))))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 3 (SAL AX, CL is encoded as 66 D3 E0 (3 bytes), so stop PC = 0x401003.)
(defthm sal_ax_cl-rip
  (equal (rip (sal_ax_cl x86))
         (+ 3 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sal_ax_cl-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sal_ax_cl x86))
                  (rgfi reg x86)))
...
(defthm sal_ax_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (sal_ax_cl x86))
                  (get-flag flag x86))))
...
(defthm sal_ax_cl-cf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
           ...
           (equal (get-flag :cf (sal_ax_cl x86))
...
(defthm sal_ax_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (sal_ax_cl x86))
...
(defthm sal_ax_cl-zf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :zf (sal_ax_cl x86))
...
(defthm sal_ax_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (sal_ax_cl x86))
...
(defthm sal_ax_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (sal_ax_cl x86))
...
(defthm sal_ax_cl-memory-unchanged
  (equal (memi address (sal_ax_cl x86))
         (memi address x86)))
