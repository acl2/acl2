; Proofs about a 1-instruction binary that shifts a register (8-bit) right by 3 (logical)
;
...
;; Lifts the functionality of shr_al_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shr_al_imm8.elf64")
;; cert_param: (uses-stp)
...
(def-unrolled shr_al_imm8
  :executable "shr_al_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
;; AL after the operation: DEST is shifted right by 3, filling with 0 on
;; the left (Intel SDM Vol 1 3.4.3.1 and Vol 2B SHR entry: logical right shift).
(defthm shr_al_imm8-al
  (equal (al (shr_al_imm8 x86))
         (bvshr 8 (al x86) 3))
  :hints (("Goal" :in-theory (enable bvshr))))

;; The RIP is advanced by 3 (SHR AL, 3 is encoded as C0 E8 03 (3 bytes), so stop PC = #x401003.)
(defthm shr_al_imm8-rip
  (equal (rip (shr_al_imm8 x86))
         (+ 3 #x401000)))

;; Registers other than RAX are unchanged.
(defthm shr_al_imm8-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (shr_al_imm8 x86))
                  (rgfi reg x86)))
...
(defthm shr_al_imm8-cf
  (equal (get-flag :cf (shr_al_imm8 x86))
         (getbit 2 (al x86))))
...
(defthm shr_al_imm8-zf
  (equal (get-flag :zf (shr_al_imm8 x86))
         (if (equal 0 (bvshr 8 (al x86) 3))
             1
           0))
  :hints (("Goal" :in-theory (enable bvshr zf-spec))))

;; The sign flag is the sign bit (bit 7) of the 8-bit result.
(defthm shr_al_imm8-sf
  (equal (get-flag :sf (shr_al_imm8 x86))
         (getbit 7 (bvshr 8 (al x86) 3)))
  :hints (("Goal" :in-theory (enable bvshr))))

;; The parity flag considers only the 8 least significant bits of the result
;; and is 1 iff they contain an even number of 1s.
(defthm shr_al_imm8-pf
  (equal (get-flag :pf (shr_al_imm8 x86))
         (if (evenp (bvcount 8 (bvshr 8 (al x86) 3)))
             1
           0))
...
(defthm shr_al_imm8-memory-unchanged
  (equal (memi address (shr_al_imm8 x86))
         (memi address x86)))
...
(defthm shr_al_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (shr_al_imm8 x86))
                  (get-flag flag x86)))
