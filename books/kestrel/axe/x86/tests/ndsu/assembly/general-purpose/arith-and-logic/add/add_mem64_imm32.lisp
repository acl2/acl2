; Proofs about a 1-instruction binary that adds a sign-extended 32-bit immediate to a memory qword
;
...
;; Lifts the functionality of add_mem64_imm32.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "add_mem64_imm32.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function add_mem64_imm32, which
;; represents the effect of the program on the x86 state.
;; ADD [RBX], 1000 is encoded as 48 81 03 E8 03 00 00 (7 bytes), so stop PC = 0x401007.
...
(def-unrolled add_mem64_imm32
  :executable "add_mem64_imm32.elf64"
  :target #x401000
  :stop-pcs '(#x401007)
...
(defthm add_mem64_imm32-memory-at-rbx
  (equal (read 8 (rbx x86) (add_mem64_imm32 x86))
         (bvplus 64 (read 8 (rbx x86) x86) 1000)))
...
(defthm add_mem64_imm32-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 8))
           (equal (read 1 address (add_mem64_imm32 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 7 (ADD [RBX], imm32 is 7 bytes: REX.W 81 03 E8 03 00 00)
(defthm add_mem64_imm32-rip
  (equal (rip (add_mem64_imm32 x86))
         (+ 7 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm add_mem64_imm32-registers
  (equal (rgfi reg (add_mem64_imm32 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff the 64-bit unsigned sum overflows (i.e., >= 2^64):
(defthm add_mem64_imm32-cf
  (equal (get-flag :cf (add_mem64_imm32 x86))
         (if (<= (expt 2 64)
                 (+ (bvchop 64 (read 8 (rbx x86) x86)) 1000))
             1
           0))
...
(defthm add_mem64_imm32-zf
  (equal (get-flag :zf (add_mem64_imm32 x86))
         (if (equal 0 (bvplus 64 (read 8 (rbx x86) x86) 1000))
             1
           0))
...
(defthm add_mem64_imm32-sf
  (equal (get-flag :sf (add_mem64_imm32 x86))
         (let ((sum (bvplus 64 (read 8 (rbx x86) x86) 1000)))
           (getbit 63 sum)))
...
(defthm add_mem64_imm32-af
  (equal (get-flag :af (add_mem64_imm32 x86))
...
(defthm add_mem64_imm32-of
  (equal (get-flag :of (add_mem64_imm32 x86))
...
(defthm add_mem64_imm32-pf
  (equal (get-flag :pf (add_mem64_imm32 x86))
         (let ((sum (+ (read 8 (rbx x86) x86) 1000)))
           (if (evenp (bvcount 8 sum))
               1
             0)))
  :hints (("Goal" :in-theory (enable pf-spec64-alt-def bvplus))))

(defthm add_mem64_imm32-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (add_mem64_imm32 x86))
                  (get-flag flag x86)))
