; Proofs about a 1-instruction binary that subtracts RAX from a memory qword (64-bit reg-to-mem)
;
...
;; Lifts the functionality of sub_mem64_rax.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sub_mem64_rax.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function sub_mem64_rax, which
;; represents the effect of the program on the x86 state.
;; SUB [RBX], RAX is encoded as 48 29 03 (3 bytes), so stop PC = 0x401003.
;; Both the base address and +7 must be canonical for the 64-bit write.
(def-unrolled sub_mem64_rax
  :executable "sub_mem64_rax.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; qword at [RBX] minus RAX (Intel SDM Vol 2A: DEST <- DEST - SRC, size = qword).
(defthm sub_mem64_rax-memory-at-rbx
  (equal (read 8 (rbx x86) (sub_mem64_rax x86))
         (bvminus 64 (read 8 (rbx x86) x86) (rax x86))))

;; All other memory bytes are unchanged (only the 8 bytes at [RBX]..[RBX+7] are written).
(defthm sub_mem64_rax-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 8))
           (equal (read 1 address (sub_mem64_rax x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SUB [RBX], RAX is 3 bytes: REX.W 29 03 = 48 29 03)
(defthm sub_mem64_rax-rip
  (equal (rip (sub_mem64_rax x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sub_mem64_rax-registers
  (equal (rgfi reg (sub_mem64_rax x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff RAX > mem[RBX] unsigned (borrow):
(defthm sub_mem64_rax-cf
  (equal (get-flag :cf (sub_mem64_rax x86))
...
(defthm sub_mem64_rax-af
  (equal (get-flag :af (sub_mem64_rax x86))
...
(defthm sub_mem64_rax-of
  (equal (get-flag :of (sub_mem64_rax x86))
...
(defthm sub_mem64_rax-other-flags
  (implies (and (member-equal flag *flags*)
           ...
           (equal (get-flag flag (sub_mem64_rax x86))
                  (get-flag flag x86)))
