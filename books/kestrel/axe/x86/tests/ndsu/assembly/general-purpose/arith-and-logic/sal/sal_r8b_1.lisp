...
;; Lifts the functionality of sal_r8b_1.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_r8b_1.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)
(include-book "kestrel/x86/register-readers-and-writers-8-16" :dir :system)

;; Lifts the subroutine into logic: Creates the function sal_r8b_1, which
;; represents the effect of the program on the x86 state.
;; SAL R8B, 1 is encoded as 41 D0 E0 (3 bytes), so stop PC = 0x401003.
(def-unrolled sal_r8b_1
  :executable "sal_r8b_1.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
(defthm sal_r8b_1-r8b
  (equal (bvchop 8 (r8 (sal_r8b_1 x86)))
         (bvshl 8 (bvchop 8 (r8 x86)) 1))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 3 (SAL R8B, 1 is encoded as 41 D0 E0 (3 bytes), so stop PC = 0x401003.)
(defthm sal_r8b_1-rip
  (equal (rip (sal_r8b_1 x86))
         (+ 3 #x401000)))

;; Registers other than R8 are unchanged.
(defthm sal_r8b_1-other-registers
  (implies (not (equal *r8* reg))
           (equal (rgfi reg (sal_r8b_1 x86))
                  (rgfi reg x86)))
...
(defthm sal_r8b_1-cf
  (equal (get-flag :cf (sal_r8b_1 x86))
...
(defthm sal_r8b_1-sf
  (equal (get-flag :sf (sal_r8b_1 x86))
         (getbit 7 (bvshl 8 (bvchop 8 (r8 x86)) 1)))
  :hints (("Goal" :in-theory (enable bvshl))))
...
(defthm sal_r8b_1-pf
  (equal (get-flag :pf (sal_r8b_1 x86))
...
(defthm sal_r8b_1-memory-unchanged
  (equal (memi address (sal_r8b_1 x86))
         (memi address x86)))
...
(defthm sal_r8b_1-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sal_r8b_1 x86))
                  (get-flag flag x86)))
