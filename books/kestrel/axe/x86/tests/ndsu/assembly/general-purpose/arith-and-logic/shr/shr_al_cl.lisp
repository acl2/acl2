; Proofs about a 1-instruction binary that shifts a register (8-bit) right by CL (logical)
;
...
;; Lifts the functionality of shr_al_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shr_al_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function shr_al_cl, which
;; represents the effect of the program on the x86 state.
;; SHR AL, CL is encoded as D2 E8 (2 bytes), so stop PC = #x401002.
(def-unrolled shr_al_cl
  :executable "shr_al_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401002))
...
;; logical right shift).
(defthm shr_al_cl-al
  (equal (al (shr_al_cl x86))
         (bvshr 8 (al x86) (bvchop 5 (cl x86))))
  :hints (("Goal" :in-theory (enable bvshr))))

;; The RIP is advanced by 2 (SHR AL, CL is encoded as D2 E8 (2 bytes), so stop PC = #x401002.)
(defthm shr_al_cl-rip
  (equal (rip (shr_al_cl x86))
         (+ 2 #x401000)))

;; Registers other than RAX are unchanged.
(defthm shr_al_cl-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (shr_al_cl x86))
                  (rgfi reg x86)))
...
(defthm shr_al_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (shr_al_cl x86))
                  (get-flag flag x86))))
...
(defthm shr_al_cl-cf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
           ...
           (equal (get-flag :cf (shr_al_cl x86))
...
(defthm shr_al_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (shr_al_cl x86))
                  (getbit 7 (al x86)))))

;; For any nonzero masked count, the zero flag is 1 iff the result is 0.
(defthm shr_al_cl-zf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :zf (shr_al_cl x86))
                  (if (equal 0 (bvshr 8 (al x86) (bvchop 5 (cl x86))))
                      1
                    0)))
...
(defthm shr_al_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (shr_al_cl x86))
                  (getbit 7 (bvshr 8 (al x86) (bvchop 5 (cl x86))))))
  :hints (("Goal" :in-theory (enable bvshr))))
...
(defthm shr_al_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (shr_al_cl x86))
                  (if (evenp (bvcount 8 (bvshr 8 (al x86) (bvchop 5 (cl x86)))))
                      1
                    0)))
...
(defthm shr_al_cl-memory-unchanged
  (equal (memi address (shr_al_cl x86))
         (memi address x86)))
