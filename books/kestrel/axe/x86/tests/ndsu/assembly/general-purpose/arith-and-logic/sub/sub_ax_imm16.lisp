; Proofs about a 1-instruction binary that subtracts an immediate from AX (16-bit)
;
...
;; Lifts the functionality of sub_ax_imm16.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sub_ax_imm16.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sub_ax_imm16, which
;; represents the effect of the program on the x86 state.
;; SUB AX, 300 is encoded as 66 2D 2C 01 (4 bytes), so stop PC = 0x401004.
(def-unrolled sub_ax_imm16
  :executable "sub_ax_imm16.elf64"
  :target #x401000
  :stop-pcs '(#x401004))
...
(defthm sub_ax_imm16-rax
  (equal (rax (sub_ax_imm16 x86))
         (bvcat 48 (slice 63 16 (rax x86)) 16 (bvminus 16 (rax x86) 300))))

;; AX after the operation: DEST <- DEST - SRC (Intel SDM Vol 2A).
(defthm sub_ax_imm16-ax
  (equal (ax (sub_ax_imm16 x86))
         (bvminus 16 (ax x86) 300)))

;; The RIP is advanced by 4 (SUB AX, imm16 is 4 bytes: 66 2D 2C 01)
(defthm sub_ax_imm16-rip
  (equal (rip (sub_ax_imm16 x86))
         (+ 4 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sub_ax_imm16-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sub_ax_imm16 x86))
                  (rgfi reg x86)))
  :hints (("Goal" :in-theory (enable set-rax))))

;; The carry flag is 1 iff 300 > AX unsigned:
(defthm sub_ax_imm16-cf
  (equal (get-flag :cf (sub_ax_imm16 x86))
         (if (bvlt 16 (ax x86) 300) 1 0)))

;; The zero flag is 1 iff the result is zero:
(defthm sub_ax_imm16-zf
  (equal (get-flag :zf (sub_ax_imm16 x86))
...
(defthm sub_ax_imm16-sf
  (equal (get-flag :sf (sub_ax_imm16 x86))
...
(defthm sub_ax_imm16-af
  (equal (get-flag :af (sub_ax_imm16 x86))
         (if (< (bvchop 4 (ax x86))
                (bvchop 4 300))
             1
           0))
...
(defthm sub_ax_imm16-of
  (equal (get-flag :of (sub_ax_imm16 x86))
...
(defthm sub_ax_imm16-pf
  (equal (get-flag :pf (sub_ax_imm16 x86))
...
(defthm sub_ax_imm16-memory-unchanged
  (equal (memi address (sub_ax_imm16 x86))
         (memi address x86)))

(defthm sub_ax_imm16-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sub_ax_imm16 x86))
                  (get-flag flag x86)))
