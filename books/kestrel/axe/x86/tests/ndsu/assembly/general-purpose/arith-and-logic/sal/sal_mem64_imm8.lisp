; Proofs about a 1-instruction binary that shifts a memory qword left by 3
;
...
;; Lifts the functionality of sal_mem64_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_mem64_imm8.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function sal_mem64_imm8, which
;; represents the effect of the program on the x86 state.
;; SAL qword [RBX], 3 is encoded as 48 C1 23 03 (4 bytes), so stop PC = 0x401004.
...
(def-unrolled sal_mem64_imm8
  :executable "sal_mem64_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401004)
...
;; qword shifted left by 3, filling with 0 on the right (Intel
;; SDM Vol 1 3.4.3.1 and Vol 2B SHL/SAL entry: logical left shift, size =
;; qword).
(defthm sal_mem64_imm8-memory-at-rbx
  (equal (read 8 (rbx x86) (sal_mem64_imm8 x86))
         (bvshl 64 (read 8 (rbx x86) x86) 3))
...
(defthm sal_mem64_imm8-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 8))
           (equal (read 1 address (sal_mem64_imm8 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 4 (SAL qword [RBX], 3 is encoded as 48 C1 23 03 (4 bytes), so stop PC = 0x401004.)
(defthm sal_mem64_imm8-rip
  (equal (rip (sal_mem64_imm8 x86))
         (+ 4 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sal_mem64_imm8-registers
  (equal (rgfi reg (sal_mem64_imm8 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit (64 - 3) = bit 61
;; of the original operand (Intel SDM Vol 2B SHL/SAL entry).
(defthm sal_mem64_imm8-cf
  (equal (get-flag :cf (sal_mem64_imm8 x86))
         (getbit 61 (read 8 (rbx x86) x86))))
...
(defthm sal_mem64_imm8-zf
  (equal (get-flag :zf (sal_mem64_imm8 x86))
         (if (equal 0 (bvshl 64 (read 8 (rbx x86) x86) 3))
             1
           0))
  :hints (("Goal" :in-theory (enable bvshl zf-spec))))

;; The sign flag is the sign bit (bit 63) of the 64-bit result.
(defthm sal_mem64_imm8-sf
  (equal (get-flag :sf (sal_mem64_imm8 x86))
         (getbit 63 (bvshl 64 (read 8 (rbx x86) x86) 3)))
  :hints (("Goal" :in-theory (enable bvshl))))
...
(defthm sal_mem64_imm8-pf
  (equal (get-flag :pf (sal_mem64_imm8 x86))
         (if (evenp (bvcount 8 (bvshl 64 (read 8 (rbx x86) x86) 3)))
             1
           0))
...
(defthm sal_mem64_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sal_mem64_imm8 x86))
                  (get-flag flag x86)))
