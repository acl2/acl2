; Proofs about a 1-instruction binary that shifts a memory byte right by CL (logical)
;
...
;; Lifts the functionality of shr_mem8_r8_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shr_mem8_r8_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function shr_mem8_r8_cl, which
;; represents the effect of the program on the x86 state.
;; SHR byte [R8], CL is encoded as 41 D2 28 (3 bytes), so stop PC = #x401003.
...
(def-unrolled shr_mem8_r8_cl
  :executable "shr_mem8_r8_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; byte shifted right by the masked count in CL, filling with 0 on the left
;; (Intel SDM Vol 1 3.4.3.1 and Vol 2B SHR entry: logical right shift, size = byte).
(defthm shr_mem8_r8_cl-memory-at-r8
  (equal (read 1 (r8 x86) (shr_mem8_r8_cl x86))
         (bvshr 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86))))
...
(defthm shr_mem8_r8_cl-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (r8 x86)) 1))
           (equal (read 1 address (shr_mem8_r8_cl x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SHR byte [R8], CL is encoded as 41 D2 28 (3 bytes), so stop PC = #x401003.)
(defthm shr_mem8_r8_cl-rip
  (equal (rip (shr_mem8_r8_cl x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm shr_mem8_r8_cl-registers
  (equal (rgfi reg (shr_mem8_r8_cl x86))
         (rgfi reg x86)))
...
(defthm shr_mem8_r8_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (shr_mem8_r8_cl x86))
                  (get-flag flag x86))))
...
(defthm shr_mem8_r8_cl-cf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
                (bvlt 5 (bvchop 5 (cl x86)) 8))
           (equal (get-flag :cf (shr_mem8_r8_cl x86))
                  (getbit (+ (bvchop 5 (cl x86)) -1) (read 1 (r8 x86) x86)))))
...
;; 1-bit shifts).
(defthm shr_mem8_r8_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (shr_mem8_r8_cl x86))
                  (getbit 7 (read 1 (r8 x86) x86)))))

;; For any nonzero masked count, the zero flag is 1 iff the result is 0.
(defthm shr_mem8_r8_cl-zf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :zf (shr_mem8_r8_cl x86))
                  (if (equal 0 (bvshr 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86))))
                      1
                    0)))
...
;; of the 8-bit result.
(defthm shr_mem8_r8_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (shr_mem8_r8_cl x86))
                  (getbit 7 (bvshr 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86))))))
  :hints (("Goal" :in-theory (enable bvshr))))
...
;; of 1s.
(defthm shr_mem8_r8_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (shr_mem8_r8_cl x86))
                  (if (evenp (bvcount 8 (bvshr 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86)))))
                      1
                    0)))
...
(defthm shr_mem8_r8_cl-other-flags
