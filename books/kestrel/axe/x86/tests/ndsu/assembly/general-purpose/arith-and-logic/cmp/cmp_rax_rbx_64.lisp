; Proofs about a 1-instruction binary that compares 2 numbers (64-bit)
;
...
;; Lifts the functionality of cmp_rax_rbx_64.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "cmp_rax_rbx_64.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function cmp_rax_rbx_64, which
;; represents the effect of the program on the x86 state.
;; CMP RAX, RBX is encoded as 48 39 D8 (3 bytes), so stop PC = 0x401003.
(def-unrolled cmp_rax_rbx_64
  :executable "cmp_rax_rbx_64.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
;; The RIP is advanced by 3 (CMP RAX,RBX is 3 bytes: REX.W 39 D8 = 48 39 D8)
(defthm cmp_rax_rbx_64-rip
  (equal (rip (cmp_rax_rbx_64 x86))
         (+ 3 #x401000)))

;; CMP computes DEST - SRC only to set flags; DEST is not updated, so all
;; registers are unchanged (Intel SDM Vol 2A: CMP entry).
(defthm cmp_rax_rbx_64-registers
  (equal (rgfi reg (cmp_rax_rbx_64 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff the 64-bit subtraction borrows (unsigned src > unsigned dst):
(defthm cmp_rax_rbx_64-cf
  (equal (get-flag :cf (cmp_rax_rbx_64 x86))
         (if (bvlt 64 (rax x86) (rbx x86)) 1 0)))

;; The zero flag is 1 iff the (uncommitted) difference is zero:
(defthm cmp_rax_rbx_64-zf
  (equal (get-flag :zf (cmp_rax_rbx_64 x86))
         (if (equal 0 (bvminus 64 (rax x86) (rbx x86))) 1 0))
...
(defthm cmp_rax_rbx_64-sf
  (equal (get-flag :sf (cmp_rax_rbx_64 x86))
         (getbit 63 (bvminus 64 (rax x86) (rbx x86))))
...
(defthm cmp_rax_rbx_64-af
  (equal (get-flag :af (cmp_rax_rbx_64 x86))
         (if (< (bvchop 4 (rax x86))
                (bvchop 4 (rbx x86)))
             1
           0))
...
(defthm cmp_rax_rbx_64-of
  (equal (get-flag :of (cmp_rax_rbx_64 x86))
...
(defthm cmp_rax_rbx_64-pf
  (equal (get-flag :pf (cmp_rax_rbx_64 x86))
         (let ((diff (bvminus 64 (rax x86) (rbx x86))))
           (if (evenp (bvcount 8 diff)) 1 0)))
  :hints (("Goal" :in-theory (enable sub-pf-spec64-to-bvcount bvminus acl2::bvchop-of-sum-cases))))

;; All memory addresses are unchanged:
(defthm cmp_rax_rbx_64-memory-unchanged
  (equal (memi address (cmp_rax_rbx_64 x86))
         (memi address x86)))

(defthm cmp_rax_rbx_64-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (cmp_rax_rbx_64 x86))
                  (get-flag flag x86)))
