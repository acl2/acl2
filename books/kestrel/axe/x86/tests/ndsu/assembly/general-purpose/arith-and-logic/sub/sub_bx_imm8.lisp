; Proofs about a 1-instruction binary that subtracts a sign-extended 8-bit immediate from BX
;
...
;; Lifts the functionality of sub_bx_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sub_bx_imm8.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sub_bx_imm8, which
;; represents the effect of the program on the x86 state.
;; SUB BX, 5 is encoded as 66 83 EB 05 (4 bytes), so stop PC = 0x401004.
...
(def-unrolled sub_bx_imm8
  :executable "sub_bx_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401004))
...
;; The BX register is updated to the 16-bit difference of BX minus 5.
;; The upper 48 bits of RBX are preserved (16-bit operations do not zero-extend).
;; (Intel SDM Vol 2A: DEST <- DEST - SignExtend(SRC), for 16-bit operand size.)
(defthm sub_bx_imm8-rbx
  (equal (rbx (sub_bx_imm8 x86))
         (bvcat 48 (slice 63 16 (rbx x86))
                16 (bvminus 16 (rbx x86) 5))))

(defthm sub_bx_imm8-bx
  (equal (bx (sub_bx_imm8 x86))
         (bvminus 16 (bx x86) 5)))

;; The RIP is advanced by 4 (SUB BX, 5 is 4 bytes: 66 83 EB 05)
(defthm sub_bx_imm8-rip
  (equal (rip (sub_bx_imm8 x86))
         (+ 4 #x401000)))

;; All other (non-RBX) registers are unchanged:
(defthm sub_bx_imm8-other-registers
  (implies (not (equal *rbx* reg))
           (equal (rgfi reg (sub_bx_imm8 x86))
                  (rgfi reg x86)))
  :hints (("Goal" :in-theory (enable set-rbx))))

;; The carry flag is 1 iff 5 > BX unsigned (borrow):
(defthm sub_bx_imm8-cf
  (equal (get-flag :cf (sub_bx_imm8 x86))
...
(defthm sub_bx_imm8-of
  (equal (get-flag :of (sub_bx_imm8 x86))
...
(defthm sub_bx_imm8-memory-unchanged
  (equal (memi address (sub_bx_imm8 x86))
         (memi address x86)))

(defthm sub_bx_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sub_bx_imm8 x86))
                  (get-flag flag x86)))
