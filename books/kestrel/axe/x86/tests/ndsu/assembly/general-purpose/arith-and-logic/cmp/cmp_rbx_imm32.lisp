; Proofs about a 1-instruction binary that compares RBX to a sign-extended 32-bit immediate
;
...
;; Lifts the functionality of cmp_rbx_imm32.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "cmp_rbx_imm32.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function cmp_rbx_imm32, which
;; represents the effect of the program on the x86 state.
;; CMP RBX, 1000 is encoded as 48 81 FB E8 03 00 00 (7 bytes), so stop PC = 0x401007.
;; The immediate 1000 is sign-extended to 64 bits; since 0 < 1000 < 2^31, its value is unchanged.
(def-unrolled cmp_rbx_imm32
  :executable "cmp_rbx_imm32.elf64"
  :target #x401000
  :stop-pcs '(#x401007))
...
;; The RIP is advanced by 7 (CMP RBX, 1000 is 7 bytes: REX.W 81 FB E8 03 00 00)
(defthm cmp_rbx_imm32-rip
  (equal (rip (cmp_rbx_imm32 x86))
         (+ 7 #x401000)))
...
(defthm cmp_rbx_imm32-registers
  (equal (rgfi reg (cmp_rbx_imm32 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff 1000 > RBX unsigned (borrow):
(defthm cmp_rbx_imm32-cf
  (equal (get-flag :cf (cmp_rbx_imm32 x86))
         (if (bvlt 64 (rbx x86) 1000) 1 0)))

;; The zero flag is 1 iff the (uncommitted) difference is zero:
(defthm cmp_rbx_imm32-zf
  (equal (get-flag :zf (cmp_rbx_imm32 x86))
...
(defthm cmp_rbx_imm32-sf
  (equal (get-flag :sf (cmp_rbx_imm32 x86))
...
(defthm cmp_rbx_imm32-af
  (equal (get-flag :af (cmp_rbx_imm32 x86))
...
(defthm cmp_rbx_imm32-of
  (equal (get-flag :of (cmp_rbx_imm32 x86))
...
(defthm cmp_rbx_imm32-pf
  (equal (get-flag :pf (cmp_rbx_imm32 x86))
...
(defthm cmp_rbx_imm32-memory-unchanged
  (equal (memi address (cmp_rbx_imm32 x86))
         (memi address x86)))

(defthm cmp_rbx_imm32-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (cmp_rbx_imm32 x86))
                  (get-flag flag x86)))
