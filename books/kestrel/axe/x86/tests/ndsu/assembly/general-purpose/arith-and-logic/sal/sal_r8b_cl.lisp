...
;; Lifts the functionality of sal_r8b_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_r8b_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_r8b_cl, which
;; represents the effect of the program on the x86 state.
;; SAL R8B, CL is encoded as 41 D2 E0 (3 bytes), so stop PC = 0x401003.
(def-unrolled sal_r8b_cl
  :executable "sal_r8b_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
(defthm sal_r8b_cl-r8b
  (equal (bvchop 8 (r8 (sal_r8b_cl x86)))
         (bvshl 8 (bvchop 8 (r8 x86)) (bvchop 5 (cl x86))))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 3 (SAL R8B, CL is encoded as 41 D2 E0 (3 bytes), so stop PC = 0x401003.)
(defthm sal_r8b_cl-rip
  (equal (rip (sal_r8b_cl x86))
         (+ 3 #x401000)))

;; Registers other than R8 are unchanged.
(defthm sal_r8b_cl-other-registers
  (implies (not (equal *r8* reg))
           (equal (rgfi reg (sal_r8b_cl x86))
                  (rgfi reg x86)))
...
(defthm sal_r8b_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (sal_r8b_cl x86))
                  (get-flag flag x86))))
...
(defthm sal_r8b_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (sal_r8b_cl x86))
...
(defthm sal_r8b_cl-zf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :zf (sal_r8b_cl x86))
...
;; the 8-bit result.
(defthm sal_r8b_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (sal_r8b_cl x86))
...
;; of 1s.
(defthm sal_r8b_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (sal_r8b_cl x86))
...
(defthm sal_r8b_cl-memory-unchanged
  (equal (memi address (sal_r8b_cl x86))
         (memi address x86)))
