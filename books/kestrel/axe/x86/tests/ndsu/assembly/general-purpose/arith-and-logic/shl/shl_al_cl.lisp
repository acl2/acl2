; Proofs about a 1-instruction binary that shifts a register (8-bit) left by CL
;
...
;; Lifts the functionality of shl_al_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shl_al_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function shl_al_cl, which
;; represents the effect of the program on the x86 state.
;; SHL AL, CL is encoded as D2 E0 (2 bytes), so stop PC = 0x401002.
(def-unrolled shl_al_cl
  :executable "shl_al_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401002))
...
;; AL after the operation: DEST is shifted left by the masked count in CL,
;; filling with 0 on the right (Intel SDM Vol 1 3.4.3.1 and Vol 2B SHL/SAL
;; entry: logical left shift).
(defthm shl_al_cl-al
  (equal (al (shl_al_cl x86))
         (bvshl 8 (al x86) (bvchop 5 (cl x86))))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 2 (SHL AL, CL is encoded as D2 E0 (2 bytes), so stop PC = 0x401002.)
(defthm shl_al_cl-rip
  (equal (rip (shl_al_cl x86))
         (+ 2 #x401000)))

;; Registers other than RAX are unchanged.
(defthm shl_al_cl-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (shl_al_cl x86))
                  (rgfi reg x86)))
...
(defthm shl_al_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (shl_al_cl x86))
                  (get-flag flag x86))))
...
(defthm shl_al_cl-cf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
           ...
           (equal (get-flag :cf (shl_al_cl x86))
                  (getbit (+ 8 (- (bvchop 5 (cl x86)))) (al x86)))))
...
(defthm shl_al_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (shl_al_cl x86))
...
(defthm shl_al_cl-zf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :zf (shl_al_cl x86))
                  (if (equal 0 (bvshl 8 (al x86) (bvchop 5 (cl x86))))
                      1
                    0)))
...
(defthm shl_al_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (shl_al_cl x86))
                  (getbit 7 (bvshl 8 (al x86) (bvchop 5 (cl x86))))))
  :hints (("Goal" :in-theory (enable bvshl))))
...
(defthm shl_al_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (shl_al_cl x86))
                  (if (evenp (bvcount 8 (bvshl 8 (al x86) (bvchop 5 (cl x86)))))
                      1
                    0)))
...
(defthm shl_al_cl-memory-unchanged
  (equal (memi address (shl_al_cl x86))
         (memi address x86)))
