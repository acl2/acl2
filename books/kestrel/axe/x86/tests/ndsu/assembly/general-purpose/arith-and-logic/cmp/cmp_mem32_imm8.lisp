; Proofs about a 1-instruction binary that compares a memory dword to a sign-extended 8-bit immediate
;
...
;; Lifts the functionality of cmp_mem32_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "cmp_mem32_imm8.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function cmp_mem32_imm8, which
;; represents the effect of the program on the x86 state.
;; CMP [RBX], 5 is encoded as 83 3B 05 (3 bytes), so stop PC = 0x401003.
...
(def-unrolled cmp_mem32_imm8
  :executable "cmp_mem32_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; The RIP is advanced by 3 (CMP [RBX], 5 is 3 bytes: 83 3B 05)
(defthm cmp_mem32_imm8-rip
  (equal (rip (cmp_mem32_imm8 x86))
         (+ 3 #x401000)))
...
(defthm cmp_mem32_imm8-memory-unchanged
  (equal (memi address (cmp_mem32_imm8 x86))
         (memi address x86)))

;; All registers are unchanged (CMP does not write to any register either).
(defthm cmp_mem32_imm8-registers
  (equal (rgfi reg (cmp_mem32_imm8 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff 5 > mem[RBX] unsigned (borrow):
(defthm cmp_mem32_imm8-cf
  (equal (get-flag :cf (cmp_mem32_imm8 x86))
         (if (bvlt 32 (read 4 (rbx x86) x86) 5) 1 0)))

;; The zero flag is 1 iff the (uncommitted) difference is zero:
(defthm cmp_mem32_imm8-zf
  (equal (get-flag :zf (cmp_mem32_imm8 x86))
...
(defthm cmp_mem32_imm8-sf
  (equal (get-flag :sf (cmp_mem32_imm8 x86))
...
(defthm cmp_mem32_imm8-af
  (equal (get-flag :af (cmp_mem32_imm8 x86))
...
(defthm cmp_mem32_imm8-of
  (equal (get-flag :of (cmp_mem32_imm8 x86))
...
(defthm cmp_mem32_imm8-pf
  (equal (get-flag :pf (cmp_mem32_imm8 x86))
...
(defthm cmp_mem32_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (cmp_mem32_imm8 x86))
                  (get-flag flag x86)))
