...
;; Lifts the functionality of sal_eax_1.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_eax_1.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_eax_1, which
;; represents the effect of the program on the x86 state.
;; SAL EAX, 1 is encoded as D1 E0 (2 bytes), so stop PC = 0x401002.
(def-unrolled sal_eax_1
  :executable "sal_eax_1.elf64"
  :target #x401000
  :stop-pcs '(#x401002))
...
(defthm sal_eax_1-rax
  (equal (rax (sal_eax_1 x86))
         (bvshl 32 (eax x86) 1))
...
(defthm sal_eax_1-eax
  (equal (eax (sal_eax_1 x86))
         (bvshl 32 (eax x86) 1))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 2 (SAL EAX, 1 is encoded as D1 E0 (2 bytes), so stop PC = 0x401002.)
(defthm sal_eax_1-rip
  (equal (rip (sal_eax_1 x86))
         (+ 2 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sal_eax_1-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sal_eax_1 x86))
                  (rgfi reg x86)))
...
(defthm sal_eax_1-cf
  (equal (get-flag :cf (sal_eax_1 x86))
         (getbit 31 (eax x86))))
...
(defthm sal_eax_1-sf
  (equal (get-flag :sf (sal_eax_1 x86))
         (getbit 31 (bvshl 32 (eax x86) 1)))
  :hints (("Goal" :in-theory (enable bvshl))))
...
(defthm sal_eax_1-pf
  (equal (get-flag :pf (sal_eax_1 x86))
...
(defthm sal_eax_1-memory-unchanged
  (equal (memi address (sal_eax_1 x86))
         (memi address x86)))
...
(defthm sal_eax_1-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sal_eax_1 x86))
                  (get-flag flag x86)))
