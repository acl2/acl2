; Proofs about a 1-instruction binary that shifts a register (16-bit) right by 1 (arithmetic)
;
...
;; Lifts the functionality of sar_ax_1.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sar_ax_1.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sar_ax_1, which
;; represents the effect of the program on the x86 state.
;; SAR AX, 1 is encoded as 66 D1 F8 (3 bytes), so stop PC = #x401003.
(def-unrolled sar_ax_1
  :executable "sar_ax_1.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
(defthm sar_ax_1-rax
  (equal (rax (sar_ax_1 x86))
         (bvcat 48 (slice 63 16 (rax x86)) 16 (bvashr 16 (ax x86) 1)))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))

;; AX after the operation: DEST is shifted right by 1, with the sign bit
...
(defthm sar_ax_1-ax
  (equal (ax (sar_ax_1 x86))
         (bvashr 16 (ax x86) 1))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))

;; The RIP is advanced by 3 (SAR AX, 1 is encoded as 66 D1 F8 (3 bytes), so stop PC = #x401003.)
(defthm sar_ax_1-rip
  (equal (rip (sar_ax_1 x86))
         (+ 3 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sar_ax_1-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sar_ax_1 x86))
                  (rgfi reg x86)))
...
(defthm sar_ax_1-sf
  (equal (get-flag :sf (sar_ax_1 x86))
         (getbit 15 (bvashr 16 (ax x86) 1)))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))
...
(defthm sar_ax_1-pf
  (equal (get-flag :pf (sar_ax_1 x86))
         (if (evenp (bvcount 8 (bvashr 16 (ax x86) 1)))
             1
           0))
  :hints (("Goal" :in-theory (enable bvashr bvshr pf-spec16-alt-def))))

;; All memory addresses are unchanged.
(defthm sar_ax_1-memory-unchanged
  (equal (memi address (sar_ax_1 x86))
         (memi address x86)))
...
(defthm sar_ax_1-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sar_ax_1 x86))
                  (get-flag flag x86)))
