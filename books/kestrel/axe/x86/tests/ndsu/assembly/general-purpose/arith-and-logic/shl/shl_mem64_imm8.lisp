...
;; Lifts the functionality of shl_mem64_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shl_mem64_imm8.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function shl_mem64_imm8, which
;; represents the effect of the program on the x86 state.
;; SHL qword [RBX], 3 is encoded as 48 C1 23 03 (4 bytes), so stop PC = 0x401004.
...
(def-unrolled shl_mem64_imm8
  :executable "shl_mem64_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401004)
...
;; qword).
(defthm shl_mem64_imm8-memory-at-rbx
  (equal (read 8 (rbx x86) (shl_mem64_imm8 x86))
...
(defthm shl_mem64_imm8-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 8))
           (equal (read 1 address (shl_mem64_imm8 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 4 (SHL qword [RBX], 3 is encoded as 48 C1 23 03 (4 bytes), so stop PC = 0x401004.)
(defthm shl_mem64_imm8-rip
  (equal (rip (shl_mem64_imm8 x86))
         (+ 4 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm shl_mem64_imm8-registers
  (equal (rgfi reg (shl_mem64_imm8 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit (64 - 3) = bit 61
;; of the original operand (Intel SDM Vol 2B SHL/SAL entry).
(defthm shl_mem64_imm8-cf
  (equal (get-flag :cf (shl_mem64_imm8 x86))
...
(defthm shl_mem64_imm8-zf
  (equal (get-flag :zf (shl_mem64_imm8 x86))
...
(defthm shl_mem64_imm8-sf
  (equal (get-flag :sf (shl_mem64_imm8 x86))
...
(defthm shl_mem64_imm8-pf
  (equal (get-flag :pf (shl_mem64_imm8 x86))
...
(defthm shl_mem64_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (shl_mem64_imm8 x86))
                  (get-flag flag x86)))
