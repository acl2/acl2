; Proofs about a 1-instruction binary that compares AL to an immediate (8-bit)
;
...
;; Lifts the functionality of cmp_al_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "cmp_al_imm8.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function cmp_al_imm8, which
;; represents the effect of the program on the x86 state.
;; CMP AL, 5 is encoded as 3C 05 (2 bytes), so stop PC = 0x401002.
(def-unrolled cmp_al_imm8
  :executable "cmp_al_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401002))
...
;; The RIP is advanced by 2 (CMP AL, imm8 is 2 bytes: 3C 05)
(defthm cmp_al_imm8-rip
  (equal (rip (cmp_al_imm8 x86))
         (+ 2 #x401000)))

;; CMP computes DEST - SRC only to set flags; DEST is not updated, so all
;; registers are unchanged (Intel SDM Vol 2A: CMP entry).
(defthm cmp_al_imm8-registers
  (equal (rgfi reg (cmp_al_imm8 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff the 8-bit subtraction borrows (5 > AL unsigned):
(defthm cmp_al_imm8-cf
  (equal (get-flag :cf (cmp_al_imm8 x86))
         (if (bvlt 8 (al x86) 5) 1 0)))

;; The zero flag is 1 iff the (uncommitted) difference is zero:
(defthm cmp_al_imm8-zf
  (equal (get-flag :zf (cmp_al_imm8 x86))
...
(defthm cmp_al_imm8-sf
  (equal (get-flag :sf (cmp_al_imm8 x86))
...
(defthm cmp_al_imm8-af
  (equal (get-flag :af (cmp_al_imm8 x86))
         (if (< (bvchop 4 (al x86))
                5)
             1
           0))
...
(defthm cmp_al_imm8-of
  (equal (get-flag :of (cmp_al_imm8 x86))
...
(defthm cmp_al_imm8-pf
  (equal (get-flag :pf (cmp_al_imm8 x86))
...
(defthm cmp_al_imm8-memory-unchanged
  (equal (memi address (cmp_al_imm8 x86))
         (memi address x86)))

(defthm cmp_al_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (cmp_al_imm8 x86))
                  (get-flag flag x86)))
