...
;; Lifts the functionality of shr_mem8_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shr_mem8_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function shr_mem8_cl, which
;; represents the effect of the program on the x86 state.
;; SHR byte [RBX], CL is encoded as D2 2B (2 bytes), so stop PC = #x401002.
...
(def-unrolled shr_mem8_cl
  :executable "shr_mem8_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401002)
...
(defthm shr_mem8_cl-memory-at-rbx
  (equal (read 1 (rbx x86) (shr_mem8_cl x86))
         (bvshr 8 (read 1 (rbx x86) x86) (bvchop 5 (cl x86))))
...
(defthm shr_mem8_cl-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 1))
           (equal (read 1 address (shr_mem8_cl x86))
                  (read 1 address x86))))

;; The RIP is advanced by 2 (SHR byte [RBX], CL is encoded as D2 2B (2 bytes), so stop PC = #x401002.)
(defthm shr_mem8_cl-rip
  (equal (rip (shr_mem8_cl x86))
         (+ 2 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm shr_mem8_cl-registers
  (equal (rgfi reg (shr_mem8_cl x86))
         (rgfi reg x86)))
...
(defthm shr_mem8_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (shr_mem8_cl x86))
                  (get-flag flag x86))))
...
;; 1-bit shifts).
(defthm shr_mem8_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (shr_mem8_cl x86))
                  (getbit 7 (read 1 (rbx x86) x86)))))

;; For any nonzero masked count, the zero flag is 1 iff the result is 0.
(defthm shr_mem8_cl-zf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :zf (shr_mem8_cl x86))
                  (if (equal 0 (bvshr 8 (read 1 (rbx x86) x86) (bvchop 5 (cl x86))))
                      1
                    0)))
...
;; of the 8-bit result.
(defthm shr_mem8_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (shr_mem8_cl x86))
                  (getbit 7 (bvshr 8 (read 1 (rbx x86) x86) (bvchop 5 (cl x86))))))
  :hints (("Goal" :in-theory (enable bvshr))))
...
;; of 1s.
(defthm shr_mem8_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (shr_mem8_cl x86))
                  (if (evenp (bvcount 8 (bvshr 8 (read 1 (rbx x86) x86) (bvchop 5 (cl x86)))))
                      1
                    0)))
...
(defthm shr_mem8_cl-other-flags
