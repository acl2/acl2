...
;; Lifts the functionality of crc32_eax_mem8.elf64 into logic using the
;; Axe-based x86 lifter and proves various properties.

;; (depends-on "crc32_eax_mem8.elf64")
;; cert_param: (uses-stp)
...
(def-unrolled crc32_eax_mem8
  :executable "crc32_eax_mem8.elf64"
  :target #x401000
  :stop-pcs '(#x401005)
...
(defthm crc32_eax_mem8-rax
  (equal (rax (crc32_eax_mem8 x86))
...
(defthm crc32_eax_mem8-eax
  (equal (eax (crc32_eax_mem8 x86))
         (spec-crc32 (eax x86) (read 1 (rbx x86) x86) 8))
  :hints (("Goal" :in-theory (enable eax spec-crc32-becomes-x86isa-crc32))))

;; The RIP is advanced by 5 (CRC32 EAX, BYTE [RBX] is 5 bytes: F2 0F 38 F0 03)
(defthm crc32_eax_mem8-rip
  (equal (rip (crc32_eax_mem8 x86))
         (+ 5 #x401000)))

;; Registers other than RAX (the destination, EAX) are unchanged.
(defthm crc32_eax_mem8-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (crc32_eax_mem8 x86))
                  (rgfi reg x86)))
  :hints (("Goal" :in-theory (enable set-rax))))

;; No flags are affected by CRC32 (Intel SDM Vol 2A: CRC32).
(defthm crc32_eax_mem8-flags
  (implies (member-equal flag *standard-flags*)
           (equal (get-flag flag (crc32_eax_mem8 x86))
                  (get-flag flag x86))))

;; All memory addresses are unchanged (instruction reads from memory but does not write).
(defthm crc32_eax_mem8-memory-unchanged
  (equal (memi address (crc32_eax_mem8 x86))
         (memi address x86)))
