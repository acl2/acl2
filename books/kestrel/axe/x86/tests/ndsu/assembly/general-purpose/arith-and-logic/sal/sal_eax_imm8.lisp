...
;; Lifts the functionality of sal_eax_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_eax_imm8.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_eax_imm8, which
;; represents the effect of the program on the x86 state.
;; SAL EAX, 3 is encoded as C1 E0 03 (3 bytes), so stop PC = 0x401003.
(def-unrolled sal_eax_imm8
  :executable "sal_eax_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
(defthm sal_eax_imm8-rax
  (equal (rax (sal_eax_imm8 x86))
         (bvshl 32 (eax x86) 3))
...
(defthm sal_eax_imm8-eax
  (equal (eax (sal_eax_imm8 x86))
         (bvshl 32 (eax x86) 3))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 3 (SAL EAX, 3 is encoded as C1 E0 03 (3 bytes), so stop PC = 0x401003.)
(defthm sal_eax_imm8-rip
  (equal (rip (sal_eax_imm8 x86))
         (+ 3 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sal_eax_imm8-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sal_eax_imm8 x86))
                  (rgfi reg x86)))
...
(defthm sal_eax_imm8-cf
  (equal (get-flag :cf (sal_eax_imm8 x86))
         (getbit 29 (eax x86))))
...
(defthm sal_eax_imm8-zf
  (equal (get-flag :zf (sal_eax_imm8 x86))
...
(defthm sal_eax_imm8-sf
  (equal (get-flag :sf (sal_eax_imm8 x86))
...
(defthm sal_eax_imm8-pf
  (equal (get-flag :pf (sal_eax_imm8 x86))
...
(defthm sal_eax_imm8-memory-unchanged
  (equal (memi address (sal_eax_imm8 x86))
         (memi address x86)))
...
(defthm sal_eax_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sal_eax_imm8 x86))
                  (get-flag flag x86)))
