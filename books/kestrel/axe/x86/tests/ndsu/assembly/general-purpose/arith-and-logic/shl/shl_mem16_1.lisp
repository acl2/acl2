...
;; Lifts the functionality of shl_mem16_1.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shl_mem16_1.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)
(include-book "kestrel/x86/register-readers-and-writers-8-16" :dir :system)

;; Lifts the subroutine into logic: Creates the function shl_mem16_1, which
;; represents the effect of the program on the x86 state.
;; SHL word [RBX], 1 is encoded as 66 D1 23 (3 bytes), so stop PC = 0x401003.
...
(def-unrolled shl_mem16_1
  :executable "shl_mem16_1.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; word).
(defthm shl_mem16_1-memory-at-rbx
  (equal (read 2 (rbx x86) (shl_mem16_1 x86))
...
(defthm shl_mem16_1-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 2))
           (equal (read 1 address (shl_mem16_1 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SHL word [RBX], 1 is encoded as 66 D1 23 (3 bytes), so stop PC = 0x401003.)
(defthm shl_mem16_1-rip
  (equal (rip (shl_mem16_1 x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm shl_mem16_1-registers
  (equal (rgfi reg (shl_mem16_1 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit (16 - 1) = bit 15
;; of the original operand (Intel SDM Vol 2B SHL/SAL entry).
(defthm shl_mem16_1-cf
  (equal (get-flag :cf (shl_mem16_1 x86))
...
(defthm shl_mem16_1-sf
  (equal (get-flag :sf (shl_mem16_1 x86))
         (getbit 15 (bvshl 16 (read 2 (rbx x86) x86) 1)))
...
(defthm shl_mem16_1-pf
  (equal (get-flag :pf (shl_mem16_1 x86))
...
(defthm shl_mem16_1-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (shl_mem16_1 x86))
                  (get-flag flag x86)))
