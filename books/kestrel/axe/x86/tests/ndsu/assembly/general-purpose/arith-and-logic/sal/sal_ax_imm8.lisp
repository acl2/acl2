...
;; Lifts the functionality of sal_ax_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_ax_imm8.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_ax_imm8, which
;; represents the effect of the program on the x86 state.
;; SAL AX, 3 is encoded as 66 C1 E0 03 (4 bytes), so stop PC = 0x401004.
(def-unrolled sal_ax_imm8
  :executable "sal_ax_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401004))
...
(defthm sal_ax_imm8-rax
  (equal (rax (sal_ax_imm8 x86))
...
(defthm sal_ax_imm8-ax
  (equal (ax (sal_ax_imm8 x86))
         (bvshl 16 (ax x86) 3))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 4 (SAL AX, 3 is encoded as 66 C1 E0 03 (4 bytes), so stop PC = 0x401004.)
(defthm sal_ax_imm8-rip
  (equal (rip (sal_ax_imm8 x86))
         (+ 4 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sal_ax_imm8-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sal_ax_imm8 x86))
                  (rgfi reg x86)))
...
(defthm sal_ax_imm8-cf
  (equal (get-flag :cf (sal_ax_imm8 x86))
         (getbit 13 (ax x86))))
...
(defthm sal_ax_imm8-zf
  (equal (get-flag :zf (sal_ax_imm8 x86))
...
(defthm sal_ax_imm8-sf
  (equal (get-flag :sf (sal_ax_imm8 x86))
...
(defthm sal_ax_imm8-pf
  (equal (get-flag :pf (sal_ax_imm8 x86))
...
(defthm sal_ax_imm8-memory-unchanged
  (equal (memi address (sal_ax_imm8 x86))
         (memi address x86)))
...
(defthm sal_ax_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sal_ax_imm8 x86))
                  (get-flag flag x86)))
