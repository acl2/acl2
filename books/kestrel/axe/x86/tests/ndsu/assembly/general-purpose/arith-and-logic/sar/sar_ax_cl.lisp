; Proofs about a 1-instruction binary that shifts a register (16-bit) right by CL (arithmetic)
;
...
;; Lifts the functionality of sar_ax_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sar_ax_cl.elf64")
;; cert_param: (uses-stp)
...
(def-unrolled sar_ax_cl
  :executable "sar_ax_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
(defthm sar_ax_cl-rax
  (implies (bvlt 5 (bvchop 5 (cl x86)) 16)
           (equal (rax (sar_ax_cl x86))
                  (bvcat 48 (slice 63 16 (rax x86)) 16 (bvashr 16 (ax x86) (bvchop 5 (cl x86))))))
...
(defthm sar_ax_cl-ax
  (implies (bvlt 5 (bvchop 5 (cl x86)) 16)
           (equal (ax (sar_ax_cl x86))
                  (bvashr 16 (ax x86) (bvchop 5 (cl x86)))))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))

;; The RIP is advanced by 3 (SAR AX, CL is encoded as 66 D3 F8 (3 bytes), so stop PC = #x401003.)
(defthm sar_ax_cl-rip
  (equal (rip (sar_ax_cl x86))
         (+ 3 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sar_ax_cl-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sar_ax_cl x86))
                  (rgfi reg x86)))
...
(defthm sar_ax_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (sar_ax_cl x86))
                  (get-flag flag x86))))
...
(defthm sar_ax_cl-cf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
           ...
           (equal (get-flag :cf (sar_ax_cl x86))
...
(defthm sar_ax_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (sar_ax_cl x86))
                  0)))
...
(defthm sar_ax_cl-zf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
                (bvlt 5 (bvchop 5 (cl x86)) 16))
           (equal (get-flag :zf (sar_ax_cl x86))
                  (if (equal 0 (bvashr 16 (ax x86) (bvchop 5 (cl x86))))
                      1
                    0)))
...
(defthm sar_ax_cl-sf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
                (bvlt 5 (bvchop 5 (cl x86)) 16))
           (equal (get-flag :sf (sar_ax_cl x86))
                  (getbit 15 (bvashr 16 (ax x86) (bvchop 5 (cl x86))))))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))
...
(defthm sar_ax_cl-pf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
                (bvlt 5 (bvchop 5 (cl x86)) 16))
           (equal (get-flag :pf (sar_ax_cl x86))
                  (if (evenp (bvcount 8 (bvashr 16 (ax x86) (bvchop 5 (cl x86)))))
                      1
                    0)))
  :hints (("Goal" :in-theory (enable bvashr bvshr pf-spec16-alt-def))))

;; All memory addresses are unchanged.
(defthm sar_ax_cl-memory-unchanged
  (equal (memi address (sar_ax_cl x86))
         (memi address x86)))
