...
; over a byte read from memory, writing the result to a 64-bit destination
; register.
;
...
;; Lifts the functionality of crc32_rax_mem8.elf64 into logic using the
;; Axe-based x86 lifter and proves various properties.

;; (depends-on "crc32_rax_mem8.elf64")
;; cert_param: (uses-stp)
...
(def-unrolled crc32_rax_mem8
  :executable "crc32_rax_mem8.elf64"
  :target #x401000
  :stop-pcs '(#x401006)
...
;; are zeroed (REX.W is present, since the destination is RAX).
(defthm crc32_rax_mem8-rax
  (equal (rax (crc32_rax_mem8 x86))
         (spec-crc32 (eax x86) (read 1 (rbx x86) x86) 8))
...
(defthm crc32_rax_mem8-upper-32-bits-zero
  (equal (slice 63 32 (rax (crc32_rax_mem8 x86)))
         0)
  :hints (("Goal" :use (crc32_rax_mem8-rax
                        (:instance unsigned-byte-p-32-of-spec-crc32
                                   (crc (eax x86)) (data (read 1 (rbx x86) x86)) (data-width 8)))
           :in-theory (e/d (slice) (crc32_rax_mem8-rax spec-crc32)))))

;; The RIP is advanced by 6 (CRC32 RAX, BYTE [RBX] is 6 bytes: F2 48 0F 38 F0 03)
(defthm crc32_rax_mem8-rip
  (equal (rip (crc32_rax_mem8 x86))
         (+ 6 #x401000)))

;; Registers other than RAX (the destination) are unchanged.
(defthm crc32_rax_mem8-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (crc32_rax_mem8 x86))
                  (rgfi reg x86)))
  :hints (("Goal" :in-theory (enable set-rax))))

;; No flags are affected by CRC32 (Intel SDM Vol 2A: CRC32).
(defthm crc32_rax_mem8-flags
  (implies (member-equal flag *standard-flags*)
           (equal (get-flag flag (crc32_rax_mem8 x86))
                  (get-flag flag x86))))

;; All memory addresses are unchanged (instruction reads from memory but does not write).
(defthm crc32_rax_mem8-memory-unchanged
  (equal (memi address (crc32_rax_mem8 x86))
         (memi address x86)))
