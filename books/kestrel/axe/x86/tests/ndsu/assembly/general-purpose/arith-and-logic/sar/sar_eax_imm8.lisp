; Proofs about a 1-instruction binary that shifts a register (32-bit) right by 3 (arithmetic)
;
...
;; Lifts the functionality of sar_eax_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sar_eax_imm8.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sar_eax_imm8, which
;; represents the effect of the program on the x86 state.
;; SAR EAX, 3 is encoded as C1 F8 03 (3 bytes), so stop PC = #x401003.
(def-unrolled sar_eax_imm8
  :executable "sar_eax_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
(defthm sar_eax_imm8-rax
  (equal (rax (sar_eax_imm8 x86))
         (bvashr 32 (eax x86) 3))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))

;; EAX after the operation: DEST is shifted right by 3, with the sign bit
...
(defthm sar_eax_imm8-eax
  (equal (eax (sar_eax_imm8 x86))
         (bvashr 32 (eax x86) 3))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))

;; The RIP is advanced by 3 (SAR EAX, 3 is encoded as C1 F8 03 (3 bytes), so stop PC = #x401003.)
(defthm sar_eax_imm8-rip
  (equal (rip (sar_eax_imm8 x86))
         (+ 3 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sar_eax_imm8-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sar_eax_imm8 x86))
                  (rgfi reg x86)))
...
(defthm sar_eax_imm8-cf
  (equal (get-flag :cf (sar_eax_imm8 x86))
         (getbit 2 (eax x86))))
...
(defthm sar_eax_imm8-zf
  (equal (get-flag :zf (sar_eax_imm8 x86))
         (if (equal 0 (bvashr 32 (eax x86) 3))
             1
           0))
  :hints (("Goal" :in-theory (enable bvashr bvshr zf-spec))))

;; The sign flag is the sign bit (bit 31) of the 32-bit result.
(defthm sar_eax_imm8-sf
  (equal (get-flag :sf (sar_eax_imm8 x86))
         (getbit 31 (bvashr 32 (eax x86) 3)))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))
...
(defthm sar_eax_imm8-pf
  (equal (get-flag :pf (sar_eax_imm8 x86))
         (if (evenp (bvcount 8 (bvashr 32 (eax x86) 3)))
             1
           0))
  :hints (("Goal" :in-theory (enable bvashr bvshr pf-spec32-alt-def))))

;; All memory addresses are unchanged.
(defthm sar_eax_imm8-memory-unchanged
  (equal (memi address (sar_eax_imm8 x86))
         (memi address x86)))
...
(defthm sar_eax_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sar_eax_imm8 x86))
                  (get-flag flag x86)))
