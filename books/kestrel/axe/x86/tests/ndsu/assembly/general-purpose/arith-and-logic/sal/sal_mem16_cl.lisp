...
;; Lifts the functionality of sal_mem16_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_mem16_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_mem16_cl, which
;; represents the effect of the program on the x86 state.
;; SAL word [RBX], CL is encoded as 66 D3 23 (3 bytes), so stop PC = 0x401003.
...
(def-unrolled sal_mem16_cl
  :executable "sal_mem16_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; shift, size = word).
(defthm sal_mem16_cl-memory-at-rbx
  (equal (read 2 (rbx x86) (sal_mem16_cl x86))
...
(defthm sal_mem16_cl-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 2))
           (equal (read 1 address (sal_mem16_cl x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SAL word [RBX], CL is encoded as 66 D3 23 (3 bytes), so stop PC = 0x401003.)
(defthm sal_mem16_cl-rip
  (equal (rip (sal_mem16_cl x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sal_mem16_cl-registers
  (equal (rgfi reg (sal_mem16_cl x86))
         (rgfi reg x86)))
...
(defthm sal_mem16_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (sal_mem16_cl x86))
                  (get-flag flag x86))))
...
(defthm sal_mem16_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (sal_mem16_cl x86))
...
(defthm sal_mem16_cl-zf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :zf (sal_mem16_cl x86))
...
(defthm sal_mem16_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (sal_mem16_cl x86))
...
;; of 1s.
(defthm sal_mem16_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (sal_mem16_cl x86))
...
(defthm sal_mem16_cl-other-flags
