...
;; Lifts the functionality of sar_mem8_r8_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sar_mem8_r8_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sar_mem8_r8_cl, which
;; represents the effect of the program on the x86 state.
;; SAR byte [R8], CL is encoded as 41 D2 38 (3 bytes), so stop PC = #x401003.
...
(def-unrolled sar_mem8_r8_cl
  :executable "sar_mem8_r8_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
(defthm sar_mem8_r8_cl-memory-at-r8
  (implies (bvlt 5 (bvchop 5 (cl x86)) 8)
           (equal (read 1 (r8 x86) (sar_mem8_r8_cl x86))
                  (bvashr 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86)))))
...
(defthm sar_mem8_r8_cl-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (r8 x86)) 1))
           (equal (read 1 address (sar_mem8_r8_cl x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SAR byte [R8], CL is encoded as 41 D2 38 (3 bytes), so stop PC = #x401003.)
(defthm sar_mem8_r8_cl-rip
  (equal (rip (sar_mem8_r8_cl x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sar_mem8_r8_cl-registers
  (equal (rgfi reg (sar_mem8_r8_cl x86))
         (rgfi reg x86)))
...
(defthm sar_mem8_r8_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (sar_mem8_r8_cl x86))
                  (get-flag flag x86))))
...
(defthm sar_mem8_r8_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (sar_mem8_r8_cl x86))
                  0)))

;; For any nonzero masked count, the zero flag is 1 iff the result is 0.
(defthm sar_mem8_r8_cl-zf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
                (bvlt 5 (bvchop 5 (cl x86)) 8))
           (equal (get-flag :zf (sar_mem8_r8_cl x86))
                  (if (equal 0 (bvashr 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86))))
                      1
                    0)))
...
;; of the 8-bit result.
(defthm sar_mem8_r8_cl-sf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
                (bvlt 5 (bvchop 5 (cl x86)) 8))
           (equal (get-flag :sf (sar_mem8_r8_cl x86))
                  (getbit 7 (bvashr 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86))))))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))
...
;; of 1s.
(defthm sar_mem8_r8_cl-pf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
                (bvlt 5 (bvchop 5 (cl x86)) 8))
           (equal (get-flag :pf (sar_mem8_r8_cl x86))
                  (if (evenp (bvcount 8 (bvashr 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86)))))
                      1
                    0)))
...
(defthm sar_mem8_r8_cl-other-flags
