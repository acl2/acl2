; Proofs about a 1-instruction binary that compares a memory word to AX (16-bit mem-to-reg)
;
...
;; Lifts the functionality of cmp_mem16_ax.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "cmp_mem16_ax.elf64")
;; cert_param: (uses-stp)
...
(def-unrolled cmp_mem16_ax
  :executable "cmp_mem16_ax.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; The RIP is advanced by 3 (CMP [RBX], AX is 3 bytes: 66 39 03)
(defthm cmp_mem16_ax-rip
  (equal (rip (cmp_mem16_ax x86))
         (+ 3 #x401000)))

;; CMP computes DEST - SRC only to set flags; DEST (memory) is not updated, so
;; all memory is unchanged (Intel SDM Vol 2A: CMP entry).
(defthm cmp_mem16_ax-memory-unchanged
  (equal (memi address (cmp_mem16_ax x86))
         (memi address x86)))

;; All registers are unchanged (CMP does not write to any register either).
(defthm cmp_mem16_ax-registers
  (equal (rgfi reg (cmp_mem16_ax x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff AX > mem[RBX] unsigned (borrow):
(defthm cmp_mem16_ax-cf
  (equal (get-flag :cf (cmp_mem16_ax x86))
         (if (bvlt 16 (read 2 (rbx x86) x86) (ax x86)) 1 0)))

;; The zero flag is 1 iff the (uncommitted) difference is zero:
(defthm cmp_mem16_ax-zf
  (equal (get-flag :zf (cmp_mem16_ax x86))
...
(defthm cmp_mem16_ax-sf
  (equal (get-flag :sf (cmp_mem16_ax x86))
...
(defthm cmp_mem16_ax-af
  (equal (get-flag :af (cmp_mem16_ax x86))
...
(defthm cmp_mem16_ax-of
  (equal (get-flag :of (cmp_mem16_ax x86))
...
(defthm cmp_mem16_ax-pf
  (equal (get-flag :pf (cmp_mem16_ax x86))
...
(defthm cmp_mem16_ax-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (cmp_mem16_ax x86))
                  (get-flag flag x86)))
