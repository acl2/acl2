; Proofs about a 1-instruction binary that shifts a memory word right by 3 (arithmetic)
;
...
;; Lifts the functionality of sar_mem16_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sar_mem16_imm8.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)
(include-book "kestrel/x86/register-readers-and-writers-8-16" :dir :system)

;; Lifts the subroutine into logic: Creates the function sar_mem16_imm8, which
;; represents the effect of the program on the x86 state.
;; SAR word [RBX], 3 is encoded as 66 C1 3B 03 (4 bytes), so stop PC = #x401004.
...
(def-unrolled sar_mem16_imm8
  :executable "sar_mem16_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401004)
...
;; word shifted right by 3, with the sign bit replicated into the vacated bits
;; on the left (Intel SDM Vol 1 3.4.3.1 and Vol 2B SAR entry: arithmetic right
;; shift, size = word).
(defthm sar_mem16_imm8-memory-at-rbx
  (equal (read 2 (rbx x86) (sar_mem16_imm8 x86))
         (bvashr 16 (read 2 (rbx x86) x86) 3))
...
(defthm sar_mem16_imm8-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 2))
           (equal (read 1 address (sar_mem16_imm8 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 4 (SAR word [RBX], 3 is encoded as 66 C1 3B 03 (4 bytes), so stop PC = #x401004.)
(defthm sar_mem16_imm8-rip
  (equal (rip (sar_mem16_imm8 x86))
         (+ 4 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sar_mem16_imm8-registers
  (equal (rgfi reg (sar_mem16_imm8 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit 2 of the original
;; operand (Intel SDM Vol 2B SAR entry).
(defthm sar_mem16_imm8-cf
  (equal (get-flag :cf (sar_mem16_imm8 x86))
...
(defthm sar_mem16_imm8-zf
  (equal (get-flag :zf (sar_mem16_imm8 x86))
         (if (equal 0 (bvashr 16 (read 2 (rbx x86) x86) 3))
             1
           0))
  :hints (("Goal" :in-theory (e/d (bvashr bvshr zf-spec) (read-2-blast)))))

;; The sign flag is the sign bit (bit 15) of the 16-bit result.
(defthm sar_mem16_imm8-sf
  (equal (get-flag :sf (sar_mem16_imm8 x86))
         (getbit 15 (bvashr 16 (read 2 (rbx x86) x86) 3)))
...
(defthm sar_mem16_imm8-pf
  (equal (get-flag :pf (sar_mem16_imm8 x86))
         (if (evenp (bvcount 8 (bvashr 16 (read 2 (rbx x86) x86) 3)))
             1
           0))
...
(defthm sar_mem16_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sar_mem16_imm8 x86))
                  (get-flag flag x86)))
