; Proofs about a 1-instruction binary that subtracts a sign-extended 8-bit immediate from EBX
;
...
;; Lifts the functionality of sub_ebx_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sub_ebx_imm8.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sub_ebx_imm8, which
;; represents the effect of the program on the x86 state.
;; SUB EBX, 5 is encoded as 83 EB 05 (3 bytes), so stop PC = 0x401003.
...
(def-unrolled sub_ebx_imm8
  :executable "sub_ebx_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
;; The RBX register is updated to the 32-bit difference of EBX minus 5, zero-extended to 64 bits.
...
(defthm sub_ebx_imm8-rbx
  (equal (rbx (sub_ebx_imm8 x86))
         (bvminus 32 (ebx x86) 5)))

(defthm sub_ebx_imm8-ebx
  (equal (ebx (sub_ebx_imm8 x86))
         (bvminus 32 (ebx x86) 5)))

;; The RIP is advanced by 3 (SUB EBX, 5 is 3 bytes: 83 EB 05)
(defthm sub_ebx_imm8-rip
  (equal (rip (sub_ebx_imm8 x86))
         (+ 3 #x401000)))

;; All other (non-RBX) registers are unchanged:
(defthm sub_ebx_imm8-other-registers
  (implies (not (equal *rbx* reg))
           (equal (rgfi reg (sub_ebx_imm8 x86))
                  (rgfi reg x86)))
  :hints (("Goal" :in-theory (enable set-rbx))))

;; The carry flag is 1 iff 5 > EBX unsigned (borrow):
(defthm sub_ebx_imm8-cf
  (equal (get-flag :cf (sub_ebx_imm8 x86))
         (if (bvlt 32 (ebx x86) 5) 1 0)))

;; The zero flag is 1 iff the result is zero:
(defthm sub_ebx_imm8-zf
  (equal (get-flag :zf (sub_ebx_imm8 x86))
...
(defthm sub_ebx_imm8-sf
  (equal (get-flag :sf (sub_ebx_imm8 x86))
...
(defthm sub_ebx_imm8-af
  (equal (get-flag :af (sub_ebx_imm8 x86))
...
(defthm sub_ebx_imm8-of
  (equal (get-flag :of (sub_ebx_imm8 x86))
...
(defthm sub_ebx_imm8-pf
  (equal (get-flag :pf (sub_ebx_imm8 x86))
...
(defthm sub_ebx_imm8-memory-unchanged
  (equal (memi address (sub_ebx_imm8 x86))
         (memi address x86)))

(defthm sub_ebx_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sub_ebx_imm8 x86))
                  (get-flag flag x86)))
