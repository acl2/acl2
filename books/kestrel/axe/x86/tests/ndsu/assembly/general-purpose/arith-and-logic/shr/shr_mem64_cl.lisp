; Proofs about a 1-instruction binary that shifts a memory qword right by CL (logical)
;
...
;; Lifts the functionality of shr_mem64_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shr_mem64_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function shr_mem64_cl, which
;; represents the effect of the program on the x86 state.
;; SHR qword [RBX], CL is encoded as 48 D3 2B (3 bytes), so stop PC = #x401003.
...
(def-unrolled shr_mem64_cl
  :executable "shr_mem64_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; 6 bits (Intel SDM Vol 2B SHR entry: "tempCOUNT <- (COUNT AND 3FH)"
;; when the operand size is 64 bits).

;; The qword at memory address [RBX] is updated to the original
;; qword shifted right by the masked count in CL, filling with 0 on the left
;; (Intel SDM Vol 1 3.4.3.1 and Vol 2B SHR entry: logical right shift, size = qword).
(defthm shr_mem64_cl-memory-at-rbx
  (equal (read 8 (rbx x86) (shr_mem64_cl x86))
         (bvshr 64 (read 8 (rbx x86) x86) (bvchop 6 (cl x86))))
...
(defthm shr_mem64_cl-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 8))
           (equal (read 1 address (shr_mem64_cl x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SHR qword [RBX], CL is encoded as 48 D3 2B (3 bytes), so stop PC = #x401003.)
(defthm shr_mem64_cl-rip
  (equal (rip (shr_mem64_cl x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm shr_mem64_cl-registers
  (equal (rgfi reg (shr_mem64_cl x86))
         (rgfi reg x86)))
...
(defthm shr_mem64_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 6 (cl x86)) 0)
           (equal (get-flag flag (shr_mem64_cl x86))
                  (get-flag flag x86))))

;; For a nonzero masked count, CF is set to the last bit shifted out, i.e.,
;; bit (count - 1) of the original operand (Intel SDM Vol 2B SHR entry).
(defthm shr_mem64_cl-cf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :cf (shr_mem64_cl x86))
...
;; 1-bit shifts).
(defthm shr_mem64_cl-of
  (implies (equal (bvchop 6 (cl x86)) 1)
           (equal (get-flag :of (shr_mem64_cl x86))
                  (getbit 63 (read 8 (rbx x86) x86)))))

;; For any nonzero masked count, the zero flag is 1 iff the result is 0.
(defthm shr_mem64_cl-zf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :zf (shr_mem64_cl x86))
                  (if (equal 0 (bvshr 64 (read 8 (rbx x86) x86) (bvchop 6 (cl x86))))
                      1
                    0)))
...
(defthm shr_mem64_cl-sf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :sf (shr_mem64_cl x86))
                  (getbit 63 (bvshr 64 (read 8 (rbx x86) x86) (bvchop 6 (cl x86))))))
  :hints (("Goal" :in-theory (enable bvshr))))
...
;; of 1s.
(defthm shr_mem64_cl-pf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :pf (shr_mem64_cl x86))
                  (if (evenp (bvcount 8 (bvshr 64 (read 8 (rbx x86) x86) (bvchop 6 (cl x86)))))
                      1
                    0)))
...
(defthm shr_mem64_cl-other-flags
