; Proofs about a 1-instruction binary that shifts a memory dword right by 3 (arithmetic)
;
...
;; Lifts the functionality of sar_mem32_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sar_mem32_imm8.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function sar_mem32_imm8, which
;; represents the effect of the program on the x86 state.
;; SAR dword [RBX], 3 is encoded as C1 3B 03 (3 bytes), so stop PC = #x401003.
...
(def-unrolled sar_mem32_imm8
  :executable "sar_mem32_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; dword shifted right by 3, with the sign bit replicated into the vacated bits
;; on the left (Intel SDM Vol 1 3.4.3.1 and Vol 2B SAR entry: arithmetic right
;; shift, size = dword).
(defthm sar_mem32_imm8-memory-at-rbx
  (equal (read 4 (rbx x86) (sar_mem32_imm8 x86))
         (bvashr 32 (read 4 (rbx x86) x86) 3))
...
(defthm sar_mem32_imm8-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 4))
           (equal (read 1 address (sar_mem32_imm8 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SAR dword [RBX], 3 is encoded as C1 3B 03 (3 bytes), so stop PC = #x401003.)
(defthm sar_mem32_imm8-rip
  (equal (rip (sar_mem32_imm8 x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sar_mem32_imm8-registers
  (equal (rgfi reg (sar_mem32_imm8 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit 2 of the original
;; operand (Intel SDM Vol 2B SAR entry).
(defthm sar_mem32_imm8-cf
  (equal (get-flag :cf (sar_mem32_imm8 x86))
         (getbit 2 (read 4 (rbx x86) x86))))
...
(defthm sar_mem32_imm8-zf
  (equal (get-flag :zf (sar_mem32_imm8 x86))
         (if (equal 0 (bvashr 32 (read 4 (rbx x86) x86) 3))
             1
           0))
  :hints (("Goal" :in-theory (enable bvashr bvshr zf-spec))))

;; The sign flag is the sign bit (bit 31) of the 32-bit result.
(defthm sar_mem32_imm8-sf
  (equal (get-flag :sf (sar_mem32_imm8 x86))
         (getbit 31 (bvashr 32 (read 4 (rbx x86) x86) 3)))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))
...
(defthm sar_mem32_imm8-pf
  (equal (get-flag :pf (sar_mem32_imm8 x86))
         (if (evenp (bvcount 8 (bvashr 32 (read 4 (rbx x86) x86) 3)))
             1
           0))
...
(defthm sar_mem32_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sar_mem32_imm8 x86))
                  (get-flag flag x86)))
