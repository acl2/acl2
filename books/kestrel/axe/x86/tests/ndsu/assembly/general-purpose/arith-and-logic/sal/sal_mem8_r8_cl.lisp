...
;; Lifts the functionality of sal_mem8_r8_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_mem8_r8_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_mem8_r8_cl, which
;; represents the effect of the program on the x86 state.
;; SAL byte [R8], CL is encoded as 41 D2 20 (3 bytes), so stop PC = 0x401003.
...
(def-unrolled sal_mem8_r8_cl
  :executable "sal_mem8_r8_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; shift, size = byte).
(defthm sal_mem8_r8_cl-memory-at-r8
  (equal (read 1 (r8 x86) (sal_mem8_r8_cl x86))
         (bvshl 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86))))
...
(defthm sal_mem8_r8_cl-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (r8 x86)) 1))
           (equal (read 1 address (sal_mem8_r8_cl x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SAL byte [R8], CL is encoded as 41 D2 20 (3 bytes), so stop PC = 0x401003.)
(defthm sal_mem8_r8_cl-rip
  (equal (rip (sal_mem8_r8_cl x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sal_mem8_r8_cl-registers
  (equal (rgfi reg (sal_mem8_r8_cl x86))
         (rgfi reg x86)))
...
(defthm sal_mem8_r8_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (sal_mem8_r8_cl x86))
                  (get-flag flag x86))))
...
(defthm sal_mem8_r8_cl-cf
  (implies (and (not (equal (bvchop 5 (cl x86)) 0))
                (bvlt 5 (bvchop 5 (cl x86)) 8))
           (equal (get-flag :cf (sal_mem8_r8_cl x86))
                  (getbit (+ 8 (- (bvchop 5 (cl x86)))) (read 1 (r8 x86) x86)))))
...
(defthm sal_mem8_r8_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (sal_mem8_r8_cl x86))
...
(defthm sal_mem8_r8_cl-zf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :zf (sal_mem8_r8_cl x86))
                  (if (equal 0 (bvshl 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86))))
                      1
                    0)))
...
;; of the 8-bit result.
(defthm sal_mem8_r8_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (sal_mem8_r8_cl x86))
                  (getbit 7 (bvshl 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86))))))
  :hints (("Goal" :in-theory (enable bvshl))))
...
;; of 1s.
(defthm sal_mem8_r8_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (sal_mem8_r8_cl x86))
                  (if (evenp (bvcount 8 (bvshl 8 (read 1 (r8 x86) x86) (bvchop 5 (cl x86)))))
                      1
                    0)))
...
(defthm sal_mem8_r8_cl-other-flags
