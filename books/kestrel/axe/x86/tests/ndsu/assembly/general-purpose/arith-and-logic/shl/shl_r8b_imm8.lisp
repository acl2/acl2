...
;; Lifts the functionality of shl_r8b_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shl_r8b_imm8.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)
(include-book "kestrel/x86/register-readers-and-writers-8-16" :dir :system)

;; Lifts the subroutine into logic: Creates the function shl_r8b_imm8, which
;; represents the effect of the program on the x86 state.
;; SHL R8B, 3 is encoded as 41 C0 E0 03 (4 bytes), so stop PC = 0x401004.
(def-unrolled shl_r8b_imm8
  :executable "shl_r8b_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401004))
...
(defthm shl_r8b_imm8-r8b
  (equal (bvchop 8 (r8 (shl_r8b_imm8 x86)))
         (bvshl 8 (bvchop 8 (r8 x86)) 3))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 4 (SHL R8B, 3 is encoded as 41 C0 E0 03 (4 bytes), so stop PC = 0x401004.)
(defthm shl_r8b_imm8-rip
  (equal (rip (shl_r8b_imm8 x86))
         (+ 4 #x401000)))

;; Registers other than R8 are unchanged.
(defthm shl_r8b_imm8-other-registers
  (implies (not (equal *r8* reg))
           (equal (rgfi reg (shl_r8b_imm8 x86))
                  (rgfi reg x86)))
...
(defthm shl_r8b_imm8-cf
  (equal (get-flag :cf (shl_r8b_imm8 x86))
...
(defthm shl_r8b_imm8-zf
  (equal (get-flag :zf (shl_r8b_imm8 x86))
...
(defthm shl_r8b_imm8-sf
  (equal (get-flag :sf (shl_r8b_imm8 x86))
...
(defthm shl_r8b_imm8-pf
  (equal (get-flag :pf (shl_r8b_imm8 x86))
...
(defthm shl_r8b_imm8-memory-unchanged
  (equal (memi address (shl_r8b_imm8 x86))
         (memi address x86)))
...
(defthm shl_r8b_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (shl_r8b_imm8 x86))
                  (get-flag flag x86)))
