...
;; Lifts the functionality of sal_al_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_al_imm8.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_al_imm8, which
;; represents the effect of the program on the x86 state.
;; SAL AL, 3 is encoded as C0 E0 03 (3 bytes), so stop PC = 0x401003.
(def-unrolled sal_al_imm8
  :executable "sal_al_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
(defthm sal_al_imm8-al
  (equal (al (sal_al_imm8 x86))
         (bvshl 8 (al x86) 3))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 3 (SAL AL, 3 is encoded as C0 E0 03 (3 bytes), so stop PC = 0x401003.)
(defthm sal_al_imm8-rip
  (equal (rip (sal_al_imm8 x86))
         (+ 3 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sal_al_imm8-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sal_al_imm8 x86))
                  (rgfi reg x86)))
...
(defthm sal_al_imm8-cf
  (equal (get-flag :cf (sal_al_imm8 x86))
         (getbit 5 (al x86))))
...
(defthm sal_al_imm8-zf
  (equal (get-flag :zf (sal_al_imm8 x86))
...
(defthm sal_al_imm8-sf
  (equal (get-flag :sf (sal_al_imm8 x86))
...
(defthm sal_al_imm8-pf
  (equal (get-flag :pf (sal_al_imm8 x86))
...
(defthm sal_al_imm8-memory-unchanged
  (equal (memi address (sal_al_imm8 x86))
         (memi address x86)))
...
(defthm sal_al_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sal_al_imm8 x86))
                  (get-flag flag x86)))
