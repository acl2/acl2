...
;; Lifts the functionality of shr_mem8_1.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shr_mem8_1.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)
(include-book "kestrel/x86/register-readers-and-writers-8-16" :dir :system)

;; Lifts the subroutine into logic: Creates the function shr_mem8_1, which
;; represents the effect of the program on the x86 state.
;; SHR byte [RBX], 1 is encoded as D0 2B (2 bytes), so stop PC = #x401002.
...
(def-unrolled shr_mem8_1
  :executable "shr_mem8_1.elf64"
  :target #x401000
  :stop-pcs '(#x401002)
...
(defthm shr_mem8_1-memory-at-rbx
  (equal (read 1 (rbx x86) (shr_mem8_1 x86))
         (bvshr 8 (read 1 (rbx x86) x86) 1))
...
(defthm shr_mem8_1-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 1))
           (equal (read 1 address (shr_mem8_1 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 2 (SHR byte [RBX], 1 is encoded as D0 2B (2 bytes), so stop PC = #x401002.)
(defthm shr_mem8_1-rip
  (equal (rip (shr_mem8_1 x86))
         (+ 2 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm shr_mem8_1-registers
  (equal (rgfi reg (shr_mem8_1 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit 0 (the original LSB) of
;; the original operand (Intel SDM Vol 2B SHR entry).
(defthm shr_mem8_1-cf
  (equal (get-flag :cf (shr_mem8_1 x86))
...
(defthm shr_mem8_1-of
  (equal (get-flag :of (shr_mem8_1 x86))
         (getbit 7 (read 1 (rbx x86) x86))))

;; The zero flag is 1 iff the result is 0.
(defthm shr_mem8_1-zf
  (equal (get-flag :zf (shr_mem8_1 x86))
         (if (equal 0 (bvshr 8 (read 1 (rbx x86) x86) 1))
             1
           0))
  :hints (("Goal" :in-theory (enable bvshr zf-spec))))

;; The sign flag is the sign bit (bit 7) of the 8-bit result.
(defthm shr_mem8_1-sf
  (equal (get-flag :sf (shr_mem8_1 x86))
         (getbit 7 (bvshr 8 (read 1 (rbx x86) x86) 1)))
  :hints (("Goal" :in-theory (enable bvshr))))

;; The parity flag considers only the 8 least significant bits of the result
;; and is 1 iff they contain an even number of 1s.
(defthm shr_mem8_1-pf
  (equal (get-flag :pf (shr_mem8_1 x86))
         (if (evenp (bvcount 8 (bvshr 8 (read 1 (rbx x86) x86) 1)))
             1
           0))
...
(defthm shr_mem8_1-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (shr_mem8_1 x86))
                  (get-flag flag x86)))
