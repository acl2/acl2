...
;; Lifts the functionality of sal_eax_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_eax_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_eax_cl, which
;; represents the effect of the program on the x86 state.
;; SAL EAX, CL is encoded as D3 E0 (2 bytes), so stop PC = 0x401002.
(def-unrolled sal_eax_cl
  :executable "sal_eax_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401002))
...
(defthm sal_eax_cl-rax
  (equal (rax (sal_eax_cl x86))
...
(defthm sal_eax_cl-eax
  (equal (eax (sal_eax_cl x86))
         (bvshl 32 (eax x86) (bvchop 5 (cl x86))))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 2 (SAL EAX, CL is encoded as D3 E0 (2 bytes), so stop PC = 0x401002.)
(defthm sal_eax_cl-rip
  (equal (rip (sal_eax_cl x86))
         (+ 2 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sal_eax_cl-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sal_eax_cl x86))
                  (rgfi reg x86)))
...
(defthm sal_eax_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (sal_eax_cl x86))
                  (get-flag flag x86))))
...
(defthm sal_eax_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (sal_eax_cl x86))
...
;; of 1s.
(defthm sal_eax_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (sal_eax_cl x86))
...
(defthm sal_eax_cl-memory-unchanged
  (equal (memi address (sal_eax_cl x86))
         (memi address x86)))
