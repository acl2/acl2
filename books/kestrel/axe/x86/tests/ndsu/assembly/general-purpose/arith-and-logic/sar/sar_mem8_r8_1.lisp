...
;; Lifts the functionality of sar_mem8_r8_1.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sar_mem8_r8_1.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)
(include-book "kestrel/x86/register-readers-and-writers-8-16" :dir :system)

;; Lifts the subroutine into logic: Creates the function sar_mem8_r8_1, which
;; represents the effect of the program on the x86 state.
;; SAR byte [R8], 1 is encoded as 41 D0 38 (3 bytes), so stop PC = #x401003.
...
(def-unrolled sar_mem8_r8_1
  :executable "sar_mem8_r8_1.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; shift, size = byte).
(defthm sar_mem8_r8_1-memory-at-r8
  (equal (read 1 (r8 x86) (sar_mem8_r8_1 x86))
         (bvashr 8 (read 1 (r8 x86) x86) 1))
...
(defthm sar_mem8_r8_1-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (r8 x86)) 1))
           (equal (read 1 address (sar_mem8_r8_1 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SAR byte [R8], 1 is encoded as 41 D0 38 (3 bytes), so stop PC = #x401003.)
(defthm sar_mem8_r8_1-rip
  (equal (rip (sar_mem8_r8_1 x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sar_mem8_r8_1-registers
  (equal (rgfi reg (sar_mem8_r8_1 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit 0 (the original LSB) of
;; the original operand (Intel SDM Vol 2B SAR entry).
(defthm sar_mem8_r8_1-cf
  (equal (get-flag :cf (sar_mem8_r8_1 x86))
...
(defthm sar_mem8_r8_1-of
  (equal (get-flag :of (sar_mem8_r8_1 x86))
         0))

;; The zero flag is 1 iff the result is 0.
(defthm sar_mem8_r8_1-zf
  (equal (get-flag :zf (sar_mem8_r8_1 x86))
         (if (equal 0 (bvashr 8 (read 1 (r8 x86) x86) 1))
             1
           0))
  :hints (("Goal" :in-theory (enable bvashr bvshr zf-spec))))

;; The sign flag is the sign bit (bit 7) of the 8-bit result.
(defthm sar_mem8_r8_1-sf
  (equal (get-flag :sf (sar_mem8_r8_1 x86))
         (getbit 7 (bvashr 8 (read 1 (r8 x86) x86) 1)))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))

;; The parity flag considers only the 8 least significant bits of the result
;; and is 1 iff they contain an even number of 1s.
(defthm sar_mem8_r8_1-pf
  (equal (get-flag :pf (sar_mem8_r8_1 x86))
         (if (evenp (bvcount 8 (bvashr 8 (read 1 (r8 x86) x86) 1)))
             1
           0))
...
(defthm sar_mem8_r8_1-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sar_mem8_r8_1 x86))
                  (get-flag flag x86)))
