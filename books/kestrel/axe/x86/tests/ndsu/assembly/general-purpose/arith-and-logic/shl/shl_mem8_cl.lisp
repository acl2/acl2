...
;; Lifts the functionality of shl_mem8_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shl_mem8_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function shl_mem8_cl, which
;; represents the effect of the program on the x86 state.
;; SHL byte [RBX], CL is encoded as D2 23 (2 bytes), so stop PC = 0x401002.
...
(def-unrolled shl_mem8_cl
  :executable "shl_mem8_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401002)
...
;; shift, size = byte).
(defthm shl_mem8_cl-memory-at-rbx
  (equal (read 1 (rbx x86) (shl_mem8_cl x86))
...
(defthm shl_mem8_cl-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 1))
           (equal (read 1 address (shl_mem8_cl x86))
                  (read 1 address x86))))

;; The RIP is advanced by 2 (SHL byte [RBX], CL is encoded as D2 23 (2 bytes), so stop PC = 0x401002.)
(defthm shl_mem8_cl-rip
  (equal (rip (shl_mem8_cl x86))
         (+ 2 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm shl_mem8_cl-registers
  (equal (rgfi reg (shl_mem8_cl x86))
         (rgfi reg x86)))
...
(defthm shl_mem8_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 5 (cl x86)) 0)
           (equal (get-flag flag (shl_mem8_cl x86))
                  (get-flag flag x86))))
...
(defthm shl_mem8_cl-of
  (implies (equal (bvchop 5 (cl x86)) 1)
           (equal (get-flag :of (shl_mem8_cl x86))
...
(defthm shl_mem8_cl-zf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :zf (shl_mem8_cl x86))
...
;; of the 8-bit result.
(defthm shl_mem8_cl-sf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :sf (shl_mem8_cl x86))
...
;; of 1s.
(defthm shl_mem8_cl-pf
  (implies (not (equal (bvchop 5 (cl x86)) 0))
           (equal (get-flag :pf (shl_mem8_cl x86))
...
(defthm shl_mem8_cl-other-flags
