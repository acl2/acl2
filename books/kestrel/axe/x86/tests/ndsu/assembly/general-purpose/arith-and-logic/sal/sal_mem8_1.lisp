...
;; Lifts the functionality of sal_mem8_1.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_mem8_1.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)
(include-book "kestrel/x86/register-readers-and-writers-8-16" :dir :system)

;; Lifts the subroutine into logic: Creates the function sal_mem8_1, which
;; represents the effect of the program on the x86 state.
;; SAL byte [RBX], 1 is encoded as D0 23 (2 bytes), so stop PC = 0x401002.
...
(def-unrolled sal_mem8_1
  :executable "sal_mem8_1.elf64"
  :target #x401000
  :stop-pcs '(#x401002)
...
;; byte).
(defthm sal_mem8_1-memory-at-rbx
  (equal (read 1 (rbx x86) (sal_mem8_1 x86))
         (bvshl 8 (read 1 (rbx x86) x86) 1))
...
(defthm sal_mem8_1-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 1))
           (equal (read 1 address (sal_mem8_1 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 2 (SAL byte [RBX], 1 is encoded as D0 23 (2 bytes), so stop PC = 0x401002.)
(defthm sal_mem8_1-rip
  (equal (rip (sal_mem8_1 x86))
         (+ 2 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sal_mem8_1-registers
  (equal (rgfi reg (sal_mem8_1 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit (8 - 1) = bit 7
;; of the original operand (Intel SDM Vol 2B SHL/SAL entry).
(defthm sal_mem8_1-cf
  (equal (get-flag :cf (sal_mem8_1 x86))
...
(defthm sal_mem8_1-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sal_mem8_1 x86))
                  (get-flag flag x86)))
