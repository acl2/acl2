...
;; Lifts the functionality of shl_rax_cl.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shl_rax_cl.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function shl_rax_cl, which
;; represents the effect of the program on the x86 state.
;; SHL RAX, CL is encoded as 48 D3 E0 (3 bytes), so stop PC = 0x401003.
(def-unrolled shl_rax_cl
  :executable "shl_rax_cl.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
(defthm shl_rax_cl-rax
  (equal (rax (shl_rax_cl x86))
         (bvshl 64 (rax x86) (bvchop 6 (cl x86))))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 3 (SHL RAX, CL is encoded as 48 D3 E0 (3 bytes), so stop PC = 0x401003.)
(defthm shl_rax_cl-rip
  (equal (rip (shl_rax_cl x86))
         (+ 3 #x401000)))

;; Registers other than RAX are unchanged.
(defthm shl_rax_cl-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (shl_rax_cl x86))
                  (rgfi reg x86)))
...
(defthm shl_rax_cl-flags-unaffected-when-count-zero
  (implies (equal (bvchop 6 (cl x86)) 0)
           (equal (get-flag flag (shl_rax_cl x86))
                  (get-flag flag x86))))
...
(defthm shl_rax_cl-sf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :sf (shl_rax_cl x86))
...
;; of 1s.
(defthm shl_rax_cl-pf
  (implies (not (equal (bvchop 6 (cl x86)) 0))
           (equal (get-flag :pf (shl_rax_cl x86))
...
(defthm shl_rax_cl-memory-unchanged
  (equal (memi address (shl_rax_cl x86))
         (memi address x86)))
