...
;; Lifts the functionality of sal_mem64_1.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_mem64_1.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function sal_mem64_1, which
;; represents the effect of the program on the x86 state.
;; SAL qword [RBX], 1 is encoded as 48 D1 23 (3 bytes), so stop PC = 0x401003.
...
(def-unrolled sal_mem64_1
  :executable "sal_mem64_1.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; qword).
(defthm sal_mem64_1-memory-at-rbx
  (equal (read 8 (rbx x86) (sal_mem64_1 x86))
...
(defthm sal_mem64_1-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 8))
           (equal (read 1 address (sal_mem64_1 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SAL qword [RBX], 1 is encoded as 48 D1 23 (3 bytes), so stop PC = 0x401003.)
(defthm sal_mem64_1-rip
  (equal (rip (sal_mem64_1 x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sal_mem64_1-registers
  (equal (rgfi reg (sal_mem64_1 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit (64 - 1) = bit 63
;; of the original operand (Intel SDM Vol 2B SHL/SAL entry).
(defthm sal_mem64_1-cf
  (equal (get-flag :cf (sal_mem64_1 x86))
...
(defthm sal_mem64_1-sf
  (equal (get-flag :sf (sal_mem64_1 x86))
         (getbit 63 (bvshl 64 (read 8 (rbx x86) x86) 1)))
  :hints (("Goal" :in-theory (enable bvshl))))
...
(defthm sal_mem64_1-pf
  (equal (get-flag :pf (sal_mem64_1 x86))
...
(defthm sal_mem64_1-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sal_mem64_1 x86))
                  (get-flag flag x86)))
