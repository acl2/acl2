; Proofs about a 1-instruction binary that shifts a memory qword right by 1 (arithmetic)
;
...
;; Lifts the functionality of sar_mem64_1.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sar_mem64_1.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function sar_mem64_1, which
;; represents the effect of the program on the x86 state.
;; SAR qword [RBX], 1 is encoded as 48 D1 3B (3 bytes), so stop PC = #x401003.
...
(def-unrolled sar_mem64_1
  :executable "sar_mem64_1.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; qword shifted right by 1, with the sign bit replicated into the vacated bit
;; on the left (Intel SDM Vol 1 3.4.3.1 and Vol 2B SAR entry: arithmetic right
;; shift, size = qword).
(defthm sar_mem64_1-memory-at-rbx
  (equal (read 8 (rbx x86) (sar_mem64_1 x86))
         (bvashr 64 (read 8 (rbx x86) x86) 1))
...
(defthm sar_mem64_1-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 8))
           (equal (read 1 address (sar_mem64_1 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SAR qword [RBX], 1 is encoded as 48 D1 3B (3 bytes), so stop PC = #x401003.)
(defthm sar_mem64_1-rip
  (equal (rip (sar_mem64_1 x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sar_mem64_1-registers
  (equal (rgfi reg (sar_mem64_1 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit 0 (the original LSB) of
;; the original operand (Intel SDM Vol 2B SAR entry).
(defthm sar_mem64_1-cf
  (equal (get-flag :cf (sar_mem64_1 x86))
         (getbit 0 (read 8 (rbx x86) x86))))

;; OF is cleared to 0 for all 1-bit shifts of SAR, since the sign bit never
;; changes (Intel SDM Vol 2B SAR entry).
(defthm sar_mem64_1-of
  (equal (get-flag :of (sar_mem64_1 x86))
         0))

;; The zero flag is 1 iff the result is 0.
(defthm sar_mem64_1-zf
  (equal (get-flag :zf (sar_mem64_1 x86))
         (if (equal 0 (bvashr 64 (read 8 (rbx x86) x86) 1))
             1
           0))
  :hints (("Goal" :in-theory (enable bvashr bvshr zf-spec))))

;; The sign flag is the sign bit (bit 63) of the 64-bit result.
(defthm sar_mem64_1-sf
  (equal (get-flag :sf (sar_mem64_1 x86))
         (getbit 63 (bvashr 64 (read 8 (rbx x86) x86) 1)))
  :hints (("Goal" :in-theory (enable bvashr bvshr))))
...
(defthm sar_mem64_1-pf
  (equal (get-flag :pf (sar_mem64_1 x86))
         (if (evenp (bvcount 8 (bvashr 64 (read 8 (rbx x86) x86) 1)))
             1
           0))
...
(defthm sar_mem64_1-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sar_mem64_1 x86))
                  (get-flag flag x86)))
