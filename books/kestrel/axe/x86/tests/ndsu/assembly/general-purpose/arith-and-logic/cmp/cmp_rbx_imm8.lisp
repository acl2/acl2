; Proofs about a 1-instruction binary that compares RBX to a sign-extended 8-bit immediate
;
...
;; Lifts the functionality of cmp_rbx_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "cmp_rbx_imm8.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function cmp_rbx_imm8, which
;; represents the effect of the program on the x86 state.
;; CMP RBX, 5 is encoded as 48 83 FB 05 (4 bytes), so stop PC = 0x401004.
...
(def-unrolled cmp_rbx_imm8
  :executable "cmp_rbx_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401004))
...
;; The RIP is advanced by 4 (CMP RBX, 5 is 4 bytes: REX.W 83 FB 05)
(defthm cmp_rbx_imm8-rip
  (equal (rip (cmp_rbx_imm8 x86))
         (+ 4 #x401000)))
...
(defthm cmp_rbx_imm8-registers
  (equal (rgfi reg (cmp_rbx_imm8 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff 5 > RBX unsigned (borrow):
(defthm cmp_rbx_imm8-cf
  (equal (get-flag :cf (cmp_rbx_imm8 x86))
         (if (bvlt 64 (rbx x86) 5) 1 0)))

;; The zero flag is 1 iff the (uncommitted) difference is zero:
(defthm cmp_rbx_imm8-zf
  (equal (get-flag :zf (cmp_rbx_imm8 x86))
...
(defthm cmp_rbx_imm8-sf
  (equal (get-flag :sf (cmp_rbx_imm8 x86))
...
(defthm cmp_rbx_imm8-af
  (equal (get-flag :af (cmp_rbx_imm8 x86))
...
(defthm cmp_rbx_imm8-of
  (equal (get-flag :of (cmp_rbx_imm8 x86))
...
(defthm cmp_rbx_imm8-pf
  (equal (get-flag :pf (cmp_rbx_imm8 x86))
...
(defthm cmp_rbx_imm8-memory-unchanged
  (equal (memi address (cmp_rbx_imm8 x86))
         (memi address x86)))

(defthm cmp_rbx_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (cmp_rbx_imm8 x86))
                  (get-flag flag x86)))
