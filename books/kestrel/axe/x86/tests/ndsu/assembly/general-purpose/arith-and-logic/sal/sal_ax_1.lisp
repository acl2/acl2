...
;; Lifts the functionality of sal_ax_1.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_ax_1.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sal_ax_1, which
;; represents the effect of the program on the x86 state.
;; SAL AX, 1 is encoded as 66 D1 E0 (3 bytes), so stop PC = 0x401003.
(def-unrolled sal_ax_1
  :executable "sal_ax_1.elf64"
  :target #x401000
  :stop-pcs '(#x401003))
...
(defthm sal_ax_1-ax
  (equal (ax (sal_ax_1 x86))
         (bvshl 16 (ax x86) 1))
  :hints (("Goal" :in-theory (enable bvshl))))

;; The RIP is advanced by 3 (SAL AX, 1 is encoded as 66 D1 E0 (3 bytes), so stop PC = 0x401003.)
(defthm sal_ax_1-rip
  (equal (rip (sal_ax_1 x86))
         (+ 3 #x401000)))

;; Registers other than RAX are unchanged.
(defthm sal_ax_1-other-registers
  (implies (not (equal *rax* reg))
           (equal (rgfi reg (sal_ax_1 x86))
                  (rgfi reg x86)))
...
(defthm sal_ax_1-cf
  (equal (get-flag :cf (sal_ax_1 x86))
         (getbit 15 (ax x86))))
...
(defthm sal_ax_1-sf
  (equal (get-flag :sf (sal_ax_1 x86))
         (getbit 15 (bvshl 16 (ax x86) 1)))
  :hints (("Goal" :in-theory (enable bvshl))))
...
(defthm sal_ax_1-pf
  (equal (get-flag :pf (sal_ax_1 x86))
...
(defthm sal_ax_1-memory-unchanged
  (equal (memi address (sal_ax_1 x86))
         (memi address x86)))
...
(defthm sal_ax_1-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sal_ax_1 x86))
                  (get-flag flag x86)))
