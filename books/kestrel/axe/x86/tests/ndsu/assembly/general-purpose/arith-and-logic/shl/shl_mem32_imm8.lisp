...
;; Lifts the functionality of shl_mem32_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "shl_mem32_imm8.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function shl_mem32_imm8, which
;; represents the effect of the program on the x86 state.
;; SHL dword [RBX], 3 is encoded as C1 23 03 (3 bytes), so stop PC = 0x401003.
...
(def-unrolled shl_mem32_imm8
  :executable "shl_mem32_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401003)
...
;; dword).
(defthm shl_mem32_imm8-memory-at-rbx
  (equal (read 4 (rbx x86) (shl_mem32_imm8 x86))
...
(defthm shl_mem32_imm8-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 4))
           (equal (read 1 address (shl_mem32_imm8 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 3 (SHL dword [RBX], 3 is encoded as C1 23 03 (3 bytes), so stop PC = 0x401003.)
(defthm shl_mem32_imm8-rip
  (equal (rip (shl_mem32_imm8 x86))
         (+ 3 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm shl_mem32_imm8-registers
  (equal (rgfi reg (shl_mem32_imm8 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit (32 - 3) = bit 29
;; of the original operand (Intel SDM Vol 2B SHL/SAL entry).
(defthm shl_mem32_imm8-cf
  (equal (get-flag :cf (shl_mem32_imm8 x86))
...
(defthm shl_mem32_imm8-zf
  (equal (get-flag :zf (shl_mem32_imm8 x86))
...
(defthm shl_mem32_imm8-sf
  (equal (get-flag :sf (shl_mem32_imm8 x86))
...
(defthm shl_mem32_imm8-pf
  (equal (get-flag :pf (shl_mem32_imm8 x86))
...
(defthm shl_mem32_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (shl_mem32_imm8 x86))
                  (get-flag flag x86)))
