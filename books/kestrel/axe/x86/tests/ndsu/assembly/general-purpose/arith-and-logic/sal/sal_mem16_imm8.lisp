...
;; Lifts the functionality of sal_mem16_imm8.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sal_mem16_imm8.elf64")
;; cert_param: (uses-stp)
...
;; SAL word [RBX], 3 is encoded as 66 C1 23 03 (4 bytes), so stop PC = 0x401004.
...
(def-unrolled sal_mem16_imm8
  :executable "sal_mem16_imm8.elf64"
  :target #x401000
  :stop-pcs '(#x401004)
...
;; word).
(defthm sal_mem16_imm8-memory-at-rbx
  (equal (read 2 (rbx x86) (sal_mem16_imm8 x86))
...
(defthm sal_mem16_imm8-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 2))
           (equal (read 1 address (sal_mem16_imm8 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 4 (SAL word [RBX], 3 is encoded as 66 C1 23 03 (4 bytes), so stop PC = 0x401004.)
(defthm sal_mem16_imm8-rip
  (equal (rip (sal_mem16_imm8 x86))
         (+ 4 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sal_mem16_imm8-registers
  (equal (rgfi reg (sal_mem16_imm8 x86))
         (rgfi reg x86)))

;; CF is set to the last bit shifted out, i.e., bit (16 - 3) = bit 13
;; of the original operand (Intel SDM Vol 2B SHL/SAL entry).
(defthm sal_mem16_imm8-cf
  (equal (get-flag :cf (sal_mem16_imm8 x86))
...
(defthm sal_mem16_imm8-zf
  (equal (get-flag :zf (sal_mem16_imm8 x86))
...
(defthm sal_mem16_imm8-sf
  (equal (get-flag :sf (sal_mem16_imm8 x86))
...
(defthm sal_mem16_imm8-pf
  (equal (get-flag :pf (sal_mem16_imm8 x86))
...
(defthm sal_mem16_imm8-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sal_mem16_imm8 x86))
                  (get-flag flag x86)))
