; Proofs about a 1-instruction binary that subtracts EAX from a memory dword (32-bit reg-to-mem)
;
...
;; Lifts the functionality of sub_mem32_eax.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sub_mem32_eax.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function sub_mem32_eax, which
;; represents the effect of the program on the x86 state.
;; SUB [RBX], EAX is encoded as 29 03 (2 bytes), so stop PC = 0x401002.
;; Both the base address and +3 must be canonical for the 32-bit write.
(def-unrolled sub_mem32_eax
  :executable "sub_mem32_eax.elf64"
  :target #x401000
  :stop-pcs '(#x401002)
...
;; dword at [RBX] minus EAX (Intel SDM Vol 2A: DEST <- DEST - SRC, size = dword).
(defthm sub_mem32_eax-memory-at-rbx
  (equal (read 4 (rbx x86) (sub_mem32_eax x86))
         (bvminus 32 (read 4 (rbx x86) x86) (eax x86))))

;; All other memory bytes are unchanged (only the 4 bytes at [RBX]..[RBX+3] are written).
(defthm sub_mem32_eax-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 4))
           (equal (read 1 address (sub_mem32_eax x86))
                  (read 1 address x86))))

;; The RIP is advanced by 2 (SUB [RBX], EAX is 2 bytes: 29 03)
(defthm sub_mem32_eax-rip
  (equal (rip (sub_mem32_eax x86))
         (+ 2 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sub_mem32_eax-registers
  (equal (rgfi reg (sub_mem32_eax x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff EAX > mem[RBX] unsigned (borrow):
(defthm sub_mem32_eax-cf
  (equal (get-flag :cf (sub_mem32_eax x86))
         (if (bvlt 32 (read 4 (rbx x86) x86) (eax x86)) 1 0)))

;; The zero flag is 1 iff the result is zero:
(defthm sub_mem32_eax-zf
  (equal (get-flag :zf (sub_mem32_eax x86))
         (if (equal 0 (bvminus 32 (read 4 (rbx x86) x86) (eax x86))) 1 0)))

;; The sign flag is the sign bit (bit 31) of the 32-bit result:
(defthm sub_mem32_eax-sf
  (equal (get-flag :sf (sub_mem32_eax x86))
...
(defthm sub_mem32_eax-af
  (equal (get-flag :af (sub_mem32_eax x86))
...
(defthm sub_mem32_eax-of
  (equal (get-flag :of (sub_mem32_eax x86))
...
(defthm sub_mem32_eax-pf
  (equal (get-flag :pf (sub_mem32_eax x86))
         (if (evenp (bvcount 8 (bvminus 32 (read 4 (rbx x86) x86) (eax x86)))) 1 0))
  :hints (("Goal" :in-theory (enable pf-spec32-alt-def bvminus))))

(defthm sub_mem32_eax-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sub_mem32_eax x86))
                  (get-flag flag x86)))
