; Proofs about a 1-instruction binary that compares EBX to a 32-bit immediate
;
...
;; Lifts the functionality of cmp_ebx_imm32.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "cmp_ebx_imm32.elf64")
;; cert_param: (uses-stp)
...
;; Lifts the subroutine into logic: Creates the function cmp_ebx_imm32, which
;; represents the effect of the program on the x86 state.
;; CMP EBX, 1000 is encoded as 81 FB E8 03 00 00 (6 bytes), so stop PC = 0x401006.
(def-unrolled cmp_ebx_imm32
  :executable "cmp_ebx_imm32.elf64"
  :target #x401000
  :stop-pcs '(#x401006))
...
;; The RIP is advanced by 6 (CMP EBX, 1000 is 6 bytes: 81 FB E8 03 00 00)
(defthm cmp_ebx_imm32-rip
  (equal (rip (cmp_ebx_imm32 x86))
         (+ 6 #x401000)))

;; CMP computes DEST - SRC only to set flags; DEST is not updated, so all
;; registers are unchanged (Intel SDM Vol 2A: CMP entry).
(defthm cmp_ebx_imm32-registers
  (equal (rgfi reg (cmp_ebx_imm32 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff 1000 > EBX unsigned (borrow):
(defthm cmp_ebx_imm32-cf
  (equal (get-flag :cf (cmp_ebx_imm32 x86))
         (if (bvlt 32 (ebx x86) 1000) 1 0)))

;; The zero flag is 1 iff the (uncommitted) difference is zero:
(defthm cmp_ebx_imm32-zf
  (equal (get-flag :zf (cmp_ebx_imm32 x86))
...
(defthm cmp_ebx_imm32-sf
  (equal (get-flag :sf (cmp_ebx_imm32 x86))
...
(defthm cmp_ebx_imm32-af
  (equal (get-flag :af (cmp_ebx_imm32 x86))
...
(defthm cmp_ebx_imm32-of
  (equal (get-flag :of (cmp_ebx_imm32 x86))
...
(defthm cmp_ebx_imm32-pf
  (equal (get-flag :pf (cmp_ebx_imm32 x86))
...
(defthm cmp_ebx_imm32-memory-unchanged
  (equal (memi address (cmp_ebx_imm32 x86))
         (memi address x86)))

(defthm cmp_ebx_imm32-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (cmp_ebx_imm32 x86))
                  (get-flag flag x86)))
