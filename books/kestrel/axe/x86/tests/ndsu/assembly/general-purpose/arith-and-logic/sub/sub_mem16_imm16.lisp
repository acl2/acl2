; Proofs about a 1-instruction binary that subtracts a 16-bit immediate from a memory word
;
...
;; Lifts the functionality of sub_mem16_imm16.elf64 into logic using the Axe-based x86
;; lifter and proves various properties.

;; (depends-on "sub_mem16_imm16.elf64")
;; cert_param: (uses-stp)

(include-book "kestrel/axe/x86/unroller" :dir :system)

;; Lifts the subroutine into logic: Creates the function sub_mem16_imm16, which
;; represents the effect of the program on the x86 state.
;; SUB [RBX], 300 is encoded as 66 81 2B 2C 01 (5 bytes), so stop PC = 0x401005.
;; Both the base address and +1 must be canonical for the 16-bit write.
(def-unrolled sub_mem16_imm16
  :executable "sub_mem16_imm16.elf64"
  :target #x401000
  :stop-pcs '(#x401005)
...
;; word at [RBX] minus 300 (Intel SDM Vol 2A: DEST <- DEST - SRC, size = word).
(defthm sub_mem16_imm16-memory-at-rbx
  (equal (read 2 (rbx x86) (sub_mem16_imm16 x86))
         (bvminus 16 (read 2 (rbx x86) x86) 300)))

;; All other memory bytes are unchanged (only the 2 bytes at [RBX] and [RBX+1] are written).
(defthm sub_mem16_imm16-other-memory
  (implies (not (bvlt 48 (bvminus 48 address (rbx x86)) 2))
           (equal (read 1 address (sub_mem16_imm16 x86))
                  (read 1 address x86))))

;; The RIP is advanced by 5 (SUB [RBX], 300 is 5 bytes: 66 81 2B 2C 01)
(defthm sub_mem16_imm16-rip
  (equal (rip (sub_mem16_imm16 x86))
         (+ 5 #x401000)))

;; All registers are unchanged (the destination is memory, not a register).
(defthm sub_mem16_imm16-registers
  (equal (rgfi reg (sub_mem16_imm16 x86))
         (rgfi reg x86)))

;; The carry flag is 1 iff 300 > mem[RBX] unsigned (borrow):
(defthm sub_mem16_imm16-cf
  (equal (get-flag :cf (sub_mem16_imm16 x86))
         (if (bvlt 16 (read 2 (rbx x86) x86) 300) 1 0)))

;; The zero flag is 1 iff the result is zero:
(defthm sub_mem16_imm16-zf
  (equal (get-flag :zf (sub_mem16_imm16 x86))
...
(defthm sub_mem16_imm16-sf
  (equal (get-flag :sf (sub_mem16_imm16 x86))
...
(defthm sub_mem16_imm16-af
  (equal (get-flag :af (sub_mem16_imm16 x86))
...
(defthm sub_mem16_imm16-of
  (equal (get-flag :of (sub_mem16_imm16 x86))
...
(defthm sub_mem16_imm16-pf
  (equal (get-flag :pf (sub_mem16_imm16 x86))
...
(defthm sub_mem16_imm16-other-flags
  (implies (and (member-equal flag *flags*)
                (not (member-eq flag *standard-flags*)))
           (equal (get-flag flag (sub_mem16_imm16 x86))
                  (get-flag flag x86)))
