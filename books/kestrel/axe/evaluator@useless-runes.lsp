(LG-UNGUARDED)
(LG-UNGUARDED-CORRECT
     (14 2 (:REWRITE FLOOR-WHEN-<))
     (8 4 (:REWRITE DEFAULT-+-2))
     (5 4 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (5 2 (:REWRITE ZIP-OPEN))
     (4 4
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (4 4 (:REWRITE DEFAULT-+-1))
     (4 4 (:REWRITE CLR-DIFFERENTIAL))
     (2 2 (:REWRITE USE-ALL-<-2))
     (2 2 (:REWRITE USE-ALL-<))
     (2 2
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (2 2
        (:REWRITE FLOOR-WHEN-NOT-RATIONALP-OF-QUOTIENT))
     (2 2
        (:REWRITE FLOOR-WHEN-NOT-RATIONALP-ARG1))
     (2 2
        (:REWRITE FLOOR-WHEN-NEGATIVE-AND-SMALL-CHEAP))
     (2 2
        (:REWRITE FLOOR-WHEN-I-IS-NOT-AN-ACL2-NUMBERP))
     (2 2
        (:REWRITE FLOOR-MINUS-NEGATIVE-CONSTANT))
     (2 2 (:REWRITE FLOOR-MINUS-ERIC-BETTER))
     (2 2 (:REWRITE DEFAULT-<-2))
     (2 2 (:REWRITE DEFAULT-<-1))
     (2 2
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (2 2 (:REWRITE <-OF-+-OF-1-STRENGTHEN)))
(BITNOT-UNGUARDED)
(BITNOT-UNGUARDED-CORRECT
     (2 1 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (1 1
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (1 1
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (1 1
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (1 1 (:REWRITE CLR-DIFFERENTIAL)))
(BITOR-UNGUARDED)
(BITOR-UNGUARDED-CORRECT
    (202 12 (:REWRITE GETBIT-IDENTITY))
    (120 10
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
    (54 27 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
    (48 14 (:REWRITE DEFAULT-<-1))
    (48 4 (:REWRITE FLOOR-WHEN-<))
    (36 2 (:DEFINITION EVENP))
    (27 27 (:REWRITE CLR-DIFFERENTIAL))
    (26 12 (:REWRITE LOGNOT-WHEN-NOT-INTEGERP))
    (24 12 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
    (18 6
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
    (14 14 (:REWRITE USE-ALL-<-2))
    (14 14 (:REWRITE USE-ALL-<))
    (14 14 (:REWRITE DEFAULT-<-2))
    (14 14
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
    (14 14 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
    (14 2 (:REWRITE COMMUTATIVITY-OF-+))
    (13 13
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
    (12 12
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
    (12 12
        (:REWRITE GETBIT-WHEN-NOT-INTEGERP-ARG1))
    (12 12 (:REWRITE GETBIT-WHEN-N-IS-NEGATIVE))
    (12 12
        (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
    (12 12 (:REWRITE GETBIT-IDENTITY-FREE))
    (12 8 (:REWRITE DEFAULT-*-2))
    (10 10
        (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
    (10 10
        (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
    (10 10 (:TYPE-PRESCRIPTION BINARY-LOGAND))
    (10 10 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
    (10 10 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
    (10 10
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
    (10 10
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
    (10 10 (:REWRITE UBP-LONGER-BETTER))
    (10 10
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
    (10 8 (:REWRITE DEFAULT-*-1))
    (10 2 (:REWRITE MULTIPLE-WHEN-MOD-0-CHEAP))
    (8 4
       (:REWRITE FLOOR-WHEN-NOT-RATIONALP-OF-QUOTIENT))
    (8 4
       (:REWRITE FLOOR-WHEN-NOT-RATIONALP-ARG1))
    (8 4
       (:REWRITE FLOOR-WHEN-NEGATIVE-AND-SMALL-CHEAP))
    (8 4
       (:REWRITE FLOOR-WHEN-I-IS-NOT-AN-ACL2-NUMBERP))
    (8 2 (:REWRITE COMMUTATIVITY-OF-*))
    (6 6 (:REWRITE USE-ALL-RATIONALP-2))
    (6 6 (:REWRITE USE-ALL-RATIONALP))
    (6 6
       (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
    (6 6
       (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
    (6 4 (:REWRITE DEFAULT-+-2))
    (6 4 (:REWRITE DEFAULT-+-1))
    (6 2 (:REWRITE UNICITY-OF-0))
    (6 2
       (:REWRITE INTEGERP-OF-*-1/2-OF-LOGNOT))
    (4 4
       (:REWRITE FLOOR-MINUS-NEGATIVE-CONSTANT))
    (4 4 (:REWRITE FLOOR-MINUS-ERIC-BETTER))
    (4 2 (:TYPE-PRESCRIPTION RATIONALP-OF-MOD))
    (4 2 (:DEFINITION FIX))
    (2 2
       (:TYPE-PRESCRIPTION NONNEG-OF-MOD-TYPE-2))
    (2 2
       (:TYPE-PRESCRIPTION INTEGERP-OF-MOD-TYPE)))
(BITXOR-UNGUARDED)
(BITXOR-UNGUARDED-CORRECT)
(BITAND-UNGUARDED)
(BITAND-UNGUARDED-CORRECT
     (78 4 (:REWRITE GETBIT-IDENTITY))
     (46 4
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (14 4 (:REWRITE DEFAULT-<-1))
     (11 5 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (8 8 (:REWRITE DEFAULT-*-2))
     (8 8 (:REWRITE DEFAULT-*-1))
     (8 4
        (:REWRITE GETBIT-WHEN-VAL-IS-NOT-AN-INTEGER))
     (8 4 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
     (6 2
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (5 5
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (5 5 (:REWRITE CLR-DIFFERENTIAL))
     (4 4
        (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
     (4 4
        (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
     (4 4 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (4 4 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (4 4 (:REWRITE USE-ALL-<-2))
     (4 4 (:REWRITE USE-ALL-<))
     (4 4
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (4 4 (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (4 4 (:REWRITE UBP-LONGER-BETTER))
     (4 4
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (4 4
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (4 4
        (:REWRITE GETBIT-WHEN-NOT-INTEGERP-ARG1))
     (4 4 (:REWRITE GETBIT-WHEN-N-IS-NEGATIVE))
     (4 4
        (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
     (4 4 (:REWRITE GETBIT-IDENTITY-FREE))
     (4 4
        (:REWRITE FLOOR-WHEN-NOT-RATIONALP-OF-QUOTIENT))
     (4 4
        (:REWRITE FLOOR-WHEN-NEGATIVE-AND-SMALL-CHEAP))
     (4 4
        (:REWRITE FLOOR-WHEN-I-IS-NOT-AN-ACL2-NUMBERP))
     (4 4
        (:REWRITE FLOOR-MINUS-NEGATIVE-CONSTANT))
     (4 4 (:REWRITE DEFAULT-<-2))
     (4 4
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (4 4 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (2 2 (:REWRITE USE-ALL-RATIONALP-2))
     (2 2 (:REWRITE USE-ALL-RATIONALP))
     (2 2
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (2 2
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM)))
(LOGTAIL-UNGUARDED)
(LOGTAIL-UNGUARDED-CORRECT
     (120 6 (:LINEAR <=-OF-LOGTAIL-SAME-LINEAR))
     (74 6 (:DEFINITION NATP))
     (18 2 (:REWRITE LOGTAIL-BECOMES-0))
     (18 2 (:DEFINITION POSP))
     (17 10 (:REWRITE DEFAULT-<-1))
     (10 10 (:REWRITE USE-ALL-<=-2))
     (10 10 (:REWRITE USE-ALL-<=))
     (10 10 (:REWRITE USE-ALL-<-2))
     (10 10 (:REWRITE USE-ALL-<))
     (10 10 (:REWRITE DEFAULT-<-2))
     (10 10
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (8 8
        (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (8 8
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (7 7
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (7 7
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (6 6 (:TYPE-PRESCRIPTION NATP))
     (6 6 (:REWRITE USE-ALL-NATP-2))
     (6 6 (:REWRITE USE-ALL-NATP))
     (6 6
        (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (6 6 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (6 6
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (4 2
        (:REWRITE LOGTAIL-WHEN-NOT-INTEGERP-ARG2))
     (3 1
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (3 1 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (2 2 (:TYPE-PRESCRIPTION POSP))
     (2 2 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (2 2 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (2 2
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (2 2
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (2 2 (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (2 2 (:REWRITE UBP-LONGER-BETTER))
     (1 1 (:REWRITE USE-ALL-RATIONALP-2))
     (1 1 (:REWRITE USE-ALL-RATIONALP))
     (1 1
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (1 1 (:REWRITE CLR-DIFFERENTIAL))
     (1 1
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (1 1
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM)))
(GETBIT-UNGUARDED)
(GETBIT-UNGUARDED-CORRECT
    (948 21 (:REWRITE BVCHOP-IDENTITY))
    (612 49
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
    (347 16 (:DEFINITION NATP))
    (276 14 (:LINEAR <=-OF-LOGTAIL-SAME-LINEAR))
    (261 11
         (:REWRITE UNSIGNED-BYTE-P-OF-LOGTAIL-STRONG))
    (250 5 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR))
    (224 104
         (:TYPE-PRESCRIPTION LOGTAIL-NON-POSITIVE))
    (151 76 (:REWRITE DEFAULT-<-1))
    (146 146 (:TYPE-PRESCRIPTION IFIX))
    (104 104 (:TYPE-PRESCRIPTION LOGTAIL-TYPE))
    (89 76 (:REWRITE DEFAULT-<-2))
    (76 76 (:REWRITE USE-ALL-<-2))
    (76 76 (:REWRITE USE-ALL-<))
    (76 76
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
    (71 11 (:REWRITE SLICE-OUT-OF-ORDER))
    (68 4 (:REWRITE GETBIT-IDENTITY))
    (63 7 (:REWRITE SLICE-TOO-HIGH-IS-0))
    (58 6 (:DEFINITION POSP))
    (55 21
        (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER-CHEAP))
    (55 21
        (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
    (54 6 (:REWRITE LOGTAIL-BECOMES-0))
    (49 49
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
    (49 49
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
    (49 49 (:REWRITE UBP-LONGER-BETTER))
    (44 12 (:REWRITE GETBIT-WHEN-N-IS-NEGATIVE))
    (41 41 (:REWRITE USE-ALL-<=-2))
    (41 41 (:REWRITE USE-ALL-<=))
    (40 3 (:REWRITE LOGTAIL-NON-NEG))
    (38 38 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
    (38 38 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
    (35 35
        (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
    (35 35
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
    (29 29
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
    (29 29
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
    (28 11
        (:REWRITE SLICE-WHEN-LOW-IS-NEGATIVE))
    (28 11
        (:REWRITE SLICE-WHEN-INDICES-ARE-NEGATIVE))
    (27 19 (:REWRITE DEFAULT-+-2))
    (25 25 (:TYPE-PRESCRIPTION NATP))
    (24 12 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
    (21 21
        (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
    (21 21 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
    (21 21
        (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
    (21 21
        (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
    (21 21
        (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
    (21 21 (:REWRITE BVCHOP-SUBST-CONSTANT))
    (21 19 (:REWRITE DEFAULT-+-1))
    (20 12
        (:REWRITE GETBIT-WHEN-VAL-IS-NOT-AN-INTEGER))
    (20 12
        (:REWRITE GETBIT-WHEN-NOT-INTEGERP-ARG1))
    (19 19
        (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
    (19 19
        (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
    (19 19 (:REWRITE USE-ALL-NATP-2))
    (19 19 (:REWRITE USE-ALL-NATP))
    (19 19
        (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
    (19 19 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
    (19 19
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
    (19 19 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
    (15 7
        (:REWRITE SLICE-WHEN-VAL-IS-NOT-AN-INTEGER))
    (12 12
        (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
    (12 6
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
    (11 11 (:REWRITE SLICE-WHEN-BVCHOP-KNOWN))
    (11 11 (:REWRITE SLICE-TOO-HIGH-LEMMA))
    (11 11
        (:REWRITE SLICE-SUBST-IN-CONSTANT-ALT))
    (11 11 (:REWRITE SLICE-SUBST-IN-CONSTANT))
    (10 10 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR-2))
    (10 8 (:REWRITE DEFAULT-UNARY-MINUS))
    (9 7
       (:REWRITE SLICE-WHEN-VAL-IS-NOT-AN-INTEGER-CHEAP))
    (6 6 (:TYPE-PRESCRIPTION POSP))
    (6 6
       (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
    (6 6
       (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
    (4 4 (:REWRITE GETBIT-IDENTITY-FREE))
    (3 3 (:REWRITE USE-ALL-RATIONALP-2))
    (3 3 (:REWRITE USE-ALL-RATIONALP))
    (2 1 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
    (1 1
       (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
    (1 1 (:REWRITE CLR-DIFFERENTIAL)))
(BVNOT-UNGUARDED)
(BVNOT-UNGUARDED-CORRECT
     (243 10 (:DEFINITION EXPT))
     (195 7 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR))
     (169 17 (:REWRITE BVCHOP-IDENTITY))
     (133 7
          (:LINEAR BVCHOP-UPPER-BOUND-LINEAR-STRONG))
     (92 8 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (86 6 (:REWRITE <-OF-LOGNOT-AND-0))
     (83 48 (:REWRITE DEFAULT-<-2))
     (54 48 (:REWRITE DEFAULT-<-1))
     (51 17 (:REWRITE DEFAULT-*-2))
     (51 17 (:REWRITE COMMUTATIVITY-OF-+))
     (48 48 (:REWRITE USE-ALL-<-2))
     (48 48 (:REWRITE USE-ALL-<))
     (48 48
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (46 46 (:TYPE-PRESCRIPTION LOGNOT))
     (43 37 (:REWRITE DEFAULT-+-1))
     (42 6 (:REWRITE UNSIGNED-BYTE-P-OF-1))
     (39 3 (:REWRITE BVCHOP-OF-MINUS-1))
     (37 37 (:REWRITE DEFAULT-+-2))
     (36 36 (:REWRITE USE-ALL-<=-2))
     (36 36 (:REWRITE USE-ALL-<=))
     (36 6 (:DEFINITION POSP))
     (31 31 (:TYPE-PRESCRIPTION NATP))
     (30 30 (:TYPE-PRESCRIPTION IFIX))
     (28 14
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
     (27 27
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (27 27
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (26 26 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (25 25
         (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (22 8
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (21 21 (:REWRITE USE-ALL-NATP-2))
     (21 21 (:REWRITE USE-ALL-NATP))
     (21 21
         (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (21 21 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (21 21
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (21 7
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (19 19
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (17 17 (:REWRITE DEFAULT-*-1))
     (17 17 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (16 16
         (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (16 16
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (16 16
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (16 16 (:REWRITE UBP-LONGER-BETTER))
     (16 8
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (16 8
         (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (16 8
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER-CHEAP))
     (16 8
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (14 14 (:TYPE-PRESCRIPTION POSP))
     (14 14 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR-2))
     (10 10 (:REWRITE ZIP-OPEN))
     (10 10 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (10 10 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (7 7 (:REWRITE USE-ALL-RATIONALP-2))
     (7 7 (:REWRITE USE-ALL-RATIONALP))
     (7 7
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (7 7
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (6 6 (:REWRITE ZP-OPEN))
     (5 4 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (4 4
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (4 4 (:REWRITE CLR-DIFFERENTIAL))
     (3 3 (:REWRITE DEFAULT-UNARY-MINUS))
     (1 1 (:REWRITE BVCHOP-IMPOSSIBLE-VALUE)))
(BVUMINUS-UNGUARDED)
(BVUMINUS-UNGUARDED-CORRECT)
(GET-ENTRIES-EQ)
(BOOLEANP-OF-IN)
(REVERSE-FAST)
(EQUAL-LST-EXEC)
(TAKE-EVERY-NTH-AUX
     (23198 726 (:REWRITE USE-ALL-<-FOR-CAR))
     (20856 472 (:DEFINITION NAT-LISTP))
     (14400 959 (:REWRITE LEN-OF-CDR))
     (14194 7263 (:REWRITE DEFAULT-+-2))
     (11562 118
            (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (10642 118 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (10156 372 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (10072 7263 (:REWRITE DEFAULT-+-1))
     (5788 372 (:DEFINITION NATP))
     (5153 4139 (:REWRITE DEFAULT-<-2))
     (4200 4139 (:REWRITE DEFAULT-<-1))
     (4139 4139 (:REWRITE USE-ALL-<-2))
     (4139 4139 (:REWRITE USE-ALL-<))
     (4139 4139
           (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (4086 4086
           (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (4086 4086
           (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (4086 4086 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (4086 4086
           (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (3115 3115 (:REWRITE USE-ALL-<=-2))
     (3115 3115 (:REWRITE USE-ALL-<=))
     (2540 372 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (2357 76 (:REWRITE EQUAL-OF-LEN-AND-0))
     (2334 2334
           (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (2283 2283
           (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (2173 2061 (:REWRITE FOLD-CONSTS-IN-+))
     (1760 1760 (:TYPE-PRESCRIPTION NAT-LISTP))
     (1756 1625 (:REWRITE DEFAULT-CAR))
     (1616 1616 (:REWRITE DEFAULT-UNARY-MINUS))
     (1184 1184 (:TYPE-PRESCRIPTION ALL-NATP))
     (1093 1093
           (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (1093 1093
           (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (1035 1035 (:REWRITE USE-ALL-CONSP-2))
     (1035 1035 (:REWRITE USE-ALL-CONSP))
     (1035 1035 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (978 978
          (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (978 978
          (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (844 844 (:TYPE-PRESCRIPTION ALL-<))
     (814 814 (:REWRITE USE-ALL-RATIONALP-2))
     (814 814 (:REWRITE USE-ALL-RATIONALP))
     (808 808 (:REWRITE DEFAULT-REALPART))
     (808 808 (:REWRITE DEFAULT-NUMERATOR))
     (808 808 (:REWRITE DEFAULT-IMAGPART))
     (808 808 (:REWRITE DEFAULT-DENOMINATOR))
     (808 808 (:REWRITE DEFAULT-COERCE-2))
     (808 808 (:REWRITE DEFAULT-COERCE-1))
     (752 140 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (744 372
          (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (726 726
          (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (640 118
          (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
     (612 68 (:REWRITE ALL-NATP-OF-CDR))
     (508 254
          (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (508 254
          (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (422 372 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (420 140
          (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (385 385
          (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN-STRONG))
     (385 385
          (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
     (372 372 (:REWRITE USE-ALL-NATP-2))
     (372 372 (:REWRITE USE-ALL-NATP))
     (372 372
          (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (372 372
          (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (372 372
          (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (372 372
          (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (280 280 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (280 280 (:TYPE-PRESCRIPTION ALL-CONSP))
     (280 140
          (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (278 163 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (254 254 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (254 254
          (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (242 242
          (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (242 242
          (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (236 236 (:TYPE-PRESCRIPTION ALL-RATIONALP))
     (234 21
          (:REWRITE <-OF-+-CANCEL-FIRST-AND-FIRST))
     (192 140 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (168 118
          (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
     (168 118 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (163 163
          (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (163 163 (:REWRITE CLR-DIFFERENTIAL))
     (140 140
          (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (140 140
          (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (140 140
          (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (140 140
          (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (140 140
          (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (118 118
          (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
     (118 118
          (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (118 118 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (118 118 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (118 118 (:REWRITE ALL-<-TRANSITIVE))
     (82 82 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (74 74 (:REWRITE WFR-HACK5))
     (74 74
         (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (42 42
         (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (18 6
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (15 1 (:REWRITE <-OF-ACL2-COUNT-OF-NTHCDR))
     (11 1 (:DEFINITION POSP))
     (6 6
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (6 6
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (1 1 (:TYPE-PRESCRIPTION POSP)))
(TAKE-EVERY-NTH)
(BVPLUS-LST)
(KEEP-ITEMS-LESS-THAN)
(ITEMS-HAVE-LEN-UNGUARDED)
(ITEMS-HAVE-LEN-UNGUARDED-CORRECT
     (545 29 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (117 9 (:REWRITE LEN-OF-CDR))
     (84 84 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (75 75 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (75 75 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (75 75 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (75 75
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (39 24 (:REWRITE DEFAULT-<-2))
     (29 29 (:REWRITE DEFAULT-CAR))
     (24 24 (:REWRITE USE-ALL-CONSP-2))
     (24 24 (:REWRITE USE-ALL-CONSP))
     (24 24 (:REWRITE USE-ALL-<-2))
     (24 24 (:REWRITE USE-ALL-<))
     (24 24 (:REWRITE DEFAULT-<-1))
     (24 24 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (24 24
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (21 21
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
     (20 20 (:REWRITE DEFAULT-CDR))
     (15 15 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (15 15
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (11 10 (:REWRITE DEFAULT-+-2))
     (10 10 (:REWRITE DEFAULT-+-1))
     (9 9
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (6 6
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (6 6 (:REWRITE CLR-DIFFERENTIAL))
     (4 4 (:TYPE-PRESCRIPTION BOOLEANP)))
(KEEP-ITEMS-LESS-THAN-UNGUARDED)
(KEEP-ITEMS-LESS-THAN-UNGUARDED-CORRECT
     (359 19 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (336 3 (:REWRITE REVAPPEND-OF-CONS))
     (327 3 (:DEFINITION BINARY-APPEND))
     (135 3 (:REWRITE CAR-OF-REVAPPEND))
     (126 3 (:DEFINITION LAST))
     (102 3 (:REWRITE CDR-OF-REVAPPEND))
     (99 3 (:DEFINITION BUTLAST))
     (87 6 (:REWRITE LEN-OF-CDR))
     (81 3 (:REWRITE CONSP-OF-REVAPPEND))
     (78 6 (:REWRITE USE-ALL-<-FOR-CAR))
     (70 11 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
     (65 13
         (:REWRITE TRUE-LIST-FIX-WHEN-TRUE-LISTP))
     (56 40 (:REWRITE DEFAULT-<-2))
     (46 43 (:REWRITE DEFAULT-CAR))
     (44 13
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (40 40 (:REWRITE USE-ALL-<-2))
     (40 40 (:REWRITE USE-ALL-<))
     (40 40 (:REWRITE DEFAULT-<-1))
     (40 40
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (33 33 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (33 33 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (33 33 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (33 33 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (33 33
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (30 12 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (30 3 (:REWRITE TAKE-DOES-NOTHING))
     (28 28 (:TYPE-PRESCRIPTION ALL-RATIONALP))
     (26 26 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (26 26 (:REWRITE DEFAULT-CDR))
     (24 24 (:REWRITE USE-ALL-<=-2))
     (24 24 (:REWRITE USE-ALL-<=))
     (22 2 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (19 19 (:REWRITE USE-ALL-RATIONALP-2))
     (19 19 (:REWRITE USE-ALL-RATIONALP))
     (19 19 (:REWRITE USE-ALL-CONSP-2))
     (19 19 (:REWRITE USE-ALL-CONSP))
     (19 19 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (15 12 (:REWRITE DEFAULT-+-2))
     (15 3 (:REWRITE ALL-RATIONALP-OF-CDR))
     (14 14
         (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
     (14 14
         (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
     (13 13
         (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (13 13 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (13 13
         (:REWRITE TRUE-LIST-FIX-WHEN-NOT-CONSP-CHEAP))
     (13 13 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (13 13
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (12 12 (:TYPE-PRESCRIPTION ALL-<))
     (12 12 (:REWRITE DEFAULT-+-1))
     (12 12 (:REWRITE CLR-DIFFERENTIAL))
     (9 9 (:TYPE-PRESCRIPTION BOOLEANP))
     (9 9
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (9 3 (:REWRITE COMMUTATIVITY-OF-+))
     (8 4 (:DEFINITION NAT-LISTP))
     (8 2 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (8 2 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (6 6 (:TYPE-PRESCRIPTION NAT-LISTP))
     (6 6
        (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (6 6
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (6 6 (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (6 6 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (6 6 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (6 6 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (6 6 (:REWRITE ALL-<-TRANSITIVE))
     (5 5
        (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (5 5
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (4 4 (:TYPE-PRESCRIPTION ALL-NATP))
     (4 2
        (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (3 3
        (:TYPE-PRESCRIPTION TRUE-LISTP-REVAPPEND-TYPE-PRESCRIPTION))
     (3 3 (:TYPE-PRESCRIPTION REVAPPEND))
     (3 3 (:TYPE-PRESCRIPTION LAST))
     (3 3 (:REWRITE EQUAL-OF-LEN-AND-0))
     (3 3 (:REWRITE APPEND-TO-NIL))
     (2 2
        (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (2 2 (:REWRITE ALL-NATP-WHEN-NOT-CONSP)))
(ALL-ITEMS-LESS-THAN)
(CONSP-OF-ASSOC-EQUAL)
(ALISTP-CONSP-HACK-EQUAL)
(REALPART-UNGUARDED)
(REALPART-UNGUARDED-CORRECT
     (7 2 (:REWRITE DEFAULT-REALPART))
     (6 2
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (2 2 (:REWRITE USE-ALL-RATIONALP-2))
     (2 2 (:REWRITE USE-ALL-RATIONALP))
     (2 2
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (2 2
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (1 1 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (1 1
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (1 1 (:REWRITE CLR-DIFFERENTIAL)))
(IMAGPART-UNGUARDED)
(IMAGPART-UNGUARDED-CORRECT
     (7 2 (:REWRITE DEFAULT-IMAGPART))
     (6 2
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (2 2 (:REWRITE USE-ALL-RATIONALP-2))
     (2 2 (:REWRITE USE-ALL-RATIONALP))
     (2 2
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (2 2
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (1 1 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (1 1
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (1 1 (:REWRITE CLR-DIFFERENTIAL)))
(STRIP-CARS-UNGUARDED)
(STRIP-CARS-UNGUARDED-CORRECT
     (356 18 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (206 24 (:REWRITE DEFAULT-CAR))
     (92 15
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (39 3 (:REWRITE LEN-OF-CDR))
     (31 31 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (31 31 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (31 31 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (31 31 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (31 31
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (18 18
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN-STRONG))
     (18 18
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
     (15 13 (:REWRITE DEFAULT-CDR))
     (15 9 (:REWRITE DEFAULT-<-2))
     (14 5 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (11 11 (:REWRITE WFR-HACK5))
     (11 11
         (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (9 9 (:REWRITE USE-ALL-CONSP-2))
     (9 9 (:REWRITE USE-ALL-CONSP))
     (9 9 (:REWRITE USE-ALL-<-2))
     (9 9 (:REWRITE USE-ALL-<))
     (9 9 (:REWRITE DEFAULT-<-1))
     (9 9 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (9 9
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (6 6 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (6 6
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (5 5
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (5 5 (:REWRITE CLR-DIFFERENTIAL))
     (4 4 (:TYPE-PRESCRIPTION BOOLEANP))
     (3 3 (:REWRITE DEFAULT-+-2))
     (3 3 (:REWRITE DEFAULT-+-1))
     (3 3
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN)))
(STRIP-CDRS-UNGUARDED)
(STRIP-CDRS-UNGUARDED-CORRECT
     (356 18 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (206 24 (:REWRITE DEFAULT-CDR))
     (92 15
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (39 3 (:REWRITE LEN-OF-CDR))
     (31 31 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (31 31 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (31 31 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (31 31 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (31 31
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (18 18
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN-STRONG))
     (18 18
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
     (15 13 (:REWRITE DEFAULT-CAR))
     (15 9 (:REWRITE DEFAULT-<-2))
     (14 5 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (9 9 (:REWRITE USE-ALL-CONSP-2))
     (9 9 (:REWRITE USE-ALL-CONSP))
     (9 9 (:REWRITE USE-ALL-<-2))
     (9 9 (:REWRITE USE-ALL-<))
     (9 9 (:REWRITE DEFAULT-<-1))
     (9 9 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (9 9
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (6 6 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (6 6
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (5 5
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (5 5 (:REWRITE CLR-DIFFERENTIAL))
     (4 4 (:TYPE-PRESCRIPTION BOOLEANP))
     (3 3 (:REWRITE DEFAULT-+-2))
     (3 3 (:REWRITE DEFAULT-+-1))
     (3 3
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN)))
(SLICE-LESS-GUARDED)
(SLICE-LESS-GUARDED-CORRECT
     (94 2 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR-2))
     (52 50 (:REWRITE DEFAULT-+-2))
     (51 1 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR))
     (50 50 (:REWRITE DEFAULT-+-1))
     (35 26 (:REWRITE DEFAULT-<-1))
     (35 16
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (30 3 (:REWRITE LOGTAIL-WHEN-NOT-POSP-ARG1))
     (27 3 (:REWRITE LOGTAIL-BECOMES-0))
     (26 26 (:REWRITE USE-ALL-<-2))
     (26 26 (:REWRITE USE-ALL-<))
     (26 26 (:REWRITE DEFAULT-<-2))
     (26 26
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (25 25 (:REWRITE USE-ALL-<=-2))
     (25 25 (:REWRITE USE-ALL-<=))
     (24 8 (:REWRITE DEFAULT-*-2))
     (24 3 (:DEFINITION POSP))
     (22 22
         (:TYPE-PRESCRIPTION LOGTAIL-NON-POSITIVE))
     (17 2 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (16 16
         (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (16 16
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (16 16 (:REWRITE UBP-LONGER-BETTER))
     (16 16
         (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (16 16
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (15 1 (:REWRITE LOGTAIL-NON-NEG))
     (8 8
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (8 8
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (8 8 (:REWRITE DEFAULT-UNARY-MINUS))
     (8 8 (:REWRITE DEFAULT-*-1))
     (8 7 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (7 7
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (7 7 (:REWRITE CLR-DIFFERENTIAL))
     (6 2
        (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER-CHEAP))
     (6 2
        (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (5 5 (:TYPE-PRESCRIPTION POSP))
     (5 5 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (5 5 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (4 4 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (4 2
        (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
     (3 3 (:TYPE-PRESCRIPTION NATP))
     (3 2
        (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (3 2
        (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (3 1
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (2 2 (:REWRITE USE-ALL-NATP-2))
     (2 2 (:REWRITE USE-ALL-NATP))
     (2 2
        (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (2 2 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (2 2
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (2 2
        (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (2 2 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (2 2 (:LINEAR <=-OF-LOGTAIL-SAME-LINEAR))
     (1 1 (:REWRITE USE-ALL-RATIONALP-2))
     (1 1 (:REWRITE USE-ALL-RATIONALP))
     (1 1 (:REWRITE BVCHOP-IMPOSSIBLE-VALUE))
     (1 1
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (1 1
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM)))
(BVCHOP-LIST-UNGUARDED)
(BVCHOP-LIST-UNGUARDED-CORRECT
     (185 10 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (87 6 (:REWRITE BVCHOP-IDENTITY))
     (76 6 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (53 17 (:REWRITE DEFAULT-<-2))
     (42 5
         (:REWRITE BVCHOP-LIST-DOES-NOTHING-BETTER))
     (42 5 (:REWRITE BVCHOP-LIST-DOES-NOTHING))
     (33 3
         (:REWRITE BVCHOP-LIST-WHEN-ARG1-IS-NEGATIVE))
     (32 17 (:REWRITE DEFAULT-<-1))
     (32 2 (:REWRITE LEN-OF-CDR))
     (27 8
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (27 6
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (25 9
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (24 3
         (:REWRITE USE-ALL-UNSIGNED-BYTE-P-FOR-CAR))
     (21 21
         (:REWRITE USE-ALL-ALL-UNSIGNED-BYTE-P-2))
     (21 21
         (:REWRITE USE-ALL-ALL-UNSIGNED-BYTE-P))
     (21 21
         (:REWRITE ALL-UNSIGNED-BYTE-P-WHEN-BV-ARRAYP))
     (21 21
         (:REWRITE ALL-UNSIGNED-BYTE-P-FROM-ALL-UNSIGNED-BYTE-P-NARROWER))
     (20 6 (:REWRITE ALL-UNSIGNED-BYTE-P-OF-CDR))
     (18 6
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (17 17 (:REWRITE USE-ALL-<-2))
     (17 17 (:REWRITE USE-ALL-<))
     (17 17
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (15 15
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (15 15
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (15 6 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (13 13 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (13 13 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (13 13 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (13 13 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (13 13
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (12 12 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (12 12 (:TYPE-PRESCRIPTION NATP))
     (12 6
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (12 6
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (12 6
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
     (12 6
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (12 6
         (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (10 10 (:REWRITE USE-ALL-CONSP-2))
     (10 10 (:REWRITE USE-ALL-CONSP))
     (10 10 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (10 8 (:REWRITE DEFAULT-CDR))
     (10 8 (:REWRITE DEFAULT-CAR))
     (9 9
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (9 9
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (9 3
        (:REWRITE UNSIGNED-BYTE-P-OF-CAR-WHEN-UNSIGNED-BYTE-LISTP-2))
     (9 3
        (:REWRITE BVCHOP-LIST-WHEN-ARG1-IS-NOT-AN-INTEGER))
     (8 8 (:REWRITE USE-ALL-RATIONALP-2))
     (8 8 (:REWRITE USE-ALL-RATIONALP))
     (6 6
        (:TYPE-PRESCRIPTION UNSIGNED-BYTE-LISTP))
     (6 6 (:TYPE-PRESCRIPTION POSP))
     (6 6 (:TYPE-PRESCRIPTION ALL-NATP))
     (6 6 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (6 6 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (6 6 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (6 6 (:REWRITE USE-ALL-<=-2))
     (6 6 (:REWRITE USE-ALL-<=))
     (6 6
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (6 6
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (6 6 (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (6 6 (:REWRITE UBP-LONGER-BETTER))
     (6 6
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (6 6
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (6 6 (:REWRITE CLR-DIFFERENTIAL))
     (6 6
        (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER-CHEAP))
     (6 6 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (6 6 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (6 6
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (5 1
        (:REWRITE TRUE-LIST-FIX-WHEN-TRUE-LISTP))
     (4 2 (:REWRITE DEFAULT-+-2))
     (3 3 (:TYPE-PRESCRIPTION BOOLEANP))
     (2 2 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (2 2 (:REWRITE EQUAL-OF-LEN-AND-0))
     (2 2 (:REWRITE DEFAULT-+-1))
     (2 2
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (2 2 (:REWRITE BVCHOP-IMPOSSIBLE-VALUE))
     (1 1
        (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (1 1 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP)))
(BVNOT-LIST-UNGUARDED)
(BVNOT-LIST-UNGUARDED-CORRECT
     (113 6 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (78 6
         (:REWRITE BVNOT-WHEN-SIZE-IS-NOT-POSITIVE))
     (48 12 (:REWRITE DEFAULT-<-2))
     (18 6
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (18 6
         (:REWRITE BVNOT-WHEN-SIZE-IS-NOT-INTEGERP))
     (12 12 (:REWRITE USE-ALL-<-2))
     (12 12 (:REWRITE USE-ALL-<))
     (12 12 (:REWRITE DEFAULT-<-1))
     (12 12
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (11 6
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (9 3 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (7 7 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (7 7 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (7 7 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (7 7 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (7 7
        (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (6 6 (:REWRITE USE-ALL-RATIONALP-2))
     (6 6 (:REWRITE USE-ALL-RATIONALP))
     (6 6 (:REWRITE USE-ALL-CONSP-2))
     (6 6 (:REWRITE USE-ALL-CONSP))
     (6 6 (:REWRITE USE-ALL-<=-2))
     (6 6 (:REWRITE USE-ALL-<=))
     (6 6
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (6 6
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (6 6 (:REWRITE DEFAULT-CAR))
     (6 6 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (6 6
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (6 6
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (6 6 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (6 6
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (4 4 (:REWRITE DEFAULT-CDR))
     (3 3 (:TYPE-PRESCRIPTION BOOLEANP))
     (3 3
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (3 3 (:REWRITE CLR-DIFFERENTIAL)))
(BVLT-UNGUARDED)
(BVLT-UNGUARDED-CORRECT
     (411 136
          (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (374 38 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR))
     (248 33 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (216 142 (:REWRITE DEFAULT-<-1))
     (210 21 (:DEFINITION NATP))
     (151 142 (:REWRITE DEFAULT-<-2))
     (142 142 (:REWRITE USE-ALL-<-2))
     (142 142 (:REWRITE USE-ALL-<))
     (142 142
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (136 136
          (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (136 136
          (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (136 136 (:REWRITE UBP-LONGER-BETTER))
     (128 128
          (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (128 128 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (128 128 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (118 90 (:REWRITE DEFAULT-+-2))
     (114 38 (:REWRITE DEFAULT-*-2))
     (114 38 (:REWRITE COMMUTATIVITY-OF-+))
     (112 112 (:REWRITE USE-ALL-<=-2))
     (112 112 (:REWRITE USE-ALL-<=))
     (90 90 (:REWRITE DEFAULT-+-1))
     (76 30 (:REWRITE UNSIGNED-BYTE-P-OF-0-ARG1))
     (65 65
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (65 65
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (58 58 (:TYPE-PRESCRIPTION POSP))
     (45 15
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (40 40
         (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (38 38 (:REWRITE ZIP-OPEN))
     (38 38 (:REWRITE DEFAULT-*-1))
     (37 37
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (37 33
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (35 33
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (35 33
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (35 33
         (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (30 30 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (25 25 (:TYPE-PRESCRIPTION NATP))
     (23 23 (:REWRITE CLR-DIFFERENTIAL))
     (21 21 (:REWRITE USE-ALL-NATP-2))
     (21 21 (:REWRITE USE-ALL-NATP))
     (21 21
         (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (21 21 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (21 21
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (15 15 (:REWRITE USE-ALL-RATIONALP-2))
     (15 15 (:REWRITE USE-ALL-RATIONALP))
     (15 15
         (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (15 15
         (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (4 4 (:REWRITE BVCHOP-BOUND)))
(BVLE-UNGUARDED)
(BVLE-UNGUARDED-CORRECT
     (412 137
          (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (375 39 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR))
     (248 33 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (217 143 (:REWRITE DEFAULT-<-1))
     (210 21 (:DEFINITION NATP))
     (152 143 (:REWRITE DEFAULT-<-2))
     (143 143 (:REWRITE USE-ALL-<-2))
     (143 143 (:REWRITE USE-ALL-<))
     (143 143
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (137 137
          (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (137 137
          (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (137 137 (:REWRITE UBP-LONGER-BETTER))
     (129 129
          (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (129 129 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (129 129 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (120 92 (:REWRITE DEFAULT-+-2))
     (117 39 (:REWRITE DEFAULT-*-2))
     (117 39 (:REWRITE COMMUTATIVITY-OF-+))
     (113 113 (:REWRITE USE-ALL-<=-2))
     (113 113 (:REWRITE USE-ALL-<=))
     (92 92 (:REWRITE DEFAULT-+-1))
     (76 30 (:REWRITE UNSIGNED-BYTE-P-OF-0-ARG1))
     (65 65
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (65 65
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (58 58 (:TYPE-PRESCRIPTION POSP))
     (45 15
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (40 40
         (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (39 39 (:REWRITE ZIP-OPEN))
     (39 39 (:REWRITE DEFAULT-*-1))
     (37 37
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (37 33
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (35 33
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (35 33
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (35 33
         (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (30 30 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (25 25 (:TYPE-PRESCRIPTION NATP))
     (23 23 (:REWRITE CLR-DIFFERENTIAL))
     (21 21 (:REWRITE USE-ALL-NATP-2))
     (21 21 (:REWRITE USE-ALL-NATP))
     (21 21
         (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (21 21 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (21 21
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (15 15 (:REWRITE USE-ALL-RATIONALP-2))
     (15 15 (:REWRITE USE-ALL-RATIONALP))
     (15 15
         (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (15 15
         (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (4 4 (:REWRITE BVCHOP-BOUND)))
(BVXOR-LIST-UNGUARDED)
(BVXOR-LIST-UNGUARDED-CORRECT
     (293 16 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (143 11
          (:REWRITE BVXOR-WHEN-SIZE-IS-NOT-POSITIVE))
     (131 6 (:REWRITE BVCHOP-IDENTITY))
     (107 28 (:REWRITE DEFAULT-<-2))
     (88 11
         (:REWRITE BVXOR-WHEN-X-IS-NOT-AN-INTEGER))
     (63 6 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (56 6
         (:REWRITE USE-ALL-UNSIGNED-BYTE-P-FOR-CAR))
     (50 25
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (50 25
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (49 13
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (48 6
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (42 42
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (42 42
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (42 14
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (42 3 (:REWRITE LEN-OF-CDR))
     (33 11
         (:REWRITE BVXOR-WHEN-SIZE-IS-NOT-AN-INTEGER))
     (28 28 (:REWRITE USE-ALL-<-2))
     (28 28 (:REWRITE USE-ALL-<))
     (28 28 (:REWRITE DEFAULT-<-1))
     (28 28
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (25 25 (:TYPE-PRESCRIPTION ALL-NATP))
     (25 25 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (25 25
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (24 24 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (24 24 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (24 24 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (24 24 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (24 24
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (21 6
         (:REWRITE UNSIGNED-BYTE-P-OF-CAR-WHEN-UNSIGNED-BYTE-LISTP-2))
     (18 6
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (17 17 (:REWRITE USE-ALL-<=-2))
     (17 17 (:REWRITE USE-ALL-<=))
     (14 14
         (:TYPE-PRESCRIPTION UNSIGNED-BYTE-LISTP))
     (14 14
         (:TYPE-PRESCRIPTION ALL-UNSIGNED-BYTE-P))
     (14 14 (:REWRITE USE-ALL-RATIONALP-2))
     (14 14 (:REWRITE USE-ALL-RATIONALP))
     (14 14
         (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (14 14
         (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (13 13 (:REWRITE BVXOR-SUBST-ARG3))
     (13 13 (:REWRITE BVXOR-SUBST-ARG2))
     (12 12 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (12 12 (:TYPE-PRESCRIPTION NATP))
     (12 12 (:REWRITE USE-ALL-CONSP-2))
     (12 12 (:REWRITE USE-ALL-CONSP))
     (12 12 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (12 6
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
     (12 6
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (12 6
         (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (9 3 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (8 8 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (8 8
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (8 1 (:REWRITE ALL-UNSIGNED-BYTE-P-OF-CDR))
     (7 7
        (:REWRITE USE-ALL-ALL-UNSIGNED-BYTE-P-2))
     (7 7
        (:REWRITE USE-ALL-ALL-UNSIGNED-BYTE-P))
     (7 7
        (:REWRITE ALL-UNSIGNED-BYTE-P-WHEN-NOT-CONSP))
     (7 7
        (:REWRITE ALL-UNSIGNED-BYTE-P-WHEN-BV-ARRAYP))
     (7 7
        (:REWRITE ALL-UNSIGNED-BYTE-P-FROM-ALL-UNSIGNED-BYTE-P-NARROWER))
     (6 6 (:TYPE-PRESCRIPTION POSP))
     (6 6 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (6 6 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (6 6
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (6 6
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (6 6 (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (6 6 (:REWRITE UBP-LONGER-BETTER))
     (6 6
        (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER-CHEAP))
     (6 6 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (4 4
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (4 3 (:REWRITE DEFAULT-+-2))
     (3 3 (:TYPE-PRESCRIPTION BOOLEANP))
     (3 3
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (3 3 (:REWRITE DEFAULT-+-1))
     (3 3 (:REWRITE CLR-DIFFERENTIAL))
     (3 1 (:REWRITE UNSIGNED-BYTE-LISTP-OF-CDR))
     (1 1 (:REWRITE EQUAL-OF-LEN-AND-0)))
(BVOR-UNGUARDED)
(BVOR-UNGUARDED-CORRECT
     (1078 6 (:DEFINITION EVENP))
     (988 6
          (:REWRITE INTEGERP-OF-*-1/2-OF-LOGNOT))
     (922 6 (:REWRITE INTEGERP-OF-SMALL))
     (320 6 (:LINEAR BVCHOP-UPPER-BOUND))
     (246 6
          (:LINEAR BVCHOP-UPPER-BOUND-LINEAR-STRONG))
     (198 10 (:DEFINITION EXPT))
     (196 12 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR-2))
     (186 24 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (162 66
          (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (106 6 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR))
     (104 10 (:DEFINITION NATP))
     (100 74 (:REWRITE DEFAULT-<-1))
     (90 21 (:REWRITE COMMUTATIVITY-OF-+))
     (90 16 (:REWRITE BVCHOP-IMPOSSIBLE-VALUE))
     (89 74 (:REWRITE DEFAULT-<-2))
     (82 47 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (74 74 (:REWRITE USE-ALL-<-2))
     (74 74 (:REWRITE USE-ALL-<))
     (74 74
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (72 6 (:REWRITE FLOOR-WHEN-<))
     (66 66
         (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (66 66
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (66 66 (:REWRITE UBP-LONGER-BETTER))
     (66 33 (:REWRITE DEFAULT-*-2))
     (63 46 (:REWRITE DEFAULT-+-2))
     (60 60
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (56 56 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (56 56 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (56 56 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (55 46 (:REWRITE DEFAULT-+-1))
     (54 54
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (54 12 (:REWRITE MULTIPLE-WHEN-MOD-0-CHEAP))
     (48 48 (:REWRITE USE-ALL-<=-2))
     (48 48 (:REWRITE USE-ALL-<=))
     (47 47 (:REWRITE CLR-DIFFERENTIAL))
     (44 44
         (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
     (44 44
         (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
     (39 33 (:REWRITE DEFAULT-*-1))
     (36 12 (:REWRITE UNSIGNED-BYTE-P-OF-0-ARG1))
     (32 24
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (32 15 (:REWRITE LOGNOT-WHEN-NOT-INTEGERP))
     (30 30 (:TYPE-PRESCRIPTION BINARY-LOGAND))
     (30 24
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (28 28 (:TYPE-PRESCRIPTION POSP))
     (28 24
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (28 24
         (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (24 24 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (24 6 (:REWRITE COMMUTATIVITY-OF-*))
     (21 7
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (18 12
         (:TYPE-PRESCRIPTION RATIONALP-OF-MOD))
     (18 6 (:REWRITE UNICITY-OF-0))
     (17 17
         (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (17 17
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (17 17
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (16 16 (:TYPE-PRESCRIPTION NATP))
     (12 12
         (:TYPE-PRESCRIPTION NONNEG-OF-MOD-TYPE-2))
     (12 12
         (:TYPE-PRESCRIPTION INTEGERP-OF-MOD-TYPE))
     (12 6
         (:REWRITE FLOOR-WHEN-NOT-RATIONALP-OF-QUOTIENT))
     (12 6
         (:REWRITE FLOOR-WHEN-NOT-RATIONALP-ARG1))
     (12 6
         (:REWRITE FLOOR-WHEN-NEGATIVE-AND-SMALL-CHEAP))
     (12 6
         (:REWRITE FLOOR-WHEN-I-IS-NOT-AN-ACL2-NUMBERP))
     (12 6 (:DEFINITION FIX))
     (10 10 (:REWRITE USE-ALL-NATP-2))
     (10 10 (:REWRITE USE-ALL-NATP))
     (10 10
         (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (10 10 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (10 10
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (7 7 (:REWRITE USE-ALL-RATIONALP-2))
     (7 7 (:REWRITE USE-ALL-RATIONALP))
     (7 7
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (7 7
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (6 6
        (:REWRITE UNSIGNED-BYTE-P-WHEN-SIZE-IS-NEGATIVE-LIMITED))
     (6 6
        (:REWRITE UNSIGNED-BYTE-P-FALSE-WHEN-NOT-LONGER))
     (6 6 (:REWRITE INTEGERP-OF-SMALL-HELPER-2))
     (6 6
        (:REWRITE FLOOR-MINUS-NEGATIVE-CONSTANT))
     (6 6 (:REWRITE FLOOR-MINUS-ERIC-BETTER))
     (6 6 (:REWRITE BVCHOP-BOUND)))
(BVAND-UNGUARDED)
(BVAND-UNGUARDED-CORRECT
     (1000 6 (:DEFINITION EVENP))
     (922 6 (:REWRITE INTEGERP-OF-SMALL))
     (922 6 (:REWRITE FLOOR-WHEN-<))
     (640 12 (:LINEAR BVCHOP-UPPER-BOUND))
     (492 12
          (:LINEAR BVCHOP-UPPER-BOUND-LINEAR-STRONG))
     (396 20 (:DEFINITION EXPT))
     (392 24 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR-2))
     (308 38 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (212 84
          (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (212 12 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR))
     (192 18 (:DEFINITION NATP))
     (135 110 (:REWRITE DEFAULT-<-2))
     (130 110 (:REWRITE DEFAULT-<-1))
     (126 33 (:REWRITE COMMUTATIVITY-OF-+))
     (110 110 (:REWRITE USE-ALL-<-2))
     (110 110 (:REWRITE USE-ALL-<))
     (110 110
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (99 74 (:REWRITE DEFAULT-+-2))
     (90 39 (:REWRITE DEFAULT-*-2))
     (84 84
         (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (84 84
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (84 84 (:REWRITE UBP-LONGER-BETTER))
     (83 74 (:REWRITE DEFAULT-+-1))
     (82 8 (:REWRITE BVCHOP-IMPOSSIBLE-VALUE))
     (80 80
         (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
     (80 80
         (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
     (80 80 (:REWRITE USE-ALL-<=-2))
     (80 80 (:REWRITE USE-ALL-<=))
     (74 74 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (74 74 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (74 74 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (62 62
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (62 62
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (50 38
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (48 38
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (48 16 (:REWRITE UNSIGNED-BYTE-P-OF-0-ARG1))
     (44 38
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (44 38
         (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (42 42 (:TYPE-PRESCRIPTION POSP))
     (39 39 (:REWRITE DEFAULT-*-1))
     (34 25 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (28 28 (:TYPE-PRESCRIPTION NATP))
     (28 28 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (27 9
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (25 25
         (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (25 25
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (25 25 (:REWRITE CLR-DIFFERENTIAL))
     (24 6 (:REWRITE MULTIPLE-WHEN-MOD-0-CHEAP))
     (18 18 (:REWRITE USE-ALL-NATP-2))
     (18 18 (:REWRITE USE-ALL-NATP))
     (18 18
         (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (18 18 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (18 18
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (18 6 (:REWRITE UNICITY-OF-0))
     (18 6 (:REWRITE COMMUTATIVITY-OF-*))
     (12 12 (:REWRITE BVCHOP-BOUND))
     (12 6 (:DEFINITION FIX))
     (9 9 (:REWRITE USE-ALL-RATIONALP-2))
     (9 9 (:REWRITE USE-ALL-RATIONALP))
     (9 9
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (9 9
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (9 9
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (6 6 (:TYPE-PRESCRIPTION RATIONALP-OF-MOD))
     (6 6
        (:TYPE-PRESCRIPTION NONNEG-OF-MOD-TYPE-2))
     (6 6
        (:TYPE-PRESCRIPTION INTEGERP-OF-MOD-TYPE))
     (6 6
        (:REWRITE UNSIGNED-BYTE-P-WHEN-SIZE-IS-NEGATIVE-LIMITED))
     (6 6
        (:REWRITE UNSIGNED-BYTE-P-FALSE-WHEN-NOT-LONGER))
     (6 6 (:REWRITE INTEGERP-OF-SMALL-HELPER-2))
     (6 6
        (:REWRITE FLOOR-WHEN-NOT-RATIONALP-OF-QUOTIENT))
     (6 6
        (:REWRITE FLOOR-WHEN-NOT-RATIONALP-ARG1))
     (6 6
        (:REWRITE FLOOR-WHEN-NEGATIVE-AND-SMALL-CHEAP))
     (6 6
        (:REWRITE FLOOR-WHEN-I-IS-NOT-AN-ACL2-NUMBERP))
     (6 6
        (:REWRITE FLOOR-MINUS-NEGATIVE-CONSTANT))
     (6 6 (:REWRITE FLOOR-MINUS-ERIC-BETTER)))
(BVMULT-UNGUARDED)
(BVMULT-UNGUARDED-CORRECT
     (144 8 (:DEFINITION NATP))
     (140 4 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR))
     (108 4 (:DEFINITION EXPT))
     (88 88 (:TYPE-PRESCRIPTION IFIX))
     (76 4
         (:LINEAR BVCHOP-UPPER-BOUND-LINEAR-STRONG))
     (66 6 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (51 6 (:REWRITE BVCHOP-IDENTITY))
     (42 17 (:REWRITE DEFAULT-<-2))
     (26 10 (:REWRITE DEFAULT-*-2))
     (25 17 (:REWRITE DEFAULT-<-1))
     (24 8 (:REWRITE COMMUTATIVITY-OF-+))
     (20 20 (:TYPE-PRESCRIPTION NATP))
     (17 17 (:REWRITE USE-ALL-<=-2))
     (17 17 (:REWRITE USE-ALL-<=))
     (17 17 (:REWRITE USE-ALL-<-2))
     (17 17 (:REWRITE USE-ALL-<))
     (17 17
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (16 16 (:REWRITE DEFAULT-+-2))
     (16 16 (:REWRITE DEFAULT-+-1))
     (16 6
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (15 5
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (14 6
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER-CHEAP))
     (14 6
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (13 13
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (13 13
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (12 6
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
     (12 6
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (12 6
         (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (10 10 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (10 10 (:REWRITE DEFAULT-*-1))
     (8 8 (:REWRITE USE-ALL-NATP-2))
     (8 8 (:REWRITE USE-ALL-NATP))
     (8 8
        (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (8 8
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (8 8
        (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (8 8 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (8 8
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (8 8 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR-2))
     (6 6 (:TYPE-PRESCRIPTION POSP))
     (6 6 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (5 5 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (5 5 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (5 5 (:REWRITE USE-ALL-RATIONALP-2))
     (5 5 (:REWRITE USE-ALL-RATIONALP))
     (5 5
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (5 5 (:REWRITE UNSIGNED-BYTE-P-OF-*-GEN))
     (5 5
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (5 5 (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (5 5 (:REWRITE UBP-LONGER-BETTER))
     (5 5
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (5 5
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (4 4 (:REWRITE ZIP-OPEN)))
(BVMINUS-UNGUARDED)
(BVMINUS-UNGUARDED-CORRECT
     (340 6 (:REWRITE BVCHOP-IDENTITY))
     (289 5 (:REWRITE USB-PLUS-FROM-BOUNDS))
     (243 9 (:DEFINITION EXPT))
     (140 8 (:DEFINITION NATP))
     (136 4 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR))
     (129 129 (:TYPE-PRESCRIPTION IFIX))
     (76 4
         (:LINEAR BVCHOP-UPPER-BOUND-LINEAR-STRONG))
     (66 27 (:REWRITE DEFAULT-<-2))
     (66 20 (:REWRITE COMMUTATIVITY-OF-+))
     (66 6 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (57 45 (:REWRITE DEFAULT-+-1))
     (54 18 (:REWRITE DEFAULT-*-2))
     (53 45 (:REWRITE DEFAULT-+-2))
     (39 27 (:REWRITE DEFAULT-<-1))
     (27 27 (:REWRITE USE-ALL-<-2))
     (27 27 (:REWRITE USE-ALL-<))
     (27 27
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (22 22 (:REWRITE USE-ALL-<=-2))
     (22 22 (:REWRITE USE-ALL-<=))
     (20 20 (:TYPE-PRESCRIPTION NATP))
     (18 18 (:REWRITE DEFAULT-*-1))
     (16 6
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (15 5
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (14 6
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER-CHEAP))
     (14 6
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (13 13
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (13 13
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (12 6
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
     (12 6
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (12 6
         (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (11 7 (:REWRITE DEFAULT-UNARY-MINUS))
     (10 10 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (9 9 (:REWRITE ZIP-OPEN))
     (8 8 (:REWRITE USE-ALL-NATP-2))
     (8 8 (:REWRITE USE-ALL-NATP))
     (8 8
        (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (8 8
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (8 8
        (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (8 8 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (8 8
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (8 8 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR-2))
     (6 6 (:TYPE-PRESCRIPTION POSP))
     (6 6
        (:REWRITE BVCHOP-SUM-SUBST-CONST-ARG2))
     (6 6 (:REWRITE BVCHOP-SUM-SUBST-CONST))
     (6 6 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (6 6
        (:REWRITE BVCHOP-CHOP-LEADING-CONSTANT))
     (5 5 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (5 5 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (5 5 (:REWRITE USE-ALL-RATIONALP-2))
     (5 5 (:REWRITE USE-ALL-RATIONALP))
     (5 5
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (5 5
        (:REWRITE UNSIGNED-BYTE-P-OF-+-OF-CONSTANT-STRONG))
     (5 5
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (5 5 (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (5 5 (:REWRITE UBP-LONGER-BETTER))
     (5 5
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (5 5
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM)))
(BVMOD-UNGUARDED)
(BVMOD-UNGUARDED-CORRECT
     (4143 239 (:DEFINITION EXPT))
     (2584 22 (:REWRITE MOD-WHEN-<))
     (2026 42 (:REWRITE MULTIPLE-WHEN-<=))
     (1958 10
           (:REWRITE UNSIGNED-BYTE-P-OF-MOD-WHEN-<=-OF-EXPT))
     (814 604 (:REWRITE DEFAULT-+-2))
     (791 289 (:REWRITE DEFAULT-*-2))
     (741 247 (:REWRITE COMMUTATIVITY-OF-+))
     (663 255
          (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (604 604 (:REWRITE DEFAULT-+-1))
     (452 372 (:REWRITE DEFAULT-<-2))
     (450 41 (:DEFINITION NATP))
     (421 372 (:REWRITE DEFAULT-<-1))
     (413 42 (:REWRITE DEFAULT-UNARY-/))
     (372 372 (:REWRITE USE-ALL-<-2))
     (372 372 (:REWRITE USE-ALL-<))
     (372 372
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (331 289 (:REWRITE DEFAULT-*-1))
     (326 326 (:REWRITE USE-ALL-<=-2))
     (326 326 (:REWRITE USE-ALL-<=))
     (261 29
          (:REWRITE UNSIGNED-BYTE-P-OF-BVCHOP-WHEN-ALREADY))
     (255 255
          (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (255 255
          (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (255 255 (:REWRITE UBP-LONGER-BETTER))
     (248 22 (:REWRITE MOD-WHEN-MULTIPLE))
     (243 243
          (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (243 243 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (241 231
          (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
     (226 122
          (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (224 224
          (:LINEAR <=-OF-BVCHOP-SAME-LINEAR-2))
     (216 6 (:REWRITE BVCHOP-IMPOSSIBLE-VALUE))
     (194 122 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (134 122
          (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (132 122
          (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (132 122
          (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (98 98
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (98 98
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (79 79 (:TYPE-PRESCRIPTION NATP))
     (48 22
         (:REWRITE MOD-WHEN-RATIONALP-ARG1-AND-NOT-RATIONALP-ARG2))
     (44 22
         (:REWRITE MOD-WHEN-NOT-RATIONALP-ARG1-AND-RATIONALP-ARG2))
     (44 22
         (:REWRITE MOD-WHEN-NOT-ACL2-NUMBERP-ARG1))
     (42 42 (:TYPE-PRESCRIPTION FIX))
     (42 42 (:REWRITE BVCHOP-BOUND))
     (41 41 (:REWRITE USE-ALL-NATP-2))
     (41 41 (:REWRITE USE-ALL-NATP))
     (41 41
         (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (41 41
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (41 41
         (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (41 41 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (41 41
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (26 22
         (:REWRITE MOD-WHEN-NOT-ACL2-NUMBERP-ARG2))
     (24 24
         (:REWRITE MOD-WHEN-EQUAL-OF-MOD-AND-0-FREE-CHEAP))
     (21 21 (:REWRITE BVCHOP-NUMERIC-BOUND))
     (20 20 (:TYPE-PRESCRIPTION POSP))
     (20 10 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (18 6
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (10 10 (:REWRITE CLR-DIFFERENTIAL))
     (9 9 (:REWRITE NOT-<-OF-EXPT-AND-BVCHOP))
     (8 8 (:REWRITE ZIP-OPEN))
     (6 6 (:REWRITE USE-ALL-RATIONALP-2))
     (6 6 (:REWRITE USE-ALL-RATIONALP))
     (6 6
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (6 6
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (6 6
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (5 5
        (:REWRITE UNSIGNED-BYTE-P-WHEN-SIZE-IS-NEGATIVE-LIMITED))
     (5 5
        (:REWRITE UNSIGNED-BYTE-P-FALSE-WHEN-NOT-LONGER)))
(BVDIV-UNGUARDED)
(BVDIV-UNGUARDED-CORRECT
     (471 23 (:DEFINITION EXPT))
     (186 16 (:DEFINITION NATP))
     (140 38
          (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (121 25 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (101 58 (:REWRITE DEFAULT-<-2))
     (96 33 (:REWRITE DEFAULT-*-2))
     (93 31 (:REWRITE COMMUTATIVITY-OF-+))
     (87 3 (:REWRITE BVCHOP-IMPOSSIBLE-VALUE))
     (80 68 (:REWRITE DEFAULT-+-2))
     (80 2 (:REWRITE DEFAULT-UNARY-/))
     (77 25
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (70 58 (:REWRITE DEFAULT-<-1))
     (68 68 (:REWRITE DEFAULT-+-1))
     (58 58 (:REWRITE USE-ALL-<-2))
     (58 58 (:REWRITE USE-ALL-<))
     (58 58
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (52 52 (:REWRITE USE-ALL-<=-2))
     (52 52 (:REWRITE USE-ALL-<=))
     (50 35
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
     (41 25
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (38 38
         (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (38 38
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (38 38 (:REWRITE UBP-LONGER-BETTER))
     (37 26
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (36 36 (:TYPE-PRESCRIPTION NATP))
     (36 36
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (36 36
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (35 35 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (35 33 (:REWRITE DEFAULT-*-1))
     (35 25
         (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (34 34 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (34 34 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (28 28
         (:TYPE-PRESCRIPTION FLOOR-TYPE-1-PART-1-BETTER))
     (28 28 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR-2))
     (24 8
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (18 8
         (:REWRITE FLOOR-WHEN-NOT-RATIONALP-OF-QUOTIENT))
     (16 16 (:REWRITE USE-ALL-NATP-2))
     (16 16 (:REWRITE USE-ALL-NATP))
     (16 16
         (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (16 16
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (16 16
         (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (16 16 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (16 16
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (15 15 (:TYPE-PRESCRIPTION POSP))
     (15 8
         (:REWRITE FLOOR-WHEN-NEGATIVE-AND-SMALL-CHEAP))
     (15 8
         (:REWRITE FLOOR-WHEN-I-IS-NOT-AN-ACL2-NUMBERP))
     (14 7
         (:REWRITE FLOOR-WHEN-NOT-RATIONALP-ARG1))
     (8 8 (:REWRITE ZIP-OPEN))
     (8 8 (:REWRITE USE-ALL-RATIONALP-2))
     (8 8 (:REWRITE USE-ALL-RATIONALP))
     (8 8
        (:REWRITE FLOOR-MINUS-NEGATIVE-CONSTANT))
     (8 8
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (8 8
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (7 7 (:REWRITE FLOOR-MINUS-ERIC-BETTER))
     (7 4 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (6 6 (:REWRITE BVCHOP-BOUND))
     (4 4 (:REWRITE CLR-DIFFERENTIAL))
     (3 3
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (2 2
        (:REWRITE UNSIGNED-BYTE-P-WHEN-SIZE-IS-NEGATIVE-LIMITED))
     (2 2
        (:REWRITE UNSIGNED-BYTE-P-FALSE-WHEN-NOT-LONGER)))
(UNPACKBV-LESS-GUARDED)
(UNPACKBV-LESS-GUARDED-CORRECT
     (358 4 (:DEFINITION UNPACKBV))
     (82 8 (:REWRITE UNPACKBV-WHEN-ZP))
     (76 12 (:REWRITE COMMUTATIVITY-OF-*))
     (70 10 (:REWRITE ZP-OPEN))
     (66 4 (:REWRITE SLICE-TOO-HIGH-IS-0))
     (52 4 (:REWRITE DISTRIBUTIVITY))
     (48 28 (:REWRITE DEFAULT-*-2))
     (40 28 (:REWRITE DEFAULT-*-1))
     (36 2 (:REWRITE UNSIGNED-BYTE-P-OF-0-ARG2))
     (32 4 (:DEFINITION NOT))
     (24 4 (:REWRITE SLICE-WHEN-LOW-IS-NEGATIVE))
     (24 4 (:REWRITE SLICE-OUT-OF-ORDER))
     (24 2 (:DEFINITION NATP))
     (18 18 (:REWRITE USE-ALL-<-2))
     (18 18 (:REWRITE USE-ALL-<))
     (18 18 (:REWRITE DEFAULT-<-2))
     (18 18 (:REWRITE DEFAULT-<-1))
     (18 18
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (16 16 (:REWRITE DEFAULT-+-2))
     (16 16 (:REWRITE DEFAULT-+-1))
     (16 8
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (10 10 (:REWRITE USE-ALL-<=-2))
     (10 10 (:REWRITE USE-ALL-<=))
     (8 8 (:TYPE-PRESCRIPTION ZP))
     (8 8
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (8 8
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (6 6 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (6 4
        (:REWRITE SLICE-WHEN-VAL-IS-NOT-AN-INTEGER))
     (5 5
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (5 5
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (4 4 (:REWRITE USE-ALL-RATIONALP-2))
     (4 4 (:REWRITE USE-ALL-RATIONALP))
     (4 4
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (4 4
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (4 4 (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (4 4 (:REWRITE UBP-LONGER-BETTER))
     (4 4
        (:REWRITE SLICE-WHEN-VAL-IS-NOT-AN-INTEGER-CHEAP))
     (4 4 (:REWRITE SLICE-WHEN-BVCHOP-KNOWN))
     (4 4 (:REWRITE SLICE-TOO-HIGH-LEMMA))
     (4 4
        (:REWRITE SLICE-SUBST-IN-CONSTANT-ALT))
     (4 4 (:REWRITE SLICE-SUBST-IN-CONSTANT))
     (3 1 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (2 2 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (2 2 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (2 2 (:REWRITE USE-ALL-NATP-2))
     (2 2 (:REWRITE USE-ALL-NATP))
     (2 2 (:REWRITE SLICE-OF-0))
     (2 2
        (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (2 2
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (2 2 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (2 2
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (1 1 (:TYPE-PRESCRIPTION BOOLEANP))
     (1 1
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (1 1 (:REWRITE CLR-DIFFERENTIAL)))
(CHAR-CODE-UNGUARDED)
(CHAR-CODE-UNGUARDED-CORRECT
     (3 3 (:REWRITE DEFAULT-CHAR-CODE))
     (1 1 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (1 1
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (1 1 (:REWRITE CLR-DIFFERENTIAL)))
(CODE-CHAR-UNGUARDED)
(CODE-CHAR-UNGUARDED-CORRECT
     (30 5 (:REWRITE DEFAULT-CODE-CHAR))
     (9 9 (:REWRITE USE-ALL-<-2))
     (9 9 (:REWRITE USE-ALL-<))
     (9 9 (:REWRITE DEFAULT-<-2))
     (9 9 (:REWRITE DEFAULT-<-1))
     (9 9
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (8 8 (:REWRITE USE-ALL-<=-2))
     (8 8 (:REWRITE USE-ALL-<=))
     (4 4
        (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (4 4
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (4 4 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (3 3
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (3 3
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (2 1 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (1 1
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (1 1 (:REWRITE CLR-DIFFERENTIAL)))
(COERCE-UNGUARDED
     (219 11 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (27 11
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (22 11 (:REWRITE DEFAULT-<-2))
     (19 13 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (15 15 (:REWRITE DEFAULT-CAR))
     (13 13 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (13 13 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (13 13 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (13 13
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (11 11 (:REWRITE USE-ALL-CONSP-2))
     (11 11 (:REWRITE USE-ALL-CONSP))
     (11 11 (:REWRITE USE-ALL-<-2))
     (11 11 (:REWRITE USE-ALL-<))
     (11 11 (:REWRITE DEFAULT-<-1))
     (11 11 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (11 11
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (11 11 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (11 11
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (10 10 (:REWRITE DEFAULT-CDR))
     (10 5 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (5 5 (:TYPE-PRESCRIPTION BOOLEANP))
     (5 5
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (5 5 (:REWRITE CLR-DIFFERENTIAL)))
(COERCE-UNGUARDED-CORRECT
     (146 8 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (100 5 (:REWRITE DEFAULT-COERCE-3))
     (81 3 (:DEFINITION MAKE-CHARACTER-LIST))
     (44 8
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (35 35 (:TYPE-PRESCRIPTION LEN))
     (19 13 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (13 13 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (13 13 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (13 13 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (13 13
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (10 5 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (6 6 (:REWRITE DEFAULT-CDR))
     (6 6 (:REWRITE DEFAULT-CAR))
     (6 3 (:REWRITE DEFAULT-<-2))
     (5 5
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (5 5 (:REWRITE CLR-DIFFERENTIAL))
     (4 4 (:TYPE-PRESCRIPTION BOOLEANP))
     (3 3 (:REWRITE USE-ALL-CONSP-2))
     (3 3 (:REWRITE USE-ALL-CONSP))
     (3 3 (:REWRITE USE-ALL-<-2))
     (3 3 (:REWRITE USE-ALL-<))
     (3 3 (:REWRITE DEFAULT-COERCE-1))
     (3 3 (:REWRITE DEFAULT-<-1))
     (3 3 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (3 3
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (3 3 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (3 3
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH)))
(SYMBOL-PACKAGE-NAME-UNGUARDED)
(SYMBOL-PACKAGE-NAME-UNGUARDED-CORRECT
     (3 2
        (:REWRITE DEFAULT-SYMBOL-PACKAGE-NAME))
     (2 2
        (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (2 1 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (1 1
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (1 1 (:REWRITE CLR-DIFFERENTIAL)))
(SYMBOL-NAME-UNGUARDED)
(SYMBOL-NAME-UNGUARDED-CORRECT
     (3 2 (:REWRITE DEFAULT-SYMBOL-NAME))
     (2 2
        (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (1 1 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (1 1
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (1 1 (:REWRITE CLR-DIFFERENTIAL)))
(SET::IN-UNGUARDED)
(SET::IN-UNGUARDED-CORRECT (20 4 (:REWRITE SET::IN-TAIL))
                           (10 10 (:TYPE-PRESCRIPTION SET::EMPTY-TYPE))
                           (8 8 (:REWRITE SET::SUBSET-IN))
                           (6 2 (:REWRITE SET::TAIL-WHEN-EMPTY))
                           (6 2 (:REWRITE SET::NEVER-IN-EMPTY))
                           (4 4 (:REWRITE SET::SUBSET-IN-2))
                           (3 1 (:REWRITE SET::NONEMPTY-MEANS-SET))
                           (2 2 (:REWRITE SET::SUBSET-IN-2-ALT)))
(BVCAT-UNGUARDED)
(BVCAT-UNGUARDED-CORRECT
     (171 171
          (:TYPE-PRESCRIPTION NONNEG-OF-MOD-TYPE))
     (168 9 (:DEFINITION EXPT))
     (98 14 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (84 38 (:REWRITE DEFAULT-*-2))
     (75 38 (:REWRITE DEFAULT-+-2))
     (70 14 (:REWRITE BVCHOP-IDENTITY))
     (69 31 (:REWRITE DEFAULT-<-2))
     (65 38 (:REWRITE DEFAULT-+-1))
     (59 38 (:REWRITE DEFAULT-*-1))
     (44 31 (:REWRITE DEFAULT-<-1))
     (37 37
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (37 37
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (35 4 (:REWRITE DEFAULT-UNARY-/))
     (33 4 (:REWRITE MULTIPLE-WHEN-<=))
     (31 31 (:REWRITE USE-ALL-<-2))
     (31 31 (:REWRITE USE-ALL-<))
     (31 31
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (29 29 (:REWRITE USE-ALL-<=-2))
     (29 29 (:REWRITE USE-ALL-<=))
     (28 28 (:TYPE-PRESCRIPTION NATP))
     (28 14
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (28 14
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
     (28 14
         (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (28 14
         (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (28 14
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (28 3 (:REWRITE MOD-WHEN-<))
     (27 9
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (27 3 (:REWRITE MOD-WHEN-MULTIPLE))
     (19 6 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (15 9 (:REWRITE ZIP-OPEN))
     (14 14 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (14 14 (:TYPE-PRESCRIPTION POSP))
     (14 14
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER-CHEAP))
     (14 14 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (11 11
         (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (11 11
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (9 9 (:REWRITE USE-ALL-RATIONALP-2))
     (9 9 (:REWRITE USE-ALL-RATIONALP))
     (9 9
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (9 9
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (8 8
        (:REWRITE MOD-WHEN-EQUAL-OF-MOD-AND-0-FREE-CHEAP))
     (8 3
        (:REWRITE MOD-WHEN-RATIONALP-ARG1-AND-NOT-RATIONALP-ARG2))
     (8 3
        (:REWRITE MOD-WHEN-NOT-ACL2-NUMBERP-ARG2))
     (7 7 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (7 7 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (7 7
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (7 7
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (7 7 (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (7 7 (:REWRITE UBP-LONGER-BETTER))
     (6 6 (:REWRITE CLR-DIFFERENTIAL))
     (4 4
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (3 3
        (:REWRITE MOD-WHEN-NOT-RATIONALP-ARG1-AND-RATIONALP-ARG2))
     (3 3
        (:REWRITE MOD-WHEN-NOT-ACL2-NUMBERP-ARG1)))
(BVIF-UNGUARDED)
(BVIF-UNGUARDED-CORRECT
     (52 4 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
     (36 4 (:REWRITE BVCHOP-IDENTITY))
     (24 4 (:REWRITE DEFAULT-<-2))
     (12 4
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (12 4
         (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
     (12 4
         (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
     (8 8 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (8 8 (:TYPE-PRESCRIPTION NATP))
     (8 8
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (8 8
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (8 4
        (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
     (8 4
        (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
     (8 4
        (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
     (4 4 (:TYPE-PRESCRIPTION POSP))
     (4 4 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
     (4 4 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
     (4 4 (:REWRITE USE-ALL-RATIONALP-2))
     (4 4 (:REWRITE USE-ALL-RATIONALP))
     (4 4 (:REWRITE USE-ALL-<=-2))
     (4 4 (:REWRITE USE-ALL-<=))
     (4 4 (:REWRITE USE-ALL-<-2))
     (4 4 (:REWRITE USE-ALL-<))
     (4 4
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
     (4 4
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
     (4 4 (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
     (4 4 (:REWRITE UBP-LONGER-BETTER))
     (4 4 (:REWRITE DEFAULT-<-1))
     (4 4
        (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER-CHEAP))
     (4 4 (:REWRITE BVCHOP-SUBST-CONSTANT))
     (4 4
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (4 4
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (4 4
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (1 1 (:REWRITE BVIF-WHEN-TRUE-CHEAP))
     (1 1 (:REWRITE BVIF-TRIM-CONSTANT-2))
     (1 1 (:REWRITE BVIF-TRIM-CONSTANT-1)))
(EQL-BECOMES-EQL)
(NEGATED-ELEMS-LISTP-UNGUARDED)
(NEGATED-ELEMS-LISTP-UNGUARDED-CORRECT
     (774 41 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (220 52
          (:REWRITE NEGATED-ELEMS-LISTP-REWRITE))
     (132 65 (:REWRITE DEFAULT-<-2))
     (84 84 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (74 74 (:REWRITE DEFAULT-CAR))
     (70 65 (:REWRITE DEFAULT-<-1))
     (65 65 (:REWRITE USE-ALL-<-2))
     (65 65 (:REWRITE USE-ALL-<))
     (65 65
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (58 58 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (58 58 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (58 58 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (58 58 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (58 58
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (52 4 (:REWRITE LEN-OF-CDR))
     (42 42
         (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (42 42 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (42 42
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (42 42
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (41 41 (:REWRITE USE-ALL-CONSP-2))
     (41 41 (:REWRITE USE-ALL-CONSP))
     (41 41 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (38 38 (:REWRITE DEFAULT-CDR))
     (38 19
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (38 19
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (37 37 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (37 37
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (31 31
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (31 31 (:REWRITE CLR-DIFFERENTIAL))
     (24 24 (:REWRITE USE-ALL-<=-2))
     (24 24 (:REWRITE USE-ALL-<=))
     (21 7
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (19 19 (:TYPE-PRESCRIPTION ALL-NATP))
     (19 19 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (19 19
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (11 11
         (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (11 11
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (7 7 (:REWRITE USE-ALL-RATIONALP-2))
     (7 7 (:REWRITE USE-ALL-RATIONALP))
     (7 7
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
     (7 7
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (4 4 (:REWRITE DEFAULT-+-2))
     (4 4 (:REWRITE DEFAULT-+-1))
     (4 4
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN)))
(FIRSTN-UNGUARDED)
(FIRSTN-UNGUARDED-CORRECT
     (99 3 (:REWRITE FIRSTN-WHEN-<=-OF-LEN))
     (18 14 (:REWRITE DEFAULT-<-1))
     (17 14 (:REWRITE DEFAULT-<-2))
     (14 14 (:REWRITE USE-ALL-<=-2))
     (14 14 (:REWRITE USE-ALL-<=))
     (14 14 (:REWRITE USE-ALL-<-2))
     (14 14 (:REWRITE USE-ALL-<))
     (14 14
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (14 4 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (13 13 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (13 13 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (13 13 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (13 13 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (13 13
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (8 8
        (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (7 7
        (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (7 7
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (5 5
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (5 5
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (4 4 (:REWRITE CLR-DIFFERENTIAL))
     (3 3 (:TYPE-PRESCRIPTION NFIX))
     (3 3
        (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (3 3 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (3 3
        (:REWRITE TRUE-LIST-FIX-WHEN-NOT-CONSP-CHEAP))
     (3 3 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (3 3
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (1 1 (:TYPE-PRESCRIPTION BOOLEANP))
     (1 1 (:REWRITE USE-ALL-NATP-2))
     (1 1 (:REWRITE USE-ALL-NATP))
     (1 1
        (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (1 1 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (1 1
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (1 1
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE)))
(GETBIT-IS-ALWAYS-0-UNGUARDED)
(GETBIT-IS-ALWAYS-0-UNGUARDED-CORRECT
    (832 22 (:REWRITE GETBIT-IDENTITY))
    (414 22
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
    (321 17 (:REWRITE CONSP-FROM-LEN-CHEAP))
    (220 70 (:REWRITE DEFAULT-<-1))
    (176 22
         (:REWRITE USE-ALL-UNSIGNED-BYTE-P-FOR-CAR))
    (176 22 (:REWRITE USE-ALL-<-FOR-CAR))
    (98 26
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
    (96 48 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
    (86 70 (:REWRITE DEFAULT-<-2))
    (70 70 (:REWRITE USE-ALL-<-2))
    (70 70 (:REWRITE USE-ALL-<))
    (70 70
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
    (66 22
        (:REWRITE UNSIGNED-BYTE-P-OF-CAR-WHEN-UNSIGNED-BYTE-LISTP-2))
    (55 55
        (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
    (48 48
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
    (48 48
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
    (48 24
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
    (48 24
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
    (44 44
        (:TYPE-PRESCRIPTION UNSIGNED-BYTE-LISTP))
    (44 44
        (:TYPE-PRESCRIPTION ALL-UNSIGNED-BYTE-P))
    (44 44 (:TYPE-PRESCRIPTION ALL-<))
    (30 30 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
    (30 30 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
    (30 30 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
    (30 30 (:REWRITE LEN-WHEN-BV-ARRAYP))
    (30 30
        (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
    (30 30 (:REWRITE DEFAULT-CAR))
    (26 26 (:REWRITE USE-ALL-RATIONALP-2))
    (26 26 (:REWRITE USE-ALL-RATIONALP))
    (26 26
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
    (26 26
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
    (24 24 (:TYPE-PRESCRIPTION ALL-NATP))
    (24 24 (:TYPE-PRESCRIPTION ALL-INTEGERP))
    (24 24
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
    (22 22
        (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
    (22 22
        (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
    (22 22 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
    (22 22 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
    (22 22
        (:REWRITE USE-ALL-ALL-UNSIGNED-BYTE-P-2))
    (22 22
        (:REWRITE USE-ALL-ALL-UNSIGNED-BYTE-P))
    (22 22
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
    (22 22
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
    (22 22 (:REWRITE UBP-LONGER-BETTER))
    (22 22 (:REWRITE GETBIT-IDENTITY-FREE))
    (22 22
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
    (22 22 (:REWRITE CLR-DIFFERENTIAL))
    (22 22
        (:REWRITE ALL-UNSIGNED-BYTE-P-WHEN-NOT-CONSP))
    (22 22
        (:REWRITE ALL-UNSIGNED-BYTE-P-WHEN-BV-ARRAYP))
    (22 22
        (:REWRITE ALL-UNSIGNED-BYTE-P-FROM-ALL-UNSIGNED-BYTE-P-NARROWER))
    (22 22
        (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
    (22 22 (:REWRITE ALL-<-WHEN-NOT-CONSP))
    (22 22 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
    (22 22 (:REWRITE ALL-<-TRANSITIVE-FREE))
    (22 22 (:REWRITE ALL-<-TRANSITIVE))
    (22 22 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
    (20 4 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
    (17 17 (:REWRITE USE-ALL-CONSP-2))
    (17 17 (:REWRITE USE-ALL-CONSP))
    (17 17 (:REWRITE CONSP-WHEN-LEN-GREATER))
    (16 16 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
    (16 16
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
    (13 1 (:REWRITE LEN-OF-CDR))
    (12 12 (:REWRITE DEFAULT-CDR))
    (9 9 (:REWRITE USE-ALL-<=-2))
    (9 9 (:REWRITE USE-ALL-<=))
    (9 9
       (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
    (9 9
       (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
    (8 8 (:TYPE-PRESCRIPTION ALL-RATIONALP))
    (4 4
       (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
    (4 4
       (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
    (1 1 (:REWRITE DEFAULT-+-2))
    (1 1 (:REWRITE DEFAULT-+-1))
    (1 1
       (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN)))
(GETBIT-IS-ALWAYS-1-UNGUARDED)
(GETBIT-IS-ALWAYS-1-UNGUARDED-CORRECT
    (832 22 (:REWRITE GETBIT-IDENTITY))
    (414 22
         (:REWRITE UNSIGNED-BYTE-P-FROM-BOUNDS))
    (375 20 (:REWRITE CONSP-FROM-LEN-CHEAP))
    (223 73 (:REWRITE DEFAULT-<-1))
    (176 22
         (:REWRITE USE-ALL-UNSIGNED-BYTE-P-FOR-CAR))
    (176 22 (:REWRITE USE-ALL-<-FOR-CAR))
    (98 26
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
    (96 48 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
    (92 73 (:REWRITE DEFAULT-<-2))
    (73 73 (:REWRITE USE-ALL-<-2))
    (73 73 (:REWRITE USE-ALL-<))
    (73 73
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
    (66 22
        (:REWRITE UNSIGNED-BYTE-P-OF-CAR-WHEN-UNSIGNED-BYTE-LISTP-2))
    (55 55
        (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
    (48 48
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
    (48 48
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
    (48 24
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
    (48 24
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
    (44 44
        (:TYPE-PRESCRIPTION UNSIGNED-BYTE-LISTP))
    (44 44
        (:TYPE-PRESCRIPTION ALL-UNSIGNED-BYTE-P))
    (44 44 (:TYPE-PRESCRIPTION ALL-<))
    (33 33 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
    (33 33 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
    (33 33 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
    (33 33 (:REWRITE LEN-WHEN-BV-ARRAYP))
    (33 33
        (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
    (30 30 (:REWRITE DEFAULT-CAR))
    (26 26 (:REWRITE USE-ALL-RATIONALP-2))
    (26 26 (:REWRITE USE-ALL-RATIONALP))
    (26 26
        (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
    (26 26
        (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
    (24 24 (:TYPE-PRESCRIPTION ALL-NATP))
    (24 24 (:TYPE-PRESCRIPTION ALL-INTEGERP))
    (24 24
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
    (22 22
        (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
    (22 22
        (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
    (22 22 (:REWRITE USE-ALL-UNSIGNED-BYTE-P-2))
    (22 22 (:REWRITE USE-ALL-UNSIGNED-BYTE-P))
    (22 22
        (:REWRITE USE-ALL-ALL-UNSIGNED-BYTE-P-2))
    (22 22
        (:REWRITE USE-ALL-ALL-UNSIGNED-BYTE-P))
    (22 22
        (:REWRITE UNSIGNED-BYTE-P-WHEN-<=-CHEAP))
    (22 22
        (:REWRITE UNSIGNED-BYTE-P-FROM-BOUND))
    (22 22 (:REWRITE UBP-LONGER-BETTER))
    (22 22 (:REWRITE GETBIT-IDENTITY-FREE))
    (22 22
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
    (22 22 (:REWRITE CLR-DIFFERENTIAL))
    (22 22
        (:REWRITE ALL-UNSIGNED-BYTE-P-WHEN-NOT-CONSP))
    (22 22
        (:REWRITE ALL-UNSIGNED-BYTE-P-WHEN-BV-ARRAYP))
    (22 22
        (:REWRITE ALL-UNSIGNED-BYTE-P-FROM-ALL-UNSIGNED-BYTE-P-NARROWER))
    (22 22
        (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
    (22 22 (:REWRITE ALL-<-WHEN-NOT-CONSP))
    (22 22 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
    (22 22 (:REWRITE ALL-<-TRANSITIVE-FREE))
    (22 22 (:REWRITE ALL-<-TRANSITIVE))
    (22 22 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
    (20 20 (:REWRITE USE-ALL-CONSP-2))
    (20 20 (:REWRITE USE-ALL-CONSP))
    (20 20 (:REWRITE CONSP-WHEN-LEN-GREATER))
    (20 4 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
    (19 19 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
    (19 19
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
    (13 1 (:REWRITE LEN-OF-CDR))
    (12 12 (:REWRITE DEFAULT-CDR))
    (9 9 (:REWRITE USE-ALL-<=-2))
    (9 9 (:REWRITE USE-ALL-<=))
    (9 9
       (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
    (9 9
       (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
    (8 8 (:TYPE-PRESCRIPTION ALL-RATIONALP))
    (4 4
       (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
    (4 4
       (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
    (1 1 (:REWRITE DEFAULT-+-2))
    (1 1 (:REWRITE DEFAULT-+-1))
    (1 1
       (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN)))
(AXE-EVALUATOR-FUNCTION-INFO)
(GET-CALLED-FNS-AUX
     (5732 14 (:DEFINITION ACL2-COUNT))
     (4584 18 (:DEFINITION INTEGER-ABS))
     (4382 57 (:REWRITE USE-ALL-<-FOR-CAR))
     (3997 34 (:DEFINITION NAT-LISTP))
     (2526 27 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (2344 51 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (2137 25 (:REWRITE ALL-NATP-OF-CDR))
     (2018 27 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (1898 74 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (1844 12 (:REWRITE ALL-<-OF-CDR))
     (1367 93 (:REWRITE LEN-OF-CDR))
     (814 34 (:DEFINITION NATP))
     (431 34 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (370 10 (:REWRITE CDR-IFF))
     (297 51 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (295 189 (:REWRITE DEFAULT-CAR))
     (254 170 (:REWRITE DEFAULT-+-2))
     (248 248 (:TYPE-PRESCRIPTION NAT-LISTP))
     (223 140 (:REWRITE DEFAULT-CDR))
     (219 3 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
     (204 170 (:REWRITE DEFAULT-+-1))
     (192 96
          (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (191 7
          (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
     (184 182 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (182 182 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (182 182 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (182 182 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (182 182
          (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (176 176 (:TYPE-PRESCRIPTION ALL-NATP))
     (168 168
          (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN-STRONG))
     (168 168
          (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
     (145 139 (:REWRITE DEFAULT-<-2))
     (140 139 (:REWRITE DEFAULT-<-1))
     (139 139 (:REWRITE USE-ALL-<-2))
     (139 139 (:REWRITE USE-ALL-<))
     (139 139
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (102 51
          (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (100 100
          (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (99 4 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (96 96 (:TYPE-PRESCRIPTION ALL-<))
     (88 46 (:REWRITE FOLD-CONSTS-IN-+))
     (83 83 (:REWRITE USE-ALL-<=-2))
     (83 83 (:REWRITE USE-ALL-<=))
     (82 41
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (82 41
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (82 4 (:REWRITE ALL-RATIONALP-OF-CDR))
     (74 74 (:REWRITE USE-ALL-CONSP-2))
     (74 74 (:REWRITE USE-ALL-CONSP))
     (74 74 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (72 72
         (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (72 72
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (72 18 (:REWRITE COMMUTATIVITY-OF-+))
     (72 9 (:DEFINITION LENGTH))
     (69 69
         (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (67 8 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (58 4 (:REWRITE ALL-CONSP-OF-CDR))
     (57 57
         (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (57 27 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (51 51
         (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (47 47
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (47 47
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (41 41 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (41 41
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (35 35 (:REWRITE WFR-HACK5))
     (35 35
         (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (34 34 (:REWRITE USE-ALL-NATP-2))
     (34 34 (:REWRITE USE-ALL-NATP))
     (34 34
         (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (34 34 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (34 34
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (34 17
         (:REWRITE INTEGERP-OF-CAR-OF-CAR-WHEN-WEAK-DAGP-AUX-CHEAP))
     (31 31 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (31 31
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (31 31 (:REWRITE CLR-DIFFERENTIAL))
     (27 27
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (27 27 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (27 27 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (27 27 (:REWRITE ALL-<-TRANSITIVE))
     (21 21
         (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (21 21
         (:REWRITE <-OF-0-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (20 20
         (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (20 20 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (20 20
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (18 18
         (:TYPE-PRESCRIPTION ADD-TO-SET-EQUAL))
     (18 18 (:REWRITE DEFAULT-UNARY-MINUS))
     (17 17 (:TYPE-PRESCRIPTION WEAK-DAGP-AUX))
     (17 17
         (:REWRITE INTEGERP-OF-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (16 16 (:TYPE-PRESCRIPTION ALL-CONSP))
     (15 15 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (15 15
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (14 14 (:TYPE-PRESCRIPTION ALL-RATIONALP))
     (12 4
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (10 10 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (10 10
         (:REWRITE NATP-OF-CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (9 9 (:REWRITE USE-ALL-RATIONALP-2))
     (9 9 (:REWRITE USE-ALL-RATIONALP))
     (9 9 (:REWRITE DEFAULT-REALPART))
     (9 9 (:REWRITE DEFAULT-NUMERATOR))
     (9 9 (:REWRITE DEFAULT-IMAGPART))
     (9 9 (:REWRITE DEFAULT-DENOMINATOR))
     (9 9 (:REWRITE DEFAULT-COERCE-2))
     (9 9 (:REWRITE DEFAULT-COERCE-1))
     (8 8 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (8 8
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (8 4
        (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (7 7
        (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
     (4 4
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (4 4
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (4 4
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX)))
(FLAG-GET-CALLED-FNS-AUX
     (5748 66 (:DEFINITION INTEGER-ABS))
     (5014 77 (:REWRITE USE-ALL-<-FOR-CAR))
     (4549 46 (:DEFINITION NAT-LISTP))
     (3059 129 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (2812 31 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (2614 63 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (2332 31 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (2155 27 (:REWRITE ALL-NATP-OF-CDR))
     (1928 138 (:REWRITE LEN-OF-CDR))
     (1844 12 (:REWRITE ALL-<-OF-CDR))
     (998 46 (:DEFINITION NATP))
     (709 414 (:REWRITE DEFAULT-+-2))
     (587 259 (:REWRITE DEFAULT-CAR))
     (531 414 (:REWRITE DEFAULT-+-1))
     (511 46 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (473 230 (:REWRITE DEFAULT-CDR))
     (370 10 (:REWRITE CDR-IFF))
     (312 312 (:TYPE-PRESCRIPTION NAT-LISTP))
     (312 308 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (309 63 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (308 308 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (308 308 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (308 308 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (308 308
          (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (277 251 (:REWRITE DEFAULT-<-2))
     (269 14 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (264 66 (:REWRITE COMMUTATIVITY-OF-+))
     (264 33 (:DEFINITION LENGTH))
     (254 251 (:REWRITE DEFAULT-<-1))
     (251 251 (:REWRITE USE-ALL-<-2))
     (251 251 (:REWRITE USE-ALL-<))
     (251 251
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (239 7 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
     (214 214 (:TYPE-PRESCRIPTION ALL-NATP))
     (212 106
          (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (203 1 (:DEFINITION SYMBOL-ALISTP))
     (195 11
          (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
     (187 24 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (182 182
          (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN-STRONG))
     (182 182
          (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
     (168 11 (:REWRITE ALL-CONSP-OF-CDR))
     (165 111 (:REWRITE FOLD-CONSTS-IN-+))
     (149 149 (:REWRITE USE-ALL-<=-2))
     (149 149 (:REWRITE USE-ALL-<=))
     (136 136
          (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (136 136
          (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (128 128 (:REWRITE USE-ALL-CONSP-2))
     (128 128 (:REWRITE USE-ALL-CONSP))
     (128 128 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (126 63
          (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (123 123
          (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (120 120 (:TYPE-PRESCRIPTION ALL-<))
     (105 105
          (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (98 49
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (98 49
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (82 4 (:REWRITE ALL-RATIONALP-OF-CDR))
     (77 77
         (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (75 75
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (75 75
         (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (66 66 (:REWRITE DEFAULT-UNARY-MINUS))
     (63 63
         (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (61 31 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (53 47 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (49 49 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (49 49 (:TYPE-PRESCRIPTION ALL-CONSP))
     (49 49
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (47 47
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (47 47 (:REWRITE CLR-DIFFERENTIAL))
     (46 46 (:REWRITE USE-ALL-NATP-2))
     (46 46 (:REWRITE USE-ALL-NATP))
     (46 46
         (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P-SIZE-ARG))
     (46 46 (:REWRITE NATP-WHEN-UNSIGNED-BYTE-P))
     (46 46
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (44 44 (:REWRITE WFR-HACK5))
     (44 44
         (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (41 14
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (34 17
         (:REWRITE INTEGERP-OF-CAR-OF-CAR-WHEN-WEAK-DAGP-AUX-CHEAP))
     (33 33 (:REWRITE USE-ALL-RATIONALP-2))
     (33 33 (:REWRITE USE-ALL-RATIONALP))
     (33 33 (:REWRITE DEFAULT-REALPART))
     (33 33 (:REWRITE DEFAULT-NUMERATOR))
     (33 33 (:REWRITE DEFAULT-IMAGPART))
     (33 33 (:REWRITE DEFAULT-DENOMINATOR))
     (33 33 (:REWRITE DEFAULT-COERCE-2))
     (33 33 (:REWRITE DEFAULT-COERCE-1))
     (32 32 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (32 32
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (31 31
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (31 31 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (31 31 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (31 31 (:REWRITE ALL-<-TRANSITIVE))
     (29 29
         (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (28 14
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (27 27 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (24 24
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (22 22 (:TYPE-PRESCRIPTION ALL-RATIONALP))
     (21 21
         (:REWRITE <-OF-0-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (20 20
         (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (20 20 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (20 20
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (17 17 (:TYPE-PRESCRIPTION WEAK-DAGP-AUX))
     (17 17
         (:REWRITE INTEGERP-OF-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (16 2 (:TYPE-PRESCRIPTION RETURN-LAST))
     (16 1 (:REWRITE EQUAL-OF-LEN-AND-0))
     (14 14
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (14 14
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (14 14
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (11 11
         (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
     (10 10 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (10 10
         (:REWRITE NATP-OF-CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (6 6 (:TYPE-PRESCRIPTION UNION-EQUAL))
     (6 3
        (:REWRITE SYMBOL-ALISTP-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (6 1 (:REWRITE SYMBOL-ALISTP-OF-CDR))
     (4 4 (:TYPE-PRESCRIPTION BOOLEANP))
     (2 2
        (:TYPE-PRESCRIPTION THROW-NONEXEC-ERROR))
     (2 2
        (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
     (2 1
        (:REWRITE SYMBOLP-OF-CAR-WHEN-SYMBOL-LISTP-CHEAP))
     (2 1
        (:REWRITE SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-ALISTP-CHEAP))
     (1 1 (:TYPE-PRESCRIPTION SYMBOL-LISTP))
     (1 1
        (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (1 1
        (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
     (1 1
        (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP)))
(FLAG::FLAG-EQUIV-LEMMA)
(FLAG-GET-CALLED-FNS-AUX-EQUIVALENCES)
(FLAG-LEMMA-FOR-SYMBOL-LISTP-OF-GET-CALLED-FNS-AUX
     (21093 782 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (6598 1067 (:REWRITE DEFAULT-CDR))
     (2239 96 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (1766 1703
           (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (1678 1678
           (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (1678 1678
           (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (1678 1678 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (1678 1678
           (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (1471 191 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (1095 832 (:REWRITE DEFAULT-+-2))
     (1061 616 (:REWRITE DEFAULT-<-2))
     (896 99 (:REWRITE ALL-CONSP-OF-CDR))
     (832 832 (:REWRITE DEFAULT-+-1))
     (700 700 (:REWRITE USE-ALL-CONSP-2))
     (700 700 (:REWRITE USE-ALL-CONSP))
     (700 700 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (616 616 (:REWRITE USE-ALL-<-2))
     (616 616 (:REWRITE USE-ALL-<))
     (616 616 (:REWRITE DEFAULT-<-1))
     (616 616
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (587 587
          (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN-STRONG))
     (587 587
          (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
     (584 420 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (556 12 (:DEFINITION STRIP-CARS))
     (420 420
          (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (420 420 (:REWRITE CLR-DIFFERENTIAL))
     (383 383
          (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (382 382 (:TYPE-PRESCRIPTION ALL-CONSP))
     (313 313
          (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (313 313
          (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (290 145
          (:REWRITE PSEUDO-TERM-LISTP-WHEN-SYMBOL-LISTP-CHEAP))
     (280 96
          (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (265 265
          (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (192 96
          (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (191 191
          (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (184 184 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (180 180 (:REWRITE EQUAL-OF-LEN-AND-0))
     (177 177 (:REWRITE WFR-HACK5))
     (177 177
          (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (145 145
          (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (129 129
          (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (120 120
          (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
     (114 57
          (:REWRITE SYMBOL-ALISTP-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (101 101
          (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (96 96
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (96 96
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (96 96
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (70 70 (:TYPE-PRESCRIPTION BOOLEANP))
     (69 69
         (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (69 69 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (46 46
         (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (37 37
         (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
     (19 19
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (18 16
         (:REWRITE PSEUDO-TERMP-OF-LAMBDA-BODY-CHEAP)))
(SYMBOL-LISTP-OF-GET-CALLED-FNS-AUX)
(SYMBOL-LISTP-OF-GET-CALLED-FNS-AUX-LST)
(GET-CALLED-FNS-AUX
     (4835 320 (:REWRITE DEFAULT-CAR))
     (4341 533 (:REWRITE DEFAULT-CDR))
     (1591 12 (:DEFINITION SYMBOL-ALISTP))
     (1419 54 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (1053 1028
           (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (1016 1016
           (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (1016 1016
           (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (1016 1016 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (1016 1016
           (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (901 133 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (736 80 (:REWRITE ALL-CONSP-OF-CDR))
     (644 606 (:REWRITE DEFAULT-+-2))
     (606 606 (:REWRITE DEFAULT-+-1))
     (418 298 (:REWRITE DEFAULT-<-2))
     (402 281 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (368 368 (:REWRITE USE-ALL-CONSP-2))
     (368 368 (:REWRITE USE-ALL-CONSP))
     (368 368 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (298 298 (:REWRITE USE-ALL-<-2))
     (298 298 (:REWRITE USE-ALL-<))
     (298 298 (:REWRITE DEFAULT-<-1))
     (298 298
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (281 281
          (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (281 281 (:REWRITE CLR-DIFFERENTIAL))
     (267 267 (:TYPE-PRESCRIPTION ALL-CONSP))
     (186 186
          (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN-STRONG))
     (186 186
          (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
     (161 54
          (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (145 5 (:DEFINITION MEMBER-EQUAL))
     (133 133
          (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (108 54
          (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (108 4 (:DEFINITION GET-CALLED-FNS-AUX-LST))
     (107 107 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (102 51
          (:REWRITE PSEUDO-TERM-LISTP-WHEN-SYMBOL-LISTP-CHEAP))
     (100 100
          (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (100 100
          (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (92 92 (:REWRITE WFR-HACK5))
     (92 92
         (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (74 1 (:DEFINITION ALISTP))
     (72 36
         (:REWRITE SYMBOL-ALISTP-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (72 12 (:REWRITE SYMBOL-ALISTP-OF-CDR))
     (54 54
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (54 54
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (54 54
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (52 52
         (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (51 51
         (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (49 49
         (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
     (48 1 (:DEFINITION STRIP-CARS))
     (44 44 (:TYPE-PRESCRIPTION BOOLEANP))
     (42 21
         (:REWRITE SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-ALISTP-CHEAP))
     (35 35
         (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (32 32
         (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
     (30 30
         (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (24 24
         (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (24 24 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (12 12
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (8 8
        (:REWRITE PSEUDO-TERMP-OF-LAMBDA-BODY-CHEAP))
     (8 1 (:REWRITE ALISTP-OF-CDR))
     (6 3
        (:REWRITE ALISTP-WHEN-PSEUDO-TERM-ALISTP-CHEAP))
     (3 3
        (:TYPE-PRESCRIPTION PSEUDO-TERM-ALISTP))
     (3 3
        (:REWRITE ALISTP-WHEN-PSEUDO-DAGP-AUX))
     (3 3
        (:REWRITE ALISTP-WHEN-BOUNDED-NATP-ALISTP)))
(GET-CALLED-FNS)
(GET-IMMEDIATE-SUPPORTING-FNS)
(SYMBOL-LISTP-OF-GET-IMMEDIATE-SUPPORTING-FNS
     (1253 1 (:DEFINITION GET-CALLED-FNS-AUX))
     (996 26 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (572 39 (:REWRITE LEN-OF-CDR))
     (501 18 (:REWRITE DEFAULT-CAR))
     (380 32 (:REWRITE DEFAULT-CDR))
     (243 243 (:TYPE-PRESCRIPTION LEN))
     (149 81 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (128 1 (:DEFINITION SYMBOL-ALISTP))
     (87 1 (:DEFINITION QUOTEP))
     (81 81
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (81 81 (:REWRITE CLR-DIFFERENTIAL))
     (75 39 (:REWRITE DEFAULT-+-2))
     (75 15
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (72 68 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (71 4 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (68 68 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (68 68 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (68 68 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (68 68
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (66 66 (:TYPE-PRESCRIPTION BOOLEANP))
     (52 1 (:DEFINITION SAFE-UNQUOTE))
     (49 21 (:REWRITE DEFAULT-<-2))
     (48 1 (:DEFINITION STRIP-CARS))
     (41 1 (:DEFINITION SYMBOL-LISTP))
     (40 10 (:REWRITE FOLD-CONSTS-IN-+))
     (39 39 (:REWRITE DEFAULT-+-1))
     (39 8 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (33 4 (:REWRITE ALL-CONSP-OF-CDR))
     (31 1 (:DEFINITION UNION-EQUAL))
     (26 26 (:REWRITE EQUAL-OF-LEN-AND-0))
     (25 25 (:REWRITE USE-ALL-CONSP-2))
     (25 25 (:REWRITE USE-ALL-CONSP))
     (25 25 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (21 21 (:REWRITE USE-ALL-<-2))
     (21 21 (:REWRITE USE-ALL-<))
     (21 21 (:REWRITE DEFAULT-<-1))
     (21 21
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (20 10
         (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (19 19
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN-STRONG))
     (19 19
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
     (17 17
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (17 17
         (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (16 16 (:TYPE-PRESCRIPTION ALL-CONSP))
     (13 13
         (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (12 4
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (8 8 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (8 8
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (8 8 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (8 8
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (8 4
        (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (7 2
        (:REWRITE SYMBOLP-OF-CAR-WHEN-SYMBOL-LISTP-CHEAP))
     (7 1 (:REWRITE SYMBOL-LISTP-OF-CDR))
     (6 6 (:REWRITE WFR-HACK5))
     (6 6
        (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (6 3
        (:REWRITE SYMBOL-ALISTP-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (6 1 (:REWRITE SYMBOL-ALISTP-OF-CDR))
     (5 5 (:TYPE-PRESCRIPTION SYMBOL-ALISTP))
     (4 4 (:TYPE-PRESCRIPTION STRIP-CARS))
     (4 4
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (4 4
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (4 4
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (2 2
        (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (2 2
        (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
     (2 2
        (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
     (2 1
        (:REWRITE SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-ALISTP-CHEAP))
     (2 1 (:DEFINITION ADD-TO-SET-EQUAL))
     (1 1
        (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP)))
(GET-ALL-SUPPORTING-FNS-AUX
     (951 45 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (609 21 (:DEFINITION MEMBER-EQUAL))
     (92 47 (:REWRITE DEFAULT-<-2))
     (80 4 (:REWRITE LEN-OF-CDR))
     (78 39 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (62 62 (:REWRITE DEFAULT-CAR))
     (56 55 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (55 55 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (55 55 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (55 55 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (55 55
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (47 47 (:REWRITE USE-ALL-<-2))
     (47 47 (:REWRITE USE-ALL-<))
     (47 47 (:REWRITE DEFAULT-CDR))
     (47 47 (:REWRITE DEFAULT-<-1))
     (47 47
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (45 45 (:REWRITE USE-ALL-CONSP-2))
     (45 45 (:REWRITE USE-ALL-CONSP))
     (45 45 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (41 41 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (41 41
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (39 39 (:TYPE-PRESCRIPTION BOOLEANP))
     (39 39
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (39 39 (:REWRITE CLR-DIFFERENTIAL))
     (25 1 (:DEFINITION BINARY-APPEND))
     (20 4 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (16 16
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN-STRONG))
     (16 16
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
     (16 8
         (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (15 15
         (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (15 15
         (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
     (12 12
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (12 4
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (10 6 (:REWRITE DEFAULT-+-2))
     (8 8 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (8 8 (:TYPE-PRESCRIPTION ALL-CONSP))
     (8 4
        (:REWRITE SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-ALISTP-CHEAP))
     (8 4
        (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (6 6 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (6 6 (:REWRITE DEFAULT-+-1))
     (5 1
        (:REWRITE TRUE-LIST-FIX-WHEN-TRUE-LISTP))
     (4 4 (:TYPE-PRESCRIPTION SYMBOL-ALISTP))
     (4 4 (:REWRITE WFR-HACK5))
     (4 4
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (4 4 (:REWRITE EQUAL-OF-LEN-AND-0))
     (4 4
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (4 4
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (4 4
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (4 4
        (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (4 4
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (4 4 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (2 2 (:REWRITE ZP-OPEN))
     (2 2 (:REWRITE USE-ALL-<=-2))
     (2 2 (:REWRITE USE-ALL-<=))
     (2 2
        (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (2 2 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (2 2
        (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
     (2 2
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (1 1
        (:REWRITE TRUE-LIST-FIX-WHEN-NOT-CONSP-CHEAP)))
(SYMBOL-LISTP-OF-GET-ALL-SUPPORTING-FNS-AUX
     (1079 57 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (114 57 (:REWRITE DEFAULT-<-2))
     (88 57
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (80 80 (:REWRITE DEFAULT-CAR))
     (75 3 (:DEFINITION BINARY-APPEND))
     (64 63 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (63 63 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (63 63 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (63 63 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (63 63
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (57 57 (:REWRITE USE-ALL-CONSP-2))
     (57 57 (:REWRITE USE-ALL-CONSP))
     (57 57 (:REWRITE USE-ALL-<-2))
     (57 57 (:REWRITE USE-ALL-<))
     (57 57 (:REWRITE DEFAULT-<-1))
     (57 57 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (57 57
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (57 57 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (57 57
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (54 54 (:REWRITE DEFAULT-CDR))
     (53 27 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (27 27
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (27 27 (:REWRITE CLR-DIFFERENTIAL))
     (26 26 (:TYPE-PRESCRIPTION BOOLEANP))
     (26 26
         (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
     (26 26
         (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
     (20 20 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (10 10 (:REWRITE ZP-OPEN))
     (6 6 (:REWRITE DEFAULT-+-2))
     (6 6 (:REWRITE DEFAULT-+-1))
     (5 1
        (:REWRITE TRUE-LIST-FIX-WHEN-TRUE-LISTP))
     (1 1
        (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (1 1 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (1 1
        (:REWRITE TRUE-LIST-FIX-WHEN-NOT-CONSP-CHEAP)))
(GET-NON-BUILT-IN-SUPPORTING-FNS-LIST)
(MAKE-ACONS-NEST)
(EQUAL-OF-TRUE-LIST-FIX-AND-LIST-OF-CAR
     (178 7 (:DEFINITION TRUE-LIST-FIX))
     (75 15
         (:REWRITE TRUE-LIST-FIX-WHEN-TRUE-LISTP))
     (30 30 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (28 15 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (26 26 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (23 23 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (23 23 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (23 23 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (23 23
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (15 15
         (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (15 15 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (15 15
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (15 15 (:REWRITE CLR-DIFFERENTIAL))
     (12 9 (:REWRITE DEFAULT-CDR))
     (12 6 (:REWRITE DEFAULT-<-2))
     (6 6 (:TYPE-PRESCRIPTION BOOLEANP))
     (6 6 (:REWRITE USE-ALL-CONSP-2))
     (6 6 (:REWRITE USE-ALL-CONSP))
     (6 6 (:REWRITE USE-ALL-<-2))
     (6 6 (:REWRITE USE-ALL-<))
     (6 6 (:REWRITE DEFAULT-<-1))
     (6 6 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (6 6
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (6 6 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (6 6
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (4 2 (:REWRITE DEFAULT-+-2))
     (2 2 (:REWRITE DEFAULT-+-1)))
(CONSP-OF-LOOKUP-EQUAL-WHEN-ITEMS-HAVE-LEN-OF-STRIP-CDRS
     (346 30 (:REWRITE DEFAULT-CDR))
     (256 56 (:REWRITE DEFAULT-CAR))
     (238 119
          (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (163 139 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (156 2 (:REWRITE CONSP-OF-ASSOC-EQUAL))
     (142 2 (:DEFINITION ALISTP))
     (139 139 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (139 139 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (139 139 (:REWRITE LEN-WHEN-BV-ARRAYP))
     (139 139
          (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (125 125
          (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (84 44 (:REWRITE DEFAULT-<-2))
     (59 59
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN-STRONG))
     (59 59
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
     (44 44 (:REWRITE USE-ALL-<-2))
     (44 44 (:REWRITE USE-ALL-<))
     (44 44 (:REWRITE DEFAULT-<-1))
     (44 44
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (39 39 (:REWRITE USE-ALL-CONSP-2))
     (39 39 (:REWRITE USE-ALL-CONSP))
     (39 39 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (38 28 (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
     (36 8 (:REWRITE ITEMS-HAVE-LEN-OF-CDR))
     (32 32 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (32 32
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (30 6 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (28 28
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (28 28 (:REWRITE CLR-DIFFERENTIAL))
     (25 13 (:REWRITE DEFAULT-+-2))
     (18 6
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (15 15 (:REWRITE WFR-HACK5))
     (15 15
         (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (13 13 (:REWRITE DEFAULT-+-1))
     (12 12 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (12 12 (:TYPE-PRESCRIPTION ALL-CONSP))
     (12 6
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (10 10 (:TYPE-PRESCRIPTION BOOLEANP))
     (10 10
         (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (8 4
        (:REWRITE ALISTP-WHEN-PSEUDO-TERM-ALISTP-CHEAP))
     (6 6
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (6 6
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (6 6
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (6 6
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (6 6 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (5 5 (:REWRITE USE-ALL-<=-2))
     (5 5 (:REWRITE USE-ALL-<=))
     (4 4
        (:TYPE-PRESCRIPTION PSEUDO-TERM-ALISTP))
     (4 4
        (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (4 4 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
     (4 4
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (4 4
        (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
     (4 4
        (:REWRITE ALISTP-WHEN-PSEUDO-DAGP-AUX))
     (4 4
        (:REWRITE ALISTP-WHEN-BOUNDED-NATP-ALISTP))
     (4 2 (:REWRITE ALISTP-OF-CDR)))
(SUPPORTING-NON-BASE-FNS
    (21950 10986
           (:REWRITE EQUAL-OF-BOOLEANS-CHEAP))
    (13590 11432 (:REWRITE DEFAULT-CAR))
    (10986 10986
           (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
    (10986 10986 (:REWRITE CLR-DIFFERENTIAL))
    (10922 10922 (:TYPE-PRESCRIPTION BOOLEANP))
    (3106 57 (:REWRITE USE-ALL-CONSP-FOR-CAR))
    (2826 66 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
    (2350 413 (:REWRITE DEFAULT-CDR))
    (1564 34 (:DEFINITION STRIP-CDRS))
    (1258 17
          (:REWRITE ALL-INTERPRETED-FUNCTION-INFOP-WHEN-NOT-CONSP))
    (1130 25 (:REWRITE ALL-CONSP-OF-CDR))
    (728 711 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
    (711 711 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
    (711 711 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
    (711 711 (:REWRITE LEN-WHEN-BV-ARRAYP))
    (711 711
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
    (561 285 (:REWRITE DEFAULT-<-2))
    (334 334
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN-STRONG))
    (334 334
         (:REWRITE LEN-OF-CAR-WHEN-ITEMS-HAVE-LEN))
    (306 306 (:REWRITE USE-ALL-CONSP-2))
    (306 306 (:REWRITE USE-ALL-CONSP))
    (306 306 (:REWRITE CONSP-WHEN-LEN-GREATER))
    (285 285 (:REWRITE USE-ALL-<-2))
    (285 285 (:REWRITE USE-ALL-<))
    (285 285 (:REWRITE DEFAULT-<-1))
    (285 285
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
    (255 255
         (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
    (255 255
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
    (198 198 (:REWRITE WFR-HACK5))
    (198 198
         (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
    (141 90 (:REWRITE DEFAULT-+-2))
    (139 57
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
    (132 132 (:TYPE-PRESCRIPTION ALL-CONSP))
    (114 57
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
    (102 102 (:TYPE-PRESCRIPTION STRIP-CDRS))
    (100 100
         (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
    (90 90 (:REWRITE DEFAULT-+-1))
    (84 84
        (:REWRITE ASSOC-EQUAL-WHEN-PSEUDO-DAGP-AUX))
    (84 42
        (:REWRITE IFF-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
    (82 82 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
    (75 3 (:DEFINITION BINARY-APPEND))
    (70 35
        (:REWRITE SYMBOL-ALISTP-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
    (66 66
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
    (62 1
        (:REWRITE INTERPRETED-FUNCTION-INFOP-OF-CDR-OF-ASSOC-EQUAL))
    (57 57
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
    (57 57
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
    (57 57
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
    (46 46
        (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
    (46 46
        (:REWRITE SYMBOLP-OF-CAR-WHEN-BOUNDED-DAG-EXPRP))
    (34 17
        (:REWRITE ALL-INTERPRETED-FUNCTION-INFOP-WHEN-NOT-CONSP-CHEAP))
    (27 27
        (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
    (27 27 (:REWRITE TRUE-LISTP-WHEN-BV-ARRAYP))
    (26 26
        (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
    (24 12
        (:REWRITE ALISTP-WHEN-PSEUDO-TERM-ALISTP-CHEAP))
    (22 22
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
    (22 1 (:LINEAR LEN-OF-CDR-LINEAR-STRONG))
    (20 1 (:LINEAR LEN-OF-CDR-LINEAR))
    (18 18 (:REWRITE USE-ALL-<=-2))
    (18 18 (:REWRITE USE-ALL-<=))
    (14 14
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
    (14 14
        (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
    (14 14
        (:REWRITE PSEUDO-TERMP-OF-LAMBDA-BODY-CHEAP))
    (14 7
        (:REWRITE PSEUDO-TERM-LISTP-WHEN-SYMBOL-LISTP-CHEAP))
    (12 12
        (:TYPE-PRESCRIPTION PSEUDO-TERM-ALISTP))
    (12 12
        (:REWRITE ALISTP-WHEN-PSEUDO-DAGP-AUX))
    (11 11
        (:REWRITE SIZE-NON-NEGATIVE-WHEN-UNSIGNED-BYTE-P-FREE))
    (11 11
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
    (9 9
       (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
    (9 9
       (:REWRITE INTEGERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
    (7 7
       (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
    (7 7
       (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
    (6 6
       (:REWRITE ALISTP-WHEN-BOUNDED-NATP-ALISTP))
    (3 1
       (:REWRITE USE-ALL-INTERPRETED-FUNCTION-INFOP))
    (3 1
       (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
    (2 2 (:TYPE-PRESCRIPTION MEMBERP))
    (1 1 (:REWRITE USE-ALL-RATIONALP-2))
    (1 1 (:REWRITE USE-ALL-RATIONALP))
    (1 1
       (:REWRITE USE-ALL-INTERPRETED-FUNCTION-INFOP-2))
    (1 1
       (:REWRITE ACL2-NUMBERP-WHEN-UNSIGNED-BYTE-P))
    (1 1
       (:REWRITE ACL2-NUMBERP-FROM-UNSIGNED-BYTE-P-SIZE-PARAM))
    (1 1 (:REWRITE <-OF-+-OF-1-STRENGTHEN)))
(SUPPORTING-INTERPRETED-FUNCTION-ALIST)
(WRAP-DAG-IN-DAG-VAL)
(EMBED-DAG-IN-TERM)
