...
(defthm alen1-of-populate-candidate-deps-array-aux
  (implies (and (natp n)
                ...
                (pseudo-dag-arrayp 'dag-array dag-array dag-array-len)
                (< max dag-array-len))
           (equal (alen1 'candidate-deps-array (populate-candidate-deps-array-aux n max candidate-deps-array dag-array dag-array-len))
                  (alen1 'candidate-deps-array candidate-deps-array)))
  :hints (("Goal" :in-theory (enable populate-candidate-deps-array-aux))))
...
(defthm alen1-of-populate-candidate-deps-array
  (implies (and (subst-candidate-listp subst-candidates)
                ...
                (or (endp subst-candidates)
                    (<= (maxelem (strip-cars subst-candidates))
                        (+ -1 dag-array-len))))
           (equal (alen1 'candidate-deps-array (populate-candidate-deps-array subst-candidates dag-array dag-array-len))
                  (+ 1 (maxelem (strip-cars subst-candidates)))))
  :hints (("Goal" :in-theory (enable populate-candidate-deps-array))))
...
;; Returns a list of subst-candidates suitabkle for simultaneous checking
...
(defun drop-irrelevant-subst-candidates (subst-candidates max-literal-nodenum acc)
  (declare (xargs :guard (and (subst-candidate-listp subst-candidates)
...
(defun mark-replacements (subst-candidates translation-array)
  (declare (xargs :guard (and (subst-candidate-listp subst-candidates)
...
(defthm translation-arrayp-aux-of-mark-replacements
  ...
  :hints (("Goal" :in-theory (enable dargp-of-cadr-when-subst-candidatep))))
...
(local (in-theory (disable set-difference-equal))) ;prevent inductions
...
;;            ;; Find a set of candidates that can be substituted together (may fine none due to self deps?):
;;            (candidate-deps-array (populate-candidate-deps-array subst-candidates dag-array dag-len))
;;            (subst-candidates (find-simultaneous-subst-candidates subst-candidates candidate-deps-array nil nil nil)))
;;         (if (not subst-candidates)
;;             ;; No change:
;;             (prog2$ (cw "No candidates are suitable.~%")
;;                     (mv (erp-nil) nil nil literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist))
...
;;                (translation-array (mark-replacements subst-candidates translation-array)) ;(aset1 'translation-array translation-array nodenum-to-replace new-nodenum-or-quotep))
...
;;   (implies (and (mv-nth 2 (substitute-var-set literal-nodenums all-literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist print))
;;                 (subsetp-equal literal-nodenums all-literal-nodenums))
;;            (< (len (mv-nth 3 (substitute-var-set literal-nodenums all-literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist print)))
...
;;   (substitute-var-set literal-nodenums all-literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist print)
...
;; ;;   (implies (and (mv-nth 2 (substitute-var-set literal-nodenums all-literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist print))
;; ;;                 (subsetp-equal literal-nodenums all-literal-nodenums))
;; ;;            (<= (mv-nth 5 (substitute-var-set literal-nodenums all-literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist print))
;; ;;                2147483646))
;; ;;   :hints (("Goal" :in-theory (enable SUBSTITUTE-VAR-SET))))

;; (defthm nat-listp-of-mv-nth-3-of-substitute-var-set
;;   (implies (and (wf-dagp 'dag-array dag-array dag-len 'dag-parent-array dag-parent-array dag-constant-alist dag-variable-alist)
;;                 (not (mv-nth 0 (substitute-var-set literal-nodenums all-literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist print)))
;;                 (nat-listp literal-nodenums)
;;                 (nat-listp all-literal-nodenums)
;;                 (all-< literal-nodenums dag-len)
;;                 (all-< all-literal-nodenums dag-len))
;;            (nat-listp (mv-nth 3 (substitute-var-set literal-nodenums all-literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist print))))
;;   :hints (("Goal" :in-theory (enable substitute-var-set))))

;; (defthm true-listp-of-mv-nth-3-of-substitute-var-set
;;   (implies (true-listp all-literal-nodenums)
;;            (true-listp (mv-nth 3 (substitute-var-set literal-nodenums all-literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist print))))
;;   :rule-classes (:rewrite :type-prescription)
;;   :hints (("Goal" :in-theory (enable substitute-var-set))))
...
;;                 (all-< literal-nodenums dag-len)
;;                 (all-< all-literal-nodenums dag-len))
;;            (all-< (mv-nth 3 (substitute-var-set literal-nodenums all-literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist print))
;;                   (mv-nth 5 (substitute-var-set literal-nodenums all-literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist print))))
;;   :hints (("Goal" :in-theory (enable substitute-var-set))))
...
;;        ((mv erp provedp changep literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist)
;;         (substitute-var-set literal-nodenums literal-nodenums dag-array dag-len dag-parent-array dag-constant-alist dag-variable-alist print))
