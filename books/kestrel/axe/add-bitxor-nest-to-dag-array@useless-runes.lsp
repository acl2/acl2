(ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX
     (632 316
          (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
     (404 404 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (316 316
          (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
     (227 20 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (205 7
          (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (145 40 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (140 25
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (105 11 (:REWRITE USE-ALL-<-FOR-CAR))
     (100 14 (:DEFINITION MEMBER-EQUAL))
     (79 31 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (57 25 (:REWRITE DEFAULT-<-1))
     (56 2
         (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
     (46 2 (:REWRITE <-OF-CAR-AND-ALEN1))
     (40 40 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (40 40 (:REWRITE ALL-<-TRANSITIVE))
     (38 38 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (33 25 (:REWRITE DEFAULT-<-2))
     (32 16
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (32 4
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (31 31
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (30 30 (:REWRITE DEFAULT-CAR))
     (25 25 (:REWRITE USE-ALL-<-2))
     (25 25 (:REWRITE USE-ALL-<))
     (21 21 (:REWRITE DEFAULT-CDR))
     (21 9 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (20 20 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (20 20 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (20 20 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (20 20
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (20 10
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (20 4 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
     (18 9
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (16 16 (:TYPE-PRESCRIPTION NO-ATOMS))
     (16 16 (:REWRITE USE-ALL-CONSP-2))
     (16 16 (:REWRITE USE-ALL-CONSP))
     (16 16 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (15 9
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (12 12 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (11 7
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (11 1 (:REWRITE LEN-OF-CDR))
     (10 4 (:REWRITE USE-ALL-DARGP-LESS-THAN))
     (9 9
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (9 9
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (9 9
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (9 9 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (9 9
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (9 1
        (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (8 8 (:TYPE-PRESCRIPTION ALL-RATIONALP))
     (8 4
        (:REWRITE DARGP-LESS-THAN-WHEN-MYQUOTEP-CHEAP))
     (8 1 (:REWRITE ALL-<-OF-CDR))
     (7 7
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (6 6 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (6 6 (:TYPE-PRESCRIPTION MEMBERP))
     (6 6 (:TYPE-PRESCRIPTION ALL-CONSP))
     (6 6
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (6 6 (:LINEAR ARRAY2P-LINEAR))
     (4 4 (:TYPE-PRESCRIPTION MYQUOTEP))
     (4 4 (:REWRITE USE-ALL-RATIONALP-2))
     (4 4 (:REWRITE USE-ALL-RATIONALP))
     (4 4 (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
     (4 4
        (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-NTH-0-AND-QUOTE))
     (4 4
        (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
     (4 4
        (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
     (4 2
        (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (3 3 (:REWRITE USE-ALL-NATP-2))
     (3 3 (:REWRITE USE-ALL-NATP))
     (3 3 (:REWRITE USE-ALL-<=-2))
     (3 3 (:REWRITE USE-ALL-<=))
     (3 3
        (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (3 3
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (3 3 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (3 3 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (2 2 (:REWRITE USE-ALL-DARGP-2))
     (2 2 (:REWRITE USE-ALL-DARGP))
     (2 2 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (2 2 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (2 1
        (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (1 1 (:REWRITE DEFAULT-+-2))
     (1 1 (:REWRITE DEFAULT-+-1))
     (1 1
        (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
     (1 1 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
     (1 1
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (1 1
        (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (1 1 (:REWRITE ALL-NATP-WHEN-NOT-CONSP)))
(NATP-OF-MV-NTH-3-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX
     (96 5 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (22 2 (:REWRITE LEN-OF-CDR))
     (18 2
         (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (17 17 (:REWRITE DEFAULT-CAR))
     (12 9 (:REWRITE DEFAULT-<-2))
     (9 9 (:REWRITE USE-ALL-<-2))
     (9 9 (:REWRITE USE-ALL-<))
     (9 9 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (9 9 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (9 9 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (9 9
        (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (9 9 (:REWRITE DEFAULT-<-1))
     (9 9
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (5 5 (:REWRITE USE-ALL-CONSP-2))
     (5 5 (:REWRITE USE-ALL-CONSP))
     (5 5 (:REWRITE DEFAULT-CDR))
     (5 5 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (5 5 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (5 5
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (4 4 (:REWRITE USE-ALL-<=-2))
     (4 4 (:REWRITE USE-ALL-<=))
     (4 4
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (4 4
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (2 2 (:REWRITE DEFAULT-+-2))
     (2 2 (:REWRITE DEFAULT-+-1))
     (2 2
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (1 1 (:REWRITE USE-ALL-NATP-2))
     (1 1 (:REWRITE USE-ALL-NATP))
     (1 1
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN)))
(BOUND-ON-MV-NTH-3-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX-3
 (96 5 (:REWRITE CONSP-FROM-LEN-CHEAP))
 (45
  1
  (:LINEAR
       BOUND-ON-MV-NTH-3-OF-ADD-FUNCTION-CALL-EXPR-TO-DAG-ARRAY-WITH-NAME-4))
 (43 12 (:REWRITE DEFAULT-<-2))
 (24 12 (:REWRITE DEFAULT-<-1))
 (22 2 (:REWRITE LEN-OF-CDR))
 (20 10 (:TYPE-PRESCRIPTION ALEN1-TYPE))
 (18 18 (:REWRITE DEFAULT-CAR))
 (18 2
     (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
 (16 1
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (12 12 (:TYPE-PRESCRIPTION ARRAY1P))
 (12 12 (:REWRITE USE-ALL-<-2))
 (12 12 (:REWRITE USE-ALL-<))
 (12 12
     (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
 (10 10 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
 (10 10 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
 (10 10 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
 (10 10 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
 (10 10
     (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
 (7 7 (:REWRITE USE-ALL-<=-2))
 (7 7 (:REWRITE USE-ALL-<=))
 (5 5 (:REWRITE USE-ALL-CONSP-2))
 (5 5 (:REWRITE USE-ALL-CONSP))
 (5 5 (:REWRITE DEFAULT-CDR))
 (5 5 (:REWRITE CONSP-WHEN-LEN-GREATER))
 (5 5 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
 (5 5
    (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
 (4 1
    (:REWRITE RATIONALP-OF-ALEN1-WHEN-ARRAY1P))
 (3 3
    (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
 (3 3
    (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
 (2 2 (:REWRITE DEFAULT-+-2))
 (2 2 (:REWRITE DEFAULT-+-1))
 (2 2
    (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
 (1 1 (:REWRITE USE-ALL-RATIONALP-2))
 (1 1 (:REWRITE USE-ALL-RATIONALP))
 (1 1
    (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP)))
(BOUND-ON-MV-NTH-3-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX-3-GEN
 (140 7 (:REWRITE CONSP-FROM-LEN-CHEAP))
 (52 18 (:REWRITE DEFAULT-<-2))
 (45
  1
  (:LINEAR
       BOUND-ON-MV-NTH-3-OF-ADD-FUNCTION-CALL-EXPR-TO-DAG-ARRAY-WITH-NAME-4))
 (44 4 (:REWRITE LEN-OF-CDR))
 (36 4
     (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
 (30 18 (:REWRITE DEFAULT-<-1))
 (27 27 (:REWRITE DEFAULT-CAR))
 (22 3
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (20 10 (:TYPE-PRESCRIPTION ALEN1-TYPE))
 (18 18 (:REWRITE USE-ALL-<-2))
 (18 18 (:REWRITE USE-ALL-<))
 (18 18
     (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
 (13 13 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
 (13 13 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
 (13 13 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
 (13 13
     (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
 (12 12 (:TYPE-PRESCRIPTION ARRAY1P))
 (11 11 (:REWRITE USE-ALL-<=-2))
 (11 11 (:REWRITE USE-ALL-<=))
 (10 10 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
 (9 9 (:REWRITE DEFAULT-CDR))
 (8
   1
   (:LINEAR <=-OF-MV-NTH-3-OF-ADD-FUNCTION-CALL-EXPR-TO-DAG-ARRAY-WITH-NAME))
 (7 7 (:REWRITE USE-ALL-CONSP-2))
 (7 7 (:REWRITE USE-ALL-CONSP))
 (7 7 (:REWRITE CONSP-WHEN-LEN-GREATER))
 (7 7 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
 (7 7
    (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
 (5 5 (:REWRITE DEFAULT-+-2))
 (5 5 (:REWRITE DEFAULT-+-1))
 (4 4
    (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
 (4 1
    (:REWRITE RATIONALP-OF-ALEN1-WHEN-ARRAY1P))
 (4
  1
  (:LINEAR
   BOUND-ON-MV-NTH-3-AND-MV-NTH-1-OF-ADD-FUNCTION-CALL-EXPR-TO-DAG-ARRAY-WITH-NAME))
 (3 3 (:REWRITE USE-ALL-RATIONALP-2))
 (3 3 (:REWRITE USE-ALL-RATIONALP))
 (3 3
    (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
 (3 3
    (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
 (2 2
    (:TYPE-PRESCRIPTION BOUNDED-DAG-CONSTANT-ALISTP))
 (1 1
    (:REWRITE BOUNDED-DAG-CONSTANT-ALISTP-MONOTONE))
 (1 1
    (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP)))
(TYPE-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX
     (632 316
          (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
     (463 35 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (417 417 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (316 316
          (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
     (221 8
          (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (171 9 (:REWRITE USE-ALL-<-FOR-CAR))
     (165 15 (:REWRITE LEN-OF-CDR))
     (140 140 (:REWRITE DEFAULT-CAR))
     (135 15
          (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (121 33 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (110 13 (:DEFINITION MEMBER-EQUAL))
     (105 2 (:REWRITE <-OF-CAR-AND-ALEN1))
     (96 35
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (64 64 (:REWRITE DEFAULT-CDR))
     (59 35 (:REWRITE DEFAULT-<-1))
     (56 2
         (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
     (52 25 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (41 35 (:REWRITE DEFAULT-<-2))
     (40 40 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (40 40 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (40 40 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (40 40
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (35 35 (:REWRITE USE-ALL-<-2))
     (35 35 (:REWRITE USE-ALL-<))
     (34 34 (:REWRITE USE-ALL-CONSP-2))
     (34 34 (:REWRITE USE-ALL-CONSP))
     (34 34 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (33 33 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (33 33 (:REWRITE ALL-<-TRANSITIVE))
     (31 31 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (26 13
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (25 25
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (24 3
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (23 11 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (22 11
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (21 21 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (21 21
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (20 20
         (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (20 2 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (17 11
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (15 15 (:REWRITE DEFAULT-+-2))
     (15 15 (:REWRITE DEFAULT-+-1))
     (15 3 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
     (14 7
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (13 13 (:TYPE-PRESCRIPTION NO-ATOMS))
     (11 11
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (11 11
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (11 11
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (9 9 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (9 1 (:REWRITE ALL-NATP-OF-CDR))
     (8 1 (:REWRITE ALL-<-OF-CDR))
     (7 3 (:REWRITE USE-ALL-DARGP-LESS-THAN))
     (6 6 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (6 6 (:TYPE-PRESCRIPTION ALL-RATIONALP))
     (6 6 (:TYPE-PRESCRIPTION ALL-CONSP))
     (6 6
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (6 6 (:LINEAR ARRAY2P-LINEAR))
     (6 3
        (:REWRITE DARGP-LESS-THAN-WHEN-MYQUOTEP-CHEAP))
     (6 3
        (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (4 4 (:TYPE-PRESCRIPTION MEMBERP))
     (4 4 (:REWRITE USE-ALL-NATP-2))
     (4 4 (:REWRITE USE-ALL-NATP))
     (4 4
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (4 4
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (4 2
        (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (3 3 (:TYPE-PRESCRIPTION MYQUOTEP))
     (3 3 (:REWRITE USE-ALL-RATIONALP-2))
     (3 3 (:REWRITE USE-ALL-RATIONALP))
     (3 3 (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
     (3 3 (:REWRITE USE-ALL-<=-2))
     (3 3 (:REWRITE USE-ALL-<=))
     (3 3
        (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-NTH-0-AND-QUOTE))
     (3 3
        (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
     (3 3
        (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
     (3 3
        (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (3 3 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (3 3
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (3 3 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (3 3 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (2 2 (:REWRITE USE-ALL-DARGP-2))
     (2 2 (:REWRITE USE-ALL-DARGP))
     (2 2
        (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (2 2 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (2 2 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (1 1
        (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
     (1 1 (:REWRITE DARGP-WHEN-CONSP-CHEAP)))
(TYPE-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX-ONE)
(TYPE-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX-TWO
     (192 96
          (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
     (105 105 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (96 96
         (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
     (39 39 (:TYPE-PRESCRIPTION NATP))
     (2 2 (:LINEAR ARRAY2P-LINEAR)))
(TYPE-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX-THREE)
(TYPE-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX-FOUR)
(BOUNDED-DAG-PARENT-ARRAYP-OF-MV-NTH-4-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX)
(BOUNDED-DAG-DAG-CONSTANT-ALISTP-OF-MV-NTH-5-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX)
(BOUNDED-DAG-DAG-VARIABLE-ALISTP-OF-MV-NTH-6-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX)
(DARGP-OF-MV-NTH-1-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX
     (1475 88 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (1144 572
           (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
     (1054 1054 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (752 34 (:DEFINITION MEMBER-EQUAL))
     (572 572
          (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
     (515 115
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (414 11
          (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (314 28 (:REWRITE LEN-OF-CDR))
     (272 28
          (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (252 11 (:REWRITE USE-ALL-<-FOR-CAR))
     (209 209 (:REWRITE DEFAULT-CAR))
     (170 38 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (160 115 (:REWRITE DEFAULT-<-2))
     (151 46 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (147 2
          (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
     (146 2 (:REWRITE <-OF-CAR-AND-ALEN1))
     (137 115 (:REWRITE DEFAULT-<-1))
     (126 126 (:REWRITE DEFAULT-CDR))
     (121 121 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (115 115 (:REWRITE USE-ALL-<-2))
     (115 115 (:REWRITE USE-ALL-<))
     (109 109 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (109 109 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (109 109 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (109 109
          (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (86 86 (:REWRITE USE-ALL-CONSP-2))
     (86 86 (:REWRITE USE-ALL-CONSP))
     (86 86 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (71 71 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (71 71
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (56 2
         (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
     (46 46 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (46 46 (:REWRITE ALL-<-TRANSITIVE))
     (46 23
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (39 3 (:REWRITE ALL-<-OF-CDR))
     (34 17
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (34 14
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (33 33 (:REWRITE USE-ALL-<=-2))
     (33 33 (:REWRITE USE-ALL-<=))
     (31 4 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (30 30
         (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (30 28 (:REWRITE DEFAULT-+-2))
     (29 29
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (28 28 (:REWRITE DEFAULT-+-1))
     (23 23 (:TYPE-PRESCRIPTION NO-ATOMS))
     (23 11 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (22 11
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (22 4
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (21 21
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (21 21
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (20 2 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (19 19 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (18 2 (:REWRITE ALL-NATP-OF-CDR))
     (17 11
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (16 16 (:LINEAR ARRAY2P-LINEAR))
     (14 14
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (14 7
         (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (11 11
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (11 11
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (11 11
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (10 2 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
     (8 4 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (7 7 (:REWRITE USE-ALL-NATP-2))
     (7 7 (:REWRITE USE-ALL-NATP))
     (7 7
        (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
     (7 7
        (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
     (7 3 (:REWRITE USE-ALL-DARGP-LESS-THAN))
     (6 6 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (6 6 (:TYPE-PRESCRIPTION ALL-CONSP))
     (6 6 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (6 3
        (:REWRITE DARGP-LESS-THAN-WHEN-MYQUOTEP-CHEAP))
     (4 4 (:TYPE-PRESCRIPTION MEMBERP))
     (4 4 (:TYPE-PRESCRIPTION ALL-RATIONALP))
     (4 4 (:REWRITE USE-ALL-RATIONALP-2))
     (4 4 (:REWRITE USE-ALL-RATIONALP))
     (4 2
        (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (4 2
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (4 2
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (4 2
        (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (3 3 (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
     (3 3 (:REWRITE USE-ALL-DARGP-2))
     (3 3 (:REWRITE USE-ALL-DARGP))
     (3 3 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (3 3
        (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-NTH-0-AND-QUOTE))
     (3 3
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (3 3 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (3 3 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (2 2 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (2 2
        (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (2 2
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (2 2 (:REWRITE EQUAL-OF-LEN-AND-0))
     (2 2
        (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
     (2 2 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
     (2 2
        (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
     (2 2
        (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
     (2 2
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (2 2 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (2 1 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP)))
(DARGP-LESS-THAN-OF-MV-NTH-1-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-AUX
     (2272 1136
           (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
     (1385 1385 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (1136 1136
           (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
     (641 42 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (259 55
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (162 15 (:DEFINITION MEMBER-EQUAL))
     (154 4
          (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (142 37 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (110 10 (:REWRITE LEN-OF-CDR))
     (98 29 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (96 96 (:REWRITE DEFAULT-CAR))
     (96 9 (:REWRITE USE-ALL-<-FOR-CAR))
     (90 10
         (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (79 55 (:REWRITE DEFAULT-<-1))
     (76 55 (:REWRITE DEFAULT-<-2))
     (56 2
         (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
     (55 55 (:REWRITE USE-ALL-<-2))
     (55 55 (:REWRITE USE-ALL-<))
     (55 55 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (55 55 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (55 55 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (55 55
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (46 46 (:REWRITE DEFAULT-CDR))
     (46 2 (:REWRITE <-OF-CAR-AND-ALEN1))
     (39 39 (:REWRITE USE-ALL-CONSP-2))
     (39 39 (:REWRITE USE-ALL-CONSP))
     (39 39 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (37 37 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (37 37 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (37 37 (:REWRITE ALL-<-TRANSITIVE))
     (34 17
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (31 31 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (31 31
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (24 3
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (22 22 (:LINEAR ARRAY2P-LINEAR))
     (22 11
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (21 9 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (18 9
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (17 17 (:TYPE-PRESCRIPTION NO-ATOMS))
     (16 8
         (:REWRITE DARGP-LESS-THAN-WHEN-MYQUOTEP-CHEAP))
     (16 8
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (15 15 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (15 9
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (15 3 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
     (13 13 (:REWRITE USE-ALL-<=-2))
     (13 13 (:REWRITE USE-ALL-<=))
     (12 8 (:REWRITE USE-ALL-DARGP-LESS-THAN))
     (10 10 (:REWRITE DEFAULT-+-2))
     (10 10 (:REWRITE DEFAULT-+-1))
     (10 10
         (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (9 9
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (9 9
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (9 9
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (8 8 (:TYPE-PRESCRIPTION MYQUOTEP))
     (8 8 (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
     (8 8
        (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-NTH-0-AND-QUOTE))
     (8 8
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (8 4
        (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (8 1 (:REWRITE ALL-<-OF-CDR))
     (7 7
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (6 6 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (6 6 (:TYPE-PRESCRIPTION ALL-RATIONALP))
     (6 6 (:TYPE-PRESCRIPTION ALL-CONSP))
     (6 6
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (6 6
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (5 5
        (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-CAR-AND-QUOTE))
     (4 4 (:TYPE-PRESCRIPTION MEMBERP))
     (4 4 (:REWRITE USE-ALL-DARGP-2))
     (4 4 (:REWRITE USE-ALL-DARGP))
     (4 4 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (4 4
        (:REWRITE DARGP-LESS-THAN-WHEN-QUOTEP-CHEAP))
     (4 4
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (4 4 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (4 2
        (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (3 3 (:REWRITE USE-ALL-RATIONALP-2))
     (3 3 (:REWRITE USE-ALL-RATIONALP))
     (3 3
        (:REWRITE DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (3 3
        (:REWRITE DARGP-LESS-THAN-WHEN-CONSP-CHEAP))
     (3 3
        (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
     (3 3
        (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
     (3 3
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (3 3 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (3 3 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (2 2
        (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
     (2 2 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
     (1 1 (:REWRITE ALL-NATP-WHEN-NOT-CONSP)))
(ADD-BITXOR-NEST-TO-DAG-ARRAY
     (1930 22 (:REWRITE ALL-<-OF-CONS))
     (884 13 (:DEFINITION NAT-LISTP))
     (784 15
          (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (667 667 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (599 36 (:REWRITE USE-ALL-<-FOR-CAR))
     (380 87 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (380 5 (:REWRITE <-OF-CAR-AND-ALEN1))
     (336 3 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (310 65
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (257 18 (:REWRITE ALL-<-OF-CDR))
     (197 65 (:REWRITE DEFAULT-<-1))
     (174 1 (:REWRITE ALL-NATP-OF-CDR))
     (173 121 (:REWRITE DEFAULT-CAR))
     (156 40 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (146 5
          (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
     (135 68 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (117 12
          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (92 88 (:REWRITE DEFAULT-CDR))
     (89 89 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (89 89 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (89 89 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (89 89
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (89 65 (:REWRITE DEFAULT-<-2))
     (87 87 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (87 87 (:REWRITE ALL-<-TRANSITIVE))
     (81 12 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
     (80 40
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (78 40
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (74 37
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (68 68
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (65 65 (:REWRITE USE-ALL-<-2))
     (65 65 (:REWRITE USE-ALL-<))
     (56 28
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (56 4
         (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (52 52 (:TYPE-PRESCRIPTION ALL-CONSP))
     (49 49 (:REWRITE USE-ALL-CONSP-2))
     (49 49 (:REWRITE USE-ALL-CONSP))
     (49 49 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (40 40
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (40 40
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (40 40
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (38 38 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (38 2 (:LINEAR LEN-OF-CDR-LINEAR-STRONG))
     (37 37 (:TYPE-PRESCRIPTION NO-ATOMS))
     (35 7 (:REWRITE ALL-CONSP-OF-CDR))
     (34 2 (:LINEAR LEN-OF-CDR-LINEAR))
     (32 32 (:TYPE-PRESCRIPTION ALL-RATIONALP))
     (31 31 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (31 26 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (29 11 (:REWRITE USE-ALL-DARGP-LESS-THAN))
     (26 26
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (26 26 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (26 26
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (25 25
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (22 11
         (:REWRITE DARGP-LESS-THAN-WHEN-MYQUOTEP-CHEAP))
     (21 16
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (20 4 (:REWRITE ALL-RATIONALP-OF-CDR))
     (18 18 (:TYPE-PRESCRIPTION MEMBERP))
     (17 16
         (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
     (17 15 (:REWRITE DEFAULT-+-2))
     (16 16
         (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
     (16 16
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (15 15 (:REWRITE DEFAULT-+-1))
     (14 14 (:LINEAR ARRAY2P-LINEAR))
     (12 12 (:REWRITE USE-ALL-RATIONALP-2))
     (12 12 (:REWRITE USE-ALL-RATIONALP))
     (11 11 (:TYPE-PRESCRIPTION MYQUOTEP))
     (11 11 (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
     (11 11
         (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-NTH-0-AND-QUOTE))
     (6 6
        (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (6 6
        (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (6 3
        (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (6 3
        (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (5 5
        (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (5 5 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (4 1
        (:REWRITE EQUAL-OF-+-WHEN-NEGATIVE-CONSTANT))
     (3 3 (:REWRITE CDR-CONS))
     (3 3 (:REWRITE CAR-CONS))
     (3 3
        (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (3 3 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (3 1 (:REWRITE +-COMBINE-CONSTANTS))
     (2 2 (:TYPE-PRESCRIPTION NATP))
     (2 2 (:REWRITE USE-ALL-DARGP-2))
     (2 2 (:REWRITE USE-ALL-DARGP))
     (2 2 (:REWRITE EQUAL-OF-LEN-AND-0))
     (2 2 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (2 2 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (2 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (2 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (2 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (1 1 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (1 1
        (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (1 1
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (1 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (1 1 (:REWRITE EQUAL-OF-CONS))
     (1 1 (:DEFINITION FIX)))
(NATP-OF-MV-NTH-3-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY
     (124 4 (:REWRITE CONSP-FROM-LEN-CHEAP))
     (53 5 (:REWRITE DEFAULT-CAR))
     (38 2
         (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (36 36 (:TYPE-PRESCRIPTION LEN))
     (30 6
         (:LINEAR LEN-POSITIVE-WHEN-CONSP-LINEAR-CHEAP))
     (28 2 (:REWRITE LEN-OF-CDR))
     (10 10 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (10 10 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (10 10 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (10 10
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (10 6 (:REWRITE DEFAULT-<-2))
     (8 4
        (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (6 6 (:REWRITE USE-ALL-<-2))
     (6 6 (:REWRITE USE-ALL-<))
     (6 6 (:REWRITE DEFAULT-<-1))
     (6 6
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (5 5
        (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (5 5 (:REWRITE DEFAULT-CDR))
     (5 1 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (4 4 (:REWRITE USE-ALL-CONSP-2))
     (4 4 (:REWRITE USE-ALL-CONSP))
     (4 4 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (4 4 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (4 4
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (4 2 (:REWRITE DEFAULT-+-2))
     (3 3 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (3 3
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (3 1 (:REWRITE GETBIT-IDENTITY))
     (3 1
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (2 2 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (2 2 (:TYPE-PRESCRIPTION ALL-CONSP))
     (2 2 (:REWRITE USE-ALL-<=-2))
     (2 2 (:REWRITE USE-ALL-<=))
     (2 2
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (2 2 (:REWRITE EQUAL-OF-LEN-AND-0))
     (2 2 (:REWRITE DEFAULT-+-1))
     (2 2
        (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
     (2 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (2 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (2 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (2 1 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
     (2 1
        (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (1 1 (:TYPE-PRESCRIPTION NAT-LISTP))
     (1 1 (:TYPE-PRESCRIPTION ALL-NATP))
     (1 1 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (1 1 (:REWRITE USE-ALL-NATP-2))
     (1 1 (:REWRITE USE-ALL-NATP))
     (1 1
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (1 1
        (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
     (1 1
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (1 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (1 1
        (:REWRITE GETBIT-WHEN-VAL-IS-NOT-AN-INTEGER))
     (1 1
        (:REWRITE GETBIT-WHEN-NOT-INTEGERP-ARG1))
     (1 1 (:REWRITE GETBIT-WHEN-N-IS-NEGATIVE))
     (1 1
        (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
     (1 1 (:REWRITE GETBIT-IDENTITY-FREE))
     (1 1
        (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (1 1
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (1 1
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (1 1
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (1 1
        (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (1 1
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (1 1 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP)))
(BOUND-ON-MV-NTH-3-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-3
 (180
  4
  (:LINEAR
       BOUND-ON-MV-NTH-3-OF-ADD-FUNCTION-CALL-EXPR-TO-DAG-ARRAY-WITH-NAME-4))
 (124 4 (:REWRITE CONSP-FROM-LEN-CHEAP))
 (114 10 (:REWRITE DEFAULT-<-2))
 (80 40 (:TYPE-PRESCRIPTION ALEN1-TYPE))
 (67 5
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (53 5 (:REWRITE DEFAULT-CAR))
 (48 48 (:TYPE-PRESCRIPTION ARRAY1P))
 (40 40 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
 (38 2
     (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
 (38 2 (:LINEAR LEN-OF-CDR-LINEAR-STRONG))
 (34 2 (:LINEAR LEN-OF-CDR-LINEAR))
 (30 30 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
 (30 30 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
 (30 30 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
 (30 30
     (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
 (20 10
     (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
 (16 4
     (:REWRITE RATIONALP-OF-ALEN1-WHEN-ARRAY1P))
 (13 10 (:REWRITE DEFAULT-<-1))
 (12 10 (:REWRITE DEFAULT-+-2))
 (11 11
     (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
 (10 10 (:TYPE-PRESCRIPTION NATP))
 (10 10 (:REWRITE USE-ALL-<-2))
 (10 10 (:REWRITE USE-ALL-<))
 (10 10 (:REWRITE DEFAULT-+-1))
 (10 10
     (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
 (9 9 (:REWRITE DEFAULT-CDR))
 (6 6 (:REWRITE USE-ALL-<=-2))
 (6 6 (:REWRITE USE-ALL-<=))
 (5 5 (:REWRITE USE-ALL-RATIONALP-2))
 (5 5 (:REWRITE USE-ALL-RATIONALP))
 (5 1 (:REWRITE USE-ALL-CONSP-FOR-CAR))
 (4 4 (:REWRITE USE-ALL-CONSP-2))
 (4 4 (:REWRITE USE-ALL-CONSP))
 (4 4 (:REWRITE CONSP-WHEN-LEN-GREATER))
 (4 4
    (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP))
 (4 4 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
 (4 4
    (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
 (3 3 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
 (3 1 (:REWRITE GETBIT-IDENTITY))
 (3 1
    (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
 (2 2 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
 (2 2 (:TYPE-PRESCRIPTION ALL-CONSP))
 (2 2
    (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
 (2 2 (:REWRITE EQUAL-OF-LEN-AND-0))
 (2 2
    (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
 (2 1
    (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
 (2 1
    (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
 (2 1
    (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
 (2 1 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
 (2 1
    (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
 (1 1 (:TYPE-PRESCRIPTION NAT-LISTP))
 (1 1 (:TYPE-PRESCRIPTION ALL-NATP))
 (1 1 (:TYPE-PRESCRIPTION ALL-INTEGERP))
 (1 1
    (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
 (1 1
    (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
 (1 1
    (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
 (1 1
    (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
 (1 1
    (:REWRITE GETBIT-WHEN-VAL-IS-NOT-AN-INTEGER))
 (1 1
    (:REWRITE GETBIT-WHEN-NOT-INTEGERP-ARG1))
 (1 1 (:REWRITE GETBIT-WHEN-N-IS-NEGATIVE))
 (1 1
    (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
 (1 1 (:REWRITE GETBIT-IDENTITY-FREE))
 (1 1
    (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
 (1 1
    (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
 (1 1
    (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
 (1 1
    (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
 (1 1
    (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
 (1 1
    (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
 (1 1 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP)))
(BOUND-ON-MV-NTH-3-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-3-GEN
 (248 8 (:REWRITE CONSP-FROM-LEN-CHEAP))
 (180
  4
  (:LINEAR
       BOUND-ON-MV-NTH-3-OF-ADD-FUNCTION-CALL-EXPR-TO-DAG-ARRAY-WITH-NAME-4))
 (126 15 (:REWRITE DEFAULT-<-2))
 (106 10 (:REWRITE DEFAULT-CAR))
 (80 40 (:TYPE-PRESCRIPTION ALEN1-TYPE))
 (76 4
     (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
 (70 6
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (48 48 (:TYPE-PRESCRIPTION ARRAY1P))
 (40 40 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
 (40 40 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
 (40 40 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
 (40 40 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
 (40 40
     (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
 (38 2 (:LINEAR LEN-OF-CDR-LINEAR-STRONG))
 (34 2 (:LINEAR LEN-OF-CDR-LINEAR))
 (24 12
     (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
 (18 15 (:REWRITE DEFAULT-<-1))
 (16 12 (:REWRITE DEFAULT-+-2))
 (16 4
     (:REWRITE RATIONALP-OF-ALEN1-WHEN-ARRAY1P))
 (15 15 (:REWRITE USE-ALL-<-2))
 (15 15 (:REWRITE USE-ALL-<))
 (15 15
     (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
 (14 14
     (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
 (14 14 (:REWRITE DEFAULT-CDR))
 (12 12 (:REWRITE DEFAULT-+-1))
 (10 2 (:REWRITE USE-ALL-CONSP-FOR-CAR))
 (8 8 (:TYPE-PRESCRIPTION NATP))
 (8 8 (:REWRITE USE-ALL-CONSP-2))
 (8 8 (:REWRITE USE-ALL-CONSP))
 (8 8 (:REWRITE CONSP-WHEN-LEN-GREATER))
 (8 8 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
 (8 8
    (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
 (7 7 (:REWRITE USE-ALL-<=-2))
 (7 7 (:REWRITE USE-ALL-<=))
 (6 6 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
 (6 6 (:REWRITE USE-ALL-RATIONALP-2))
 (6 6 (:REWRITE USE-ALL-RATIONALP))
 (6 2 (:REWRITE GETBIT-IDENTITY))
 (6 2
    (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
 (4 4 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
 (4 4 (:TYPE-PRESCRIPTION ALL-CONSP))
 (4 4 (:REWRITE EQUAL-OF-LEN-AND-0))
 (4 4
    (:REWRITE CONSP-OF-CDR-WHEN-LEN-KNOWN))
 (4 4
    (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP))
 (4 2
    (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
 (4 2
    (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
 (4 2
    (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
 (4 2 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
 (4 2
    (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
 (3 3
    (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
 (2 2 (:TYPE-PRESCRIPTION NAT-LISTP))
 (2 2 (:TYPE-PRESCRIPTION ALL-NATP))
 (2 2 (:TYPE-PRESCRIPTION ALL-INTEGERP))
 (2 2
    (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
 (2 2
    (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
 (2 2
    (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
 (2 2
    (:REWRITE GETBIT-WHEN-VAL-IS-NOT-AN-INTEGER))
 (2 2
    (:REWRITE GETBIT-WHEN-NOT-INTEGERP-ARG1))
 (2 2 (:REWRITE GETBIT-WHEN-N-IS-NEGATIVE))
 (2 2
    (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
 (2 2 (:REWRITE GETBIT-IDENTITY-FREE))
 (2 2
    (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
 (2 2
    (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
 (2 2
    (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
 (2 2
    (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
 (2 2
    (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
 (2 2
    (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
 (2 2 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
 (1 1
    (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN)))
(TYPE-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY
    (2472 19 (:REWRITE ALL-<-OF-CONS))
    (1537 19 (:DEFINITION NAT-LISTP))
    (936 31 (:REWRITE USE-ALL-<-FOR-CAR))
    (873 247 (:REWRITE DEFAULT-CAR))
    (844 21
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
    (667 5 (:REWRITE <-OF-CAR-AND-ALEN1))
    (643 12 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
    (492 492 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
    (423 111
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
    (378 16 (:REWRITE ALL-<-OF-CDR))
    (371 78 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
    (335 7 (:REWRITE ALL-NATP-OF-CDR))
    (275 15
         (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
    (246 111 (:REWRITE DEFAULT-<-1))
    (216 56 (:REWRITE USE-ALL-CONSP-FOR-CAR))
    (207 203 (:REWRITE DEFAULT-CDR))
    (182 182 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
    (182 182 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
    (182 182 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
    (182 182
         (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
    (177 111 (:REWRITE DEFAULT-<-2))
    (169 61 (:REWRITE ALL-<-WHEN-NOT-CONSP))
    (146 5
         (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
    (112 56
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
    (111 111 (:REWRITE USE-ALL-<-2))
    (111 111 (:REWRITE USE-ALL-<))
    (108 56
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
    (107 107 (:REWRITE USE-ALL-CONSP-2))
    (107 107 (:REWRITE USE-ALL-CONSP))
    (107 107 (:REWRITE CONSP-WHEN-LEN-GREATER))
    (105 13
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
    (89 6 (:REWRITE USE-ALL-NATP-FOR-CAR))
    (78 78 (:REWRITE ALL-<-TRANSITIVE-FREE))
    (78 78 (:REWRITE ALL-<-TRANSITIVE))
    (74 74 (:TYPE-PRESCRIPTION ALL-CONSP))
    (68 34
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
    (67 67 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
    (67 67
        (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
    (66 10 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
    (61 61
        (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
    (58 44 (:REWRITE DEFAULT-+-2))
    (56 56
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
    (56 56
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
    (56 56
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
    (55 11 (:REWRITE ALL-CONSP-OF-CDR))
    (52 52 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
    (50 50
        (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
    (50 25
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
    (44 44 (:REWRITE DEFAULT-+-1))
    (39 39 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
    (39 13 (:REWRITE GETBIT-IDENTITY))
    (38 37 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
    (38 2 (:LINEAR LEN-OF-CDR-LINEAR-STRONG))
    (37 37
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
    (34 34 (:TYPE-PRESCRIPTION NO-ATOMS))
    (34 2 (:LINEAR LEN-OF-CDR-LINEAR))
    (28 28 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
    (28 7
        (:REWRITE EQUAL-OF-+-WHEN-NEGATIVE-CONSTANT))
    (26 26 (:TYPE-PRESCRIPTION ALL-RATIONALP))
    (26 14
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
    (26 13
        (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
    (26 13
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
    (26 13
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
    (26 13 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
    (24 12
        (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
    (23 9 (:REWRITE USE-ALL-DARGP-LESS-THAN))
    (21 7 (:REWRITE +-COMBINE-CONSTANTS))
    (18 9
        (:REWRITE DARGP-LESS-THAN-WHEN-MYQUOTEP-CHEAP))
    (15 3 (:REWRITE ALL-RATIONALP-OF-CDR))
    (14 14 (:TYPE-PRESCRIPTION MEMBERP))
    (14 14 (:REWRITE EQUAL-OF-LEN-AND-0))
    (14 14
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
    (14 13
        (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
    (13 13 (:TYPE-PRESCRIPTION ALL-INTEGERP))
    (13 13 (:REWRITE USE-ALL-RATIONALP-2))
    (13 13 (:REWRITE USE-ALL-RATIONALP))
    (13 13
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
    (13 13
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
    (13 13
        (:REWRITE GETBIT-WHEN-VAL-IS-NOT-AN-INTEGER))
    (13 13
        (:REWRITE GETBIT-WHEN-NOT-INTEGERP-ARG1))
    (13 13 (:REWRITE GETBIT-WHEN-N-IS-NEGATIVE))
    (13 13
        (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
    (13 13 (:REWRITE GETBIT-IDENTITY-FREE))
    (13 13
        (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
    (13 13
        (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
    (12 12
        (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
    (12 12 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
    (12 12 (:LINEAR ARRAY2P-LINEAR))
    (9 9 (:TYPE-PRESCRIPTION MYQUOTEP))
    (9 9 (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
    (9 9
       (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-NTH-0-AND-QUOTE))
    (7 7
       (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
    (7 7 (:DEFINITION FIX))
    (6 6 (:REWRITE USE-ALL-NATP-2))
    (6 6 (:REWRITE USE-ALL-NATP))
    (6 6
       (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
    (6 3
       (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
    (5 5 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
    (4 4 (:REWRITE USE-ALL-<=-2))
    (4 4 (:REWRITE USE-ALL-<=))
    (3 3 (:REWRITE CDR-CONS))
    (3 3 (:REWRITE CAR-CONS))
    (2 2 (:REWRITE USE-ALL-DARGP-2))
    (2 2 (:REWRITE USE-ALL-DARGP))
    (2 2 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
    (2 2
       (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
    (2 2 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP)))
(TYPE-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-ONE)
(TYPE-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-TWO
     (105 105 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (39 39 (:TYPE-PRESCRIPTION NATP))
     (2 2 (:LINEAR ARRAY2P-LINEAR)))
(TYPE-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-THREE)
(TYPE-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY-FOUR)
(BOUNDED-DAG-PARENT-ARRAYP-OF-MV-NTH-4-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY)
(BOUNDED-DAG-DAG-CONSTANT-ALISTP-OF-MV-NTH-5-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY)
(BOUNDED-DAG-DAG-VARIABLE-ALISTP-OF-MV-NTH-6-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY)
(DARGP-OF-MV-NTH-1-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY
     (2747 16 (:REWRITE ALL-<-OF-CONS))
     (1989 19 (:DEFINITION NAT-LISTP))
     (1310 33 (:REWRITE USE-ALL-<-FOR-CAR))
     (844 28 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (844 21
          (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (837 5 (:REWRITE <-OF-CAR-AND-ALEN1))
     (625 27 (:REWRITE ALL-<-OF-CDR))
     (458 6 (:DEFINITION NATP))
     (447 16 (:REWRITE ALL-NATP-OF-CDR))
     (377 84 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (314 314 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (288 142 (:REWRITE DEFAULT-CAR))
     (215 72
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (186 46 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (166 18 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (146 72 (:REWRITE DEFAULT-<-1))
     (146 5
          (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
     (136 71 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (122 122 (:REWRITE DEFAULT-CDR))
     (103 72 (:REWRITE DEFAULT-<-2))
     (101 101 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (101 101 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (101 101 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (101 101
          (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (92 46
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (89 6 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (88 46
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (85 5
         (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (84 84 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (84 84 (:REWRITE ALL-<-TRANSITIVE))
     (74 8
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (72 72 (:REWRITE USE-ALL-<-2))
     (72 72 (:REWRITE USE-ALL-<))
     (71 71
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (65 65 (:REWRITE USE-ALL-CONSP-2))
     (65 65 (:REWRITE USE-ALL-CONSP))
     (65 65 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (64 64 (:TYPE-PRESCRIPTION ALL-CONSP))
     (56 28
         (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (56 28
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (55 11 (:REWRITE ALL-CONSP-OF-CDR))
     (50 8 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
     (46 46
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (46 46
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (46 46
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (42 42 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (42 21
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (40 18 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (34 34
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (34 30 (:REWRITE DEFAULT-+-2))
     (33 32 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (32 32
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (32 32 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (32 32
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (30 30 (:REWRITE DEFAULT-+-1))
     (28 28 (:TYPE-PRESCRIPTION NO-ATOMS))
     (28 28
         (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (28 28 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (28 7
         (:REWRITE EQUAL-OF-+-WHEN-NEGATIVE-CONSTANT))
     (26 26 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (21 7 (:REWRITE +-COMBINE-CONSTANTS))
     (20 20 (:TYPE-PRESCRIPTION ALL-RATIONALP))
     (19 1 (:LINEAR LEN-OF-CDR-LINEAR-STRONG))
     (18 9
         (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (18 9
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (18 9
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (17 7 (:REWRITE USE-ALL-DARGP-LESS-THAN))
     (17 1 (:LINEAR LEN-OF-CDR-LINEAR))
     (14 10
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (14 7
         (:REWRITE DARGP-LESS-THAN-WHEN-MYQUOTEP-CHEAP))
     (10 10 (:TYPE-PRESCRIPTION MYQUOTEP))
     (10 10 (:TYPE-PRESCRIPTION MEMBERP))
     (10 10
         (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
     (10 10
         (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
     (10 10
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (10 5
         (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (10 2 (:REWRITE ALL-RATIONALP-OF-CDR))
     (9 9 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (9 9 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (9 9
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (9 9
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (9 3 (:REWRITE GETBIT-IDENTITY))
     (8 8 (:REWRITE USE-ALL-RATIONALP-2))
     (8 8 (:REWRITE USE-ALL-RATIONALP))
     (8 8 (:LINEAR ARRAY2P-LINEAR))
     (7 7 (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
     (7 7
        (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (7 7
        (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-NTH-0-AND-QUOTE))
     (7 7 (:DEFINITION FIX))
     (6 6 (:REWRITE USE-ALL-NATP-2))
     (6 6 (:REWRITE USE-ALL-NATP))
     (6 6 (:REWRITE USE-ALL-DARGP-2))
     (6 6 (:REWRITE USE-ALL-DARGP))
     (6 6 (:REWRITE USE-ALL-<=-2))
     (6 6 (:REWRITE USE-ALL-<=))
     (6 6
        (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (6 6
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (6 6 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (6 3 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
     (6 3 (:REWRITE DARGP-WHEN-NATP-CHEAP))
     (6 3 (:REWRITE DARGP-WHEN-MYQUOTEP-CHEAP))
     (5 5 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (4 4 (:REWRITE EQUAL-OF-LEN-AND-0))
     (4 4
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (4 4 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (3 3
        (:REWRITE GETBIT-WHEN-VAL-IS-NOT-AN-INTEGER))
     (3 3
        (:REWRITE GETBIT-WHEN-NOT-INTEGERP-ARG1))
     (3 3 (:REWRITE GETBIT-WHEN-N-IS-NEGATIVE))
     (3 3
        (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
     (3 3 (:REWRITE GETBIT-IDENTITY-FREE))
     (3 3
        (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
     (3 3 (:REWRITE DARGP-WHEN-CONSP-CHEAP))
     (3 3 (:REWRITE CDR-CONS))
     (3 3
        (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (3 3 (:REWRITE CAR-CONS)))
(DARGP-LESS-THAN-OF-MV-NTH-1-OF-ADD-BITXOR-NEST-TO-DAG-ARRAY
     (3106 26 (:REWRITE ALL-<-OF-CONS))
     (1345 45 (:REWRITE USE-ALL-<-FOR-CAR))
     (859 24
          (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (837 5 (:REWRITE <-OF-CAR-AND-ALEN1))
     (625 27 (:REWRITE ALL-<-OF-CDR))
     (534 105 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (447 16 (:REWRITE ALL-NATP-OF-CDR))
     (424 182 (:REWRITE DEFAULT-CAR))
     (346 97
          (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (314 314 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (246 94 (:REWRITE DEFAULT-<-1))
     (192 48 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (166 18 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (156 148 (:REWRITE DEFAULT-CDR))
     (151 84 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (146 5
          (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
     (131 94 (:REWRITE DEFAULT-<-2))
     (125 9 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (123 7
          (:REWRITE <-OF-+-ARG2-WHEN-NEGATIVE-CONSTANT))
     (121 121 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
     (121 121 (:REWRITE LEN-WHEN-NOT-CONSP-CHEAP))
     (121 121 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
     (121 121
          (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
     (105 105 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (105 105 (:REWRITE ALL-<-TRANSITIVE))
     (96 48
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (94 94 (:REWRITE USE-ALL-<-2))
     (94 94 (:REWRITE USE-ALL-<))
     (92 48
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (92 10
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (84 84
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (74 37
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (72 72 (:REWRITE USE-ALL-CONSP-2))
     (72 72 (:REWRITE USE-ALL-CONSP))
     (72 72 (:REWRITE CONSP-WHEN-LEN-GREATER))
     (66 66 (:TYPE-PRESCRIPTION ALL-CONSP))
     (64 32
         (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (62 10 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
     (55 11 (:REWRITE ALL-CONSP-OF-CDR))
     (52 26
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (48 48
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (48 48
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (48 48
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (44 44 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (40 40
         (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
     (40 18 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (38 38 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
     (38 38
         (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
     (38 32 (:REWRITE DEFAULT-+-2))
     (37 37 (:TYPE-PRESCRIPTION NO-ATOMS))
     (36 16 (:REWRITE USE-ALL-DARGP-LESS-THAN))
     (35 32 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (34 33 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (33 33
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (32 32 (:REWRITE DEFAULT-+-1))
     (32 32
         (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (32 16
         (:REWRITE DARGP-LESS-THAN-WHEN-MYQUOTEP-CHEAP))
     (29 29 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (28 14
         (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (28 7
         (:REWRITE EQUAL-OF-+-WHEN-NEGATIVE-CONSTANT))
     (24 24 (:TYPE-PRESCRIPTION ALL-RATIONALP))
     (22 11
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (22 11
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (21 7 (:REWRITE +-COMBINE-CONSTANTS))
     (20 20 (:TYPE-PRESCRIPTION MEMBERP))
     (19 15
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (19 1 (:LINEAR LEN-OF-CDR-LINEAR-STRONG))
     (17 1 (:LINEAR LEN-OF-CDR-LINEAR))
     (16 16 (:TYPE-PRESCRIPTION MYQUOTEP))
     (16 16 (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
     (16 16
         (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-NTH-0-AND-QUOTE))
     (15 15 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (15 15
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (15 5 (:REWRITE GETBIT-IDENTITY))
     (14 12
         (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
     (12 12
         (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
     (11 11 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (11 11
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (10 10 (:REWRITE USE-ALL-RATIONALP-2))
     (10 10 (:REWRITE USE-ALL-RATIONALP))
     (10 5 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
     (10 2 (:REWRITE ALL-RATIONALP-OF-CDR))
     (9 9 (:REWRITE USE-ALL-NATP-2))
     (9 9 (:REWRITE USE-ALL-NATP))
     (9 9 (:REWRITE USE-ALL-<=-2))
     (9 9 (:REWRITE USE-ALL-<=))
     (8 8 (:LINEAR ARRAY2P-LINEAR))
     (7 7
        (:REWRITE LEN-OF-CDDR-WHEN-EQUAL-OF-LEN))
     (7 7 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
     (7 7 (:DEFINITION FIX))
     (6 6
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (6 6 (:REWRITE EQUAL-OF-LEN-AND-0))
     (6 3
        (:REWRITE DARGP-LESS-THAN-WHEN-NATP-CHEAP))
     (6 3
        (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (5 5
        (:REWRITE GETBIT-WHEN-VAL-IS-NOT-AN-INTEGER))
     (5 5
        (:REWRITE GETBIT-WHEN-NOT-INTEGERP-ARG1))
     (5 5 (:REWRITE GETBIT-WHEN-N-IS-NEGATIVE))
     (5 5
        (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
     (5 5 (:REWRITE GETBIT-IDENTITY-FREE))
     (5 5
        (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (3 3
        (:REWRITE DARGP-LESS-THAN-WHEN-QUOTEP-CHEAP))
     (3 3
        (:REWRITE DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (3 3
        (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-CAR-AND-QUOTE))
     (3 3
        (:REWRITE DARGP-LESS-THAN-WHEN-CONSP-CHEAP))
     (3 3 (:REWRITE DARGP-LESS-THAN-MONO))
     (2 2 (:REWRITE USE-ALL-DARGP-2))
     (2 2 (:REWRITE USE-ALL-DARGP))
     (2 2 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (2 2 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (1 1 (:REWRITE ALL-NATP-OF-CONS)))
