(FIND-SHORTEST-PARENT-LST
  (123 123 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
  (117 80
       (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
  (114 4
       (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
  (80 80
      (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
  (80 16 (:REWRITE USE-ALL-CONSP-FOR-CAR))
  (60 6
      (:REWRITE ALL-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (48 16
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
  (46 4 (:DEFINITION LEN))
  (42 42
      (:TYPE-PRESCRIPTION TRUE-LISTP-OF-DIMENSIONS))
  (36 12
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
  (34 2
      (:REWRITE LEN-OF-AREF1-WHEN-QUOTEP-AND-PSEUDO-DAG-ARRAYP-AUX))
  (32 32 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
  (32 32 (:TYPE-PRESCRIPTION ALL-CONSP))
  (32 16
      (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
  (28 2 (:REWRITE ALL-<-OF-CDR))
  (24 24
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
  (24 24 (:REWRITE USE-ALL-CONSP-2))
  (24 24 (:REWRITE USE-ALL-CONSP))
  (24 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-OF-CDR))
  (18 2
      (:REWRITE PSEUDO-DAG-ARRAYP-AUX-OF-CAR))
  (16 16
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (16 16
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
  (16 16
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (16 16
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
  (16 16 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
  (16 8 (:REWRITE DEFAULT-+-2))
  (12 12 (:TYPE-PRESCRIPTION ALL-<))
  (12 12
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
  (12 12
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
  (12 6 (:REWRITE ALL-<-WHEN-NOT-CONSP))
  (12 1
      (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (10 10 (:REWRITE DEFAULT-CDR))
  (8 8
     (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP))
  (8 8 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
  (8 8 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
  (8 8
     (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
  (8 8 (:REWRITE DEFAULT-CAR))
  (8 8 (:REWRITE DEFAULT-+-1))
  (8 4
     (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
  (8 4
     (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
  (8 2
     (:REWRITE PSEUDO-DAG-ARRAYP-AUX-WHEN-PSEUDO-DAGP-AUX))
  (6 6 (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
  (6 6 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
  (6 6 (:REWRITE ALL-<-TRANSITIVE-FREE))
  (6 6 (:REWRITE ALL-<-TRANSITIVE))
  (6 3 (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
  (6 1 (:REWRITE <-OF-CAR-AND-ALEN1))
  (5 5
     (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
  (4 4 (:TYPE-PRESCRIPTION PSEUDO-DAGP-AUX))
  (4 4
     (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-AUX))
  (4 4 (:TYPE-PRESCRIPTION NO-ATOMS))
  (4 4 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
  (4 4 (:REWRITE PSEUDO-DAG-ARRAYP-MONOTONE))
  (4 4
     (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
  (4 4 (:LINEAR ARRAY2P-LINEAR))
  (4 2 (:REWRITE DEFAULT-<-2))
  (4 2 (:REWRITE DEFAULT-<-1))
  (3 3 (:REWRITE USE-ALL-<=-2))
  (3 3 (:REWRITE USE-ALL-<=))
  (2 2 (:REWRITE USE-ALL-<-2))
  (2 2 (:REWRITE USE-ALL-<))
  (2 2
     (:REWRITE PSEUDO-DAGP-AUX-WHEN-NOT-CONSP-CHEAP))
  (2 2
     (:REWRITE PSEUDO-DAG-ARRAYP-AUX-MONOTONE))
  (2 2
     (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
  (2 2
     (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
  (2 2 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
  (2 2
     (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
  (2 1
     (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
  (1 1 (:TYPE-PRESCRIPTION NAT-LISTP))
  (1 1
     (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP)))
(NAT-LISTP-OF-FIND-SHORTEST-PARENT-LST
   (369 9 (:DEFINITION NATP))
   (207 9 (:REWRITE USE-ALL-<-FOR-CAR))
   (142 2 (:REWRITE SHORTER-LST-REWRITE))
   (126 126 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
   (99 9 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
   (52 4 (:DEFINITION LEN))
   (36 18
       (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
   (36 9 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
   (32 32 (:TYPE-PRESCRIPTION LEN))
   (31 31 (:REWRITE USE-ALL-CONSP-2))
   (31 31 (:REWRITE USE-ALL-CONSP))
   (30 6 (:REWRITE USE-ALL-CONSP-FOR-CAR))
   (28 2
       (:REWRITE LEN-OF-AREF1-WHEN-QUOTEP-AND-PSEUDO-DAG-ARRAYP-AUX))
   (22 22 (:REWRITE DEFAULT-CDR))
   (20 20 (:REWRITE DEFAULT-CAR))
   (18 18 (:TYPE-PRESCRIPTION ALL-<))
   (18 18
       (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
   (18 18 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
   (18 9
       (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
   (18 9
       (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
   (18 9
       (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
   (18 6
       (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
   (14 14 (:LINEAR ARRAY2P-LINEAR))
   (14 7
       (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
   (13 11 (:REWRITE DEFAULT-<-2))
   (13 11 (:REWRITE DEFAULT-<-1))
   (12 12 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
   (12 12 (:TYPE-PRESCRIPTION ALL-CONSP))
   (12 12 (:REWRITE USE-ALL-NATP-2))
   (12 12 (:REWRITE USE-ALL-NATP))
   (12 12
       (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
   (12 6
       (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
   (12 2
       (:REWRITE PSEUDO-DAG-ARRAYP-AUX-OF-CAR))
   (11 11 (:REWRITE USE-ALL-<=-2))
   (11 11 (:REWRITE USE-ALL-<=))
   (11 11 (:REWRITE USE-ALL-<-2))
   (11 11 (:REWRITE USE-ALL-<))
   (11 11
       (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
   (9 9 (:TYPE-PRESCRIPTION ALL-INTEGERP))
   (9 9
      (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
   (9 9
      (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
   (9 9
      (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
   (9 9
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
   (9 9 (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
   (9 9 (:REWRITE ALL-<-WHEN-NOT-CONSP))
   (9 9 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
   (9 9 (:REWRITE ALL-<-TRANSITIVE-FREE))
   (9 9 (:REWRITE ALL-<-TRANSITIVE))
   (8 8 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
   (8 8 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
   (8 8
      (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
   (8 4
      (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
   (8 4 (:REWRITE DEFAULT-+-2))
   (8 4 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
   (8 2
      (:REWRITE PSEUDO-DAG-ARRAYP-AUX-WHEN-PSEUDO-DAGP-AUX))
   (7 7 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
   (7 7
      (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
   (6 6
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
   (6 6
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
   (6 6
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
   (6 6
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
   (6 6 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
   (6 3 (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
   (4 4
      (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
   (4 4 (:TYPE-PRESCRIPTION PSEUDO-DAGP-AUX))
   (4 4
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
   (4 4 (:TYPE-PRESCRIPTION NATP))
   (4 4 (:REWRITE DEFAULT-+-1))
   (2 2
      (:REWRITE PSEUDO-DAGP-AUX-WHEN-NOT-CONSP-CHEAP))
   (2 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
   (2 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
   (2 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
   (2 2
      (:REWRITE PSEUDO-DAG-ARRAYP-AUX-MONOTONE))
   (2 2 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
   (2 2
      (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH)))
(TRUE-LISTP-OF-FIND-SHORTEST-PARENT-LST
   (142 2 (:REWRITE SHORTER-LST-REWRITE))
   (120 120 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
   (52 4 (:DEFINITION LEN))
   (32 32 (:TYPE-PRESCRIPTION LEN))
   (30 6 (:REWRITE USE-ALL-CONSP-FOR-CAR))
   (28 2
       (:REWRITE LEN-OF-AREF1-WHEN-QUOTEP-AND-PSEUDO-DAG-ARRAYP-AUX))
   (19 19 (:REWRITE USE-ALL-CONSP-2))
   (19 19 (:REWRITE USE-ALL-CONSP))
   (18 6
       (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
   (14 14 (:LINEAR ARRAY2P-LINEAR))
   (14 7
       (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
   (12 12 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
   (12 12 (:TYPE-PRESCRIPTION ALL-CONSP))
   (12 6
       (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
   (12 2
       (:REWRITE PSEUDO-DAG-ARRAYP-AUX-OF-CAR))
   (10 10 (:REWRITE DEFAULT-CDR))
   (9 9
      (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
   (8 8 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
   (8 8 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
   (8 8
      (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
   (8 8 (:REWRITE DEFAULT-CAR))
   (8 4 (:REWRITE DEFAULT-+-2))
   (8 4 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
   (8 2
      (:REWRITE PSEUDO-DAG-ARRAYP-AUX-WHEN-PSEUDO-DAGP-AUX))
   (7 7 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
   (7 7
      (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
   (6 6
      (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
   (6 6
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
   (6 6
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
   (6 6
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
   (6 6
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
   (6 6 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
   (6 3 (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
   (5 5 (:TYPE-PRESCRIPTION NATP))
   (4 4 (:TYPE-PRESCRIPTION PSEUDO-DAGP-AUX))
   (4 4
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
   (4 4 (:REWRITE DEFAULT-+-1))
   (4 2 (:REWRITE DEFAULT-<-2))
   (4 2 (:REWRITE DEFAULT-<-1))
   (2 2 (:REWRITE USE-ALL-<=-2))
   (2 2 (:REWRITE USE-ALL-<=))
   (2 2 (:REWRITE USE-ALL-<-2))
   (2 2 (:REWRITE USE-ALL-<))
   (2 2
      (:REWRITE PSEUDO-DAGP-AUX-WHEN-NOT-CONSP-CHEAP))
   (2 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
   (2 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
   (2 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
   (2 2
      (:REWRITE PSEUDO-DAG-ARRAYP-AUX-MONOTONE))
   (2 2
      (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
   (2 2 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
   (2 2
      (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH)))
(ALL-<-OF-FIND-SHORTEST-PARENT-LST
  (841 39 (:REWRITE ALL-<-WHEN-NOT-CONSP))
  (478 34 (:DEFINITION LEN))
  (429 23
       (:REWRITE LEN-OF-AREF1-WHEN-QUOTEP-AND-PSEUDO-DAG-ARRAYP-AUX))
  (252 21
       (:REWRITE PSEUDO-DAG-ARRAYP-AUX-OF-CAR))
  (140 35
       (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
  (136 68
       (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
  (129 3
       (:REWRITE BOUND-LEMMA-FOR-CAR-WHEN-ALL-DARGP-LESS-THAN))
  (120 18 (:REWRITE USE-ALL-CONSP-FOR-CAR))
  (117 27
       (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
  (102 102 (:REWRITE USE-ALL-CONSP-2))
  (102 102 (:REWRITE USE-ALL-CONSP))
  (97 63 (:REWRITE DEFAULT-+-2))
  (88 22
      (:REWRITE PSEUDO-DAG-ARRAYP-AUX-WHEN-PSEUDO-DAGP-AUX))
  (88 13
      (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
  (74 23
      (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
  (73 73 (:REWRITE DEFAULT-CDR))
  (72 6
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-OF-CDR))
  (70 35 (:TYPE-PRESCRIPTION ALEN1-TYPE))
  (68 68
      (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
  (68 68
      (:TYPE-PRESCRIPTION DAG-PARENT-ARRAYP))
  (68 68 (:REWRITE LEN-WHEN-PSEUDO-DAGP-AUX))
  (68 68 (:REWRITE LEN-WHEN-DARGP-LESS-THAN))
  (68 68
      (:REWRITE LEN-WHEN-BOUNDED-DAG-EXPRP-AND-QUOTEP))
  (63 63 (:REWRITE DEFAULT-+-1))
  (63 23 (:REWRITE DEFAULT-<-2))
  (54 54
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
  (54 18
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
  (51 51 (:REWRITE DEFAULT-CAR))
  (49 43 (:REWRITE ALL-<-TRANSITIVE-FREE))
  (48 48 (:TYPE-PRESCRIPTION ALL-CONSP))
  (48 6 (:DEFINITION MEMBER-EQUAL))
  (45 15 (:REWRITE FOLD-CONSTS-IN-+))
  (44 44 (:TYPE-PRESCRIPTION PSEUDO-DAGP-AUX))
  (44 44
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-AUX))
  (43 43 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
  (40 23 (:REWRITE DEFAULT-<-1))
  (36 36 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
  (36 18
      (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
  (35 35 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
  (35 35 (:TYPE-PRESCRIPTION ARRAY1P))
  (30 30
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP))
  (30 6 (:REWRITE ALL-CONSP-OF-CDR))
  (30 3 (:REWRITE NOT-<-OF-CAR-WHEN-ALL-<-2))
  (28 14
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
  (27 27
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
  (27 27
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
  (26 13
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
  (24 24
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
  (24 24 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
  (24 3
      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
  (23 23 (:REWRITE USE-ALL-<=-2))
  (23 23 (:REWRITE USE-ALL-<=))
  (23 23 (:REWRITE USE-ALL-<-2))
  (23 23 (:REWRITE USE-ALL-<))
  (22 22
      (:REWRITE PSEUDO-DAGP-AUX-WHEN-NOT-CONSP-CHEAP))
  (22 22
      (:REWRITE PSEUDO-DAG-ARRAYP-AUX-MONOTONE))
  (21 21 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
  (21 21 (:REWRITE NOT-ALL-<-WHEN-MEMBERP))
  (21 21
      (:REWRITE NOT-ALL-<-WHEN-MEMBER-EQUAL))
  (18 18
      (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
  (18 18
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (18 18
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
  (18 18
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (18 3 (:REWRITE USE-ALL-<=-FOR-CAR))
  (17 17 (:REWRITE <-OF-LEN-WHEN-NTH-NON-NIL))
  (17 17
      (:REWRITE <-OF-LEN-WHEN-INTEGERP-OF-NTH))
  (16 4
      (:REWRITE ALL-<-OF-AREF1-WHEN-BOUNDED-DAG-PARENT-ARRAYP))
  (16 2 (:REWRITE ALL-<-OF-CDR))
  (15 15
      (:REWRITE PSEUDO-DAG-ARRAYP-MONOTONE))
  (15 11
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
  (15 3 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
  (14 14 (:TYPE-PRESCRIPTION NO-ATOMS))
  (13 13 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
  (11 11
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
  (9 9
     (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
  (9 9
     (:REWRITE BOUNDED-DAG-PARENT-ENTRIESP-MONOTONE-GEN))
  (9 9
     (:REWRITE BOUNDED-DAG-PARENT-ENTRIESP-MONOTONE))
  (8 8
     (:TYPE-PRESCRIPTION BOUNDED-DAG-PARENT-ARRAYP))
  (8 2 (:TYPE-PRESCRIPTION SHORTER-LST))
  (6 6 (:TYPE-PRESCRIPTION ALL-RATIONALP))
  (6 6 (:TYPE-PRESCRIPTION ALL-<=))
  (6 6 (:REWRITE USE-ALL-RATIONALP-2))
  (6 6 (:REWRITE USE-ALL-RATIONALP))
  (4 4
     (:REWRITE BOUNDED-DAG-PARENT-ARRAYP-MONOTONE))
  (3 3
     (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
  (3 3
     (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
  (3 3
     (:REWRITE ALL-<=-WHEN-NOT-CONSP-CHEAP))
  (3 3 (:REWRITE ALL-<=-WHEN-NOT-CONSP))
  (3 3 (:REWRITE ALL-<=-MONOTONE))
  (3 3 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
  (3 1 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
  (2 2 (:TYPE-PRESCRIPTION NAT-LISTP))
  (2 1
     (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
  (1 1 (:DEFINITION NAT-LISTP)))
(FIRST-ATOM)
(ALL-DARGP-LESS-THAN-OF-MV-NTH-1-OF-FIRST-ATOM
     (70 12 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (57 7
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (45 9 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (32 32 (:REWRITE USE-ALL-CONSP-2))
     (32 32 (:REWRITE USE-ALL-CONSP))
     (27 9
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (24 12
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (20 10
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (18 18 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (18 18 (:TYPE-PRESCRIPTION ALL-CONSP))
     (18 9
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (16 2 (:REWRITE ALL-<-OF-CDR))
     (14 14 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (14 14 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (14 14 (:REWRITE ALL-<-TRANSITIVE))
     (12 12 (:TYPE-PRESCRIPTION NO-ATOMS))
     (11 11 (:REWRITE DEFAULT-CDR))
     (11 11 (:REWRITE DEFAULT-CAR))
     (10 10 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (9 9
        (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (9 9
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (9 9
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (9 9
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (9 9
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (9 9 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (7 7
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (3 1 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (2 2 (:TYPE-PRESCRIPTION NAT-LISTP))
     (2 1
        (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (1 1 (:DEFINITION NAT-LISTP)))
(<-OF-MV-NTH-0-OF-FIRST-ATOM
     (1167 41 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (1133 17
           (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
     (902 40 (:DEFINITION NAT-LISTP))
     (673 12
          (:REWRITE USE-ALL-DARGP-LESS-THAN-FOR-CAR))
     (624 22 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (562 22 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (486 40 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (408 23 (:REWRITE USE-ALL-<-FOR-CAR))
     (248 248 (:TYPE-PRESCRIPTION NAT-LISTP))
     (181 30 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (176 4 (:REWRITE ALL-<-OF-CDR))
     (134 4 (:REWRITE ALL-NATP-OF-CDR))
     (127 27
          (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (111 111 (:TYPE-PRESCRIPTION ALL-NATP))
     (100 50
          (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
     (96 96 (:TYPE-PRESCRIPTION ALL-<))
     (93 36
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (80 40
         (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (78 78 (:REWRITE DEFAULT-CAR))
     (73 73 (:REWRITE DEFAULT-CDR))
     (66 66
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (64 16 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (61 61 (:REWRITE USE-ALL-CONSP-2))
     (61 61 (:REWRITE USE-ALL-CONSP))
     (54 7 (:REWRITE NO-ATOMS-WHEN-ALL-MYQUOTEP))
     (51 41 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (50 40 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (48 6 (:DEFINITION MEMBER-EQUAL))
     (44 32 (:REWRITE DEFAULT-<-1))
     (44 22
         (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (41 41
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (41 41 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (41 41 (:REWRITE ALL-<-TRANSITIVE))
     (40 40
         (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (40 16
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (36 18
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (36 18
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (36 12
         (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-CAR-AND-QUOTE))
     (36 2 (:REWRITE ALL-DARGP-LESS-THAN-OF-CDR))
     (34 34 (:REWRITE USE-ALL-<=-2))
     (34 34 (:REWRITE USE-ALL-<=))
     (34 17
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (33 33 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (32 32 (:REWRITE USE-ALL-<-2))
     (32 32 (:REWRITE USE-ALL-<))
     (32 32 (:REWRITE DEFAULT-<-2))
     (32 16
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (30 30 (:REWRITE USE-ALL-NATP-2))
     (30 30 (:REWRITE USE-ALL-NATP))
     (30 30
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (27 27 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
     (27 27
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (27 17
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (25 25
         (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (24 24 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (24 24 (:TYPE-PRESCRIPTION ALL-CONSP))
     (24 12
         (:REWRITE DARGP-LESS-THAN-WHEN-MYQUOTEP-CHEAP))
     (22 22 (:TYPE-PRESCRIPTION NATP))
     (22 22
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (19 17
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (18 18 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (18 12
         (:REWRITE DARGP-LESS-THAN-WHEN-NATP-CHEAP))
     (17 17
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (17 17
         (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
     (16 16
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (16 16
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (16 16
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (15 7
         (:REWRITE ALL-MYQUOTEP-WHEN-NOT-CONSP))
     (14 7
         (:REWRITE ALL-MYQUOTEP-WHEN-NO-ATOMS-AND-ALL-DARGP-CHEAP))
     (12 12 (:TYPE-PRESCRIPTION MYQUOTEP))
     (12 12 (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
     (12 12 (:REWRITE USE-ALL-DARGP-LESS-THAN))
     (12 12
         (:REWRITE DARGP-LESS-THAN-WHEN-QUOTEP-CHEAP))
     (12 12
         (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-NTH-0-AND-QUOTE))
     (12 12 (:REWRITE DARGP-LESS-THAN-MONO))
     (12 12
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (12 12 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (12 4
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (6 6
        (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
     (6 6
        (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
     (6 6
        (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
     (6 6
        (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (4 4 (:REWRITE USE-ALL-RATIONALP-2))
     (4 4 (:REWRITE USE-ALL-RATIONALP))
     (4 2 (:REWRITE ALL-MYQUOTEP-OF-CDR-CHEAP))
     (3 3
        (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (2 2 (:TYPE-PRESCRIPTION DARGP-LESS-THAN)))
(TRUE-LISTP-OF-MV-NTH-1-OF-FIRST-ATOM
     (20 4 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (12 4
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (9 9 (:REWRITE USE-ALL-CONSP-2))
     (9 9 (:REWRITE USE-ALL-CONSP))
     (8 8 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (8 8 (:TYPE-PRESCRIPTION ALL-CONSP))
     (8 8
        (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
     (8 4
        (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (6 6 (:REWRITE DEFAULT-CAR))
     (4 4
        (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (4 4
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (4 4 (:REWRITE DEFAULT-CDR))
     (4 4
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (4 4
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (4 4
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (4 4 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP)))
(NATP-OF-MV-NTH-0-OF-FIRST-ATOM
     (401 4
          (:REWRITE <-OF-MV-NTH-0-OF-FIRST-ATOM))
     (361 4
          (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
     (232 10 (:DEFINITION NAT-LISTP))
     (164 5 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (148 5 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (122 11 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (108 13
          (:REWRITE NO-ATOMS-WHEN-ALL-MYQUOTEP))
     (61 61 (:TYPE-PRESCRIPTION NAT-LISTP))
     (60 12 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (59 59 (:REWRITE USE-ALL-CONSP-2))
     (59 59 (:REWRITE USE-ALL-CONSP))
     (52 8 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (45 45 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (45 17
         (:REWRITE ALL-MYQUOTEP-WHEN-NOT-CONSP))
     (36 12
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (31 17
         (:REWRITE ALL-MYQUOTEP-WHEN-NO-ATOMS-AND-ALL-DARGP-CHEAP))
     (30 30 (:TYPE-PRESCRIPTION ALL-NATP))
     (27 27 (:REWRITE DEFAULT-CAR))
     (27 4 (:REWRITE ALL-DARGP-LESS-THAN-OF-0))
     (24 24 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (24 24 (:TYPE-PRESCRIPTION ALL-CONSP))
     (24 12
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (23 23 (:REWRITE DEFAULT-CDR))
     (22 11
         (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (20 11 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (20 1 (:REWRITE ALL-<-OF-CDR))
     (19 2 (:REWRITE ALL-NATP-OF-CDR))
     (18 9
         (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (18 8 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (14 14 (:TYPE-PRESCRIPTION ALL-<))
     (14 8 (:REWRITE DEFAULT-<-1))
     (14 5 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (12 12
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (12 12
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (12 12
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (12 12
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (12 12
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (12 12 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (12 6 (:REWRITE ALL-MYQUOTEP-OF-CDR-CHEAP))
     (11 11
         (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (10 10 (:REWRITE USE-ALL-NATP-2))
     (10 10 (:REWRITE USE-ALL-NATP))
     (10 10
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (9 9
        (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
     (8 8 (:REWRITE USE-ALL-<=-2))
     (8 8 (:REWRITE USE-ALL-<=))
     (8 8 (:REWRITE USE-ALL-<-2))
     (8 8 (:REWRITE USE-ALL-<))
     (8 8
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (8 8
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (8 8 (:REWRITE DEFAULT-<-2))
     (8 8
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (8 8
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (8 4 (:REWRITE USE-ALL-<-FOR-CAR))
     (8 4
        (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (8 4
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (8 4
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (6 2
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (5 5 (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (5 5 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (5 5 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (5 5 (:REWRITE ALL-<-TRANSITIVE))
     (5 4
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (4 4 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (4 4
        (:TYPE-PRESCRIPTION ALL-DARGP-LESS-THAN))
     (4 4
        (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (4 4
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (2 2 (:REWRITE USE-ALL-RATIONALP-2))
     (2 2 (:REWRITE USE-ALL-RATIONALP)))
(INTEGERP-OF-MV-NTH-0-OF-FIRST-ATOM)
(<=-OF-0-AND-MV-NTH-0-OF-FIRST-ATOM)
(ALL-DARGP-OF-MV-NTH1-OF-FIRST-ATOM
     (60 8 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (35 7 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (22 11
         (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (21 21 (:REWRITE USE-ALL-CONSP-2))
     (21 21 (:REWRITE USE-ALL-CONSP))
     (21 7
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (14 14 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (14 14 (:TYPE-PRESCRIPTION ALL-CONSP))
     (14 7
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (11 11 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (11 11
         (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
     (9 9 (:REWRITE DEFAULT-CAR))
     (7 7
        (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (7 7
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (7 7 (:REWRITE DEFAULT-CDR))
     (7 7
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (7 7
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (7 7
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (7 7 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP)))
(NOT-<-OF-LARGEST-NON-QUOTEP-AND-MV-NTH-0-OF-FIRST-ATOM
     (952 476
          (:TYPE-PRESCRIPTION NATP-OF-LARGEST-NON-QUOTEP))
     (376 12 (:REWRITE USE-ALL-<-FOR-CAR))
     (374 374 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (348 9 (:REWRITE <-OF-LARGEST-NON-QUOTEP))
     (296 12 (:DEFINITION NAT-LISTP))
     (182 14
          (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
     (180 4 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (164 4 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (146 10 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (128 8 (:DEFINITION NATP))
     (104 6 (:REWRITE ALL-DARGP-LESS-THAN-OF-CDR))
     (96 23 (:REWRITE DEFAULT-<-1))
     (80 4 (:REWRITE NOT-<-OF-CAR-WHEN-ALL-<-2))
     (70 23 (:REWRITE DEFAULT-<-2))
     (68 68 (:TYPE-PRESCRIPTION NAT-LISTP))
     (64 64 (:TYPE-PRESCRIPTION ALL-<))
     (60 12 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (60 8 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (60 8 (:REWRITE NO-ATOMS-WHEN-ALL-MYQUOTEP))
     (51 51 (:REWRITE USE-ALL-CONSP-2))
     (51 51 (:REWRITE USE-ALL-CONSP))
     (50 28 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (48 6 (:REWRITE ALL-<-OF-CDR))
     (44 4 (:REWRITE DEFAULT-+-2))
     (41 41 (:REWRITE DEFAULT-CDR))
     (36 14
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (36 12
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (34 34 (:REWRITE DEFAULT-CAR))
     (30 30 (:TYPE-PRESCRIPTION ALL-NATP))
     (30 15
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (29 29
         (:TYPE-PRESCRIPTION ALL-DARGP-LESS-THAN))
     (28 28
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (28 28 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (28 28 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (28 28 (:REWRITE ALL-<-TRANSITIVE))
     (28 14
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (27 3
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (24 24 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (24 24 (:TYPE-PRESCRIPTION ALL-CONSP))
     (24 12
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (24 4 (:REWRITE USE-ALL-<=-FOR-CAR))
     (23 23 (:REWRITE USE-ALL-<=-2))
     (23 23 (:REWRITE USE-ALL-<=))
     (23 23 (:REWRITE USE-ALL-<-2))
     (23 23 (:REWRITE USE-ALL-<))
     (23 23
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (20 10
         (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (19 9
         (:REWRITE ALL-MYQUOTEP-WHEN-NOT-CONSP))
     (18 9
         (:REWRITE LARGEST-NON-QUOTEP-WHEN-ALL-MYQUOTEP-CHEAP))
     (18 9
         (:REWRITE ALL-MYQUOTEP-WHEN-NO-ATOMS-AND-ALL-DARGP-CHEAP))
     (18 2 (:REWRITE ALL-NATP-OF-CDR))
     (14 14
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (14 14
         (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
     (14 7
         (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (12 12
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (12 12
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (12 12
         (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (12 12
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (12 12
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (12 12
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (12 12
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (12 12 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (12 6
         (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (12 6
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (12 6
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (10 10
         (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (10 10 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (9 5 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (8 8 (:TYPE-PRESCRIPTION ALL-<=))
     (8 8 (:REWRITE USE-ALL-NATP-2))
     (8 8 (:REWRITE USE-ALL-NATP))
     (8 8
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (7 7
        (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
     (6 6 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (6 6
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (6 6
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (6 3 (:REWRITE ALL-MYQUOTEP-OF-CDR-CHEAP))
     (5 5
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (4 4 (:REWRITE DEFAULT-+-1))
     (4 4
        (:REWRITE ALL-<=-WHEN-NOT-CONSP-CHEAP))
     (4 4 (:REWRITE ALL-<=-WHEN-NOT-CONSP))
     (4 4 (:REWRITE ALL-<=-MONOTONE))
     (3 3 (:REWRITE USE-ALL-RATIONALP-2))
     (3 3 (:REWRITE USE-ALL-RATIONALP)))
(FIRST-ATOM-BOUND-LEMMA
     (386 2 (:DEFINITION LARGEST-NON-QUOTEP))
     (220 8 (:REWRITE USE-ALL-<-FOR-CAR))
     (184 7 (:DEFINITION NAT-LISTP))
     (182 2 (:DEFINITION NFIX))
     (169 7 (:DEFINITION NATP))
     (154 2 (:DEFINITION MAX))
     (143 1 (:LINEAR LARGEST-NON-QUOTEP-BOUND))
     (120 7 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (98 2 (:REWRITE <-OF-LARGEST-NON-QUOTEP))
     (95 4 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (91 4 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (86 8 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (80 40
         (:TYPE-PRESCRIPTION NATP-OF-LARGEST-NON-QUOTEP))
     (54 4
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
     (48 48 (:TYPE-PRESCRIPTION NAT-LISTP))
     (34 2 (:REWRITE ALL-DARGP-LESS-THAN-OF-CDR))
     (27 27 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (24 24 (:TYPE-PRESCRIPTION ALL-<))
     (23 23 (:TYPE-PRESCRIPTION ALL-NATP))
     (22 1
         (:LINEAR LARGEST-NON-QUOTEP-BOUND-ALT))
     (21 11 (:REWRITE DEFAULT-<-1))
     (20 1 (:DEFINITION FIRST-ATOM))
     (18 18 (:REWRITE USE-ALL-CONSP-2))
     (18 18 (:REWRITE USE-ALL-CONSP))
     (18 10 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (16 16 (:REWRITE DEFAULT-CDR))
     (16 2 (:REWRITE ALL-<-OF-CDR))
     (15 3 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (14 12 (:REWRITE DEFAULT-CAR))
     (14 7
         (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (12 6
         (:REWRITE LARGEST-NON-QUOTEP-WHEN-ALL-MYQUOTEP-CHEAP))
     (11 11 (:REWRITE USE-ALL-<=-2))
     (11 11 (:REWRITE USE-ALL-<=))
     (11 11 (:REWRITE USE-ALL-<-2))
     (11 11 (:REWRITE USE-ALL-<))
     (11 11 (:REWRITE DEFAULT-<-2))
     (11 11
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (11 7 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (10 10
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (10 10 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (10 10 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (10 10 (:REWRITE ALL-<-TRANSITIVE))
     (10 4
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (9 9 (:REWRITE USE-ALL-NATP-2))
     (9 9 (:REWRITE USE-ALL-NATP))
     (9 9
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (9 9
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (9 3
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (8 8 (:TYPE-PRESCRIPTION NATP))
     (8 8
        (:TYPE-PRESCRIPTION ALL-DARGP-LESS-THAN))
     (8 8
        (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (8 4
        (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (8 4
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (8 4
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (8 4
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (7 7
        (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (6 6 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (6 6 (:TYPE-PRESCRIPTION ALL-CONSP))
     (6 4
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (6 3
        (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (5 5
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (4 4 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (4 4
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (4 4
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (4 4
        (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
     (4 1 (:DEFINITION NTH))
     (3 3
        (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (3 3
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (3 3
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (3 3
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (3 3
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (3 3 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (1 1 (:REWRITE NTH-WHEN-NOT-CDDR))
     (1 1 (:LINEAR <-OF-LARGEST-NON-QUOTEP)))
(ALL-DARGP-LESS-THAN-OF-PLUS1-OF-LARGEST-NON-QUOTEP
 (22863 863 (:DEFINITION NAT-LISTP))
 (18448 594 (:REWRITE USE-ALL-<-FOR-CAR))
 (14143 275
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
 (13934 800 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
 (11965 406 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
 (11719 180
        (:REWRITE ALL-DARGP-LESS-THAN-OF-CDR))
 (11422 406
        (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
 (9836 522 (:REWRITE ALL-<-OF-CDR))
 (9242 1093 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
 (8665 565 (:REWRITE USE-ALL-NATP-FOR-CAR))
 (7242 363 (:REWRITE ALL-NATP-OF-CDR))
 (4969 4969 (:TYPE-PRESCRIPTION NAT-LISTP))
 (3763 672
       (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
 (2546 2546 (:TYPE-PRESCRIPTION ALL-<))
 (2245 2245 (:TYPE-PRESCRIPTION ALL-NATP))
 (2218 2214 (:REWRITE DEFAULT-CDR))
 (2050 326 (:REWRITE USE-ALL-CONSP-FOR-CAR))
 (1806 1093 (:REWRITE ALL-<-WHEN-NOT-CONSP))
 (1600 800
       (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
 (1535 1485 (:REWRITE DEFAULT-CAR))
 (1377 1377 (:REWRITE USE-ALL-CONSP-2))
 (1377 1377 (:REWRITE USE-ALL-CONSP))
 (1275 141
       (:REWRITE USE-ALL-DARGP-LESS-THAN-FOR-CAR))
 (1172 729 (:REWRITE DEFAULT-<-1))
 (1093 1093
       (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
 (1093 1093 (:REWRITE ALL-<-TRANSITIVE-FREE))
 (1093 1093 (:REWRITE ALL-<-TRANSITIVE))
 (1020 800 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
 (967 797
      (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
 (916
    19
    (:REWRITE NOT-<-OF-PLUS1-OF-LARGEST-NON-QUOTEP-WHEN-ALL-DARGP-LESS-THAN))
 (902 262
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
 (900 180 (:REWRITE ALL-CONSP-OF-CDR))
 (800 800
      (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
 (784 729 (:REWRITE DEFAULT-<-2))
 (772 772 (:TYPE-PRESCRIPTION ALL-CONSP))
 (760 760 (:REWRITE USE-ALL-<=-2))
 (760 760 (:REWRITE USE-ALL-<=))
 (749 320
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
 (738 326
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
 (729 729 (:REWRITE USE-ALL-<-2))
 (729 729 (:REWRITE USE-ALL-<))
 (652 326
      (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
 (640 320
      (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
 (619 619
      (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
 (618 139 (:DEFINITION MEMBER-EQUAL))
 (590 590 (:REWRITE USE-ALL-NATP-2))
 (590 590 (:REWRITE USE-ALL-NATP))
 (555 41 (:REWRITE NOT-<-OF-CAR-WHEN-ALL-<-2))
 (550 275
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
 (550 275
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
 (536 268
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
 (512 512
      (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
 (440 60
      (:REWRITE NO-ATOMS-WHEN-ALL-MYQUOTEP))
 (419 141
      (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-CAR-AND-QUOTE))
 (412 412 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
 (397 221
      (:REWRITE LARGEST-NON-QUOTEP-WHEN-ALL-MYQUOTEP-CHEAP))
 (386 386
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
 (386 386 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
 (378 74 (:REWRITE DEFAULT-+-2))
 (372 186
      (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-CAR-WHEN-SYMBOL-TERM-ALISTP))
 (326 326
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
 (326 326
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
 (326 326
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
 (315 315 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
 (308 138
      (:REWRITE ALL-MYQUOTEP-WHEN-NOT-CONSP))
 (282 141
      (:REWRITE DARGP-LESS-THAN-WHEN-MYQUOTEP-CHEAP))
 (275 275 (:TYPE-PRESCRIPTION ALL-INTEGERP))
 (262 262
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
 (246 41 (:REWRITE USE-ALL-<=-FOR-CAR))
 (224 116
      (:REWRITE ALL-MYQUOTEP-OF-CDR-CHEAP))
 (160 27 (:LINEAR <-OF-LARGEST-NON-QUOTEP))
 (149 141 (:REWRITE USE-ALL-DARGP-LESS-THAN))
 (141 141 (:TYPE-PRESCRIPTION MYQUOTEP))
 (141 141
      (:REWRITE USE-ALL-DARGP-LESS-THAN-2))
 (141 141
      (:REWRITE DARGP-LESS-THAN-WHEN-EQUAL-OF-NTH-0-AND-QUOTE))
 (131 131
      (:REWRITE DARGP-LESS-THAN-WHEN-QUOTEP-CHEAP))
 (87 44
     (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
 (82 82 (:TYPE-PRESCRIPTION ALL-<=))
 (80 25 (:TYPE-PRESCRIPTION MAX))
 (74 74 (:REWRITE DEFAULT-+-1))
 (62 7 (:REWRITE ALL-DARGP-LESS-THAN-OF-0))
 (44 44
     (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
 (43 37 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
 (41 41
     (:REWRITE ALL-<=-WHEN-NOT-CONSP-CHEAP))
 (41 41 (:REWRITE ALL-<=-WHEN-NOT-CONSP))
 (41 41 (:REWRITE ALL-<=-MONOTONE))
 (37 37
     (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
 (29 29 (:REWRITE NTH-WHEN-NOT-CDDR))
 (14 14 (:TYPE-PRESCRIPTION MEMBERP))
 (11 11
     (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
 (11 11
     (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
 (10 10
     (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
 (9 3 (:REWRITE USE-ALL-DARGP))
 (8 8
    (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
 (8 1
    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (5 5
    (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
 (5 1 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
 (4 4
    (:REWRITE NOT-<-OF-+-1-AND-CAR-WHEN-ALL-<))
 (3 3 (:REWRITE USE-ALL-DARGP-2))
 (3 3
    (:REWRITE DARGP-WHEN-EQUAL-OF-QUOTE-AND-CAR-CHEAP))
 (3 3 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
 (2 2 (:TYPE-PRESCRIPTION ALL-RATIONALP))
 (1 1 (:REWRITE USE-ALL-RATIONALP-2))
 (1 1 (:REWRITE USE-ALL-RATIONALP))
 (1 1
    (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
 (1 1
    (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP)))
(ALL-DARGP-LESS-THAN-LEMMA2
     (1616 10 (:LINEAR LARGEST-NON-QUOTEP-BOUND))
     (1412 706
           (:TYPE-PRESCRIPTION NATP-OF-LARGEST-NON-QUOTEP))
     (1339 29 (:DEFINITION NATP))
     (916 20 (:REWRITE USE-ALL-<-FOR-CAR))
     (810 31 (:DEFINITION NAT-LISTP))
     (734 36 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (664 37 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (381 22 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (338 338 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (332 5 (:REWRITE <=-OF-LARGEST-NON-QUOTEP))
     (325 26
          (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
     (302 5
          (:REWRITE <=-OF-LARGEST-NON-QUOTEP-WHEN-ALL-DARGP-LESS-THAN))
     (285 22 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (241 8
          (:REWRITE ALL-DARGP-LESS-THAN-OF-MV-NTH-1-OF-FIRST-ATOM))
     (224 224 (:TYPE-PRESCRIPTION NAT-LISTP))
     (203 10
          (:LINEAR LARGEST-NON-QUOTEP-BOUND-ALT))
     (190 55 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (185 37 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (142 11 (:REWRITE ALL-<-OF-CDR))
     (135 7 (:REWRITE ALL-DARGP-LESS-THAN-OF-CDR))
     (134 134 (:REWRITE USE-ALL-CONSP-2))
     (134 134 (:REWRITE USE-ALL-CONSP))
     (119 119 (:TYPE-PRESCRIPTION ALL-NATP))
     (113 5 (:REWRITE <-OF-LARGEST-NON-QUOTEP))
     (112 112 (:TYPE-PRESCRIPTION ALL-<))
     (103 85 (:REWRITE DEFAULT-CAR))
     (90 18 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (87 26
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (74 37
         (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (71 71 (:TYPE-PRESCRIPTION NATP))
     (71 71 (:REWRITE DEFAULT-CDR))
     (62 9 (:REWRITE NO-ATOMS-WHEN-ALL-MYQUOTEP))
     (56 56
         (:TYPE-PRESCRIPTION ALL-DARGP-LESS-THAN))
     (55 55
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (55 55 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (55 55 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (55 55 (:REWRITE ALL-<-TRANSITIVE))
     (54 18
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (54 6 (:REWRITE ALL-NATP-OF-CDR))
     (53 33 (:REWRITE DEFAULT-<-2))
     (52 26
         (:REWRITE LARGEST-NON-QUOTEP-WHEN-ALL-MYQUOTEP-CHEAP))
     (52 26
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (50 33 (:REWRITE DEFAULT-<-1))
     (47 47
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (45 45 (:REWRITE USE-ALL-NATP-2))
     (45 45 (:REWRITE USE-ALL-NATP))
     (43 26
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (38 10 (:DEFINITION NTH))
     (37 37
         (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (36 36 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (36 36 (:TYPE-PRESCRIPTION ALL-CONSP))
     (36 18
         (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (36 18
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (36 18
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (36 18
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (34 34 (:REWRITE USE-ALL-<=-2))
     (34 34 (:REWRITE USE-ALL-<=))
     (34 34
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (34 2
         (:REWRITE NATP-OF-LARGEST-NON-QUOTEP-2))
     (33 33 (:REWRITE USE-ALL-<-2))
     (33 33 (:REWRITE USE-ALL-<))
     (29 29
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (27 27
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (26 26
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (26 26
         (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
     (26 5 (:REWRITE DEFAULT-+-2))
     (23 11
         (:REWRITE ALL-MYQUOTEP-WHEN-NOT-CONSP))
     (22 13
         (:REWRITE ALL-MYQUOTEP-WHEN-NO-ATOMS-AND-ALL-DARGP-CHEAP))
     (20 20
         (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (18 18
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (18 18 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (18 18
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (18 18
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (18 18
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (18 18
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (18 18
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (18 18 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (17 5
         (:REWRITE ALL-DARGP-LESS-THAN-OF-PLUS1-OF-LARGEST-NON-QUOTEP))
     (10 10 (:REWRITE NTH-WHEN-NOT-CDDR))
     (10 10 (:LINEAR <-OF-LARGEST-NON-QUOTEP))
     (10 5
         (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (10 2
         (:REWRITE EQUAL-OF--1-AND-LARGEST-NON-QUOTEP))
     (8 4 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (5 5 (:REWRITE DEFAULT-+-1))
     (5 5
        (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
     (4 4
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (4 2 (:REWRITE ALL-MYQUOTEP-OF-CDR-CHEAP)))
(<-OF-LARGEST-NON-QUOTEP-AND-0
     (692 346
          (:TYPE-PRESCRIPTION NATP-OF-LARGEST-NON-QUOTEP))
     (572 29 (:DEFINITION NAT-LISTP))
     (358 14 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (309 29 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (280 14 (:DEFINITION NATP))
     (143 143 (:TYPE-PRESCRIPTION NAT-LISTP))
     (101 14 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (85 13 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (77 77 (:REWRITE USE-ALL-CONSP-2))
     (77 77 (:REWRITE USE-ALL-CONSP))
     (75 11 (:REWRITE ALL-DARGP-LESS-THAN-OF-0))
     (72 25 (:REWRITE DEFAULT-<-1))
     (60 30
         (:REWRITE ALL-MYQUOTEP-WHEN-NO-ATOMS-AND-ALL-DARGP-CHEAP))
     (60 3 (:REWRITE ALL-<-OF-CDR))
     (58 29
         (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (57 6 (:REWRITE ALL-NATP-OF-CDR))
     (47 47 (:REWRITE DEFAULT-CAR))
     (44 44 (:REWRITE DEFAULT-CDR))
     (44 27 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (39 13
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (34 34 (:TYPE-PRESCRIPTION ALL-CONSP))
     (31 14 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (30 15 (:REWRITE ALL-MYQUOTEP-OF-CDR-CHEAP))
     (28 14 (:REWRITE USE-ALL-<-FOR-CAR))
     (28 14
         (:REWRITE LARGEST-NON-QUOTEP-WHEN-ALL-MYQUOTEP-CHEAP))
     (28 14
         (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (28 14
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (28 14
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (26 26 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (26 26 (:REWRITE USE-ALL-<=-2))
     (26 26 (:REWRITE USE-ALL-<=))
     (26 26
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (26 13
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (25 25 (:REWRITE USE-ALL-<-2))
     (25 25 (:REWRITE USE-ALL-<))
     (25 25
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (25 25 (:REWRITE DEFAULT-<-2))
     (24 12
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (20 4 (:REWRITE ALL-CONSP-OF-CDR))
     (18 9
         (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (17 17
         (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (17 17 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (14 14 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (14 14 (:REWRITE USE-ALL-NATP-2))
     (14 14 (:REWRITE USE-ALL-NATP))
     (14 14
         (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (14 14
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (14 14
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (14 14
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (14 14
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (14 14 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (14 14 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (14 14 (:REWRITE ALL-<-TRANSITIVE))
     (13 13
         (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (13 13
         (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (13 13
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (13 13
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (13 7 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (9 9
        (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
     (6 2 (:REWRITE USE-ALL-DARGP))
     (4 4 (:TYPE-PRESCRIPTION MEMBERP))
     (2 2 (:REWRITE USE-ALL-DARGP-2))
     (2 2 (:REWRITE NTH-WHEN-NOT-CDDR))
     (2 2
        (:REWRITE MYQUOTEP-WHEN-DARGP-LESS-THAN))
     (2 2
        (:REWRITE MYQUOTEP-WHEN-BOUNDED-DAG-EXPRP))
     (2 2 (:REWRITE DARGP-WHEN-DARGP-LESS-THAN))
     (1 1
        (:REWRITE QUOTE-LEMMA-FOR-ALL-DARGP-LESS-THAN-GEN-ALT))
     (1 1 (:LINEAR <-OF-LARGEST-NON-QUOTEP)))
(ALL-DARGP-LESS-THAN-LEMMA
     (193 1 (:DEFINITION LARGEST-NON-QUOTEP))
     (91 1 (:DEFINITION NFIX))
     (77 1 (:DEFINITION MAX))
     (73 3 (:REWRITE USE-ALL-<-FOR-CAR))
     (52 2 (:DEFINITION NAT-LISTP))
     (49 1 (:REWRITE <-OF-LARGEST-NON-QUOTEP))
     (33 1 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (32 1 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (30 1 (:DEFINITION NO-ATOMS))
     (27 2 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (27 2
         (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
     (24 2 (:DEFINITION NATP))
     (20 1 (:DEFINITION FIRST-ATOM))
     (18 2 (:REWRITE NO-ATOMS-WHEN-ALL-MYQUOTEP))
     (17 1 (:REWRITE ALL-DARGP-LESS-THAN-OF-CDR))
     (15 3 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (13 13 (:TYPE-PRESCRIPTION NAT-LISTP))
     (12 12 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (11 11 (:REWRITE USE-ALL-CONSP-2))
     (11 11 (:REWRITE USE-ALL-CONSP))
     (10 10 (:TYPE-PRESCRIPTION ALL-<))
     (10 5
         (:TYPE-PRESCRIPTION NATP-OF-LARGEST-NON-QUOTEP))
     (10 2 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (9 5
        (:TYPE-PRESCRIPTION ACL2-NUMBERP-OF-LARGEST-NON-QUOTEP))
     (9 4 (:REWRITE DEFAULT-<-1))
     (9 3
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (8 8 (:REWRITE DEFAULT-CDR))
     (8 1 (:REWRITE ALL-<-OF-CDR))
     (7 4 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (6 6 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (6 6 (:TYPE-PRESCRIPTION ALL-NATP))
     (6 6 (:TYPE-PRESCRIPTION ALL-CONSP))
     (6 3
        (:REWRITE LARGEST-NON-QUOTEP-WHEN-ALL-MYQUOTEP-CHEAP))
     (6 3
        (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (6 2
        (:REWRITE ALL-MYQUOTEP-WHEN-NOT-CONSP))
     (5 5
        (:TYPE-PRESCRIPTION INTEGERP-OF-LARGEST-NON-QUOTEP))
     (5 5 (:REWRITE DEFAULT-CAR))
     (5 2
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
     (5 1 (:REWRITE DEFAULT-+-2))
     (4 4 (:REWRITE USE-ALL-<=-2))
     (4 4 (:REWRITE USE-ALL-<=))
     (4 4 (:REWRITE USE-ALL-<-2))
     (4 4 (:REWRITE USE-ALL-<))
     (4 4 (:REWRITE DEFAULT-<-2))
     (4 4 (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (4 4 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (4 4 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (4 4 (:REWRITE ALL-<-TRANSITIVE))
     (4 4
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (4 2
        (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (4 2
        (:REWRITE ALL-MYQUOTEP-WHEN-NO-ATOMS-AND-ALL-DARGP-CHEAP))
     (4 2
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
     (3 3
        (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (3 3
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (3 3
        (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (3 3
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (3 3
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (3 3
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (3 3
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (3 3 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
     (3 2
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
     (3 1 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (2 2 (:REWRITE USE-ALL-NATP-2))
     (2 2 (:REWRITE USE-ALL-NATP))
     (2 2
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (2 2
        (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (2 2 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (2 2
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
     (2 2
        (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
     (2 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (2 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (2 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (2 1 (:REWRITE ALL-MYQUOTEP-OF-CDR-CHEAP))
     (2 1
        (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (1 1 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (1 1
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (1 1
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (1 1 (:REWRITE DEFAULT-+-1))
     (1 1
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (1 1
        (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN)))
(FIND-MATCHING-NODE
   (185 115
        (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
   (115 115
        (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
   (66 33
       (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-AREF1))
   (55 5 (:REWRITE USE-ALL-NATP-FOR-CAR))
   (47 5 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
   (38 11 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
   (36 5
       (:REWRITE ALL-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
   (33 33
       (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
   (23 23 (:TYPE-PRESCRIPTION ALL-NATP))
   (18 9
       (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
   (16 16
       (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
   (16 4 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
   (13 13 (:REWRITE DEFAULT-CDR))
   (13 9 (:REWRITE ALL-<-TRANSITIVE-FREE))
   (13 9 (:REWRITE ALL-<-TRANSITIVE))
   (13 5 (:REWRITE DEFAULT-<-1))
   (12 12 (:REWRITE DEFAULT-CAR))
   (9 9 (:REWRITE USE-ALL-CONSP-2))
   (9 9 (:REWRITE USE-ALL-CONSP))
   (9 9 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
   (8 8
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
   (8 8
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
   (8 8
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
   (8 8
      (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
   (8 8 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
   (8 8 (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
   (8 8 (:REWRITE ALL-<-WHEN-NOT-CONSP))
   (8 8 (:LINEAR ARRAY2P-LINEAR))
   (8 6 (:REWRITE USE-ALL-<))
   (8 4
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
   (8 4
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
   (8 1
      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
   (6 6 (:REWRITE USE-ALL-<=-2))
   (6 6 (:REWRITE USE-ALL-<=))
   (6 6 (:REWRITE USE-ALL-<-2))
   (6 6
      (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
   (6 6
      (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
   (6 1
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-OF-CDR))
   (6 1
      (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
   (6 1 (:REWRITE <-OF-CAR-AND-ALEN1))
   (5 5 (:REWRITE USE-ALL-NATP-2))
   (5 5 (:REWRITE USE-ALL-NATP))
   (5 5 (:REWRITE NOT-ALL-<-WHEN-MEMBERP))
   (5 5
      (:REWRITE NOT-ALL-<-WHEN-MEMBER-EQUAL))
   (5 5
      (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
   (5 5
      (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
   (5 5 (:REWRITE DEFAULT-<-2))
   (5 1 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
   (4 4 (:TYPE-PRESCRIPTION ALL-INTEGERP))
   (4 4
      (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
   (4 4
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
   (3 3
      (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP))
   (3 1 (:REWRITE ALL-NATP-OF-CDR))
   (2 2 (:TYPE-PRESCRIPTION MEMBERP))
   (2 2 (:TYPE-PRESCRIPTION ALL-RATIONALP))
   (2 1 (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
   (1 1 (:REWRITE USE-ALL-RATIONALP-2))
   (1 1 (:REWRITE USE-ALL-RATIONALP))
   (1 1
      (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
   (1 1
      (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
   (1 1
      (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP)))
(INTEGERP-OF-FIND-MATCHING-NODE
     (684 25 (:REWRITE USE-ALL-<-FOR-CAR))
     (666 44 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (595 39 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (280 140
          (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
     (244 25 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (240 25 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (186 93
          (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-AREF1))
     (140 140
          (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
     (133 133 (:TYPE-PRESCRIPTION ALL-NATP))
     (116 29
          (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
     (93 93
         (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
     (91 91 (:REWRITE DEFAULT-CAR))
     (78 39
         (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (58 29 (:TYPE-PRESCRIPTION ALEN1-TYPE))
     (50 50 (:TYPE-PRESCRIPTION ALL-<))
     (50 25
         (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (50 25
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (50 25
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (44 44 (:REWRITE USE-ALL-NATP-2))
     (44 44 (:REWRITE USE-ALL-NATP))
     (44 44
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (39 39 (:REWRITE DEFAULT-CDR))
     (39 39
         (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (39 39 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (38 38 (:TYPE-PRESCRIPTION NATP))
     (32 32 (:REWRITE USE-ALL-CONSP-2))
     (32 32 (:REWRITE USE-ALL-CONSP))
     (31 31
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (29 29 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (29 29 (:TYPE-PRESCRIPTION ARRAY1P))
     (25 25 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (25 25 (:REWRITE USE-ALL-<=-2))
     (25 25 (:REWRITE USE-ALL-<=))
     (25 25 (:REWRITE USE-ALL-<-2))
     (25 25 (:REWRITE USE-ALL-<))
     (25 25
         (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (25 25
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (25 25
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (25 25 (:REWRITE DEFAULT-<-2))
     (25 25 (:REWRITE DEFAULT-<-1))
     (25 25
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (25 25 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (25 25 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (25 25 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (25 25 (:REWRITE ALL-<-TRANSITIVE))
     (25 25
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN)))
(NONNEG-OF-FIND-MATCHING-NODE
     (493 19 (:REWRITE USE-ALL-<-FOR-CAR))
     (481 31 (:REWRITE USE-ALL-NATP-FOR-CAR))
     (427 29 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
     (178 89
          (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
     (177 18 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
     (170 19 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
     (124 62
          (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-AREF1))
     (96 96 (:TYPE-PRESCRIPTION ALL-NATP))
     (89 89
         (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
     (62 62
         (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
     (60 15
         (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
     (56 28
         (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
     (55 55 (:REWRITE DEFAULT-CAR))
     (51 27 (:REWRITE DEFAULT-<-1))
     (37 37 (:TYPE-PRESCRIPTION ALL-<))
     (36 18
         (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
     (36 18
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
     (36 18
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
     (31 31 (:REWRITE USE-ALL-NATP-2))
     (31 31 (:REWRITE USE-ALL-NATP))
     (31 31
         (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (30 15 (:TYPE-PRESCRIPTION ALEN1-TYPE))
     (28 28
         (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
     (28 28 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
     (27 27 (:REWRITE USE-ALL-<=-2))
     (27 27 (:REWRITE USE-ALL-<=))
     (27 27 (:REWRITE USE-ALL-<-2))
     (27 27 (:REWRITE USE-ALL-<))
     (27 27
         (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
     (27 27 (:REWRITE DEFAULT-<-2))
     (27 27
         (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
     (26 26 (:TYPE-PRESCRIPTION NATP))
     (24 8
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (19 19
         (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
     (18 18 (:TYPE-PRESCRIPTION ALL-INTEGERP))
     (18 18
         (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
     (18 18
         (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
     (18 18
         (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
     (18 18 (:REWRITE ALL-<-WHEN-NOT-CONSP))
     (18 18 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
     (18 18 (:REWRITE ALL-<-TRANSITIVE-FREE))
     (18 18 (:REWRITE ALL-<-TRANSITIVE))
     (15 15 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (15 15 (:TYPE-PRESCRIPTION ARRAY1P))
     (15 15 (:REWRITE USE-ALL-CONSP-2))
     (15 15 (:REWRITE USE-ALL-CONSP))
     (15 15 (:REWRITE DEFAULT-CDR))
     (8 8 (:REWRITE USE-ALL-RATIONALP-2))
     (8 8 (:REWRITE USE-ALL-RATIONALP)))
(FIND-EXPR-USING-PARENTS
 (753 33
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
 (616 19
      (:REWRITE ALL-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
 (451 451 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
 (300 35
      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (214 15
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
 (207 1
      (:REWRITE ALL-<-OF-AREF1-WHEN-BOUNDED-DAG-PARENT-ENTRIESP))
 (200 100
      (:TYPE-PRESCRIPTION NATP-OF-MV-NTH-0-OF-FIRST-ATOM))
 (182 14
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
 (168 12
      (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
 (146 41 (:REWRITE ALL-<-WHEN-NOT-CONSP))
 (138 1 (:REWRITE FIRST-ATOM-BOUND-LEMMA))
 (133 14
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
 (108 12 (:DEFINITION MEMBER-EQUAL))
 (105 13
      (:REWRITE NO-ATOMS-WHEN-ALL-MYQUOTEP))
 (100 25
      (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
 (82 19
     (:REWRITE PSEUDO-DAG-ARRAYP-MONOTONE))
 (79 21
     (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
 (77 77 (:REWRITE USE-ALL-CONSP-2))
 (77 77 (:REWRITE USE-ALL-CONSP))
 (70 7 (:DEFINITION NATP))
 (66 33
     (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
 (60 41 (:REWRITE ALL-<-TRANSITIVE))
 (59 1
     (:REWRITE <=-OF-LARGEST-NON-QUOTEP-WHEN-ALL-DARGP-LESS-THAN))
 (56 56 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
 (56 42 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
 (54 23 (:REWRITE DEFAULT-<-1))
 (48 33
     (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
 (46 41
     (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
 (39 13
     (:REWRITE ALL-MYQUOTEP-WHEN-NOT-CONSP))
 (37 1 (:DEFINITION NAT-LISTP))
 (29 29
     (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
 (28 28 (:REWRITE USE-ALL-RATIONALP-2))
 (28 28 (:REWRITE USE-ALL-RATIONALP))
 (27 13
     (:REWRITE ALL-MYQUOTEP-WHEN-NO-ATOMS-AND-ALL-DARGP-CHEAP))
 (26 26 (:REWRITE DEFAULT-+-1))
 (26 24 (:REWRITE USE-ALL-<))
 (24 24 (:REWRITE USE-ALL-<=-2))
 (24 24 (:REWRITE USE-ALL-<=))
 (24 24 (:REWRITE USE-ALL-<-2))
 (24 24
     (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
 (21 21
     (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
 (18 1 (:REWRITE <=-OF-LARGEST-NON-QUOTEP))
 (14 14
     (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
 (14 14
     (:REWRITE BOUNDED-DAG-PARENT-ENTRIESP-MONOTONE))
 (14 14
     (:REWRITE BOUNDED-DAG-PARENT-ARRAYP-MONOTONE))
 (14 7
     (:TYPE-PRESCRIPTION NATP-OF-LARGEST-NON-QUOTEP))
 (14 7
     (:TYPE-PRESCRIPTION ACL2-NUMBERP-OF-LARGEST-NON-QUOTEP))
 (13 13 (:REWRITE DEFAULT-CAR))
 (12 12
     (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
 (12 12
     (:REWRITE SYMBOLP-WHEN-BOUNDED-DAG-EXPRP))
 (12 12
     (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
 (12 12 (:REWRITE DEFAULT-CDR))
 (12 3 (:REWRITE INTEGERP-OF-ALEN1))
 (10 10 (:REWRITE NOT-ALL-<-WHEN-MEMBERP))
 (10 10
     (:REWRITE NOT-ALL-<-WHEN-MEMBER-EQUAL))
 (8 8
    (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
 (7 7 (:REWRITE USE-ALL-NATP-2))
 (7 7 (:REWRITE USE-ALL-NATP))
 (7 7 (:REWRITE <-OF-+-OF-1-STRENGTHEN))
 (6 6 (:LINEAR ARRAY2P-LINEAR))
 (6 3
    (:REWRITE LARGEST-NON-QUOTEP-WHEN-ALL-MYQUOTEP-CHEAP))
 (6 2 (:REWRITE USE-ALL-CONSP-FOR-CAR))
 (4 2
    (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
 (4 2
    (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
 (3 3 (:REWRITE INTEGERP-OF-ALEN1-GEN))
 (2 2 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
 (2 2 (:TYPE-PRESCRIPTION MEMBERP))
 (2 2 (:TYPE-PRESCRIPTION ALL-CONSP))
 (2 2
    (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
 (2 2
    (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
 (2 2
    (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
 (2 2 (:LINEAR FIRST-ATOM-BOUND-LEMMA))
 (1 1
    (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
 (1 1 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP)))
(INTEGERP-OF-FIND-EXPR-USING-PARENTS
     (18 18 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (9 3 (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
     (8 1 (:REWRITE NO-ATOMS-WHEN-ALL-MYQUOTEP))
     (3 3 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (3 1
        (:REWRITE ALL-MYQUOTEP-WHEN-NOT-CONSP))
     (3 1 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (2 2 (:REWRITE USE-ALL-CONSP-2))
     (2 2 (:REWRITE USE-ALL-CONSP))
     (2 2 (:LINEAR ARRAY2P-LINEAR))
     (2 1
        (:REWRITE ALL-MYQUOTEP-WHEN-NO-ATOMS-AND-ALL-DARGP-CHEAP))
     (2 1
        (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (1 1
        (:REWRITE ALL-DARGP-WHEN-NOT-CONSP-CHEAP))
     (1 1
        (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN)))
(NONNEG-OF-FIND-EXPR-USING-PARENTS (6 6 (:TYPE-PRESCRIPTION POSP-OF-ALEN1)))
(<-OF-FIND-MATCHING-NODE
    (842 44 (:REWRITE USE-ALL-NATP-FOR-CAR))
    (821 55 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
    (464 232
         (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
    (346 38 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
    (263 38 (:REWRITE ALL-<-TRANSITIVE-FREE))
    (252 126
         (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-AREF1))
    (244 25 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
    (232 232
         (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
    (213 38 (:REWRITE ALL-<-TRANSITIVE))
    (178 178 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
    (169 99 (:REWRITE USE-ALL-<))
    (149 149 (:TYPE-PRESCRIPTION ALL-NATP))
    (136 34
         (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
    (126 126
         (:TYPE-PRESCRIPTION SYMBOLP-OF-CAR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
    (125 98 (:REWRITE DEFAULT-<-1))
    (105 105 (:REWRITE DEFAULT-CAR))
    (99 99 (:REWRITE USE-ALL-<=-2))
    (99 99 (:REWRITE USE-ALL-<=))
    (99 99 (:REWRITE USE-ALL-<-2))
    (99 99
        (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
    (98 98 (:REWRITE DEFAULT-<-2))
    (84 42
        (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
    (70 70 (:TYPE-PRESCRIPTION MEMBERP))
    (50 25
        (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
    (50 25
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
    (50 25
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
    (44 44 (:REWRITE USE-ALL-NATP-2))
    (44 44 (:REWRITE USE-ALL-NATP))
    (44 44
        (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
    (41 41 (:REWRITE USE-ALL-CONSP-2))
    (41 41 (:REWRITE USE-ALL-CONSP))
    (41 41 (:REWRITE DEFAULT-CDR))
    (41 35 (:REWRITE ALL-<-WHEN-NOT-CONSP))
    (39 39
        (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
    (39 39
        (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
    (39 39
        (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
    (39 39 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
    (38 38 (:TYPE-PRESCRIPTION NATP))
    (35 35
        (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
    (27 9
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
    (26 26
        (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
    (25 25 (:TYPE-PRESCRIPTION ALL-INTEGERP))
    (25 25
        (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
    (18 18 (:LINEAR ARRAY2P-LINEAR))
    (18 6 (:DEFINITION ATOM))
    (14 14
        (:REWRITE BOUNDED-DAG-PARENT-ARRAYP-MONOTONE))
    (13 13 (:REWRITE NOT-ALL-<-WHEN-MEMBERP))
    (13 13
        (:REWRITE NOT-ALL-<-WHEN-MEMBER-EQUAL))
    (9 9 (:REWRITE USE-ALL-RATIONALP-2))
    (9 9 (:REWRITE USE-ALL-RATIONALP))
    (9 3 (:REWRITE ALL-NATP-OF-CDR)))
(<-OF-FIND-EXPR-USING-PARENTS
   (257 1 (:REWRITE FIRST-ATOM-BOUND-LEMMA))
   (174 87
        (:TYPE-PRESCRIPTION NATP-OF-MV-NTH-0-OF-FIRST-ATOM))
   (161 161 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
   (100 1 (:LINEAR LARGEST-NON-QUOTEP-BOUND))
   (74 3 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
   (52 6 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
   (50 4
       (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
   (49 1 (:REWRITE USE-ALL-NATP-FOR-CAR))
   (48 12
       (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
   (48 8
       (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
   (48 3
       (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
   (44 2 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
   (38 1 (:REWRITE <=-OF-LARGEST-NON-QUOTEP))
   (37 1 (:DEFINITION NAT-LISTP))
   (35 15 (:REWRITE DEFAULT-<-2))
   (32 1
       (:LINEAR LARGEST-NON-QUOTEP-BOUND-ALT))
   (30 15
       (:TYPE-PRESCRIPTION NATP-OF-LARGEST-NON-QUOTEP))
   (30 15
       (:TYPE-PRESCRIPTION ACL2-NUMBERP-OF-LARGEST-NON-QUOTEP))
   (29 1
       (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
   (26 16
       (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
   (26 1
       (:REWRITE <=-OF-LARGEST-NON-QUOTEP-WHEN-ALL-DARGP-LESS-THAN))
   (25 4
       (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
   (24 24 (:REWRITE USE-ALL-CONSP-2))
   (24 24 (:REWRITE USE-ALL-CONSP))
   (24 4 (:DEFINITION MEMBER-EQUAL))
   (23 15 (:REWRITE DEFAULT-<-1))
   (19 15 (:REWRITE USE-ALL-<))
   (19 5
       (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
   (17 5 (:REWRITE ALL-<-WHEN-NOT-CONSP))
   (16 16 (:REWRITE USE-ALL-<=-2))
   (16 16 (:REWRITE USE-ALL-<=))
   (16 2 (:REWRITE NO-ATOMS-WHEN-ALL-MYQUOTEP))
   (15 15 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
   (15 15 (:REWRITE USE-ALL-<-2))
   (14 7
       (:REWRITE LARGEST-NON-QUOTEP-WHEN-ALL-MYQUOTEP-CHEAP))
   (13 10 (:REWRITE DEFAULT-+-2))
   (13 1
       (:LINEAR LARGEST-NON-QUOTEP-BOUND-LINEAR))
   (12 12 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
   (12 4 (:REWRITE USE-ALL-CONSP-FOR-CAR))
   (12 2
       (:REWRITE ALL-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
   (10 10 (:REWRITE DEFAULT-+-1))
   (10 8 (:REWRITE DEFAULT-CAR))
   (10 4
       (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
   (8 8
      (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
   (8 8
      (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
   (8 4
      (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
   (8 4
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
   (8 4
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
   (6 6
      (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
   (6 2
      (:REWRITE ALL-MYQUOTEP-WHEN-NOT-CONSP))
   (6 2 (:DEFINITION ATOM))
   (5 5 (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
   (5 5 (:REWRITE ALL-<-TRANSITIVE))
   (4 4 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
   (4 4
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
   (4 4 (:TYPE-PRESCRIPTION MEMBERP))
   (4 4 (:TYPE-PRESCRIPTION ALL-NATP))
   (4 4 (:TYPE-PRESCRIPTION ALL-CONSP))
   (4 4 (:REWRITE USE-ALL-RATIONALP-2))
   (4 4 (:REWRITE USE-ALL-RATIONALP))
   (4 4
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
   (4 4
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
   (4 4
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
   (4 4
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
   (4 4
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
   (4 2
      (:REWRITE ALL-MYQUOTEP-WHEN-NO-ATOMS-AND-ALL-DARGP-CHEAP))
   (4 1 (:DEFINITION NTH))
   (3 3 (:REWRITE DEFAULT-CDR))
   (3 3
      (:REWRITE BOUNDED-DAG-PARENT-ENTRIESP-MONOTONE))
   (3 3
      (:REWRITE BOUNDED-DAG-PARENT-ARRAYP-MONOTONE))
   (3 1 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
   (2 2 (:REWRITE USE-ALL-NATP-2))
   (2 2 (:REWRITE USE-ALL-NATP))
   (2 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
   (2 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
   (2 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
   (2 2
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
   (2 2 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
   (2 2 (:REWRITE <-OF-FIND-MATCHING-NODE))
   (2 2 (:LINEAR FIRST-ATOM-BOUND-LEMMA))
   (2 2 (:LINEAR ARRAY2P-LINEAR))
   (2 1
      (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
   (2 1
      (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
   (1 1 (:REWRITE NTH-WHEN-NOT-CDDR))
   (1 1 (:REWRITE NOT-ALL-<-WHEN-MEMBERP))
   (1 1
      (:REWRITE NOT-ALL-<-WHEN-MEMBER-EQUAL))
   (1 1
      (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
   (1 1
      (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP)))
(ADD-TO-PARENTS-OF-ATOMS
  (2940 19
        (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
  (2106 41 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
  (1494 12 (:REWRITE USE-ALL-<-FOR-CAR))
  (1016 12 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
  (957 19
       (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
  (903 12 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
  (869 18 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
  (829 208 (:REWRITE DEFAULT-CAR))
  (705 47 (:DEFINITION ASSOC-KEYWORD))
  (700 291 (:REWRITE DEFAULT-CDR))
  (557 42 (:REWRITE USE-ALL-CONSP-FOR-CAR))
  (381 37 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
  (267 267 (:REWRITE USE-ALL-CONSP-2))
  (267 267 (:REWRITE USE-ALL-CONSP))
  (235 224
       (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
  (224 224
       (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
  (222 9 (:REWRITE ALL-CONSP-OF-CDR))
  (194 27 (:REWRITE DEFAULT-<-1))
  (153 153 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
  (145 9 (:REWRITE ALL-<-OF-CDR))
  (142 4
       (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
  (126 41 (:REWRITE ALL-<-WHEN-NOT-CONSP))
  (114 10
       (:REWRITE ALL-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (98 42
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
  (97 22
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
  (94 47
      (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
  (90 90 (:TYPE-PRESCRIPTION ALL-<))
  (88 18 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
  (84 42
      (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
  (76 2 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
  (74 74 (:TYPE-PRESCRIPTION ALL-CONSP))
  (67 27 (:REWRITE DEFAULT-<-2))
  (66 2
      (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
  (63 5
      (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (61 8 (:REWRITE USE-ALL-NATP-FOR-CAR))
  (58 4 (:REWRITE ALL-NATP-OF-CDR))
  (56 56 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
  (51 37
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
  (48 48 (:TYPE-PRESCRIPTION ALL-NATP))
  (47 47
      (:REWRITE NOT-CDR-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
  (47 47
      (:REWRITE CONSP-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX-AND-QUOTEP))
  (47 47
      (:REWRITE CONSP-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
  (47 41
      (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
  (44 44
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
  (42 42
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (42 42
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
  (42 42
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (41 41 (:REWRITE ALL-<-TRANSITIVE-FREE))
  (41 41 (:REWRITE ALL-<-TRANSITIVE))
  (38 19
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
  (38 19
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
  (36 18
      (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
  (34 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-OF-CDR))
  (31 31
      (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (30 5 (:REWRITE <-OF-CAR-AND-ALEN1))
  (29 29 (:REWRITE USE-ALL-<=-2))
  (29 29 (:REWRITE USE-ALL-<=))
  (27 27 (:REWRITE USE-ALL-<-2))
  (27 27 (:REWRITE USE-ALL-<))
  (24 24
      (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (24 15
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
  (22 22
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
  (22 22
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
  (20 18
      (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
  (20 10
      (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
  (19 19 (:TYPE-PRESCRIPTION NO-ATOMS))
  (19 19 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
  (19 14 (:REWRITE DEFAULT-+-2))
  (18 18
      (:REWRITE PSEUDO-DAG-ARRAYP-MONOTONE))
  (16 8
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
  (16 8
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
  (16 4
      (:REWRITE NOT-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP))
  (15 15
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
  (14 14 (:REWRITE DEFAULT-+-1))
  (12 12
      (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
  (12 12
      (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
  (8 8 (:TYPE-PRESCRIPTION ALL-INTEGERP))
  (8 8 (:REWRITE USE-ALL-NATP-2))
  (8 8 (:REWRITE USE-ALL-NATP))
  (8 8 (:LINEAR ARRAY2P-LINEAR))
  (8 4
     (:REWRITE INTEGERP-OF-CAR-OF-CAR-WHEN-WEAK-DAGP-AUX-CHEAP))
  (4 4 (:TYPE-PRESCRIPTION WEAK-DAGP-AUX))
  (4 4 (:TYPE-PRESCRIPTION ALL-RATIONALP))
  (4 4 (:REWRITE USE-ALL-RATIONALP-2))
  (4 4 (:REWRITE USE-ALL-RATIONALP))
  (4 4
     (:REWRITE TRUE-LISTP-WHEN-PSEUDO-DAGP-AUX))
  (4 4
     (:REWRITE NATP-OF-CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
  (4 4
     (:REWRITE INTEGERP-OF-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (4 4
     (:REWRITE <-OF-0-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (4 2
     (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
  (2 2
     (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP)))
(ARRAY1P-OF-ADD-TO-PARENTS-OF-ATOMS
  (800 16
       (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
  (482 71 (:REWRITE DEFAULT-CAR))
  (407 20 (:REWRITE USE-ALL-CONSP-FOR-CAR))
  (357 21 (:DEFINITION ASSOC-KEYWORD))
  (297 24 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
  (251 113 (:REWRITE DEFAULT-CDR))
  (182 8 (:REWRITE ALL-CONSP-OF-CDR))
  (179 12 (:REWRITE DEFAULT-<-1))
  (128 3
       (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
  (120 60
       (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
  (117 117 (:REWRITE USE-ALL-CONSP-2))
  (117 117 (:REWRITE USE-ALL-CONSP))
  (114 15
       (:REWRITE ALL-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (110 23 (:REWRITE ALL-<-WHEN-NOT-CONSP))
  (98 2 (:REWRITE USE-ALL-<-FOR-CAR))
  (80 2 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
  (75 33
      (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
  (70 2
      (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
  (60 60
      (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
  (60 60
      (:TYPE-PRESCRIPTION DAG-PARENT-ARRAYP))
  (52 20
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
  (52 5 (:REWRITE ALL-<-OF-CDR))
  (48 48 (:TYPE-PRESCRIPTION ALL-CONSP))
  (46 46
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
  (41 23
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
  (40 20
      (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
  (34 24
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
  (34 17
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
  (32 32 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
  (32 16
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
  (27 14
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
  (26 26 (:LINEAR ARRAY2P-LINEAR))
  (24 24 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
  (24 24 (:REWRITE ALL-<-TRANSITIVE-FREE))
  (24 24 (:REWRITE ALL-<-TRANSITIVE))
  (24 3
      (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (24 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-OF-CDR))
  (23 23
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
  (23 23
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
  (21 21
      (:REWRITE NOT-CDR-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
  (21 21
      (:REWRITE CONSP-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX-AND-QUOTEP))
  (21 21
      (:REWRITE CONSP-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
  (21 21
      (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP))
  (20 20
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (20 20
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
  (20 20
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (18 3 (:REWRITE <-OF-CAR-AND-ALEN1))
  (17 17 (:TYPE-PRESCRIPTION NO-ATOMS))
  (16 16 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
  (15 12 (:REWRITE DEFAULT-<-2))
  (14 14
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
  (12 12 (:REWRITE USE-ALL-<=-2))
  (12 12 (:REWRITE USE-ALL-<=))
  (12 12 (:REWRITE USE-ALL-<-2))
  (12 12 (:REWRITE USE-ALL-<))
  (9 9
     (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
  (9 9
     (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
  (7 7
     (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (6 6
     (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP))
  (6 3 (:REWRITE DEFAULT-+-2))
  (4 4 (:TYPE-PRESCRIPTION ALL-RATIONALP))
  (4 4
     (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (4 2
     (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
  (3 3 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
  (3 3 (:REWRITE USE-ALL-RATIONALP-2))
  (3 3 (:REWRITE USE-ALL-RATIONALP))
  (3 3 (:REWRITE PSEUDO-DAG-ARRAYP-MONOTONE))
  (3 3 (:REWRITE DEFAULT-+-1))
  (3 3 (:REWRITE AREF1-OF-ASET1-DIFF))
  (3 3 (:REWRITE AREF1-OF-ASET1-BOTH))
  (3 1 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
  (2 2 (:TYPE-PRESCRIPTION NAT-LISTP))
  (2 2
     (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
  (2 1
     (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
  (1 1 (:DEFINITION NAT-LISTP)))
(DEFAULT-OF-ADD-TO-PARENTS-OF-ATOMS
  (800 16
       (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
  (485 74 (:REWRITE DEFAULT-CAR))
  (407 20 (:REWRITE USE-ALL-CONSP-FOR-CAR))
  (357 21 (:DEFINITION ASSOC-KEYWORD))
  (297 24 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
  (251 113 (:REWRITE DEFAULT-CDR))
  (182 8 (:REWRITE ALL-CONSP-OF-CDR))
  (179 12 (:REWRITE DEFAULT-<-1))
  (128 3
       (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
  (120 60
       (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
  (117 117 (:REWRITE USE-ALL-CONSP-2))
  (117 117 (:REWRITE USE-ALL-CONSP))
  (114 15
       (:REWRITE ALL-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (110 23 (:REWRITE ALL-<-WHEN-NOT-CONSP))
  (98 2 (:REWRITE USE-ALL-<-FOR-CAR))
  (80 2 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
  (77 34
      (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
  (70 2
      (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
  (60 60
      (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
  (52 20
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
  (52 5 (:REWRITE ALL-<-OF-CDR))
  (48 48 (:TYPE-PRESCRIPTION ALL-CONSP))
  (46 46
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
  (46 23
      (:REWRITE DEFAULT-WHEN-DAG-PARENT-ARRAYP))
  (41 23
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
  (40 20
      (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
  (34 24
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
  (34 17
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
  (32 32 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
  (32 16
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
  (27 14
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
  (24 24 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
  (24 24 (:REWRITE ALL-<-TRANSITIVE-FREE))
  (24 24 (:REWRITE ALL-<-TRANSITIVE))
  (24 3
      (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (24 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-OF-CDR))
  (23 23
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
  (23 23
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
  (22 22 (:LINEAR ARRAY2P-LINEAR))
  (21 21
      (:REWRITE NOT-CDR-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
  (21 21
      (:REWRITE CONSP-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX-AND-QUOTEP))
  (21 21
      (:REWRITE CONSP-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
  (20 20
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (20 20
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
  (20 20
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (18 3 (:REWRITE <-OF-CAR-AND-ALEN1))
  (17 17 (:TYPE-PRESCRIPTION NO-ATOMS))
  (16 16 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
  (15 12 (:REWRITE DEFAULT-<-2))
  (14 14
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
  (12 12 (:REWRITE USE-ALL-<=-2))
  (12 12 (:REWRITE USE-ALL-<=))
  (12 12 (:REWRITE USE-ALL-<-2))
  (12 12 (:REWRITE USE-ALL-<))
  (10 10
      (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP))
  (9 9
     (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
  (9 9
     (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
  (7 7
     (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (6 6
     (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP))
  (6 3 (:REWRITE DEFAULT-+-2))
  (4 4 (:TYPE-PRESCRIPTION ALL-RATIONALP))
  (4 4
     (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (4 2
     (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
  (3 3 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
  (3 3 (:TYPE-PRESCRIPTION NAT-LISTP))
  (3 3 (:TYPE-PRESCRIPTION ASET1))
  (3 3 (:REWRITE USE-ALL-RATIONALP-2))
  (3 3 (:REWRITE USE-ALL-RATIONALP))
  (3 3 (:REWRITE PSEUDO-DAG-ARRAYP-MONOTONE))
  (3 3 (:REWRITE DEFAULT-+-1))
  (3 3 (:REWRITE AREF1-OF-ASET1-DIFF))
  (3 3 (:REWRITE AREF1-OF-ASET1-BOTH))
  (3 1 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
  (2 2
     (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
  (2 1
     (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
  (2 1
     (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
  (1 1 (:DEFINITION NAT-LISTP)))
(ALEN1-OF-ADD-TO-PARENTS-OF-ATOMS
     (482 482 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
     (30 6 (:REWRITE USE-ALL-CONSP-FOR-CAR))
     (18 6
         (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
     (16 4 (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
     (15 15 (:REWRITE USE-ALL-CONSP-2))
     (15 15 (:REWRITE USE-ALL-CONSP))
     (14 7
         (:REWRITE ALL-DARGP-WHEN-ALL-MYQUOTEP-CHEAP))
     (12 12 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
     (12 12 (:TYPE-PRESCRIPTION ALL-CONSP))
     (12 6
         (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
     (10 10 (:REWRITE DEFAULT-CAR))
     (8 4 (:REWRITE ALL-DARGP-WHEN-NOT-CONSP))
     (7 7 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
     (7 7
        (:REWRITE ALL-DARGP-WHEN-ALL-DARGP-LESS-THAN))
     (6 6
        (:TYPE-PRESCRIPTION SYMBOL-TERM-ALISTP))
     (6 6
        (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
     (6 6 (:REWRITE DEFAULT-CDR))
     (6 6
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
     (6 6
        (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
     (6 6
        (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
     (6 6 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP)))
(ALL-DAG-PARENT-ENTRIESP-OF-ADD-TO-PARENTS-OF-ATOMS
  (3636 36
        (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
  (1860 18
        (:REWRITE <-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
  (1260 355 (:REWRITE DEFAULT-CAR))
  (1242 19 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
  (1181 34 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
  (1088 64 (:DEFINITION ASSOC-KEYWORD))
  (1040 461 (:REWRITE DEFAULT-CDR))
  (833 55 (:REWRITE USE-ALL-CONSP-FOR-CAR))
  (595 49 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
  (430 215
       (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
  (387 387 (:REWRITE USE-ALL-CONSP-2))
  (387 387 (:REWRITE USE-ALL-CONSP))
  (364 16 (:REWRITE ALL-CONSP-OF-CDR))
  (298 78 (:REWRITE DEFAULT-<-2))
  (280 9
       (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
  (256 78 (:REWRITE DEFAULT-<-1))
  (236 71 (:REWRITE ALL-<-WHEN-NOT-CONSP))
  (219 219
       (:TYPE-PRESCRIPTION DAG-PARENT-ARRAYP))
  (215 215
       (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
  (213 9 (:REWRITE ALL-<-OF-CDR))
  (199 20
       (:REWRITE ALL-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (188 85
       (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
  (175 7 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
  (143 7
       (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
  (139 7 (:REWRITE ALL-NATP-OF-CDR))
  (127 28
       (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
  (123 14 (:REWRITE USE-ALL-NATP-FOR-CAR))
  (122 5
       (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
  (121 55
       (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
  (120 5 (:REWRITE NOT-<-OF-CAR-WHEN-ALL-<-2))
  (117 32
       (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
  (110 55
       (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
  (109 31 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
  (104 52
       (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
  (101 101 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
  (100 5 (:REWRITE USE-ALL-<=-FOR-CAR))
  (98 98 (:TYPE-PRESCRIPTION ALL-CONSP))
  (90 14 (:REWRITE ALL-DARGP-LESS-THAN-OF-0))
  (78 78 (:REWRITE USE-ALL-<=-2))
  (78 78 (:REWRITE USE-ALL-<=))
  (78 78 (:REWRITE USE-ALL-<-2))
  (78 78 (:REWRITE USE-ALL-<))
  (73 5 (:REWRITE ALL-<=-WHEN-NOT-CONSP))
  (72 72 (:REWRITE ALL-<-TRANSITIVE-FREE))
  (72 72 (:REWRITE ALL-<-TRANSITIVE))
  (72 36
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
  (69 49
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
  (68 34
      (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
  (66 66 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
  (66 66 (:TYPE-PRESCRIPTION NO-ATOMS))
  (66 18 (:REWRITE USE-ALL-<-FOR-CAR))
  (64 64
      (:REWRITE NOT-CDR-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
  (64 64
      (:REWRITE CONSP-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX-AND-QUOTEP))
  (64 64
      (:REWRITE CONSP-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
  (60 60
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP))
  (60 60 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
  (56 56
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
  (55 55
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (55 55
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
  (55 55
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (46 4
      (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (44 11
      (:REWRITE NOT-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP))
  (38 38
      (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
  (36 36 (:LINEAR ARRAY2P-LINEAR))
  (36 32
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
  (34 34
      (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (34 30 (:REWRITE DEFAULT-+-2))
  (34 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-OF-CDR))
  (30 30
      (:REWRITE PSEUDO-DAG-ARRAYP-MONOTONE))
  (30 30 (:REWRITE DEFAULT-+-1))
  (28 28
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
  (28 28
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
  (28 14
      (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
  (28 14
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
  (28 14
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
  (28 14
      (:REWRITE ALL-MYQUOTEP-WHEN-NO-ATOMS-AND-ALL-DARGP-CHEAP))
  (26 26
      (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
  (24 24
      (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (24 4 (:REWRITE <-OF-CAR-AND-ALEN1))
  (20 10 (:REWRITE ALL-MYQUOTEP-OF-CDR-CHEAP))
  (18 18
      (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
  (14 14 (:TYPE-PRESCRIPTION ALL-RATIONALP))
  (14 14 (:TYPE-PRESCRIPTION ALL-INTEGERP))
  (14 14 (:REWRITE USE-ALL-NATP-2))
  (14 14 (:REWRITE USE-ALL-NATP))
  (14 14
      (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
  (14 14
      (:REWRITE ALL-MYQUOTEP-WHEN-NOT-CONSP))
  (13 13
      (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP))
  (11 7
      (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
  (10 10 (:TYPE-PRESCRIPTION ALL-<=))
  (9 9 (:REWRITE USE-ALL-RATIONALP-2))
  (9 9 (:REWRITE USE-ALL-RATIONALP))
  (8 4
     (:REWRITE INTEGERP-OF-CAR-OF-CAR-WHEN-WEAK-DAGP-AUX-CHEAP))
  (7 5
     (:REWRITE ALL-<=-WHEN-NOT-CONSP-CHEAP))
  (6 6 (:REWRITE AREF1-OF-ASET1-DIFF))
  (6 6 (:REWRITE AREF1-OF-ASET1-BOTH))
  (5 5 (:REWRITE ALL-<=-MONOTONE))
  (4 4 (:TYPE-PRESCRIPTION WEAK-DAGP-AUX))
  (4 4
     (:REWRITE NATP-OF-CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
  (4 4
     (:REWRITE INTEGERP-OF-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (4 4
     (:REWRITE <-OF-0-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (3 1
     (:REWRITE NAT-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
  (3 1
     (:REWRITE ALL->-OF-AREF1-WHEN-DAG-PARENT-ARRAYP)))
(DAG-PARENT-ARRAYP-OF-ADD-TO-PARENTS-OF-ATOMS
   (127 2
        (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
   (100 3 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
   (74 2 (:DEFINITION NAT-LISTP))
   (58 2
       (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
   (52 2
       (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
   (49 1 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
   (40 1 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
   (40 1 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
   (25 3
       (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
   (23 1 (:DEFINITION ADD-TO-PARENTS-OF-ATOMS))
   (22 4 (:DEFINITION MEMBER-EQUAL))
   (17 5 (:REWRITE USE-ALL-CONSP-FOR-CAR))
   (15 15 (:REWRITE USE-ALL-CONSP-2))
   (15 15 (:REWRITE USE-ALL-CONSP))
   (12 12 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
   (11 11 (:TYPE-PRESCRIPTION NAT-LISTP))
   (11 5
       (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
   (11 1
       (:REWRITE ALL-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
   (10 5
       (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
   (7 7 (:REWRITE DEFAULT-CAR))
   (7 3 (:REWRITE ALL-<-WHEN-NOT-CONSP))
   (6 6 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
   (6 6 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
   (6 6 (:TYPE-PRESCRIPTION ALL-CONSP))
   (6 6 (:TYPE-PRESCRIPTION ALL-<))
   (6 6 (:REWRITE DEFAULT-CDR))
   (6 2
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
   (6 1
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
   (5 5
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
   (5 5
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
   (5 5
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
   (5 3 (:REWRITE DEFAULT-+-2))
   (4 4
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP))
   (4 3 (:REWRITE DEFAULT-<-2))
   (4 3 (:REWRITE DEFAULT-<-1))
   (4 2
      (:REWRITE DEFAULT-WHEN-DAG-PARENT-ARRAYP))
   (4 2
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
   (4 2
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
   (4 1
      (:REWRITE NOT-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP))
   (3 3 (:REWRITE USE-ALL-<=-2))
   (3 3 (:REWRITE USE-ALL-<=))
   (3 3 (:REWRITE USE-ALL-<-2))
   (3 3 (:REWRITE USE-ALL-<))
   (3 3 (:REWRITE DEFAULT-+-1))
   (3 3
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
   (3 3 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
   (3 3 (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
   (3 3 (:REWRITE ALL-<-TRANSITIVE-FREE))
   (3 3 (:REWRITE ALL-<-TRANSITIVE))
   (3 1 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
   (2 2
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
   (2 2 (:TYPE-PRESCRIPTION NO-ATOMS))
   (2 2 (:TYPE-PRESCRIPTION ALL-NATP))
   (2 2 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
   (2 2 (:REWRITE PSEUDO-DAG-ARRAYP-MONOTONE))
   (2 2
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
   (2 2 (:LINEAR ARRAY2P-LINEAR))
   (2 1 (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
   (2 1
      (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
   (1 1
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
   (1 1
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
   (1 1
      (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
   (1 1
      (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
   (1 1
      (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP))
   (1 1
      (:REWRITE ALL-NATP-WHEN-NOT-CONSP-CHEAP))
   (1 1
      (:REWRITE ALL-DAG-PARENT-ENTRIESP-MONOTONE-1)))
(ALL-<-OF-AREF1-OF-ADD-TO-PARENTS-OF-ATOMS
  (1199 64 (:REWRITE ALL-<-WHEN-NOT-CONSP))
  (837 173 (:REWRITE DEFAULT-CAR))
  (804 20
       (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
  (750 44 (:REWRITE USE-ALL-CONSP-FOR-CAR))
  (612 36 (:DEFINITION ASSOC-KEYWORD))
  (500 56 (:REWRITE ALL-CONSP-WHEN-NOT-CONSP))
  (486 254 (:REWRITE DEFAULT-CDR))
  (390 6 (:REWRITE USE-ALL-<-FOR-CAR))
  (380 133
       (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
  (342 22 (:REWRITE ALL-CONSP-OF-CDR))
  (295 19
       (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
  (283 62 (:REWRITE DEFAULT-<-2))
  (269 269
       (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
  (263 13 (:DEFINITION NAT-LISTP))
  (242 242 (:REWRITE USE-ALL-CONSP-2))
  (242 242 (:REWRITE USE-ALL-CONSP))
  (229 62 (:REWRITE DEFAULT-<-1))
  (225 15 (:REWRITE ALL-NATP-WHEN-NAT-LISTP))
  (202 8 (:REWRITE USE-ALL-NATP-FOR-CAR))
  (160 4 (:REWRITE USE-ALL-RATIONALP-FOR-CAR))
  (148 9 (:REWRITE ALL-<-OF-CDR))
  (140 140 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
  (140 4
       (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP))
  (138 19
       (:REWRITE ALL-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (136 8 (:REWRITE ALL-<-OF-0-WHEN-ALL-NATP))
  (116 29
       (:REWRITE ALL-<-OF-AREF1-WHEN-BOUNDED-DAG-PARENT-ARRAYP))
  (112 112 (:TYPE-PRESCRIPTION ALL-CONSP))
  (112 44
       (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP))
  (94 94 (:TYPE-PRESCRIPTION NAT-LISTP))
  (94 62 (:REWRITE USE-ALL-<))
  (92 8 (:REWRITE ALL-<-OF-0-WHEN-NAT-LISTP))
  (90 2 (:REWRITE NOT-<-OF-CAR-WHEN-ALL-<-2))
  (88 44
      (:REWRITE CONSP-OF-CAR-WHEN-SYMBOL-TERM-ALISTP-CHEAP))
  (82 2 (:REWRITE USE-ALL-<=-FOR-CAR))
  (77 69 (:REWRITE ALL-<-TRANSITIVE-FREE))
  (72 56
      (:REWRITE ALL-CONSP-WHEN-NOT-CONSP-CHEAP))
  (70 2 (:REWRITE ALL-<=-WHEN-NOT-CONSP))
  (69 69 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
  (68 68 (:TYPE-PRESCRIPTION PSEUDO-DAGP))
  (66 6 (:REWRITE ALL-NATP-OF-CDR))
  (62 62 (:REWRITE USE-ALL-<=-2))
  (62 62 (:REWRITE USE-ALL-<=))
  (62 62 (:REWRITE USE-ALL-<-2))
  (58 58
      (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
  (58 58
      (:TYPE-PRESCRIPTION BOUNDED-DAG-PARENT-ARRAYP))
  (53 29
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
  (44 44
      (:REWRITE TRUE-LISTP-OF-CAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (44 44
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX-2))
  (44 44
      (:REWRITE CONSP-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (42 21
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
  (40 20
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
  (36 36
      (:REWRITE NOT-CDR-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
  (36 36
      (:REWRITE CONSP-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX-AND-QUOTEP))
  (36 36
      (:REWRITE CONSP-OF-CDR-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP))
  (36 4
      (:REWRITE <-OF-CAR-AND-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
  (32 32 (:TYPE-PRESCRIPTION MEMBERP))
  (32 32 (:LINEAR ARRAY2P-LINEAR))
  (31 18
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
  (30 30
      (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
  (30 30
      (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
  (30 15
      (:REWRITE ALL-NATP-WHEN-NAT-LISTP-CHEAP))
  (29 29
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
  (29 29
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
  (29 29
      (:REWRITE BOUNDED-DAG-PARENT-ARRAYP-MONOTONE))
  (29 29
      (:REWRITE ALL-<-OF-AREF1-WHEN-BOUNDED-DAG-PARENT-ENTRIESP))
  (27 27 (:REWRITE NOT-ALL-<-WHEN-MEMBERP))
  (27 27
      (:REWRITE NOT-ALL-<-WHEN-MEMBER-EQUAL))
  (24 4 (:REWRITE <-OF-CAR-AND-ALEN1))
  (24 2
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-OF-CDR))
  (23 23 (:REWRITE USE-ALL-RATIONALP-2))
  (23 23 (:REWRITE USE-ALL-RATIONALP))
  (21 21 (:TYPE-PRESCRIPTION NO-ATOMS))
  (20 20 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
  (18 18
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
  (14 14
      (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP))
  (14 14 (:REWRITE AREF1-OF-ASET1-DIFF))
  (14 14 (:REWRITE ALL-NATP-WHEN-NOT-CONSP))
  (10 10
      (:REWRITE CAR-OF-CAR-WHEN-PSEUDO-DAGP-AUX))
  (10 6 (:REWRITE DEFAULT-+-2))
  (8 8 (:TYPE-PRESCRIPTION ALL-RATIONALP))
  (8 8 (:REWRITE USE-ALL-NATP-2))
  (8 8 (:REWRITE USE-ALL-NATP))
  (8 4
     (:REWRITE INTEGERP-OF-CAR-WHEN-NAT-LISTP-CHEAP))
  (8 4
     (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-NATP-CHEAP))
  (8 4
     (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-INTEGERP-CHEAP))
  (8 4
     (:REWRITE ALL-RATIONALP-WHEN-NOT-CONSP-CHEAP))
  (6 6
     (:REWRITE NOT-<-OF-CAR-WHEN-ALL-DARGP-LESS-THAN-2))
  (6 6 (:REWRITE DEFAULT-+-1))
  (4 4 (:TYPE-PRESCRIPTION ALL-INTEGERP))
  (4 4 (:TYPE-PRESCRIPTION ALL-<=))
  (4 4 (:REWRITE PSEUDO-DAG-ARRAYP-MONOTONE))
  (4 4
     (:REWRITE INTEGERP-OF-CAR-WHEN-ALL-DARGP-LESS-THAN))
  (4 4
     (:REWRITE EQUAL-OF-NON-NATP-AND-CAAR-WHEN-WHEN-BOUNDED-NATP-ALISTP))
  (4 2
     (:REWRITE ALL-<=-WHEN-NOT-CONSP-CHEAP))
  (2 2 (:REWRITE ALL-<=-MONOTONE)))
(BOUNDED-DAG-PARENT-ENTRIESP-OF-ADD-TO-PARENTS-OF-ATOMS
   (80 30
       (:REWRITE <-WHEN-ALL-DARGP-LESS-THAN-GEN))
   (80 10 (:DEFINITION MEMBER-EQUAL))
   (80 5
       (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-<))
   (67 9 (:REWRITE ALL-<-WHEN-NOT-CONSP))
   (55 5
       (:REWRITE NATP-WHEN-ALL-DARGP-LESS-THAN-GEN))
   (50 50 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
   (50 22
       (:TYPE-PRESCRIPTION TRUE-LISTP-OF-AREF1-WHEN-DAG-PARENT-ARRAYP))
   (43 19
       (:REWRITE AREF1-WHEN-TOO-LARGE-CHEAP))
   (39 30 (:REWRITE DEFAULT-<-1))
   (36 9
       (:REWRITE ALL-<-OF-AREF1-WHEN-BOUNDED-DAG-PARENT-ARRAYP))
   (35 30 (:REWRITE DEFAULT-<-2))
   (32 32 (:TYPE-PRESCRIPTION POSP-OF-ALEN1))
   (32 30 (:REWRITE USE-ALL-<))
   (32 15
       (:REWRITE BOUNDED-DAG-PARENT-ENTRIESP-MONOTONE-GEN))
   (30 30 (:REWRITE USE-ALL-<=-2))
   (30 30 (:REWRITE USE-ALL-<=))
   (30 30 (:REWRITE USE-ALL-<-2))
   (30 5
       (:REWRITE ALL-<-OF-ALEN1-WHEN-PSEUDO-DAG-ARRAYP-LIST))
   (24 24 (:REWRITE USE-ALL-CONSP-2))
   (24 24 (:REWRITE USE-ALL-CONSP))
   (22 22
       (:TYPE-PRESCRIPTION TYPE-OF-AREF1-WHEN-PSEUDO-DAG-ARRAYP-AUX))
   (21 9 (:REWRITE ALL-<-WHEN-NOT-CONSP-CHEAP))
   (19 19
       (:REWRITE NONNEG-WHEN-DARGP-LESS-THAN))
   (18 18
       (:TYPE-PRESCRIPTION BOUNDED-DAG-PARENT-ARRAYP))
   (18 18
       (:REWRITE INTEGERP-WHEN-DARGP-LESS-THAN))
   (15 15
       (:REWRITE BOUNDED-DAG-PARENT-ENTRIESP-MONOTONE))
   (15 5
       (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP))
   (12 12
       (:TYPE-PRESCRIPTION ADD-TO-PARENTS-OF-ATOMS))
   (12 12 (:LINEAR ARRAY2P-LINEAR))
   (10 10
       (:TYPE-PRESCRIPTION PSEUDO-DAG-ARRAYP-LIST))
   (10 10 (:REWRITE DEFAULT-CDR))
   (10 10 (:REWRITE DEFAULT-CAR))
   (10 5
       (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NO-ATOMS-CHEAP))
   (10 5
       (:REWRITE ALL-DARGP-LESS-THAN-WHEN-ALL-MYQUOTEP-CHEAP))
   (9 9
      (:REWRITE BOUNDED-DAG-PARENT-ARRAYP-MONOTONE))
   (9 9 (:REWRITE ALL-<-TRANSITIVE-FREE-2))
   (9 9 (:REWRITE ALL-<-TRANSITIVE-FREE))
   (9 9 (:REWRITE ALL-<-TRANSITIVE))
   (9 3
      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
   (9 1 (:DEFINITION NFIX))
   (5 5 (:TYPE-PRESCRIPTION NO-ATOMS))
   (5 5 (:TYPE-PRESCRIPTION ALL-MYQUOTEP))
   (5 5 (:REWRITE USE-ALL-NATP-2))
   (5 5 (:REWRITE USE-ALL-NATP))
   (5 5
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL-ALT))
   (5 5
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN-SPECIAL))
   (5 5
      (:REWRITE PSEUDO-DAG-ARRAYP-LIST-WHEN-ALL-DARGP-LESS-THAN))
   (5 5
      (:REWRITE ARRAY1P-WHEN-PSEUDO-DAG-ARRAYP))
   (5 5
      (:REWRITE ALL-DARGP-LESS-THAN-WHEN-NOT-CONSP-CHEAP))
   (5 5
      (:REWRITE ALL-DARGP-LESS-THAN-MONOTONE))
   (4 4 (:REWRITE NOT-ALL-<-WHEN-MEMBERP))
   (4 4
      (:REWRITE NOT-ALL-<-WHEN-MEMBER-EQUAL))
   (4 4 (:REWRITE DEFAULT-+-2))
   (4 4 (:REWRITE DEFAULT-+-1))
   (3 3 (:REWRITE USE-ALL-RATIONALP-2))
   (3 3 (:REWRITE USE-ALL-RATIONALP))
   (2 2 (:TYPE-PRESCRIPTION MEMBERP))
   (1 1 (:TYPE-PRESCRIPTION NFIX)))
