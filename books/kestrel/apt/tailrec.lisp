...
(define tailrec-event
  ...
  "<p>
   ...
   otherwise, only error output is shown
   (unless error output was disabled when the transformation was called).
   If @(':verbose') is @('t'),
   an empty line is printed before the exported function and theorem.
   </p>
   <p>
   If @(':show-only') is @('t'),
   an event to just show the @(tsee encapsulate) on screen is returned.
  ...
  (b* ((wrld (w state))
       ...
       (encapsulate+ (restore-output? verbose encapsulate))
...
(define tailrec-fn
  ...
  (b* ((encapsulate? (previous-transformation-expansion call (w state)))
       ...
       (event (tailrec-event
               ...
               make-non-executable do-verify-guards
               hints-alist verbose show-only app-conds
               call state)))
    (value event)))
