...
(define tailrec-process-new-to-old-name (new-to-old-name
                                         (old$ symbolp)
                                         (new-name$ symbolp)
                                         (names-to-avoid symbol-listp)
                                         ctx
  ...
  (b* ((wrld (w state))
       ((er &) (ensure-symbol$ new-to-old-name
                               "The :NEW-TO-OLD-NAME input" t nil))
       (name (cond ((eq new-to-old-name :auto)
                    (make-paired-name new-name$ old$ 1 wrld))
                   ((keywordp new-to-old-name)
                    (intern-in-package-of-symbol
                     (concatenate 'string
                                  (symbol-name new-name$)
                                  (symbol-name new-to-old-name)
                                  (symbol-name old$))
                     new-name$))
                   (t new-to-old-name)))
       (description (msg "The name ~x0 of the theorem ~
...
(define tailrec-process-wrapper-to-old-name
  (wrapper-to-old-name
  ...
  :returns (mv erp
               ...
               state)
  :mode :program
  :short "Process the @(':wrapper-to-old-name') input."
  (if (not wrapper$)
         ...
         (name (cond ((eq wrapper-to-old-name :auto)
                     ...
                     ((keywordp wrapper-to-old-name)
                      (intern-in-package-of-symbol
                       (concatenate 'string
                                    (symbol-name wrapper-name$)
                                    (symbol-name wrapper-to-old-name)
                                    (symbol-name old$))
                       wrapper-name$))
                     (t wrapper-to-old-name)))
         (description (msg "The name ~x0 of the theorem ~
                            that relates the old function ~x1 ~
                            to the wrapper function ~x2, ~
                            specified (perhaps by default) ~
...
(define tailrec-process-wrapper-to-old-enable
  (wrapper-to-old-enable
   ...
   state)
  :returns (mv erp (nothing null) state)
  :short "Process the @(':wrapper-to-old-enable') input."
  (if (not wrapper$)
    ...
    (b* (((er &) (ensure-value-is-boolean$
                  wrapper-to-old-enable
                  "The :WRAPPER-TO-OLD-ENABLE input" t nil)))
      (value nil))))
...
(define tailrec-process-inputs (old
  ...
  (b* ((wrld (w state))
       ...
       ((er (list new-to-old-name$ names-to-avoid))
        (tailrec-process-new-to-old-name new-to-old-name
                                         old$
                                         new-name$
                                         names-to-avoid
                                         ctx
       ...
       ((er (list wrapper-to-old-name$ names-to-avoid))
        (tailrec-process-wrapper-to-old-name wrapper-to-old-name
                                             ...
                                             state))
       ((er &)
        (tailrec-process-wrapper-to-old-enable wrapper-to-old-enable
                                               wrapper-to-old-enable-present
                                               wrapper
                                               ctx
                                               state))
