...
(defun h-fn (state)
  (declare (xargs :mode :program
                  :stobjs state))
  (help-with-fn (most-recent-failed-theorem state) ;throws an error if there isn't one, TODO: What if the theorem is in an encapsulate?  Better to look for the checkpoints?
                state))
