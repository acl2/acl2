(CHECK-LAMBDA-CALL (32 32 (:REWRITE DEFAULT-CDR))
                   (27 27 (:REWRITE DEFAULT-CAR))
                   (25 3 (:DEFINITION LENGTH))
                   (20 4 (:DEFINITION LEN))
                   (9 9 (:TYPE-PRESCRIPTION LEN))
                   (8 4 (:REWRITE DEFAULT-+-2))
                   (5 5 (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
                   (4 4
                      (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
                   (4 4 (:REWRITE DEFAULT-+-1))
                   (3 1 (:DEFINITION SYMBOL-LISTP))
                   (1 1 (:TYPE-PRESCRIPTION SYMBOL-LISTP))
                   (1 1 (:REWRITE DEFAULT-COERCE-2))
                   (1 1 (:REWRITE DEFAULT-COERCE-1)))
(BOOLEANP-OF-CHECK-LAMBDA-CALL.YES/NO (4 4 (:REWRITE DEFAULT-CDR))
                                      (4 4 (:REWRITE DEFAULT-CAR)))
(SYMBOL-LISTP-OF-CHECK-LAMBDA-CALL.FORMALS
     (31 31 (:REWRITE DEFAULT-CDR))
     (30 30 (:REWRITE DEFAULT-CAR))
     (14 7 (:REWRITE DEFAULT-+-2))
     (7 7 (:REWRITE DEFAULT-+-1))
     (3 3 (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (2 2
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (1 1 (:REWRITE DEFAULT-COERCE-2))
     (1 1 (:REWRITE DEFAULT-COERCE-1)))
(PSEUDO-TERMP-OF-CHECK-LAMBDA-CALL.BODY
     (57 57 (:REWRITE DEFAULT-CDR))
     (49 49 (:REWRITE DEFAULT-CAR))
     (26 13 (:REWRITE DEFAULT-+-2))
     (13 13 (:REWRITE DEFAULT-+-1))
     (9 3 (:DEFINITION SYMBOL-LISTP))
     (6 6 (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (3 3
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (2 2 (:REWRITE DEFAULT-COERCE-2))
     (2 2 (:REWRITE DEFAULT-COERCE-1)))
(PSEUDO-TERM-LISTP-OF-CHECK-LAMBDA-CALL.ARGS
     (25 25 (:REWRITE DEFAULT-CDR))
     (25 3 (:DEFINITION LENGTH))
     (20 4 (:DEFINITION LEN))
     (19 19 (:REWRITE DEFAULT-CAR))
     (9 9 (:TYPE-PRESCRIPTION LEN))
     (8 4 (:REWRITE DEFAULT-+-2))
     (6 1 (:DEFINITION PSEUDO-TERM-LISTP))
     (4 4 (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (4 4
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (4 4 (:REWRITE DEFAULT-+-1))
     (3 1 (:DEFINITION SYMBOL-LISTP))
     (1 1 (:TYPE-PRESCRIPTION SYMBOL-LISTP))
     (1 1 (:REWRITE DEFAULT-COERCE-2))
     (1 1 (:REWRITE DEFAULT-COERCE-1)))
(LEN-OF-CHECK-LAMBDA-CALLS.FORMALS-IS-ARGS
     (99 99 (:REWRITE DEFAULT-CDR))
     (83 83 (:REWRITE DEFAULT-CAR))
     (50 25 (:REWRITE DEFAULT-+-2))
     (25 25 (:REWRITE DEFAULT-+-1))
     (12 4 (:DEFINITION SYMBOL-LISTP))
     (7 7 (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (6 1 (:DEFINITION PSEUDO-TERM-LISTP))
     (5 5
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (4 4 (:REWRITE DEFAULT-COERCE-2))
     (4 4 (:REWRITE DEFAULT-COERCE-1)))
(LEN-OF-CHECK-LAMBDA-CALLS.ARGS-IS-FORMALS
     (99 99 (:REWRITE DEFAULT-CDR))
     (83 83 (:REWRITE DEFAULT-CAR))
     (50 25 (:REWRITE DEFAULT-+-2))
     (25 25 (:REWRITE DEFAULT-+-1))
     (12 4 (:DEFINITION SYMBOL-LISTP))
     (7 7 (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (6 1 (:DEFINITION PSEUDO-TERM-LISTP))
     (5 5
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (4 4 (:REWRITE DEFAULT-COERCE-2))
     (4 4 (:REWRITE DEFAULT-COERCE-1)))
(TRUE-LISTP-OF-CHECK-LAMBDA-CALL.FORMALS
     (121 121 (:REWRITE DEFAULT-CDR))
     (104 104 (:REWRITE DEFAULT-CAR))
     (78 39 (:REWRITE DEFAULT-+-2))
     (39 39 (:REWRITE DEFAULT-+-1))
     (21 7 (:DEFINITION SYMBOL-LISTP))
     (13 13
         (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (8 8
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (7 7 (:REWRITE DEFAULT-COERCE-2))
     (7 7 (:REWRITE DEFAULT-COERCE-1))
     (6 1 (:DEFINITION PSEUDO-TERM-LISTP)))
(TRUE-LISTP-OF-CHECK-LAMBDA-CALL.ARGS
     (25 3 (:DEFINITION LENGTH))
     (22 22 (:REWRITE DEFAULT-CDR))
     (20 4 (:DEFINITION LEN))
     (18 18 (:REWRITE DEFAULT-CAR))
     (9 9 (:TYPE-PRESCRIPTION LEN))
     (8 4 (:REWRITE DEFAULT-+-2))
     (4 4 (:REWRITE DEFAULT-+-1))
     (3 3 (:REWRITE TERMP-IMPLIES-PSEUDO-TERMP))
     (3 1 (:DEFINITION SYMBOL-LISTP))
     (1 1 (:TYPE-PRESCRIPTION SYMBOL-LISTP))
     (1 1
        (:TYPE-PRESCRIPTION PSEUDO-TERM-LISTP))
     (1 1
        (:REWRITE TERM-LISTP-IMPLIES-PSEUDO-TERM-LISTP))
     (1 1 (:REWRITE DEFAULT-COERCE-2))
     (1 1 (:REWRITE DEFAULT-COERCE-1)))
(ACL2-COUNT-OF-CHECK-LAMBDA-CALL.BODY (523 235 (:REWRITE DEFAULT-+-2))
                                      (333 235 (:REWRITE DEFAULT-+-1))
                                      (192 48 (:DEFINITION INTEGER-ABS))
                                      (192 24 (:DEFINITION LENGTH))
                                      (120 24 (:DEFINITION LEN))
                                      (75 55 (:REWRITE DEFAULT-<-2))
                                      (59 55 (:REWRITE DEFAULT-<-1))
                                      (48 48 (:REWRITE DEFAULT-UNARY-MINUS))
                                      (24 24 (:TYPE-PRESCRIPTION LEN))
                                      (24 24 (:REWRITE DEFAULT-REALPART))
                                      (24 24 (:REWRITE DEFAULT-NUMERATOR))
                                      (24 24 (:REWRITE DEFAULT-IMAGPART))
                                      (24 24 (:REWRITE DEFAULT-DENOMINATOR))
                                      (24 24 (:REWRITE DEFAULT-COERCE-2))
                                      (24 24 (:REWRITE DEFAULT-COERCE-1)))
(ACL2-COUNT-OF-CHECK-LAMBDA-CALL.ARGS (58 28 (:REWRITE DEFAULT-+-2))
                                      (39 28 (:REWRITE DEFAULT-+-1))
                                      (24 6 (:REWRITE COMMUTATIVITY-OF-+))
                                      (24 6 (:DEFINITION INTEGER-ABS))
                                      (24 3 (:DEFINITION LENGTH))
                                      (16 16 (:REWRITE DEFAULT-CDR))
                                      (15 15 (:REWRITE DEFAULT-CAR))
                                      (15 3 (:DEFINITION LEN))
                                      (9 7 (:REWRITE DEFAULT-<-2))
                                      (8 8 (:REWRITE FOLD-CONSTS-IN-+))
                                      (8 7 (:REWRITE DEFAULT-<-1))
                                      (6 6 (:REWRITE DEFAULT-UNARY-MINUS))
                                      (3 3 (:TYPE-PRESCRIPTION LEN))
                                      (3 3 (:REWRITE DEFAULT-REALPART))
                                      (3 3 (:REWRITE DEFAULT-NUMERATOR))
                                      (3 3 (:REWRITE DEFAULT-IMAGPART))
                                      (3 3 (:REWRITE DEFAULT-DENOMINATOR))
                                      (3 3 (:REWRITE DEFAULT-COERCE-2))
                                      (3 3 (:REWRITE DEFAULT-COERCE-1)))
