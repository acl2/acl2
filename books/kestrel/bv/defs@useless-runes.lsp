(BVASHR)
(BVDIV)
(BVMOD)
(SBVREM
    (727 9 (:REWRITE LOGEXT-WHEN-SIGNED-BYTE-P))
    (709 9 (:DEFINITION SIGNED-BYTE-P))
    (655 9 (:DEFINITION INTEGER-RANGE-P))
    (554 1 (:REWRITE DEFAULT-UNARY-/))
    (216 21 (:LINEAR GETBIT-BOUND-LINEAR))
    (207 9 (:REWRITE LOGEXT-WHEN-TOP-BIT-0))
    (129 78 (:REWRITE DEFAULT-+-2))
    (113 78 (:REWRITE DEFAULT-+-1))
    (84 84 (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
    (76 2
        (:DEFINITION NONNEGATIVE-INTEGER-QUOTIENT))
    (70 30 (:REWRITE DEFAULT-<-2))
    (64 32 (:REWRITE GETBIT-TOO-HIGH-CHEAP-2))
    (57 17 (:REWRITE DEFAULT-UNARY-MINUS))
    (42 14 (:REWRITE GETBIT-IDENTITY))
    (36 30 (:REWRITE DEFAULT-<-1))
    (32 32
        (:REWRITE GETBIT-WHEN-VAL-IS-NOT-AN-INTEGER))
    (32 32
        (:REWRITE GETBIT-WHEN-NOT-INTEGERP-ARG1))
    (32 32 (:REWRITE GETBIT-WHEN-N-IS-NEGATIVE))
    (32 32
        (:REWRITE GETBIT-WHEN-EQUAL-OF-CONSTANT-AND-BVCHOP-CONSTANT-VERSION))
    (30 10 (:REWRITE BVCHOP-IDENTITY))
    (18 2 (:DEFINITION NFIX))
    (15 5 (:DEFINITION NATP))
    (15 3 (:REWRITE DEFAULT-*-2))
    (15 3 (:REWRITE DEFAULT-*-1))
    (15 3 (:LINEAR <-OF-LOGEXT-SAME-LINEAR))
    (14 14 (:REWRITE GETBIT-IDENTITY-FREE))
    (10 10
        (:REWRITE BVCHOP-WITH-N-NOT-AN-INTEGER))
    (10 10 (:REWRITE BVCHOP-WITH-N-NEGATIVE))
    (10 10
        (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-POSP))
    (10 10
        (:REWRITE BVCHOP-WHEN-SIZE-IS-NOT-NATP))
    (10 10
        (:REWRITE BVCHOP-WHEN-NOT-NATP-ARG1-CHEAP))
    (10 10
        (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER-CHEAP))
    (10 10
        (:REWRITE BVCHOP-WHEN-I-IS-NOT-AN-INTEGER))
    (10 10 (:REWRITE BVCHOP-SUBST-CONSTANT))
    (10 2 (:REWRITE COMMUTATIVITY-OF-+))
    (10 2 (:LINEAR <=-OF-BVCHOP-SAME-LINEAR))
    (9 9 (:TYPE-PRESCRIPTION SIGNED-BYTE-P))
    (9 9 (:REWRITE LOGEXT-WHEN-SIZE-NOT-POSP))
    (9 9
       (:REWRITE LOGEXT-WHEN-I-IS-NOT-AN-INTEGER))
    (6 2 (:REWRITE BVCHOP-IMPOSSIBLE-VALUE))
    (5 5 (:TYPE-PRESCRIPTION NATP))
    (3 3
       (:REWRITE EQUAL-CONSTANT-WHEN-BVCHOP-EQUAL-CONSTANT-FALSE))
    (3 1 (:REWRITE DEFAULT-NUMERATOR))
    (3 1 (:REWRITE DEFAULT-DENOMINATOR))
    (2 2 (:DEFINITION IFIX)))
(SBVMODDOWN (4 4
               (:TYPE-PRESCRIPTION NONNEG-OF-MOD-TYPE-2))
            (4 4
               (:TYPE-PRESCRIPTION NONNEG-OF-MOD-TYPE)))
(BVLT)
(BVLE)
(BVGT)
(BVGE)
(SBVLT)
(SBVGT)
(SBVLE)
(SBVGE)
(BITXNOR)
(BVIF)
(LG)
(TRIM)
(BOOL-FIX$INLINE)
(XXXJOIN2)
(BVINC)
(BVDEC)
(BIT-TO-BOOL)
(BOOL-TO-BIT)
