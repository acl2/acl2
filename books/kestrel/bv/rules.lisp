...
;move
(defthm mod-of-logand-by-2
  (implies (and (integerp i)
                (integerp j))
           (equal (mod (logand i j) 2)
                  (logand (mod i 2) (mod j 2))))
  :hints (("Goal" :use (:instance mod-of-logand-and-expt
                                  (n 1))
...
(defthm bitxor-of-slice-arg1
  ...
  :hints (("Goal" :in-theory (e/d (bitxor BVAND) (BVXOR-1-BECOMES-BITXOR)))))
...
(defthm bitor-x-not-x
  ...
  :hints (("Goal" :cases ((equal (bvchop 1 x) 1)
           ...
           :in-theory (e/d (bitor bvor
                                  ...
                                  bvxor
                                  bitnot
                                  ) (bvchop-1-becomes-getbit slice-becomes-getbit bvxor-1-becomes-bitxor)))))
...
(defthm getbit-1-of-bvnot-1
  (equal (getbit 1 (bvnot 1 x))
         0)
  :hints (("Goal" :in-theory (enable bvnot bitnot))))

(in-theory (disable bvminus))
...
(defthm bvand-of-expt
  (implies (and (equal k (expt 2 (+ -1 (integer-length k))))
...
(defthm equal-of-bitxor-same-alt
  ...
  :hints (("Goal" :use (:instance equal-of-bitxor-same)
           :in-theory (disable equal-of-bitxor-same))))
...
(defthm bvlt-of-bvmod-and-constant
  (implies (and (posp k)
           ...
           (equal (bvlt size (bvmod size x k) k)
                  t))
...
(defthmd <-of-logext-and-0-alt
  (equal (< (LOGEXT 32 x) 0)
         (sbvlt 32 x 0))
  :hints (("Goal" :in-theory (enable sbvlt-rewrite <-of-logext-and-0))))
  ...
  (implies (and (integerp x)
           ...
           (equal (logtail size (- x))
                  (if (equal 0 (bvchop size x))
                      (- (logtail size x))
                    (+ -1 (- (logtail size x))))))
  :hints (("Goal" :in-theory (e/d (logtail bvchop) (MOD-OF-EXPT-OF-2)))))
...
(defthm sbvlt-of-0-arg2-polarity
  ...
  :hints (("Goal" :use (:instance <-of-bvchop-and-bvchop-same (s1 size)
                                  (s2 (+ -1 size)))
           :in-theory (e/d (sbvmoddown bvlt sbvlt)
