...
public class TestFibDeep {
    ...
    // Make n calls of the Fibonacci function on each input,
    // printing the time taken by each call
    // as well as minimum, maximum, and average for each input.
    private static void runTests(Acl2Symbol fib, int[] inputs, int n)
        throws Acl2EvaluationException {
        int len = inputs.length;
        for (int i = 0; i < len; ++i) {
            int input = inputs[i];
            System.out.format("%nTimes (in seconds) to run Fibonacci on %d:%n",
                              input);
    ...
    // Make a specified number of calls of the Fibonacci function
