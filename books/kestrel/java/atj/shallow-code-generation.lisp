...
(define atj-gen-shallow-fnname ((fn symbolp)
  ...
  (b* ((pkg (symbol-package-name fn))
       (jclass? (if (or (equal pkg curr-pkg)
...
(define atj-gen-shallow-fnsynonym ((fn symbolp)
  ...
  (b* ((fn-type (atj-get-function-type fn guards$ wrld))
       ...
       (jmethod-name (atj-gen-shallow-fnname fn
                                             ...
                                             fn-method-names
                                             fn-pkg))
       (jmethod-param-names (atj-gen-shallow-fnsynonym-formals (arity fn wrld)))
       (jmethod-params (atj-gen-jparamlist jmethod-param-names
                                           (atj-types-to-jtypes in-types)))
...
(define atj-gen-shallow-fnsynonyms ((fns-synonym symbol-listp)
                                    (pkg-class-names string-string-alistp)
                                    (fn-method-names symbol-string-alistp)
                                    (guards$ booleanp)
                                    (curr-pkg stringp)
                                    (wrld plist-worldp))
  :guard (subsetp-equal fns-synonym (pkg-imports curr-pkg))
  :returns (jmethods jmethod-listp)
  :verify-guards nil
  :short "Lift @(tsee atj-gen-shallow-fnsynonym) to lists."
  (cond ((endp fns-synonym) nil)
        (t (cons (atj-gen-shallow-fnsynonym (car fns-synonym)
                                            pkg-class-names
                                            fn-method-names
                                            guards$
                                            curr-pkg
                                            wrld)
                 (atj-gen-shallow-fnsynonyms (cdr fns-synonym)
                                             pkg-class-names
                                             fn-method-names
                                             guards$
                                             curr-pkg
