...
; The tutorial pages are linked in a sequence with 'next' and 'previous' links;
; see :DOC ATJ-TUTORIAL for details.
; In order to automate the creation of such links, we introduce and use:
; (1) A macro DEF-ATJ-TUTORIAL-PAGE to record the contents of a page.
;     The relative order of its calls determines the order of the pages.
; (2) A macro CREATE-ATJ-TUTORIAL-TOPICS to generate, from the table,
;     the XDOC topics with suitable 'next' and 'previous' links.

; The macro DEF-ATJ-TUTORIAL-PAGE uses a table ATJ-TUTORIAL-PAGES
; to store, under the unique key PAGES, the page contents in reverse order
; (reverse because it CONSes each page contents to the existing ones;
; it is reversed back by CREATE-ATJ-TUTORIAL-TOPICS).
; The macro takes as arguments:
; (i) a symbol that, appended after ATJ-TUTORIAL-,
; forms the XDOC topic name for the page;
; (ii) a string that is the title of the tutorial page,
; from which the :SHORT is generated by CREATE-ATJ-TUTORIAL-TOPIC;
; (iii) zero or more forms that must evaluate to XDOC trees
; from which the :LONG is generated by CREATE-ATJ-TUTORIAL-TOPIC.
; These three components form the page content,
; which is stored in the table as (<name> <title> <form1> <form2> ...).

(defun def-atj-tutorial-page-fn (name title text wrld)
  (b* ((pages (cdr (assoc-eq 'pages (table-alist 'atj-tutorial-pages wrld))))
       (name (packn (list 'atj-tutorial- name)))
       (page (list* name title text))
       (new-pages (cons page pages)))
    `(table atj-tutorial-pages 'pages ',new-pages)))

(defmacro def-atj-tutorial-page (name title &rest text)
  `(make-event (def-atj-tutorial-page-fn ',name ',title ',text (w state))))

; The macro DEF-ATJ-TUTORIAL-TOPICS must be called
; after all the page contents are recorded with DEF-ATJ-TUTORIAL-PAGE.
; It retrieves the list of pages from the table,
; reverses it so that they are in the right order,
; and loops through each page, creating (and submitting) a DEFXOC for each.
...
; because those are available in the list being CDR'd through.

(defun def-atj-tutorial-topics-loop (pages previous-name previous-title)
  (b* (((when (endp pages)) nil)
       ...
       (topic `(defxdoc ,name
                 ...
                 (xdoc::topstring
                  ,@text
                  (xdoc::box) ; empty box, appears as a dividing line
                  ,@(and previous-link? (list previous-link?))
                  ,@(and next-link? (list next-link?)))))
       (topics (def-atj-tutorial-topics-loop (cdr pages) name title)))
    (cons topic topics)))

(defun def-atj-tutorial-topics-fn (wrld)
  (b* ((pages (cdr (assoc-eq 'pages (table-alist 'atj-tutorial-pages wrld))))
       (pages (reverse pages))
       (topics (def-atj-tutorial-topics-loop pages nil nil)))
    `(progn
       ,@topics)))

(defmacro def-atj-tutorial-topics ()
  `(make-event (def-atj-tutorial-topics-fn (w state))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

; Create a title for a section of a tutorial page.
; We use HTML's level-5 heading to keep it relatively small.

(defun atj-tutorial-section (section)
  (xdoc::h5 section))
...
(defxdoc+ atj-tutorial
  ...
  (xdoc::topstring
   ...
   (xdoc::p "<b>Start:</b> "
            ;; keep this consistent with the first DEF-ATJ-TUTORIAL-PAGE below:
            (xdoc::seetopic "atj-tutorial-motivation"
...
; Tutorial pages (except the top one, above),
; in the order desired for the 'next' and 'previous' links,
...
(def-atj-tutorial-page motivation
  ...
  ;; keep this tutorial page's name and title consistent with
  ;; the 'Start' link at the end of the ATJ-TUTORIAL topic above
...
; Generate XDOC topics for the (non-top) tutorial pages,
; with 'next' and 'previous' links according to the order above.
