...
(defxdoc+ atj-post-translation-remove-array-write-calls
  ...
  (xdoc::topstring
   ...
   (xdoc::p
    "For now we turn assignment expression statements of the form")
   (xdoc::codeblock
   ...
   (xdoc::p
    "These is not the only kind of occurrences of the array writing methods
...
(define atj-remove-array-write-calls-in-expr ((expr jexprp))
  :returns (new-expr jexprp)
  :short "Replace an assignment of a call of an array write method
  ...
  (xdoc::topstring
   (xdoc::p
    "This is called on the top-level expression of an expression statement,
     i.e. the expression that is terminated by a semicolon to for the statement.
...
(defines atj-remove-array-write-calls-in-jstatems+jblocks
  :short "Replace assignments calls of array write methods
          with the corresponding assignments to the array components."
  :long
  (xdoc::topstring
   (xdoc::p
    "We apply @(tsee atj-remove-array-write-calls-in-expr)
     to all the top-level expressions in expression statements."))

  (define atj-remove-array-write-calls-in-jstatem ((statem jstatemp))
    :returns (new-statem jstatemp)
    (jstatem-case
     statem
     :locvar (jstatem-fix statem)
     :expr (jstatem-expr (atj-remove-array-write-calls-in-expr statem.get))
     :return (jstatem-fix statem)
     :throw (jstatem-fix statem)
     :break (jstatem-fix statem)
     :continue (jstatem-fix statem)
     :if (jstatem-if statem.test
                     (atj-remove-array-write-calls-in-jblock statem.then))
     :ifelse (jstatem-ifelse
              statem.test
              (atj-remove-array-write-calls-in-jblock statem.then)
              (atj-remove-array-write-calls-in-jblock statem.else))
     :while (jstatem-while statem.test
                           (atj-remove-array-write-calls-in-jblock statem.body))
     :do (jstatem-do (atj-remove-array-write-calls-in-jblock statem.body)
                     statem.test)
  ...
  (define atj-remove-array-write-calls-in-jblock ((block jblockp))
    :returns (new-block jblockp)
    (cond ((endp block) nil)
          (t (cons (atj-remove-array-write-calls-in-jstatem (car block))
                   (atj-remove-array-write-calls-in-jblock (cdr block)))))
    :measure (jblock-count block))

  :verify-guards nil ; done below
  ///
  (verify-guards atj-remove-array-write-calls-in-jstatem)

  (fty::deffixequiv-mutual atj-remove-array-write-calls-in-jstatems+jblocks))
...
(define atj-post-translate ((name stringp)
  ...
  (b* ((body (atj-fold-returns body))
       ...
       (body (atj-remove-array-write-calls-in-jblock body)))
    body))
