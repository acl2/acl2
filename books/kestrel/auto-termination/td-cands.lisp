...
(((JUSTIFICATION (I MAXIMUM)
 ...
 ((JUSTIFICATION (VL2014::I VL2014::N)
  ...
  ((JUSTIFICATION (SV::N MAX)
   ...
   (SVL::SVEX-APPLY-CONSTANTS-COLLECT-ARG-META :SYSTEM
                                               . "centaur/svl/svex-reduce-with-env")
 ...
 ((JUSTIFICATION (L2 L1)
  ...
  ((JUSTIFICATION (Y X)
   ...
   (SV::SVTV-PIPELINE-EXTRACT-OVERRIDE-TRIPLES :SYSTEM
  ...
  ((JUSTIFICATION (RP::TERM)
   ...
   (((O-P (ACL2-COUNT RP::TERM)))
    ((NOT (CONSP RP::TERM))
     ...
     (NOT (CONSP (CDR (CDR (CDR RP::TERM)))))
     (NOT (EQUAL (CAR (CDR (CDR (CDR RP::TERM))))
                 ''NIL))
     (CDR (CDR (CDR (CDR RP::TERM))))
     (O< (ACL2-COUNT (CAR (CDR RP::TERM)))
         (ACL2-COUNT RP::TERM)))
    ((IF (CONSP RP::TERM)
         (IF (EQUAL (CAR RP::TERM)
                    'RP::DONT-RW-CONTEXT)
             (IF (CONSP (CDR RP::TERM))
                 (EQUAL (CDR (CDR RP::TERM)) 'NIL)
                 'NIL)
             'NIL)
         'NIL)
     (NOT (CONSP RP::TERM))
    ...
    ((IF (CONSP RP::TERM)
         ...
         'NIL)
     (IF (CONSP RP::TERM)
         (IF (EQUAL (CAR RP::TERM) 'IF)
             (IF (CONSP (CDR RP::TERM))
                 (IF (CONSP (CDR (CDR RP::TERM)))
                     (IF (CONSP (CDR (CDR (CDR RP::TERM))))
                         (IF (EQUAL (CAR (CDR (CDR (CDR RP::TERM))))
                                    ''NIL)
                             (EQUAL (CDR (CDR (CDR (CDR RP::TERM))))
                                    'NIL)
                             'NIL)
                         'NIL)
                     'NIL)
     ...
     (O< (ACL2-COUNT (CAR (CDR (CDR (CDR RP::TERM)))))
         (ACL2-COUNT RP::TERM)))
    ((NOT (CONSP RP::TERM))
     (NOT (EQUAL (CAR RP::TERM)
                 'RP::DONT-RW-CONTEXT))
     (NOT (CONSP (CDR RP::TERM)))
     (CDR (CDR RP::TERM))
     (O< (ACL2-COUNT (CAR (CDR RP::TERM)))
         (ACL2-COUNT RP::TERM))))
   (RP::RP-EXTRACT-CONTEXT 1 :SYSTEM
                           . "projects/rp-rewriter/rp-rewriter"))
  ((JUSTIFICATION (RP::KEYS)
                  (NIL O-P . O<)
                  ((ACL2-COUNT RP::KEYS)
                   IF MV-LIST RETURN-LAST))
 ...
 ((JUSTIFICATION (SV::CYCLE SV::FSM-EVAL)
                 (NIL O-P . O<)
                 ((NFIX (BINARY-+ (LEN SV::FSM-EVAL)
                                  (UNARY-- (NFIX SV::CYCLE))))
                  IF MV-LIST RETURN-LAST))
  ((JUSTIFICATION (SV::CYCLE EVAL)
                  (NIL O-P . O<)
                  ((NFIX (BINARY-+ (LEN EVAL)
                                   (UNARY-- (NFIX SV::CYCLE))))
                   IF MV-LIST RETURN-LAST))
   (((O-P (NFIX (BINARY-+ (LEN EVAL)
                          (UNARY-- (NFIX SV::CYCLE))))))
    ((NOT (INTEGERP (BINARY-+ (LEN EVAL)
                              (UNARY-- (LNFIX$INLINE SV::CYCLE)))))
     (NOT (< '0
             (BINARY-+ (LEN EVAL)
                       (UNARY-- (LNFIX$INLINE SV::CYCLE)))))
     (O< (NFIX (BINARY-+ (LEN EVAL)
                         (UNARY-- (NFIX (BINARY-+ '1
                                                  (LNFIX$INLINE SV::CYCLE))))))
         (NFIX (BINARY-+ (LEN EVAL)
                         (UNARY-- (NFIX SV::CYCLE)))))))
   (SV::SVAR-OVERRIDE-TRIPLELIST-FSM-INPUTS-OK*-OF-FSM-EVAL 4 :SYSTEM
                                                            . "centaur/sv/svtv/svtv-fsm-override"))
  ((JUSTIFICATION (SV::CYCLE SV::FSM-EVAL)
                  (NIL O-P . O<)
                  ((NFIX (BINARY-+ (LEN SV::FSM-EVAL)
                                   (UNARY-- (NFIX SV::CYCLE))))
                   IF MV-LIST RETURN-LAST))
   (((O-P (NFIX (BINARY-+ (LEN SV::FSM-EVAL)
                          (UNARY-- (NFIX SV::CYCLE))))))
    ((NOT (INTEGERP (BINARY-+ (LEN SV::FSM-EVAL)
                              (UNARY-- (NFIX SV::CYCLE)))))
     (NOT (< '0
             (BINARY-+ (LEN SV::FSM-EVAL)
                       (UNARY-- (NFIX SV::CYCLE)))))
     (O< (NFIX (BINARY-+ (LEN SV::FSM-EVAL)
                         (UNARY-- (NFIX (BINARY-+ '1
                                                  (LNFIX$INLINE SV::CYCLE))))))
         (NFIX (BINARY-+ (LEN SV::FSM-EVAL)
                         (UNARY-- (NFIX SV::CYCLE)))))))
 ...
 ((JUSTIFICATION (X)
  ...
  ((JUSTIFICATION (X)
   ...
   (SV::SVTV-DELAY-ALIST :SYSTEM . "centaur/sv/svtv/design-fsm")
