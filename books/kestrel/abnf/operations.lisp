...
; This file provides operations on ABNF grammars.
...
(defxdoc in-terminal-set
  ...
  "<p>
   ...
   then the language of that rule list
   consists of only terminals that belong to that set.
   This is proved below.
   </p>
   <p>
   For example, if all the terminal value notations are octets,
   the language consists of octets
...
(define symbol-in-termset-p ((symbol symbolp) (termset nat-setp))
  ...
  :long
  "<p>
   To prove that the language generated by a rule list
   consists of only terminals in a set,
...
(defrule language-in-termset-when-rules-in-termset
  ...
  :use (:instance leaves-in-termset-when-match-element-in-termset
                  (tree (string-parsablep-witness nats
                                                  (languagep-witness nats rules)
                                                  rules))
                  (element (element-rulename (languagep-witness nats rules)))))
