...
(defun def-inst-fn (mnemonic body check-condition check-condition-all-ones alt-body alt-body-hints guard-hints guard-debug)
  ...
  (b* ((encoding-pattern (lookup-eq-safe mnemonic *desugared-patterns*))
       ((when (not (encoding-patternp encoding-pattern)))
        (er hard? 'def-inst-fn "Bad pattern for ~x0: ~x1." mnemonic encoding-pattern))
       ...
       (check-condition (if (eq :auto check-condition)
                            (and (consp encoding-pattern)
                                 (equal '(cond 4) (car encoding-pattern)))
                          check-condition))
    ...
    (let* ((body (if check-condition-all-ones
                     `(if (= cond #b1111)
                          (execute-unconditional-instruction inst-address (read 4 inst-address arm) arm)
                        ,body)
                   body))
      ...
      `(encapsulate ()
         ...
         (defund ,fn (args inst-address arm)
           ...
           (let ,(let-bindings-for-encoding-fields encoding-pattern)
             ,body))
