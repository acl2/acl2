...
(mutual-recursion
 ...
 (defund subst-var-deep (var replacement term)
         ...
         (if (flambdap fn)
             ...
             (b* ((formals (lambda-formals fn))
               ...
               (if (or (not (member-eq var formals)) ; no need to go into the body
                   ...
                   (cons-with-hint fn (subst-var-deep-lst var replacement args) term)
                 ...
                 (make-lambda-application-simple non-trivial-formals
                                                 ...
                                                 (subst-var-deep-lst var replacement non-trivial-args)
                                                 (subst-var-deep var replacement body))))
           ;; Not a lambda application:
           (cons-with-hint fn (subst-var-deep-lst var replacement (fargs term)) term))))))

 (defund subst-var-deep-lst (var replacement terms)
   (declare (xargs :guard (and (symbolp var)
                               (pseudo-termp replacement)
   ...
   (if (endp terms)
       nil
     (cons-with-hint (subst-var-deep var replacement (first terms))
...
(defthm-flag-subst-var-deep
  (defthm pseudo-termp-of-subst-var-deep
    (implies (and (symbolp var)
                  (pseudo-termp replacement)
                  (pseudo-termp term))
             (pseudo-termp (subst-var-deep var replacement term)))
    :flag subst-var-deep)
  (defthm pseudo-term-listp-of-subst-var-deep-lst
    (implies (and (symbolp var)
                  (pseudo-termp replacement)
                  (pseudo-term-listp terms))
             (pseudo-term-listp (subst-var-deep-lst var replacement terms)))
    :flag subst-var-deep-lst)
  :hints (("Goal" :expand (pseudo-termp (cons (car term)
                                              (subst-var-deep-lst var replacement (cdr term))))
           :in-theory (enable subst-var-deep
                              subst-var-deep-lst
                              pseudo-termp))))

(verify-guards subst-var-deep)
