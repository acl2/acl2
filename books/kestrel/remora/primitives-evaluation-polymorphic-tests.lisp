...
(assert-equal (prim-head *tv-int* 2 nil *vec3*) (iv 1))

; A one-element vector has a head.
(assert-equal (prim-head *tv-int* 0 nil *vec1*) (iv 1))
...
(assert-equal (prim-reverse *tv-int* 1 nil *vec1*) *vec1*)
...
(assert-equal (prim-index *tv-int* 3 *vec3* (iv 0)) (iv 1))
(assert-equal (prim-index *tv-int* 3 *vec3* (iv 2)) (iv 3))
...
; Summing a vector.
(assert-equal (prim-sum (list 3) *vec3*) (iv 6))

; Summing a matrix adds all of its elements.
(assert-equal (prim-sum (list 2 3) *mat23*) (iv 21))

; Summing a scalar yields the scalar.
(assert-equal (prim-sum nil (iv 5)) (iv 5))
...
(assert-equal (prim-reshape *tv-int* (list 1) nil *vec1*) (iv 1))
(assert-equal
