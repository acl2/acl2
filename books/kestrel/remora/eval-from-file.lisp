...
(define eval-from-file ((filename stringp) state &key ((limit natp) '1000000))
  ...
  :short "Parse a Remora source file, type-check it, run it,
  ...
  (xdoc::topstring
   (xdoc::p
    "Parses the Remora source file @('filename')
     ...
     reduces it to the expression that running it evaluates
     (via @(tsee file-to-expr)),
   ...
   (xdoc::p
    "Running a file means running its @('main') entry point, in the scope
     of the declarations that precede it.  The file must therefore have an
     entry point named @('main'), and it must have no parameters; and the
     file must have no imports.  See @(tsee file-to-expr), whose errors
     these are.")
   (xdoc::p
