...
(defthm true-listp-of-make-standard-assumptions64-new
  (true-listp (make-standard-assumptions64-new stack-slots-needed existing-stack-slots state-var base-var target-offset position-independentp new-canonicalp)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; Creates assumptions about STATE-VAR and BASE-VAR.
;; Returns (mv erp assumptions).
(defund assumptions-for-memory-regions (regions base-var state-var stack-slots-needed existing-stack-slots position-independentp new-canonicalp acc)
  ...
  (if (endp regions)
    ...
    (b* ((region (first regions))
      ...
      (assumptions-for-memory-regions (rest regions)
                                      base-var state-var stack-slots-needed existing-stack-slots position-independentp new-canonicalp
...
(local
  (defthm true-list-of-mv-nth-1-of-assumptions-for-memory-regions
    (implies (true-listp acc)
             (true-listp (mv-nth 1 (assumptions-for-memory-regions regions base-var state-var stack-slots-needed existing-stack-slots position-independentp new-canonicalp acc))))
...
(defund assumptions-elf64-new (target
  ...
  (b* ((base-var 'base-address) ; arbitrary base address, only used if position-independentp
       ...
       (standard-assumptions (make-standard-assumptions64-new stack-slots-needed existing-stack-slots state-var base-var target-offset position-independentp new-canonicalp))
       ...
       ((mv erp memory-region-assumptions)
        (assumptions-for-memory-regions regions-to-load base-var state-var stack-slots-needed existing-stack-slots position-independentp new-canonicalp nil))
       ((when erp) (mv erp nil nil))
...
(defthm true-list-of-mv-nth-1-of-assumptions-elf64-new
  (true-listp (mv-nth 1 (assumptions-elf64-new target position-independentp stack-slots-needed existing-stack-slots state-var inputs type-assumptions-for-array-varsp inputs-disjoint-from new-canonicalp parsed-elf)))
...
(defthm true-list-of-mv-nth-1-of-assumptions-macho64-new
  (true-listp (mv-nth 1 (assumptions-macho64-new target position-independentp stack-slots-needed existing-stack-slots state-var inputs type-assumptions-for-array-varsp inputs-disjoint-from new-canonicalp parsed-macho)))
...
(defthm true-list-of-mv-nth-1-of-assumptions-pe64-new
  (true-listp (mv-nth 1 (assumptions-pe64-new target position-independentp stack-slots-needed existing-stack-slots state-var inputs type-assumptions-for-array-varsp inputs-disjoint-from new-canonicalp parsed-pe)))
...
;;         (assumptions-for-memory-chunk section-offset section-bytes relp state-var base-var stack-slots-needed existing-stack-slots bvp t ;new-canonicalp
;;                                       ))
