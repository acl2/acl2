...
(defund in-regionp (ad len start-ad)
  (declare (xargs :guard (and (unsigned-byte-p 48 ad)
                              (natp len) ; note that len >= 2^48 covers the whole region
                              (unsigned-byte-p 48 start-ad))))
  (and (natp len) ; len=0 falsifies the bvlt below
       (if (<= (expt 2 48) len) ; we handle (expt 2 48) here, since the len gets chopped to 48 bits below
...
(defthm in-regionp-same-end
  (equal (in-regionp (bvplus 48 ad (bvplus 48 -1 len)) len ad)
         (posp len))
  :hints (("Goal" :in-theory (enable in-regionp bvuminus bvplus ifix acl2::bvchop-of-sum-cases zp bvlt))))

(defthm in-regionp-same-end-alt
  (equal (in-regionp (bvplus 48 281474976710655 (bvplus 48 ad len)) len ad)
         (posp len))
  :hints (("Goal" :in-theory (enable in-regionp bvuminus bvplus ifix acl2::bvchop-of-sum-cases zp bvlt))))

;; note that the region size can't be 2^48
(defthm not-in-regionp-one-past-end
  (equal (in-regionp (bvplus 48 ad len) len ad)
         (and (<= (expt 2 48) len)
              (natp len)))
  :hints (("Goal" :in-theory (enable in-regionp bvuminus bvplus ifix acl2::bvchop-of-sum-cases zp bvlt))))
...
(defund disjoint-regionsp (len1 ad1 len2 ad2)
  (declare (xargs :guard (and (natp len1) ; note that len >= 2^48 covers the whole region
                              (unsigned-byte-p 48 ad1)
                              (natp len2) ; note that len >= 2^48 covers the whole region
                              (unsigned-byte-p 48 ad2))))
  (if (or (zp len1)
          (zp len2))
      t ; one region is empty
    (if (or (<= (expt 2 48) len1)
            (<= (expt 2 48) len2))
...
(encapsulate ()
  (local (defthmd disjoint-regionsp-spec-of-bvchop-arg2-fw
           (implies (disjoint-regionsp-spec len1 (bvchop 48 ad1) len2 ad2)
  ...
  (local (defthmd disjoint-regionsp-spec-of-bvchop-arg2-bk
           (implies (disjoint-regionsp-spec len1 ad1 len2 ad2)
                    (disjoint-regionsp-spec len1 (bvchop 48 ad1) len2 ad2))
           :hints (("Goal"  :expand (disjoint-regionsp-spec len1 (bvchop 48 ad1) len2 ad2)
                    :in-theory (enable ;disjoint-regionsp-spec
                                 )))))

  (defthm disjoint-regionsp-spec-of-bvchop-arg2
    (equal (disjoint-regionsp-spec len1 (bvchop 48 ad1) len2 ad2)
           (disjoint-regionsp-spec len1 ad1 len2 ad2))
...
(encapsulate ()
  (local (defthmd disjoint-regionsp-spec-of-bvchop-arg4-fw
           (implies (disjoint-regionsp-spec len1 ad1 len2 (bvchop 48 ad2))
                    (disjoint-regionsp-spec len1 ad1 len2 ad2))
           :hints (("Goal"  :expand (disjoint-regionsp-spec len1 ad1 len2 ad2)
                    :in-theory (enable ;disjoint-regionsp-spec
                                 )))))
  ...
  (local (defthmd disjoint-regionsp-spec-of-bvchop-arg4-bk
           (implies (disjoint-regionsp-spec len1 ad1 len2 ad2)
                    (disjoint-regionsp-spec len1 ad1 len2 (bvchop 48 ad2)))
           :hints (("Goal" :expand (disjoint-regionsp-spec len1 ad1 len2 (bvchop 48 ad2))
                    :in-theory (enable ;disjoint-regionsp-spec
                                 )))))

  (defthm disjoint-regionsp-spec-of-bvchop-arg4
    (equal (disjoint-regionsp-spec len1 ad1 len2 (bvchop 48 ad2))
           (disjoint-regionsp-spec len1 ad1 len2 ad2))
...
(defthm disjoint-regionsp-correct
  (equal (disjoint-regionsp-spec len1 ad1 len2 ad2)
         (disjoint-regionsp len1 ad1 len2 ad2))
  :hints (("Goal" :use ((:instance disjoint-regionsp-correct-forward (ad1 (bvchop 48 ad1)) (ad2 (bvchop 48 ad2)))
                        (:instance disjoint-regionsp-correct-backward (ad1 (bvchop 48 ad1)) (ad2 (bvchop 48 ad2))))
      ...
      (if (<= (expt 2 48) len2)
        ...
        (if (<= (expt 2 48) len1)
          ...
          (and (in-regionp ad1 len2 ad2)
               (bvle 48 len1 len2) ; ensures the difference below is not negative
...
(encapsulate ()
  (local
    (defthmd subregionp-spec-of-bvchop-arg2-fw
      (implies (subregionp-spec len1 (bvchop 48 ad1) len2 ad2)
  ...
  (local
    (defthmd subregionp-spec-of-bvchop-arg2-bk
      (implies (subregionp-spec len1 ad1 len2 ad2)
               (subregionp-spec len1 (bvchop 48 ad1) len2 ad2))
      :hints (("Goal" :expand (subregionp-spec len1 (bvchop 48 ad1) len2 ad2)))))

  (defthm subregionp-spec-of-bvchop-arg2
    (equal (subregionp-spec len1 (bvchop 48 ad1) len2 ad2)
           (subregionp-spec len1 ad1 len2 ad2))
...
(encapsulate ()
  (local
    (defthmd subregionp-spec-of-bvchop-arg4-fw
      (implies (subregionp-spec len1 ad1 len2 (bvchop 48 ad2))
               (subregionp-spec len1 ad1 len2 ad2))
      :hints (("Goal" :expand (subregionp-spec len1 ad1 len2 ad2)))))
  ...
  (local
    (defthmd subregionp-spec-of-bvchop-arg4-bk
      (implies (subregionp-spec len1 ad1 len2 ad2)
               (subregionp-spec len1 ad1 len2 (bvchop 48 ad2)))
      :hints (("Goal" :expand (subregionp-spec len1 ad1 len2 (bvchop 48 ad2))))))

  (defthm subregionp-spec-of-bvchop-arg4
    (equal (subregionp-spec len1 ad1 len2 (bvchop 48 ad2))
           (subregionp-spec len1 ad1 len2 ad2))
...
(defthmd subregionp-correct-back
  (implies (and (natp len1) ;(unsigned-byte-p 48 len1)
               ; (unsigned-byte-p 48 len2)
                (unsigned-byte-p 48 ad1)
                (unsigned-byte-p 48 ad2))
           (implies (subregionp-spec len1 ad1 len2 ad2)
...
(defthm subregionp-correct
  (equal (subregionp-spec len1 ad1 len2 ad2)
         (subregionp len1 ad1 len2 ad2))
  :hints (("Goal" :use (:instance subregionp-correct-back (ad1 (bvchop 48 ad1)) (ad2 (bvchop 48 ad2))))))
