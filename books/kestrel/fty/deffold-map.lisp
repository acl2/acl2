  ...
  (xdoc::topstring
   ...
   (xdoc::p
    "If any product or summand of the types in the clique
     ...
     cannot be verified as part of the @(tsee define) or @(tsee defines),
     for two reasons:
     the guard obligations from the boilerplate constructor calls
     need rules that are disabled by default
     (the @('<constructor>-requirements') theorems generated by FTY,
     and the theorems that accompany the list map functions);
     furthermore, if the involved list types are in the same clique,
     those accompanying theorems do not yet exist
     at the time when @(tsee defines) would verify the guards,
     because they are generated after the @('///') of the @(tsee defines).
