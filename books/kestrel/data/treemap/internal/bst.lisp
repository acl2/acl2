...
(define <<-all-l
  ...
  (or (tree-empty-p tree)
      (and (<< (tree-element->key (tree->head tree)) x)
...
(define <<-all-r
  ...
  (or (tree-empty-p tree)
      (and (<< x (tree-element->key (tree->head tree)))
...
(defrule <<-all-l-of-arg1-and-tree->head
  (equal (<<-all-l tree (tree-element->key (tree->head tree)))
         (tree-empty-p tree))
  :enable (<<-all-l
           tree->head
           data::<<-rules))
...
(defrule <<-all-r-of-tree->head
  (equal (<<-all-r (tree-element->key (tree->head tree)) tree)
         (tree-empty-p tree))
  :enable (<<-all-r
           tree->head
           data::<<-rules))
...
(defrule <<-of-tree->head-when-<<-all-l
  (implies (and (<<-all-l tree x)
                (not (tree-empty-p tree)))
           (<< (tree-element->key (tree->head tree)) x))
...
(defrule <<-of-arg1-and-tree->head-when-<<-all-r-arg1
  (implies (and (<<-all-r x tree)
                (not (tree-empty-p tree)))
           (<< x (tree-element->key (tree->head tree))))
  :rule-classes ((:rewrite :backchain-limit-lst (0 nil)))
  :enable <<-all-r)
  ...
  (if (mbt (not (tree-empty-p tree)))
      (mv-let (bstp min)
              (if (tree-empty-p (tree->left tree))
                  (mv t (tree-element->key (tree->head tree)))
                (mv-let (bstp$ min$ max$)
                        (fast-bstp-nonempty (tree->left tree))
                  (mv (and bstp$
                           (<< max$ (tree-element->key (tree->head tree))))
                      min$)))
        (if bstp
            (if (tree-empty-p (tree->right tree))
                (mv t min (tree-element->key (tree->head tree)))
              (mv-let (bstp$ min$ max$)
                      (fast-bstp-nonempty (tree->right tree))
                (mv (and bstp$
                         (<< (tree-element->key (tree->head tree)) min$))
                    min
                    max$)))
          (mv nil nil nil)))
    (mv t nil nil)))
...
(defrule <<-all-l-of-tree->left-and-tree->head-when-bstp
  (implies (bstp set)
           (<<-all-l (tree->left set)
                     (tree-element->key (tree->head set)))))
