...
(define heap<-with-tree-element
  ((x tree-element-p)
   (y tree-element-p))
  (heap<-with-hashes (tree-element->key x)
                     (tree-element->key y)
                     (tree-element->hash x)
                     (tree-element->hash y))
  :enabled t
...
(define heap<-all-l
  ...
  (or (tree-empty-p tree)
      (and (heap< (tree-element->key (tree->head tree)) x)
...
(defrule heap<-all-l-of-arg1-and-tree->head
  (equal (heap<-all-l tree (tree-element->key (tree->head tree)))
         (tree-empty-p tree))
  :enable (heap<-all-l
           heap<-rules))
...
(defrule heap<-all-l-of-tree-node
  (equal (heap<-all-l (tree-node head left right) x)
         (and (heap< (tree-element->key head) x)
              (heap<-all-l left x)
              (heap<-all-l right x)))
  :enable heap<-all-l)
...
(defruled heap<-all-l-weaken2
  ...
  :use (:instance treeset::heap<-trichotomy
                  (x x)
                  (y y)))
    ...
    (if (tree-empty-p (tree->left tree))
        (if (tree-empty-p (tree->right tree))
          ...
          (mv-let (heapp hash-right)
            ...
            (mv (and heapp
                     (heap<-with-hashes (tree-element->key
...
(defrulel fast-nonempty-heapp.hash-max
  (equal (mv-nth 1 (fast-nonempty-heapp tree))
         (hash (tree-element->key (tree->head tree))))
...
(defruled heap<-of-tree->head-and-tree->head-of-tree->left
  (implies (and (heapp tree)
           ...
           (heap< (tree-element->key (tree->head (tree->left tree)))
                  (tree-element->key (tree->head tree)))))

(defruled heap<-of-tree->head-and-tree->head-of-tree->right
  (implies (and (heapp tree)
                (not (tree-empty-p (tree->right tree))))
           (heap< (tree-element->key (tree->head (tree->right tree)))
                  (tree-element->key (tree->head tree)))))
