...
(defthm reduce-additive-constant-equal
  (implies (and (bind-free (find-constant-addend lhs rhs)
			   (c))
		(not (equal c 0))
...
(defthm reduce-multiplicative-constant-equal
  (implies (and (bind-free (find-constant-factor-equal lhs rhs)
			   (c))
		(not (equal c 0))
		(not (equal (abs c) 1))
		(syntaxp (simplify-ok-p `(EQUAL ,lhs ,rhs)
...
(defthm reduce-rational-multiplicative-constant-<
  (implies (and (bind-free (find-rational-constant-factor-< lhs rhs)
			   (c))
		(not (equal c 0))
		(not (equal (abs c) 1))
		(syntaxp (simplify-ok-p `(< ,lhs ,rhs)
...
(defthm reduce-multiplicative-constant-<
  (implies (and (bind-free (find-constant-factor-< lhs rhs)
			   (c))
		(not (equal c 0))
		(not (equal (abs c) 1))
		(syntaxp (simplify-ok-p `(< ,lhs ,rhs)
