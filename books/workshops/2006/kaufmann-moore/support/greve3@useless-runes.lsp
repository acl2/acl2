(PERM)
(UNIQUE)
(DISJOINT)
(FLAT)
(GOO)
(HOO)
(BOO)
(RED)
(PERM-IS-AN-EQUIVALENCE)
(PERM-IMPLIES-IFF-UNIQUE-1)
(PERM-IMPLIES-EQUAL-DISJOINT-2)
(PERM-IMPLIES-EQUAL-DISJOINT-1)
(PERM-IMPLIES-PERM-APPEND-1 (36 18
                                (:TYPE-PRESCRIPTION TRUE-LISTP-APPEND))
                            (18 18 (:TYPE-PRESCRIPTION TRUE-LISTP))
                            (18 18 (:TYPE-PRESCRIPTION BINARY-APPEND)))
(PERM-IMPLIES-PERM-APPEND-2 (36 18
                                (:TYPE-PRESCRIPTION TRUE-LISTP-APPEND))
                            (18 18 (:TYPE-PRESCRIPTION TRUE-LISTP))
                            (18 18 (:TYPE-PRESCRIPTION BINARY-APPEND)))
(PERM-IMPLIES-PERM-FLAT-1)
(FLAT-APPEND)
(UNIQUE-DISJOINT)
(GOO-PERM)
(RED-DISJOINT)
(UNIQUE-DISJOINT-TEST (18 6 (:DEFINITION BINARY-APPEND))
                      (6 6 (:REWRITE DEFAULT-CDR))
                      (6 6 (:REWRITE DEFAULT-CAR)))
(TIS-SO (12 4 (:DEFINITION BINARY-APPEND))
        (4 4 (:REWRITE DEFAULT-CDR))
        (4 4 (:REWRITE DEFAULT-CAR)))
(WHYNOT (12 4 (:DEFINITION BINARY-APPEND))
        (4 4 (:REWRITE DEFAULT-CDR))
        (4 4 (:REWRITE DEFAULT-CAR)))
(RED-DISJOINT-BAD)
