...
(defun product-recognizer (product)
  (or (kwassoc :recognizer nil (product-kwalist product))
      (appsyms (list (product-name product) 'p))))
...
(defun defsum-munge-components (components)
  (if (atom components)
      nil
...
(defun defsum-munge-product (product)
    (mv-let (product kwalist)
            ...
            (cons
             ...
             (cons (car product)
                   (defsum-munge-components (cdr product)))
             ;; keywords
             kwalist)))
...
(defun constructor-def (product guard-opt)
  ...
  `(defun ,constr ,args
     ,@(if guard-opt
         ...
         nil)
     (list (quote ,constr) ,@args))))

;; Definition for the recognizer of a product
(defun product-recognizer-def (product)
  `(defun ,(product-recognizer product) (x)
     (declare (xargs :guard t))
     (and (true-listp x)
          (= (length x) ,(1+ (len (product-components product))))
          (eq (car x) (quote ,(product-name product))))))

;; Definition of the nth accessor for a product
(defun accessor-def (product component n guard-opt accessor-opt)
...
(defun accessors-def (product components n guard-opt accessor-opt)
  (if (consp components)
      (cons (accessor-def product (car components) n guard-opt accessor-opt)
            (accessors-def product (cdr components) (1+ n) guard-opt accessor-opt))
    nil))

(defun product-function-defs (product guard-opt accessor-opt)
  (let* ((kwalist (product-kwalist product))
    ...
    (append (and (not (or recog predef))
                 (list (product-recognizer-def product)))
            (and (not predef)
                 (cons (constructor-def product guard-opt)
                       (accessors-def product
                                      (product-components product) 1
                                      guard-opt
                                      accessor-opt))))))
...
(defun product-compound-rec-thm (product)
  ...
  (let ((acc (accessor-name product component))
    ...
    `(defthm ,(appsyms (list acc 'acl2-count))
     (implies (,recognizer x)
              (< (acl2-count (,acc x))
                 (acl2-count x)))
     :hints (("Goal" :in-theory (disable nth acl2-count)))
...
(defun product-elim-thm (product)
...
(defun product-theorems (product accessor-opt)
  (let* ((kwalist (product-kwalist product))
         (predef (kwassoc :predef nil kwalist))
         (recog (kwassoc :recognizer nil kwalist))
         (components (product-components product)))
    `(,@(and (not predef) (not recog)
             (list (product-compound-rec-thm product)))
        ,@(and (not predef)
        ...
        ,(product-elim-thm product)
        ,@(product-type-thms product)
        ,@(product-arg-thms product components))))



(defun basic-product-defs (product guard-opt accessor-opt)
  (append (product-function-defs product guard-opt accessor-opt)
          (product-theorems product accessor-opt)
...
(defun basic-products-defs (products guard-opt accessor-opt)
  (if (atom products)
      nil
    (append (basic-product-defs (car products) guard-opt accessor-opt)
            (basic-products-defs (cdr products) guard-opt accessor-opt))))
...
(defun sum-compound-rec-thm (sum)
...
(defun sum-compound-rec-thms (sums)
  (if (consp sums)
      (cons (sum-compound-rec-thm (car sums))
            (sum-compound-rec-thms (cdr sums)))
    nil))
