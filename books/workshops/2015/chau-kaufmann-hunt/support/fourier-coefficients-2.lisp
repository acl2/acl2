...
;; Define Fourier coefficients r_i and s_i as encapsulated functions.
...
(defun fourier-sum-2 (x i c)
  (if (zp i)
    ...
    (+ (* (r i)
       ...
       (* (s i)
          (fixed-sin x i c))
       (fourier-sum-2 x (1- i) c))))

(defthm realp-fourier-sum-2
  (realp (fourier-sum-2 x i c))
  :rule-classes :type-prescription)

(defun fourier-sum-2-*-fixed-cos (x i k c)
  (* (fourier-sum-2 x i c)
     (fixed-cos x k c)))

(defthm realp-fourier-sum-2-*-fixed-cos
  (realp (fourier-sum-2-*-fixed-cos x i k c))
  :rule-classes :type-prescription)

(local
 (defthm fourier-sum-2-*-fixed-cos-rewrite
   (equal (fourier-sum-2-*-fixed-cos x i k c)
          (sum-fn-cos-i-prime x i k c))
   :hints (("Goal" :in-theory (enable fn-cos-i-prime
                                      cos-orthog-prime
                                      sin-cos-orthog-prime)))))

(in-theory (disable fourier-sum-2-*-fixed-cos))

(defun map-fourier-sum-2-*-fixed-cos (p i k c)
  (if (consp p)
      (cons (fourier-sum-2-*-fixed-cos (car p) i k c)
	    (map-fourier-sum-2-*-fixed-cos (cdr p) i k c))
    nil))

(local
 (defthm map-fourier-sum-2-*-fixed-cos-rewrite
   (equal (map-fourier-sum-2-*-fixed-cos p i k c)
          (map-sum-fn-cos-i-prime p i k c))))

(defun riemann-fourier-sum-2-*-fixed-cos (p i k c)
  (dotprod (deltas p)
	   (map-fourier-sum-2-*-fixed-cos (cdr p) i k c)))

(local
 (defthm riemann-fourier-sum-2-*-fixed-cos-rewrite
   (equal (riemann-fourier-sum-2-*-fixed-cos p i k c)
          (riemann-sum-fn-cos-i-prime p i k c))))

(in-theory (disable riemann-fourier-sum-2-*-fixed-cos))

(local (in-theory (disable riemann-sum-fn-cos-i-prime)))

(defun-std strict-fourier-sum-2-*-fixed-cos (a b i k c)
  (if (and (realp a)
           (realp b)
           (inside-interval-p a (fn-i-domain))
           (inside-interval-p b (fn-i-domain))
           (< a b))
      (standard-part (riemann-fourier-sum-2-*-fixed-cos
                      (make-small-partition a b)
                      i k c))
    0))

(defun int-fourier-sum-2-*-fixed-cos (a b i k c)
  (if (<= a b)
      (strict-fourier-sum-2-*-fixed-cos a b i k c)
    (- (strict-fourier-sum-2-*-fixed-cos b a i k c))))

(local
 (defthm-std int-fourier-sum-2-*-fixed-cos-rewrite
   (equal (int-fourier-sum-2-*-fixed-cos a b i k c)
          (int-sum-fn-cos-i-prime a b i k c))
   :hints (("Goal" :in-theory (enable int-sum-fn-cos-i-prime)))))

(in-theory (disable int-fourier-sum-2-*-fixed-cos))

(defthm int-fourier-sum-2-*-fixed-cos-thm-1
  (implies (and (natp i)
                (natp k)
                (< i k)
                (realp L)
                (not (equal L 0)))
           (equal (* (/ L)
                     (int-fourier-sum-2-*-fixed-cos (- L) L
                                                  i k (/ (acl2-pi) L)))
                  0))
  :hints (("Goal"
           :in-theory (disable sum-rule-of-int-sum-fn-cos-i-prime))))

(defthm int-fourier-sum-2-*-fixed-cos-thm-2
  (implies (and (natp i)
                (realp L)
                (not (equal L 0)))
           (equal (* 1/2 (/ L)
...
(defthm int-fourier-sum-2-*-fixed-cos-thm-3
  (implies (and (natp i)
                (posp k)
                (<= k i)
                (realp L)
...
(defun fourier-sum-2-*-fixed-sin (x i k c)
  (* (fourier-sum-2 x i c)
     (fixed-sin x k c)))

(defthm realp-fourier-sum-2-*-fixed-sin
  (realp (fourier-sum-2-*-fixed-sin x i k c))
  :rule-classes :type-prescription)

(local
 (defthm fourier-sum-2-*-fixed-sin-rewrite
   (equal (fourier-sum-2-*-fixed-sin x i k c)
          (sum-fn-sin-i-prime x i k c))
   :hints (("Goal" :in-theory (enable fn-sin-i-prime
                                      sin-orthog-prime
                                      sin-cos-orthog-prime)))))

(in-theory (disable fourier-sum-2 fourier-sum-2-*-fixed-sin))

(defun map-fourier-sum-2-*-fixed-sin (p i k c)
  (if (consp p)
      (cons (fourier-sum-2-*-fixed-sin (car p) i k c)
	    (map-fourier-sum-2-*-fixed-sin (cdr p) i k c))
    nil))

(local
 (defthm map-fourier-sum-2-*-fixed-sin-rewrite
   (equal (map-fourier-sum-2-*-fixed-sin p i k c)
          (map-sum-fn-sin-i-prime p i k c))))

(defun riemann-fourier-sum-2-*-fixed-sin (p i k c)
  (dotprod (deltas p)
	   (map-fourier-sum-2-*-fixed-sin (cdr p) i k c)))

(local
 (defthm riemann-fourier-sum-2-*-fixed-sin-rewrite
   (equal (riemann-fourier-sum-2-*-fixed-sin p i k c)
          (riemann-sum-fn-sin-i-prime p i k c))))

(in-theory (disable riemann-fourier-sum-2-*-fixed-sin))

(local (in-theory (disable riemann-sum-fn-sin-i-prime)))

(defun-std strict-fourier-sum-2-*-fixed-sin (a b i k c)
  (if (and (realp a)
           (realp b)
           (inside-interval-p a (fn-i-domain))
           (inside-interval-p b (fn-i-domain))
           (< a b))
      (standard-part (riemann-fourier-sum-2-*-fixed-sin
                      (make-small-partition a b)
                      i k c))
    0))

(defun int-fourier-sum-2-*-fixed-sin (a b i k c)
  (if (<= a b)
      (strict-fourier-sum-2-*-fixed-sin a b i k c)
    (- (strict-fourier-sum-2-*-fixed-sin b a i k c))))

(local
 (defthm-std int-fourier-sum-2-*-fixed-sin-rewrite
   (equal (int-fourier-sum-2-*-fixed-sin a b i k c)
          (int-sum-fn-sin-i-prime a b i k c))
   :hints (("Goal" :in-theory (enable int-sum-fn-sin-i-prime)))))

(in-theory (disable int-fourier-sum-2-*-fixed-sin))

(defthm int-fourier-sum-2-*-fixed-sin-thm-1
  (implies (and (natp i)
                (natp k)
                (< i k)
                (realp L)
                (not (equal L 0)))
           (equal (* (/ L)
                     (int-fourier-sum-2-*-fixed-sin (- L) L
                                                  i k (/ (acl2-pi) L)))
                  0))
  :hints (("Goal"
           :in-theory (disable sum-rule-of-int-sum-fn-sin-i-prime))))

(defthm int-fourier-sum-2-*-fixed-sin-thm-2
  (implies (and (natp i)
                (realp L)
                (not (equal L 0)))
           (equal (* (/ L)
...
(defthm int-fourier-sum-2-*-fixed-sin-thm-3
  (implies (and (natp i)
                (posp k)
                (<= k i)
                (realp L)
