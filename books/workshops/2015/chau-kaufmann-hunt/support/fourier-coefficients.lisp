...
;; Define Fourier coefficients a_i and b_i as encapsulated functions.
...
(local
 (defthm fn-cos-i-prime-continuous
   (implies (and (standardp i)
                 ...
                 (standardp c)
                 (standardp x)
                 (inside-interval-p x (fn-i-domain))
                 (inside-interval-p y (fn-i-domain))
                 (i-close x y))
            (i-close (fn-cos-i-prime x i k c) (fn-cos-i-prime y i k c)))
   :hints (("Goal"
...
(local
 (defthm fn-cos-i-derivative
   (implies (and (standardp i)
                 ...
                 (standardp x)
                 (inside-interval-p x (fn-i-domain))
                 (inside-interval-p x1 (fn-i-domain))
                 (i-close x x1)
                 (not (equal x x1)))
            (i-close (/ (- (fn-cos-i x i k c) (fn-cos-i x1 i k c))
                        (- x x1))
...
(local
 (defthm limited-riemann-fn-cos-i-prime-small-partition-lemma
   (implies (and (standardp arg)
                 (realp a) (standardp a)
                 (realp b) (standardp b)
                 (inside-interval-p a (fn-i-domain))
                 (inside-interval-p b (fn-i-domain))
                 (< a b))
 ...
 (encapsulate
  ...
  (defun-std strict-int-fn-cos-i-prime (a b i k c)
    (if (and (realp a)
             ...
             (inside-interval-p b (fn-i-domain))
             (< a b))
        (standard-part (riemann-fn-cos-i-prime (make-small-partition a b)
                                               i k c))
      0))))
...
(local
 (defthm fn-cos-i-ftc-2-lemma
   (implies (and (inside-interval-p a (fn-i-domain))
                 (inside-interval-p b (fn-i-domain)))
...
;; Define sum-fn-cos-i and sum-fn-cos-i-prime.
...
;; Prove the sum rule of the integral of sum-fi-prime.
...
(local
 (defthm fn-sin-i-prime-continuous
   (implies (and (standardp i)
                 ...
                 (inside-interval-p y (fn-i-domain))
                 (i-close x y))
            (i-close (fn-sin-i-prime x i k c) (fn-sin-i-prime y i k c)))
   :hints (("Goal"
            :use ((:instance i-close-times
                             (x (a i))
...
(local
 (defthm fn-sin-i-derivative
   (implies (and (standardp i)
                 ...
                 (i-close x x1)
                 (not (equal x x1)))
            (i-close (/ (- (fn-sin-i x i k c) (fn-sin-i x1 i k c))
                        (- x x1))
 ...
 (encapsulate
  ...
  (defun-std strict-int-fn-sin-i-prime (a b i k c)
    (if (and (realp a)
             ...
             (inside-interval-p b (fn-i-domain))
             (< a b))
        (standard-part (riemann-fn-sin-i-prime (make-small-partition a b)
                                               i k c))
      0))))
...
;; Define sum-fn-sin-i and sum-fn-sin-i-prime.
 ...
 (encapsulate
  ...
  (defun-std strict-int-sum-fn-sin-i-prime (a b i k c)
    (if (and (realp a)
             ...
             (inside-interval-p b (fn-i-domain))
             (< a b))
        (standard-part (riemann-sum-fn-sin-i-prime (make-small-partition a b)
                                                   i k c))
      0))))
...
;; Formalize the Fourier coefficients a_k.

(defun fourier-sum (x i c)
  (if (zp i)
      (a 0)
    (+ (* (a i)
          (fixed-cos x i c))
...
(defthm realp-fourier-sum
  (realp (fourier-sum x i c))
  :rule-classes :type-prescription)

(defun fourier-sum-*-fixed-cos (x i k c)
  (* (fourier-sum x i c)
     (fixed-cos x k c)))

(defthm realp-fourier-sum-*-fixed-cos
  (realp (fourier-sum-*-fixed-cos x i k c))
  :rule-classes :type-prescription)

(local
 (defthm fourier-sum-*-fixed-cos-rewrite
   (equal (fourier-sum-*-fixed-cos x i k c)
          (sum-fn-cos-i-prime x i k c))
   :hints (("Goal" :in-theory (enable fn-cos-i-prime
                                      cos-orthog-prime
                                      sin-cos-orthog-prime)))))

(in-theory (disable fourier-sum-*-fixed-cos))

(defun map-fourier-sum-*-fixed-cos (p i k c)
  (if (consp p)
      (cons (fourier-sum-*-fixed-cos (car p) i k c)
	    (map-fourier-sum-*-fixed-cos (cdr p) i k c))
    nil))

(local
 (defthm map-fourier-sum-*-fixed-cos-rewrite
   (equal (map-fourier-sum-*-fixed-cos p i k c)
          (map-sum-fn-cos-i-prime p i k c))))

(defun riemann-fourier-sum-*-fixed-cos (p i k c)
  (dotprod (deltas p)
	   (map-fourier-sum-*-fixed-cos (cdr p) i k c)))

(local
 (defthm riemann-fourier-sum-*-fixed-cos-rewrite
   (equal (riemann-fourier-sum-*-fixed-cos p i k c)
          (riemann-sum-fn-cos-i-prime p i k c))))

(in-theory (disable riemann-fourier-sum-*-fixed-cos))

(local (in-theory (disable riemann-sum-fn-cos-i-prime)))

(defun-std strict-fourier-sum-*-fixed-cos (a b i k c)
  (if (and (realp a)
           (realp b)
           (inside-interval-p a (fn-i-domain))
           (inside-interval-p b (fn-i-domain))
           (< a b))
      (standard-part (riemann-fourier-sum-*-fixed-cos
                      (make-small-partition a b)
                      i k c))
    0))

(defun int-fourier-sum-*-fixed-cos (a b i k c)
  (if (<= a b)
      (strict-fourier-sum-*-fixed-cos a b i k c)
    (- (strict-fourier-sum-*-fixed-cos b a i k c))))

(local
 (defthm-std int-fourier-sum-*-fixed-cos-rewrite
   (equal (int-fourier-sum-*-fixed-cos a b i k c)
          (int-sum-fn-cos-i-prime a b i k c))
   :hints (("Goal" :in-theory (enable int-sum-fn-cos-i-prime)))))

(in-theory (disable int-fourier-sum-*-fixed-cos))

(defthm int-fourier-sum-*-fixed-cos-thm-1
  (implies (and (natp i)
                (natp k)
                (< i k)
                (realp L)
                (not (equal L 0)))
           (equal (* (/ L)
                     (int-fourier-sum-*-fixed-cos (- L) L
                                                  i k (/ (acl2-pi) L)))
                  0))
  :hints (("Goal"
           :in-theory (disable sum-rule-of-int-sum-fn-cos-i-prime))))

(defthm int-fourier-sum-*-fixed-cos-thm-2
  (implies (and (natp i)
                (realp L)
                (not (equal L 0)))
           (equal (* 1/2 (/ L)
...
(defthm int-fourier-sum-*-fixed-cos-thm-3
  (implies (and (natp i)
                (posp k)
                (<= k i)
                (realp L)
...
;; Formalize the Fourier coefficients b_k.

(defun fourier-sum-*-fixed-sin (x i k c)
  (* (fourier-sum x i c)
     (fixed-sin x k c)))

(defthm realp-fourier-sum-*-fixed-sin
  (realp (fourier-sum-*-fixed-sin x i k c))
  :rule-classes :type-prescription)

(local
 (defthm fourier-sum-*-fixed-sin-rewrite
...
(in-theory (disable fourier-sum fourier-sum-*-fixed-sin))

(defun map-fourier-sum-*-fixed-sin (p i k c)
  (if (consp p)
      (cons (fourier-sum-*-fixed-sin (car p) i k c)
	    (map-fourier-sum-*-fixed-sin (cdr p) i k c))
    nil))

(local
 (defthm map-fourier-sum-*-fixed-sin-rewrite
   (equal (map-fourier-sum-*-fixed-sin p i k c)
          (map-sum-fn-sin-i-prime p i k c))))

(defun riemann-fourier-sum-*-fixed-sin (p i k c)
  (dotprod (deltas p)
	   (map-fourier-sum-*-fixed-sin (cdr p) i k c)))

(local
 (defthm riemann-fourier-sum-*-fixed-sin-rewrite
   (equal (riemann-fourier-sum-*-fixed-sin p i k c)
          (riemann-sum-fn-sin-i-prime p i k c))))

(in-theory (disable riemann-fourier-sum-*-fixed-sin))

(local (in-theory (disable riemann-sum-fn-sin-i-prime)))

(defun-std strict-fourier-sum-*-fixed-sin (a b i k c)
  (if (and (realp a)
           (realp b)
           (inside-interval-p a (fn-i-domain))
           (inside-interval-p b (fn-i-domain))
           (< a b))
      (standard-part (riemann-fourier-sum-*-fixed-sin
                      (make-small-partition a b)
                      i k c))
    0))

(defun int-fourier-sum-*-fixed-sin (a b i k c)
  (if (<= a b)
      (strict-fourier-sum-*-fixed-sin a b i k c)
    (- (strict-fourier-sum-*-fixed-sin b a i k c))))

(local
 (defthm-std int-fourier-sum-*-fixed-sin-rewrite
...
(in-theory (disable int-fourier-sum-*-fixed-sin))

(defthm int-fourier-sum-*-fixed-sin-thm-1
  (implies (and (natp i)
                (natp k)
                (< i k)
                (realp L)
                (not (equal L 0)))
           (equal (* (/ L)
                     (int-fourier-sum-*-fixed-sin (- L) L
                                                  i k (/ (acl2-pi) L)))
                  0))
  :hints (("Goal"
           :in-theory (disable sum-rule-of-int-sum-fn-sin-i-prime))))

(defthm int-fourier-sum-*-fixed-sin-thm-2
  (implies (and (natp i)
                (realp L)
                (not (equal L 0)))
           (equal (* (/ L)
...
(defthm int-fourier-sum-*-fixed-sin-thm-3
  (implies (and (natp i)
                (posp k)
                (<= k i)
                (realp L)
