(BOR-MACRO)
(ENCAP-ABS-STEP)
(ENCAP-ABS-P)
(ENCAP-CON-STEP)
(ENCAP-CON-P)
(ENCAP-CON-TO-ABS)
(ENCAP-GOOD-CON)
(ENCAP-CON-RANK)
(ENCAP-WF-REL)
(ENCAP-B-IS-A-WF-BISIM-CORE)
(ENCAP-ABS-STEP-TYPE)
(ENCAP-CON-STEP-TYPE)
(ENCAP-CON-NOT-ABS)
(ENCAP-ABS-NOT-CON)
(ENCAP-CON-TO-ABS-TYPE)
(ENCAP-R)
(ENCAP-B)
(ENCAP-B-IS-AN-EQUIVALENCE)
(ENCAP-RANK)
(ENCAP-TAKE-APPROPRIATE-STEP)
(ENCAP-B-IS-A-WF-BISIM-0 (140 140 (:REWRITE DEFAULT-CAR))
                         (111 111 (:REWRITE DEFAULT-<-2))
                         (111 111 (:REWRITE DEFAULT-<-1))
                         (70 70 (:REWRITE DEFAULT-CDR)))
(ENCAP-B-IS-A-WF-BISIM-1 (32 32 (:REWRITE DEFAULT-CAR))
                         (30 30 (:REWRITE DEFAULT-<-2))
                         (30 30 (:REWRITE DEFAULT-<-1))
                         (16 16 (:REWRITE DEFAULT-CDR)))
(ENCAP-B-IS-A-WF-BISIM-2 (11 11 (:REWRITE ENCAP-CON-NOT-ABS)))
(ENCAP-B-IS-A-WF-BISIM-3 (594 594 (:REWRITE ENCAP-ABS-NOT-CON))
                         (480 480 (:REWRITE DEFAULT-CAR))
                         (383 383 (:REWRITE DEFAULT-<-2))
                         (383 383 (:REWRITE DEFAULT-<-1))
                         (240 240 (:REWRITE DEFAULT-CDR)))
(ENCAP-B-IS-A-WF-BISIM (2 2 (:REWRITE ENCAP-ABS-NOT-CON)))
(ENCAP-EXISTS-W-SUCC-FOR-U-WITNESS-CONSTRAINT)
(ENCAP-EXISTS-W-SUCC-FOR-U)
(ENCAP-EXISTS-W-SUCC-FOR-U-SUFF)
(ENCAP-EXISTS-W-SUCC-FOR-S-WITNESS-CONSTRAINT)
(ENCAP-EXISTS-W-SUCC-FOR-S)
(ENCAP-EXISTS-W-SUCC-FOR-S-SUFF)
(R-TAKE-STEP (6 6 (:REWRITE ENCAP-CON-NOT-ABS)))
(B-IS-A-WF-BISIM-SK)
