(SEL-MATCH (469 204 (:REWRITE DEFAULT-+-2))
           (290 204 (:REWRITE DEFAULT-+-1))
           (128 32 (:DEFINITION INTEGER-ABS))
           (128 16 (:DEFINITION LENGTH))
           (99 99 (:REWRITE DEFAULT-CDR))
           (80 16 (:DEFINITION LEN))
           (62 43 (:REWRITE DEFAULT-<-2))
           (54 43 (:REWRITE DEFAULT-<-1))
           (34 10
               (:REWRITE SYSTEM-P-TERM-P-AUX-ARGUMENTS))
           (32 32 (:REWRITE DEFAULT-UNARY-MINUS))
           (16 16 (:TYPE-PRESCRIPTION LEN))
           (16 16 (:REWRITE DEFAULT-REALPART))
           (16 16 (:REWRITE DEFAULT-NUMERATOR))
           (16 16 (:REWRITE DEFAULT-IMAGPART))
           (16 16 (:REWRITE DEFAULT-DENOMINATOR))
           (16 16 (:REWRITE DEFAULT-COERCE-2))
           (16 16 (:REWRITE DEFAULT-COERCE-1))
           (8 4 (:DEFINITION MEMBER-EQUAL))
           (4 4 (:TYPE-PRESCRIPTION MEMBER-EQUAL)))
(SEL-MATCH-SELECT-A-PAIR (68 68 (:REWRITE DEFAULT-CAR))
                         (30 30 (:REWRITE DEFAULT-CDR)))
(SEL-MATCH-CONSP (63 63 (:REWRITE DEFAULT-CAR))
                 (33 33 (:REWRITE DEFAULT-CDR)))
(TRANSFORM-SUBS-GUARD-VERIFICATION-STUFF-1
     (86 86 (:REWRITE DEFAULT-CAR))
     (46 46 (:REWRITE DEFAULT-CDR))
     (27 19
         (:REWRITE SYSTEM-P-TERM-P-AUX-ARGUMENTS)))
(TRANSFORM-SUBS-GUARD-VERIFICATION-STUFF-2
     (268 268 (:REWRITE DEFAULT-CAR))
     (215 215 (:REWRITE DEFAULT-CDR))
     (130 50
          (:REWRITE SYSTEM-P-TERM-P-AUX-ARGUMENTS))
     (36 18 (:DEFINITION TRUE-LISTP)))
(TRANSFORM-SUBS (216 4 (:DEFINITION SEL-MATCH))
                (178 133 (:REWRITE DEFAULT-CAR))
                (112 80 (:REWRITE DEFAULT-CDR))
                (70 33 (:DEFINITION MEMBER-EQUAL))
                (66 3 (:DEFINITION SYSTEM-P))
                (39 6 (:DEFINITION TERM-P-AUX))
                (18 1 (:DEFINITION PAIR-ARGS))
                (6 6 (:TYPE-PRESCRIPTION TERM-P-AUX))
                (6 6
                   (:REWRITE SYSTEM-P-TERM-P-AUX-ARGUMENTS))
                (5 1 (:DEFINITION ASSOC-EQUAL)))
(TRANSFORM-SUBS-BRIDGE)
(TRANSFORM-SUBS-DECREASES-LENGTH-OF-FIRST-SYSTEM
     (866 22 (:DEFINITION LENGTH-SYSTEM))
     (584 365
          (:TYPE-PRESCRIPTION SEL-MATCH-CONSP))
     (370 37 (:DEFINITION SEL-MATCH))
     (286 132 (:REWRITE DEFAULT-+-2))
     (242 22 (:REWRITE COMMUTATIVITY-2-OF-+))
     (220 132 (:REWRITE DEFAULT-+-1))
     (220 44 (:DEFINITION LENGTH-TERM))
     (176 176 (:TYPE-PRESCRIPTION LENGTH-TERM))
     (126 7 (:DEFINITION PAIR-ARGS))
     (119 15 (:DEFINITION ELIMINATE))
     (110 22 (:REWRITE COMMUTATIVITY-OF-+))
     (44 44 (:REWRITE FOLD-CONSTS-IN-+))
     (25 5 (:DEFINITION BINARY-APPEND))
     (22 13 (:REWRITE DEFAULT-<-1))
     (18 13 (:REWRITE DEFAULT-<-2))
     (12 4
         (:TYPE-PRESCRIPTION TRUE-LISTP-APPEND))
     (4 4 (:TYPE-PRESCRIPTION BINARY-APPEND)))
(SUBS-SYSTEM (228 197 (:REWRITE DEFAULT-CAR))
             (190 19 (:DEFINITION SEL-MATCH))
             (165 154 (:REWRITE DEFAULT-CDR))
             (113 3 (:DEFINITION LENGTH-SYSTEM))
             (68 4 (:DEFINITION SYSTEM-P))
             (54 3 (:DEFINITION PAIR-ARGS))
             (39 18 (:REWRITE DEFAULT-+-2))
             (33 3 (:REWRITE COMMUTATIVITY-2-OF-+))
             (32 8 (:DEFINITION TERM-P-AUX))
             (30 18 (:REWRITE DEFAULT-+-1))
             (30 6 (:DEFINITION LENGTH-TERM))
             (28 4 (:DEFINITION ELIMINATE))
             (24 24 (:TYPE-PRESCRIPTION LENGTH-TERM))
             (15 3 (:REWRITE COMMUTATIVITY-OF-+))
             (15 3 (:DEFINITION ASSOC-EQUAL))
             (12 2 (:DEFINITION VAL))
             (10 2 (:DEFINITION BINARY-APPEND))
             (6 6 (:REWRITE FOLD-CONSTS-IN-+))
             (2 1 (:REWRITE DEFAULT-<-1))
             (1 1 (:REWRITE DEFAULT-<-2)))
(MATCH-MV)
(SUBS-SYSTEM-BRIDGE (1 1
                       (:TYPE-PRESCRIPTION SUBS-SYSTEM-BRIDGE)))
(MATCH-MV-BRIDGE)
(BOOLEANP-THIRD-SUBS-SYSTEM (2012 1009
                                  (:TYPE-PRESCRIPTION SEL-MATCH-CONSP))
                            (620 62 (:DEFINITION SEL-MATCH))
                            (360 20 (:DEFINITION PAIR-ARGS))
                            (333 45 (:DEFINITION ELIMINATE))
                            (95 19 (:DEFINITION BINARY-APPEND))
                            (16 16 (:TYPE-PRESCRIPTION ELIMINATE))
                            (12 4
                                (:TYPE-PRESCRIPTION TRUE-LISTP-APPEND))
                            (4 4 (:TYPE-PRESCRIPTION BINARY-APPEND)))
(NIL-THIRD-IMPLIES-NIL-SECOND-SUBS-SYSTEM
     (4041 2025
           (:TYPE-PRESCRIPTION SEL-MATCH-CONSP))
     (1310 131 (:DEFINITION SEL-MATCH))
     (740 100 (:DEFINITION ELIMINATE))
     (720 40 (:DEFINITION PAIR-ARGS))
     (180 36 (:DEFINITION BINARY-APPEND)))
(MATCH-MV-SOUNDNESS (6182 3129
                          (:TYPE-PRESCRIPTION SEL-MATCH-CONSP))
                    (2640 264 (:DEFINITION SEL-MATCH))
                    (1098 61 (:DEFINITION PAIR-ARGS))
                    (37 3 (:DEFINITION APPLY-SUBST)))
(MATCH-MV-COMPLETENESS
     (278 82 (:REWRITE DEFAULT-CAR))
     (264 3 (:DEFINITION TRANSFORM-SUBS))
     (228 114
          (:TYPE-PRESCRIPTION SEL-MATCH-CONSP))
     (216 74 (:REWRITE DEFAULT-CDR))
     (198 2
          (:REWRITE NIL-THIRD-IMPLIES-NIL-SECOND-SUBS-SYSTEM))
     (162 162 (:TYPE-PRESCRIPTION PAIR-ARGS))
     (114 114 (:TYPE-PRESCRIPTION ALISTP))
     (54 3 (:DEFINITION PAIR-ARGS))
     (30 3 (:DEFINITION SEL-MATCH))
     (27 3 (:DEFINITION ELIMINATE))
     (18 2 (:DEFINITION APPLY-SUBST))
     (15 3 (:REWRITE CONS-CAR-CDR))
     (15 3 (:DEFINITION BINARY-APPEND))
     (12 2 (:DEFINITION VAL))
     (3 3 (:DEFINITION ASSOC-EQUAL)))
(MATCH-MV-SUBSTITUTION-S-P (1274 7 (:DEFINITION SUBS-SYSTEM))
                           (664 194 (:REWRITE DEFAULT-CAR))
                           (616 7 (:DEFINITION TRANSFORM-SUBS))
                           (532 266
                                (:TYPE-PRESCRIPTION SEL-MATCH-CONSP))
                           (503 179 (:REWRITE DEFAULT-CDR))
                           (378 378 (:TYPE-PRESCRIPTION PAIR-ARGS))
                           (266 266 (:TYPE-PRESCRIPTION ALISTP))
                           (126 9 (:DEFINITION TERM-S-P-AUX))
                           (126 7 (:DEFINITION PAIR-ARGS))
                           (70 7 (:DEFINITION SEL-MATCH))
                           (63 7 (:DEFINITION ELIMINATE))
                           (45 9
                               (:REWRITE SYSTEM-S-P-TERM-P-AUX-ARGUMENTS))
                           (45 9 (:DEFINITION LEN))
                           (35 7 (:REWRITE CONS-CAR-CDR))
                           (35 7 (:DEFINITION BINARY-APPEND))
                           (22 5 (:DEFINITION MEMBER-EQUAL))
                           (18 9 (:REWRITE DEFAULT-+-2))
                           (17 17 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
                           (9 9 (:TYPE-PRESCRIPTION LEN))
                           (9 9 (:REWRITE DEFAULT-+-1))
                           (7 7 (:DEFINITION ASSOC-EQUAL))
                           (4 4 (:TYPE-PRESCRIPTION VARIABLE-P)))
(SUBS-MV (8 8 (:REWRITE DEFAULT-CAR))
         (6 6 (:REWRITE DEFAULT-CDR)))
(SUBS)
(MATCHING)
(SUBS-SOUNDNESS (1030 10 (:DEFINITION SUBS-SYSTEM))
                (570 10 (:DEFINITION TRANSFORM-SUBS))
                (540 540 (:TYPE-PRESCRIPTION PAIR-ARGS))
                (484 173 (:REWRITE DEFAULT-CAR))
                (353 156 (:REWRITE DEFAULT-CDR))
                (180 10 (:DEFINITION PAIR-ARGS))
                (60 20 (:DEFINITION ELIMINATE))
                (50 10 (:REWRITE APPEND-NIL))
                (50 10 (:DEFINITION SEL-MATCH))
                (22 3 (:DEFINITION VAL))
                (10 10 (:TYPE-PRESCRIPTION TRUE-LISTP))
                (10 10 (:REWRITE PAIR-ARGS-TRUE-LISTP))
                (10 10 (:DEFINITION ASSOC-EQUAL)))
(SUBS-COMPLETENESS (339 3 (:DEFINITION SUBS-SYSTEM))
                   (201 3 (:DEFINITION TRANSFORM-SUBS))
                   (172 79 (:REWRITE DEFAULT-CAR))
                   (162 162 (:TYPE-PRESCRIPTION PAIR-ARGS))
                   (129 2
                        (:REWRITE NIL-THIRD-IMPLIES-NIL-SECOND-SUBS-SYSTEM))
                   (128 69 (:REWRITE DEFAULT-CDR))
                   (63 7 (:DEFINITION APPLY-SUBST))
                   (54 3 (:DEFINITION PAIR-ARGS))
                   (45 6 (:DEFINITION ELIMINATE))
                   (42 7 (:DEFINITION VAL))
                   (33 9 (:REWRITE MV-NTH-0-FIRST))
                   (15 3 (:REWRITE APPEND-NIL))
                   (15 3 (:DEFINITION SEL-MATCH))
                   (3 3 (:TYPE-PRESCRIPTION TRUE-LISTP))
                   (3 3 (:REWRITE PAIR-ARGS-TRUE-LISTP))
                   (3 3
                      (:REWRITE APPLY-RETURNS-VARIABLE-IMPLIES-VARIABLE))
                   (3 3 (:DEFINITION ASSOC-EQUAL)))
(MATCHING-SUBSTITUTION-S-P
     (412 4 (:DEFINITION SUBS-SYSTEM))
     (236 2
          (:REWRITE NIL-THIRD-IMPLIES-NIL-SECOND-SUBS-SYSTEM))
     (228 4 (:DEFINITION TRANSFORM-SUBS))
     (216 216 (:TYPE-PRESCRIPTION PAIR-ARGS))
     (210 82 (:REWRITE DEFAULT-CAR))
     (162 82 (:REWRITE DEFAULT-CDR))
     (72 4 (:DEFINITION PAIR-ARGS))
     (30 6 (:DEFINITION LEN))
     (24 8 (:DEFINITION ELIMINATE))
     (20 4 (:REWRITE APPEND-NIL))
     (20 4 (:DEFINITION SEL-MATCH))
     (12 6 (:REWRITE DEFAULT-+-2))
     (6 6 (:TYPE-PRESCRIPTION LEN))
     (6 6 (:REWRITE DEFAULT-+-1))
     (4 4 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (4 4
        (:TYPE-PRESCRIPTION BOOLEANP-THIRD-SUBS-SYSTEM))
     (4 4 (:REWRITE PAIR-ARGS-TRUE-LISTP))
     (4 4 (:DEFINITION ASSOC-EQUAL))
     (2 2
        (:REWRITE SYSTEM-S-P-TERM-P-AUX-ARGUMENTS)))
(MATCHING-SUBSTITUTION-P (37 37 (:REWRITE DEFAULT-CAR))
                         (27 27 (:REWRITE DEFAULT-CDR))
                         (11 11
                             (:REWRITE SYSTEM-P-TERM-P-AUX-ARGUMENTS)))
(SUBS-MV-SUBS)
(SUBS-MV-MATCHING)
(SUBS-LIST-MV (41 33 (:REWRITE DEFAULT-CAR))
              (29 27 (:REWRITE DEFAULT-CDR))
              (18 1 (:DEFINITION PAIR-ARGS))
              (8 4 (:DEFINITION TRUE-LISTP))
              (4 4
                 (:REWRITE SYSTEM-P-TERM-P-AUX-ARGUMENTS)))
(SUBS-LIST)
(MATCHING-LIST)
(MATCHER-APPLY-SUBST-NIL (510 308 (:REWRITE DEFAULT-CAR))
                         (258 191 (:REWRITE DEFAULT-CDR))
                         (132 22 (:DEFINITION VAL))
                         (18 18
                             (:REWRITE APPLY-CONSP-IS-CONSP-LIST-OF-TERMS)))
(APPLY-SUBST-NIL-PAIR-ARGS (47 24 (:REWRITE DEFAULT-CAR))
                           (28 19 (:REWRITE DEFAULT-CDR))
                           (6 6
                              (:REWRITE APPLY-CONSP-IS-CONSP-LIST-OF-TERMS))
                           (6 1 (:DEFINITION VAL)))
(SUBS-LIST-SOUNDNESS (1656 9 (:DEFINITION SUBS-SYSTEM))
                     (900 253 (:REWRITE DEFAULT-CAR))
                     (792 9 (:DEFINITION TRANSFORM-SUBS))
                     (684 342
                          (:TYPE-PRESCRIPTION SEL-MATCH-CONSP))
                     (644 215 (:REWRITE DEFAULT-CDR))
                     (342 342 (:TYPE-PRESCRIPTION ALISTP))
                     (324 18 (:DEFINITION PAIR-ARGS))
                     (90 9 (:DEFINITION SEL-MATCH))
                     (81 9 (:DEFINITION ELIMINATE))
                     (45 9 (:REWRITE CONS-CAR-CDR))
                     (45 9 (:DEFINITION BINARY-APPEND))
                     (19 1 (:DEFINITION MATCHER))
                     (17 3 (:DEFINITION APPLY-SUBST))
                     (9 9 (:DEFINITION ASSOC-EQUAL))
                     (8 1 (:DEFINITION VAL)))
(SUBS-LIST-COMPLETENESS
     (552 3 (:DEFINITION SUBS-SYSTEM))
     (321 91 (:REWRITE DEFAULT-CAR))
     (264 3 (:DEFINITION TRANSFORM-SUBS))
     (229 79 (:REWRITE DEFAULT-CDR))
     (228 114
          (:TYPE-PRESCRIPTION SEL-MATCH-CONSP))
     (202 2
          (:REWRITE NIL-THIRD-IMPLIES-NIL-SECOND-SUBS-SYSTEM))
     (138 7 (:DEFINITION PAIR-ARGS))
     (114 114 (:TYPE-PRESCRIPTION ALISTP))
     (30 3 (:DEFINITION SEL-MATCH))
     (27 3 (:DEFINITION ELIMINATE))
     (15 3 (:REWRITE CONS-CAR-CDR))
     (15 3 (:DEFINITION BINARY-APPEND))
     (12 12
         (:REWRITE APPLY-CONSP-IS-CONSP-LIST-OF-TERMS))
     (12 4 (:DEFINITION APPLY-SUBST))
     (3 3 (:DEFINITION ASSOC-EQUAL)))
(MATCHING-LIST-SUBSTITUTION-S-P
     (736 4 (:DEFINITION SUBS-SYSTEM))
     (402 2
          (:REWRITE NIL-THIRD-IMPLIES-NIL-SECOND-SUBS-SYSTEM))
     (401 116 (:REWRITE DEFAULT-CAR))
     (352 4 (:DEFINITION TRANSFORM-SUBS))
     (304 152
          (:TYPE-PRESCRIPTION SEL-MATCH-CONSP))
     (288 100 (:REWRITE DEFAULT-CDR))
     (152 152 (:TYPE-PRESCRIPTION ALISTP))
     (126 7 (:DEFINITION PAIR-ARGS))
     (44 2 (:DEFINITION SUBSTITUTION-S-P))
     (40 4 (:DEFINITION SEL-MATCH))
     (36 4 (:DEFINITION ELIMINATE))
     (30 30 (:TYPE-PRESCRIPTION SUBS-SYSTEM))
     (28 4 (:DEFINITION TERM-S-P-AUX))
     (20 4 (:REWRITE CONS-CAR-CDR))
     (20 4 (:DEFINITION BINARY-APPEND))
     (10 2 (:DEFINITION LEN))
     (8 4
        (:TYPE-PRESCRIPTION BOOLEANP-THIRD-SUBS-SYSTEM))
     (4 4 (:DEFINITION ASSOC-EQUAL))
     (4 2 (:REWRITE DEFAULT-+-2))
     (2 2 (:TYPE-PRESCRIPTION LEN))
     (2 2
        (:REWRITE SYSTEM-S-P-TERM-P-AUX-ARGUMENTS))
     (2 2 (:REWRITE DEFAULT-+-1)))
(MATCHING-LIST-SUBSTITUTION-P)
(SUBS-LIST-MV-SUBS-LIST)
(SUBS-LIST-MV-MATCHING-LIST)
(SUBSUMPTION-REFLEXIVE)
(SUBSUMPTION-TRANSITIVE (15 1 (:DEFINITION COMPOSITION))
                        (9 1 (:DEFINITION APPLY-SUBST))
                        (7 7 (:REWRITE DEFAULT-CAR))
                        (6 1 (:DEFINITION VAL))
                        (5 5 (:REWRITE DEFAULT-CDR)))
(SUBSUMPTION-APPLY-SUBST)
(VARIABLE-MINIMUM-SUBSUMPTION (3 3 (:REWRITE DEFAULT-CAR))
                              (1 1 (:REWRITE DEFAULT-CDR)))
(MINIMUM-SUBSUMPTION-IMPLIES-VARIABLE)
