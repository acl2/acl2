; BibTeX to XDOC Documentation Generator
...
(defun generate-bibtex-entry-html (entry)
  "Generate HTML list item for a single BibTeX entry"
...
(defun generate-bibtex-entries-html (entries)
  "Generate HTML list items for all BibTeX entries"
  (if (endp entries)
      ""
    (let ((entry-pair (car entries)))
      (if (and (consp entry-pair)
          ...
          (concatenate 'string
                       (generate-bibtex-entry-html (cdr entry-pair))
                       (generate-bibtex-entries-html (cdr entries)))
        (generate-bibtex-entries-html (cdr entries))))))
...
(defun generate-workshop-documentation-xdoc (entries)
  ...
  (let ((entry-count (len entries))
        (entries-html (generate-bibtex-entries-html entries)))
