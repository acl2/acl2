...
\bookpath{kestrel/xml}: Add a simple XML parser

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Improvements to existing libraries start here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item New summary of instruction coverage (see \code{:doc sdm-instruction-set-summary}).
\end{itemize}
...
\bookpath{doc/100-theorems}: More of the 100 Theorems now in ACL2
\begin{itemize}
\item Cramer's Rule
...
\bookpath{build}: New build options
\begin{itemize}
\item Add new \code{cert\_param} \code{non-acl2p}, to exclude books that don't work with ACL2(p)
\item New \code{FAILURE\_LINES} var limits printing for failed/interrupted builds
\end{itemize}
...
\bookpath{kestrel}: Many improvements to basic libraries
\begin{itemize}
\item \code{acl2-arrays} (e.g., much longer arrays)
\item \code{arithmetic-light}
\item \code{booleans}
\item \code{bv} and \code{bv-lists} (including bv-arrays)
\item \code{clause-processors}
\item \code{evaluators} (e.g., \code{defevaluator+})
\item \code{file-io-light}
\item \code{floats} (rounding, connection to RTL)
\item \code{json-parser}
\item \code{lists-light}, \code{typed-lists-light}, \code{alists-light}
\item \code{prime-fields}
\item \code{sequences}
\item \code{strings-light} and \code{unicode-light}
\item \code{utilities}
\item \code{world-light}
...
\bookpath{kestrel/crypto}: Add formalizations
\begin{itemize}
...
\bookpath{kestrel/terms-light}: Add several verified term simplifiers
\begin{itemize}
...
\bookpath{kestrel/axe}: Major improvements to:
\begin{itemize}
...
\bookpath{kestrel/x86}: Added/improved:
\begin{itemize}
...
\bookpath{kestrel/helpers}: Improvements to tools that
\begin{itemize}
