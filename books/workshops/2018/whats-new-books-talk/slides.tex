...
{\tt DefunT is a variant of [defun] that can prove termination
...
{\tt Defthm<w} will attempt to prove a theorem directly
  from previously-proved theorems by generating
  suitable hints, using {\tt previous-subsumer-hints}.

\bookpath{kestrel/utilities/skip-in-book.lisp}
The utility, {\tt skip-in-book}, wraps around a form to prevent its
...
Logic-mode utilities {\tt er-soft+} and {\tt er-soft-logic} produce
...
List paths via {\tt include-book} down to a given book; may be useful
...
\bookpath{kestrel/utilities/magic-macroexpand}:
...
\begin{itemize}
\item {\tt when-not-proved} to skip instructions when all goals have
  been proved;
\item {\tt prove-guard} and {\tt prove-termination}, for using
  previously-proved guard or termination theorems efficiently; and
\item a more general macro, {\tt fancy-use}, for using lemma
...
These form an archived version of a new tool for proving termination,
{\tt defunt}; see \bookpath{kestrel/auto-termination/}.
...
Made improvements: better handling of {\tt mutual-recursion} and of
the {\tt :equiv} argument, and generated {\tt :expand} hint for better
handling of recursion.

\

\bookpath{kestrel/utilities/directed-untranslate.lisp}:
Made several improvements to {\tt directed-untranslate}, in particular
for {\tt let}, {\tt let*}, {\tt mv}, {\tt mv-let}, and {\tt b*},
...
Improved {\tt install-not-normalized} to handle cases in which
...
Updated books pertaining to {\tt apply\$}.
...
The new keyword argument {\tt :last-body} of {\tt make-flag}
specifies use of the most recent definition rule.

\

\bookpath{tools/include-raw.lisp}:
Fixed an issue with option {\tt :do-not-compile t} by extending
``fns-with-raw-code'' state globals.

\

\bookpath{tools/removable-runes.lisp}:
Improved {\tt removable-runes} and added related utility, {\tt
  minimal-runes}, which returns a list of runes to enable that is
