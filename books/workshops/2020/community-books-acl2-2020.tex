...
\item
Some fancy rewriter features discussed in the paper
\textit{New Rewriter Features in FGL} at this workshop.
\end{itemize}
...
\begin{itemize}
\item
Based on \bookpath{clause-processors/pseudo-term-fty.lisp},
treats pseudo-term as an FTY sum type.
\item
...
\item
Utility that uses meta-extract plus a runtime check to
...
\begin{itemize}
\item
Uses \code{centaur/sv} and \code{centaur/vl} books to generate
...
\begin{itemize}
\item
Covers \code{acons}, \code{assoc-equal}, \code{strip-cars}, \code{strip-cdrs},
\code{pairlis\$}, etc.
\item
Introduces \code{lookup-equal}, etc.
\item
``Minimalist'' approach seeks to minimize include-books,
especially non-local ones.
Example: ``Just give me some theorems about function X.
Don't make me include books about Y or Z.
Don't make me include the books you used to prove the theorems about X.''
\end{itemize}
...
\begin{itemize}
\item
Covers built-in arithmetic operations (\code{+}, \code{*}, \code{floor},
\code{mod}, \code{numerator}, etc.).
\item
Minimalist approach: One book per function. 
Almost zero non-local \code{include-books}.
\end{itemize}
...
\item
Claim to fame: Structure-shared representation of terms as DAGs allows
rewriting over very large terms (e.g., verifying crypto algorithms by
unrolling loops).
\item
See \code{www.kestrel.edu/axe}.
\end{itemize}
...
\begin{itemize}
\item
Covers \code{bool-fix}, \code{boolif}, \code{boolor}, \code{booland},
\code{boolxor}.
\item
Functions are guaranteed to return booleans, so you can easily tell the
...
\item
Functions are guaranteed to return bit vectors, so you can easily tell the
type.  No need to examine every leaf of large nests.
\item
Covers \code{bvchop}, \code{slice}, \code{bvcat}, \code{bvplus}, \code{bvxor},
BV rotation, etc.
\item
...
\item
Conversions between lists of bits and lists of bytes
(big-endian and little-endian).
\item
Operations that map BV operations over lists:
\code{bvchop-list}, \code{bvxor-list}.
\end{itemize}
...
\item
Defines some new functions (e.g., \code{write-bytes-to-channel}).
\end{itemize}
...
\begin{itemize}
\item
Each book includes some other library or book and then disables rules
that may cause problems and/or introduces improved versions.
\item
Eventually, the wrapped libraries could be improved
(and the wrapper books eliminated).
\end{itemize}
...
\begin{itemize}
\item
Small books about many built-in functions
(\code{cons}, \code{append}, \code{take}, \code{union-equal}, etc.).
\item
Also defines some new functions
(\code{perm}, \code{memberp}, \code{subrange}, etc.).
\item
Uses a minimalist style, as with other ``light'' libraries.
Also helps with ``auditing'' a development
(e.g., when reading all defintions in all books included by a spec).
\item
Perhaps list libraries are best included only locally in other
...
\begin{itemize}
\item
Includes \code{add}, \code{sub}, \code{neg}, \code{mul}, \code{pow},
\code{inv}, and \code{div}, all modulo a supplied prime.
\item
\code{pow} calls existing \code{mod-expt-fast} for speed.
\item
Provides many simplification rules (in-progress; normal forms may change).
\item
Includes \code{bind-free} rules for equalities: canceling common addends
...
\item
These will be gradually moved to the \code{std/basic} directory.
\end{itemize}
...
\item
This is being gradually moved to the \code{std/system} directory.
\end{itemize}
...
\item
These will be gradually moved to the \code{std/util} directory.
\end{itemize}
...
\bookpath{kestrel/utilities/conjunctions.lisp}:
Utilities for manipulating conjunctions.

\separation

\bookpath{kestrel/utilities/declares0.lisp}:
Basic utilities for manipulating declares (more will be added).

\separation

\bookpath{kestrel/utilities/def-constant-opener.lisp}:
A utility that generates an opener theorem for a function when all arguments
are constant (used by Axe).

\separation

\bookpath{kestrel/utilities/defopeners.lisp}:
A utility for making opener rules for recursive functions.

\separation

\bookpath{kestrel/utilities/deftest.lisp}:
A utility for isolating tests and running them with extensive guard checking.

\separation

\bookpath{kestrel/utilities/defthm-events.lisp}:
...
\bookpath{kestrel/utilities/defun-events.lisp}:
Utilities for processing defun forms.

\separation

\bookpath{kestrel/utilities/disables.lisp}:
A book that disables some built-in functions that may be convenient to have
disabled from the start.

\separation

\bookpath{kestrel/utilities/doublets2.lisp}:
Utilities that deal with doublets (true lists of length 2).

\separation

\bookpath{kestrel/utilities/equal-of-booleans.lisp}:
Rules to break an equality of two booleans into the equivalent conjunction of
two implications.

\separation

\bookpath{kestrel/utilities/erp.lisp}:
Utilities for returning errors
(which are often assigned to a variable called \code{erp}).

\separation

\bookpath{kestrel/utilities/forms.lisp}:
...
\bookpath{kestrel/utilities/keyword-value-lists2.lisp}:
Utilities about keyword-value-lists.

\separation

\bookpath{kestrel/utilities/make-and.lisp}:
A utility to make an untranslated conjunction.

\separation

\bookpath{kestrel/utilities/make-and-nice.lisp}:
A utility to make a, possibly simplified, untranslated conjunction.

\separation

\bookpath{kestrel/utilities/make-or.lisp}:
A utility to make an untranslated disjunction.

\separation

\bookpath{kestrel/utilities/make-or-nice.lisp}:
A utility to make a, possibly simplified, untranslated disjunction.

\separation

\bookpath{kestrel/utilities/my-get-event.lisp}:
...
\bookpath{kestrel/utilities/pack.lisp}:
Utilities for making symbols from strings, natural numbers, characters,
and other symbols.

\separation

\bookpath{kestrel/utilities/remove-duplicates-equal-dollar.lisp}:
...
\bookpath{kestrel/utilities/smaller-termp.lisp}:
...
\bookpath{kestrel/utilities/substitution.lisp}:
Utilities that perform substitution.

\separation

\bookpath{kestrel/utilities/terms.lisp}:
Various utilities for manipulating terms.

\separation

\bookpath{kestrel/utilities/world.lisp}:
...
\begin{itemize}
\item
\bookpath{build/*.certdep}:
Track dependencies of books on certain ACL2 system features---see XDOC topic
\code{acl2-system-feature-dependencies}.
\item
Add \code{ifdef-(un)define(!)} for setting/unsetting environment
variables (non-)locally, with effects on \code{if(n)def} forms
understood by the build system.
\item
\bookpath{build/cert.pl} supporting libraries refactored into proper (?)
Perl modules.
\end{itemize}
...
\item
Fixed the problem of rewrite-loops by changing computed-hints structure
in Smtlink.
\item
Fixed the {\color{red} \bf{soundness bug}} that \code{unary-/} can be
interpreted as integer division in Smtlink.
\item
Added a link to an example using Smtlink for verifying an ASP* Pipeline.
(This example will be moved to the ACL2 repository once we have the next stable
version of Smtlink.)
\item
...
\item
Updates to program instrumentation utilities to allow logging (parts of)
the x86 state to a specified file in both CCL and SBCL.
\end{itemize}
...
\item
Added \code{printtree} library for efficiently composing large strings
from pieces.
\end{itemize}
...
\item Added \code{defret-mutual-generate}, discussed in the paper
\textit{Generating Mutually Inductive Theorems from Concise Descriptions}
at this workshop.
\item
Like \code{defun-nx}, \code{define} macro also disables the
executable-counterpart of a non-executable function now.
\end{itemize}
