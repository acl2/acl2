...
\begin{itemize}
\item Small books about many built-in functions (\code{cons}, \code{append}, \code{take},
  \code{union-equal}, etc.).
\item Also defines some new functions (\code{perm}, \code{memberp}, \code{subrange}, etc.).
...
\begin{itemize}
\item Includes \code{add}, \code{sub}, \code{neg}, \code{mul}, \code{pow},
  \code{inv}, and \code{div}, all modulo a supplied prime.
\item \code{pow} calls existing \code{mod-expt-fast} for speed.
\item Provides many simplification rules (in-progress; normal forms may change).
\item Includes \code{bind-free} rules for equalities: canceling common addends
