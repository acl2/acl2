(F)
(RANGE-F)
(G (17 9 (:REWRITE DEFAULT-+-2))
   (12 9 (:REWRITE DEFAULT-+-1))
   (10 9 (:REWRITE DEFAULT-<-1))
   (9 9 (:REWRITE DEFAULT-<-2))
   (8 2 (:REWRITE COMMUTATIVITY-OF-+))
   (8 1 (:DEFINITION LENGTH))
   (5 1 (:DEFINITION LEN))
   (2 2 (:REWRITE FOLD-CONSTS-IN-+))
   (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
   (2 2 (:REWRITE DEFAULT-CDR))
   (1 1 (:TYPE-PRESCRIPTION LEN))
   (1 1 (:REWRITE NUMERATOR-WHEN-INTEGERP))
   (1 1 (:REWRITE INTEGERP==>DENOMINATOR=1))
   (1 1 (:REWRITE DEFAULT-REALPART))
   (1 1 (:REWRITE DEFAULT-NUMERATOR))
   (1 1 (:REWRITE DEFAULT-IMAGPART))
   (1 1 (:REWRITE DEFAULT-DENOMINATOR))
   (1 1 (:REWRITE DEFAULT-COERCE-2))
   (1 1 (:REWRITE DEFAULT-COERCE-1))
   (1 1 (:REWRITE DEFAULT-CAR)))
(Q-WITNESS)
(Q-WITNESS-TYPE)
(Q-PROPERTY (13 11 (:REWRITE DEFAULT-<-1))
            (11 11 (:REWRITE DEFAULT-<-2))
            (2 2 (:REWRITE DEFAULT-+-2))
            (2 2 (:REWRITE DEFAULT-+-1))
            (1 1
               (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(IND-HINT (28 26 (:REWRITE DEFAULT-CAR))
          (18 14 (:REWRITE DEFAULT-+-2))
          (16 15 (:REWRITE DEFAULT-CDR))
          (16 15 (:REWRITE DEFAULT-<-1))
          (15 15 (:REWRITE DEFAULT-<-2))
          (14 14 (:REWRITE DEFAULT-+-1))
          (4 4
             (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
          (2 2 (:REWRITE EQUAL-CONSTANT-+)))
(F-I-X->=-X-LEMMA (124 103 (:REWRITE DEFAULT-<-1))
                  (105 103 (:REWRITE DEFAULT-<-2))
                  (48 44 (:REWRITE DEFAULT-+-2))
                  (44 44 (:REWRITE DEFAULT-+-1))
                  (35 7 (:REWRITE ZP-OPEN))
                  (21 7 (:REWRITE FOLD-CONSTS-IN-+))
                  (21 7 (:REWRITE <-0-+-NEGATIVE-1)))
(F-I-X->=-X (39 34 (:REWRITE DEFAULT-<-1))
            (35 34 (:REWRITE DEFAULT-<-2))
            (5 5 (:REWRITE ZP-OPEN))
            (5 5 (:REWRITE DEFAULT-+-2))
            (5 5 (:REWRITE DEFAULT-+-1))
            (1 1
               (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(F-LOWER-BOUND (17 11 (:REWRITE DEFAULT-<-1))
               (12 11 (:REWRITE DEFAULT-<-2))
               (1 1
                  (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(F-MONOTONICITY-LEMMA (25 15 (:REWRITE DEFAULT-<-1))
                      (18 15 (:REWRITE DEFAULT-<-2))
                      (16 2 (:REWRITE ZP-OPEN))
                      (14 8 (:REWRITE DEFAULT-+-2))
                      (8 8 (:REWRITE DEFAULT-+-1))
                      (8 2 (:REWRITE FOLD-CONSTS-IN-+))
                      (8 2 (:REWRITE <-0-+-NEGATIVE-1)))
(NAT-IND (17 9 (:REWRITE DEFAULT-+-2))
         (12 9 (:REWRITE DEFAULT-+-1))
         (8 2 (:REWRITE COMMUTATIVITY-OF-+))
         (8 1 (:DEFINITION LENGTH))
         (7 6 (:REWRITE DEFAULT-<-1))
         (6 6 (:REWRITE DEFAULT-<-2))
         (5 1 (:DEFINITION LEN))
         (2 2 (:REWRITE FOLD-CONSTS-IN-+))
         (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
         (2 2 (:REWRITE DEFAULT-CDR))
         (1 1 (:TYPE-PRESCRIPTION LEN))
         (1 1 (:REWRITE ZP-OPEN))
         (1 1 (:REWRITE NUMERATOR-WHEN-INTEGERP))
         (1 1 (:REWRITE INTEGERP==>DENOMINATOR=1))
         (1 1 (:REWRITE DEFAULT-REALPART))
         (1 1 (:REWRITE DEFAULT-NUMERATOR))
         (1 1 (:REWRITE DEFAULT-IMAGPART))
         (1 1 (:REWRITE DEFAULT-DENOMINATOR))
         (1 1 (:REWRITE DEFAULT-COERCE-2))
         (1 1 (:REWRITE DEFAULT-COERCE-1))
         (1 1 (:REWRITE DEFAULT-CAR)))
(F-MONOTONICITY (18 14 (:REWRITE DEFAULT-<-1))
                (17 14 (:REWRITE DEFAULT-<-2))
                (5 5 (:REWRITE DEFAULT-+-2))
                (5 5 (:REWRITE DEFAULT-+-1))
                (1 1 (:REWRITE ZP-OPEN)))
(Q-WITNESS-LESS-THAN-TWO)
(G-F-REWRITE (25 18 (:REWRITE DEFAULT-<-1))
             (18 18 (:REWRITE DEFAULT-<-2))
             (8 2 (:REWRITE ZP-OPEN))
             (4 4 (:REWRITE DEFAULT-+-2))
             (4 4 (:REWRITE DEFAULT-+-1))
             (1 1
                (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(F-UPPER-BOUND (15 11 (:REWRITE DEFAULT-<-1))
               (13 11 (:REWRITE DEFAULT-<-2))
               (10 7 (:REWRITE DEFAULT-+-2))
               (7 7 (:REWRITE DEFAULT-+-1)))
(F-IS-ID (6 5 (:REWRITE DEFAULT-<-2))
         (6 5 (:REWRITE DEFAULT-<-1)))
