(DEFPUN-ADD-TEST (34 32 (:REWRITE DEFAULT-CAR))
                 (31 29 (:REWRITE DEFAULT-CDR))
                 (14 7 (:DEFINITION TRUE-LISTP)))
(DEFPUN-CALLS-TESTS-AND-ARGS-LST (354 164 (:REWRITE DEFAULT-+-2))
                                 (222 164 (:REWRITE DEFAULT-+-1))
                                 (123 123 (:REWRITE DEFAULT-CDR))
                                 (112 28 (:DEFINITION INTEGER-ABS))
                                 (112 14 (:DEFINITION LENGTH))
                                 (70 14 (:DEFINITION LEN))
                                 (48 36 (:REWRITE DEFAULT-<-2))
                                 (44 36 (:REWRITE DEFAULT-<-1))
                                 (40 40 (:REWRITE DEFAULT-CAR))
                                 (28 28 (:REWRITE DEFAULT-UNARY-MINUS))
                                 (14 14 (:TYPE-PRESCRIPTION LEN))
                                 (14 14 (:REWRITE DEFAULT-REALPART))
                                 (14 14 (:REWRITE DEFAULT-NUMERATOR))
                                 (14 14 (:REWRITE DEFAULT-IMAGPART))
                                 (14 14 (:REWRITE DEFAULT-DENOMINATOR))
                                 (14 14 (:REWRITE DEFAULT-COERCE-2))
                                 (14 14 (:REWRITE DEFAULT-COERCE-1)))
(DEFPUN-TESTS-AND-BASE-LST (354 164 (:REWRITE DEFAULT-+-2))
                           (222 164 (:REWRITE DEFAULT-+-1))
                           (123 123 (:REWRITE DEFAULT-CDR))
                           (112 28 (:DEFINITION INTEGER-ABS))
                           (112 14 (:DEFINITION LENGTH))
                           (70 14 (:DEFINITION LEN))
                           (48 36 (:REWRITE DEFAULT-<-2))
                           (44 36 (:REWRITE DEFAULT-<-1))
                           (40 40 (:REWRITE DEFAULT-CAR))
                           (28 28 (:REWRITE DEFAULT-UNARY-MINUS))
                           (14 14 (:TYPE-PRESCRIPTION LEN))
                           (14 14 (:REWRITE DEFAULT-REALPART))
                           (14 14 (:REWRITE DEFAULT-NUMERATOR))
                           (14 14 (:REWRITE DEFAULT-IMAGPART))
                           (14 14 (:REWRITE DEFAULT-DENOMINATOR))
                           (14 14 (:REWRITE DEFAULT-COERCE-2))
                           (14 14 (:REWRITE DEFAULT-COERCE-1)))
(MAP-CONJOIN (2 2 (:REWRITE DEFAULT-CDR))
             (2 2 (:REWRITE DEFAULT-CAR))
             (2 1 (:DEFINITION TRUE-LISTP)))
(DEFPUN-DISJOIN-TESTS)
(DEFPUN-MAKE-NTH-ARG)
(DEFPUN-MAKE-CALL)
(DEFPUN-MAKE-BASE)
