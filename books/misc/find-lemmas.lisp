...
(defun find-funs-fn (fns omit-boot-strap acc wrld-tail wrld)
  (declare (xargs :mode :program))
  (if (or (endp wrld-tail)
          (and omit-boot-strap
               (and (eq (caar wrld-tail) 'command-landmark)
    ...
    (let* ((trip (car wrld-tail))
           ...
           (formula (and namex
                         (symbolp namex)
                         (member-eq type '(defun))
                         (formula namex t wrld))))
      (if (and formula
               (subsetp-eq fns (all-fnnames formula)))
          (find-funs-fn fns omit-boot-strap
                          (cons (access-event-tuple-form ev-tuple) acc)
                          (cdr wrld-tail)
                          wrld)
        (find-funs-fn fns omit-boot-strap acc (cdr wrld-tail) wrld)))))
...
; (find-funs (fn1 fn2 ...)) returns all lemmas in which all of the indicated
; function symbols occur, except those lemmas in the ground-zero world.  In
; order to include those as well, give a second argument of nil:
; (find-funs (fn1 fn2 ...) nil).
  ...
  (let* ((fns (if (and (true-listp fns)
         ...
         (fns (cond
               ...
               (t (er hard 'find-funs
                      ...
                      fns))))
         (fns `(deref-macro-name-list ',fns (macro-aliases (w state)))))
    `(find-funs-fn ,fns ',omit-boot-strap nil (w state) (w state))))
