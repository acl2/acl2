...
(defmacro defopener (&whole ev-form
  ...
  (DEFTHM NAME
    (IMPLIES HYP
             (EQUAL TERM rhs)))
  ~ev[]
  where ~c[rhs] is generated by ACL2's simplification routines.  If ~c[:hyp] is
  omitted, then of course the resulting form has the expected shape:
  ~bv[]
  (DEFTHM NAME
    (EQUAL TERM rhs)).
  ~ev[]
  If an equivalence relation symbol is supplied for ~c[:equiv], then ~c[EQUAL]
  above will be replaced by that symbol.

  The output can be rather verbose.  Once ~c[rhs] as above has been produced,
  ...
  The simplification that takes place uses a prover interface that is also used
  in the distributed book ~c[misc/bash], in which the following hint is
  automatically generated for ~c[\"Goal\"], though they can be overridden if
  explicitly supplied in the ~c[defopener] form for ~c[\"Goal\"]:
  ~bv[]
  :do-not (generalize eliminate-destructors fertilize eliminate-irrelevance)
  ~ev[]
  A suitable ~c[:do-not-induct] hint is also generated, so that induction is
  avoided during the simplification process.  This too can be overridden.

  If you only want to see the generated theorem, and not the attempted proof of
  it, use ~c[:debug t].  Alternatively, you may want to run without that
  addition and then submit ~c[:]~ilc[pcb!] to grab the generated
  ~ilc[encapsulate] form to put into the book that you are developing.
  Otherwise, the ~c[defopener] form will call the ACL2 simplifier twice each
  time you certify your book: once to generate the theorem, and once to prove
  it.

  The ~c[:flatten] keyword is ~c[t] by default, and causes the result to be of
  the form ~c[(cond (c1 v1) (c2 v2) ... (ck vk) (t v))].  That result is
  actually produced from a more primitive tree-based result, of the form
  ~c[(if c v1 v2)], where ~c[v1] and ~c[v2] can themselves be calls of ~c[if].
  If you prefer the more primitive form, use ~c[:flatten nil].
