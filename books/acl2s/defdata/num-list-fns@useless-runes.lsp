(DEFDATA::2+-LISTP)
(DEFDATA::POS-LISTP)
(DEFDATA::2+-LISTP-FORWARD-TO-POS-LISTP (19 19 (:REWRITE DEFAULT-CAR))
                                        (10 10 (:REWRITE DEFAULT-<-2))
                                        (10 10 (:REWRITE DEFAULT-<-1))
                                        (4 4 (:REWRITE DEFAULT-CDR)))
(DEFDATA::POS-LISTP-FORWARD-TO-NAT-LISTP (14 14 (:REWRITE DEFAULT-CAR))
                                         (9 9 (:REWRITE DEFAULT-<-2))
                                         (9 9 (:REWRITE DEFAULT-<-1))
                                         (4 4 (:REWRITE DEFAULT-CDR)))
(DEFDATA::NAT-LISTP-FORWARD-TO-INTEGER-LISTP)
(DEFDATA::RATIONAL-LISTP-FORWARD-TO-ACL2-NUMBER-LISTP)
(DEFDATA::ACL2-NUMBER-LISTP-FORWARD-TO-TRUE-LISTP)
(DEFDATA::SUM-LIST (2 2 (:REWRITE DEFAULT-CAR))
                   (1 1
                      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                   (1 1 (:REWRITE DEFAULT-CDR)))
(DEFDATA::PRODUCT-LIST (2 2 (:REWRITE DEFAULT-CAR))
                       (1 1
                          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                       (1 1 (:REWRITE DEFAULT-CDR)))
(DEFDATA::MAX-NAT-LIST (19 19 (:REWRITE DEFAULT-CAR))
                       (18 18 (:REWRITE DEFAULT-CDR))
                       (11 6 (:REWRITE DEFAULT-<-2))
                       (11 6 (:REWRITE DEFAULT-<-1))
                       (10 10
                           (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                       (5 5 (:TYPE-PRESCRIPTION MAX)))
(DEFDATA::SCALE (3 3
                   (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                (2 2 (:REWRITE DEFAULT-CAR))
                (1 1 (:REWRITE DEFAULT-CDR)))
(DEFDATA::SHIFT (3 3
                   (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                (2 2 (:REWRITE DEFAULT-CAR))
                (1 1 (:REWRITE DEFAULT-CDR)))
(DEFDATA::LIST-EXPT
     (3 3 (:REWRITE DEFAULT-CAR))
     (1 1
        (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
     (1 1
        (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
     (1 1 (:REWRITE DEFAULT-CDR))
     (1 1 (:REWRITE DEFAULT-<-2))
     (1 1 (:REWRITE DEFAULT-<-1)))
(DEFDATA::<=-LISTS (139 69 (:REWRITE DEFAULT-+-2))
                   (80 69 (:REWRITE DEFAULT-+-1))
                   (74 74 (:REWRITE DEFAULT-CDR))
                   (41 41 (:REWRITE DEFAULT-CAR))
                   (24 6 (:REWRITE COMMUTATIVITY-OF-+))
                   (24 6 (:DEFINITION INTEGER-ABS))
                   (24 3 (:DEFINITION LENGTH))
                   (12 9 (:REWRITE DEFAULT-<-2))
                   (11 9 (:REWRITE DEFAULT-<-1))
                   (8 8 (:REWRITE FOLD-CONSTS-IN-+))
                   (6 6 (:REWRITE DEFAULT-UNARY-MINUS))
                   (3 3 (:REWRITE DEFAULT-REALPART))
                   (3 3 (:REWRITE DEFAULT-NUMERATOR))
                   (3 3 (:REWRITE DEFAULT-IMAGPART))
                   (3 3 (:REWRITE DEFAULT-DENOMINATOR))
                   (3 3 (:REWRITE DEFAULT-COERCE-2))
                   (3 3 (:REWRITE DEFAULT-COERCE-1))
                   (2 2
                      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(DEFDATA::ALL-<= (2 2 (:REWRITE DEFAULT-CAR))
                 (1 1 (:REWRITE DEFAULT-CDR)))
(DEFDATA::*-LISTS (182 89 (:REWRITE DEFAULT-+-2))
                  (112 89 (:REWRITE DEFAULT-+-1))
                  (64 64 (:REWRITE DEFAULT-CDR))
                  (48 12 (:REWRITE COMMUTATIVITY-OF-+))
                  (48 12 (:DEFINITION INTEGER-ABS))
                  (48 6 (:DEFINITION LENGTH))
                  (32 32 (:REWRITE DEFAULT-CAR))
                  (21 16 (:REWRITE DEFAULT-<-2))
                  (20 16 (:REWRITE DEFAULT-<-1))
                  (17 17 (:REWRITE FOLD-CONSTS-IN-+))
                  (12 12 (:REWRITE DEFAULT-UNARY-MINUS))
                  (6 6 (:REWRITE DEFAULT-REALPART))
                  (6 6 (:REWRITE DEFAULT-NUMERATOR))
                  (6 6 (:REWRITE DEFAULT-IMAGPART))
                  (6 6 (:REWRITE DEFAULT-DENOMINATOR))
                  (6 6 (:REWRITE DEFAULT-COERCE-2))
                  (6 6 (:REWRITE DEFAULT-COERCE-1)))
(DEFDATA::+-LISTS (182 89 (:REWRITE DEFAULT-+-2))
                  (112 89 (:REWRITE DEFAULT-+-1))
                  (64 64 (:REWRITE DEFAULT-CDR))
                  (48 12 (:REWRITE COMMUTATIVITY-OF-+))
                  (48 12 (:DEFINITION INTEGER-ABS))
                  (48 6 (:DEFINITION LENGTH))
                  (32 32 (:REWRITE DEFAULT-CAR))
                  (21 16 (:REWRITE DEFAULT-<-2))
                  (20 16 (:REWRITE DEFAULT-<-1))
                  (17 17 (:REWRITE FOLD-CONSTS-IN-+))
                  (12 12 (:REWRITE DEFAULT-UNARY-MINUS))
                  (6 6 (:REWRITE DEFAULT-REALPART))
                  (6 6 (:REWRITE DEFAULT-NUMERATOR))
                  (6 6 (:REWRITE DEFAULT-IMAGPART))
                  (6 6 (:REWRITE DEFAULT-DENOMINATOR))
                  (6 6 (:REWRITE DEFAULT-COERCE-2))
                  (6 6 (:REWRITE DEFAULT-COERCE-1)))
(DEFDATA::MAKE-LIST-LOGIC)
(DEFDATA::PFIX)
(DEFDATA::POS-LIST-FIX)
