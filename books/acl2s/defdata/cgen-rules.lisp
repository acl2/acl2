...
(defconst *cgen-rule-keywords* '(:hyp :meta-precondition :rule :rule-classes :override-check))
(defconst *cgen-rule-classes* '(:fixer :preservation))


(defmacro cgen-rule  (name &rest keys)
  (b* ((verbosep (let ((lst (member :verbose keys)))
                   (and lst (cadr lst))))
       (keys (remove-keywords-from-args '(:verbose) keys)))
...
(defun cgen-rule-events (name keys verbosep wrld state)
  (declare (xargs :mode :program))
  (b* ((ctx 'cgen-rule)
       ((mv kwd-alist rest) (extract-keywords ctx *cgen-rule-keywords* keys nil))
