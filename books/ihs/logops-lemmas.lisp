...
(defsection ihs/logif-lemmas
  :parents (logif logops-lemmas)
  :short "Lemmas about @(see logif) from the @(see logops-lemmas) book."

  (defthm simplify-logif
    (and (equal (logif 0 then else) (ifix else))
         (equal (logif -1 then else) (ifix then)))))
...
(defrule simplify-bit-functions
  ...
  (and (equal (b-and x y) (b-and y x))
       ...
       (equal (b-if 1 then else) (bfix then))
       (equal (b-if 0 then else) (bfix else))
       (equal (b-if test 1 0) (bfix test))
       (equal (b-if test 0 1) (b-not test))
       (equal (b-if test then then) (bfix then))
       (equal (b-if test then 0)    (b-and test then))
       (equal (b-if test then 1)    (b-ior (b-not test) then))
       (equal (b-if test then test) (b-and test then))
       (equal (b-if test 1 else) (b-ior test else))
       (equal (b-if test 0 else) (b-and (b-not test) else))
       (equal (b-if test test else) (b-ior test else)))

  :enable (bitp b-and b-ior b-xor b-not b-eqv b-nand b-nor
                b-andc1 b-andc2 b-orc1 b-orc2 b-if))
