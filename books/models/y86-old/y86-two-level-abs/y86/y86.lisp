...
;; Shilpi: Note that with abstract stobjs, the need for specifying (x86-32p
;; x86-32) as a guard has vanished.  Just the :stobjs argument suffices.
...
(encapsulate ()
(local (include-book "../common/arithmetic-5++"))
(defthm y86-condition-less
  (< (y86-condition x86-32 condition-index) 2)
  :rule-classes :linear)) ;; End of encapsulate
...
;; TO-DO@Shilpi: Ugly hacks to get things done quickly.
;; Don't I have a !rgfi-rgfi-different kind of theorem?

(local (defthmd crock-1
         (implies (x86-32p x86-32)
                  (x86-32p (!RGFI 4 (RGFI 5 X86-32) X86-32)))
         :rule-classes :forward-chaining))

(defthmd y86-leave-helper
  (implies (x86-32p x86-32)
           (<= 0
               (rm32 (rgfi 5 x86-32)
                     (!rgfi 4 (rgfi 5 x86-32) x86-32))))
  :hints (("Goal" :in-theory (enable crock-1))))

(defund y86-leave (x86-32)
  (declare (xargs :stobjs (x86-32)
                  :guard-hints (("Goal"
                                 :in-theory (enable y86-leave-helper)))))
  (b* ((pc (eip x86-32))
