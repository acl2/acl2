(PSEUDO-TERMP-FLG (571 257 (:REWRITE DEFAULT-+-2))
                  (359 257 (:REWRITE DEFAULT-+-1))
                  (200 40 (:DEFINITION INTEGER-ABS))
                  (144 144 (:REWRITE DEFAULT-CDR))
                  (111 111 (:REWRITE DEFAULT-CAR))
                  (68 50 (:REWRITE DEFAULT-<-2))
                  (56 50 (:REWRITE DEFAULT-<-1))
                  (40 40 (:REWRITE DEFAULT-UNARY-MINUS))
                  (20 20 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                  (20 20 (:REWRITE INTEGERP==>DENOMINATOR=1))
                  (20 20 (:REWRITE DEFAULT-REALPART))
                  (20 20 (:REWRITE DEFAULT-NUMERATOR))
                  (20 20 (:REWRITE DEFAULT-IMAGPART))
                  (20 20 (:REWRITE DEFAULT-DENOMINATOR))
                  (20 20 (:REWRITE DEFAULT-COERCE-2))
                  (20 20 (:REWRITE DEFAULT-COERCE-1))
                  (16 16 (:META CANCEL_TIMES-EQUAL-CORRECT))
                  (12 4 (:DEFINITION SYMBOL-LISTP)))
(FLAG::FLAG-EQUIV-LEMMA)
(PSEUDO-TERMP-FLG-EQUIVALENCES)
(VAR-COUNTS1-FLG (1095 519 (:REWRITE DEFAULT-+-2))
                 (712 519 (:REWRITE DEFAULT-+-1))
                 (550 110 (:DEFINITION INTEGER-ABS))
                 (440 110 (:REWRITE COMMUTATIVITY-OF-+))
                 (440 55 (:DEFINITION LENGTH))
                 (275 55 (:DEFINITION LEN))
                 (161 132 (:REWRITE DEFAULT-<-2))
                 (144 132 (:REWRITE DEFAULT-<-1))
                 (110 110 (:REWRITE DEFAULT-UNARY-MINUS))
                 (55 55 (:TYPE-PRESCRIPTION LEN))
                 (55 55 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                 (55 55 (:REWRITE INTEGERP==>DENOMINATOR=1))
                 (55 55 (:REWRITE DEFAULT-REALPART))
                 (55 55 (:REWRITE DEFAULT-NUMERATOR))
                 (55 55 (:REWRITE DEFAULT-IMAGPART))
                 (55 55 (:REWRITE DEFAULT-DENOMINATOR))
                 (55 55 (:REWRITE DEFAULT-COERCE-2))
                 (55 55 (:REWRITE DEFAULT-COERCE-1))
                 (31 31 (:META CANCEL_TIMES-EQUAL-CORRECT))
                 (31 31 (:META CANCEL_PLUS-EQUAL-CORRECT))
                 (8 2 (:TYPE-PRESCRIPTION RETURN-LAST))
                 (2 2
                    (:TYPE-PRESCRIPTION THROW-NONEXEC-ERROR)))
(FLAG::FLAG-EQUIV-LEMMA)
(VAR-COUNTS1-FLG-EQUIVALENCES)
(FLAG-LEMMA-FOR-NATP-VAR-COUNTS1
     (90 90 (:REWRITE DEFAULT-CDR))
     (66 66 (:REWRITE DEFAULT-CAR))
     (40 40 (:META CANCEL_TIMES-EQUAL-CORRECT))
     (40 40 (:META CANCEL_PLUS-EQUAL-CORRECT))
     (22 12 (:REWRITE DEFAULT-<-2))
     (22 12 (:REWRITE DEFAULT-<-1))
     (20 20
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (13 13 (:REWRITE NATP-RW))
     (12 12 (:META CANCEL_PLUS-LESSP-CORRECT))
     (11 11 (:REWRITE DEFAULT-+-2))
     (11 11 (:REWRITE DEFAULT-+-1)))
(NATP-VAR-COUNTS1-TERM)
(NATP-VAR-COUNTS1-LIST)
(INTEGER-LISTP-VAR-COUNTS (87 3 (:DEFINITION VAR-COUNTS1))
                          (36 3 (:DEFINITION MAX))
                          (25 24 (:REWRITE DEFAULT-CDR))
                          (20 19 (:REWRITE DEFAULT-CAR))
                          (9 9 (:META CANCEL_TIMES-EQUAL-CORRECT))
                          (9 9 (:META CANCEL_PLUS-EQUAL-CORRECT))
                          (6 3 (:REWRITE DEFAULT-<-2))
                          (6 3 (:REWRITE DEFAULT-<-1))
                          (3 3 (:META CANCEL_PLUS-LESSP-CORRECT)))
(LEN-VAR-COUNTS (87 3 (:DEFINITION VAR-COUNTS1))
                (36 3 (:DEFINITION MAX))
                (27 26 (:REWRITE DEFAULT-CDR))
                (24 24
                    (:TYPE-PRESCRIPTION NATP-VAR-COUNTS1-TERM))
                (15 15 (:REWRITE DEFAULT-CAR))
                (14 7 (:REWRITE DEFAULT-+-2))
                (12 12 (:META CANCEL_TIMES-EQUAL-CORRECT))
                (12 12 (:META CANCEL_PLUS-EQUAL-CORRECT))
                (7 7 (:REWRITE DEFAULT-+-1))
                (6 3 (:REWRITE DEFAULT-<-2))
                (6 3 (:REWRITE DEFAULT-<-1))
                (3 3 (:META CANCEL_PLUS-LESSP-CORRECT)))
(OCCUR-CNT-BOUNDED-FLG (323 157 (:REWRITE DEFAULT-+-2))
                       (217 157 (:REWRITE DEFAULT-+-1))
                       (170 34 (:DEFINITION INTEGER-ABS))
                       (136 34 (:REWRITE COMMUTATIVITY-OF-+))
                       (136 17 (:DEFINITION LENGTH))
                       (85 17 (:DEFINITION LEN))
                       (46 40 (:REWRITE DEFAULT-<-2))
                       (44 44 (:REWRITE DEFAULT-CDR))
                       (43 43 (:REWRITE FOLD-CONSTS-IN-+))
                       (43 40 (:REWRITE DEFAULT-<-1))
                       (34 34 (:REWRITE DEFAULT-UNARY-MINUS))
                       (32 32 (:REWRITE DEFAULT-CAR))
                       (17 17 (:TYPE-PRESCRIPTION LEN))
                       (17 17 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                       (17 17 (:REWRITE INTEGERP==>DENOMINATOR=1))
                       (17 17 (:REWRITE DEFAULT-REALPART))
                       (17 17 (:REWRITE DEFAULT-NUMERATOR))
                       (17 17 (:REWRITE DEFAULT-IMAGPART))
                       (17 17 (:REWRITE DEFAULT-DENOMINATOR))
                       (17 17 (:REWRITE DEFAULT-COERCE-2))
                       (17 17 (:REWRITE DEFAULT-COERCE-1))
                       (9 9 (:META CANCEL_TIMES-EQUAL-CORRECT))
                       (9 9 (:META CANCEL_PLUS-EQUAL-CORRECT))
                       (4 2 (:TYPE-PRESCRIPTION RETURN-LAST))
                       (2 2
                          (:TYPE-PRESCRIPTION THROW-NONEXEC-ERROR)))
(FLAG::FLAG-EQUIV-LEMMA)
(OCCUR-CNT-BOUNDED-FLG-EQUIVALENCES)
(FLAG-LEMMA-FOR-INTEGERP-OCCUR-CNT-BOUNDED
     (26 26 (:META CANCEL_TIMES-EQUAL-CORRECT))
     (26 26 (:META CANCEL_PLUS-EQUAL-CORRECT))
     (22 22 (:REWRITE DEFAULT-CAR))
     (20 10 (:REWRITE DEFAULT-<-1))
     (16 16 (:REWRITE DEFAULT-CDR))
     (10 10
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (10 10 (:REWRITE DEFAULT-<-2))
     (10 10 (:META CANCEL_PLUS-LESSP-CORRECT))
     (7 7 (:REWRITE DEFAULT-+-2))
     (7 7 (:REWRITE DEFAULT-+-1)))
(INTEGERP-OCCUR-CNT-BOUNDED-TERM)
(INTEGERP-OCCUR-CNT-BOUNDED-LIST)
(FLAG-LEMMA-FOR-SIGNED-BYTE-P-30-OCCUR-CNT-BOUNDED-FLG
     (331 264 (:REWRITE DEFAULT-<-1))
     (277 264 (:REWRITE DEFAULT-<-2))
     (121 73 (:REWRITE DEFAULT-+-1))
     (92 92
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (73 73 (:REWRITE DEFAULT-+-2))
     (72 72 (:REWRITE DEFAULT-CAR))
     (68 68 (:META CANCEL_TIMES-EQUAL-CORRECT))
     (68 68 (:META CANCEL_PLUS-EQUAL-CORRECT))
     (58 58 (:REWRITE DEFAULT-CDR)))
(SIGNED-BYTE-P-30-OCCUR-CNT-BOUNDED-FLG-TERM
     (4 4
        (:TYPE-PRESCRIPTION OCCUR-CNT-BOUNDED))
     (4 4
        (:TYPE-PRESCRIPTION INTEGERP-OCCUR-CNT-BOUNDED-TERM)))
(SIGNED-BYTE-P-30-OCCUR-CNT-BOUNDED-FLG-LIST
     (4 4
        (:TYPE-PRESCRIPTION OCCUR-CNT-BOUNDED-LST))
     (4 4
        (:TYPE-PRESCRIPTION INTEGERP-OCCUR-CNT-BOUNDED-LIST)))
