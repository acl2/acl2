...
; This book provides support for an attachable stobj introduced in the book,
; gen.lisp.  It is based on books/demos/defabsstobj-example-1.lisp, which has
...
; Being based on that book is why the generic (attachable) stobj introduced
; here has the name, st, instead of a more appropriate name (perhaps), gen: the
; name is taken from that existing book.
...
; Addition for testing attachments: We cause an error when attempting to access
; the misc$c field of st$c.
(defun misc$c+ (st$c)
  (declare (xargs :stobjs st$c))
  (prog2$ (er hard? 'misc$c+
              "Illegal attempt to access misc using generic executable!")
          (misc$c st$c)))
...
(defun-nx create-st$a ()
  (declare (xargs :guard t))
  (list 0 ; (nth 1 (create-st$c)) ; initial value of misc$c, i.e., 0
        nil                   ; mem
        ))
