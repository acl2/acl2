...
; We return an xdoc database entry based on doc-tuple.
  ...
  (let* ((name (if (stringp (nth 0 doc-tuple))
         ...
         (parents (cons parent ; put "real" parent first
                        (remove1-equal parent
                                       (cdr (hons-get name parents-fal)))))
...
(defun xdoc-alist1 (doc-alist fmt-alist parents-fal channel state acc)
  (cond ((endp doc-alist) (acl2::value acc))
        (t (acl2::er-let*
            ((entry (write-a-doc-section (car doc-alist) fmt-alist parents-fal
                                         channel state)))
            (xdoc-alist1 (cdr doc-alist) fmt-alist parents-fal channel state
                         (cons entry acc))))))
...
(defun hons-acons-add (key val fal)
  (let* ((old (cdr (hons-get key fal)))
         (new (cons val old)))
    (hons-acons key new fal)))

(defun parents-fal-1 (children parent acc)
  (cond ((endp children) acc)
        (t (parents-fal-1 (cdr children)
                          parent
                          (hons-acons-add (car children) parent acc)))))

(defun parents-fal (doc-alist acc)
  (cond ((endp doc-alist) acc)
        (t (let* ((tuple (car doc-alist))
                  (parent (car tuple))
                  (children (caddr tuple)))
             (parents-fal (cdr doc-alist)
                          (parents-fal-1 children parent acc))))))
...
(defun write-xdoc-alist-fn (write-p return-p skip-topics-fal state)
  (acl2::state-global-let*
   ...
   (let* ((doc-alist (global-val 'acl2::documentation-alist
                                 (acl2::w state)))
          (parents-fal (parents-fal doc-alist nil)))
     (mv-let
      (channel state)
