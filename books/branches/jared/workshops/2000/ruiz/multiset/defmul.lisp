    ...
    (let ((fn0
	  ...
	  (map-fn
	   (packn-in-pkg (append prefix (list "MAP-" n-fn "-E0-ORD"))
			 n-fn))
	  (final-map-fn
	   (packn-in-pkg (append prefix (list "MAP-" n-fn "-OP"))
			 n-fn))
	  (new-mp
      ...
      (this-symbol-list-is-not-new
       (list fn0 rel-bigg for-exis m-rel map-fn final-map-fn new-mp
	     new-mp-multiset-diff cong-name1 cong-name2 n-rewr mult-th mult-lm)
       world))))
...
; The following was modified for v2-8 for ordinals changes.
(defun defmul-events-fn (prefix v-g n-fn n-mp theorem varx)
  (declare (xargs :mode :program))
  (let ((fn0
	 (packn-in-pkg (append prefix (list n-fn "0"))
		       n-fn))
	(map-fn
	 (packn-in-pkg (append prefix (list "MAP-" n-fn "-E0-ORD"))
		       n-fn))
...
(defun defmul-events (list prefix verify-guards rule-classes)
  ...
  (let ((n-rel (car list))
	...
	(n-fn (cadddr list))
	(final-map-fn
	 (packn-in-pkg (append prefix (list "MAP-" (cadddr list) "-OP"))
		       (cadddr list)))
    ...
    (append 
     ...
     (defmul-events-fn l-prefix verify-guards n-fn n-mp theorem varx)
     (cond ((eq n-mp t)
	 ...
	 (list `(verify-guards ,final-map-fn))
       nil)
     (if (eq verify-guards t)
