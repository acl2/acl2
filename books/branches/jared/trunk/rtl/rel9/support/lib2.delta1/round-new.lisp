...
(local (include-book "round-new-proofs"))
...
(defun roundup_alt (x mode n)
  (case mode
    (near+ (= (bitn_alt x (- (expo x) n)) 1))
    (near (and (= (bitn_alt x (- (expo x) n)) 1)
               (or (not (exactp x (1+ n)))
                   (= (bitn_alt x (- (1+ (expo x)) n)) 1))))
    ((inf away) (not (exactp x n)))
    (otherwise ())))



(defthm roundup_alt-thm
    (implies (and (common-rounding-mode-p mode)
		  (integerp n)
		  (> n 1)
		  (integerp x)
