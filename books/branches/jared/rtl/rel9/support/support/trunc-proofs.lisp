...
(local (defthm trunc-exactp-a-helper
    (implies (and; (rationalp x)
		  (integerp n) ;drop?
                  )
	     (exactp (trunc x n) n))
...
(defthm trunc-exactp-a
  (exactp (trunc x n) n)
  :hints (("goal" :in-theory (e/d () (trunc-exactp-a-helper))
           :use (trunc-exactp-a-helper))))
...
(defthm trunc-exactp-c
    (implies (and (exactp a n)
	     ...
	     (<= a (trunc x n)))
  :hints (("goal" :in-theory (disable abs-trunc trunc-exactp-a)
		  :use ((:instance trunc-exactp-a)
			(:instance trunc-exactp-b)
			(:instance fp+2 (x (trunc x n)) (y a))
			(:instance trunc-lower-1)
                        (:instance trunc-upper-bound)
;			(:instance trunc-pos)
...
(defthm trunc-exactp-c-eric
    (implies (and (exactp a n)
	     ...
	     (<= (abs a) (abs (trunc x n))))
  :hints (("goal" :in-theory (disable abs-trunc trunc-rewrite trunc-exactp-a)
		  :use (trunc-exactp-c
                        trunc-upper-bound
