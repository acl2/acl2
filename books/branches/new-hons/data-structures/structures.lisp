...
; (set-ignore-ok t) ; eliminated after ACL2 Version_3.2.1, since we made
...
(defun predicate-body (db)
  ...
  (db-let (weak-predicate-call slot-names assertions)
...
(defun predicate (db)
  (db-let (predicate name predicate-call weak-predicate-call
...
(defun lift-if-lemma (db)
  (db-let (name lift-if-lemma slot-names)
...
(defloop predicate-slot-writers-lemma-fn
  ...
  (for ((slot in slot-names))
    (unless (db slot :READ-ONLY)
      (collect
       (db-let (name predicate-call weak-predicate-call
...
(defun prepare-for-code-gen (db)
  ...
  (db-let (slot-names representation name predicate read-only tag
                      #|normalize|# read-write write-write
