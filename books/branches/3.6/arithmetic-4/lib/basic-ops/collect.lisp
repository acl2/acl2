...
(defthm |(collect-+ y x)|
    (equal (collect-+ y x)
           (collect-+ x y)))

(theory-invariant (and (active-runep '(:rewrite |(+ x x)|))
...
(theory-invariant (not (active-runep '(:rewrite bubble-down-+-problem-finder)))
                  :error nil)
...
(theory-invariant (and (active-runep '(:rewrite bubble-down-+-bubble-down))
