; Here is an example of a double-rewrite warning from the right-hand side of a
; rewrite rule (see also rhs2.lisp).
...
(skip-proofs
 (progn
   ...
   (defstub h (x) t)
   (defstub i (x) t)
; The following warns if we omit double-rewrite, in which case the proof of
; test (below) fails.
