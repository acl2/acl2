...
(defthmd exactp-shift-2
  (implies (and (rationalp x)
           ...
           (equal (exactp (* x (expt 2 n)) m)
                  (exactp x m)))
  :hints (("Goal" :use  ((:instance exactp-shift (k n) (n m))))))
...
(defthm exactp-x2-not-zero
  ...
  :hints (("Goal" :in-theory (enable exactp2 expt-split)
		  :use ((:instance exactp-x2-6)
			(:instance x-2xx (x (* x (expt 2 (- (1- n) (expo x))))))))))
...
(defthm fp+1-1
  ...
  :rule-classes ()
  :hints (("Goal"  :in-theory (enable exactp2)
		  :use ((:instance fp+2 (y (expt 2 (1+ (expo x)))))
			(:instance expo-upper-bound)))))
...
(defthm expo-diff-min-neg
    (implies (and (rationalp x)
	     ...
	     (>= (expo (- y x)) (- (1+ (min (expo x) (expo y))) n)))
  :rule-classes ()
  :hints (("Goal" :use ((:instance expo-diff-min-pos
                                   (x (- y))
                                   (y (- x)))
                        (:instance expo-minus (x x))
                        (:instance expo-minus (x y))
                        (:instance expo-minus (x (- (- (- x) (- y))))))
           :in-theory (disable expo-comparison-rewrite-to-bound
                               expo-comparison-rewrite-to-bound-2))))

(defthm expo-diff-min-mixed
  (implies (and (rationalp x)
                (rationalp y)
                (<= x 0)
                (<= 0 y)
...
(local (defthm expo-diff
         (equal (expo (+ x (* -1 y)))
                (expo (+ y (* -1 x))))
         :hints (("Goal" :use ((:instance expo-minus (x (+ x (* -1 y)))))))))
...
(defthm expo-diff-min
  ...
  :hints (("Goal" :in-theory (disable EXPO-COMPARISON-REWRITE-TO-BOUND
                                      EXPO-COMPARISON-REWRITE-TO-BOUND-2)
           :use ((:instance expo-diff-min-pos)
                 (:instance expo-diff-min-neg)
                 (:instance expo-diff-min-mixed)
                 (:instance expo-diff-min-mixed (x y) (y x))))))

(local (in-theory (disable expo-diff)))
