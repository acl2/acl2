...
(verify-guards in
  :hints(("Goal" :in-theory (enable sets::primitives-theory))))

;; On CCL, the :exec version is almost 3x faster on the following
;; microbenchmark; logic mode: 5.014 seconds, exec mode: 1.732 seconds;
;; inlining without unrolling 2.15 seconds.  Further unrolling doesn't appear
;; to be beneficial.
;;
...
;;; Subset Testing.
;;;
;;; Now we introduce subset.  This becomes complicated because we want to use
;;; MBE to make it fast.  The fast-subset function is a tail re-cursive, linear
;;; pass through both lists.  Subset, by comparison, is a nice to reason about
;;; definition and much simpler, but would require quadratic time if we didn't
;;; use MBE here.

(defun fast-subset (X Y)
  (declare (xargs :guard (and (setp X) (setp Y))
                  :guard-hints(("Goal" :in-theory (enable primitives-theory <<)))))
  (mbe :logic
       (cond ((empty X) t)
             ((empty Y) nil)
             ((<< (head X) (head Y)) nil)
