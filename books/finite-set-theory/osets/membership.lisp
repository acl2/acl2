...
; Set Membership.
;
; We could go ahead and write another version of in, which could use the total
; order to stop early if it ever encountered an element too big.  I.e., looking
; for 1 in the list '(2 3 4), it could say that since 1 << 2, we are done.
;
; Should we do so?  Really the only question is whether or not it would be
; faster.  Certainly we can contrive situations in which it would be better,
; i.e. (in 1 '(2 3 4 .... 100000)), where we would save 100,000 calls to in.
; But we can also contrive situations that it would be slower, for example (in
; 100001 '(1 2 3 4 ... 100000)), where we would incur the extra cost of 100,000
; calls to <<.
;
; I have arbitrarily decided not to implement short-circuiting.  My reasoning
; is that (1) it is not clear which would be faster, (2) it is not clear what
; "typical" usage behavior of in would be, so even if we wanted to benchmark
; the two solutions, we could probably not come up with a good benchmarking
; suite, (3) both solutions are O(n) anyway so I don't think there's much to be
; gained here, and (4) the current method is arguably "no less efficient" than
; an unordered implementation.
...
; On CCL, the :exec version is almost 3x faster on the following
; microbenchmark; logic mode: 5.014 seconds, exec mode: 1.732 seconds; inlining
; without unrolling 2.15 seconds.  Further unrolling doesn't appear to be
; beneficial.
;
...
; The behavior of insert is, of course, determined by the set order.  Yet, we
; often need to induct upon insert's definition to prove theorems.  So, here we
; introduce a new induction scheme which hides the set order, yet still allows
; us to induct on insert very nicely.  We then disable the induction scheme
; that insert normally uses, and set up an induction hint so that
; weak-insert-induction will automatically be used instead.
...
; Subset Testing.
;
; Now we introduce subset.  This becomes complicated because we want to use MBE
; to make it fast.  The fast-subset function is a tail recursive, linear pass
; through both lists.  Subset, by comparison, is a nice to reason about
; definition and much simpler, but would require quadratic time if we didn't
; use MBE here.
...
; Per the following microbenchmark, the :exec version is about 2.3x faster
; than the logic mode definition; logic mode: 3.74 seconds; exec mode: 1.62
; seconds
;
; (let ((x (loop for i from 1 to 1000 collect i)))
;   (gc$)
;   (time$ (loop for i fixnum from 1 to 100000 do (sets::fast-subset x x))))
...
; BOZO we should probably switch to the witness-cp stuff.
;
; Up until version 0.81, we used an explicit argument to reduce subset proofs
; to pick-a-point style membership arguments.  Starting in 0.9, we generalize
; this argument to arbitrary predicates instead.
;
; I'll begin with some background on what we historically wanted to do.  The
; "ultimate goal" was to use pick-a-point reasoning to prove subset
; relationships.  In traditional mathematics, subset is defined using
; quantification over members, e.g., as follows:
;
;    (subset X Y) iff "forall a : (in a X) implies (in a Y)"
;
; Traditionally, one would prove the membership part of this statement, and use
; it to conclude whether or not (subset X Y) is true.  But, in ACL2, we cannot
; write a theorem of the following form, because all variables are implicitly
...
;    (subset ... ___)
;
; However, we can take the contrapositive of this theorem, which looks like the
; following:
;
;    ~(subset ... ___) => "exists a : (in a ...) ^ ~(in a ___)
;
; And we can prove this, by using a new function to search for an element which
; satisfies the existential predicate.
;
; Once we prove the above, we still need to be able to "reduce" a proof of
; (subset X Y) to a proof of (in a X) => (in a Y).  While we can't do this with
; a direct rewrite rule, we can sort of fake it using functional instantiation.
; In other words, we set up an encapsulated event with (in a X) => (in a Y) as
; its constraint, then we can use this constraint to prove that (subset X Y).
; By functionally instantiating the resulting theorem, we can reduce the subset
; problem to a membership problem.
;
; All of this was an explicit argument until version 0.9.  But, now the process
; is generalized.  First notice that subset is really nothing more than "forall
; a, (in a x) => (in a Y)."  If you let (P x) = (in a Y), then this is "forall
; a, (in a x) => (P a)".  This is a more general concept which can capture not
; only subset, but also many other ideas by simply changing the meaning of (P
; x).  For example, if P = integerp, then we can test if every element in the
; set is an integer, and so forth.  So, starting in v0.9, we use this more
; general form of encapsulate instead of the more specific version used in
; previous versions.
;
; We begin by introducing a completely arbitrary predicate.  Based on this
; generic predicate we will introduce a new function, all, which checks to see
; if every member in a set satisfies a predicate.  We also introduce find-not,
; which can be used to find us a witness for proving all-by-membership.
; Finally, we set up an encapsulate which allows us to assume that if some
; hypotheses are true, then any member of some set satisfies the predicate, and
; use this constraint to prove that if those same hypotheses are satisfied,
; then all is true for the same set.
...
; The theorem all-by-membership is massively important.  It is the basis for
; reducing subset arguments to membership arguments.  It also has a
; sufficiently general character that we can reuse it to make similar
; reductions for problems other than subset.
;
; However, for the time being, it's important that we actually do apply this
; theorem to the subset function, which is a particularly useful way of
; reasoning about sets.  Two important questions are: how can we apply the
; theorem, and when should we apply it?
;
; To support these notions, a framework is introduced in the file
; computed-hints.lisp.  Basically, we introduce a "trigger" function which is
; nothing more than an alias for subset.  We then use the "tagging theorem"
; pick-a-point-subset-strategy in order to rewrite subsets into
; subset-triggers.  This theorem is constrained so that it will only rewrite
; conclusions and it will not apply while backchaining, via syntaxp hypotheses.
; This process is described in more detail in computed-hints.lisp.
...
; We now show the basic properties of subset.  The first theorems are mundane
; but then we get more interesting, showing that subset is reflexive and
; transitive.  The goal here is to build up sufficient theorems to do
; pick-a-point proofs, which come next.
...
; Proofs of equality by double containment are also very common.  So, to
; support these, we want to show that double containment is the same as
; equality.
;
...
; BOZO if we switch to rulesets we could add these to the primitives theory.
