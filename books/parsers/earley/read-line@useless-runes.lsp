(READ-LINE!1 (7 7 (:REWRITE DEFAULT-CAR))
             (4 4 (:REWRITE DEFAULT-CDR)))
(CHARACTER-LISTP-OF-READ-LINE!1.LINE (34 34 (:REWRITE DEFAULT-CAR))
                                     (27 27 (:REWRITE DEFAULT-CDR)))
(CHARACTER-LISTP-OF-READ-LINE!1.NEW-FILE-INPUT
     (37 37 (:REWRITE DEFAULT-CAR))
     (29 29 (:REWRITE DEFAULT-CDR)))
(READ-LINE!1-WEAK (237 119 (:REWRITE DEFAULT-+-2))
                  (163 119 (:REWRITE DEFAULT-+-1))
                  (75 15 (:DEFINITION LEN))
                  (56 43 (:REWRITE DEFAULT-<-1))
                  (55 43 (:REWRITE DEFAULT-<-2))
                  (39 39 (:REWRITE DEFAULT-CDR))
                  (30 30 (:REWRITE DEFAULT-UNARY-MINUS))
                  (29 29 (:REWRITE FOLD-CONSTS-IN-+))
                  (22 22 (:REWRITE DEFAULT-CAR))
                  (15 15 (:REWRITE DEFAULT-NUMERATOR))
                  (15 15 (:REWRITE DEFAULT-DENOMINATOR))
                  (15 15 (:REWRITE DEFAULT-COERCE-2))
                  (15 15 (:REWRITE DEFAULT-COERCE-1))
                  (13 13 (:REWRITE DEFAULT-REALPART))
                  (13 13 (:REWRITE DEFAULT-IMAGPART)))
(READ-LINE!1-STRONG-LEMMA (72 34 (:REWRITE DEFAULT-+-2))
                          (47 34 (:REWRITE DEFAULT-+-1))
                          (24 6 (:REWRITE COMMUTATIVITY-OF-+))
                          (24 6 (:DEFINITION INTEGER-ABS))
                          (24 3 (:DEFINITION LENGTH))
                          (23 23 (:REWRITE DEFAULT-CDR))
                          (20 20 (:REWRITE DEFAULT-CAR))
                          (20 4 (:DEFINITION READ-LINE!1))
                          (15 3 (:DEFINITION LEN))
                          (10 10 (:REWRITE FOLD-CONSTS-IN-+))
                          (8 7 (:REWRITE DEFAULT-<-2))
                          (8 7 (:REWRITE DEFAULT-<-1))
                          (6 6 (:REWRITE DEFAULT-UNARY-MINUS))
                          (3 3 (:TYPE-PRESCRIPTION LEN))
                          (3 3 (:REWRITE DEFAULT-REALPART))
                          (3 3 (:REWRITE DEFAULT-NUMERATOR))
                          (3 3 (:REWRITE DEFAULT-IMAGPART))
                          (3 3 (:REWRITE DEFAULT-DENOMINATOR))
                          (3 3 (:REWRITE DEFAULT-COERCE-2))
                          (3 3 (:REWRITE DEFAULT-COERCE-1))
                          (3 1 (:DEFINITION CHARACTER-LISTP)))
(RECUR-ON-CDR-ONLY)
(READ-LINE!1-STRONG (355 178 (:REWRITE DEFAULT-+-2))
                    (246 178 (:REWRITE DEFAULT-+-1))
                    (168 42 (:REWRITE COMMUTATIVITY-OF-+))
                    (168 42 (:DEFINITION INTEGER-ABS))
                    (168 21 (:DEFINITION LENGTH))
                    (117 117 (:REWRITE DEFAULT-CDR))
                    (105 21 (:DEFINITION LEN))
                    (91 91 (:REWRITE DEFAULT-CAR))
                    (55 48 (:REWRITE DEFAULT-<-2))
                    (54 48 (:REWRITE DEFAULT-<-1))
                    (47 47 (:REWRITE FOLD-CONSTS-IN-+))
                    (42 42 (:REWRITE DEFAULT-UNARY-MINUS))
                    (21 21 (:TYPE-PRESCRIPTION LEN))
                    (21 21 (:REWRITE DEFAULT-REALPART))
                    (21 21 (:REWRITE DEFAULT-NUMERATOR))
                    (21 21 (:REWRITE DEFAULT-IMAGPART))
                    (21 21 (:REWRITE DEFAULT-DENOMINATOR))
                    (21 21 (:REWRITE DEFAULT-COERCE-2))
                    (21 21 (:REWRITE DEFAULT-COERCE-1))
                    (11 3 (:LINEAR READ-LINE!1-STRONG-LEMMA))
                    (9 2 (:REWRITE READ-LINE!1-STRONG-LEMMA)))
(READ-LINE!)
(STRINGP-OF-READ-LINE!.LINE (3 1 (:DEFINITION CHARACTER-LISTP))
                            (1 1 (:REWRITE DEFAULT-COERCE-3))
                            (1 1 (:REWRITE DEFAULT-COERCE-2))
                            (1 1 (:REWRITE DEFAULT-CDR))
                            (1 1 (:REWRITE DEFAULT-CAR)))
(CHARACTER-LISTP-OF-READ-LINE!.NEW-FILE-INPUT
     (3 1 (:DEFINITION CHARACTER-LISTP))
     (1 1 (:REWRITE DEFAULT-COERCE-3))
     (1 1 (:REWRITE DEFAULT-COERCE-2))
     (1 1 (:REWRITE DEFAULT-CDR))
     (1 1 (:REWRITE DEFAULT-CAR)))
(READ-LINE!-WEAK (204 4 (:DEFINITION ACL2-COUNT))
                 (64 32 (:REWRITE DEFAULT-+-2))
                 (44 32 (:REWRITE DEFAULT-+-1))
                 (32 8 (:REWRITE COMMUTATIVITY-OF-+))
                 (32 8 (:DEFINITION INTEGER-ABS))
                 (32 4 (:DEFINITION LENGTH))
                 (20 4 (:DEFINITION LEN))
                 (10 9 (:REWRITE DEFAULT-<-2))
                 (10 9 (:REWRITE DEFAULT-<-1))
                 (8 8 (:REWRITE FOLD-CONSTS-IN-+))
                 (8 8 (:REWRITE DEFAULT-UNARY-MINUS))
                 (8 8 (:REWRITE DEFAULT-CDR))
                 (5 5 (:REWRITE DEFAULT-COERCE-2))
                 (4 4 (:TYPE-PRESCRIPTION LEN))
                 (4 4 (:REWRITE DEFAULT-REALPART))
                 (4 4 (:REWRITE DEFAULT-NUMERATOR))
                 (4 4 (:REWRITE DEFAULT-IMAGPART))
                 (4 4 (:REWRITE DEFAULT-DENOMINATOR))
                 (4 4 (:REWRITE DEFAULT-COERCE-1))
                 (4 4 (:REWRITE DEFAULT-CAR))
                 (1 1 (:REWRITE DEFAULT-COERCE-3))
                 (1 1 (:LINEAR READ-LINE!1-STRONG)))
(READ-LINE!-STRONG-BUT-WITH-AN-INNER-HYP (204 4 (:DEFINITION ACL2-COUNT))
                                         (64 32 (:REWRITE DEFAULT-+-2))
                                         (44 32 (:REWRITE DEFAULT-+-1))
                                         (32 8 (:REWRITE COMMUTATIVITY-OF-+))
                                         (32 8 (:DEFINITION INTEGER-ABS))
                                         (32 4 (:DEFINITION LENGTH))
                                         (20 4 (:DEFINITION LEN))
                                         (10 9 (:REWRITE DEFAULT-<-2))
                                         (10 9 (:REWRITE DEFAULT-<-1))
                                         (8 8 (:REWRITE FOLD-CONSTS-IN-+))
                                         (8 8 (:REWRITE DEFAULT-UNARY-MINUS))
                                         (8 8 (:REWRITE DEFAULT-CDR))
                                         (5 5 (:REWRITE DEFAULT-COERCE-2))
                                         (4 4 (:TYPE-PRESCRIPTION LEN))
                                         (4 4 (:REWRITE DEFAULT-REALPART))
                                         (4 4 (:REWRITE DEFAULT-NUMERATOR))
                                         (4 4 (:REWRITE DEFAULT-IMAGPART))
                                         (4 4 (:REWRITE DEFAULT-DENOMINATOR))
                                         (4 4 (:REWRITE DEFAULT-COERCE-1))
                                         (4 4 (:REWRITE DEFAULT-CAR))
                                         (1 1 (:REWRITE DEFAULT-COERCE-3)))
(READ-LINE!-STRONG (612 12 (:DEFINITION ACL2-COUNT))
                   (194 97 (:REWRITE DEFAULT-+-2))
                   (133 97 (:REWRITE DEFAULT-+-1))
                   (96 24 (:REWRITE COMMUTATIVITY-OF-+))
                   (96 24 (:DEFINITION INTEGER-ABS))
                   (65 13 (:DEFINITION LEN))
                   (34 30 (:REWRITE DEFAULT-<-1))
                   (32 30 (:REWRITE DEFAULT-<-2))
                   (26 26 (:REWRITE DEFAULT-CDR))
                   (24 24 (:REWRITE FOLD-CONSTS-IN-+))
                   (24 24 (:REWRITE DEFAULT-UNARY-MINUS))
                   (17 17 (:REWRITE DEFAULT-COERCE-2))
                   (13 13 (:REWRITE DEFAULT-COERCE-1))
                   (13 13 (:REWRITE DEFAULT-CAR))
                   (12 12 (:REWRITE DEFAULT-REALPART))
                   (12 12 (:REWRITE DEFAULT-NUMERATOR))
                   (12 12 (:REWRITE DEFAULT-IMAGPART))
                   (12 12 (:REWRITE DEFAULT-DENOMINATOR))
                   (6 1 (:DEFINITION CHARACTER-LISTP))
                   (4 4 (:REWRITE DEFAULT-COERCE-3))
                   (1 1 (:LINEAR READ-LINE!1-STRONG)))
(READ-LINE!-STRONG-ALTERNATIVE (204 4 (:DEFINITION ACL2-COUNT))
                               (64 32 (:REWRITE DEFAULT-+-2))
                               (44 32 (:REWRITE DEFAULT-+-1))
                               (32 8 (:REWRITE COMMUTATIVITY-OF-+))
                               (32 8 (:DEFINITION INTEGER-ABS))
                               (32 4 (:DEFINITION LENGTH))
                               (20 4 (:DEFINITION LEN))
                               (12 10 (:REWRITE DEFAULT-<-1))
                               (11 10 (:REWRITE DEFAULT-<-2))
                               (8 8 (:REWRITE FOLD-CONSTS-IN-+))
                               (8 8 (:REWRITE DEFAULT-UNARY-MINUS))
                               (8 8 (:REWRITE DEFAULT-CDR))
                               (7 7 (:REWRITE DEFAULT-COERCE-2))
                               (4 4 (:TYPE-PRESCRIPTION LEN))
                               (4 4 (:REWRITE DEFAULT-REALPART))
                               (4 4 (:REWRITE DEFAULT-NUMERATOR))
                               (4 4 (:REWRITE DEFAULT-IMAGPART))
                               (4 4 (:REWRITE DEFAULT-DENOMINATOR))
                               (4 4 (:REWRITE DEFAULT-COERCE-1))
                               (4 4 (:REWRITE DEFAULT-CAR))
                               (3 3 (:REWRITE DEFAULT-COERCE-3)))
