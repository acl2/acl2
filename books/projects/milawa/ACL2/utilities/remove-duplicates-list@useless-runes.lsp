(MILAWA::REMOVE-DUPLICATES-LIST
     (19 2
         (:REWRITE MILAWA::RANK-WHEN-NOT-CONSP))
     (8 2
        (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (7 2
        (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (4 4 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (4 1
        (:REWRITE MILAWA::RANK-WHEN-MEMBERP-WEAK))
     (4 1 (:REWRITE MILAWA::RANK-WHEN-MEMBERP))
     (2 2
        (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (2 2
        (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (2 2 (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (2 2
        (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (2 2
        (:REWRITE MILAWA::MEMBERP-WHEN-MEMBERP-OF-CDR))
     (2 2
        (:REWRITE MILAWA::IN-SUPERSET-WHEN-IN-SUBSET-TWO))
     (2 2
        (:REWRITE MILAWA::IN-SUPERSET-WHEN-IN-SUBSET-ONE))
     (2 2
        (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (2 2
        (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (2 2
        (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (2 2
        (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (1 1 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (1 1
        (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (1 1
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (1 1
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (1 1
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (1 1
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
     (1 1
        (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (1 1
        (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP)))
(MILAWA::FAST-REMOVE-DUPLICATES-LIST$
     (19 2
         (:REWRITE MILAWA::RANK-WHEN-NOT-CONSP))
     (8 2
        (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (7 2
        (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (4 4 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (4 1
        (:REWRITE MILAWA::RANK-WHEN-MEMBERP-WEAK))
     (4 1 (:REWRITE MILAWA::RANK-WHEN-MEMBERP))
     (2 2
        (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (2 2
        (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (2 2 (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (2 2
        (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (2 2
        (:REWRITE MILAWA::MEMBERP-WHEN-MEMBERP-OF-CDR))
     (2 2
        (:REWRITE MILAWA::IN-SUPERSET-WHEN-IN-SUBSET-TWO))
     (2 2
        (:REWRITE MILAWA::IN-SUPERSET-WHEN-IN-SUBSET-ONE))
     (2 2
        (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (2 2
        (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (2 2
        (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (2 2
        (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (1 1 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (1 1
        (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (1 1
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (1 1
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (1 1
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (1 1
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
     (1 1
        (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (1 1
        (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP)))
(MILAWA::REMOVE-DUPLICATES-LIST-WHEN-NOT-CONSP
     (4 1
        (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (3 1
        (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (1 1
        (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (1 1
        (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (1 1 (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (1 1
        (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (1 1 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (1 1
        (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (1 1
        (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (1 1
        (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (1 1
        (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP)))
(MILAWA::REMOVE-DUPLICATES-LIST-OF-CONS
     (78 5
         (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-UNIQUE))
     (73 6
         (:REWRITE MILAWA::REMOVE-DUPLICATES-LIST-WHEN-NOT-CONSP))
     (68 5
         (:REWRITE MILAWA::UNIQUEP-WHEN-NOT-CONSP))
     (47 17
         (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (39 17
         (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (37 5
         (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP))
     (17 17
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (17 17
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (17 17
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (17 17
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (10 10
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (10 10
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (10 10
         (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (10 10
         (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (10 10 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (10 4
         (:REWRITE MILAWA::CONSP-OF-CAR-WHEN-CONS-LISTP))
     (5 5
        (:REWRITE MILAWA::UNIQUEP-WHEN-UNIQUEP-OF-DOMAIN))
     (3 3
        (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (3 3
        (:REWRITE MILAWA::CAR-WHEN-MEMBERP-OF-SINGLETON-LIST-CHEAP))
     (3 3
        (:REWRITE MILAWA::CAR-WHEN-MEMBERP-AND-NOT-MEMBERP-OF-CDR-CHEAP))
     (2 2 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (2 2
        (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (2 2
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (2 2
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (2 2
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (2 2
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
     (2 2
        (:REWRITE MILAWA::CONS-LISTP-WHEN-NOT-CONSP))
     (2 2
        (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP-ALT))
     (2 2
        (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP))
     (2 2 (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
     (2 2
        (:REWRITE MILAWA::CAR-WHEN-NOT-CONSP)))
(MILAWA::TRUE-LISTP-OF-REMOVE-DUPLICATES-LIST
     (131 31
          (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (88 25
         (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (72 25
         (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (32 2
         (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
     (31 31
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (28 2
         (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
     (25 25
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (25 25
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (25 25
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (25 25
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (21 21
         (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (21 21
         (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (21 21 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (12 2
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
     (12 2
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
     (5 5 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (5 5
        (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (5 5
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (5 5
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (5 5
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (5 5
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
     (5 5
        (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (5 5 (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
     (4 4 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
     (4 4
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
     (4 4
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
     (4 4 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
     (4 4
        (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
     (2 2
        (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY)))
(MILAWA::LEN-OF-REMOVE-DUPLICATES-LIST
     (73 27
         (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (54 9
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
     (54 9
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
     (50 17
         (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (36 9
         (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
     (27 27
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (27 27
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (27 27
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (27 27
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (21 21 (:REWRITE MILAWA::LEN-WHEN-TUPLEP))
     (18 18 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
     (18 18
         (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
     (18 18
         (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
     (18 18 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
     (18 18
         (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
     (11 11
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (11 11
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (11 11
         (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (11 11
         (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (10 10
         (:REWRITE MILAWA::TRUE-LISTP-OF-REMOVE-DUPLICATES-LIST))
     (9 9
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
     (9 9
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
     (9 9
        (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
     (9 9
        (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE))
     (8 8
        (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (8 8
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (8 8
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (8 8
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (8 8
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
     (8 8
        (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (8 8 (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
     (4 4 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (4 2 (:REWRITE MILAWA::NFIX-WHEN-ZP-CHEAP))
     (3 2
        (:REWRITE MILAWA::NFIX-WHEN-NOT-NATP-CHEAP))
     (1 1
        (:REWRITE MILAWA::ZP-WHEN-NOT-NATP-CHEAP))
     (1 1
        (:REWRITE MILAWA::ZP-WHEN-NATP-CHEAP)))
(MILAWA::CONSP-OF-REMOVE-DUPLICATES-LIST
     (94 25
         (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (28 28
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (28 28
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (28 28
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (28 28
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (24 24
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (24 24
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (23 23
         (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (23 23
         (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (23 23 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (18 18
         (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (18 18
         (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
     (13 13
         (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (13 13
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (13 13
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (13 13
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (13 13
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
     (11 11 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (6 1
        (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
     (6 1
        (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
     (5 1
        (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
     (2 2 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
     (2 2
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
     (2 2
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
     (2 2 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
     (2 2
        (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
     (1 1
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
     (1 1
        (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
     (1 1
        (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE)))
(MILAWA::CAR-OF-REMOVE-DUPLICATES-LIST
     (96 4
         (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-UNIQUE))
     (88 4
         (:REWRITE MILAWA::UNIQUEP-WHEN-NOT-CONSP))
     (52 25
         (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (44 8
         (:REWRITE MILAWA::CONSP-OF-CAR-WHEN-CONS-LISTP))
     (43 25
         (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (32 4
         (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP))
     (28 4
         (:REWRITE MILAWA::CONSP-OF-REMOVE-DUPLICATES-LIST))
     (28 4
         (:REWRITE MILAWA::CONS-LISTP-WHEN-NOT-CONSP))
     (25 25
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (25 25
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (25 25
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (25 25
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (24 4
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
     (24 4
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
     (16 4
         (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
     (9 9
        (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (9 9
        (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (9 9 (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (9 9
        (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (9 9 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (9 9
        (:REWRITE MILAWA::CAR-WHEN-MEMBERP-OF-SINGLETON-LIST-CHEAP))
     (9 9
        (:REWRITE MILAWA::CAR-WHEN-MEMBERP-AND-NOT-MEMBERP-OF-CDR-CHEAP))
     (8 8 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
     (8 8
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
     (8 8
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
     (8 8 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
     (8 8
        (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
     (4 4
        (:REWRITE MILAWA::UNIQUEP-WHEN-UNIQUEP-OF-DOMAIN))
     (4 4
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
     (4 4
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
     (4 4
        (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
     (4 4
        (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE))
     (4 4
        (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP-ALT))
     (4 4
        (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP)))
(MILAWA::CDR-OF-REMOVE-DUPLICATES-LIST
     (44 17
         (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (39 17
         (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (28 4
         (:REWRITE MILAWA::CONSP-OF-REMOVE-DUPLICATES-LIST))
     (24 4
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
     (24 4
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
     (17 17
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (17 17
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (17 17
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (17 17
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (16 4
         (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
     (9 9
        (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (9 9
        (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (9 9 (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (9 9
        (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (9 9 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (9 9
        (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (8 8 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
     (8 8
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
     (8 8
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
     (8 8 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
     (8 8
        (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
     (4 4 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (4 4
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
     (4 4
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
     (4 4
        (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
     (4 4
        (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE))
     (4 4
        (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (4 4
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (4 4
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (4 4
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (4 4
        (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP)))
(MILAWA::REMOVE-DUPLICATES-LIST-UNDER-IFF
     (92 23
         (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (81 23
         (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (23 23
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (23 23
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (23 23
         (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (23 23
         (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (23 23 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (23 23
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (23 23
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (23 23
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (23 23
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (17 17
         (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (17 17
         (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
     (12 12 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (12 12
         (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (12 12
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (12 12
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (12 12
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (12 12
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP)))
(MILAWA::REMOVE-DUPLICATES-LIST-OF-LIST-FIX
     (119 11
          (:REWRITE MILAWA::LIST-FIX-WHEN-TRUE-LISTP))
     (101 27
          (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (67 31
         (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (66 10 (:REWRITE MILAWA::CONSP-OF-LIST-FIX))
     (59 31
         (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (54 9
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
     (54 9
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
     (41 41
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (41 41
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (41 41
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (41 41
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (36 9
         (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
     (27 27
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (18 18 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
     (18 18
         (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
     (18 18
         (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
     (18 18 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
     (18 18
         (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
     (16 16
         (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (16 16
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (16 16
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (16 16
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (16 16
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
     (16 8 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (15 2
         (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-UNIQUE))
     (12 12
         (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (12 12
         (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (12 12 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (12 1
         (:REWRITE MILAWA::UNIQUEP-WHEN-NOT-CONSP))
     (9 9
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
     (9 9
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
     (9 9
        (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
     (9 9
        (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE))
     (8 8
        (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (8 8 (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
     (8 2
        (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP))
     (1 1
        (:REWRITE MILAWA::UNIQUEP-WHEN-UNIQUEP-OF-DOMAIN)))
(MILAWA::REMOVE-DUPLICATES-LIST-OF-APP
     (220 22
          (:REWRITE MILAWA::APP-WHEN-NOT-CONSP-TWO))
     (159 72
          (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (143 21
          (:REWRITE MILAWA::CONSP-OF-REMOVE-DUPLICATES-LIST))
     (134 72
          (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (132 10 (:REWRITE MILAWA::CONSP-OF-APP))
     (82 82
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (82 82
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (72 72
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (72 72
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (54 9
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
     (54 9
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
     (36 30
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (36 9
         (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
     (30 30
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (29 29
         (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (29 29
         (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (29 29 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (22 22
         (:REWRITE MILAWA::APP-OF-SINGLETON-LIST-CHEAP))
     (18 18 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
     (18 18
         (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
     (18 18
         (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
     (18 18 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
     (18 18
         (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
     (16 16
         (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (16 16
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (16 16
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (16 16
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (16 16
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
     (15 2
         (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-UNIQUE))
     (12 1
         (:REWRITE MILAWA::UNIQUEP-WHEN-NOT-CONSP))
     (9 9
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
     (9 9
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
     (9 9
        (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
     (9 9
        (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE))
     (8 8
        (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (8 8 (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
     (8 2
        (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP))
     (7 1
        (:REWRITE MILAWA::LIST-FIX-WHEN-NOT-CONSP))
     (4 4 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (1 1
        (:REWRITE MILAWA::UNIQUEP-WHEN-UNIQUEP-OF-DOMAIN)))
(MILAWA::REMOVE-DUPLICATES-LIST-OF-REV
     (221 50
          (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (87 11
         (:REWRITE MILAWA::CONSP-OF-REMOVE-DUPLICATES-LIST))
     (75 39
         (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (73 11 (:REWRITE MILAWA::REV-UNDER-IFF))
     (54 9
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
     (54 9
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
     (50 50
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (50 50
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (50 50
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (50 50
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (36 9
         (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
     (23 23
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (23 2 (:REWRITE MILAWA::APP-WHEN-NOT-CONSP))
     (20 20
         (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (20 20
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (20 20
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (20 20
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (20 20
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
     (18 18 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
     (18 18
         (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
     (18 18
         (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
     (18 18 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
     (18 18
         (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
     (15 2
         (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-UNIQUE))
     (12 12
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (12 12
         (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (12 12
         (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (12 12 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (12 1
         (:REWRITE MILAWA::UNIQUEP-WHEN-NOT-CONSP))
     (11 11 (:REWRITE MILAWA::TRUE-LISTP-OF-REV))
     (9 9
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
     (9 9
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
     (9 9
        (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
     (9 9
        (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE))
     (8 8
        (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (8 8 (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
     (8 2
        (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP))
     (4 4 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (2 2
        (:REWRITE MILAWA::APP-WHEN-NOT-CONSP-TWO))
     (2 2
        (:REWRITE MILAWA::APP-OF-SINGLETON-LIST-CHEAP))
     (2 2
        (:REWRITE MILAWA::APP-OF-CONS-WHEN-NOT-CONSP-RIGHT))
     (1 1
        (:REWRITE MILAWA::UNIQUEP-WHEN-UNIQUEP-OF-DOMAIN)))
(MILAWA::FIRSTN-OF-REMOVE-DUPLICATES-LIST
 (868 9 (:REWRITE MILAWA::|(< a (- b c))|))
 (485 74
      (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
 (356 13 (:REWRITE MILAWA::LESS-OF-ONE-LEFT))
 (353 41
      (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
 (243 6 (:REWRITE MILAWA::SQUEEZE-LAW-THREE))
 (235 5
      (:REWRITE MILAWA::MINUS-WHEN-NOT-LESS))
 (161 2 (:REWRITE MILAWA::CONSP-OF-FIRSTN))
 (141 75
      (:REWRITE MILAWA::ZP-WHEN-NOT-NATP-CHEAP))
 (136 85
      (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
 (129 86
      (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
 (117 117 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
 (100 6
      (:REWRITE MILAWA::EQUAL-OF-LEN-AND-ZERO))
 (98 5
     (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-UNIQUE))
 (96 2 (:REWRITE MILAWA::|(= c (- a b))|))
 (94 2 (:REWRITE MILAWA::|(- (- a b) c)|))
 (88 88
     (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
 (88 88
     (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
 (88 5
     (:REWRITE MILAWA::UNIQUEP-WHEN-NOT-CONSP))
 (86 86
     (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
 (86 86
     (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
 (86 7
     (:REWRITE MILAWA::LEN-OF-REMOVE-DUPLICATES-LIST))
 (82 70
     (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
 (81 6 (:REWRITE MILAWA::SQUEEZE-LAW-ONE))
 (77 77
     (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
 (77 77
     (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
 (64 35
     (:REWRITE MILAWA::NFIX-WHEN-NOT-NATP-CHEAP))
 (56 36
     (:REWRITE MILAWA::NFIX-WHEN-ZP-CHEAP))
 (42 42 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
 (41 41
     (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
 (40 5
     (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP))
 (34 34
     (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE))
 (33 10
     (:REWRITE MILAWA::CONSP-OF-CAR-WHEN-CONS-LISTP))
 (25 5 (:REWRITE MILAWA::COMMUTATIVITY-OF-+))
 (24 8 (:REWRITE MILAWA::LESS-OF-ONE-RIGHT))
 (24 2 (:REWRITE MILAWA::|(< (- a b) c)|))
 (23 23 (:REWRITE MILAWA::NATP-OF-LEN))
 (21 21
     (:REWRITE MILAWA::TRANSITIVITY-OF-<-FOUR))
 (20 20 (:REWRITE MILAWA::NATP-OF-PLUS))
 (20 20
     (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
 (20 20
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
 (20 20
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
 (20 20
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
 (20 20
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
 (20 10
     (:REWRITE MILAWA::PLUS-WHEN-ZP-RIGHT-CHEAP))
 (20 10
     (:REWRITE MILAWA::PLUS-WHEN-ZP-LEFT-CHEAP))
 (18 18
     (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
 (18 18
     (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
 (17 17
     (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
 (11 7
     (:REWRITE MILAWA::MINUS-WHEN-ZP-LEFT-CHEAP))
 (10 10 (:REWRITE MILAWA::LEN-WHEN-TUPLEP))
 (10 10
     (:REWRITE MILAWA::GATHER-CONSTANTS-FROM-LESS-OF-PLUS))
 (10 10
     (:REWRITE MILAWA::GATHER-CONSTANTS-FROM-EQUAL-OF-PLUS))
 (9 9 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
 (8 2
    (:REWRITE MILAWA::CONS-LISTP-OF-CDR-WHEN-CONS-LISTP))
 (7 7
    (:REWRITE MILAWA::MINUS-WHEN-ZP-RIGHT-CHEAP))
 (7 7
    (:REWRITE MILAWA::CONS-LISTP-WHEN-NOT-CONSP))
 (7 7
    (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP-ALT))
 (7 7
    (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP))
 (5 5
    (:REWRITE MILAWA::|a <= b, c != 0 --> a < c+b|))
 (5 5
    (:REWRITE MILAWA::|a <= b, c != 0 --> a < b+c|))
 (5 5
    (:REWRITE MILAWA::UNIQUEP-WHEN-UNIQUEP-OF-DOMAIN))
 (5 5
    (:REWRITE MILAWA::GATHER-CONSTANTS-FROM-LESS-OF-PLUS-TWO))
 (5 5
    (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
 (5 5 (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
 (5 5 (:REWRITE MILAWA::CAR-WHEN-NOT-CONSP))
 (5 5
    (:REWRITE MILAWA::CAR-WHEN-MEMBERP-OF-SINGLETON-LIST-CHEAP))
 (5 5
    (:REWRITE MILAWA::CAR-WHEN-MEMBERP-AND-NOT-MEMBERP-OF-CDR-CHEAP))
 (4 4 (:REWRITE MILAWA::NATP-OF-MINUS))
 (4 4
    (:REWRITE MILAWA::CONSP-OF-CDR-OF-CDR-WITH-LEN-FREE))
 (4 4
    (:REWRITE MILAWA::CONSP-OF-CDR-OF-CDR-WHEN-TUPLEP-3-CHEAP))
 (3 3
    (:REWRITE MILAWA::|b <= c, a <= b --> a < 1+c|))
 (3 3
    (:REWRITE MILAWA::|a <= b, b <= c --> a < 1+c|))
 (3 3 (:REWRITE MILAWA::SQUEEZE-LAW-TWO))
 (3 1
    (:REWRITE MILAWA::LIST-FIX-WHEN-TRUE-LISTP))
 (2 2
    (:REWRITE MILAWA::CDR-OF-CDR-WITH-LEN-FREE-PAST-THE-END))
 (2
   2
   (:REWRITE MILAWA::CDR-OF-CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END)))
(MILAWA::RESTN-OF-REMOVE-DUPLICATES-LIST
 (824 8
      (:REWRITE MILAWA::REMOVE-DUPLICATES-LIST-WHEN-NOT-CONSP))
 (737 2 (:REWRITE MILAWA::CONSP-OF-RESTN))
 (551 3 (:REWRITE MILAWA::|(< (- a b) c)|))
 (372 45 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
 (281 46
      (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
 (229 9 (:REWRITE MILAWA::ANTISYMMETRY-OF-<))
 (199 24
      (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
 (187 7 (:REWRITE MILAWA::LESS-OF-ONE-LEFT))
 (177 39
      (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
 (164 2
      (:REWRITE MILAWA::|a < b --> a <= b-1|))
 (139 3
      (:REWRITE MILAWA::MINUS-WHEN-NOT-LESS))
 (98 23
     (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
 (92 44
     (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
 (84 44
     (:REWRITE MILAWA::ZP-WHEN-NOT-NATP-CHEAP))
 (82 26
     (:REWRITE MILAWA::NFIX-WHEN-NATP-CHEAP))
 (71 71 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
 (63 6
     (:REWRITE MILAWA::|a <= b, c != 0 --> a < b+c|))
 (61 43
     (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
 (60 6
     (:REWRITE MILAWA::|a <= b, c != 0 --> a < c+b|))
 (59 43
     (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
 (59 5 (:REWRITE MILAWA::LESS-OF-ONE-RIGHT))
 (57 57
     (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
 (57 57
     (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
 (56 6 (:REWRITE MILAWA::SQUEEZE-LAW-TWO))
 (56 4 (:REWRITE MILAWA::SQUEEZE-LAW-ONE))
 (54 7
     (:REWRITE MILAWA::RESTN-WHEN-NOT-NATP))
 (50 1 (:REWRITE MILAWA::|(- (- a b) c)|))
 (48 2
     (:REWRITE MILAWA::EQUAL-OF-LEN-AND-ZERO))
 (48 1 (:REWRITE MILAWA::|(= c (- a b))|))
 (45 45
     (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
 (45 45
     (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
 (43 43
     (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
 (43 43
     (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
 (43 26
     (:REWRITE MILAWA::NFIX-WHEN-NOT-NATP-CHEAP))
 (39 39 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
 (34 26
     (:REWRITE MILAWA::NFIX-WHEN-ZP-CHEAP))
 (30 2 (:REWRITE MILAWA::LEN-WHEN-NOT-CONSP))
 (26 3 (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
 (24 24
     (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
 (22 22
     (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE))
 (20 20
     (:REWRITE MILAWA::TRANSITIVITY-OF-<-FOUR))
 (15 1 (:REWRITE MILAWA::|(< a (- b c))|))
 (14 8
     (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
 (14 6
     (:REWRITE MILAWA::LESS-WHEN-ZP-RIGHT-CHEAP))
 (14 4 (:REWRITE MILAWA::SQUEEZE-LAW-THREE))
 (12 12 (:REWRITE MILAWA::NATP-OF-LEN))
 (12 2 (:REWRITE MILAWA::ONE-PLUS-TRICK))
 (10 10
     (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
 (10 10 (:REWRITE MILAWA::NATP-OF-PLUS))
 (10 10
     (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
 (10 10
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
 (10 10
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
 (10 10
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
 (10 10
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
 (8 8
    (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
 (7 1
    (:REWRITE MILAWA::LIST-FIX-WHEN-NOT-CONSP))
 (6 6
    (:REWRITE MILAWA::|b <= c, a <= b --> a < 1+c|))
 (6 6
    (:REWRITE MILAWA::|a <= b, b <= c --> a < 1+c|))
 (6 6
    (:REWRITE MILAWA::GATHER-CONSTANTS-FROM-LESS-OF-PLUS-TWO))
 (6 6
    (:REWRITE MILAWA::GATHER-CONSTANTS-FROM-EQUAL-OF-PLUS))
 (6 4
    (:REWRITE MILAWA::MINUS-WHEN-ZP-LEFT-CHEAP))
 (6 2
    (:REWRITE MILAWA::PLUS-WHEN-ZP-RIGHT-CHEAP))
 (5 5 (:REWRITE MILAWA::NATP-OF-MINUS))
 (4 4 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
 (4 4
    (:REWRITE MILAWA::MINUS-WHEN-ZP-RIGHT-CHEAP))
 (4 4
    (:REWRITE MILAWA::GATHER-CONSTANTS-FROM-LESS-OF-PLUS))
 (3 3
    (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
 (2 2
    (:REWRITE MILAWA::PLUS-WHEN-ZP-LEFT-CHEAP))
 (2 2 (:REWRITE MILAWA::LEN-WHEN-TUPLEP))
 (2 2
    (:REWRITE MILAWA::EQUAL-OF-NFIX-WITH-POSITIVE-CONSTANT))
 (1 1
    (:REWRITE MILAWA::CONSP-OF-CDR-OF-CDR-WITH-LEN-FREE))
 (1 1
    (:REWRITE MILAWA::CONSP-OF-CDR-OF-CDR-WHEN-TUPLEP-3-CHEAP))
 (1 1
    (:REWRITE MILAWA::CDR-OF-CDR-WITH-LEN-FREE-PAST-THE-END))
 (1
   1
   (:REWRITE MILAWA::CDR-OF-CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END)))
(MILAWA::REV-OF-REMOVE-DUPLICATES-LIST)
(MILAWA::MEMBERP-OF-REMOVE-DUPLICATES-IN-REMOVE-DUPLICATES-LIST-WHEN-MEMBERP
 (294 17
      (:REWRITE MILAWA::SUBSETP-WHEN-NOT-CONSP-TWO))
 (188 74
      (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
 (188 16
      (:REWRITE MILAWA::REMOVE-DUPLICATES-LIST-WHEN-NOT-CONSP))
 (172 74
      (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
 (162 9
      (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-UNIQUE))
 (157 18
      (:REWRITE MILAWA::MEMBERP-WHEN-NOT-CONSP))
 (155 113
      (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
 (147 24
      (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
 (145 8
      (:REWRITE MILAWA::UNIQUEP-WHEN-NOT-CONSP))
 (121 17
      (:REWRITE MILAWA::SUBSETP-WHEN-NOT-CONSP))
 (113 113
      (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
 (113 113
      (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
 (96 24
     (:REWRITE MILAWA::SUBSETP-WHEN-PREFIXP-CHEAP))
 (74 3 (:REWRITE MILAWA::SUBSETP-OF-CONS))
 (60 8
     (:REWRITE MILAWA::CONSP-OF-REMOVE-DUPLICATES-LIST))
 (57 9
     (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP))
 (53 27
     (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
 (43 43
     (:REWRITE MILAWA::IN-SUPERSET-WHEN-IN-SUBSET-ONE))
 (38 38
     (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
 (38 38
     (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
 (38 38
     (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
 (38 38
     (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
 (38 38 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
 (37 37
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
 (37 37
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
 (37 37
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
 (37 4
     (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
 (36 36
     (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
 (36 36
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
 (35 7
     (:REWRITE MILAWA::CONS-LISTP-WHEN-NOT-CONSP))
 (27 27
     (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
 (26 26
     (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
 (25 25
     (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
 (24 24
     (:REWRITE MILAWA::SUBSETP-TRANSITIVE-TWO))
 (24 24
     (:REWRITE MILAWA::SUBSETP-TRANSITIVE-ONE))
 (22 22 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
 (22 22
     (:REWRITE MILAWA::NOT-IN-SUBSET-WHEN-NOT-IN-SUPERSET-TWO))
 (22 22
     (:REWRITE MILAWA::MEMBERP-WHEN-DISJOINTP-TWO))
 (22 22
     (:REWRITE MILAWA::MEMBERP-WHEN-DISJOINTP-ONE))
 (19 4
     (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
 (19 4
     (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
 (8 8
    (:REWRITE MILAWA::UNIQUEP-WHEN-UNIQUEP-OF-DOMAIN))
 (7 7
    (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP-ALT))
 (7 7
    (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP))
 (6 6 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
 (6 6
    (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
 (6 6
    (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
 (6 6 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
 (6 6
    (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
 (6 6
    (:REWRITE MILAWA::CONSP-OF-CDR-OF-CDR-WITH-LEN-FREE))
 (6 6
    (:REWRITE MILAWA::CONSP-OF-CDR-OF-CDR-WHEN-TUPLEP-3-CHEAP))
 (6 6
    (:REWRITE MILAWA::CDR-OF-CDR-WITH-LEN-FREE-PAST-THE-END))
 (6 6
    (:REWRITE MILAWA::CDR-OF-CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
 (2 2 (:REWRITE MILAWA::CDR-OF-CONS)))
(MILAWA::SUBSETP-OF-REMOVE-DUPLICATES-LISTS-WHEN-SUBSETP
     (249 26
          (:REWRITE MILAWA::SUBSETP-WHEN-NOT-CONSP-TWO))
     (224 29
          (:REWRITE MILAWA::CONSP-OF-REMOVE-DUPLICATES-LIST))
     (131 44
          (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (121 26
          (:REWRITE MILAWA::SUBSETP-WHEN-NOT-CONSP))
     (113 44
          (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
     (78 72
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (72 72
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (72 72
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (68 7
         (:REWRITE MILAWA::MEMBERP-WHEN-MEMBERP-OF-CDR))
     (66 32
         (:REWRITE MILAWA::SUBSETP-TRANSITIVE-ONE))
     (63 37
         (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
     (51 51
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (44 7
         (:REWRITE MILAWA::IN-SUPERSET-WHEN-IN-SUBSET-TWO))
     (37 37
         (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
     (32 2
         (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
     (29 29
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (29 29
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (29 29
         (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (29 29
         (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (29 29 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (27 1
         (:REWRITE MILAWA::CDR-OF-REMOVE-DUPLICATES-LIST))
     (21 1
         (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-UNIQUE))
     (20 8 (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
     (19 1
         (:REWRITE MILAWA::UNIQUEP-WHEN-NOT-CONSP))
     (17 17
         (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (17 17
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (17 17
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (17 17
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (17 17
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
     (12 2
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
     (12 2
         (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
     (12 1
         (:REWRITE MILAWA::MEMBERP-WHEN-NOT-CONSP))
     (11 11 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (9 7
        (:REWRITE MILAWA::IN-SUPERSET-WHEN-IN-SUBSET-ONE))
     (8 8
        (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (7 1
        (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP))
     (5 1
        (:REWRITE MILAWA::CONS-LISTP-WHEN-NOT-CONSP))
     (4 4 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
     (4 4
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
     (4 4
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
     (4 4 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
     (4 4
        (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
     (1 1
        (:REWRITE MILAWA::UNIQUEP-WHEN-UNIQUEP-OF-DOMAIN))
     (1 1
        (:REWRITE MILAWA::NOT-IN-SUBSET-WHEN-NOT-IN-SUPERSET-TWO))
     (1 1
        (:REWRITE MILAWA::NOT-IN-SUBSET-WHEN-NOT-IN-SUPERSET-ONE))
     (1 1
        (:REWRITE MILAWA::MEMBERP-WHEN-DISJOINTP-TWO))
     (1 1
        (:REWRITE MILAWA::MEMBERP-WHEN-DISJOINTP-ONE))
     (1 1
        (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP-ALT))
     (1 1
        (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP)))
(MILAWA::NTH-OF-REMOVE-DUPLICATES-LIST
 (416 7 (:REWRITE MILAWA::|(= c (- a b))|))
 (409 15 (:REWRITE MILAWA::LESS-OF-ONE-LEFT))
 (394 55
      (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
 (311 25
      (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
 (241 5
      (:REWRITE MILAWA::MINUS-WHEN-NOT-LESS))
 (185 39 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
 (168 7 (:REWRITE MILAWA::|(< (- a b) c)|))
 (105 66
      (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
 (100 2 (:REWRITE MILAWA::|(- (- a b) c)|))
 (97 66
     (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
 (92 28
     (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
 (91 6
     (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-UNIQUE))
 (80 52
     (:REWRITE MILAWA::ZP-WHEN-NOT-NATP-CHEAP))
 (80 5
     (:REWRITE MILAWA::UNIQUEP-WHEN-NOT-CONSP))
 (66 66
     (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
 (66 66
     (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
 (66 66
     (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
 (66 66
     (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
 (64 64
     (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
 (64 64
     (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
 (57 17 (:REWRITE MILAWA::ANTISYMMETRY-OF-<))
 (53 53 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
 (45 35
     (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
 (41 41
     (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
 (39 6
     (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP))
 (35 35 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
 (32 32
     (:REWRITE MILAWA::TRANSITIVITY-OF-<-FOUR))
 (32 8
     (:REWRITE MILAWA::NFIX-WHEN-NATP-CHEAP))
 (31 31
     (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE))
 (30 10 (:REWRITE MILAWA::LESS-OF-ONE-RIGHT))
 (30 2 (:REWRITE MILAWA::|(< a (- b c))|))
 (28 2 (:REWRITE MILAWA::|(< 0 (len x))|))
 (25 25
     (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
 (23 6
     (:REWRITE MILAWA::CONSP-OF-CAR-WHEN-CONS-LISTP))
 (21 7
     (:REWRITE MILAWA::LEN-OF-REMOVE-DUPLICATES-LIST))
 (14 8
     (:REWRITE MILAWA::NFIX-WHEN-NOT-NATP-CHEAP))
 (13 13
     (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
 (13 13
     (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
 (13 13
     (:REWRITE MILAWA::LESS-WHEN-ZP-RIGHT-CHEAP))
 (12 12 (:REWRITE MILAWA::NATP-OF-MINUS))
 (11 7
     (:REWRITE MILAWA::MINUS-WHEN-ZP-LEFT-CHEAP))
 (10 10 (:REWRITE MILAWA::LEN-WHEN-TUPLEP))
 (9 9
    (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
 (9 9
    (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
 (9 9
    (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
 (9 9
    (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
 (9 9
    (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
 (8 8 (:REWRITE MILAWA::NFIX-WHEN-ZP-CHEAP))
 (8 2
    (:REWRITE MILAWA::CONS-LISTP-OF-CDR-WHEN-CONS-LISTP))
 (7 7
    (:REWRITE MILAWA::MINUS-WHEN-ZP-RIGHT-CHEAP))
 (5 5
    (:REWRITE MILAWA::|a <= b, c != 0 --> a < c+b|))
 (5 5
    (:REWRITE MILAWA::|a <= b, c != 0 --> a < b+c|))
 (5 5
    (:REWRITE MILAWA::UNIQUEP-WHEN-UNIQUEP-OF-DOMAIN))
 (5 5 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
 (5 5
    (:REWRITE MILAWA::PLUS-WHEN-ZP-RIGHT-CHEAP))
 (5 5
    (:REWRITE MILAWA::PLUS-WHEN-ZP-LEFT-CHEAP))
 (5 5
    (:REWRITE MILAWA::GATHER-CONSTANTS-FROM-LESS-OF-PLUS-TWO))
 (5 5
    (:REWRITE MILAWA::CONS-LISTP-WHEN-NOT-CONSP))
 (5 5
    (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP-ALT))
 (5 5
    (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP))
 (5 5
    (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
 (5 5 (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
 (4 4
    (:REWRITE MILAWA::CONSP-OF-CDR-OF-CDR-WITH-LEN-FREE))
 (4 4
    (:REWRITE MILAWA::CONSP-OF-CDR-OF-CDR-WHEN-TUPLEP-3-CHEAP))
 (4 4 (:REWRITE MILAWA::CAR-WHEN-NOT-CONSP))
 (4 4
    (:REWRITE MILAWA::CAR-WHEN-MEMBERP-OF-SINGLETON-LIST-CHEAP))
 (4 4
    (:REWRITE MILAWA::CAR-WHEN-MEMBERP-AND-NOT-MEMBERP-OF-CDR-CHEAP))
 (3 3
    (:REWRITE MILAWA::|b <= c, a <= b --> a < 1+c|))
 (3 3
    (:REWRITE MILAWA::|a <= b, b <= c --> a < 1+c|))
 (3 3
    (:REWRITE MILAWA::|a < b --> a <= b-1|))
 (3 3 (:REWRITE MILAWA::SQUEEZE-LAW-TWO))
 (3 3
    (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
 (3 3 (:REWRITE MILAWA::ONE-PLUS-TRICK))
 (2 2
    (:REWRITE MILAWA::CDR-OF-CDR-WITH-LEN-FREE-PAST-THE-END))
 (2
   2
   (:REWRITE MILAWA::CDR-OF-CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END)))
(MILAWA::FAST-REMOVE-DUPLICATES-LIST$-REMOVAL
 (796 199
      (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
 (498 32
      (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-UNIQUE))
 (440 26
      (:REWRITE MILAWA::UNIQUEP-WHEN-NOT-CONSP))
 (359 146
      (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
 (359 18
      (:REWRITE MILAWA::REMOVE-DUPLICATES-LIST-WHEN-NOT-CONSP))
 (347 17
      (:REWRITE MILAWA::CONSP-OF-REMOVE-DUPLICATES-LIST))
 (228 34 (:REWRITE MILAWA::REV-UNDER-IFF))
 (212 32
      (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP))
 (203 83
      (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
 (199 199
      (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
 (199 199
      (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
 (199 199
      (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
 (199 199
      (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
 (152 48
      (:REWRITE MILAWA::CONSP-OF-CAR-WHEN-CONS-LISTP))
 (117 117
      (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
 (96 16
     (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
 (96 16
     (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
 (79 19
     (:REWRITE MILAWA::APP-WHEN-NOT-CONSP-TWO))
 (71 71
     (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
 (71 71
     (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
 (71 71 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
 (64 16
     (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
 (42 7
     (:REWRITE MILAWA::APP-OF-CONS-WHEN-NOT-CONSP-RIGHT))
 (34 34 (:REWRITE MILAWA::TRUE-LISTP-OF-REV))
 (33 33
     (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
 (33 33
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
 (33 33
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
 (33 33
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
 (33 33
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
 (32 32 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
 (32 32
     (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
 (32 32
     (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
 (32 32 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
 (32 32
     (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
 (32 32
     (:REWRITE MILAWA::CONS-LISTP-WHEN-NOT-CONSP))
 (32 32
     (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP-ALT))
 (32 32
     (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP))
 (32 8
     (:REWRITE MILAWA::CONS-LISTP-OF-CDR-WHEN-CONS-LISTP))
 (28 28
     (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
 (28 28
     (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
 (28 28
     (:REWRITE MILAWA::CAR-WHEN-NOT-CONSP))
 (28 28
     (:REWRITE MILAWA::CAR-WHEN-MEMBERP-OF-SINGLETON-LIST-CHEAP))
 (28 28
     (:REWRITE MILAWA::CAR-WHEN-MEMBERP-AND-NOT-MEMBERP-OF-CDR-CHEAP))
 (26 26
     (:REWRITE MILAWA::UNIQUEP-WHEN-UNIQUEP-OF-DOMAIN))
 (19 19
     (:REWRITE MILAWA::APP-OF-SINGLETON-LIST-CHEAP))
 (16 16
     (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
 (16 16
     (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
 (16 16
     (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
 (16 16
     (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE))
 (13 13 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
 (8 8
    (:REWRITE MILAWA::CDR-OF-CDR-WITH-LEN-FREE-PAST-THE-END))
 (8
   8
   (:REWRITE MILAWA::CDR-OF-CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END)))
(MILAWA::CONSP-OF-REMOVE-DUPLICATES
     (103 28
          (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
     (34 34
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
     (32 32
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
     (32 32
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
     (32 21
         (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP))
     (31 31
         (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
     (31 31
         (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
     (30 30
         (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
     (25 25
         (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
     (25 25
         (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
     (25 25 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
     (21 7
         (:REWRITE MILAWA::UNIQUEP-OF-CDR-WHEN-UNIQUEP))
     (20 20
         (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
     (20 20
         (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
     (17 7
         (:REWRITE MILAWA::MEMBERP-WHEN-MEMBERP-OF-CDR))
     (14 14
         (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
     (14 14
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
     (14 14
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
     (14 14
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
     (14 14
         (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
     (14 12 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
     (14 3
         (:REWRITE MILAWA::LIST-FIX-WHEN-TRUE-LISTP))
     (7 7
        (:REWRITE MILAWA::IN-SUPERSET-WHEN-IN-SUBSET-TWO))
     (7 7
        (:REWRITE MILAWA::IN-SUPERSET-WHEN-IN-SUBSET-ONE))
     (6 1
        (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
     (6 1
        (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
     (5 1
        (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
     (4 4
        (:REWRITE MILAWA::NOT-IN-SUBSET-WHEN-NOT-IN-SUPERSET-TWO))
     (4 4
        (:REWRITE MILAWA::NOT-IN-SUBSET-WHEN-NOT-IN-SUPERSET-ONE))
     (4 4
        (:REWRITE MILAWA::MEMBERP-WHEN-DISJOINTP-TWO))
     (4 4
        (:REWRITE MILAWA::MEMBERP-WHEN-DISJOINTP-ONE))
     (3 1 (:REWRITE MILAWA::TRUE-LISTP-OF-CONS))
     (2 2 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
     (2 2
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
     (2 2
        (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
     (2 2 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
     (2 2
        (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
     (1 1
        (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
     (1 1
        (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
     (1 1
        (:REWRITE MILAWA::EQUAL-OF-BOOLEANS-REWRITE))
     (1 1 (:REWRITE MILAWA::CONSP-OF-DOMAIN)))
(MILAWA::CONS-LISTP-OF-REMOVE-DUPLICATES-LIST
 (670 64
      (:REWRITE MILAWA::SUBSETP-WHEN-NOT-CONSP-TWO))
 (432 64
      (:REWRITE MILAWA::CONSP-OF-REMOVE-DUPLICATES-LIST))
 (354 96
      (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP))
 (305 80
      (:REWRITE MILAWA::SUBSETP-WHEN-PREFIXP-CHEAP))
 (278 64
      (:REWRITE MILAWA::SUBSETP-WHEN-NOT-CONSP))
 (265 94
      (:REWRITE MILAWA::CONSP-WHEN-NATP-CHEAP))
 (245 94
      (:REWRITE MILAWA::CONSP-WHEN-TRUE-LISTP-CHEAP))
 (176 4 (:REWRITE MILAWA::SUBSETP-OF-CONS))
 (173 96
      (:REWRITE MILAWA::MEMBERP-WHEN-MEMBERP-OF-CDR))
 (96 96
     (:REWRITE MILAWA::IN-SUPERSET-WHEN-IN-SUBSET-TWO))
 (96 96
     (:REWRITE MILAWA::IN-SUPERSET-WHEN-IN-SUBSET-ONE))
 (96 96
     (:REWRITE MILAWA::CONSP-WHEN-NONEMPTY-SUBSET-CHEAP))
 (96 96
     (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-OF-CONS-LISTP-ALT))
 (96 96
     (:REWRITE MILAWA::CONSP-WHEN-MEMBERP-CHEAP))
 (80 80
     (:REWRITE MILAWA::SUBSETP-TRANSITIVE-TWO))
 (80 80
     (:REWRITE MILAWA::SUBSETP-TRANSITIVE-ONE))
 (78 78
     (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-TWO))
 (78 78
     (:REWRITE MILAWA::PREFIXP-WHEN-NOT-CONSP-ONE))
 (78 78
     (:REWRITE MILAWA::FORCING-PREFIXP-WHEN-NOT-PREFIXP-BADGUY))
 (63 63
     (:REWRITE MILAWA::CONSP-OF-CDR-WITH-LEN-FREE))
 (63 63
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-3-CHEAP))
 (63 63
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-TUPLEP-2-CHEAP))
 (63 63
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-MEMBERP-NOT-CAR-CHEAP))
 (63 63
     (:REWRITE MILAWA::CONSP-OF-CDR-WHEN-LEN-TWO-CHEAP))
 (60 2
     (:REWRITE MILAWA::CDR-OF-REMOVE-DUPLICATES-LIST))
 (57 57
     (:REWRITE MILAWA::TRUE-LISTP-WHEN-TUPLEP))
 (57 57
     (:REWRITE MILAWA::TRUE-LISTP-WHEN-NOT-CONSP))
 (57 57
     (:REWRITE MILAWA::NATP-WHEN-ZP-CHEAP))
 (57 57
     (:REWRITE MILAWA::NATP-WHEN-NOT-ZP-CHEAP))
 (57 57 (:REWRITE MILAWA::NATP-OF-LEN-FREE))
 (50 50
     (:REWRITE MILAWA::CONS-LISTP-OF-SUBSETP-WHEN-CONS-LISTP-ALT))
 (37 37 (:REWRITE MILAWA::TRUE-LISTP-OF-CDR))
 (33 21
     (:REWRITE MILAWA::CDR-WHEN-NOT-CONSP))
 (21 21
     (:REWRITE MILAWA::CDR-WHEN-TRUE-LISTP-WITH-LEN-FREE-PAST-THE-END))
 (18 3
     (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS-TWO))
 (18 3
     (:REWRITE MILAWA::NOT-EQUAL-WHEN-LESS))
 (12 3
     (:REWRITE MILAWA::SAME-LENGTH-PREFIXES-EQUAL-CHEAP))
 (9 6
    (:REWRITE MILAWA::CONS-LISTP-OF-CDR-WHEN-CONS-LISTP))
 (8
  2
  (:REWRITE
   MILAWA::MEMBERP-OF-REMOVE-DUPLICATES-IN-REMOVE-DUPLICATES-LIST-WHEN-MEMBERP))
 (6 6 (:REWRITE MILAWA::TRICHOTOMY-OF-<))
 (6 6
    (:REWRITE MILAWA::TRANSITIVITY-OF-<-TWO))
 (6 6
    (:REWRITE MILAWA::TRANSITIVITY-OF-<-THREE))
 (6 6 (:REWRITE MILAWA::TRANSITIVITY-OF-<))
 (6 6
    (:REWRITE MILAWA::LESS-WHEN-ZP-LEFT-CHEAP))
 (3 1
    (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-UNIQUE))
 (1 1
    (:REWRITE MILAWA::UNIQUEP-WHEN-UNIQUEP-OF-DOMAIN))
 (1 1
    (:REWRITE MILAWA::UNIQUEP-WHEN-NOT-CONSP))
 (1 1
    (:REWRITE MILAWA::REMOVE-DUPLICATES-WHEN-NOT-CONSP)))
