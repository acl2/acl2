...
(defun adjust-field-table-okp (i end size-diff st)
  (declare (xargs :guard (and (stp st)
                              (sb60p (mem-len st))
                              (integerp i)
                              (<= 0 i)
  ...
  (if (not (mbt (and (stp st)
      ...
      nil
    (if (equal i end)
        (and (<= 0 (+ (memi i st) size-diff))
             (< (+ (memi i st) size-diff) (mem-len st)))
      (and (<= 0 (+ (memi i st) size-diff))
           (< (+ (memi i st) size-diff) (mem-len st))
           (adjust-field-table-okp (1+ i) end size-diff st)))))

(defthm adjust-field-table-okp-!memi
  (implies (and (adjust-field-table-okp i end size-diff st)
                (integerp j)
                (<= 0 j)
                (< j (mem-len st))
                (< j i)
                (sb60p v))
           (adjust-field-table-okp i end size-diff
                                   (!memi j v st))))
...
(defun adjust-field-table (i end size-diff st)
  (declare (xargs :guard (and (stp st)
                              (sb60p (mem-len st))
                              (integerp i)
                              (<= 0 i)
                              ...
                              (<= i end)
                              (integerp size-diff)
                              (adjust-field-table-okp i end size-diff st))
                  :guard-debug t
                  :stobjs (st)
                  :measure (nfix (- end i))))
  (if (not (mbt (and (stp st)
                     ...
                     (adjust-field-table-okp i end size-diff st))))
      st
    (let* ((st (!memi i (+ (memi i st) size-diff) st)))
      (if (equal i end)
          st
        (adjust-field-table (1+ i) end size-diff st)))))


(defthm stp-adjust-field-table-okp
  (equal (stp (adjust-field-table i end size-diff st))
         (stp st)))
...
(defthm asdf
  (implies (and (stp st)
           ...
           (equal (memi j (adjust-field-table i end size-diff st))
                  (if (< j i)
                      (memi j st)
                    (if (< end j)
                        (memi j st)
                      (+ (memi j st) size-diff)))))
  :hints (("Goal" :induct (adjust-field-table i end size-diff st))))
...
(defthm asdf2
  (implies (and (stp st)
           ...
           (equal (mem-len (adjust-field-table i end size-diff st))
                  (mem-len st)))
  :hints (("Goal" :induct (adjust-field-table i end size-diff st))))


(defthm field-tablep-!memi-2
  (implies (and (field-tablep i end st)
                (integerp j)
                (<= 0 j)
                (< j i)
           ...
           (field-tablep i
                         ...
                         (!memi j v st)))
  :hints (("Goal" :in-theory (enable field-tablep))))

(defthm field-tablep-adjust-field-table
  (implies (and (field-tablep i end st)
                (stp st)
                (sb60p (mem-len st))
                (integerp i)
                ...
                (integerp size-diff)
                (adjust-field-table-okp i end size-diff st)
                (< end (+ (memi i st) size-diff))
                )
           (field-tablep i
                         end
                         (adjust-field-table i end size-diff st)))
  :hints (("Goal" :in-theory (enable field-tablep)
           :induct (adjust-field-table i end size-diff st))))



(include-book "arithmetic/top-with-meta" :dir :system)

(defthm field-tablep-adjust-field-table-2
  (implies (and (field-tablep j end st)
                (stp st)
                (sb60p (mem-len st))
                (integerp i)
                (<= 0 i)
                ...
                (<= 0 j)
                (< j i)
                (< (memi (1- i) st) (+ (memi i st) size-diff))
                )
           (field-tablep j
                         end
                         (adjust-field-table i end size-diff st)))
...
;; (defun fresize (f new-size start st)
;;   (declare (xargs :guard (and (farrayp start st)
;;                               (fieldp f start st))
;;                   :stobjs (st)))
;;   (let* ((last-pos (memi (+ start (memi start st) 1) st))
;;          (length-diff (- new-size (flength f start st)))
;;          (st (if (<= (mem-len st)
;;                      (+ last-pos length-diff))
;;                  (resize-mem (+ (mem-len st) length-diff) st)
;;                st))
;;          (st (adjust-field-table))
;;          (st (move-field-data)))
;;     st))
