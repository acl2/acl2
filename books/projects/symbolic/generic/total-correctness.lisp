...
Date: Thu Dec 16 10:22:00 2004

This book extends the idea of John Matthews to total correctness. Most of the
comments here are legacy from the partial correctness proof and most of this
book is identical with the other book on partial correctness. But I show how we
can get total correctness if we provide a measure.

Note: This is not the shortest proof of total correctness. Most of the
complications, for example with defpun are not required for total
correctness. But since I had alreadt done the partial correctness part. I say
what the heck!!
...
(encapsulate
 ...
 (local (defun pre (s) (declare (ignore s)) t))
...
;          nil))
 ...
 ;; Finally here are the theorems about cutpoint measure. These allow total
 ;; correctness.
 ...
 (defthm |cutpoint measure decreases|
   (implies (and (cutpoint s)
                 (assertion s)
                 (not (exitpoint s)))
            (o< (cutpoint-measure (nextt-cutpoint (nextt s)))
                (cutpoint-measure s))))

 ;; Note: The above two theorems just specify that if we jump from cutpoint to
 ;; cutpoint then the measure decreases. But it does not say that some cutpoint
 ;; will be reached starting from a cutpoint that is not an exitpoint. This is
 ;; because it is possible that we jump from one cutpoint to the nextt, and then
 ;; transit to a default state. So then we move from a cutpoint to possibly not
 ;; a cutpoint and then nothing is guaranteed any more. To insure against that
 ;; possibility we require now that some cutpoint is reachable if we are not at
 ;; an exitpoint.

 (defthm |some cutpoint is reachable from a cutpoint|
   (implies (and (cutpoint s)
                 (assertion s)
                 (not (exitpoint s)))
...
;; collection of events inside the encapsulate below. In the
;; encapsulate I summarize the theorems about steps-to-cutpoint.
...
;; need to know about steps-to-cutpoint-tail.
 ...
 ;; The nextt one is required for total correctness and termination. I leave it
 ;; disabled until I get around to total correctness.

 (defthmd steps-to-cutpoint-natp-implies-cutpoint
...
;; Please forgive me for this lemma. This lemma has a plethora of :use
;; hints. The reason is that this is (almost) the final one of this
;; book, and I am not spending time to clean this up. If John and
;; Daron like the book then I will clean it some time.
...
;; The theorem below is partial correctness.

(local
 (defthmd |partial correctness|
   (implies (and (pre s)
                 (exitpoint (run s n)))
...
;; with assertion to an exitpoint state.

(local
 (defun clock-fn (s)
  (declare (xargs :measure (cutpoint-measure s)
                  :hints (("Goal"
                           :in-theory (disable nextt-cutpoint)))))
  (if (or (not (cutpoint s))
          (not (assertion s))
          (exitpoint s))
      0
    (+ 1 (steps-to-cutpoint (nextt s))
       (clock-fn (nextt-cutpoint (nextt s)))))))
