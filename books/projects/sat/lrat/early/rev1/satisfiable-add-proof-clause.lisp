...
clause C that is verified for addition to F.  There are two cases,
according to the definition of verify-clause, hence two main
sub-lemmas.  First, the easier one:

Case 1: U(F,N(C)) = t (Lemma satisfiable-add-proof-clause-rup).

Proof sketch.  By unit-propagation-correct, any assignment satisfying
F also satisfies C; so we're done.
...
Assume Claim 2 for the moment.  It clearly implies that B |= F.  Also
B |= C because L is in C.  This contradicts (*), proving Case 2, Lemma
...
Claim 2.3.5.  Now U(F,RAT-NC) = t by the RAT check for D.
