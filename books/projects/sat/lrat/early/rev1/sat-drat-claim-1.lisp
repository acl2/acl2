...
; The following book includes the lemma
; evaluate-formula-fal-t-monotone
; which is useful for evaluate-formula-add-proof-clause.
(local (include-book "satisfiable-add-proof-clause-rup"))

(defthm evaluate-formula-add-proof-clause
  (implies (and (clause-or-assignment-p clause)
                (member lit assignment)
                (member lit clause)
                (integerp index)
                ...
                (equal (evaluate-formula formula assignment)
                       t))
           (equal (evaluate-formula (add-proof-clause index clause formula)
                                    assignment)
                  t))
  :hints (("Goal"
           :in-theory (enable evaluate-formula formula-p)
                            ...
                            (assignment (add-to-set
                                         (negate
                                          (non-subsetp-witness
                                           ...
                                           assignment))
                                         assignment))))
           :restrict ((evaluate-formula-add-proof-clause
