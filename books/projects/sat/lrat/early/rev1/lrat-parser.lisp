...
(defun parse-cnf-channel-formula (channel max fal ctx state)
...
; Warning: This function is responsible for closing the given input channel.
  ...
  (b* (((er val)
       ...
       ((when (eq val :eof))
        (pprogn
         (close-input-channel channel state)
         (value (make-formula max fal))))
       (next (1+ max)))
    (parse-cnf-channel-formula channel next
                               (hons-acons next (reverse val) fal)
                               ctx state)))
...
(defun verify-lrat-proof (cnf-file lrat-file state)
  (b* (((er formula) (parse-cnf-file cnf-file state))
       ((er proof) (parse-lrat-file lrat-file state)))
    (value (valid-proofp formula proof))))
   ...
   (t (let* ((d (car doublets))
        ...
        (pprogn
         (fms "Starting ~x0.~|"
              ...
              chan state nil)
         (er-let* ((val (time$ (verify-lrat-proof cnf lrat state))))
           (pprogn
