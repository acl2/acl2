...
; We will locally include system/apply/apply and then explicitly identify some
; basic lemmas from it that we believe are generally useful to user-level
; proofs involving apply$.  A lot of these lemmas are redundant during the
; certification of this book but will be available to the user of this book.
; Recall that system/apply/apply is used during the build of ACL2 itself, to
; support the sources' claims that the functions terminate and are guard
; verified.  Many of the redundant lemmas mentioned below are critical to those
; proofs -- so they have to be in system/apply/apply -- but are tedious to
; prove so we don't want to prove them again from scratch here.
...
(defthm apply$-prim-takes-arity-args
  (implies (apply$-primp fn)
           (equal (apply$-prim fn args)
                  (apply$-prim fn (take (caddr (badge-prim fn)) args))))
