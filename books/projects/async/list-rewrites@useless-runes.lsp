(ADE::LIST-REWRITE-1 (45 45 (:REWRITE DEFAULT-CDR))
                     (37 19 (:REWRITE DEFAULT-+-2))
                     (36 36 (:LINEAR LEN-WHEN-PREFIXP))
                     (21 21 (:REWRITE DEFAULT-CAR))
                     (19 19 (:REWRITE DEFAULT-+-1))
                     (18 18
                         (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS)))
(ADE::LIST-REWRITE-2 (116 116 (:REWRITE DEFAULT-CDR))
                     (77 39 (:REWRITE DEFAULT-+-2))
                     (68 68 (:REWRITE DEFAULT-CAR))
                     (42 42 (:LINEAR LEN-WHEN-PREFIXP))
                     (39 39 (:REWRITE DEFAULT-+-1))
                     (21 21
                         (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS)))
(ADE::LIST-REWRITE-4 (465 465 (:REWRITE DEFAULT-CDR))
                     (202 202 (:REWRITE DEFAULT-CAR))
                     (152 77 (:REWRITE DEFAULT-+-2))
                     (77 77 (:REWRITE DEFAULT-+-1))
                     (50 50 (:LINEAR LEN-WHEN-PREFIXP))
                     (25 25
                         (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS)))
(ADE::LIST-REWRITE-5 (700 700 (:REWRITE DEFAULT-CDR))
                     (270 270 (:REWRITE DEFAULT-CAR))
                     (182 93 (:REWRITE DEFAULT-+-2))
                     (93 93 (:REWRITE DEFAULT-+-1))
                     (52 52 (:LINEAR LEN-WHEN-PREFIXP))
                     (26 26
                         (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS)))
(ADE::LIST-REWRITE-10
     (2610 2610 (:REWRITE DEFAULT-CDR))
     (650 650 (:REWRITE DEFAULT-CAR))
     (367 198 (:REWRITE DEFAULT-+-2))
     (198 198 (:REWRITE DEFAULT-+-1))
     (52 52 (:LINEAR LEN-WHEN-PREFIXP))
     (26 26
         (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS)))
(ADE::LIST-AS-COLLECTED-NTH)
(ADE::OPEN-LIST-AS-COLLECTED-NTH
     (88 7 (:DEFINITION NTH))
     (32 21 (:REWRITE DEFAULT-+-2))
     (29 11 (:REWRITE ZP-OPEN))
     (28 6 (:REWRITE FOLD-CONSTS-IN-+))
     (21 21 (:REWRITE DEFAULT-+-1))
     (15 15
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (9 9 (:REWRITE DEFAULT-CDR))
     (9 9 (:REWRITE DEFAULT-CAR))
     (8 8 (:REWRITE DEFAULT-<-2))
     (8 8 (:REWRITE DEFAULT-<-1))
     (7 7 (:REWRITE NTH-WHEN-PREFIXP))
     (6 2 (:REWRITE UNICITY-OF-0))
     (4 2 (:DEFINITION FIX)))
(ADE::EQUAL-LEN-4-AS-COLLECTED-NTH
     (136 136 (:REWRITE DEFAULT-CDR))
     (64 64 (:REWRITE DEFAULT-CAR))
     (44 22 (:REWRITE DEFAULT-+-2))
     (22 22 (:REWRITE DEFAULT-+-1))
     (16 16 (:LINEAR LEN-WHEN-PREFIXP))
     (8 8
        (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS)))
(ADE::EQUAL-LEN-32-AS-COLLECTED-NTH
     (6898 6898 (:REWRITE DEFAULT-CDR))
     (1464 1464 (:REWRITE DEFAULT-CAR))
     (212 106 (:REWRITE DEFAULT-+-2))
     (106 106 (:REWRITE DEFAULT-+-1))
     (16 16 (:LINEAR LEN-WHEN-PREFIXP))
     (8 8
        (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS)))
