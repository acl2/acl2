(EQUAL-OF-APPENDS-WHEN-TRUE-LISTPS
     (12 6
         (:TYPE-PRESCRIPTION TRUE-LISTP-APPEND))
     (6 6 (:TYPE-PRESCRIPTION BINARY-APPEND))
     (4 2 (:DEFINITION TRUE-LISTP))
     (2 2 (:REWRITE DEFAULT-CDR)))
(ADE::STR-APPEND-SYMBOL-UNDERSCORE)
(ADE::APPEND-SYMBOL-STRING)
(ADE::SI)
(ADE::SYMBOL-INEQUALITY)
(ADE::NOT-IPREFIXP-APPEND
     (1011 23
           (:REWRITE PREFIXP-WHEN-EQUAL-LENGTHS))
     (500 70 (:DEFINITION LEN))
     (397 397 (:TYPE-PRESCRIPTION LEN))
     (240 12 (:REWRITE LEN-OF-APPEND))
     (211 103 (:REWRITE DEFAULT-+-2))
     (198 3 (:REWRITE PREFIXP-OF-CONS-LEFT))
     (160 80
          (:TYPE-PRESCRIPTION TRUE-LISTP-APPEND))
     (136 93 (:REWRITE DEFAULT-CDR))
     (127 103 (:REWRITE DEFAULT-+-1))
     (102 102 (:LINEAR LEN-WHEN-PREFIXP))
     (80 80 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (80 80 (:TYPE-PRESCRIPTION BINARY-APPEND))
     (78 14
         (:REWRITE STR::IPREFIXP-WHEN-NOT-CONSP-LEFT))
     (60 12 (:REWRITE COMMUTATIVITY-OF-+))
     (51 51
         (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS))
     (33 33 (:REWRITE DEFAULT-CAR))
     (29 23
         (:REWRITE PREFIXP-WHEN-NOT-CONSP-RIGHT))
     (23 23 (:REWRITE PREFIXP-TRANSITIVE . 2))
     (23 23 (:REWRITE PREFIXP-TRANSITIVE . 1))
     (23 23
         (:REWRITE PREFIXP-ONE-WAY-OR-ANOTHER . 2))
     (23 23
         (:REWRITE PREFIXP-ONE-WAY-OR-ANOTHER . 1))
     (9 9 (:REWRITE APPEND-WHEN-NOT-CONSP))
     (9 9
        (:REWRITE APPEND-ATOM-UNDER-LIST-EQUIV))
     (5 5 (:REWRITE FOLD-CONSTS-IN-+))
     (5 5 (:REWRITE CDR-CONS))
     (4 4 (:REWRITE CDR-OF-APPEND-WHEN-CONSP)))
(ADE::NOT-ISTRPREFIXP-STR-APPEND-SYMBOL-UNDERSCORE
     (820 9 (:REWRITE STR::IPREFIXP-WHEN-PREFIXP))
     (658 9 (:REWRITE PREFIXP-WHEN-EQUAL-LENGTHS))
     (654 33 (:DEFINITION BINARY-APPEND))
     (417 165 (:REWRITE STR::CONSP-OF-EXPLODE))
     (374 22 (:DEFINITION LEN))
     (343 11 (:REWRITE LEN-OF-APPEND))
     (254 57 (:REWRITE DEFAULT-CDR))
     (158 158 (:TYPE-PRESCRIPTION LEN))
     (158 35 (:REWRITE DEFAULT-CAR))
     (119 91 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (87 47 (:REWRITE DEFAULT-+-2))
     (86 64
         (:REWRITE APPEND-ATOM-UNDER-LIST-EQUIV))
     (59 47 (:REWRITE DEFAULT-+-1))
     (57 9
         (:REWRITE PREFIXP-WHEN-NOT-CONSP-RIGHT))
     (56 56 (:LINEAR LEN-WHEN-PREFIXP))
     (55 47 (:REWRITE DEFAULT-SYMBOL-NAME))
     (49 49
         (:TYPE-PRESCRIPTION STR::STRINGP-OF-NATSTR))
     (49 9
         (:REWRITE STR::IPREFIXP-WHEN-NOT-CONSP-RIGHT))
     (42 9
         (:REWRITE PREFIXP-WHEN-NOT-CONSP-LEFT))
     (42 9
         (:REWRITE STR::IPREFIXP-WHEN-NOT-CONSP-LEFT))
     (41 10 (:REWRITE COMMUTATIVITY-OF-+))
     (28 28
         (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS))
     (28 23
         (:REWRITE STR::EXPLODE-WHEN-NOT-STRINGP))
     (25 2 (:REWRITE COMMUTATIVITY-2-OF-+))
     (21 21 (:REWRITE STR::NATSTR-NONEMPTY))
     (20 9
         (:REWRITE STR::DIGIT-LISTP-WHEN-NOT-CONSP))
     (18 18 (:TYPE-PRESCRIPTION PREFIXP))
     (12 6 (:REWRITE FOLD-CONSTS-IN-+))
     (12 3
         (:REWRITE INTERN-IN-PACKAGE-OF-SYMBOL-IS-IDENTITY))
     (9 9 (:REWRITE PREFIXP-TRANSITIVE . 2))
     (9 9 (:REWRITE PREFIXP-TRANSITIVE . 1))
     (9 9
        (:REWRITE PREFIXP-ONE-WAY-OR-ANOTHER . 2))
     (9 9
        (:REWRITE PREFIXP-ONE-WAY-OR-ANOTHER . 1))
     (6 6
        (:TYPE-PRESCRIPTION MEMBER-SYMBOL-NAME))
     (6 3
        (:REWRITE DEFAULT-INTERN-IN-PACKAGE-OF-SYMBOL))
     (4 4 (:REWRITE DEFAULT-<-2))
     (4 4 (:REWRITE DEFAULT-<-1))
     (4 2 (:REWRITE STR::DIGIT-LISTP-OF-APPEND))
     (3 3
        (:TYPE-PRESCRIPTION SYMBOLP-INTERN-IN-PACKAGE-OF-SYMBOL))
     (3 1 (:DEFINITION NATP))
     (2 2 (:REWRITE STR::DIGIT-LISTP-OF-CONS))
     (1 1 (:TYPE-PRESCRIPTION NATP)))
(ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
     (865 175 (:REWRITE STR::CONSP-OF-EXPLODE))
     (480 20 (:DEFINITION BINARY-APPEND))
     (469 469
          (:TYPE-PRESCRIPTION ADE::STR-APPEND-SYMBOL-UNDERSCORE))
     (464 28 (:DEFINITION LEN))
     (266 40 (:REWRITE APPEND-WHEN-NOT-CONSP))
     (239 48 (:REWRITE DEFAULT-CDR))
     (222 117 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (190 6 (:REWRITE LEN-OF-APPEND))
     (179 179 (:TYPE-PRESCRIPTION LEN))
     (129 80 (:REWRITE DEFAULT-SYMBOL-NAME))
     (100 20 (:REWRITE DEFAULT-CAR))
     (91 7
         (:REWRITE PREFIXP-WHEN-NOT-CONSP-LEFT))
     (85 40
         (:REWRITE APPEND-ATOM-UNDER-LIST-EQUIV))
     (77 7
         (:REWRITE STR::IPREFIXP-WHEN-NOT-CONSP-LEFT))
     (76 40 (:REWRITE DEFAULT-+-2))
     (73 40
         (:REWRITE STR::EXPLODE-WHEN-NOT-STRINGP))
     (66 6 (:REWRITE LIST-EQUIV-WHEN-ATOM-RIGHT))
     (66 6 (:REWRITE LIST-EQUIV-WHEN-ATOM-LEFT))
     (49 7
         (:REWRITE PREFIXP-WHEN-NOT-CONSP-RIGHT))
     (48 40 (:REWRITE DEFAULT-+-1))
     (46 46 (:LINEAR LEN-WHEN-PREFIXP))
     (42 7
         (:REWRITE STR::IPREFIXP-WHEN-NOT-CONSP-RIGHT))
     (29 29
         (:TYPE-PRESCRIPTION STR::STRINGP-OF-NATSTR))
     (26 6 (:REWRITE COMMUTATIVITY-OF-+))
     (23 23
         (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS))
     (20 5
         (:REWRITE INTERN-IN-PACKAGE-OF-SYMBOL-IS-IDENTITY))
     (19 19 (:REWRITE PREFIXP-TRANSITIVE . 2))
     (19 19 (:REWRITE PREFIXP-TRANSITIVE . 1))
     (19 19
         (:REWRITE PREFIXP-ONE-WAY-OR-ANOTHER . 2))
     (19 19
         (:REWRITE PREFIXP-ONE-WAY-OR-ANOTHER . 1))
     (12 6
         (:REWRITE STR::DIGIT-LISTP-WHEN-NOT-CONSP))
     (10 10
         (:TYPE-PRESCRIPTION MEMBER-SYMBOL-NAME))
     (10 8 (:REWRITE DEFAULT-<-1))
     (10 5
         (:REWRITE DEFAULT-INTERN-IN-PACKAGE-OF-SYMBOL))
     (10 2 (:REWRITE ADE::NFIX-OF-NAT))
     (9 9 (:REWRITE STR::NATSTR-NONEMPTY))
     (8 8 (:REWRITE DEFAULT-<-2))
     (6 2 (:DEFINITION NATP))
     (2 2 (:TYPE-PRESCRIPTION NATP))
     (2 2
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(ADE::NOT-MEMBER-UNDERSCORE-NATSTR
     (4244 1486 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (2758 1440 (:REWRITE DEFAULT-SYMBOL-NAME))
     (1935 49 (:DEFINITION MEMBER-EQUAL))
     (1897 143
           (:REWRITE STR::BASIC-NATCHARS-WHEN-ZP))
     (1345 859 (:REWRITE DEFAULT-<-2))
     (1290 72 (:REWRITE REV-WHEN-NOT-CONSP))
     (1247 35 (:DEFINITION BINARY-APPEND))
     (1080 768 (:REWRITE DEFAULT-+-2))
     (859 859 (:REWRITE DEFAULT-<-1))
     (845 743 (:REWRITE DEFAULT-*-2))
     (839 70 (:REWRITE APPEND-WHEN-NOT-CONSP))
     (815 743 (:REWRITE DEFAULT-*-1))
     (768 768 (:REWRITE DEFAULT-+-1))
     (535 88 (:REWRITE DEFAULT-CAR))
     (532 85 (:REWRITE DEFAULT-CDR))
     (446 344 (:REWRITE DEFAULT-UNARY-MINUS))
     (274 274 (:REWRITE DEFAULT-NUMERATOR))
     (261 261 (:REWRITE DEFAULT-DENOMINATOR))
     (216 3 (:REWRITE ADE::MEMBER-APPEND))
     (70 70
         (:REWRITE APPEND-ATOM-UNDER-LIST-EQUIV))
     (42 3 (:REWRITE CAR-OF-APPEND))
     (27 3 (:REWRITE CDR-OF-APPEND-WHEN-CONSP))
     (14 1 (:DEFINITION CHARACTER-LISTP))
     (12 6
         (:TYPE-PRESCRIPTION TRUE-LISTP-APPEND))
     (11 2
         (:REWRITE STR::CHARACTER-LISTP-WHEN-DIGIT-LISTP))
     (10 10
         (:TYPE-PRESCRIPTION MAKE-CHARACTER-LIST))
     (6 6 (:TYPE-PRESCRIPTION BINARY-APPEND))
     (6 6 (:REWRITE REV-UNDER-IFF))
     (6 6 (:REWRITE CAR-CONS))
     (5 5 (:TYPE-PRESCRIPTION STR::DIGIT-LISTP))
     (4 3
        (:REWRITE STR::CONSP-OF-MAKE-CHARACTER-LIST))
     (4 1
        (:REWRITE STR::MAKE-CHARACTER-LIST-WHEN-ATOM))
     (3 3 (:REWRITE CDR-CONS))
     (3 2
        (:REWRITE STR::DIGIT-LISTP-WHEN-NOT-CONSP))
     (2 1
        (:REWRITE STR::DIGIT-LISTP-OF-CDR-WHEN-DIGIT-LISTP)))
(ADE::SI-OF-DIFF-SYMBOLS-AUX-1
     (933 14 (:DEFINITION MEMBER-EQUAL))
     (590 12
          (:REWRITE ADE::DISJOINT=>NOT-MEMBER-NTH))
     (510 16 (:REWRITE ADE::SUBSETP=>MEMBER-NTH))
     (476 28 (:REWRITE ADE::DISJOINT-ATOM))
     (441 14 (:DEFINITION SUBSETP-EQUAL))
     (384 35 (:DEFINITION LEN))
     (374 181 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (355 30 (:REWRITE ADE::DISJOINT-COMMUTATIVE))
     (332 82 (:REWRITE DEFAULT-CDR))
     (302 54 (:DEFINITION ATOM))
     (197 50 (:REWRITE DEFAULT-CAR))
     (193 136 (:REWRITE DEFAULT-SYMBOL-NAME))
     (175 2 (:REWRITE ADE::MEMBER-APPEND))
     (134 74 (:REWRITE DEFAULT-+-2))
     (120 5 (:DEFINITION BINARY-APPEND))
     (120 2 (:REWRITE ADE::DISJOINT-APPEND))
     (101 1 (:REWRITE ADE::NOT-MEMBER-APPEND))
     (75 74 (:REWRITE DEFAULT-+-1))
     (70 70 (:TYPE-PRESCRIPTION ADE::DISJOINT))
     (63 63 (:TYPE-PRESCRIPTION SUBSETP-EQUAL))
     (62 1 (:REWRITE LEN-WHEN-PREFIXP))
     (58 10
         (:REWRITE APPEND-ATOM-UNDER-LIST-EQUIV))
     (48 4 (:REWRITE CONSP-OF-APPEND))
     (43 10 (:REWRITE APPEND-WHEN-NOT-CONSP))
     (41 1 (:REWRITE PREFIXP-WHEN-EQUAL-LENGTHS))
     (36 21 (:REWRITE DEFAULT-<-2))
     (32 2 (:REWRITE CAR-OF-APPEND))
     (29 29 (:REWRITE ADE::SUBSETP-TRANSITIVITY))
     (25 21 (:REWRITE DEFAULT-<-1))
     (24 24 (:TYPE-PRESCRIPTION ATOM))
     (21 21 (:REWRITE NTH-WHEN-PREFIXP))
     (20 4
         (:REWRITE STR::CHARACTER-LISTP-WHEN-DIGIT-LISTP))
     (15 12 (:REWRITE STR::EXPLODE-UNDER-IFF))
     (14 2 (:REWRITE CDR-OF-APPEND-WHEN-CONSP))
     (12 12 (:LINEAR LEN-WHEN-PREFIXP))
     (12 1 (:REWRITE APPEND-UNDER-IFF))
     (8 8 (:TYPE-PRESCRIPTION STR::DIGIT-LISTP))
     (8 4
        (:REWRITE STR::DIGIT-LISTP-WHEN-NOT-CONSP))
     (7 1
        (:REWRITE PREFIXP-WHEN-NOT-CONSP-RIGHT))
     (7 1
        (:REWRITE PREFIXP-WHEN-NOT-CONSP-LEFT))
     (6 6
        (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS))
     (3 3 (:REWRITE ADE::SUBSETP-IDENTITY))
     (2 2 (:TYPE-PRESCRIPTION PREFIXP))
     (2 1 (:REWRITE DEFAULT-UNARY-MINUS))
     (1 1 (:REWRITE PREFIXP-TRANSITIVE . 2))
     (1 1 (:REWRITE PREFIXP-TRANSITIVE . 1))
     (1 1
        (:REWRITE PREFIXP-ONE-WAY-OR-ANOTHER . 2))
     (1 1
        (:REWRITE PREFIXP-ONE-WAY-OR-ANOTHER . 1)))
(ADE::SI-OF-DIFF-SYMBOLS-AUX-2)
(ADE::SI-OF-DIFF-SYMBOLS-AUX-3
     (33102 1826 (:DEFINITION BINARY-APPEND))
     (12807 3265 (:REWRITE DEFAULT-CDR))
     (11358 916 (:DEFINITION LEN))
     (9234 2440 (:REWRITE DEFAULT-CAR))
     (9062 5122 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (5540 3118 (:REWRITE DEFAULT-SYMBOL-NAME))
     (5072 77 (:REWRITE LEN-WHEN-PREFIXP))
     (4120 3652
           (:REWRITE APPEND-ATOM-UNDER-LIST-EQUIV))
     (3537 77
           (:REWRITE PREFIXP-WHEN-EQUAL-LENGTHS))
     (3024 77 (:DEFINITION NTH))
     (2457 351 (:REWRITE CDR-OF-APPEND-WHEN-CONSP))
     (2396 77 (:REWRITE ADE::NTH-APPEND-<-LEN))
     (2291 1204 (:REWRITE DEFAULT-+-2))
     (2140 362
           (:REWRITE STR::DIGIT-LISTP-OF-APPEND))
     (1964 982
           (:REWRITE STR::DIGIT-LISTP-WHEN-NOT-CONSP))
     (1345 1204 (:REWRITE DEFAULT-+-1))
     (1300 1123
           (:REWRITE STR::EXPLODE-WHEN-NOT-STRINGP))
     (1000 1000 (:LINEAR LEN-WHEN-PREFIXP))
     (872 218
          (:REWRITE INTERN-IN-PACKAGE-OF-SYMBOL-IS-IDENTITY))
     (714 473 (:REWRITE DEFAULT-<-2))
     (639 473 (:REWRITE DEFAULT-<-1))
     (616 77 (:REWRITE ZP-OPEN))
     (534 77
          (:REWRITE PREFIXP-WHEN-NOT-CONSP-RIGHT))
     (500 500
          (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS))
     (462 77
          (:REWRITE PREFIXP-WHEN-NOT-CONSP-LEFT))
     (436 436
          (:TYPE-PRESCRIPTION MEMBER-SYMBOL-NAME))
     (436 218
          (:REWRITE DEFAULT-INTERN-IN-PACKAGE-OF-SYMBOL))
     (228 228 (:REWRITE STR::DIGIT-LISTP-OF-CONS))
     (218 218
          (:TYPE-PRESCRIPTION SYMBOLP-INTERN-IN-PACKAGE-OF-SYMBOL))
     (202 101 (:REWRITE DEFAULT-UNARY-MINUS))
     (177 177
          (:TYPE-PRESCRIPTION STR::STRINGP-OF-NATSTR))
     (154 154 (:TYPE-PRESCRIPTION PREFIXP))
     (104 104 (:REWRITE NTH-WHEN-PREFIXP))
     (77 77 (:REWRITE PREFIXP-TRANSITIVE . 2))
     (77 77 (:REWRITE PREFIXP-TRANSITIVE . 1))
     (77 77
         (:REWRITE PREFIXP-ONE-WAY-OR-ANOTHER . 2))
     (77 77
         (:REWRITE PREFIXP-ONE-WAY-OR-ANOTHER . 1))
     (60 20 (:DEFINITION NATP))
     (20 20 (:TYPE-PRESCRIPTION NATP))
     (2 2
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(ADE::DIFF-ELEMENTP)
(ADE::NOT-EQUAL-EQUIV-DIFF-ELEMENTP (69 33 (:REWRITE ADE::SYMBOL-INEQUALITY))
                                    (43 43 (:REWRITE DEFAULT-CDR))
                                    (36 34 (:REWRITE DEFAULT-SYMBOL-NAME))
                                    (28 28 (:REWRITE DEFAULT-CAR)))
(ADE::DIFF-ELEMENTP-APPEND
     (104 52
          (:TYPE-PRESCRIPTION TRUE-LISTP-APPEND))
     (92 24 (:REWRITE DEFAULT-CDR))
     (88 32 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (82 52 (:REWRITE DEFAULT-CAR))
     (56 40 (:REWRITE DEFAULT-SYMBOL-NAME))
     (52 52 (:TYPE-PRESCRIPTION TRUE-LISTP))
     (52 52 (:TYPE-PRESCRIPTION BINARY-APPEND))
     (38 19 (:REWRITE DEFAULT-+-2))
     (30 30 (:LINEAR LEN-WHEN-PREFIXP))
     (24 8 (:REWRITE CAR-OF-APPEND))
     (20 20 (:REWRITE CONSP-OF-APPEND))
     (19 19 (:REWRITE DEFAULT-+-1))
     (18 18 (:REWRITE APPEND-WHEN-NOT-CONSP))
     (15 15
         (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS))
     (8 8 (:REWRITE CDR-OF-APPEND-WHEN-CONSP)))
(ADE::SI-OF-DIFF-SYMBOLS-AUX-4
     (1978 869 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (1724 430 (:REWRITE DEFAULT-CDR))
     (1507 868 (:REWRITE DEFAULT-SYMBOL-NAME))
     (1378 106 (:DEFINITION LEN))
     (1332 356 (:REWRITE DEFAULT-CAR))
     (560 92 (:REWRITE CDR-OF-APPEND-WHEN-CONSP))
     (338 296
          (:REWRITE STR::EXPLODE-WHEN-NOT-STRINGP))
     (240 216
          (:REWRITE APPEND-ATOM-UNDER-LIST-EQUIV))
     (212 106 (:REWRITE DEFAULT-+-2))
     (200 34
          (:REWRITE STR::DIGIT-LISTP-OF-APPEND))
     (180 90
          (:REWRITE STR::DIGIT-LISTP-WHEN-NOT-CONSP))
     (141 36
          (:REWRITE INTERN-IN-PACKAGE-OF-SYMBOL-IS-IDENTITY))
     (106 106 (:REWRITE DEFAULT-+-1))
     (72 36
         (:REWRITE DEFAULT-INTERN-IN-PACKAGE-OF-SYMBOL))
     (70 70
         (:TYPE-PRESCRIPTION MEMBER-SYMBOL-NAME))
     (42 42
         (:TYPE-PRESCRIPTION STR::STRINGP-OF-NATSTR))
     (42 40 (:REWRITE DEFAULT-<-1))
     (40 40 (:REWRITE DEFAULT-<-2))
     (36 36
         (:TYPE-PRESCRIPTION SYMBOLP-INTERN-IN-PACKAGE-OF-SYMBOL))
     (20 20 (:REWRITE STR::DIGIT-LISTP-OF-CONS))
     (20 20 (:LINEAR LEN-WHEN-PREFIXP))
     (10 10
         (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS))
     (6 2 (:DEFINITION NATP))
     (2 2 (:TYPE-PRESCRIPTION NATP))
     (2 2
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(ADE::SI-OF-DIFF-SYMBOLS (64 50 (:REWRITE DEFAULT-SYMBOL-NAME))
                         (56 14 (:REWRITE DEFAULT-CDR))
                         (28 14 (:REWRITE DEFAULT-+-2))
                         (14 14
                             (:REWRITE STR::EXPLODE-WHEN-NOT-STRINGP))
                         (14 14 (:REWRITE DEFAULT-+-1))
                         (10 2
                             (:REWRITE ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
                                       . 2))
                         (4 4
                            (:TYPE-PRESCRIPTION STR::ISTRPREFIXP$INLINE)))
(ADE::APPEND-SYMBOL-DIFF-STRINGS
     (58 2 (:DEFINITION BINARY-APPEND))
     (57 13 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (44 11 (:REWRITE DEFAULT-SYMBOL-NAME))
     (44 11 (:REWRITE STR::CONSP-OF-EXPLODE))
     (36 6
         (:REWRITE STR::CHARACTER-LISTP-WHEN-DIGIT-LISTP))
     (26 4
         (:REWRITE APPEND-ATOM-UNDER-LIST-EQUIV))
     (22 4 (:REWRITE APPEND-WHEN-NOT-CONSP))
     (20 20
         (:TYPE-PRESCRIPTION ADE::STR-APPEND-SYMBOL-UNDERSCORE))
     (16 2 (:REWRITE STR::DIGIT-LISTP-OF-APPEND))
     (14 14
         (:TYPE-PRESCRIPTION STR::DIGIT-LISTP))
     (14 7
         (:REWRITE STR::DIGIT-LISTP-WHEN-NOT-CONSP))
     (10 2 (:REWRITE DEFAULT-CDR))
     (10 2 (:REWRITE DEFAULT-CAR))
     (6 4
        (:REWRITE STR::EXPLODE-WHEN-NOT-STRINGP)))
(ADE::SI-OF-DIFF-IDXES
     (14 6 (:REWRITE DEFAULT-SYMBOL-NAME))
     (10 4
         (:REWRITE INTERN-IN-PACKAGE-OF-SYMBOL-IS-IDENTITY))
     (8 4
        (:REWRITE DEFAULT-INTERN-IN-PACKAGE-OF-SYMBOL))
     (4 4
        (:TYPE-PRESCRIPTION MEMBER-SYMBOL-NAME))
     (2 2 (:REWRITE DEFAULT-<-2))
     (2 2 (:REWRITE DEFAULT-<-1)))
(ADE::SIS)
(ADE::CONSP-SIS (9 6 (:REWRITE DEFAULT-+-2))
                (7 2 (:REWRITE FOLD-CONSTS-IN-+))
                (6 6 (:REWRITE DEFAULT-+-1))
                (4 1 (:REWRITE ZP-OPEN))
                (3 3
                   (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                (2 2 (:REWRITE DEFAULT-<-2))
                (2 2 (:REWRITE DEFAULT-<-1)))
(ADE::NOT-MEMBER-WITH-SIS-OF-DIFF-SYMBOL
     (12 6 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (10 10 (:REWRITE DEFAULT-SYMBOL-NAME))
     (7 4 (:REWRITE DEFAULT-CDR))
     (7 4 (:REWRITE DEFAULT-CAR))
     (6 4 (:REWRITE DEFAULT-+-2))
     (4 4 (:REWRITE ZP-OPEN))
     (4 4 (:REWRITE DEFAULT-+-1))
     (2 2
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(ADE::SI-SIS-DIFF-SYMBOLS
     (36 29 (:REWRITE DEFAULT-SYMBOL-NAME))
     (30 10 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (20 4
         (:REWRITE ADE::NOT-MEMBER-WITH-SIS-OF-DIFF-SYMBOL))
     (14 14
         (:TYPE-PRESCRIPTION STR::ISTRPREFIXP$INLINE))
     (12 3
         (:REWRITE ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
                   . 2))
     (7 4 (:REWRITE DEFAULT-CDR))
     (7 4 (:REWRITE DEFAULT-CAR))
     (6 4 (:REWRITE DEFAULT-+-2))
     (4 4 (:REWRITE ZP-OPEN))
     (4 4 (:REWRITE DEFAULT-+-1))
     (3 3
        (:REWRITE ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
                  . 1))
     (2 2
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(ADE::SI-SIS-DIFF-IDXES
     (20 13 (:REWRITE DEFAULT-SYMBOL-NAME))
     (20 4
         (:REWRITE ADE::NOT-MEMBER-WITH-SIS-OF-DIFF-SYMBOL))
     (18 6 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (14 14
         (:TYPE-PRESCRIPTION STR::ISTRPREFIXP$INLINE))
     (12 12 (:REWRITE DEFAULT-<-2))
     (12 12 (:REWRITE DEFAULT-<-1))
     (12 3
         (:REWRITE ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
                   . 2))
     (7 4 (:REWRITE DEFAULT-CDR))
     (7 4 (:REWRITE DEFAULT-CAR))
     (4 4 (:REWRITE ZP-OPEN))
     (4 4 (:REWRITE ADE::SI-SIS-DIFF-SYMBOLS))
     (4 4 (:REWRITE DEFAULT-+-2))
     (4 4 (:REWRITE DEFAULT-+-1))
     (3 3
        (:REWRITE ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
                  . 1)))
(ADE::LEN-SIS (26 26 (:LINEAR LEN-WHEN-PREFIXP))
              (19 12 (:REWRITE DEFAULT-+-2))
              (16 4 (:REWRITE ADE::SYMBOL-INEQUALITY))
              (13 13
                  (:LINEAR ADE::A-HELPFUL-LEMMA-FOR-TREE-INDUCTIONS))
              (12 12 (:REWRITE DEFAULT-+-1))
              (12 8 (:REWRITE DEFAULT-SYMBOL-NAME))
              (8 5 (:REWRITE DEFAULT-CDR))
              (7 6 (:REWRITE DEFAULT-<-1))
              (6 6 (:REWRITE DEFAULT-<-2))
              (5 1 (:REWRITE ADE::NFIX-OF-NAT))
              (4 4 (:REWRITE ZP-OPEN))
              (4 4
                 (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
              (3 1 (:DEFINITION NATP))
              (1 1 (:TYPE-PRESCRIPTION NATP)))
(ADE::NO-DUPLICATESP-SIS (21 3 (:DEFINITION MEMBER-EQUAL))
                         (17 11 (:REWRITE DEFAULT-CDR))
                         (12 6 (:REWRITE ADE::SYMBOL-INEQUALITY))
                         (10 7 (:REWRITE DEFAULT-CAR))
                         (6 6 (:REWRITE DEFAULT-SYMBOL-NAME))
                         (4 4 (:REWRITE ZP-OPEN))
                         (4 4 (:REWRITE DEFAULT-<-2))
                         (4 4 (:REWRITE DEFAULT-<-1))
                         (4 4 (:REWRITE DEFAULT-+-2))
                         (4 4 (:REWRITE DEFAULT-+-1))
                         (3 3 (:TYPE-PRESCRIPTION MEMBER-EQUAL)))
(ADE::SIS-OF-SINGLETON (4 2 (:REWRITE DEFAULT-+-2))
                       (2 2
                          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                       (2 2 (:REWRITE DEFAULT-+-1)))
(ADE::DISJOINT-SIS-DIFF-SYMS (134 67 (:TYPE-PRESCRIPTION ADE::CONSP-SIS))
                             (120 12 (:DEFINITION ATOM))
                             (67 67 (:TYPE-PRESCRIPTION POSP))
                             (21 14 (:REWRITE DEFAULT-+-2))
                             (16 16 (:REWRITE DEFAULT-SYMBOL-NAME))
                             (14 14 (:REWRITE DEFAULT-+-1))
                             (12 4 (:REWRITE ADE::SYMBOL-INEQUALITY))
                             (9 9 (:REWRITE ZP-OPEN))
                             (7 7
                                (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                             (6 6 (:TYPE-PRESCRIPTION ATOM)))
(ADE::DISJOINT-SIS-SAME-SYM-1 (134 67 (:TYPE-PRESCRIPTION ADE::CONSP-SIS))
                              (120 12 (:DEFINITION ATOM))
                              (67 67 (:TYPE-PRESCRIPTION POSP))
                              (21 18 (:REWRITE DEFAULT-+-2))
                              (18 18 (:REWRITE DEFAULT-+-1))
                              (12 12 (:REWRITE DEFAULT-<-2))
                              (12 12 (:REWRITE DEFAULT-<-1))
                              (9 9 (:REWRITE ZP-OPEN))
                              (7 7
                                 (:REWRITE ADE::DISJOINT-SIS-DIFF-SYMS))
                              (6 6 (:TYPE-PRESCRIPTION ATOM))
                              (3 3
                                 (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(ADE::DISJOINT-SIS-SAME-SYM-2 (12 2 (:DEFINITION ADE::SIS))
                              (8 6 (:REWRITE DEFAULT-+-2))
                              (6 6 (:REWRITE DEFAULT-+-1))
                              (4 4 (:REWRITE DEFAULT-<-2))
                              (4 4 (:REWRITE DEFAULT-<-1))
                              (2 2 (:REWRITE ZP-OPEN))
                              (2 2
                                 (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                              (1 1
                                 (:REWRITE ADE::DISJOINT-SIS-DIFF-SYMS)))
(ADE::SI-MEMBER-SIS (28 9 (:REWRITE ADE::SYMBOL-INEQUALITY))
                    (26 16 (:REWRITE DEFAULT-SYMBOL-NAME))
                    (23 23 (:REWRITE DEFAULT-<-2))
                    (23 23 (:REWRITE DEFAULT-<-1))
                    (20 4
                        (:REWRITE ADE::NOT-MEMBER-WITH-SIS-OF-DIFF-SYMBOL))
                    (17 4
                        (:REWRITE ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
                                  . 2))
                    (16 16
                        (:TYPE-PRESCRIPTION STR::ISTRPREFIXP$INLINE))
                    (15 15 (:REWRITE DEFAULT-+-2))
                    (15 15 (:REWRITE DEFAULT-+-1))
                    (11 5 (:REWRITE DEFAULT-CAR))
                    (8 5 (:REWRITE ZP-OPEN))
                    (8 4 (:REWRITE ADE::SI-SIS-DIFF-IDXES))
                    (7 4 (:REWRITE DEFAULT-CDR))
                    (6 2 (:REWRITE FOLD-CONSTS-IN-+))
                    (4 4 (:REWRITE ADE::SI-SIS-DIFF-SYMBOLS))
                    (4 4
                       (:REWRITE ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
                                 . 1))
                    (2 1 (:REWRITE ADE::SI-OF-DIFF-IDXES . 2))
                    (1 1
                       (:REWRITE ADE::SI-OF-DIFF-SYMBOLS . 2))
                    (1 1
                       (:REWRITE ADE::SI-OF-DIFF-SYMBOLS . 1))
                    (1 1 (:REWRITE ADE::SI-OF-DIFF-IDXES . 1)))
(ADE::SIS-SUBSET-SIS (39 3 (:DEFINITION MEMBER-EQUAL))
                     (26 23 (:REWRITE DEFAULT-+-2))
                     (23 23 (:REWRITE DEFAULT-+-1))
                     (20 20 (:REWRITE DEFAULT-<-2))
                     (20 20 (:REWRITE DEFAULT-<-1))
                     (12 6 (:REWRITE ADE::SYMBOL-INEQUALITY))
                     (12 3
                         (:REWRITE ADE::NOT-MEMBER-WITH-SIS-OF-DIFF-SYMBOL))
                     (10 7 (:REWRITE DEFAULT-CDR))
                     (10 7 (:REWRITE DEFAULT-CAR))
                     (9 9 (:REWRITE ZP-OPEN))
                     (9 9 (:REWRITE DEFAULT-SYMBOL-NAME))
                     (6 6
                        (:TYPE-PRESCRIPTION STR::ISTRPREFIXP$INLINE))
                     (3 3 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
                     (3 3
                        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(ADE::SI-IS-NTH-OF-SIS (26 8 (:REWRITE ADE::SYMBOL-INEQUALITY))
                       (25 25 (:REWRITE DEFAULT-<-2))
                       (25 25 (:REWRITE DEFAULT-<-1))
                       (25 25 (:REWRITE DEFAULT-+-2))
                       (25 25 (:REWRITE DEFAULT-+-1))
                       (20 12 (:REWRITE DEFAULT-SYMBOL-NAME))
                       (18 4
                           (:REWRITE ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
                                     . 2))
                       (15 9 (:REWRITE FOLD-CONSTS-IN-+))
                       (8 8
                          (:TYPE-PRESCRIPTION STR::ISTRPREFIXP$INLINE))
                       (7 4 (:REWRITE DEFAULT-CDR))
                       (7 4 (:REWRITE DEFAULT-CAR))
                       (5 5 (:REWRITE NTH-WHEN-PREFIXP))
                       (4 4
                          (:REWRITE ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
                                    . 1))
                       (4 4 (:REWRITE DEFAULT-UNARY-MINUS)))
(ADE::NTH-OF-SIS-IS-SI)
(ADE::POSITION1-SI-SIS (243 9 (:REWRITE ADE::MEMBER==>POSITION1))
                       (213 9 (:DEFINITION MEMBER-EQUAL))
                       (173 54 (:REWRITE ADE::SYMBOL-INEQUALITY))
                       (136 72 (:REWRITE DEFAULT-SYMBOL-NAME))
                       (91 18
                           (:REWRITE ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
                                     . 2))
                       (54 54 (:REWRITE DEFAULT-+-2))
                       (54 54 (:REWRITE DEFAULT-+-1))
                       (53 53 (:REWRITE DEFAULT-<-2))
                       (53 53 (:REWRITE DEFAULT-<-1))
                       (45 45 (:TYPE-PRESCRIPTION MEMBER-EQUAL))
                       (45 27 (:REWRITE DEFAULT-CDR))
                       (36 36
                           (:TYPE-PRESCRIPTION STR::ISTRPREFIXP$INLINE))
                       (35 21 (:REWRITE DEFAULT-CAR))
                       (18 18
                           (:REWRITE ADE::NOT-EQUAL-WITH-SI-OF-DIFF-SYMBOL
                                     . 1))
                       (5 5 (:REWRITE DEFAULT-UNARY-MINUS))
                       (3 1 (:REWRITE ADE::SI-MEMBER-SIS))
                       (1 1
                          (:REWRITE ADE::SI-OF-DIFF-SYMBOLS . 2))
                       (1 1
                          (:REWRITE ADE::SI-OF-DIFF-SYMBOLS . 1))
                       (1 1 (:REWRITE ADE::SI-OF-DIFF-IDXES . 1)))
(ADE::SIS-OF-SUBSET
     (186 18 (:REWRITE DEFAULT-CDR))
     (100 97 (:REWRITE DEFAULT-+-2))
     (97 97 (:REWRITE DEFAULT-+-1))
     (48 6 (:REWRITE ADE::SYMBOL-INEQUALITY))
     (42 6 (:REWRITE DEFAULT-SYMBOL-NAME))
     (41 41 (:REWRITE DEFAULT-<-2))
     (41 41 (:REWRITE DEFAULT-<-1))
     (24 6
         (:TYPE-PRESCRIPTION TRUE-LISTP-NTHCDR-TYPE-PRESCRIPTION))
     (16 1 (:REWRITE DEFAULT-CAR))
     (13 13 (:REWRITE DEFAULT-UNARY-MINUS))
     (3 3
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
