...
(local
 (defun v-to-nat-equality-induct (a b)
   (if (atom a)
       b
     (v-to-nat-equality-induct (cdr a) (cdr b)))))

(defthm v-to-nat-equality
  (implies (bv2p a b)
           (equal (equal (v-to-nat a) (v-to-nat b))
                  (equal a b)))
  :hints (("Goal"
           :induct (v-to-nat-equality-induct a b)
           :in-theory (enable bvp))))

(defun v-to-nat-lst (x)
  (declare (xargs :guard t))
  (if (atom x)
      nil
    (cons (v-to-nat (car x))
