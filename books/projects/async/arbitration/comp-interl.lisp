...
(defun comp-interl$data-ins-len (data-width)
  (declare (xargs :guard (natp data-width)))
  (+ 5 (* 4 (mbe :logic (nfix data-width)
                 :exec  data-width))))

(defun comp-interl$ins-len (data-width)
  (declare (xargs :guard (natp data-width)))
  (+ (comp-interl$data-ins-len data-width)
...
(module-generator
 ...
 (list
  ...
  (list 'l0
        ...
        (si 'link data-width)
        (list* 'interl0-out-act 'interl2-in0-act (sis 'd0-in 0 data-width)))

  ;; L1
  (list 'l1
        (list* 'l1-status (sis 'd1-out 0 data-width))
        (si 'link data-width)
        (list* 'interl1-out-act 'interl2-in1-act (sis 'd1-in 0 data-width)))
  ...
  (list 'interl1
        ...
        (list* 'full-in2 'full-in3 'l1-status
               (append (sis 'data2-in 0 data-width)
                       (sis 'data3-in 0 data-width)
...
(defun comp-interl$netlist (data-width)
  (declare (xargs :guard (natp data-width)))
  (cons (comp-interl* data-width)
        (union$ (interl$netlist data-width)
                :test 'equal)))

;; Recognizer for COMP-INTERL

(defund comp-interl& (netlist data-width)
  (declare (xargs :guard (and (alistp netlist)
                              (natp data-width))))
  (and (equal (assoc (si 'comp-interl data-width) netlist)
              (comp-interl* data-width))
       (b* ((netlist (delete-to-eq (si 'comp-interl data-width) netlist)))
...
(defund comp-interl$st-format (st data-width)
  (b* ((l0 (get-field *comp-interl$l0* st))
    ...
    (and (< 0 data-width)
         ...
         (interl$st-format interl0 data-width)
         (interl$st-format interl1 data-width)
         (interl$st-format interl2 data-width))))

(defthm comp-interl$st-format=>data-width-constraint
  (implies (comp-interl$st-format st data-width)
           (posp data-width))
  :hints (("Goal" :in-theory (enable comp-interl$st-format)))
  :rule-classes :forward-chaining)

(defund comp-interl$valid-st (st data-width)
  (b* ((l0 (get-field *comp-interl$l0* st))
       (l1 (get-field *comp-interl$l1* st))
       (interl0 (get-field *comp-interl$interl0* st))
       (interl1 (get-field *comp-interl$interl1* st))
    ...
    (and (< 0 data-width)
         ...
         (interl$valid-st interl0 data-width)
         (interl$valid-st interl1 data-width)
         (interl$valid-st interl2 data-width))))

(defthmd comp-interl$valid-st=>data-width-constraint
  (implies (comp-interl$valid-st st data-width)
           (posp data-width))
...
(defthmd comp-interl$valid-st=>st-format
  (implies (comp-interl$valid-st st data-width)
           (comp-interl$st-format st data-width))
...
(progn
  ...
  (defun comp-interl$data0-in (inputs data-width)
  ...
  (defthm len-comp-interl$data0-in
    (equal (len (comp-interl$data0-in inputs data-width))
           (nfix data-width)))
  ...
  (defun comp-interl$data1-in (inputs data-width)
    (declare (xargs :guard (and (true-listp inputs)
                                (natp data-width))))
    (b* ((width (mbe :logic (nfix data-width)
                     :exec  data-width)))
      (take width
            (nthcdr (+ 5 width) inputs))))

  (defthm len-comp-interl$data1-in
    (equal (len (comp-interl$data1-in inputs data-width))
           (nfix data-width)))
  ...
  (defun comp-interl$data2-in (inputs data-width)
    (declare (xargs :guard (and (true-listp inputs)
                                (natp data-width))))
    (b* ((width (mbe :logic (nfix data-width)
                     :exec  data-width)))
      (take width
            (nthcdr (+ 5 (* 2 width)) inputs))))

  (defthm len-comp-interl$data2-in
    (equal (len (comp-interl$data2-in inputs data-width))
           (nfix data-width)))
  ...
  (defun comp-interl$data3-in (inputs data-width)
    (declare (xargs :guard (and (true-listp inputs)
                                (natp data-width))))
    (b* ((width (mbe :logic (nfix data-width)
                     :exec  data-width)))
      (take width
            (nthcdr (+ 5 (* 3 width)) inputs))))

  (defthm len-comp-interl$data3-in
    (equal (len (comp-interl$data3-in inputs data-width))
           (nfix data-width)))
  ...
  (defund comp-interl$interl0-inputs (inputs st data-width)
    (b* ((full-in0 (nth 0 inputs))
         (full-in1 (nth 1 inputs))
         (data0-in (comp-interl$data0-in inputs data-width))
         (data1-in (comp-interl$data1-in inputs data-width))
         (select0  (nth (comp-interl$data-ins-len data-width) inputs))
         (go-signals (nthcdr (+ (comp-interl$data-ins-len data-width)
                                *comp-interl$select-num*)
                             inputs))
  ...
  (defund comp-interl$interl0-out-act0 (inputs st data-width)
    (b* ((interl0-inputs (comp-interl$interl0-inputs inputs st data-width))
         (interl0 (get-field *comp-interl$interl0* st)))
      (interl$out-act0 interl0-inputs interl0 data-width)))

  ;; Extract the "out-act1" signal for joint INTERL0

  (defund comp-interl$interl0-out-act1 (inputs st data-width)
    (b* ((interl0-inputs (comp-interl$interl0-inputs inputs st data-width))
         (interl0 (get-field *comp-interl$interl0* st)))
      (interl$out-act1 interl0-inputs interl0 data-width)))

  (defthm comp-interl$interl0-out-act-mutually-exclusive
    (implies (and (comp-interl$valid-st st data-width)
                  (comp-interl$interl0-out-act0 inputs st data-width))
             (not (comp-interl$interl0-out-act1 inputs st data-width)))
    :hints (("Goal" :in-theory (enable comp-interl$valid-st
  ...
  (defund comp-interl$interl0-out-act (inputs st data-width)
    (f-or (comp-interl$interl0-out-act0 inputs st data-width)
          (comp-interl$interl0-out-act1 inputs st data-width)))

  ;; Extract the output data from joint INTERL0

  (defund comp-interl$interl0-data-out (inputs st data-width)
    (b* ((interl0-inputs (comp-interl$interl0-inputs inputs st data-width))
         (interl0 (get-field *comp-interl$interl0* st)))
      (interl$data-out interl0-inputs interl0 data-width)))

  ;; Extract the inputs for joint INTERL1

  (defund comp-interl$interl1-inputs (inputs st data-width)
    (b* ((full-in2 (nth 2 inputs))
         (full-in3 (nth 3 inputs))
         (data2-in (comp-interl$data2-in inputs data-width))
         (data3-in (comp-interl$data3-in inputs data-width))
         (select1  (nth (+ (comp-interl$data-ins-len data-width)
                           *interl$select-num*)
                        inputs))
         (go-signals (nthcdr (+ (comp-interl$data-ins-len data-width)
                                *comp-interl$select-num*)
  ...
  (defund comp-interl$interl1-out-act0 (inputs st data-width)
    (b* ((interl1-inputs (comp-interl$interl1-inputs inputs st data-width))
         (interl1 (get-field *comp-interl$interl1* st)))
      (interl$out-act0 interl1-inputs interl1 data-width)))

  ;; Extract the "out-act1" signal for joint INTERL1

  (defund comp-interl$interl1-out-act1 (inputs st data-width)
    (b* ((interl1-inputs (comp-interl$interl1-inputs inputs st data-width))
         (interl1 (get-field *comp-interl$interl1* st)))
      (interl$out-act1 interl1-inputs interl1 data-width)))

  (defthm comp-interl$interl1-out-act-mutually-exclusive
    (implies (and (comp-interl$valid-st st data-width)
                  (comp-interl$interl1-out-act0 inputs st data-width))
             (not (comp-interl$interl1-out-act1 inputs st data-width)))
    :hints (("Goal" :in-theory (enable comp-interl$valid-st
  ...
  (defund comp-interl$interl1-out-act (inputs st data-width)
    (f-or (comp-interl$interl1-out-act0 inputs st data-width)
          (comp-interl$interl1-out-act1 inputs st data-width)))

  ;; Extract the output data from joint INTERL1

  (defund comp-interl$interl1-data-out (inputs st data-width)
    (b* ((interl1-inputs (comp-interl$interl1-inputs inputs st data-width))
         (interl1 (get-field *comp-interl$interl1* st)))
      (interl$data-out interl1-inputs interl1 data-width)))

  ;; Extract the inputs for joint INTERL2

  (defund comp-interl$interl2-inputs (inputs st data-width)
    (b* ((empty-out- (nth 4 inputs))
         (select2 (nth (+ (comp-interl$data-ins-len data-width)
                          (* 2 *interl$select-num*))
                       inputs))
         (go-signals (nthcdr (+ (comp-interl$data-ins-len data-width)
                                *comp-interl$select-num*)
  ...
  (defund comp-interl$in0-act (inputs st data-width)
    (b* ((interl0-inputs (comp-interl$interl0-inputs inputs st data-width))
         (interl0 (get-field *comp-interl$interl0* st)))
      (interl$in0-act interl0-inputs interl0 data-width)))

  (defthm comp-interl$in0-act-inactive
    (implies (not (nth 0 inputs))
             (not (comp-interl$in0-act inputs st data-width)))
    :hints (("Goal" :in-theory (enable comp-interl$interl0-inputs
                                       comp-interl$in0-act))))

  ;; Extract the "in1-act" signal

  (defund comp-interl$in1-act (inputs st data-width)
    (b* ((interl0-inputs (comp-interl$interl0-inputs inputs st data-width))
         (interl0 (get-field *comp-interl$interl0* st)))
      (interl$in1-act interl0-inputs interl0 data-width)))

  (defthm comp-interl$in1-act-inactive
    (implies (not (nth 1 inputs))
             (not (comp-interl$in1-act inputs st data-width)))
    :hints (("Goal" :in-theory (enable comp-interl$interl0-inputs
                                       comp-interl$in1-act))))

  ;; Extract the "in2-act" signal

  (defund comp-interl$in2-act (inputs st data-width)
    (b* ((interl1-inputs (comp-interl$interl1-inputs inputs st data-width))
         (interl1 (get-field *comp-interl$interl1* st)))
      (interl$in0-act interl1-inputs interl1 data-width)))

  (defthm comp-interl$in2-act-inactive
    (implies (not (nth 2 inputs))
             (not (comp-interl$in2-act inputs st data-width)))
    :hints (("Goal" :in-theory (enable comp-interl$interl1-inputs
                                       comp-interl$in2-act))))

  ;; Extract the "in3-act" signal

  (defund comp-interl$in3-act (inputs st data-width)
    (b* ((interl1-inputs (comp-interl$interl1-inputs inputs st data-width))
         (interl1 (get-field *comp-interl$interl1* st)))
      (interl$in1-act interl1-inputs interl1 data-width)))

  (defthm comp-interl$in3-act-inactive
    (implies (not (nth 3 inputs))
             (not (comp-interl$in3-act inputs st data-width)))
    :hints (("Goal" :in-theory (enable comp-interl$interl1-inputs
                                       comp-interl$in3-act))))

  ;; Extract the "out-act0" signal

  (defund comp-interl$out-act0 (inputs st data-width)
    (b* ((interl2-inputs (comp-interl$interl2-inputs inputs st data-width))
         (interl2 (get-field *comp-interl$interl2* st)))
      (interl$out-act0 interl2-inputs interl2 data-width)))

  (defthm comp-interl$out-act0-inactive
    (implies (equal (nth 4 inputs) t)
             (not (comp-interl$out-act0 inputs st data-width)))
    :hints (("Goal" :in-theory (enable comp-interl$interl2-inputs
                                       comp-interl$out-act0))))

  ;; Extract the "out-act1" signal

  (defund comp-interl$out-act1 (inputs st data-width)
    (b* ((interl2-inputs (comp-interl$interl2-inputs inputs st data-width))
         (interl2 (get-field *comp-interl$interl2* st)))
      (interl$out-act1 interl2-inputs interl2 data-width)))

  (defthm comp-interl$out-act1-inactive
    (implies (equal (nth 4 inputs) t)
             (not (comp-interl$out-act1 inputs st data-width)))
    :hints (("Goal" :in-theory (enable comp-interl$interl2-inputs
                                       comp-interl$out-act1))))

  (defthm comp-interl$out-act-mutually-exclusive
    (implies (and (comp-interl$valid-st st data-width)
                  (comp-interl$out-act0 inputs st data-width))
             (not (comp-interl$out-act1 inputs st data-width)))
    :hints (("Goal" :in-theory (enable comp-interl$valid-st
                                       comp-interl$out-act0
                                       comp-interl$out-act1))))
  ...
  (defund comp-interl$out-act (inputs st data-width)
    (f-or (comp-interl$out-act0 inputs st data-width)
          (comp-interl$out-act1 inputs st data-width)))

  (defthm comp-interl$out-act-inactive
    (implies (equal (nth 4 inputs) t)
             (not (comp-interl$out-act inputs st data-width)))
    :hints (("Goal" :in-theory (enable comp-interl$out-act))))

  ;; Extract the output data

  (defund comp-interl$data-out (inputs st data-width)
    (b* ((interl2-inputs (comp-interl$interl2-inputs inputs st data-width))
         (interl2 (get-field *comp-interl$interl2* st)))
      (interl$data-out interl2-inputs interl2 data-width)))

  (defthm len-comp-interl$data-out-1
    (implies (comp-interl$st-format st data-width)
             (equal (len (comp-interl$data-out inputs st data-width))
                    data-width))
    :hints (("Goal" :in-theory (enable comp-interl$st-format
  ...
  (defun comp-interl$outputs (inputs st data-width)
    (list* (comp-interl$in0-act inputs st data-width)
           (comp-interl$in1-act inputs st data-width)
           (comp-interl$in2-act inputs st data-width)
           (comp-interl$in3-act inputs st data-width)
...
(defthmd comp-interl$value
  (b* ((inputs (list* full-in0 full-in1 full-in2 full-in3 empty-out-
                      (append data0-in data1-in data2-in data3-in
                              selects go-signals))))
    (implies (and (comp-interl& netlist data-width)
                  (true-listp data0-in)
                  (equal (len data0-in) data-width)
                  (true-listp data1-in)
                  (equal (len data1-in) data-width)
                  (true-listp data2-in)
                  (equal (len data2-in) data-width)
                  (true-listp data3-in)
                  (equal (len data3-in) data-width)
                  (true-listp selects)
                  (equal (len selects) *comp-interl$select-num*)
                  (true-listp go-signals)
                  (equal (len go-signals) *comp-interl$go-num*)
                  (comp-interl$st-format st data-width))
             (equal (se (si 'comp-interl data-width) inputs st netlist)
                    (comp-interl$outputs inputs st data-width))))
  :hints (("Goal"
           :do-not-induct t
           :expand (:free (inputs data-width)
                          (se (si 'comp-interl data-width)
                              inputs st netlist))
...
(defun comp-interl$step (inputs st data-width)
  (b* ((l0 (get-field *comp-interl$l0* st))
       (l1 (get-field *comp-interl$l1* st))
       (interl0 (get-field *comp-interl$interl0* st))
       (interl1 (get-field *comp-interl$interl1* st))
       ...
       (interl0-inputs (comp-interl$interl0-inputs inputs st data-width))
       (interl1-inputs (comp-interl$interl1-inputs inputs st data-width))
       (interl2-inputs (comp-interl$interl2-inputs inputs st data-width))

       (interl0-out-act (interl$out-act interl0-inputs interl0 data-width))
       (interl1-out-act (interl$out-act interl1-inputs interl1 data-width))
       (interl2-in0-act (interl$in0-act interl2-inputs interl2 data-width))
       (interl2-in1-act (interl$in1-act interl2-inputs interl2 data-width))

       (d0-in (interl$data-out interl0-inputs interl0 data-width))
       (d1-in (interl$data-out interl1-inputs interl1 data-width))
    ...
    (list
     ...
     ;; Joint INTERL0
     (interl$step interl0-inputs interl0 data-width)
     ;; Joint INTERL1
     (interl$step interl1-inputs interl1 data-width)
     ;; Joint INTERL2
     (interl$step interl2-inputs interl2 data-width))))

(defthm len-of-comp-interl$step
  (equal (len (comp-interl$step inputs st data-width))
         *comp-interl$st-len*))
...
(defthmd comp-interl$state
  (b* ((inputs (list* full-in0 full-in1 full-in2 full-in3 empty-out-
    ...
    (implies (and (comp-interl& netlist data-width)
                  ...
                  (equal (len go-signals) *comp-interl$go-num*)
                  (comp-interl$st-format st data-width))
             (equal (de (si 'comp-interl data-width) inputs st netlist)
                    (comp-interl$step inputs st data-width))))
  :hints (("Goal"
           :do-not-induct t
           :expand (:free (inputs data-width)
                          (de (si 'comp-interl data-width)
                              inputs st netlist))
           :in-theory (e/d (de-rules
                            comp-interl&
                            comp-interl*$destructure
...
(defund comp-interl$input-format (inputs data-width)
  (declare (xargs :guard (and (true-listp inputs)
                              (natp data-width))))
  (b* ((full-in0   (nth 0 inputs))
       (full-in1   (nth 1 inputs))
       ...
       (selects    (take *comp-interl$select-num*
                         (nthcdr (comp-interl$data-ins-len data-width)
                                 inputs)))
       (go-signals (nthcdr (+ (comp-interl$data-ins-len data-width)
                              *comp-interl$select-num*)
                           inputs)))
...
(local
 (defthm comp-interl$input-format=>interl0$input-format
   (implies (and (comp-interl$input-format inputs data-width)
                 (comp-interl$valid-st st data-width))
            (interl$input-format
             (comp-interl$interl0-inputs inputs st data-width)
             data-width))
   :hints (("Goal"
...
(local
 (defthm comp-interl$input-format=>interl1$input-format
   (implies (and (comp-interl$input-format inputs data-width)
                 (comp-interl$valid-st st data-width))
            (interl$input-format
             (comp-interl$interl1-inputs inputs st data-width)
             data-width))
   :hints (("Goal"
            :in-theory (e/d (open-nth
                             interl$input-format
...
(local
 (defthm comp-interl$input-format=>interl2$input-format
   (implies (and (comp-interl$input-format inputs data-width)
                 (comp-interl$valid-st st data-width))
            (interl$input-format
             (comp-interl$interl2-inputs inputs st data-width)
             data-width))
   :hints (("Goal"
            :in-theory (e/d (open-nth
                             interl$input-format
...
(defthm booleanp-comp-interl$in0-act
  (implies (and (comp-interl$input-format inputs data-width)
                (comp-interl$valid-st st data-width))
           (booleanp (comp-interl$in0-act inputs st data-width)))
  :hints (("Goal"
...
(defthm booleanp-comp-interl$in1-act
  (implies (and (comp-interl$input-format inputs data-width)
                (comp-interl$valid-st st data-width))
           (booleanp (comp-interl$in1-act inputs st data-width)))
  :hints (("Goal"
           :use comp-interl$input-format=>interl0$input-format
           :in-theory (e/d (comp-interl$valid-st
                            comp-interl$in1-act)
...
(defthm booleanp-comp-interl$in2-act
  (implies (and (comp-interl$input-format inputs data-width)
                (comp-interl$valid-st st data-width))
           (booleanp (comp-interl$in2-act inputs st data-width)))
  :hints (("Goal"
           :use comp-interl$input-format=>interl1$input-format
           :in-theory (e/d (comp-interl$valid-st
                            comp-interl$in2-act)
...
(defthm booleanp-comp-interl$in3-act
  (implies (and (comp-interl$input-format inputs data-width)
                (comp-interl$valid-st st data-width))
           (booleanp (comp-interl$in3-act inputs st data-width)))
  :hints (("Goal"
           :use comp-interl$input-format=>interl1$input-format
           :in-theory (e/d (comp-interl$valid-st
                            comp-interl$in3-act)
...
(defthm booleanp-comp-interl$out-act0
  (implies (and (comp-interl$input-format inputs data-width)
                (comp-interl$valid-st st data-width))
           (booleanp (comp-interl$out-act0 inputs st data-width)))
  :hints (("Goal"
           :use comp-interl$input-format=>interl2$input-format
           :in-theory (e/d (comp-interl$valid-st
                            comp-interl$out-act0)
...
(defthm booleanp-comp-interl$out-act1
  (implies (and (comp-interl$input-format inputs data-width)
                (comp-interl$valid-st st data-width))
           (booleanp (comp-interl$out-act1 inputs st data-width)))
  :hints (("Goal"
           :use comp-interl$input-format=>interl2$input-format
           :in-theory (e/d (comp-interl$valid-st
                            comp-interl$out-act1)
...
(defthm booleanp-comp-interl$out-act
  (implies (and (comp-interl$input-format inputs data-width)
                (comp-interl$valid-st st data-width))
           (booleanp (comp-interl$out-act inputs st data-width)))
  :hints (("Goal"
...
(encapsulate
  ...
  (local
   (defthm comp-interl$out-act-rewrite
     (equal (comp-interl$out-act inputs st data-width)
            (b* ((interl2-inputs
                  (comp-interl$interl2-inputs inputs st data-width))
                 (interl2 (get-field *comp-interl$interl2* st)))
              (interl$out-act interl2-inputs interl2 data-width)))
  ...
  (defthm bvp-comp-interl$data-out
    (implies (and (comp-interl$input-format inputs data-width)
                  (comp-interl$valid-st st data-width)
                  (comp-interl$out-act inputs st data-width))
             (bvp (comp-interl$data-out inputs st data-width)))
    :hints (("Goal" :in-theory (enable comp-interl$valid-st
                                       comp-interl$data-out))))
  )
...
(defthm comp-interl$extract0-not-empty
  (implies (and (comp-interl$interl0-out-act0 inputs st data-width)
                (comp-interl$valid-st st data-width))
...
(defthm comp-interl$extract1-not-empty
  (implies (and (comp-interl$interl0-out-act1 inputs st data-width)
                (comp-interl$valid-st st data-width))
...
(defthm comp-interl$extract2-not-empty
  (implies (and (comp-interl$interl1-out-act0 inputs st data-width)
                (comp-interl$valid-st st data-width))
...
(defthm comp-interl$extract3-not-empty
  (implies (and (comp-interl$interl1-out-act1 inputs st data-width)
                (comp-interl$valid-st st data-width))
...
(defthm comp-interl$extract4-not-empty
  (implies (and (comp-interl$out-act0 inputs st data-width)
                (comp-interl$valid-st st data-width))
...
(defthm comp-interl$extract5-not-empty
  (implies (and (comp-interl$out-act1 inputs st data-width)
                (comp-interl$valid-st st data-width))
...
(defund comp-interl$extracted0-step (inputs st data-width)
  (b* ((data (comp-interl$data0-in inputs data-width))
       (extracted-st (comp-interl$extract0 st))
       (n (1- (len extracted-st))))
    (cond
     ((equal (comp-interl$interl0-out-act0 inputs st data-width) t)
      (cond
       ((equal (comp-interl$in0-act inputs st data-width) t)
...
(defund comp-interl$extracted1-step (inputs st data-width)
  (b* ((data (comp-interl$data1-in inputs data-width))
       (extracted-st (comp-interl$extract1 st))
       (n (1- (len extracted-st))))
    (cond
     ((equal (comp-interl$interl0-out-act1 inputs st data-width) t)
      (cond
       ((equal (comp-interl$in1-act inputs st data-width) t)
        (cons data (take n extracted-st)))
       (t (take n extracted-st))))
     (t (cond
         ((equal (comp-interl$in1-act inputs st data-width) t)
...
(defund comp-interl$extracted2-step (inputs st data-width)
  (b* ((data (comp-interl$data2-in inputs data-width))
       (extracted-st (comp-interl$extract2 st))
       (n (1- (len extracted-st))))
    (cond
     ((equal (comp-interl$interl1-out-act0 inputs st data-width) t)
      (cond
       ((equal (comp-interl$in2-act inputs st data-width) t)
        (cons data (take n extracted-st)))
       (t (take n extracted-st))))
     (t (cond
         ((equal (comp-interl$in2-act inputs st data-width) t)
...
(defund comp-interl$extracted3-step (inputs st data-width)
  (b* ((data (comp-interl$data3-in inputs data-width))
       (extracted-st (comp-interl$extract3 st))
       (n (1- (len extracted-st))))
    (cond
     ((equal (comp-interl$interl1-out-act1 inputs st data-width) t)
      (cond
       ((equal (comp-interl$in3-act inputs st data-width) t)
        (cons data (take n extracted-st)))
       (t (take n extracted-st))))
     (t (cond
         ((equal (comp-interl$in3-act inputs st data-width) t)
...
(defund comp-interl$extracted4-step (inputs st data-width)
  (b* ((data (comp-interl$interl0-data-out inputs st data-width))
       (extracted-st (comp-interl$extract4 st))
       (n (1- (len extracted-st))))
    (cond
     ((equal (comp-interl$out-act0 inputs st data-width) t)
      (cond
       ((equal (comp-interl$interl0-out-act inputs st data-width) t)
        (cons data (take n extracted-st)))
       (t (take n extracted-st))))
     (t (cond
         ((equal (comp-interl$interl0-out-act inputs st data-width) t)
...
(defund comp-interl$extracted5-step (inputs st data-width)
  (b* ((data (comp-interl$interl1-data-out inputs st data-width))
       (extracted-st (comp-interl$extract5 st))
       (n (1- (len extracted-st))))
    (cond
     ((equal (comp-interl$out-act1 inputs st data-width) t)
      (cond
       ((equal (comp-interl$interl1-out-act inputs st data-width) t)
        (cons data (take n extracted-st)))
       (t (take n extracted-st))))
     (t (cond
         ((equal (comp-interl$interl1-out-act inputs st data-width) t)
...
(encapsulate
  ...
  (local
   (defthm comp-interl$extracted0+1-step-correct-aux-1
     (equal (interl$data0-in (comp-interl$interl0-inputs inputs st data-width)
                             data-width)
            (comp-interl$data0-in inputs data-width))
     :hints (("Goal" :in-theory (enable comp-interl$interl0-inputs
                                        interl$data0-in)))))

  (local
   (defthm comp-interl$extracted0+1-step-correct-aux-2
     (equal (interl$data1-in (comp-interl$interl0-inputs inputs st data-width)
                             data-width)
            (comp-interl$data1-in inputs data-width))
     :hints (("Goal" :in-theory (enable comp-interl$interl0-inputs
                                        comp-interl$data1-in
                                        interl$data1-in)))))

  (defthm comp-interl$extracted0+1-step-correct
    (b* ((next-st (comp-interl$step inputs st data-width)))
      (implies
       (and (comp-interl$input-format inputs data-width)
            (comp-interl$valid-st st data-width))
       (and (equal (comp-interl$extract0 next-st)
                   (comp-interl$extracted0-step inputs st data-width))
            (equal (comp-interl$extract1 next-st)
                   (comp-interl$extracted1-step inputs st data-width)))))
    :hints (("Goal"
  ...
  (local
   (defthm comp-interl$extracted2+3-step-correct-aux-1
     (equal (interl$data0-in (comp-interl$interl1-inputs inputs st data-width)
                             data-width)
            (comp-interl$data2-in inputs data-width))
     :hints (("Goal" :in-theory (enable comp-interl$interl1-inputs
                                        interl$data0-in)))))

  (local
   (defthm comp-interl$extracted2+3-step-correct-aux-2
     (equal (interl$data1-in (comp-interl$interl1-inputs inputs st data-width)
                             data-width)
            (comp-interl$data3-in inputs data-width))
     :hints (("Goal" :in-theory (enable comp-interl$interl1-inputs
                                        comp-interl$data3-in
                                        interl$data1-in)))))
    ...
    (b* ((next-st (comp-interl$step inputs st data-width)))
      (implies
       ...
       (and (equal (comp-interl$extract2 next-st)
                   (comp-interl$extracted2-step inputs st data-width))
            (equal (comp-interl$extract3 next-st)
                   (comp-interl$extracted3-step inputs st data-width)))))
    :hints (("Goal"
             :in-theory (e/d (get-field
                              interl$extracted0-step
                              interl$extracted1-step
   ...
   (defthm comp-interl$extracted4+5-step-correct-aux-1
     (b* ((l0 (nth *comp-interl$l0* st))
       ...
       (implies (and (natp data-width)
                     (equal data-width (len l0.d)))
                (equal (interl$data0-in interl2-inputs data-width)
  ...
  (local
   (defthm comp-interl$extracted4+5-step-correct-aux-2
     (b* ((l0 (nth *comp-interl$l0* st))
       ...
       (implies (and (natp data-width)
                     (equal data-width (len l0.d))
                     (equal data-width (len l1.d)))
                (equal (interl$data1-in interl2-inputs data-width)
                       (v-threefix (strip-cars l1.d)))))
     :hints (("Goal" :in-theory (enable get-field
                                        comp-interl$interl2-inputs
                                        interl$data1-in)))))
    ...
    (b* ((next-st (comp-interl$step inputs st data-width)))
      (implies
       ...
       (and (equal (comp-interl$extract4 next-st)
                   (comp-interl$extracted4-step inputs st data-width))
            (equal (comp-interl$extract5 next-st)
                   (comp-interl$extracted5-step inputs st data-width)))))
    :hints (("Goal"
             :use (comp-interl$input-format=>interl0$input-format
                   comp-interl$input-format=>interl1$input-format
                   comp-interl$input-format=>interl2$input-format)
...
(encapsulate
   ...
   (defthm comp-interl$valid-st-preserved-aux-1
     (implies (and (equal (nth 2 inputs2) (nth 0 inputs1))
              ...
              (not (and (interl$out-act inputs2 st2 data-width2)
                        (interl$in0-act inputs1 st1 data-width1))))
  ...
  (local
   (defthm comp-interl$valid-st-preserved-aux-2
     (implies (and (equal (nth 2 inputs2) (nth 1 inputs1))
                   (booleanp (nth 2 inputs2)))
              (not (and (interl$out-act inputs2 st2 data-width2)
                        (interl$in1-act inputs1 st1 data-width1))))
     :hints (("Goal" :cases ((nth 2 inputs2))))))
  ...
  (defthm comp-interl$valid-st-preserved
    (implies (and (comp-interl$input-format inputs data-width)
                  (comp-interl$valid-st st data-width))
             (comp-interl$valid-st (comp-interl$step inputs st data-width)
                                   data-width))
    :hints (("Goal"
             :use (comp-interl$input-format=>interl0$input-format
                   comp-interl$input-format=>interl1$input-format
...
(defthm comp-interl$extract0-lemma
  (implies
   (and (comp-interl$input-format inputs data-width)
        (comp-interl$valid-st st data-width)
        (comp-interl$interl0-out-act0 inputs st data-width))
   (equal (list (comp-interl$interl0-data-out inputs st data-width))
...
(defthm comp-interl$extract2-lemma
  (implies
   (and (comp-interl$input-format inputs data-width)
        (comp-interl$valid-st st data-width)
        (comp-interl$interl1-out-act0 inputs st data-width))
   (equal (list (comp-interl$interl1-data-out inputs st data-width))
...
(defthm comp-interl$extract4-lemma
  (implies (and (comp-interl$input-format inputs data-width)
                (comp-interl$valid-st st data-width)
                (comp-interl$out-act0 inputs st data-width))
           (equal (list (comp-interl$data-out inputs st data-width))
...
(defthmd comp-interl$dataflow-correct
  (b* ((extracted0-st (comp-interl$extract0 st))
       ...
       (final-st (comp-interl$run inputs-seq st data-width n))
    ...
    (implies
     (and (comp-interl$input-format-n inputs-seq data-width n)
          (comp-interl$valid-st st data-width)
          (member x
                  (interleave2
                   ...
                   (prepend-rec (interleave final-extracted2-st
                                ...
                                final-extracted5-st))))
     (member
      (append x (comp-interl$out-seq inputs-seq st data-width n))
      (interleave2
       (prepend-rec
        (interleave
         (append (comp-interl$in0-seq inputs-seq st data-width n)
                 extracted0-st)
         (append (comp-interl$in1-seq inputs-seq st data-width n)
                 extracted1-st))
        extracted4-st)
       (prepend-rec
        (interleave
         (append (comp-interl$in2-seq inputs-seq st data-width n)
                 extracted2-st)
         (append (comp-interl$in3-seq inputs-seq st data-width n)
                 extracted3-st))
        extracted5-st)))))
...
(defthmd comp-interl$functionally-correct
  (b* ((extracted0-st (comp-interl$extract0 st))
       ...
       (extracted4-st (comp-interl$extract4 st))
       (extracted5-st (comp-interl$extract5 st))
       (final-st (de-n (si 'comp-interl data-width) inputs-seq st netlist n))
       (final-extracted0-st (comp-interl$extract0 final-st))
       (final-extracted1-st (comp-interl$extract1 final-st))
       (final-extracted2-st (comp-interl$extract2 final-st))
       (final-extracted3-st (comp-interl$extract3 final-st))
       ...
       (final-extracted5-st (comp-interl$extract5 final-st)))
    (implies
     (and (comp-interl& netlist data-width)
          (comp-interl$input-format-n inputs-seq data-width n)
          (comp-interl$valid-st st data-width)
          (member x
                  (interleave2
                   (prepend-rec (interleave final-extracted0-st
                   ...
                   (prepend-rec (interleave final-extracted2-st
                                ...
                                final-extracted5-st))))
     (member
      (append x (comp-interl$netlist-out-seq
                 inputs-seq st netlist data-width n))
      (interleave2
       (prepend-rec
        (interleave
         (append (comp-interl$netlist-in0-seq
                  inputs-seq st netlist data-width n)
                 extracted0-st)
         (append (comp-interl$netlist-in1-seq
                  inputs-seq st netlist data-width n)
                 extracted1-st))
        extracted4-st)
       (prepend-rec
        (interleave
         (append (comp-interl$netlist-in2-seq
                  inputs-seq st netlist data-width n)
                 extracted2-st)
         (append (comp-interl$netlist-in3-seq
                  inputs-seq st netlist data-width n)
                 extracted3-st))
        extracted5-st)))))
  :hints (("Goal"
