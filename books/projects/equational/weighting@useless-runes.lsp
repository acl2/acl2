(FIND-MATCHING-SCHEME)
(GW-TERM-SIGNATURE* (155 72 (:REWRITE DEFAULT-+-2))
                    (101 72 (:REWRITE DEFAULT-+-1))
                    (56 14 (:DEFINITION INTEGER-ABS))
                    (56 7 (:DEFINITION LENGTH))
                    (35 7 (:DEFINITION LEN))
                    (24 18 (:REWRITE DEFAULT-<-2))
                    (22 22 (:REWRITE DEFAULT-CDR))
                    (22 18 (:REWRITE DEFAULT-<-1))
                    (18 18 (:REWRITE DEFAULT-CAR))
                    (14 14 (:REWRITE DEFAULT-UNARY-MINUS))
                    (7 7 (:REWRITE DEFAULT-REALPART))
                    (7 7 (:REWRITE DEFAULT-NUMERATOR))
                    (7 7 (:REWRITE DEFAULT-IMAGPART))
                    (7 7 (:REWRITE DEFAULT-DENOMINATOR))
                    (7 7 (:REWRITE DEFAULT-COERCE-2))
                    (7 7 (:REWRITE DEFAULT-COERCE-1))
                    (3 3 (:TYPE-PRESCRIPTION TRUE-LISTP)))
(GW-TERM-SIGNATURE)
(GENERIC-WEIGHT*)
(GENERIC-WEIGHT)
(GENERIC-CLAUSE-WEIGHT (5 5 (:TYPE-PRESCRIPTION GENERIC-WEIGHT)))
(FIND-MATCHING-SCHEME*)
(ANNOTATE-WF-INSTANCE (1170 556 (:REWRITE DEFAULT-+-2))
                      (750 556 (:REWRITE DEFAULT-+-1))
                      (384 96 (:REWRITE COMMUTATIVITY-OF-+))
                      (384 96 (:DEFINITION INTEGER-ABS))
                      (384 48 (:DEFINITION LENGTH))
                      (247 247 (:REWRITE DEFAULT-CDR))
                      (240 48 (:DEFINITION LEN))
                      (174 130 (:REWRITE DEFAULT-<-2))
                      (165 165 (:REWRITE DEFAULT-CAR))
                      (164 130 (:REWRITE DEFAULT-<-1))
                      (96 96 (:REWRITE DEFAULT-UNARY-MINUS))
                      (48 48 (:TYPE-PRESCRIPTION LEN))
                      (48 48 (:REWRITE DEFAULT-REALPART))
                      (48 48 (:REWRITE DEFAULT-NUMERATOR))
                      (48 48 (:REWRITE DEFAULT-IMAGPART))
                      (48 48 (:REWRITE DEFAULT-DENOMINATOR))
                      (48 48 (:REWRITE DEFAULT-COERCE-2))
                      (48 48 (:REWRITE DEFAULT-COERCE-1))
                      (5 5 (:TYPE-PRESCRIPTION GENERIC-WEIGHT)))
(FILTER-OUT-HEAVY-CLAUSES)
