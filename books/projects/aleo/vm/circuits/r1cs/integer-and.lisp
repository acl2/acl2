...
; This gadget calculates the bitwise 'and' of two integers.
; It is the same for signed and unsigned intgers.

(define integer-and-gadget ((xs symbol-listp)
                            (ys symbol-listp)
                            (zs symbol-listp))
  :guard (and (equal (len ys) (len xs))
              (equal (len zs) (len xs)))
  :returns (constrs r1cs::r1cs-constraint-listp :hyp :guard)
  (boolean-and-gadget-list xs ys zs))

(defruled integer-and-to-bitand-list
           ...
           (b* ((xs$ (lookup-equal-list xs asg))
             ...
             (implies (and (bit-listp xs$)
                      ...
                      (equal (r1cs::r1cs-constraints-holdp
                              (integer-and-gadget xs ys zs) asg p)
                             (equal zs$ (bitand-list xs$ ys$))))))
  :enable (integer-and-gadget
           boolean-and-gadget-list-to-bitand-list))
