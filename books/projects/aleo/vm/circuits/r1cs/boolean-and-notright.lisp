...
; that also negates its right operand.
...
(define boolean-and-notright-gadget ((x symbolp)
                                     (y symbolp)
                                     (z symbolp)
                                     (one symbolp)
                                     (p primep))
...
(defruled boolean-and-notright-gadget-to-booland-boolnot
           ...
           (b* ((x$ (lookup-equal x asg))
             ...
             (implies (and (bitp x$)
                      ...
                      (equal (r1cs::r1cs-constraints-holdp
                              (boolean-and-notright-gadget x y z one p) asg p)
                             (equal z$
                                    (if (and (equal x$ 1)
                                             (equal y$ 0))
                                        1
                                      0))))))
  :enable (boolean-and-notright-gadget
