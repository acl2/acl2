...
;; (aleo::p1cs (aleovm::psd-rate2-3-wef--constraints))

;; Lift the R1CS into logic:
(local
  (r1cs::lift-r1cs *psd-rate2-3*
                   (r1cs-constraint-list-vars (aleovm::psd-rate2-3-wef--constraints))
...
(defun spec (in1 in2 in3 out)
  (declare (xargs :guard (and (fep in1 primes::*bls12-377-scalar-field-prime*)
                              (fep in2 primes::*bls12-377-scalar-field-prime*)
                              (fep in3 primes::*bls12-377-scalar-field-prime*)
                              (fep out primes::*bls12-377-scalar-field-prime*))))
  (equal out
         (poseidon::hash2 (list in1 in2 in3))))

;; Assumes:
;; 1. The R1CS holds
;; 2. w548 is the constant 1
;; 3. All the vars are field elements
;; Proves that the spec holds.
(acl2::prove-implication-with-r1cs-prover
  (acl2::make-conjunction-dag! (acl2::make-term-into-dag-basic! '(equal |w548| '1) nil)
  (acl2::make-conjunction-dag! (acl2::make-term-into-dag-basic!
                                 ;; todo: tool could translate the fe-listp assumption
                                 (pfield::gen-fe-listp-assumption (acl2::dag-vars *psd-rate2-3*)
                                                                  ''8444461749428370424248824938781546531375899335154063827935233455917409239041)
                                 nil)
                               *psd-rate2-3*))
    `(spec |w0| |w1| |w2| |w549|)
...
;  :print t
  :rule-lists '(;;First, solve and subsitute using all the bitxor and bitnot constraints.  Requires several rounds of substitution:
                ...
                (pfield::add-commutative-axe
                  pfield::add-commutative-2-axe ; expensive if we get rid of the intervening MULs
