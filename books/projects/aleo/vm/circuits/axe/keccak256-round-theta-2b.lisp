...
;; (aleo::p1cs (aleovm::keccak256-round-theta-2b--constraints))

;; todo: are there unnecesary constraints and vars?

(defconst *theta-2b-input-vars-a*
  (acl2::make-var-names '|w| 1600))

(defconst *theta-2b-input-vars-d*
  (acl2::make-var-names-aux '|w| 1600 (+ 319 1600)))

(defconst *theta-2b-input-vars*
  (append *theta-2b-input-vars-a*
          *theta-2b-input-vars-d*))

(defconst *theta-2b-output-vars*
  (acl2::make-var-names-aux '|w| 3200 (+ 1599 3200)))

(set-rewrite-stack-limit 10000)

(make-event
  `(defun spec-theta-2b (,@*theta-2b-input-vars* ,@*theta-2b-output-vars*)
     (declare (xargs ;; :guard (and ,@(acl2::make-bitp-claims *theta-2b-input-vars*)
                     ;;             ,@(acl2::make-bitp-claims *theta-2b-output-vars*)
                     ;;             ) ; todo: put back
     ...
     (equal (sha3::theta-planes 0
                                (sha3::bits-to-state-array (list ,@*theta-2b-input-vars-a*) 64)
                                (sha3::bit-string-to-plane (list ,@*theta-2b-input-vars-d*) 64)
                                64)
            (sha3::bits-to-state-array (list ,@*theta-2b-output-vars*) 64))))

;; theta-2b only:
(local
  (r1cs::lift-r1cs *keccak256-round-theta-2b*
                   (r1cs-constraint-list-vars (aleovm::keccak256-round-theta-2b--constraints))
                   ;; only include the constraints that we need:
                   ;; of the 4800 constraints, we only need the first 1920 and the last 1600
                   (append (take 1920 (aleovm::keccak256-round-theta-2b--constraints))
                           (take 1600 (nthcdr 3200 (aleovm::keccak256-round-theta-2b--constraints))))
...
;; Proves that the spec (spec-theta-2b) holds.
(acl2::prove-implication-with-r1cs-prover
  (acl2::make-conjunction-dag! (acl2::make-term-into-dag-basic! '(equal r1cs::|x0| '1) ; x0 is always equal to 1 !
                               ...
                               (acl2::make-conjunction-dag! (acl2::make-term-into-dag-basic!
                                                              ...
                                                              (pfield::gen-fe-listp-assumption (acl2::dag-vars *keccak256-round-theta-2b*)
                                                                                               ''8444461749428370424248824938781546531375899335154063827935233455917409239041)
                                                              nil)
                                                            *keccak256-round-theta-2b*))
  `(spec-theta-2b ,@*theta-2b-input-vars* ,@*theta-2b-output-vars*)
  :no-splitp t
