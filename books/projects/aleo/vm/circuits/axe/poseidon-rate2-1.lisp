...
;; (aleo::p1cs (aleovm::psd-rate2-1-wef--constraints))

;; Lift the R1CS into logic:
(local
  (r1cs::lift-r1cs *psd-rate2-1*
                   (r1cs-constraint-list-vars (aleovm::psd-rate2-1-wef--constraints))
...
(defun spec (in out)
  ...
  (equal out
         (poseidon::hash2 (list in))))

;; |w0|: '0
;; |w255|: '2406985119367428983954417126449171063400135114006879591034779696647676148614
;; |w260|: '5931053489589506448253640939421178214348920871890116789852085376112438331928
;; |w265|: '5478525966447457291405395440729464962396373994207337487717874548751673345728)

;; |w0|: '2222
;; |w255|: '1552269859485498239397752295160255909480264601884240276902493476542810904131
;; |w260|: '386094353636404731620478423344107625951120539240333348114272538871798983495
;; |w265|: '7769469862772422569698121821617114200860590351866623553670673039729561806467

;; todo: 4 versions: rate 2/3, 1 or more inputs (3 or 5)?

;; todo: make a version of make-term-into-dag-basic! that translates

;; Assumes:
;; 1. The R1CS holds
;; 2. w266 is the constant 1
;; 3. All the vars are field elements
;; Proves that the spec holds.
(acl2::prove-implication-with-r1cs-prover
  (acl2::make-conjunction-dag! (acl2::make-term-into-dag-basic! '(equal |w266| '1) nil)
  (acl2::make-conjunction-dag! (acl2::make-term-into-dag-basic!
                                 ;; todo: tool could translate the fe-listp assumption
                                 (pfield::gen-fe-listp-assumption (acl2::dag-vars *psd-rate2-1*)
                                                                  ''8444461749428370424248824938781546531375899335154063827935233455917409239041)
                                 nil)
                               *psd-rate2-1*))
    `(spec |w0| |w267|)
