...
(define create-signer-possiblep ((cert certificatep) (vstate validator-statep))
  ...
  (b* (((certificate cert) cert)
       ...
       ((unless commtt)
        nil)
       ((unless (set::in (certificate->author cert) (committee-members commtt)))
        nil)
       ((when (cert-with-author+round (certificate->author cert)
                                      (certificate->round cert) vstate.dag))
        nil)
       ((when (= (certificate->round cert) 1))
        (set::emptyp (certificate->previous cert)))
       ((when (set::emptyp (certificate->previous cert)))
        nil)
       ...
       ((unless (set::subset (certificate->previous cert)
        ...
        nil)
       ((unless (>= (committee-members-stake (certificate->previous cert)
                                             prev-commtt)
...
(define create-author-possiblep ((cert certificatep) (vstate validator-statep))
  ...
  (b* (((certificate cert) cert)
       ...
       ((unless (create-signer-possiblep cert vstate))
        nil)
       ((unless (= (certificate->round cert) vstate.round))
        nil)
       ((when (set::in (certificate->author cert) cert.endorsers))
        nil)
       (commtt (active-committee-at-round (certificate->round cert)
                                          vstate.blockchain))
...
(define create-endorser-possiblep ((cert certificatep)
  ...
  (b* (((certificate cert) cert)
       ...
       ((unless (create-signer-possiblep cert vstate))
        nil)
       ((when (set::in (make-address+pos :address (certificate->author cert)
...
(define create-possiblep ((cert certificatep) (systate system-statep))
  ...
  (b* (((certificate cert) cert)
       ((unless (or (not (set::in (certificate->author cert)
       ...
       (systate
        (if (set::in (certificate->author cert) (correct-addresses systate))
            (b* ((vstate
              ...
              (update-validator-state (certificate->author cert)
                                      new-vstate
                                      systate))
          systate))
