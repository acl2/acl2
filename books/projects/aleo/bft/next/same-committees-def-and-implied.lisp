...
(defruled trim-blocks-for-round-no-change
  :short "Trimming a blockchain to a round after the last one causes no change."
  :long
  (xdoc::topstring
   (xdoc::p
...
(defruled trim-blocks-for-round-of-append-yields-first
  ...
  :enable (trim-blocks-for-round
           ...
           newest-geq-oldest-when-blocks-orderedp))
...
(defruled trim-blocks-for-round-of-shorter
  ...
  (implies (and (blocks-orderedp blocks2)
           ...
           (equal (trim-blocks-for-round
                   ...
                   blocks1)
                  (block-list-fix blocks1)))
  :enable trim-blocks-for-round-no-change
  :use
  ...
  (xdoc::topstring
   (xdoc::p
    "This proves most of what we need for our overall proof.
     ...
     We also need the fact that the rounds are ordered;
     together with the fact that block rounds are even,
