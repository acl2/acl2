...
(define bonded-committee-at-round ((round posp) (blocks block-listp))
  ...
  (xdoc::topstring
   ...
   (xdoc::p
    "There seems to be no real criterion to choose between the two options,
     ...
     and more in general the bonded committee at an even round
     is always the same as the one at the immediately preceding odd round.")
   (xdoc::p
   ...
   (xdoc::p
    "Since, as explained above, we regard the committe as
     ...
     Note that the termination of this function
     does not require the blocks to be ordered:
     when the blockchain is empty,
     @(tsee blocks-last-round) returns 0,
     which is less than any round number.")
   (xdoc::p
  ...
  ((define bonded-committee-at-round-loop ((round posp) (blocks block-listp))
     ...
     (fty::deffixequiv bonded-committee-at-round-loop
       :hints (("Goal"
                :induct t
...
(define active-committee-at-round ((round posp) (blocks block-listp))
  ...
  (defruled active-committee-at-round-of-round-leq-2+lookback
    (implies (<= (pos-fix round) (+ 2 (lookback)))
...
(define committee-max-faulty-stake ((commtt committeep))
  ...
  (xdoc::topstring
   (xdoc::p
    "This is the @($f$) discussed in @(tsee max-faulty-for-total),
     which applies to a committee, not to the whole system,
