...
;; in the system-level marking mode, I could do something like the
...
;; Theorems about the projection of model state's fields:

(defthmd preconditions-imply-ms-fault-mode
  (implies (rewire_dst_to_src-effects-preconditions x86)
...
(defthmd ms-fault-programmer-level-and-marking-mode-projection
  (implies (rewire_dst_to_src-effects-preconditions x86)
           (and
            (equal (xr :ms                          0 (zeroCopy-state x86)) nil)
            (equal (xr :fault                       0 (zeroCopy-state x86)) nil)
            (equal (xr :programmer-level-mode       0 (zeroCopy-state x86)) nil)
            (equal (xr :page-structure-marking-mode 0 (zeroCopy-state x86)) t)))
  :hints (("Goal"
           :do-not '(preprocess)
           :use ((:instance fault-projection))
           :hands-off (x86-run)
           :in-theory
           (e/d* (preconditions-imply-ms-fault-mode)
...
(defthmd program-at-alt-projection
  ...
  :hints (("Goal"
           :use ((:instance preconditions-imply-ms-fault-mode)
                 (:instance preconditions-imply-x86p))
           :in-theory (e/d* (program-pas-projection
                             ms-fault-programmer-level-and-marking-mode-projection
                             page-size
                             pdpt-base-addr
...
(defthmd program-at-projection
  ...
  :hints (("Goal"
           ...
           :in-theory (e/d* (disjoint-p$
                             program-at-alt
                             ms-fault-programmer-level-and-marking-mode-projection
                             preconditions-imply-ms-fault-mode)
                            (rewrite-program-at-to-program-at-alt
                             zeroCopy-state
                             rewire_dst_to_src-effects-preconditions)))))
...
(defthmd no-errors-during-program-execution
  ;; Derived from ms-fault-programmer-level-and-marking-mode-projection.
  (implies
   (rewire_dst_to_src-effects-preconditions x86)
   (and (equal (xr :ms  0 (x86-run (rewire_dst_to_src-clk) x86)) nil)
        ...
        (equal (xr :page-structure-marking-mode 0 (x86-run (rewire_dst_to_src-clk) x86))
               t)))
  :hints (("Goal"
           :use ((:instance ms-fault-programmer-level-and-marking-mode-projection)
                 (:instance rewire_dst_to_src-effects))
