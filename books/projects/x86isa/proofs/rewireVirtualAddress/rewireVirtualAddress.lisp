...
(defthm unsigned-byte-p-64-of-source-pml4-table-entry
  (implies (and (equal l-addrs
                       (create-canonical-address-list
                        8
                        (pml4-table-entry-addr
                ...
                (canonical-address-p
                 (+ 7
                    (pml4-table-entry-addr
                     (xr :rgf *rdi* x86) (pml4-table-base-addr x86))))
                (not (mv-nth 0 (las-to-pas l-addrs :r (cpl x86) x86)))
                (not (xr :programmer-level-mode 0 x86))
                (x86p x86))
           (unsigned-byte-p 64 (combine-bytes (mv-nth 1 (rb l-addrs :r x86))))))

(defthm unsigned-byte-p-64-of-destination-pml4-table-entry
  (implies (and (equal l-addrs
                       (create-canonical-address-list
                        8
                        (pml4-table-entry-addr
...
(defthm unsigned-byte-p-64-of-source-page-dir-ptr-table-entry
  (implies (and (equal l-addrs
                       (create-canonical-address-list
                        8
...
(defthm unsigned-byte-p-64-of-destination-page-dir-ptr-table-entry
  (implies (and (equal l-addrs
                       (create-canonical-address-list
                        8
                        (page-dir-ptr-table-entry-addr
...
;; (defthm unsigned-byte-p-64-of-pml4-table-entry
;;   (implies (and (bind-free '(((lin-addr . (XR ':RGF '7 X86)))
;;                              ((lin-addr . (XR ':RGF '6 X86))))
;;                            (lin-addr))
;;                 (or (equal lin-addr (xr :rgf *rsi* x86))
;;                     (equal lin-addr (xr :rgf *rdi* x86)))
;;                 (equal l-addrs
;;                        (create-canonical-address-list
;;                         8
...
;; (defthm unsigned-byte-p-64-of-page-dir-ptr-table-entry
;;   (implies (and
;;             (bind-free '(((lin-addr . (XR ':RGF '7 X86)))
;;                          ((lin-addr . (XR ':RGF '6 X86))))
;;                        (lin-addr))
;;             (or (equal lin-addr (xr :rgf *rsi* x86))
;;                 (equal lin-addr (xr :rgf *rdi* x86)))
;;             (equal l-addrs (create-canonical-address-list
;;                             8
;;                             (page-dir-ptr-table-entry-addr
;;                              lin-addr
...
(def-gl-export page-dir-ptr-table-entry-P=1-and-PS=1-zf-spec-helper-2
  ...
  :concl (equal (zf-spec (loghead 64
                                  (+ -129
                                     (logand 129 (logior (loghead 30 destination-entry)
  ...
  :hints (("Goal"
           ...
           :in-theory (e/d* (instruction-decoding-and-spec-rules
                            ...
                            (member-p-strip-cars-of-remove-duplicate-keys
                             ...
                             (:REWRITE
                              DISJOINTNESS-OF-ALL-TRANSLATION-GOVERNING-ADDRESSES-FROM-ALL-TRANSLATION-GOVERNING-ADDRESSES)
...
(defthmd expand-combine-bytes-of-len-8-helper-final
  (implies (and
            (implies
             (and (unsigned-byte-p 8 (car bytes))
                  (unsigned-byte-p 8 (cadr bytes))
           ...
           (equal (combine-bytes bytes)
                  (logior (car bytes)
                          ...
                          (ash (logior (car (cddddr bytes))
                               ...
                               32))))
  :hints (("Goal"
           :use ((:instance expand-combine-bytes-of-len-8-helper-final)
           ...
           :in-theory (e/d* () (acl2::simplify-logior not)))))
...
;;   (implies (and
;;             (equal p-addrs (translation-governing-addresses-for-page-table lin-addr base-addr x86))
;;             (not
;;              (mv-nth
;;               0
;;               (ia32e-la-to-pa-page-table
...
;;             (equal (page-present (rm-low-64 (page-table-entry-addr lin-addr base-addr) x86))
;;                    (page-present (combine-bytes bytes)))
;;             (equal (page-read-write (rm-low-64 (page-table-entry-addr lin-addr base-addr) x86))
;;                    (page-read-write (combine-bytes bytes)))
;;             (equal (page-user-supervisor (rm-low-64 (page-table-entry-addr lin-addr base-addr) x86))
;;                    (page-user-supervisor (combine-bytes bytes)))
;;             (equal (page-execute-disable (rm-low-64 (page-table-entry-addr lin-addr base-addr) x86))
;;                    (page-execute-disable (combine-bytes bytes)))
;;             (equal (page-size (rm-low-64 (page-table-entry-addr lin-addr base-addr) x86))
...
;;                                (ash (loghead 40 (logtail 12 (combine-bytes bytes)))
;;                                     12)))))
;;   :hints (("Goal"
;;            :do-not-induct t
;;            :in-theory (e/d* (disjoint-p
...
;;   (implies (and
;;             (equal p-addrs (addr-range 8 (page-dir-ptr-table-entry-addr lin-addr base-addr)))
;;             (not
;;              (mv-nth
;;               0
;;               (ia32e-la-to-pa-page-dir-ptr-table
...
;;             (equal (page-present (rm-low-64 (page-dir-ptr-table-entry-addr lin-addr base-addr) x86))
;;                    (page-present (combine-bytes bytes)))
;;             (equal (page-read-write (rm-low-64 (page-dir-ptr-table-entry-addr lin-addr base-addr) x86))
;;                    (page-read-write (combine-bytes bytes)))
;;             (equal (page-user-supervisor (rm-low-64 (page-dir-ptr-table-entry-addr lin-addr base-addr) x86))
;;                    (page-user-supervisor (combine-bytes bytes)))
;;             (equal (page-execute-disable (rm-low-64 (page-dir-ptr-table-entry-addr lin-addr base-addr) x86))
;;                    (page-execute-disable (combine-bytes bytes)))
;;             (equal (page-size (rm-low-64 (page-dir-ptr-table-entry-addr lin-addr base-addr) x86))
;;                    (page-size (combine-bytes bytes)))
;;             ;; 1 G pages
;;             (equal (page-size (rm-low-64 (page-dir-ptr-table-entry-addr lin-addr base-addr) x86)) 1)

;;             (equal
;;              (part-select (rm-low-64 (page-dir-ptr-table-entry-addr lin-addr base-addr) x86) :low 13 :high 29)
...
;;                                       x86)))
;;                         12))
;;             (equal p-addrs (addr-range 8 (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr)))
;;             (disjoint-p (addr-range 8 (pml4-table-entry-addr lin-addr base-addr))
...
;;             (equal (page-present (rm-low-64
;;                                   (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr)
;;                                   x86))
;;                    (page-present (combine-bytes bytes)))
;;             (equal (page-read-write (rm-low-64
;;                                      (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr)
;;                                      x86))
;;                    (page-read-write (combine-bytes bytes)))
;;             (equal (page-user-supervisor (rm-low-64
;;                                           (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr)
;;                                           x86))
;;                    (page-user-supervisor (combine-bytes bytes)))
;;             (equal (page-execute-disable (rm-low-64
;;                                           (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr)
;;                                           x86))
;;                    (page-execute-disable (combine-bytes bytes)))
...
;;             (equal
;;              (part-select
;;               (rm-low-64 (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr) x86)
;;               :low 13 :high 29)
;;              (part-select (combine-bytes bytes) :low 13 :high 29))
...
;;   (implies (and
;;             (equal cpl (cpl x86))
;;             (equal pml4-table-base-addr (ash (cr3-slice :cr3-pdb (ctri *cr3* x86)) 12))
;;             (equal pml4-table-entry (rm-low-64 (pml4-table-entry-addr lin-addr pml4-table-base-addr) x86))
...
;;             (not (page-structure-marking-mode x86))
;;             (x86p x86))
;;            (and (equal (mv-nth 0 (ia32e-la-to-pa lin-addr r-w-x cpl (mv-nth 1 (wb addr-lst x86))))
;;                        nil)
;;                 (equal (mv-nth 1 (ia32e-la-to-pa lin-addr r-w-x cpl (mv-nth 1 (wb addr-lst x86))))
;;                        (logior (loghead 30 lin-addr)
...
(defthm rb-and-rm-low-64-for-direct-map
  ...
  :hints (("Goal" :in-theory (e/d* (rb rm-low-64 rm-low-32
                                       read-from-physical-memory) ()))))

(local (in-theory (e/d () (rb-and-rm-low-64-for-direct-map-helper rb-and-rm-low-64-for-direct-map-helper-gl))))
...
(def-gl-export same-pml4-table-entry-addr-for-8-lin-addrs
  :hyp (and (physical-address-p pml4-table-base-addr)
            (canonical-address-p lin-addr)
            ;; 1G aligned linear address
            (equal (loghead 30 lin-addr) 0))
  :concl (and
          (equal (pml4-table-entry-addr (+ 1 lin-addr) pml4-table-base-addr)
                 (pml4-table-entry-addr lin-addr pml4-table-base-addr))
          (equal (pml4-table-entry-addr (+ 2 lin-addr) pml4-table-base-addr)
                 (pml4-table-entry-addr lin-addr pml4-table-base-addr))
          (equal (pml4-table-entry-addr (+ 3 lin-addr) pml4-table-base-addr)
                 (pml4-table-entry-addr lin-addr pml4-table-base-addr))
          (equal (pml4-table-entry-addr (+ 4 lin-addr) pml4-table-base-addr)
                 (pml4-table-entry-addr lin-addr pml4-table-base-addr))
          (equal (pml4-table-entry-addr (+ 5 lin-addr) pml4-table-base-addr)
                 (pml4-table-entry-addr lin-addr pml4-table-base-addr))
          (equal (pml4-table-entry-addr (+ 6 lin-addr) pml4-table-base-addr)
                 (pml4-table-entry-addr lin-addr pml4-table-base-addr))
          (equal (pml4-table-entry-addr (+ 7 lin-addr) pml4-table-base-addr)
                 (pml4-table-entry-addr lin-addr pml4-table-base-addr))
          (equal (pml4-table-entry-addr (+ 8 lin-addr) pml4-table-base-addr)
                 (pml4-table-entry-addr lin-addr pml4-table-base-addr)))
  :g-bindings
  (gl::auto-bindings
   (:mix (:nat pml4-table-base-addr 64) (:nat lin-addr 64))))

(def-gl-export same-pdp-table-entry-addr-for-8-lin-addrs
  :hyp (and (physical-address-p page-dir-ptr-table-base-addr)
            (canonical-address-p lin-addr)
            ;; 1G aligned linear address
            (equal (loghead 30 lin-addr) 0))
  :concl (and
          (equal (page-dir-ptr-table-entry-addr (+ 1 lin-addr) page-dir-ptr-table-base-addr)
                 (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr))
          (equal (page-dir-ptr-table-entry-addr (+ 2 lin-addr) page-dir-ptr-table-base-addr)
                 (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr))
          (equal (page-dir-ptr-table-entry-addr (+ 3 lin-addr) page-dir-ptr-table-base-addr)
                 (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr))
          (equal (page-dir-ptr-table-entry-addr (+ 4 lin-addr) page-dir-ptr-table-base-addr)
                 (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr))
          (equal (page-dir-ptr-table-entry-addr (+ 5 lin-addr) page-dir-ptr-table-base-addr)
                 (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr))
          (equal (page-dir-ptr-table-entry-addr (+ 6 lin-addr) page-dir-ptr-table-base-addr)
                 (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr))
          (equal (page-dir-ptr-table-entry-addr (+ 7 lin-addr) page-dir-ptr-table-base-addr)
                 (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr))
          (equal (page-dir-ptr-table-entry-addr (+ 8 lin-addr) page-dir-ptr-table-base-addr)
                 (page-dir-ptr-table-entry-addr lin-addr page-dir-ptr-table-base-addr)))
  :g-bindings
  (gl::auto-bindings
   (:mix (:nat page-dir-ptr-table-base-addr 64) (:nat lin-addr 64))))
...
(def-gl-export simplify-list-of-phy-addr-1
  :hyp (and (canonical-address-p lin-addr)
            (physical-address-p phy-addr)
            (equal (loghead 30 phy-addr) 0)
            (equal (loghead 30 lin-addr) 0))
  :concl (equal
          (list
           (logior (loghead 30 lin-addr) phy-addr)
           (logior (loghead 30 (+ 1 lin-addr)) phy-addr)
           (logior (loghead 30 (+ 2 lin-addr)) phy-addr)
           (logior (loghead 30 (+ 3 lin-addr)) phy-addr)
           (logior (loghead 30 (+ 4 lin-addr)) phy-addr)
           (logior (loghead 30 (+ 5 lin-addr)) phy-addr)
           (logior (loghead 30 (+ 6 lin-addr)) phy-addr)
           (logior (loghead 30 (+ 7 lin-addr)) phy-addr))
          (addr-range 8 phy-addr))
  :g-bindings
  (gl::auto-bindings (:mix (:nat lin-addr 64) (:nat phy-addr 64))))

(def-gl-export simplify-list-of-phy-addr-2
  :hyp (and (canonical-address-p lin-addr)
            (unsigned-byte-p 64 value)
            (equal (loghead 30 lin-addr) 0))
  :concl (equal
          (list (ash (loghead 22 (logtail 30 value)) 30)
                (logior (loghead 30 (+ 1 lin-addr)) (ash (loghead 22 (logtail 30 value)) 30))
                (logior (loghead 30 (+ 2 lin-addr)) (ash (loghead 22 (logtail 30 value)) 30))
                (logior (loghead 30 (+ 3 lin-addr)) (ash (loghead 22 (logtail 30 value)) 30))
                (logior (loghead 30 (+ 4 lin-addr)) (ash (loghead 22 (logtail 30 value)) 30))
                (logior (loghead 30 (+ 5 lin-addr)) (ash (loghead 22 (logtail 30 value)) 30))
                (logior (loghead 30 (+ 6 lin-addr)) (ash (loghead 22 (logtail 30 value)) 30))
                (logior (loghead 30 (+ 7 lin-addr)) (ash (loghead 22 (logtail 30 value)) 30)))
          (addr-range 8 (ash (loghead 22 (logtail 30 value)) 30)))
  :g-bindings
  (gl::auto-bindings (:mix (:nat lin-addr 64) (:nat value 64))))
...
(defthm las-to-pas-values-1G-pages-and-wb-to-page-dir-ptr-table-entry-addr
  (b* ((pml4-table-base-addr (pml4-table-base-addr x86))
       ...
       (page-dir-ptr-table-entry
        (combine-bytes
         (mv-nth 1 (rb (create-canonical-address-list 8 page-dir-ptr-table-entry-addr) :r x86))))
       (l-addrs (create-canonical-address-list 8 lin-addr))
       (bytes (strip-cdrs addr-lst))
       (value (combine-bytes bytes)))
    (implies
     (and
     ...
     ;; "l-addrs" after the PDPTE corresponding to these "l-addrs"
     ;; have been modified --- the new PDPTE is "value".
     (and
      (equal (mv-nth 0 (las-to-pas l-addrs r-w-x cpl (mv-nth 1 (wb addr-lst x86))))
             nil)
      (equal (mv-nth 1 (las-to-pas l-addrs r-w-x cpl (mv-nth 1 (wb addr-lst x86))))
  ...
  (b* ((pml4-table-base-addr (pml4-table-base-addr x86))
    ...
    (implies
     (and
      ...
      (not (page-structure-marking-mode x86))
      (x86p x86))
     (and (equal (mv-nth 0 (rb l-addrs r-w-x (mv-nth 1 (wb addr-lst x86)))) nil)
          (equal (mv-nth 1 (rb l-addrs r-w-x (mv-nth 1 (wb addr-lst x86))))
                 (read-from-physical-memory
                  (addr-range 8 (ash (loghead 22 (logtail 30 value)) 30))
                  x86)))))
  :hints (("Goal"
           :use ((:instance read-from-physical-memory-and-wb-with-modified-1G-page-map-in-system-level-non-marking-mode))
           :do-not-induct t
           :do-not '(preprocess)
...
(defthm rewire_dst_to_src-after-the-copy-source-in-terms-of-read-from-physical-memory
  (implies (and
            (not (programmer-level-mode x86))
            (not (page-structure-marking-mode x86))
            (equal (cpl x86) 0)
            ...
            (not
             (mv-nth 0
                     (las-to-pas
                      (create-canonical-address-list 8 (xr :rgf *rdi* x86))
                      :r (cpl x86) x86))))
           (equal
            (mv-nth 1 (rb (create-canonical-address-list 8 (xr :rgf *rdi* x86)) :r x86))
            (read-from-physical-memory
             (mv-nth 1 (las-to-pas (create-canonical-address-list 8 (xr :rgf *rdi* x86)) :r (cpl x86) x86))
             x86)))
  :hints (("Goal"
           :do-not '(preprocess)
           :do-not-induct t
...
;; (defthm ia32e-la-to-pa-open-up-for-a-1G-direct-map-page-setup
;;   (implies (and
;;          ;; No errors encountered in the translation of lin-addr.
;;          (not (mv-nth 0 (ia32e-la-to-pa lin-addr r-w-x cpl x86)))
...
;;          ;;  (pml4-table-entry-addr lin-addr (pml4-table-base-addr x86)))
;;          (canonical-address-p
;;           (+ 7 (pml4-table-entry-addr lin-addr (pml4-table-base-addr x86))))

;;          ;; No errors encountered while translating the PML4TE linear addresses.
;;          (not (mv-nth 0 (las-to-pas
;;                          (create-canonical-address-list
;;                           8 (pml4-table-entry-addr
;;				lin-addr
;;				(pml4-table-base-addr x86)))
;;                          :r cpl x86)))
...
;;          ;; Direct Map for PML4E and PDPTE:
...
(defthmd ia32e-la-to-pa-open-up-for-a-1G-page-setup
  (implies (and
            (not (mv-nth 0 (ia32e-la-to-pa lin-addr r-w-x cpl x86)))
            (equal (loghead 30 lin-addr) 0)
            (canonical-address-p lin-addr)
...
(def-gl-export pml4-table-entry-for-a-1G-aligned-address
  :hyp (and (canonical-address-p lin-addr)
            (equal (loghead 30 lin-addr) 0)
            (physical-address-p base-addr)
            (unsigned-byte-p 30 n))
  :concl (equal (pml4-table-entry-addr (+ n lin-addr) base-addr)
                (pml4-table-entry-addr lin-addr base-addr))
  :g-bindings
  (gl::auto-bindings (:mix (:nat lin-addr 64) (:nat base-addr 64) (:nat n 64))))

(def-gl-export page-dir-ptr-table-entry-for-a-1G-aligned-address
  :hyp (and (canonical-address-p lin-addr)
            (equal (loghead 30 lin-addr) 0)
            (physical-address-p base-addr)
            (unsigned-byte-p 30 n))
  :concl (equal (page-dir-ptr-table-entry-addr (+ n lin-addr) base-addr)
                (page-dir-ptr-table-entry-addr lin-addr base-addr))
  :g-bindings
  (gl::auto-bindings (:mix (:nat lin-addr 64) (:nat base-addr 64) (:nat n 64))))

(def-gl-export loghead-30-of-1G-aligned-lin-addr-+-n
  :hyp (and (canonical-address-p lin-addr)
            (equal (loghead 30 lin-addr) 0)
            (unsigned-byte-p 30 n))
...
(defthmd ia32e-la-to-pa-+-n-open-up-for-a-1G-page-setup
  (implies (and
            (not (mv-nth 0 (ia32e-la-to-pa (+ n lin-addr) r-w-x cpl x86)))
            (equal (loghead 30 lin-addr) 0)
            (canonical-address-p lin-addr)
            (unsigned-byte-p 30 n)
            (equal
            ...
            (x86p x86))
           (equal
            (mv-nth 1 (ia32e-la-to-pa (+ n lin-addr) r-w-x cpl x86))
            (+
             n
             (ash
              (loghead
...
(defthmd las-to-pas-open-up-for-a-1G-page-setup
  (implies (and
            (not (mv-nth 0 (las-to-pas
              ...
              (loghead
               ...
               (logtail
                ...
                (rm-low-64
                 ...
                 x86)))
              30))))
  :hints (("Goal" :in-theory (e/d* (ia32e-la-to-pa-open-up-for-a-1G-page-setup
                                    ia32e-la-to-pa-+-n-open-up-for-a-1G-page-setup
                                    las-to-pas
                                    open-mv-nth-1-las-to-pas
                                    open-mv-nth-0-las-to-pas)
                                   (page-dir-ptr-table-entry-addr-to-c-program-optimized-form
...
(defthm rewire_dst_to_src-after-the-copy-source-p-addrs-open
  (implies (and
            ...
            (not
             (mv-nth 0
                     (las-to-pas (create-canonical-address-list 8 (xr :rgf *rdi* x86))
                                 :r (cpl x86) x86)))
           ...
           (equal
            (mv-nth 1 (las-to-pas (create-canonical-address-list 8 (xr :rgf *rdi* x86)) :r cpl x86))
            (addr-range 8
                        (ash (loghead 22
                                      (logtail 30
                                               (rm-low-64
 ...
 ;; See preconditions.lsp for a discussion about the pre-conditions
;; (acl2::why rb-wb-equal-with-modified-1G-page-map-in-system-level-non-marking-mode)
;; (acl2::why la-to-pas-values-and-mv-nth-1-wb)
...
(defthm rewire_dst_to_src-after-the-copy-destination==source
  (implies (and
            ...
            (disjoint-p
             (all-translation-governing-addresses
              (create-canonical-address-list 8 (xr :rgf *rsi* x86)) x86)
             (mv-nth 1 (las-to-pas
                        (create-canonical-address-list 8 (+ -24 (xr :rgf *rsp* x86))) :w (cpl x86) x86)))
            ...
            (not
             (mv-nth 0
                     (las-to-pas (create-canonical-address-list 8 (xr :rgf *rsi* x86))
                                 :r (cpl x86) x86)))
           ...
           (equal
            ...
            (mv-nth 1 (rb  (create-canonical-address-list 8 (xr :rgf *rsi* x86))
                           :r (x86-run (rewire_dst_to_src-clk) x86)))
            ;; Source, before the copy:
            (mv-nth 1 (rb (create-canonical-address-list 8 (xr :rgf *rdi* x86))
                          :r x86))))
  :hints (("Goal"
           :do-not '(preprocess)
           :do-not-induct t
...
;; (mv-nth 1 (rb l-addrs :r
;;               (mv-nth 1 (wb (create-addr-bytes-alist
;;                              (addr-range 8 page-dir-ptr-table-entry-addr-for-l-addrs)
;;                              bytes)
;;                             x86))))



;; (defthm rewire_dst_to_src-destination-pdpte-after-the-copy
;;   (implies (and
;;             (equal prog-len (len *rewire_dst_to_src*))
;;             (x86p x86)
;;             (not (programmer-level-mode x86))
...
;;            (equal
;;             ;; Destination PDPTE, after the copy:
;;             (mv-nth 1 (rb
;;                        (create-canonical-address-list
;;                         8
;;                         (page-dir-ptr-table-entry-addr
