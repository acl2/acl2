...
(defun generate-x86-components-equiv-relations-aux (keyword)
  ...
  (b* ((name (mk-name keyword))
    ...
    `((define ,equiv-name (x86-1 x86-2)
        ...
        (if (system-level-state-p x86-1)
            (if (system-level-state-p x86-2)
                (,equiv-aux-name ,len x86-1 x86-2)
              nil)
          (not (system-level-state-p x86-2)))
        ...
        (defthm ,(mk-name equiv-name "-OPEN")
          (implies (and (bind-free
                        ...
                        (,equiv-name (double-rewrite x86-1) x86-2)
                        (natp j)
                        (< j ,len))
                   (equal (xr ,keyword j x86-1) (xr ,keyword j x86-2))))
        ...
        (defthm ,(mk-name equiv-name "-AND-XW-1")
          (implies (and (not (equal fld ,keyword))
                        (system-level-state-p x)
                        (system-level-state-p (xw fld index val x)))
        ...
        (defthm ,(mk-name equiv-name "-AND-XW-2")
          (implies (and (not (equal fld ,keyword))
                        ...
                        (system-level-state-p (xw fld index val y)))
...
(define all-mem-except-paging-structures-equal
  ...
  (defthm all-mem-except-paging-structures-equal-and-xw-mem-except-paging-structure
    (implies (and (bind-free (find-equiv-x86-for-components y mfc state))
                  ...
                  (x86p (xw :mem index val x))
                  (x86p (xw :mem index val y)))
...
(define xlate-equiv-structures (x86-1 x86-2)
  ...
  (defthm xlate-equiv-structures-and-xw
    (implies (and (not (equal fld :mem))
                  ...
                  (not (equal fld :programmer-level-mode))
                  (system-level-state-p (double-rewrite x86))
                  (system-level-state-p (xw fld index val x86)))
...
(define xlate-equiv-x86s (x86-1 x86-2)
  ...
  two states must be exactly equal.</p>"
  ...
  (defthm xlate-equiv-x86s-and-xw
    (implies (and
              ...
              (system-level-state-p (double-rewrite x86-1))
              (system-level-state-p (xw fld index val x86-1))
              (system-level-state-p (xw fld index val x86-2)))
...
(defthm xlate-equiv-structures-and-system-level-state-p
  (implies (xlate-equiv-structures x86-1 x86-2)
           (equal (system-level-state-p x86-1)
                  (system-level-state-p x86-2)))
  :hints (("Goal" :in-theory (e/d* (xlate-equiv-structures) ())))
  :rule-classes :congruence)

(defthm xlate-equiv-x86s-and-system-level-state-p
