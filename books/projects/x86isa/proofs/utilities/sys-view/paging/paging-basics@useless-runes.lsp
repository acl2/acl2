(X86ISA::LOGHEAD-1-BOOL->BIT-RULE
     (14 2 (:LINEAR LOGHEAD-UPPER-BOUND))
     (3 3
        (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR)))
(X86ISA::LOW-3-BITS-0-ADD-7-PRESERVE-BOUND
     (624 624
          (:TYPE-PRESCRIPTION NOT-INTEGERP-3B))
     (624 624
          (:TYPE-PRESCRIPTION NOT-INTEGERP-2B))
     (624 624
          (:TYPE-PRESCRIPTION NOT-INTEGERP-1B))
     (503 9 (:REWRITE CANCEL-MOD-+))
     (270 54 (:TYPE-PRESCRIPTION MOD-ZERO . 2))
     (255 38 (:REWRITE INTEGERP-MINUS-X))
     (234 167 (:REWRITE DEFAULT-TIMES-2))
     (217 9 (:REWRITE MOD-X-Y-=-X . 4))
     (216 9 (:REWRITE MOD-X-Y-=-X . 3))
     (210 9 (:REWRITE MOD-X-Y-=-X+Y . 2))
     (208 9 (:REWRITE MOD-ZERO . 3))
     (198 54
          (:TYPE-PRESCRIPTION MOD-POSITIVE . 2))
     (198 54
          (:TYPE-PRESCRIPTION MOD-NEGATIVE . 2))
     (176 167 (:REWRITE DEFAULT-TIMES-1))
     (161 18 (:REWRITE |(* (- x) y)|))
     (138 9 (:REWRITE MOD-X-Y-=-X-Y . 2))
     (127 107
          (:TYPE-PRESCRIPTION NOT-INTEGERP-4A))
     (110 9 (:REWRITE MOD-ZERO . 4))
     (107 107
          (:TYPE-PRESCRIPTION NOT-INTEGERP-3A))
     (107 107
          (:TYPE-PRESCRIPTION NOT-INTEGERP-2A))
     (107 107
          (:TYPE-PRESCRIPTION NOT-INTEGERP-1A))
     (106 12
          (:REWRITE SIMPLIFY-PRODUCTS-GATHER-EXPONENTS-EQUAL))
     (106 12
          (:REWRITE PREFER-POSITIVE-ADDENDS-EQUAL))
     (105 9 (:REWRITE DEFAULT-MOD-RATIO))
     (96 3 (:REWRITE MOD-X-Y-=-X-Y . 1))
     (94 18 (:REWRITE DEFAULT-MINUS))
     (92 17
         (:REWRITE REDUCE-ADDITIVE-CONSTANT-EQUAL))
     (85 18 (:REWRITE |(- (* c x))|))
     (82 82
         (:REWRITE NORMALIZE-FACTORS-GATHER-EXPONENTS))
     (78 3 (:REWRITE MOD-ZERO . 1))
     (71 3 (:LINEAR MOD-BOUNDS-3))
     (70 4 (:REWRITE IFIX-NEGATIVE-TO-NEGP))
     (66 2 (:REWRITE IFIX-POSITIVE-TO-NON-ZP))
     (62 4 (:REWRITE NEGP-WHEN-INTEGERP))
     (62 2 (:REWRITE ZP-WHEN-INTEGERP))
     (54 54 (:TYPE-PRESCRIPTION MOD-ZERO . 3))
     (54 54
         (:TYPE-PRESCRIPTION MOD-POSITIVE . 1))
     (54 54
         (:TYPE-PRESCRIPTION MOD-NEGATIVE . 1))
     (53 53 (:TYPE-PRESCRIPTION MOD-NONPOSITIVE))
     (53 53 (:TYPE-PRESCRIPTION INTEGERP-MOD-2))
     (49 47 (:REWRITE DEFAULT-PLUS-1))
     (49 41 (:REWRITE DEFAULT-LESS-THAN-1))
     (49 9 (:REWRITE MOD-X-Y-=-X . 2))
     (49 9 (:REWRITE |(mod (+ x (mod a b)) y)|))
     (49 9
         (:REWRITE |(mod (+ x (- (mod a b))) y)|))
     (49 3 (:REWRITE MOD-X-Y-=-X+Y . 1))
     (47 47 (:REWRITE DEFAULT-PLUS-2))
     (47 9 (:REWRITE MOD-X-Y-=-X-Y . 3))
     (47 9 (:REWRITE MOD-X-Y-=-X+Y . 3))
     (47 9
         (:REWRITE MOD-CANCEL-*-REWRITING-GOAL-LITERAL))
     (47 9 (:REWRITE MOD-CANCEL-*-CONST))
     (47 9
         (:REWRITE |(mod (* x (/ y)) z) rewriting-goal-literal|))
     (43 41 (:REWRITE DEFAULT-LESS-THAN-2))
     (41 41 (:REWRITE THE-FLOOR-BELOW))
     (41 41 (:REWRITE THE-FLOOR-ABOVE))
     (41 31 (:REWRITE PREFER-POSITIVE-ADDENDS-<))
     (39 3 (:TYPE-PRESCRIPTION FLOOR-ZERO . 2))
     (38 36
         (:REWRITE REDUCE-RATIONAL-MULTIPLICATIVE-CONSTANT-<))
     (34 34 (:REWRITE CONSTANT-<-INTEGERP))
     (34 34
         (:REWRITE |(< c (/ x)) positive c --- present in goal|))
     (34 34
         (:REWRITE |(< c (/ x)) positive c --- obj t or nil|))
     (34 34
         (:REWRITE |(< c (/ x)) negative c --- present in goal|))
     (34 34
         (:REWRITE |(< c (/ x)) negative c --- obj t or nil|))
     (34 34 (:REWRITE |(< c (- x))|))
     (34 34
         (:REWRITE |(< (/ x) c) positive c --- present in goal|))
     (34 34
         (:REWRITE |(< (/ x) c) positive c --- obj t or nil|))
     (34 34
         (:REWRITE |(< (/ x) c) negative c --- present in goal|))
     (34 34
         (:REWRITE |(< (/ x) c) negative c --- obj t or nil|))
     (34 34 (:REWRITE |(< (/ x) (/ y))|))
     (34 34 (:REWRITE |(< (- x) c)|))
     (34 34 (:REWRITE |(< (- x) (- y))|))
     (32 6 (:LINEAR MOD-BOUNDS-2))
     (30 1
         (:REWRITE |(<= (/ x) y) with (< x 0)|))
     (30 1 (:REWRITE |(< x (/ y)) with (< y 0)|))
     (29 29 (:REWRITE REDUCE-INTEGERP-+))
     (29 29 (:META META-INTEGERP-CORRECT))
     (27 3
         (:TYPE-PRESCRIPTION FLOOR-NONPOSITIVE . 1))
     (27 3
         (:TYPE-PRESCRIPTION FLOOR-NEGATIVE . 1))
     (21 21 (:REWRITE REMOVE-WEAK-INEQUALITIES))
     (17 17 (:TYPE-PRESCRIPTION RATIONALP-MOD))
     (17 17
         (:REWRITE REDUCE-MULTIPLICATIVE-CONSTANT-EQUAL))
     (15 3 (:TYPE-PRESCRIPTION FLOOR-ZERO . 4))
     (15 3 (:TYPE-PRESCRIPTION FLOOR-ZERO . 3))
     (15 3 (:TYPE-PRESCRIPTION FLOOR-ZERO . 1))
     (15 3
         (:TYPE-PRESCRIPTION FLOOR-POSITIVE . 3))
     (15 3
         (:TYPE-PRESCRIPTION FLOOR-NONPOSITIVE . 3))
     (15 3
         (:TYPE-PRESCRIPTION FLOOR-NONPOSITIVE . 2))
     (15 3
         (:TYPE-PRESCRIPTION FLOOR-NEGATIVE . 3))
     (15 3
         (:TYPE-PRESCRIPTION FLOOR-NEGATIVE . 2))
     (15 3 (:REWRITE MOD-ZERO . 2))
     (13 13 (:REWRITE |(equal c (/ x))|))
     (13 13 (:REWRITE |(equal (/ x) (/ y))|))
     (13 13 (:REWRITE |(equal (- x) (- y))|))
     (12 12 (:REWRITE SIMPLIFY-SUMS-EQUAL))
     (12 12
         (:REWRITE EQUAL-OF-PREDICATES-REWRITE))
     (12 12 (:REWRITE |(equal c (- x))|))
     (12 12 (:REWRITE |(equal (- x) c)|))
     (11 9 (:REWRITE DEFAULT-MOD-1))
     (9 9
        (:REWRITE THIS-NEEDS-TO-BE-ADDED-TO-QUOTIENT-REMAINDER-LEMMAS))
     (9 9
        (:REWRITE SIMPLIFY-TERMS-SUCH-AS-AX+BX-<-0-RATIONAL-REMAINDER))
     (9 9
        (:REWRITE SIMPLIFY-TERMS-SUCH-AS-AX+BX-<-0-RATIONAL-COMMON))
     (9 9
        (:REWRITE MOD-CANCEL-*-NOT-REWRITING-GOAL-LITERAL))
     (9 9 (:REWRITE DEFAULT-MOD-2))
     (9 9 (:REWRITE |(mod x (- y))| . 3))
     (9 9 (:REWRITE |(mod x (- y))| . 2))
     (9 9 (:REWRITE |(mod x (- y))| . 1))
     (9 9
        (:REWRITE |(mod x (* y (/ z))) rewriting-goal-literal|))
     (9 9
        (:REWRITE |(mod x (* y (/ z))) not rewriting-goal-literal|))
     (9 9 (:REWRITE |(mod (- x) y)| . 3))
     (9 9 (:REWRITE |(mod (- x) y)| . 2))
     (9 9 (:REWRITE |(mod (- x) y)| . 1))
     (9 9
        (:REWRITE |(mod (* x (/ y)) z) not rewriting-goal-literal|))
     (9 9 (:REWRITE |(< (/ x) 0)|))
     (9 9 (:REWRITE |(< (* x y) 0)|))
     (8 8
        (:REWRITE SIMPLIFY-TERMS-SUCH-AS-AX+BX-=-0))
     (8 8
        (:REWRITE NORMALIZE-TERMS-SUCH-AS-A/A+B-+-B/A+B))
     (7 7 (:REWRITE |(< 0 (* x y))|))
     (6 6 (:REWRITE |(< 0 (/ x))|))
     (6 2 (:REWRITE IFIX-EQUAL-TO-0))
     (5 5
        (:REWRITE SIMPLIFY-TERMS-SUCH-AS-0-<-AX+BX-RATIONAL-REMAINDER))
     (5 5
        (:REWRITE SIMPLIFY-TERMS-SUCH-AS-0-<-AX+BX-RATIONAL-COMMON))
     (4 4 (:REWRITE NEGP-WHEN-LESS-THAN-0))
     (3 3
        (:REWRITE |(equal (mod (+ x y) z) x)|))
     (2 2 (:TYPE-PRESCRIPTION ZIP))
     (2 2 (:TYPE-PRESCRIPTION BUBBLE-DOWN))
     (2 2 (:REWRITE ZP-WHEN-GT-0))
     (2 2 (:REWRITE ZIP-OPEN))
     (2 2
        (:REWRITE IFIX-EQUAL-TO-NONZERO-CONST))
     (2 2 (:REWRITE IFIX-EQUAL-TO-NONZERO))
     (2 2 (:REWRITE |(< (+ c/d x) y)|))
     (1 1 (:REWRITE IFIX-WHEN-NOT-INTEGERP))
     (1 1 (:REWRITE DEFAULT-DIVIDE))
     (1 1 (:REWRITE |(not (equal x (/ y)))|))
     (1 1 (:REWRITE |(equal x (/ y))|))
     (1 1
        (:REWRITE |(<= (/ x) y) with (< 0 x)|))
     (1 1 (:REWRITE |(< (+ (- c) x) y)|)))
(X86ISA::RM-LOW-64-LOGAND-LOGIOR-HELPER-1
 (176 2
      (:REWRITE UNSIGNED-BYTE-P-OF-LOGAND-2))
 (165 11
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PTE-4K-PAGEBITS-P))
 (142 7
      (:REWRITE X86ISA::IA32E-PTE-4K-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (138 4 (:REWRITE DIFFERENCE-UNSIGNED-BYTE-P))
 (60 1 (:LINEAR BITOPS::LOGIOR-<-0-LINEAR-2))
 (44 44 (:TYPE-PRESCRIPTION BITMASKP$INLINE))
 (41 31 (:REWRITE DEFAULT-<-1))
 (39 11
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-XCR0BITS-P))
 (39 11
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PML4EBITS-P))
 (39 11
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-PG-DIRBITS-P))
 (39 11
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-1GB-PAGEBITS-P))
 (39 11
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-PG-TABLEBITS-P))
 (39 11
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-2MB-PAGEBITS-P))
 (39 11
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PAGE-TABLESBITS-P))
 (39 11
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTORBITS-P))
 (39 11
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR3BITS-P))
 (39 11
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTORBITS-P))
 (39 11
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-64BITS-P))
 (36 14
     (:REWRITE BITOPS::LOGAND-WITH-NEGATED-BITMASK))
 (36 14
     (:REWRITE BITOPS::LOGAND-WITH-BITMASK))
 (31 31 (:REWRITE DEFAULT-<-2))
 (26 2
     (:LINEAR BITOPS::LOGAND->=-0-LINEAR-1))
 (16 16
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PTE-4K-PAGEBITS-P))
 (14 14
     (:TYPE-PRESCRIPTION X86ISA::XCR0BITS-P$INLINE))
 (14 14
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PML4EBITS-P))
 (14 14
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-PG-DIRBITS-P))
 (14 14
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P))
 (14 14
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-PG-TABLEBITS-P))
 (14 14
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-2MB-PAGEBITS-P))
 (14 14
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS-P))
 (14 14
     (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P))
 (14 14
     (:TYPE-PRESCRIPTION X86ISA::CR3BITS-P$INLINE))
 (14 14
     (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P))
 (14 14
     (:TYPE-PRESCRIPTION X86ISA::64BITS-P))
 (14 10 (:REWRITE IFIX-WHEN-NOT-INTEGERP))
 (14 7
     (:REWRITE X86ISA::XCR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (14 7
     (:REWRITE X86ISA::IA32E-PML4EBITS-P-WHEN-UNSIGNED-BYTE-P))
 (14 7
     (:REWRITE X86ISA::IA32E-PDPTE-PG-DIRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (14 7
     (:REWRITE X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (14 7
     (:REWRITE X86ISA::IA32E-PDE-PG-TABLEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (14 7
     (:REWRITE X86ISA::IA32E-PDE-2MB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (14 7
     (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (14 7
     (:REWRITE X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (14 7
     (:REWRITE X86ISA::CR3BITS-P-WHEN-UNSIGNED-BYTE-P))
 (14 7
     (:REWRITE X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (14 7
     (:REWRITE X86ISA::64BITS-P-WHEN-UNSIGNED-BYTE-P))
 (12 2
     (:LINEAR BITOPS::LOGAND->=-0-LINEAR-2))
 (12 2 (:LINEAR BITOPS::LOGAND-<-0-LINEAR))
 (10
   10
   (:REWRITE X86ISA::WEED-OUT-IRRELEVANT-LOGAND-WHEN-FIRST-OPERAND-CONSTANT))
 (10 10
     (:REWRITE X86ISA::NEGATIVE-LOGAND-TO-POSITIVE-LOGAND-WITH-INTEGERP-X))
 (10 10 (:REWRITE X86ISA::LOGAND-REDUNDANT))
 (9 3 (:REWRITE NATP-WHEN-GTE-0))
 (6 1 (:LINEAR BITOPS::LOGIOR-<-0-LINEAR-1))
 (5 5 (:REWRITE DEFAULT-UNARY-MINUS))
 (5 5 (:REWRITE DEFAULT-+-2))
 (5 5 (:REWRITE DEFAULT-+-1))
 (4 2
    (:TYPE-PRESCRIPTION X86ISA::RM-LOW-64-LOGAND-LOGIOR-HELPER-1))
 (3 3 (:REWRITE NATP-WHEN-INTEGERP))
 (3 3
    (:LINEAR BITOPS::UPPER-BOUND-OF-LOGAND . 2))
 (1 1
    (:LINEAR BITOPS::UPPER-BOUND-OF-LOGIOR-FOR-NATURALS)))
(X86ISA::PML4-TABLE-BASE-ADDR)
(X86ISA::N52P-OF-PML4-TABLE-BASE-ADDR
 (11 1 (:REWRITE ASH-0))
 (7 1 (:REWRITE ZIP-OPEN))
 (2 1 (:REWRITE RIGHT-SHIFT-TO-LOGTAIL))
 (1 1 (:TYPE-PRESCRIPTION ZIP))
 (1 1 (:TYPE-PRESCRIPTION IFIX))
 (1 1
    (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR))
 (1 1
    (:REWRITE X86ISA::CR3BITS->PDB-OF-WRITE-WITH-MASK))
 (1 1
    (:REWRITE X86ISA::CR3BITS->PDB$INLINE-OF-CR3BITS-FIX-X-NORMALIZE-CONST)))
(X86ISA::PML4-TABLE-BASE-ADDR-AND-MV-NTH-1-WB)
(X86ISA::PAGE-DIR-PTR-TABLE-BASE-ADDR)
(X86ISA::PAGE-DIRECTORY-BASE-ADDR)
(X86ISA::PAGE-TABLE-BASE-ADDR)
(X86ISA::LOGHEAD-SMALLER-EQUALITY
     (6086 47 (:REWRITE LOGHEAD-IDENTITY))
     (4808 147 (:DEFINITION UNSIGNED-BYTE-P**))
     (2654 74 (:DEFINITION INTEGER-RANGE-P))
     (2550 67 (:DEFINITION UNSIGNED-BYTE-P))
     (2476 174 (:REWRITE UNSIGNED-BYTE-P-PLUS))
     (2389 147
           (:REWRITE FTY::BITP-WHEN-UNSIGNED-BYTE-P-1))
     (1807 890
           (:TYPE-PRESCRIPTION BITOPS::LOGCDR-NATP))
     (1662 154 (:REWRITE BITOPS::LOGCDR-OF-BIT))
     (1643 53 (:REWRITE BITOPS::LOGCAR-OF-BIT))
     (1227 60
           (:REWRITE BITOPS::UNSIGNED-BYTE-P-OF-0-X))
     (1138 1138
           (:TYPE-PRESCRIPTION UNSIGNED-BYTE-P))
     (890 890 (:TYPE-PRESCRIPTION LOGCDR-TYPE))
     (864 7 (:REWRITE X86ISA::LOGHEAD-UNEQUAL))
     (488 322 (:REWRITE DEFAULT-<-2))
     (480 80 (:REWRITE BITOPS::LOGCDR-<-CONST))
     (448 322 (:REWRITE DEFAULT-<-1))
     (431 3
          (:REWRITE BITOPS::SIGNED-BYTE-P-OF-LOGCDR))
     (428 428
          (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR))
     (421 421 (:TYPE-PRESCRIPTION BITP))
     (420 420 (:LINEAR BITOPS::LOGCDR-<=-LOGCDR))
     (395 7 (:DEFINITION SIGNED-BYTE-P**))
     (393 7 (:DEFINITION SIGNED-BYTE-P))
     (254 67
          (:REWRITE FTY::UNSIGNED-BYTE-P-1-WHEN-BITP))
     (240 40 (:REWRITE IFIX-NEGATIVE-TO-NEGP))
     (230 78 (:REWRITE ZP-WHEN-GT-0))
     (214 214
          (:REWRITE EXPT-WITH-VIOLATED-GUARDS))
     (156 4 (:REWRITE BFIX-WHEN-NOT-1))
     (132 4 (:LINEAR BITOPS::LOGCAR-BOUND))
     (120 40 (:REWRITE NEGP-WHEN-LESS-THAN-0))
     (114 44
          (:TYPE-PRESCRIPTION BITOPS::LOGCONS-POSP-1))
     (107 47
          (:REWRITE X86ISA::LOGHEAD-OF-NON-INTEGERP))
     (104 104 (:REWRITE DEFAULT-+-2))
     (104 104 (:REWRITE DEFAULT-+-1))
     (88 44
         (:TYPE-PRESCRIPTION BITOPS::LOGCONS-POSP-2))
     (75 23 (:REWRITE FOLD-CONSTS-IN-+))
     (74 74
         (:LINEAR BITOPS::EXPT-2-LOWER-BOUND-BY-LOGBITP))
     (70 70
         (:LINEAR X86ISA::SIGNED-BYTE-P-OF-TRUNCATE-WHEN-Y-IS-POSITIVE))
     (70 70
         (:LINEAR X86ISA::SIGNED-BYTE-P-OF-TRUNCATE-WHEN-Y-IS-NEGATIVE))
     (70 70
         (:LINEAR X86ISA::SIGNED-BYTE-P-OF-TRUNCATE))
     (70 70
         (:LINEAR X86ISA::SIGNED-BYTE-P-OF-REM))
     (52 52
         (:TYPE-PRESCRIPTION BIT->BOOL$INLINE))
     (51 47 (:REWRITE IFIX-WHEN-NOT-INTEGERP))
     (49 7 (:REWRITE POSP-REDEFINITION))
     (47 47
         (:REWRITE X86ISA::LOGHEAD-ZERO-SMALLER))
     (44 44 (:TYPE-PRESCRIPTION POSP))
     (40 40 (:TYPE-PRESCRIPTION NEGP))
     (40 40 (:REWRITE NEGP-WHEN-INTEGERP))
     (35 35 (:TYPE-PRESCRIPTION SIGNED-BYTE-P))
     (21 7 (:REWRITE DEFAULT-UNARY-MINUS))
     (17 17 (:REWRITE ZP-OPEN))
     (17 17
         (:REWRITE BITOPS::SIGNED-BYTE-P-MONOTONICITY))
     (16 16 (:REWRITE NATP-WHEN-INTEGERP))
     (14 14
         (:REWRITE BITOPS::SIGNED-BYTE-P-INCR))
     (9 3 (:REWRITE IFIX-EQUAL-TO-0))
     (8 4 (:REWRITE BFIX-WHEN-NOT-BITP))
     (8 4 (:REWRITE BFIX-WHEN-NOT-BIT->BOOL))
     (8 4 (:REWRITE BFIX-WHEN-BIT->BOOL))
     (6 6 (:REWRITE IFIX-EQUAL-TO-NONZERO))
     (6 3 (:REWRITE UNICITY-OF-0))
     (4 4
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (3 3 (:TYPE-PRESCRIPTION ZIP))
     (3 3 (:REWRITE ZIP-OPEN))
     (3 3
        (:REWRITE IFIX-EQUAL-TO-NONZERO-CONST))
     (3 3 (:DEFINITION FIX))
     (2 2
        (:REWRITE BITOPS::LOGCONS-EQUAL-CONSTANT)))
(X86ISA::LOGBITP-N-OF-SET-ACCESSED-BIT
 (1564 26 (:REWRITE LOGHEAD-IDENTITY))
 (1138 39
       (:REWRITE FTY::BITP-WHEN-UNSIGNED-BYTE-P-1))
 (706 26
      (:REWRITE BITOPS::LOGBITP-NONZERO-OF-BIT))
 (562 17 (:REWRITE BITOPS::LOGBITP-WHEN-BIT))
 (131 118 (:REWRITE DEFAULT-<-1))
 (118 118 (:REWRITE DEFAULT-<-2))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-XCR0BITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PTE-4K-PAGEBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PML4EBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-PG-DIRBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-1GB-PAGEBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-PG-TABLEBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-2MB-PAGEBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PAGE-TABLESBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTORBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR3BITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTORBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-64BITS-P))
 (109 109 (:TYPE-PRESCRIPTION BITP))
 (107 32 (:REWRITE BITOPS::LOGBITP-OF-MASK))
 (87 32
     (:REWRITE BITOPS::LOGBITP-WHEN-BITMASKP))
 (87 32 (:REWRITE BITOPS::LOGBITP-OF-CONST))
 (82 33
     (:REWRITE FTY::UNSIGNED-BYTE-P-1-WHEN-BITP))
 (80 20
     (:TYPE-PRESCRIPTION BITOPS::ASH-NATP-TYPE))
 (63 63 (:TYPE-PRESCRIPTION BITMASKP$INLINE))
 (59 59
     (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR))
 (50 10
     (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::XCR0BITS-P$INLINE))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PTE-4K-PAGEBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PML4EBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-PG-DIRBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-PG-TABLEBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-2MB-PAGEBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::CR3BITS-P$INLINE))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::64BITS-P))
 (46 23
     (:REWRITE X86ISA::XCR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PTE-4K-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PML4EBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PDPTE-PG-DIRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PDE-PG-TABLEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PDE-2MB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::CR3BITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::64BITS-P-WHEN-UNSIGNED-BYTE-P))
 (43 7
     (:REWRITE BITOPS::LOGBITP-OF-LOGHEAD-OUT-OF-BOUNDS))
 (43 7
     (:REWRITE BITOPS::LOGBITP-OF-LOGHEAD-IN-BOUNDS))
 (32 32
     (:REWRITE BITOPS::NORMALIZE-LOGBITP-WHEN-MODS-EQUAL))
 (32 32
     (:REWRITE BITOPS::LOGBITP-OF-NEGATIVE-CONST))
 (31 31 (:REWRITE NFIX-WHEN-NOT-NATP))
 (26 26
     (:REWRITE X86ISA::LOGHEAD-ZERO-SMALLER))
 (26 26
     (:REWRITE X86ISA::LOGHEAD-OF-NON-INTEGERP))
 (25 25
     (:TYPE-PRESCRIPTION
          BITOPS::INTEGER-LENGTH-TYPE-PRESCRIPTION-STRONG-NEGATIVE))
 (25 25
     (:TYPE-PRESCRIPTION BITOPS::INTEGER-LENGTH-TYPE-PRESCRIPTION-STRONG))
 (25 25 (:TYPE-PRESCRIPTION INTEGER-LENGTH))
 (24 3 (:REWRITE BFIX-WHEN-NOT-1))
 (14 1 (:REWRITE BITOPS::LOGBITP-0-OF-BIT))
 (13 13 (:REWRITE UNSIGNED-BYTE-P-LOGHEAD))
 (12 4 (:REWRITE ZP-WHEN-GT-0))
 (11 11 (:REWRITE NATP-WHEN-INTEGERP))
 (10 10
     (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NATP))
 (9 1 (:REWRITE EQUAL-1-OF-BOOL->BIT))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE1-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX2-BYTE1-P))
 (8
  1
  (:REWRITE
    X86ISA::UNSIGNED-BYTE-P-WHEN-SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SIB-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SEGMENT-SELECTORBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-RFLAGSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MXCSRBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MODR/M-P))
 (8
  1
  (:REWRITE
   X86ISA::UNSIGNED-BYTE-P-WHEN-INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-FP-STATUSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-PREFIXES-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE3-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE1-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR0BITS-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE
         X86ISA::UNSIGNED-BYTE-P-WHEN-CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-8BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-32BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-16BITS-P))
 (7 3
    (:REWRITE BITOPS::LOGAND-WITH-NEGATED-BITMASK))
 (7 3
    (:REWRITE BITOPS::LOGAND-WITH-BITMASK))
 (6 6 (:TYPE-PRESCRIPTION BIT->BOOL$INLINE))
 (6 3 (:REWRITE BFIX-WHEN-NOT-BITP))
 (6 3 (:REWRITE BFIX-WHEN-NOT-BIT->BOOL))
 (6 3 (:REWRITE BFIX-WHEN-BIT->BOOL))
 (5 5 (:TYPE-PRESCRIPTION LOGNOT))
 (3 3
    (:REWRITE BITOPS::B-AND-EQUAL-1-IN-HYP))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX2-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SIB-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SEGMENT-SELECTORBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::RFLAGSBITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MXCSRBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MODR/M-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION
         X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::FP-STATUSBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-PREFIXES-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE3-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CR0BITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::8BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::32BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::16BITS-P))
 (2
   2
   (:REWRITE X86ISA::WEED-OUT-IRRELEVANT-LOGAND-WHEN-FIRST-OPERAND-CONSTANT))
 (2 2 (:REWRITE X86ISA::LOGAND-REDUNDANT))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX2-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
    X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SIB-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SEGMENT-SELECTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::RFLAGSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MXCSRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MODR/M-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
   X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1 (:REWRITE IFIX-WHEN-NOT-INTEGERP))
 (2 1
    (:REWRITE X86ISA::FP-STATUSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-PREFIXES-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE3-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::CR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE
         X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::8BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::32BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::16BITS-P-WHEN-UNSIGNED-BYTE-P)))
(X86ISA::LOGBITP-N-OF-SET-DIRTY-BIT
 (1536 24 (:REWRITE LOGHEAD-IDENTITY))
 (1079 32
       (:REWRITE FTY::BITP-WHEN-UNSIGNED-BYTE-P-1))
 (678 24
      (:REWRITE BITOPS::LOGBITP-NONZERO-OF-BIT))
 (534 15 (:REWRITE BITOPS::LOGBITP-WHEN-BIT))
 (131 118 (:REWRITE DEFAULT-<-1))
 (118 118 (:REWRITE DEFAULT-<-2))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-XCR0BITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PTE-4K-PAGEBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PML4EBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-PG-DIRBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-1GB-PAGEBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-PG-TABLEBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-2MB-PAGEBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PAGE-TABLESBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTORBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR3BITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTORBITS-P))
 (115 23
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-64BITS-P))
 (105 30 (:REWRITE BITOPS::LOGBITP-OF-MASK))
 (90 90 (:TYPE-PRESCRIPTION BITP))
 (85 30 (:REWRITE BITOPS::LOGBITP-OF-CONST))
 (81 30
     (:REWRITE BITOPS::LOGBITP-WHEN-BITMASKP))
 (80 20
     (:TYPE-PRESCRIPTION BITOPS::ASH-NATP-TYPE))
 (62 26
     (:REWRITE FTY::UNSIGNED-BYTE-P-1-WHEN-BITP))
 (59 59 (:TYPE-PRESCRIPTION BITMASKP$INLINE))
 (52 52
     (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR))
 (50 10
     (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::XCR0BITS-P$INLINE))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PTE-4K-PAGEBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PML4EBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-PG-DIRBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-PG-TABLEBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-2MB-PAGEBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::CR3BITS-P$INLINE))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P))
 (46 46
     (:TYPE-PRESCRIPTION X86ISA::64BITS-P))
 (46 23
     (:REWRITE X86ISA::XCR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PTE-4K-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PML4EBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PDPTE-PG-DIRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PDE-PG-TABLEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PDE-2MB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::CR3BITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (46 23
     (:REWRITE X86ISA::64BITS-P-WHEN-UNSIGNED-BYTE-P))
 (43 7
     (:REWRITE BITOPS::LOGBITP-OF-LOGHEAD-OUT-OF-BOUNDS))
 (43 7
     (:REWRITE BITOPS::LOGBITP-OF-LOGHEAD-IN-BOUNDS))
 (30 30
     (:REWRITE BITOPS::NORMALIZE-LOGBITP-WHEN-MODS-EQUAL))
 (30 30
     (:REWRITE BITOPS::LOGBITP-OF-NEGATIVE-CONST))
 (28 28 (:REWRITE NFIX-WHEN-NOT-NATP))
 (25 25
     (:TYPE-PRESCRIPTION
          BITOPS::INTEGER-LENGTH-TYPE-PRESCRIPTION-STRONG-NEGATIVE))
 (25 25
     (:TYPE-PRESCRIPTION BITOPS::INTEGER-LENGTH-TYPE-PRESCRIPTION-STRONG))
 (25 25 (:TYPE-PRESCRIPTION INTEGER-LENGTH))
 (24 24
     (:REWRITE X86ISA::LOGHEAD-ZERO-SMALLER))
 (24 24
     (:REWRITE X86ISA::LOGHEAD-OF-NON-INTEGERP))
 (24 3 (:REWRITE BFIX-WHEN-NOT-1))
 (13 13 (:REWRITE UNSIGNED-BYTE-P-LOGHEAD))
 (11 11 (:REWRITE NATP-WHEN-INTEGERP))
 (10 10
     (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NATP))
 (9 1 (:REWRITE EQUAL-1-OF-BOOL->BIT))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE1-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX2-BYTE1-P))
 (8
  1
  (:REWRITE
    X86ISA::UNSIGNED-BYTE-P-WHEN-SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SIB-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SEGMENT-SELECTORBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-RFLAGSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MXCSRBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MODR/M-P))
 (8
  1
  (:REWRITE
   X86ISA::UNSIGNED-BYTE-P-WHEN-INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-FP-STATUSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-PREFIXES-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE3-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE1-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR0BITS-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE
         X86ISA::UNSIGNED-BYTE-P-WHEN-CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-8BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-32BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-16BITS-P))
 (7 3
    (:REWRITE BITOPS::LOGAND-WITH-NEGATED-BITMASK))
 (7 3
    (:REWRITE BITOPS::LOGAND-WITH-BITMASK))
 (6 6 (:TYPE-PRESCRIPTION BIT->BOOL$INLINE))
 (6 3 (:REWRITE BFIX-WHEN-NOT-BITP))
 (6 3 (:REWRITE BFIX-WHEN-NOT-BIT->BOOL))
 (6 3 (:REWRITE BFIX-WHEN-BIT->BOOL))
 (5 5 (:TYPE-PRESCRIPTION LOGNOT))
 (3 3
    (:REWRITE BITOPS::B-AND-EQUAL-1-IN-HYP))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX2-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SIB-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SEGMENT-SELECTORBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::RFLAGSBITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MXCSRBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MODR/M-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION
         X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::FP-STATUSBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-PREFIXES-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE3-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CR0BITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::8BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::32BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::16BITS-P))
 (2
   2
   (:REWRITE X86ISA::WEED-OUT-IRRELEVANT-LOGAND-WHEN-FIRST-OPERAND-CONSTANT))
 (2 2 (:REWRITE X86ISA::LOGAND-REDUNDANT))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX2-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
    X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SIB-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SEGMENT-SELECTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::RFLAGSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MXCSRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MODR/M-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
   X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1 (:REWRITE IFIX-WHEN-NOT-INTEGERP))
 (2 1
    (:REWRITE X86ISA::FP-STATUSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-PREFIXES-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE3-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::CR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE
         X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::8BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::32BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::16BITS-P-WHEN-UNSIGNED-BYTE-P)))
(X86ISA::LOGBITP-N-OF-SET-DIRTY-AND-ACCESSED-BITS
 (246 4
      (:REWRITE BITOPS::LOGBITP-NONZERO-OF-BIT))
 (222 4
      (:REWRITE FTY::BITP-WHEN-UNSIGNED-BYTE-P-1))
 (201 5 (:DEFINITION UNSIGNED-BYTE-P))
 (196 5 (:DEFINITION INTEGER-RANGE-P))
 (70 1 (:LINEAR LOGHEAD-UPPER-BOUND))
 (67 7
     (:REWRITE X86ISA::ACCESSED-BIT-SET-ACCESSED-BIT-IDENTITY))
 (54 4
     (:LINEAR X86ISA::N64P-SET-ACCESSED-BIT))
 (33 3
     (:REWRITE X86ISA::DIRTY-BIT-SET-DIRTY-BIT-IDENTITY))
 (30 30
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS->A$INLINE))
 (27 2 (:LINEAR X86ISA::N64P-SET-DIRTY-BIT))
 (18 6 (:DEFINITION X86ISA::ACCESSED-BIT))
 (15 15
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS->D$INLINE))
 (12 12 (:TYPE-PRESCRIPTION BITP))
 (12 8 (:REWRITE DEFAULT-<-1))
 (12 4
     (:REWRITE FTY::UNSIGNED-BYTE-P-1-WHEN-BITP))
 (12 4
     (:REWRITE BITOPS::LOGBITP-WHEN-BITMASKP))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-XCR0BITS-P))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PTE-4K-PAGEBITS-P))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PML4EBITS-P))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-PG-DIRBITS-P))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-1GB-PAGEBITS-P))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-PG-TABLEBITS-P))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-2MB-PAGEBITS-P))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PAGE-TABLESBITS-P))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTORBITS-P))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR3BITS-P))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTORBITS-P))
 (10 2
     (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-64BITS-P))
 (9 3 (:DEFINITION X86ISA::DIRTY-BIT))
 (8 8 (:TYPE-PRESCRIPTION BITMASKP$INLINE))
 (8 8 (:REWRITE DEFAULT-<-2))
 (7 7
    (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR))
 (7 1
    (:REWRITE BITOPS::LOGBITP-OF-LOGHEAD-OUT-OF-BOUNDS))
 (7 1
    (:REWRITE BITOPS::LOGBITP-OF-LOGHEAD-IN-BOUNDS))
 (6 6
    (:TYPE-PRESCRIPTION X86ISA::N01P-ACCESSED-BIT))
 (6 6
    (:TYPE-PRESCRIPTION X86ISA::ACCESSED-BIT))
 (6 6
    (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS->A-OF-WRITE-WITH-MASK))
 (6
  6
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->A$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (5 5 (:TYPE-PRESCRIPTION NFIX))
 (4 4
    (:TYPE-PRESCRIPTION X86ISA::XCR0BITS-P$INLINE))
 (4 4
    (:TYPE-PRESCRIPTION X86ISA::IA32E-PTE-4K-PAGEBITS-P))
 (4 4
    (:TYPE-PRESCRIPTION X86ISA::IA32E-PML4EBITS-P))
 (4 4
    (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-PG-DIRBITS-P))
 (4 4
    (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P))
 (4 4
    (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-PG-TABLEBITS-P))
 (4 4
    (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-2MB-PAGEBITS-P))
 (4 4
    (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS-P))
 (4 4
    (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P))
 (4 4
    (:TYPE-PRESCRIPTION X86ISA::CR3BITS-P$INLINE))
 (4 4
    (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P))
 (4 4 (:TYPE-PRESCRIPTION X86ISA::64BITS-P))
 (4 4
    (:REWRITE BITOPS::NORMALIZE-LOGBITP-WHEN-MODS-EQUAL))
 (4 4
    (:REWRITE BITOPS::LOGBITP-OF-NEGATIVE-CONST))
 (4 4 (:REWRITE BITOPS::LOGBITP-OF-MASK))
 (4 4 (:REWRITE BITOPS::LOGBITP-OF-CONST))
 (4 4
    (:META BITOPS::OPEN-LOGBITP-OF-CONST-LITE-META))
 (4 2
    (:REWRITE X86ISA::XCR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (4 2
    (:REWRITE X86ISA::IA32E-PTE-4K-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (4 2
    (:REWRITE X86ISA::IA32E-PML4EBITS-P-WHEN-UNSIGNED-BYTE-P))
 (4 2
    (:REWRITE X86ISA::IA32E-PDPTE-PG-DIRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (4 2
    (:REWRITE X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (4 2
    (:REWRITE X86ISA::IA32E-PDE-PG-TABLEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (4 2
    (:REWRITE X86ISA::IA32E-PDE-2MB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (4 2
    (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (4 2
    (:REWRITE X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (4 2
    (:REWRITE X86ISA::CR3BITS-P-WHEN-UNSIGNED-BYTE-P))
 (4 2
    (:REWRITE X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (4 2
    (:REWRITE X86ISA::64BITS-P-WHEN-UNSIGNED-BYTE-P))
 (3 3
    (:TYPE-PRESCRIPTION X86ISA::N01P-DIRTY-BIT))
 (3 3
    (:TYPE-PRESCRIPTION X86ISA::DIRTY-BIT))
 (3 3
    (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS->D-OF-WRITE-WITH-MASK))
 (3
  3
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->D$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (2 2 (:REWRITE NFIX-WHEN-NOT-NATP))
 (2 2 (:REWRITE NFIX-WHEN-NATP))
 (2 2
    (:REWRITE X86ISA::LOGHEAD-ZERO-SMALLER))
 (2 2
    (:REWRITE X86ISA::LOGHEAD-OF-NON-INTEGERP))
 (2 1 (:REWRITE UNSIGNED-BYTE-P-OF-LOGHEAD))
 (1 1 (:REWRITE UNSIGNED-BYTE-P-LOGHEAD)))
(X86ISA::LOGTAIL-N-OF-SET-ACCESSED-BIT
 (7711 91
       (:REWRITE FTY::BITP-WHEN-UNSIGNED-BYTE-P-1))
 (7650 51 (:REWRITE BITOPS::LOGBITP-WHEN-BIT))
 (6570 40 (:REWRITE LOGHEAD-IDENTITY))
 (6342 12
       (:LINEAR X86ISA::RM-LOW-64-LOGAND-LOGIOR-HELPER-1))
 (6318 12 (:DEFINITION X86ISA::N64P$INLINE))
 (5878 110 (:REWRITE UNSIGNED-BYTE-P-PLUS))
 (3217 48
       (:REWRITE UNSIGNED-BYTE-P-OF-LOGTAIL))
 (2666 80
       (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PTE-4K-PAGEBITS-P))
 (2216 68
       (:REWRITE X86ISA::IA32E-PTE-4K-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (1542 6 (:LINEAR BITOPS::LOGIOR-<-0-LINEAR-2))
 (910 674 (:REWRITE DEFAULT-<-2))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-XCR0BITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PML4EBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-PG-DIRBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-1GB-PAGEBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-PG-TABLEBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-2MB-PAGEBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PAGE-TABLESBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTORBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR3BITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTORBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-64BITS-P))
 (848 674 (:REWRITE DEFAULT-<-1))
 (582 147 (:REWRITE NFIX-WHEN-NOT-NATP))
 (494 2
      (:REWRITE UNSIGNED-BYTE-P-OF-LOGAND-2))
 (445 28 (:REWRITE LOGTAIL-IDENTITY))
 (440 35
      (:REWRITE BITOPS::LOGBITP-NONZERO-OF-BIT))
 (376 68
      (:REWRITE X86ISA::XCR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PML4EBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PDPTE-PG-DIRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PDE-PG-TABLEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PDE-2MB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::CR3BITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::64BITS-P-WHEN-UNSIGNED-BYTE-P))
 (285 285
      (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR))
 (282 47 (:REWRITE IFIX-NEGATIVE-TO-NEGP))
 (253 253 (:TYPE-PRESCRIPTION BITP))
 (228 206
      (:REWRITE EXPT-WITH-VIOLATED-GUARDS))
 (215 78
      (:REWRITE FTY::UNSIGNED-BYTE-P-1-WHEN-BITP))
 (169 122 (:REWRITE DEFAULT-+-1))
 (157 74 (:REWRITE IFIX-WHEN-NOT-INTEGERP))
 (151 51
      (:REWRITE BITOPS::LOGBITP-WHEN-BITMASKP))
 (148 148
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PTE-4K-PAGEBITS-P))
 (145 40
      (:REWRITE X86ISA::LOGHEAD-OF-NON-INTEGERP))
 (141 47 (:REWRITE NEGP-WHEN-LESS-THAN-0))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::XCR0BITS-P$INLINE))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PML4EBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-PG-DIRBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-PG-TABLEBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-2MB-PAGEBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::CR3BITS-P$INLINE))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::64BITS-P))
 (134 122 (:REWRITE DEFAULT-+-2))
 (134 54 (:REWRITE UNSIGNED-BYTE-P-LOGHEAD))
 (126 51
      (:REWRITE BITOPS::LOGAND-WITH-NEGATED-BITMASK))
 (113 2 (:REWRITE UNSIGNED-BYTE-P-OF-LOGIOR))
 (113 2
      (:REWRITE UNSIGNED-BYTE-P-OF-LOGAND-1))
 (110 10 (:LINEAR BITOPS::LOGAND-<-0-LINEAR))
 (88 88
     (:LINEAR BITOPS::EXPT-2-LOWER-BOUND-BY-LOGBITP))
 (84 42
     (:REWRITE REDUCE-INTEGERP-+-CONSTANT))
 (72 6 (:LINEAR BITOPS::LOGIOR-<-0-LINEAR-1))
 (56 56
     (:TYPE-PRESCRIPTION BITOPS::ASH-NATP-TYPE))
 (51 51
     (:REWRITE BITOPS::NORMALIZE-LOGBITP-WHEN-MODS-EQUAL))
 (51 51
     (:REWRITE BITOPS::LOGBITP-OF-NEGATIVE-CONST))
 (51 51 (:REWRITE BITOPS::LOGBITP-OF-MASK))
 (51 51 (:REWRITE BITOPS::LOGBITP-OF-CONST))
 (48
   48
   (:REWRITE X86ISA::WEED-OUT-IRRELEVANT-LOGAND-WHEN-FIRST-OPERAND-CONSTANT))
 (48 48 (:REWRITE X86ISA::LOGAND-REDUNDANT))
 (47 47 (:TYPE-PRESCRIPTION NEGP))
 (47 47 (:REWRITE NEGP-WHEN-INTEGERP))
 (45 40
     (:REWRITE X86ISA::LOGHEAD-ZERO-SMALLER))
 (42 42 (:DEFINITION FIX))
 (34 34
     (:TYPE-PRESCRIPTION BIT->BOOL$INLINE))
 (34 17 (:REWRITE BFIX-WHEN-NOT-BIT->BOOL))
 (34 17 (:REWRITE BFIX-WHEN-BIT->BOOL))
 (32 16 (:REWRITE BFIX-WHEN-NOT-BITP))
 (28 28
     (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NATP))
 (28 28
     (:TYPE-PRESCRIPTION BITOPS::LOGAND-NATP-TYPE-1))
 (25 21 (:REWRITE DEFAULT-UNARY-MINUS))
 (20 2
     (:REWRITE X86ISA::CONSTANT-UPPER-BOUND-OF-LOGIOR-FOR-NATURALS))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE1-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX2-BYTE1-P))
 (8
  1
  (:REWRITE
    X86ISA::UNSIGNED-BYTE-P-WHEN-SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SIB-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SEGMENT-SELECTORBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-RFLAGSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MXCSRBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MODR/M-P))
 (8
  1
  (:REWRITE
   X86ISA::UNSIGNED-BYTE-P-WHEN-INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-FP-STATUSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-PREFIXES-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE3-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE1-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR0BITS-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE
         X86ISA::UNSIGNED-BYTE-P-WHEN-CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-8BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-32BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-16BITS-P))
 (6 6
    (:REWRITE BITOPS::B-IOR-EQUAL-1-IN-CONCL))
 (6 6
    (:LINEAR BITOPS::UPPER-BOUND-OF-LOGIOR-FOR-NATURALS))
 (4 4
    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (4 4
    (:REWRITE BITOPS::B-AND-EQUAL-1-IN-HYP))
 (4 2 (:LINEAR BITOPS::B-AND-BOUND))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX2-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SIB-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SEGMENT-SELECTORBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::RFLAGSBITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MXCSRBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MODR/M-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION
         X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::FP-STATUSBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-PREFIXES-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE3-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CR0BITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::8BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::32BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::16BITS-P))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX2-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
    X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SIB-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SEGMENT-SELECTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::RFLAGSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MXCSRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MODR/M-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
   X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::FP-STATUSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-PREFIXES-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE3-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::CR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE
         X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::8BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::32BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::16BITS-P-WHEN-UNSIGNED-BYTE-P)))
(X86ISA::LOGTAIL-N-OF-SET-DIRTY-BIT
 (7711 91
       (:REWRITE FTY::BITP-WHEN-UNSIGNED-BYTE-P-1))
 (7650 51 (:REWRITE BITOPS::LOGBITP-WHEN-BIT))
 (6570 40 (:REWRITE LOGHEAD-IDENTITY))
 (6342 12
       (:LINEAR X86ISA::RM-LOW-64-LOGAND-LOGIOR-HELPER-1))
 (6318 12 (:DEFINITION X86ISA::N64P$INLINE))
 (5878 110 (:REWRITE UNSIGNED-BYTE-P-PLUS))
 (3217 48
       (:REWRITE UNSIGNED-BYTE-P-OF-LOGTAIL))
 (2666 80
       (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PTE-4K-PAGEBITS-P))
 (2216 68
       (:REWRITE X86ISA::IA32E-PTE-4K-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (1542 6 (:LINEAR BITOPS::LOGIOR-<-0-LINEAR-2))
 (910 674 (:REWRITE DEFAULT-<-2))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-XCR0BITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PML4EBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-PG-DIRBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-1GB-PAGEBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-PG-TABLEBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-2MB-PAGEBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PAGE-TABLESBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTORBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR3BITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTORBITS-P))
 (884 80
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-64BITS-P))
 (848 674 (:REWRITE DEFAULT-<-1))
 (582 147 (:REWRITE NFIX-WHEN-NOT-NATP))
 (494 2
      (:REWRITE UNSIGNED-BYTE-P-OF-LOGAND-2))
 (445 28 (:REWRITE LOGTAIL-IDENTITY))
 (440 35
      (:REWRITE BITOPS::LOGBITP-NONZERO-OF-BIT))
 (376 68
      (:REWRITE X86ISA::XCR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PML4EBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PDPTE-PG-DIRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PDE-PG-TABLEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PDE-2MB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::CR3BITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (376 68
      (:REWRITE X86ISA::64BITS-P-WHEN-UNSIGNED-BYTE-P))
 (285 285
      (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR))
 (282 47 (:REWRITE IFIX-NEGATIVE-TO-NEGP))
 (253 253 (:TYPE-PRESCRIPTION BITP))
 (228 206
      (:REWRITE EXPT-WITH-VIOLATED-GUARDS))
 (215 78
      (:REWRITE FTY::UNSIGNED-BYTE-P-1-WHEN-BITP))
 (169 122 (:REWRITE DEFAULT-+-1))
 (157 74 (:REWRITE IFIX-WHEN-NOT-INTEGERP))
 (151 51
      (:REWRITE BITOPS::LOGBITP-WHEN-BITMASKP))
 (148 148
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PTE-4K-PAGEBITS-P))
 (145 40
      (:REWRITE X86ISA::LOGHEAD-OF-NON-INTEGERP))
 (141 47 (:REWRITE NEGP-WHEN-LESS-THAN-0))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::XCR0BITS-P$INLINE))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PML4EBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-PG-DIRBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-PG-TABLEBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-2MB-PAGEBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::CR3BITS-P$INLINE))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P))
 (138 138
      (:TYPE-PRESCRIPTION X86ISA::64BITS-P))
 (134 122 (:REWRITE DEFAULT-+-2))
 (134 54 (:REWRITE UNSIGNED-BYTE-P-LOGHEAD))
 (126 51
      (:REWRITE BITOPS::LOGAND-WITH-NEGATED-BITMASK))
 (113 2 (:REWRITE UNSIGNED-BYTE-P-OF-LOGIOR))
 (113 2
      (:REWRITE UNSIGNED-BYTE-P-OF-LOGAND-1))
 (110 10 (:LINEAR BITOPS::LOGAND-<-0-LINEAR))
 (88 88
     (:LINEAR BITOPS::EXPT-2-LOWER-BOUND-BY-LOGBITP))
 (84 42
     (:REWRITE REDUCE-INTEGERP-+-CONSTANT))
 (72 6 (:LINEAR BITOPS::LOGIOR-<-0-LINEAR-1))
 (56 56
     (:TYPE-PRESCRIPTION BITOPS::ASH-NATP-TYPE))
 (51 51
     (:REWRITE BITOPS::NORMALIZE-LOGBITP-WHEN-MODS-EQUAL))
 (51 51
     (:REWRITE BITOPS::LOGBITP-OF-NEGATIVE-CONST))
 (51 51 (:REWRITE BITOPS::LOGBITP-OF-MASK))
 (51 51 (:REWRITE BITOPS::LOGBITP-OF-CONST))
 (48
   48
   (:REWRITE X86ISA::WEED-OUT-IRRELEVANT-LOGAND-WHEN-FIRST-OPERAND-CONSTANT))
 (48 48 (:REWRITE X86ISA::LOGAND-REDUNDANT))
 (47 47 (:TYPE-PRESCRIPTION NEGP))
 (47 47 (:REWRITE NEGP-WHEN-INTEGERP))
 (45 40
     (:REWRITE X86ISA::LOGHEAD-ZERO-SMALLER))
 (42 42 (:DEFINITION FIX))
 (34 34
     (:TYPE-PRESCRIPTION BIT->BOOL$INLINE))
 (34 17 (:REWRITE BFIX-WHEN-NOT-BIT->BOOL))
 (34 17 (:REWRITE BFIX-WHEN-BIT->BOOL))
 (32 16 (:REWRITE BFIX-WHEN-NOT-BITP))
 (28 28
     (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NATP))
 (28 28
     (:TYPE-PRESCRIPTION BITOPS::LOGAND-NATP-TYPE-1))
 (25 21 (:REWRITE DEFAULT-UNARY-MINUS))
 (20 2
     (:REWRITE X86ISA::CONSTANT-UPPER-BOUND-OF-LOGIOR-FOR-NATURALS))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE1-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX2-BYTE1-P))
 (8
  1
  (:REWRITE
    X86ISA::UNSIGNED-BYTE-P-WHEN-SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SIB-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SEGMENT-SELECTORBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-RFLAGSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MXCSRBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MODR/M-P))
 (8
  1
  (:REWRITE
   X86ISA::UNSIGNED-BYTE-P-WHEN-INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-FP-STATUSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-PREFIXES-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE3-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE1-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR0BITS-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE
         X86ISA::UNSIGNED-BYTE-P-WHEN-CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-8BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-32BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-16BITS-P))
 (6 6
    (:REWRITE BITOPS::B-IOR-EQUAL-1-IN-CONCL))
 (6 6
    (:LINEAR BITOPS::UPPER-BOUND-OF-LOGIOR-FOR-NATURALS))
 (4 4
    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (4 4
    (:REWRITE BITOPS::B-AND-EQUAL-1-IN-HYP))
 (4 2 (:LINEAR BITOPS::B-AND-BOUND))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX2-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SIB-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SEGMENT-SELECTORBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::RFLAGSBITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MXCSRBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MODR/M-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION
         X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::FP-STATUSBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-PREFIXES-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE3-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CR0BITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::8BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::32BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::16BITS-P))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX2-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
    X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SIB-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SEGMENT-SELECTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::RFLAGSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MXCSRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MODR/M-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
   X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::FP-STATUSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-PREFIXES-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE3-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::CR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE
         X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::8BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::32BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::16BITS-P-WHEN-UNSIGNED-BYTE-P)))
(X86ISA::LOGTAIL-N-OF-SET-DIRTY-AND-ACCESSED-BITS
 (76 2 (:REWRITE LOGTAIL-IDENTITY))
 (70 2 (:DEFINITION UNSIGNED-BYTE-P))
 (64 2 (:DEFINITION INTEGER-RANGE-P))
 (33 3
     (:REWRITE X86ISA::DIRTY-BIT-SET-DIRTY-BIT-IDENTITY))
 (29 2 (:LINEAR X86ISA::N64P-SET-DIRTY-BIT))
 (15 15
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS->D$INLINE))
 (12 2
     (:REWRITE X86ISA::ACCESSED-BIT-SET-ACCESSED-BIT-IDENTITY))
 (9 3 (:DEFINITION X86ISA::DIRTY-BIT))
 (8 8
    (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (8 8
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (7 3 (:REWRITE DEFAULT-<-2))
 (5 5
    (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS->A$INLINE))
 (5 3 (:REWRITE DEFAULT-<-1))
 (4 4 (:REWRITE EXPT-WITH-VIOLATED-GUARDS))
 (3 3
    (:TYPE-PRESCRIPTION X86ISA::N01P-DIRTY-BIT))
 (3 3
    (:TYPE-PRESCRIPTION X86ISA::DIRTY-BIT))
 (3 3
    (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR))
 (3 3
    (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS->D-OF-WRITE-WITH-MASK))
 (3
  3
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->D$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (3 1 (:DEFINITION X86ISA::ACCESSED-BIT))
 (2 2
    (:LINEAR BITOPS::EXPT-2-LOWER-BOUND-BY-LOGBITP))
 (1 1
    (:TYPE-PRESCRIPTION X86ISA::N01P-ACCESSED-BIT))
 (1 1
    (:TYPE-PRESCRIPTION X86ISA::ACCESSED-BIT))
 (1 1
    (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS->A-OF-WRITE-WITH-MASK))
 (1
  1
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->A$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST)))
(X86ISA::LOGHEAD-N-OF-SET-ACCESSED-BIT
 (5512 91
       (:REWRITE FTY::BITP-WHEN-UNSIGNED-BYTE-P-1))
 (5407 49 (:REWRITE BITOPS::LOGBITP-WHEN-BIT))
 (3180 12
       (:LINEAR X86ISA::RM-LOW-64-LOGAND-LOGIOR-HELPER-1))
 (3156 12 (:DEFINITION X86ISA::N64P$INLINE))
 (1504 80 (:REWRITE LOGHEAD-IDENTITY))
 (862 6 (:LINEAR BITOPS::LOGIOR-<-0-LINEAR-2))
 (577 415 (:REWRITE DEFAULT-<-2))
 (523 415 (:REWRITE DEFAULT-<-1))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-XCR0BITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PTE-4K-PAGEBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PML4EBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-PG-DIRBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-1GB-PAGEBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-PG-TABLEBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-2MB-PAGEBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PAGE-TABLESBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTORBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR3BITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTORBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-64BITS-P))
 (448 37
      (:REWRITE BITOPS::LOGBITP-NONZERO-OF-BIT))
 (294 28
      (:REWRITE X86ISA::XCR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PTE-4K-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PML4EBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PDPTE-PG-DIRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PDE-PG-TABLEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PDE-2MB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::CR3BITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::64BITS-P-WHEN-UNSIGNED-BYTE-P))
 (257 257 (:TYPE-PRESCRIPTION BITP))
 (233 77 (:REWRITE NFIX-WHEN-NOT-NATP))
 (227 84
      (:REWRITE FTY::UNSIGNED-BYTE-P-1-WHEN-BITP))
 (218 6 (:LINEAR BITOPS::LOGIOR-<-0-LINEAR-1))
 (187 187
      (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR))
 (184 184
      (:REWRITE EXPT-WITH-VIOLATED-GUARDS))
 (145 49
      (:REWRITE BITOPS::LOGBITP-WHEN-BITMASKP))
 (143 42 (:REWRITE UNSIGNED-BYTE-P-LOGHEAD))
 (140 57 (:REWRITE IFIX-WHEN-NOT-INTEGERP))
 (134 8 (:REWRITE BFIX-WHEN-NOT-1))
 (126 51
      (:REWRITE BITOPS::LOGAND-WITH-NEGATED-BITMASK))
 (114 80
      (:REWRITE X86ISA::LOGHEAD-OF-NON-INTEGERP))
 (112 2 (:REWRITE UNSIGNED-BYTE-P-OF-LOGIOR))
 (110 10 (:LINEAR BITOPS::LOGAND-<-0-LINEAR))
 (105 2
      (:REWRITE UNSIGNED-BYTE-P-OF-LOGAND-2))
 (105 2
      (:REWRITE UNSIGNED-BYTE-P-OF-LOGAND-1))
 (92 92 (:REWRITE NATP-WHEN-INTEGERP))
 (59 59
     (:LINEAR BITOPS::EXPT-2-LOWER-BOUND-BY-LOGBITP))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::XCR0BITS-P$INLINE))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PTE-4K-PAGEBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PML4EBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-PG-DIRBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-PG-TABLEBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-2MB-PAGEBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::CR3BITS-P$INLINE))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::64BITS-P))
 (54 18
     (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (54 2
     (:REWRITE X86ISA::CONSTANT-UPPER-BOUND-OF-LOGIOR-FOR-NATURALS))
 (49 49
     (:REWRITE BITOPS::NORMALIZE-LOGBITP-WHEN-MODS-EQUAL))
 (49 49
     (:REWRITE BITOPS::LOGBITP-OF-NEGATIVE-CONST))
 (49 49 (:REWRITE BITOPS::LOGBITP-OF-MASK))
 (49 49 (:REWRITE BITOPS::LOGBITP-OF-CONST))
 (48
   48
   (:REWRITE X86ISA::WEED-OUT-IRRELEVANT-LOGAND-WHEN-FIRST-OPERAND-CONSTANT))
 (48 48 (:REWRITE X86ISA::LOGAND-REDUNDANT))
 (39 39
     (:REWRITE INEQUALITY-WITH-NFIX-HYP-1))
 (36 36
     (:TYPE-PRESCRIPTION BITOPS::ASH-NATP-TYPE))
 (36 36 (:TYPE-PRESCRIPTION ASH))
 (33 33
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (29 5
     (:REWRITE BITOPS::LOGHEAD-OF-LOGHEAD-2))
 (18 18
     (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NATP))
 (18 18 (:TYPE-PRESCRIPTION LOGNOT))
 (18 18
     (:TYPE-PRESCRIPTION BITOPS::LOGAND-NATP-TYPE-1))
 (18 18 (:TYPE-PRESCRIPTION BINARY-LOGIOR))
 (18 18 (:TYPE-PRESCRIPTION BINARY-LOGAND))
 (16 16
     (:TYPE-PRESCRIPTION BIT->BOOL$INLINE))
 (16 8 (:REWRITE BFIX-WHEN-NOT-BITP))
 (16 8 (:REWRITE BFIX-WHEN-NOT-BIT->BOOL))
 (16 8 (:REWRITE BFIX-WHEN-BIT->BOOL))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE1-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX2-BYTE1-P))
 (8
  1
  (:REWRITE
    X86ISA::UNSIGNED-BYTE-P-WHEN-SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SIB-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SEGMENT-SELECTORBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-RFLAGSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MXCSRBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MODR/M-P))
 (8
  1
  (:REWRITE
   X86ISA::UNSIGNED-BYTE-P-WHEN-INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-FP-STATUSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-PREFIXES-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE3-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE1-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR0BITS-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE
         X86ISA::UNSIGNED-BYTE-P-WHEN-CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-8BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-32BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-16BITS-P))
 (6 6
    (:REWRITE BITOPS::B-IOR-EQUAL-1-IN-CONCL))
 (6 6
    (:LINEAR BITOPS::UPPER-BOUND-OF-LOGIOR-FOR-NATURALS))
 (4 4
    (:REWRITE BITOPS::B-AND-EQUAL-1-IN-HYP))
 (4 2 (:LINEAR BITOPS::B-AND-BOUND))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX2-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SIB-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SEGMENT-SELECTORBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::RFLAGSBITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MXCSRBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MODR/M-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION
         X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::FP-STATUSBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-PREFIXES-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE3-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CR0BITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::8BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::32BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::16BITS-P))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX2-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
    X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SIB-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SEGMENT-SELECTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::RFLAGSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MXCSRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MODR/M-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
   X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::FP-STATUSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-PREFIXES-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE3-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::CR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE
         X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::8BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::32BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::16BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1 (:LINEAR BITOPS::B-NOT-BOUND)))
(X86ISA::LOGHEAD-N-OF-SET-DIRTY-BIT
 (5512 91
       (:REWRITE FTY::BITP-WHEN-UNSIGNED-BYTE-P-1))
 (5407 49 (:REWRITE BITOPS::LOGBITP-WHEN-BIT))
 (3180 12
       (:LINEAR X86ISA::RM-LOW-64-LOGAND-LOGIOR-HELPER-1))
 (3156 12 (:DEFINITION X86ISA::N64P$INLINE))
 (1504 80 (:REWRITE LOGHEAD-IDENTITY))
 (862 6 (:LINEAR BITOPS::LOGIOR-<-0-LINEAR-2))
 (577 415 (:REWRITE DEFAULT-<-2))
 (523 415 (:REWRITE DEFAULT-<-1))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-XCR0BITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PTE-4K-PAGEBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PML4EBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-PG-DIRBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDPTE-1GB-PAGEBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-PG-TABLEBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PDE-2MB-PAGEBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-IA32E-PAGE-TABLESBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTORBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR3BITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTORBITS-P))
 (450 30
      (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-64BITS-P))
 (448 37
      (:REWRITE BITOPS::LOGBITP-NONZERO-OF-BIT))
 (294 28
      (:REWRITE X86ISA::XCR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PTE-4K-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PML4EBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PDPTE-PG-DIRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PDE-PG-TABLEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PDE-2MB-PAGEBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::CR3BITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (294 28
      (:REWRITE X86ISA::64BITS-P-WHEN-UNSIGNED-BYTE-P))
 (257 257 (:TYPE-PRESCRIPTION BITP))
 (233 77 (:REWRITE NFIX-WHEN-NOT-NATP))
 (227 84
      (:REWRITE FTY::UNSIGNED-BYTE-P-1-WHEN-BITP))
 (218 6 (:LINEAR BITOPS::LOGIOR-<-0-LINEAR-1))
 (187 187
      (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR))
 (184 184
      (:REWRITE EXPT-WITH-VIOLATED-GUARDS))
 (145 49
      (:REWRITE BITOPS::LOGBITP-WHEN-BITMASKP))
 (143 42 (:REWRITE UNSIGNED-BYTE-P-LOGHEAD))
 (140 57 (:REWRITE IFIX-WHEN-NOT-INTEGERP))
 (134 8 (:REWRITE BFIX-WHEN-NOT-1))
 (126 51
      (:REWRITE BITOPS::LOGAND-WITH-NEGATED-BITMASK))
 (114 80
      (:REWRITE X86ISA::LOGHEAD-OF-NON-INTEGERP))
 (112 2 (:REWRITE UNSIGNED-BYTE-P-OF-LOGIOR))
 (110 10 (:LINEAR BITOPS::LOGAND-<-0-LINEAR))
 (105 2
      (:REWRITE UNSIGNED-BYTE-P-OF-LOGAND-2))
 (105 2
      (:REWRITE UNSIGNED-BYTE-P-OF-LOGAND-1))
 (92 92 (:REWRITE NATP-WHEN-INTEGERP))
 (59 59
     (:LINEAR BITOPS::EXPT-2-LOWER-BOUND-BY-LOGBITP))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::XCR0BITS-P$INLINE))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PTE-4K-PAGEBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PML4EBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-PG-DIRBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDPTE-1GB-PAGEBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-PG-TABLEBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PDE-2MB-PAGEBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTORBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::CR3BITS-P$INLINE))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTORBITS-P))
 (58 58
     (:TYPE-PRESCRIPTION X86ISA::64BITS-P))
 (54 18
     (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (54 2
     (:REWRITE X86ISA::CONSTANT-UPPER-BOUND-OF-LOGIOR-FOR-NATURALS))
 (49 49
     (:REWRITE BITOPS::NORMALIZE-LOGBITP-WHEN-MODS-EQUAL))
 (49 49
     (:REWRITE BITOPS::LOGBITP-OF-NEGATIVE-CONST))
 (49 49 (:REWRITE BITOPS::LOGBITP-OF-MASK))
 (49 49 (:REWRITE BITOPS::LOGBITP-OF-CONST))
 (48
   48
   (:REWRITE X86ISA::WEED-OUT-IRRELEVANT-LOGAND-WHEN-FIRST-OPERAND-CONSTANT))
 (48 48 (:REWRITE X86ISA::LOGAND-REDUNDANT))
 (39 39
     (:REWRITE INEQUALITY-WITH-NFIX-HYP-1))
 (36 36
     (:TYPE-PRESCRIPTION BITOPS::ASH-NATP-TYPE))
 (36 36 (:TYPE-PRESCRIPTION ASH))
 (33 33
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (29 5
     (:REWRITE BITOPS::LOGHEAD-OF-LOGHEAD-2))
 (18 18
     (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NATP))
 (18 18 (:TYPE-PRESCRIPTION LOGNOT))
 (18 18
     (:TYPE-PRESCRIPTION BITOPS::LOGAND-NATP-TYPE-1))
 (18 18 (:TYPE-PRESCRIPTION BINARY-LOGIOR))
 (18 18 (:TYPE-PRESCRIPTION BINARY-LOGAND))
 (16 16
     (:TYPE-PRESCRIPTION BIT->BOOL$INLINE))
 (16 8 (:REWRITE BFIX-WHEN-NOT-BITP))
 (16 8 (:REWRITE BFIX-WHEN-NOT-BIT->BOOL))
 (16 8 (:REWRITE BFIX-WHEN-BIT->BOOL))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX3-BYTE1-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-VEX2-BYTE1-P))
 (8
  1
  (:REWRITE
    X86ISA::UNSIGNED-BYTE-P-WHEN-SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SIB-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-SEGMENT-SELECTORBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-RFLAGSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MXCSRBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-MODR/M-P))
 (8
  1
  (:REWRITE
   X86ISA::UNSIGNED-BYTE-P-WHEN-INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-FP-STATUSBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-PREFIXES-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE3-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE2-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-EVEX-BYTE1-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-CR0BITS-P))
 (8
  1
  (:REWRITE
      X86ISA::UNSIGNED-BYTE-P-WHEN-CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE
         X86ISA::UNSIGNED-BYTE-P-WHEN-CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-8BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-32BITS-P))
 (8 1
    (:REWRITE X86ISA::UNSIGNED-BYTE-P-WHEN-16BITS-P))
 (6 6
    (:REWRITE BITOPS::B-IOR-EQUAL-1-IN-CONCL))
 (6 6
    (:LINEAR BITOPS::UPPER-BOUND-OF-LOGIOR-FOR-NATURALS))
 (4 4
    (:REWRITE BITOPS::B-AND-EQUAL-1-IN-HYP))
 (4 2 (:LINEAR BITOPS::B-AND-BOUND))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX3-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::VEX2-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SIB-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::SEGMENT-SELECTORBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::RFLAGSBITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MXCSRBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::MODR/M-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION
         X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::FP-STATUSBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-PREFIXES-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE3-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE2-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::EVEX-BYTE1-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CR0BITS-P$INLINE))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2
    (:TYPE-PRESCRIPTION X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::8BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::32BITS-P))
 (2 2 (:TYPE-PRESCRIPTION X86ISA::16BITS-P))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX3-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::VEX2-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
    X86ISA::SYSTEM-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SIB-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::SEGMENT-SELECTORBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::RFLAGSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MXCSRBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::MODR/M-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
   X86ISA::INTERRUPT/TRAP-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::FP-STATUSBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-PREFIXES-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE3-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE2-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::EVEX-BYTE1-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::DATA-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::CR0BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2
  1
  (:REWRITE
      X86ISA::CODE-SEGMENT-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE
         X86ISA::CALL-GATE-DESCRIPTOR-ATTRIBUTESBITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::8BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::32BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1
    (:REWRITE X86ISA::16BITS-P-WHEN-UNSIGNED-BYTE-P))
 (2 1 (:LINEAR BITOPS::B-NOT-BOUND)))
(X86ISA::LOGHEAD-N-OF-SET-DIRTY-AND-ACCESSED-BITS
 (263 9 (:DEFINITION INTEGER-RANGE-P))
 (232 8 (:DEFINITION UNSIGNED-BYTE-P))
 (87 6 (:LINEAR X86ISA::N64P-SET-DIRTY-BIT))
 (77 7
     (:REWRITE X86ISA::DIRTY-BIT-SET-DIRTY-BIT-IDENTITY))
 (69 1 (:REWRITE X86ISA::LOGHEAD-UNEQUAL))
 (65 1 (:DEFINITION SIGNED-BYTE-P))
 (43 43
     (:TYPE-PRESCRIPTION X86ISA::N64P-SET-DIRTY-BIT))
 (38 18 (:REWRITE DEFAULT-<-2))
 (35 35
     (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS->D$INLINE))
 (30 30 (:REWRITE EXPT-WITH-VIOLATED-GUARDS))
 (24 18 (:REWRITE DEFAULT-<-1))
 (21 7 (:DEFINITION X86ISA::DIRTY-BIT))
 (14 2
     (:REWRITE BITOPS::LOGHEAD-OF-LOGHEAD-2))
 (12 8
     (:REWRITE X86ISA::LOGHEAD-OF-NON-INTEGERP))
 (12 2
     (:REWRITE X86ISA::ACCESSED-BIT-SET-ACCESSED-BIT-IDENTITY))
 (9 9
    (:REWRITE BITOPS::UNSIGNED-BYTE-P-INCR))
 (8 8
    (:REWRITE X86ISA::LOGHEAD-ZERO-SMALLER))
 (8 8
    (:LINEAR BITOPS::EXPT-2-LOWER-BOUND-BY-LOGBITP))
 (7 7
    (:TYPE-PRESCRIPTION X86ISA::N64P-SET-ACCESSED-BIT))
 (7 7
    (:TYPE-PRESCRIPTION X86ISA::N01P-DIRTY-BIT))
 (7 7
    (:TYPE-PRESCRIPTION X86ISA::DIRTY-BIT))
 (7 7
    (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS->D-OF-WRITE-WITH-MASK))
 (7
  7
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->D$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (5 5
    (:TYPE-PRESCRIPTION X86ISA::IA32E-PAGE-TABLESBITS->A$INLINE))
 (4 4 (:REWRITE NFIX-WHEN-NOT-NATP))
 (4 4
    (:LINEAR X86ISA::SIGNED-BYTE-P-OF-TRUNCATE-WHEN-Y-IS-POSITIVE))
 (4 4
    (:LINEAR X86ISA::SIGNED-BYTE-P-OF-TRUNCATE-WHEN-Y-IS-NEGATIVE))
 (4 4
    (:LINEAR X86ISA::SIGNED-BYTE-P-OF-TRUNCATE))
 (4 4
    (:LINEAR X86ISA::SIGNED-BYTE-P-OF-REM))
 (3 1 (:REWRITE DEFAULT-UNARY-MINUS))
 (3 1 (:DEFINITION X86ISA::ACCESSED-BIT))
 (1 1 (:TYPE-PRESCRIPTION SIGNED-BYTE-P))
 (1 1
    (:TYPE-PRESCRIPTION X86ISA::N01P-ACCESSED-BIT))
 (1 1
    (:TYPE-PRESCRIPTION X86ISA::ACCESSED-BIT))
 (1 1
    (:REWRITE BITOPS::SIGNED-BYTE-P-MONOTONICITY))
 (1 1 (:REWRITE BITOPS::SIGNED-BYTE-P-INCR))
 (1 1
    (:REWRITE X86ISA::IA32E-PAGE-TABLESBITS->A-OF-WRITE-WITH-MASK))
 (1
  1
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->A$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1 (:REWRITE DEFAULT-+-2))
 (1 1 (:REWRITE DEFAULT-+-1)))
(X86ISA::ACCESSED-BIT-SET-ACCESSED-BIT)
(X86ISA::ACCESSED-BIT-SET-DIRTY-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->A$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-BFIX-D-NORMALIZE-CONST)))
(X86ISA::DIRTY-BIT-SET-DIRTY-BIT)
(X86ISA::DIRTY-BIT-SET-ACCESSED-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->D$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-BFIX-A-NORMALIZE-CONST)))
(X86ISA::PAGE-SIZE-SET-ACCESSED-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->PS$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-BFIX-A-NORMALIZE-CONST)))
(X86ISA::PAGE-SIZE-SET-DIRTY-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->PS$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-BFIX-D-NORMALIZE-CONST)))
(X86ISA::PAGE-PRESENT-SET-ACCESSED-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->P$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-BFIX-A-NORMALIZE-CONST)))
(X86ISA::PAGE-PRESENT-SET-DIRTY-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->P$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-BFIX-D-NORMALIZE-CONST)))
(X86ISA::PAGE-READ-WRITE-SET-ACCESSED-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->R/W$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-BFIX-A-NORMALIZE-CONST)))
(X86ISA::PAGE-READ-WRITE-SET-DIRTY-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->R/W$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-BFIX-D-NORMALIZE-CONST)))
(X86ISA::PAGE-USER-SUPERVISOR-SET-ACCESSED-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->U/S$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-BFIX-A-NORMALIZE-CONST)))
(X86ISA::PAGE-USER-SUPERVISOR-SET-DIRTY-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->U/S$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-BFIX-D-NORMALIZE-CONST)))
(X86ISA::PAGE-EXECUTE-DISABLE-SET-ACCESSED-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->XD$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->A$INLINE-OF-BFIX-A-NORMALIZE-CONST)))
(X86ISA::PAGE-EXECUTE-DISABLE-SET-DIRTY-BIT
 (2
  2
  (:REWRITE
   X86ISA::IA32E-PAGE-TABLESBITS->XD$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1 1
    (:TYPE-PRESCRIPTION BITOPS::LOGNOT-NEGP))
 (1
  1
  (:REWRITE
   X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-IA32E-PAGE-TABLESBITS-FIX-X-NORMALIZE-CONST))
 (1
   1
   (:REWRITE
        X86ISA::!IA32E-PAGE-TABLESBITS->D$INLINE-OF-BFIX-D-NORMALIZE-CONST)))
