 ...
 (defun printing-x86-components (x86 base state)
   ...
   (with-open-file
    ...
    (b* ((rax (n64 (rgfi *rax* x86)))
         ...
         (flg (rflags x86))
         (rip (n48 (rip x86)))
         (old-radix (f-get-global 'print-radix state))
         (old-base (f-get-global 'print-base state))
         (state (f-put-global 'print-radix t state))
         (state (f-put-global 'print-base base state))
         ((mv ?col state)
          (fmt! "(@@GPR . ~%~tI((#.*RAX* . ~s0)~%~tI (#.*RBX* . ~s1)~% ~tI (#.*RCX* . ~s2)~% ~tI (#.*RDX* . ~s3)~% ~tI (#.*RSI* . ~s4)~% ~tI (#.*RDI* . ~s5)~% ~tI (#.*RBP* . ~s6)~% ~tI (#.*RSP* . ~s7)~% ~tI (#.*R8*  . ~s8)~% ~tI (#.*R9*  . ~s9)~% ~tI (#.*R10* . ~sA)~% ~tI (#.*R11* . ~sB)~% ~tI (#.*R12* . ~sC)~% ~tI (#.*R13* . ~sD)~% ~tI (#.*R14* . ~sE)~% ~tI (#.*R15* . ~sF))~%@@)~%(@@RFLAGS . ~%~tI~sG~%@@)~%(@@RIP . ~%~tI~sH~%@@)~%~%"
                (list (cons #\0 rax)
                      (cons #\1 rbx)
                      (cons #\2 rcx)
                      (cons #\3 rdx)
         ...
         ;; Restoring original values of the global variables...
         (state (f-put-global 'print-radix old-radix state))
         (state (f-put-global 'print-base old-base state)))
        state))))
...
(defun printing-flgs (x86 state)
  ...
  (b* ((cf  (flgi #.*cf* x86))
       ...
       (of  (flgi #.*of* x86))
       ((mv ?col state)
        (fmt! "(@@FLGS . ~%~tI((CF . ~s0)~%~tI (PF . ~s1)~% ~tI (AF . ~s2)~% ~tI (ZF . ~s3)~% ~tI (SF . ~s4)~% ~tI (OF . ~s5))~%@@)~%~%"
              (list (cons #\0 cf)
                    (cons #\1 pf)
                    (cons #\2 af)
                    (cons #\3 zf)
