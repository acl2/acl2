...
(defsection reasoning-about-memory-reads-and-writes
 ...
 @('wb') during reasoning.</p>"
  ...
  (define las-to-pas ((l-addrs canonical-address-listp)
    ...
    (if (atom l-addrs)
      ...
      (b* (((mv flg p-addr x86)
            (ia32e-la-to-pa (car l-addrs) r-x cpl x86))
           ((when flg) (mv flg nil x86))
           ((mv flgs p-addrs x86)
            (las-to-pas (cdr l-addrs) r-x cpl x86)))
        (mv flgs (if flgs nil (cons p-addr p-addrs)) x86)))

    ///

    (defthm consp-mv-nth-1-las-to-pas
      (implies (and (not (mv-nth 0 (las-to-pas l-addrs r-x cpl x86)))
                    (consp l-addrs))
               (consp (mv-nth 1 (las-to-pas l-addrs r-x cpl x86))))
      :hints (("Goal" :in-theory (e/d* (las-to-pas) ())))
      :rule-classes (:rewrite :type-prescription))

    (defthm true-listp-mv-nth-1-las-to-pas
      (true-listp (mv-nth 1 (las-to-pas l-addrs r-x cpl x86)))
      :rule-classes (:rewrite :type-prescription))

    (defthm car-mv-nth-1-las-to-pas
      (implies (and (not (mv-nth 0 (las-to-pas l-addrs r-x cpl x86)))
                    (consp l-addrs))
               (equal (car (mv-nth 1 (las-to-pas l-addrs r-x cpl x86)))
                      (mv-nth 1 (ia32e-la-to-pa (car l-addrs) r-x cpl x86))))
      :hints (("Goal" :in-theory (e/d* (las-to-pas) ()))))

    (defthm physical-address-listp-mv-nth-1-las-to-pas
      (physical-address-listp (mv-nth 1 (las-to-pas l-addrs r-x cpl x86))))

    (defthm x86p-mv-nth-2-las-to-pas
      (implies (x86p x86)
               (x86p (mv-nth 2 (las-to-pas l-addrs r-x cpl x86)))))

    (defthm las-to-pas-l-addrs=nil
      (and (equal (mv-nth 0 (las-to-pas nil r-x cpl x86)) nil)
           (equal (mv-nth 1 (las-to-pas nil r-x cpl x86)) nil)
           (equal (mv-nth 2 (las-to-pas nil r-x cpl x86)) x86)))
     ...
     (defthm xr-las-to-pas
       (implies
        (and (not (equal fld :mem))
             (not (equal fld :fault)))
        (equal (xr fld index (mv-nth 2 (las-to-pas l-addrs r-x cpl x86)))
               (xr fld index x86)))
    ...
    (defthm flgi-las-to-pas
      (equal (flgi index (mv-nth 2 (las-to-pas l-addrs r-x cpl x86)))
             (flgi index x86))
      :hints (("Goal" :in-theory (e/d* (flgi) (force (force))))))

    (defthm xr-fault-las-to-pas
      (implies (not (mv-nth 0 (las-to-pas l-addrs r-x cpl (double-rewrite x86))))
               (equal (xr :fault 0 (mv-nth 2 (las-to-pas l-addrs r-x cpl x86)))
                      (xr :fault 0 x86)))
      :hints (("Goal" :in-theory (e/d* () (force (force))))))
    ...
    (defthm las-to-pas-xw-rflags-not-ac
      (implies (equal (rflags-slice :ac value)
               ...
               (and
                (equal (mv-nth 0 (las-to-pas l-addrs r-x cpl (xw :rflags 0 value x86)))
                       (mv-nth 0 (las-to-pas l-addrs r-x cpl x86)))
                (equal (mv-nth 1 (las-to-pas l-addrs r-x cpl (xw :rflags 0 value x86)))
                       (mv-nth 1 (las-to-pas l-addrs r-x cpl x86))))))
    ...
    (defthm las-to-pas-xw-rflags-state-not-ac
      (implies (equal (rflags-slice :ac value)
                      (rflags-slice :ac (rflags x86)))
               (equal (mv-nth 2 (las-to-pas l-addrs r-x cpl (xw :rflags 0 value x86)))
                      (xw :rflags 0 value (mv-nth 2 (las-to-pas l-addrs r-x cpl x86)))))
      :hints (("Goal" :in-theory (e/d* () (force (force))))))

    (defthm mv-nth-2-las-to-pas-system-level-non-marking-mode
      (implies (and (not (page-structure-marking-mode x86))
                    (not (mv-nth 0 (las-to-pas l-addrs r-x cpl x86))))
               (equal (mv-nth 2 (las-to-pas l-addrs r-x cpl x86))
                      x86))
      :hints (("Goal" :in-theory (e/d (las-to-pas) (force (force))))))

    (defthm len-of-mv-nth-1-las-to-pas
      (implies (not (mv-nth 0 (las-to-pas l-addrs r-x cpl x86)))
               (equal (len (mv-nth 1 (las-to-pas l-addrs r-x cpl x86)))
                      (len l-addrs))))

    (defthm las-to-pas-values-and-!flgi
      (implies (and (not (equal index *ac*))
                    (x86p x86))
               (and (equal (mv-nth 0 (las-to-pas l-addrs r-x cpl (!flgi index value x86)))
                           (mv-nth 0 (las-to-pas l-addrs r-x cpl (double-rewrite x86))))
                    (equal (mv-nth 1 (las-to-pas l-addrs r-x cpl (!flgi index value x86)))
                           (mv-nth 1 (las-to-pas l-addrs r-x cpl (double-rewrite x86))))))
      :hints
      (("Goal"
        :do-not-induct t
    ...
    (defthm las-to-pas-values-and-!flgi-undefined
      (implies (and (not (equal index *ac*))
                    (x86p x86))
               (and (equal (mv-nth 0 (las-to-pas l-addrs r-x cpl (!flgi-undefined index x86)))
                           (mv-nth 0 (las-to-pas l-addrs r-x cpl x86)))
                    (equal (mv-nth 1 (las-to-pas l-addrs r-x cpl (!flgi-undefined index x86)))
                           (mv-nth 1 (las-to-pas l-addrs r-x cpl x86)))))
    ...
    (defthm mv-nth-2-las-to-pas-and-!flgi-not-ac-commute
      (implies (and (not (equal index *ac*))
                    (x86p x86))
               (equal (mv-nth 2 (las-to-pas l-addrs r-x cpl (!flgi index value x86)))
                      (!flgi index value (mv-nth 2 (las-to-pas l-addrs r-x cpl x86)))))
      :hints
      (("Goal"
        :do-not-induct t
        :cases ((equal index *iopl*))
  ...
  (define las-to-pas-tailrec ((l-addrs canonical-address-listp)
    ...
    (if (atom l-addrs)
      ...
      (b* (((mv flg p-addr x86)
           ...
           ((when flg) (mv flg nil x86)))
        (las-to-pas-tailrec
         (cdr l-addrs) (cons p-addr p-addrs) r-x cpl x86)))
    ...
    (defthmd las-to-pas-tailrec-opener
      (implies
       (consp l-addrs)
       (equal
        (las-to-pas-tailrec l-addrs p-addrs r-x cpl x86)
        (mv-let
          (flg p-addr x86)
          (ia32e-la-to-pa (car l-addrs) r-x cpl x86)
          (if flg
              (mv flg () x86)
            (las-to-pas-tailrec (cdr l-addrs) (cons p-addr p-addrs) r-x cpl x86))))))

    (defthmd las-to-pas-tailrec-nil
      (equal
       (las-to-pas-tailrec () p-addrs r-x cpl x86)
       (mv nil (acl2::rev p-addrs) x86)))

    (local
     (defthmd las-to-pas-tailrec-as-las-to-pas-lemma
       (b* (((mv flg-new p-addrs-new x86-new)
             (las-to-pas l-addrs r-x cpl x86))
       ...
       :hints (("Goal" :induct (las-to-pas-tailrec
                                l-addrs p-addrs r-x cpl x86)))))

    (local
     (defthm las-to-pas-as-list
       (let ((mv (las-to-pas l-addrs r-x cpl x86)))
    ...
    (local
     (defthm las-to-pas-tailrec-as-list
       (let ((mv (las-to-pas-tailrec l-addrs p-addrs r-x cpl x86)))
         (equal mv (list (mv-nth 0 mv)
                         (mv-nth 1 mv)
                         (mv-nth 2 mv))))
       :rule-classes ()))
    ...
    (local
     (defrule last-to-pas-when-flg
       (b* (((mv flg p-addrs ?x86) (las-to-pas l-addrs r-x cpl x86)))
         (implies flg (not p-addrs)))))

    (defthmd las-to-pas-as-las-to-pas-tailrec
      (equal (las-to-pas l-addrs r-x cpl x86)
             (las-to-pas-tailrec l-addrs () r-x cpl x86))
      :hints (("Goal" :use
               (las-to-pas-as-list
  ...
  (define rb ((n natp "Number of bytes to be read")
    ...
    (if (programmer-level-mode x86)
      ...
      (b* (((mv flgs p-addrs x86)
            (las-to-pas (create-canonical-address-list n addr) r-x (cpl x86) x86))
  ...
  (define wb ((n natp "Number of bytes to be written")
    ...
    (if (programmer-level-mode x86)
      ...
      (b* (((mv flgs p-addrs x86)
            (las-to-pas (create-canonical-address-list n addr) :w (cpl x86) x86))
  ...
  (defthm xr-fault-wb-in-system-level-marking-mode
    (implies
     (not (mv-nth 0 (las-to-pas (create-canonical-address-list n addr)
  ...
  (defthm xr-fault-wb-in-system-level-mode
    (implies (and (not (mv-nth 0 (las-to-pas
                                  (create-canonical-address-list n addr) :w (cpl x86) x86)))
                  (not (page-structure-marking-mode x86)))
             (equal (xr :fault 0 (mv-nth 1 (wb n addr w val x86)))
                    (xr :fault 0 x86)))
    :hints
...
;;          (program-byte (car bytes))
;;          ((mv flg byte-read ?x86)
;;           (rb 1 addr :x x86))
;;          ((when flg) nil)
;;          ((when (not (equal program-byte byte-read)))
;;           nil))
;;       (program-at (cdr addresses) (cdr bytes) x86)))
