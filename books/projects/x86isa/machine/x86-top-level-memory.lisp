		...
		(mbe
		 ...
		 (rvm16 lin-addr x86))
	      ...
	      (let* ((cs-segment (the (unsigned-byte 16) (seg-visiblei *cs* x86)))
		     (cpl (the (unsigned-byte 2) (seg-sel-layout-slice :rpl cs-segment)))
		     (low-bits (mbe :logic (loghead 12 lin-addr)
				    :exec (the (unsigned-byte 12) (logand #.*2^12-1* lin-addr)))))

		(cond ((< low-bits (+ -1 #.*2^12*))
		       ;; We will not cross a page boundary.
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr r-w-x cpl x86))
			    ((when flag) (mv (list 'rm16 flag) 0 x86))
			    (1+p-addr (the (unsigned-byte #.*physical-address-size*) (1+ p-addr)))

			    (byte0 (the (unsigned-byte 8) (memi   p-addr  x86)))
			    (byte1 (the (unsigned-byte 8) (memi 1+p-addr x86)))
		      ...
		      (t
		       ;; We might cross a page boundary.
  ...
  (if (mbt (canonical-address-p lin-addr))
	...
	(if (mbe :logic (canonical-address-p 1+lin-addr)
		...
		(cond ((< low-bits (+ -1 #.*2^12*))
		       ...
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr :w cpl x86))
			    ((when flag) (mv (list 'wm16 flag) x86))

			    ((the (unsigned-byte #.*physical-address-size*) 1+p-addr)
			     (1+ p-addr))
			    ...
			    (x86 (!memi   p-addr  byte0 x86))
			    (x86 (!memi 1+p-addr byte1 x86)))
			   (mv nil x86)))
...
(define rm32
  ...
  (if (mbt (canonical-address-p lin-addr))
	...
	(if (mbe :logic (canonical-address-p 3+lin-addr)
		...
		(cond ((< low-bits (+ -3 #.*2^12*))
		       ;; We will not cross a page boundary.
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr r-w-x cpl x86))
			    ((when flag) (mv (list 'rm32 flag) 0 x86))

			    ((the (unsigned-byte #.*physical-address-size*) 1+p-addr)
			     (+ 1 p-addr))
			    ((the (unsigned-byte #.*physical-address-size*) 2+p-addr)
			     (+ 2 p-addr))
			    ((the (unsigned-byte #.*physical-address-size*) 3+p-addr)
			     (+ 3 p-addr))

			    (byte0 (the (unsigned-byte 8) (memi   p-addr x86)))
			    (byte1 (the (unsigned-byte 8) (memi 1+p-addr x86)))
			    (byte2 (the (unsigned-byte 8) (memi 2+p-addr x86)))
			    (byte3 (the (unsigned-byte 8) (memi 3+p-addr x86)))
		      ...
		      ((equal low-bits (+ -2 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr r-w-x cpl x86))
			    ((when flag) (mv (list 'rm32 flag) 0 x86))

			    (2+lin-addr (the (signed-byte #.*max-linear-address-size+1*)
			    ...
			    ((the (unsigned-byte #.*physical-address-size*) 1+p-addr)
			     (+ 1 p-addr))
			    ((the (unsigned-byte #.*physical-address-size*) 1+p-addr2)
			     (+ 1 p-addr2))

			    (byte0 (the (unsigned-byte 8) (memi p-addr       x86)))
			    (byte1 (the (unsigned-byte 8) (memi 1+p-addr     x86)))
			    (byte2 (the (unsigned-byte 8) (memi p-addr2      x86)))
			    (byte3 (the (unsigned-byte 8) (memi 1+p-addr2    x86)))
		      ...
		      (t
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    ((the (unsigned-byte #.*physical-address-size*) 1+p-addr1)
			     (+ 1 p-addr1))
			    ((the (unsigned-byte #.*physical-address-size*) 2+p-addr1)
			     (+ 2 p-addr1))

			    (byte0 (the (unsigned-byte 8) (memi p-addr    x86)))
			    (byte1 (the (unsigned-byte 8) (memi p-addr1   x86)))
			    (byte2 (the (unsigned-byte 8) (memi 1+p-addr1 x86)))
			    (byte3 (the (unsigned-byte 8) (memi 2+p-addr1 x86)))
  ...
  (if (mbt (canonical-address-p lin-addr))
	...
	(if (mbe :logic (canonical-address-p 3+lin-addr)
		...
		(cond ((< low-bits (+ -3 #.*2^12*))
		       ...
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr :w cpl x86))
			    ((when flag) (mv (list 'wm32 flag) x86))
			    ...
			    (1+p-addr (the (signed-byte #.*physical-address-size+1*)
					(+ 1 p-addr)))
			    (2+p-addr (the (signed-byte #.*physical-address-size+1*)
					(+ 2 p-addr)))
			    (3+p-addr (the (signed-byte #.*physical-address-size+1*)
					(+ 3 p-addr)))

			    (x86 (!memi      p-addr  byte0 x86))
			    (x86 (!memi 1+p-addr byte1 x86))
			    (x86 (!memi 2+p-addr byte2 x86))
			    (x86 (!memi 3+p-addr byte3 x86)))
			   (mv nil x86)))
		      ...
		      ((equal low-bits (+ -3 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    ((the (signed-byte #.*max-linear-address-size+1*) 3+lin-addr)
			     (+ 3 lin-addr))
			    ((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr3) x86)
			    ...
			    ((the (unsigned-byte #.*physical-address-size+1*) 1+p-addr)
			     (+ 1 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+1*) 2+p-addr)
			     (+ 2 p-addr))
		      ...
		      ((equal low-bits (+ -2 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    ((the (signed-byte #.*max-linear-address-size+1*) 2+lin-addr)
			     (+ 2 lin-addr))
			    ((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr2) x86)
			    ...
			    ((the (unsigned-byte #.*physical-address-size+1*) 1+p-addr)
			     (+ 1 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+1*) 1+p-addr2)
			     (+ 1 p-addr2))

			    (x86 (!memi      p-addr  byte0 x86))
			    (x86 (!memi (+ 1 p-addr) byte1 x86))
			    (x86 (!memi      p-addr2  byte2 x86))
			    (x86 (!memi (+ 1 p-addr2) byte3 x86)))
			   (mv nil x86)))
		      ...
		      (t
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    ((the (unsigned-byte #.*physical-address-size+1*) 1+p-addr1)
			     (+ 1 p-addr1))
			    ((the (unsigned-byte #.*physical-address-size+1*) 2+p-addr1)
			     (+ 2 p-addr1))

			    (x86 (!memi      p-addr  byte0 x86))
			    (x86 (!memi      p-addr1  byte1 x86))
			    (x86 (!memi (+ 1 p-addr1)  byte2 x86))
			    (x86 (!memi (+ 2 p-addr1) byte3 x86)))
			   (mv nil x86))))))
...
(define rm64
  ...
  (if (mbt (canonical-address-p lin-addr))
	...
	(if (mbe :logic (canonical-address-p 7+lin-addr)
		...
		(cond ((< low-bits (+ -7 #.*2^12*))
		       ;; We will not cross a page boundary.
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr r-w-x cpl x86))
			    ((when flag) (mv (list 'rm64 flag) 0 x86))

			    ((the (unsigned-byte #.*physical-address-size+1*) 1+p-addr)
			     (+ 1 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+1*) 2+p-addr)
			     (+ 2 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+1*) 3+p-addr)
			     (+ 3 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+1*) 4+p-addr)
			     (+ 4 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+1*) 5+p-addr)
			     (+ 5 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+1*) 6+p-addr)
			     (+ 6 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+2*) 7+p-addr)
			     (+ 7 p-addr))
			    ...
			    (byte2 (the (unsigned-byte 8) (memi 2+p-addr x86)))
			    (byte3 (the (unsigned-byte 8) (memi 3+p-addr x86)))
			    (byte4 (the (unsigned-byte 8) (memi 4+p-addr x86)))
			    (byte5 (the (unsigned-byte 8) (memi 5+p-addr x86)))
			    (byte6 (the (unsigned-byte 8) (memi 6+p-addr x86)))
			    (byte7 (the (unsigned-byte 8) (memi 7+p-addr x86)))
			    ...
			    (word2 (the (unsigned-byte 16) (logior (the (unsigned-byte 16) (ash byte5 8))
								   byte4)))
			    (word3 (the (unsigned-byte 16) (logior (the (unsigned-byte 16) (ash byte7 8))
								   byte6)))

			    (dword0 (the (unsigned-byte 32) (logior (the (unsigned-byte 32) (ash word1 16))
								    word0)))
			    (dword1 (the (unsigned-byte 32) (logior (the (unsigned-byte 32) (ash word3 16))
								    word2)))

			    (qword (the (unsigned-byte 64) (logior (the (unsigned-byte 64) (ash dword1 32))
								   dword0))))

			   (mv nil qword x86)))

		      ((equal low-bits (+ -7 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr r-w-x cpl x86))
			    ((when flag) (mv (list 'rm64 flag) 0 x86))
			    ...
			    (byte5 (the (unsigned-byte 8) (memi 5+p-addr x86)))
			    (byte6 (the (unsigned-byte 8) (memi 6+p-addr x86)))
			    (byte7 (the (unsigned-byte 8) (memi  p-addr7  x86)))
		      ...
		      ((equal low-bits (+ -6 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr r-w-x cpl x86))
			    ((when flag) (mv (list 'rm64 flag) 0 x86))

			    ((the (signed-byte #.*max-linear-address-size+1*) 6+lin-addr)
			     (+ 6 lin-addr))
			    ...
			    ((the (unsigned-byte #.*physical-address-size+1*) 5+p-addr)
			     (+ 5 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+1*) 1+p-addr6)
			     (+ 1 p-addr6))
			    ...
			    (byte4 (the (unsigned-byte 8) (memi 4+p-addr  x86)))
			    (byte5 (the (unsigned-byte 8) (memi 5+p-addr  x86)))
			    (byte6 (the (unsigned-byte 8) (memi   p-addr6 x86)))
			    (byte7 (the (unsigned-byte 8) (memi 1+p-addr6 x86)))
		      ...
		      ((equal low-bits (+ -5 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr r-w-x cpl x86))
			    ((when flag) (mv (list 'rm64 flag) 0 x86))

			    ((the (signed-byte #.*max-linear-address-size+1*) 5+lin-addr)
			     (+ 5 lin-addr))
			    ...
			    ((the (unsigned-byte #.*physical-address-size+1*) 4+p-addr)
			     (+ 4 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+1*) 1+p-addr5)
			     (+ 1 p-addr5))
			    ((the (unsigned-byte #.*physical-address-size+1*) 2+p-addr5)
			     (+ 2 p-addr5))
			    ...
			    (byte3 (the (unsigned-byte 8) (memi 3+p-addr  x86)))
			    (byte4 (the (unsigned-byte 8) (memi 4+p-addr  x86)))
			    (byte5 (the (unsigned-byte 8) (memi  p-addr5  x86)))
			    (byte6 (the (unsigned-byte 8) (memi 1+p-addr5 x86)))
			    (byte7 (the (unsigned-byte 8) (memi 2+p-addr5 x86)))
		      ...
		      ((equal low-bits (+ -4 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr r-w-x cpl x86))
			    ((when flag) (mv (list 'rm64 flag) 0 x86))

			    ((the (signed-byte #.*max-linear-address-size+1*) 4+lin-addr)
			     (+ 4 lin-addr))
			    ...
			    ((the (unsigned-byte #.*physical-address-size+1*) 3+p-addr)
			     (+ 3 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+1*) 1+p-addr4)
			     (+ 1 p-addr4))
			    ((the (unsigned-byte #.*physical-address-size+1*) 2+p-addr4)
			     (+ 2 p-addr4))
			    ((the (unsigned-byte #.*physical-address-size+1*) 3+p-addr4)
			     (+ 3 p-addr4))
			    ...
			    (byte2 (the (unsigned-byte 8) (memi 2+p-addr  x86)))
			    (byte3 (the (unsigned-byte 8) (memi 3+p-addr  x86)))
			    (byte4 (the (unsigned-byte 8) (memi   p-addr4 x86)))
			    (byte5 (the (unsigned-byte 8) (memi 1+p-addr4 x86)))
			    (byte6 (the (unsigned-byte 8) (memi 2+p-addr4 x86)))
			    (byte7 (the (unsigned-byte 8) (memi 3+p-addr4 x86)))
		      ...
		      ((equal low-bits (+ -3 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    ((the (unsigned-byte #.*physical-address-size+1*) 2+p-addr)
			     (+ 2 p-addr))
			    ((the (unsigned-byte #.*physical-address-size+1*) 1+p-addr3)
			     (+ 1 p-addr3))
			    ((the (unsigned-byte #.*physical-address-size+1*) 2+p-addr3)
			     (+ 2 p-addr3))
			    ((the (unsigned-byte #.*physical-address-size+1*) 3+p-addr3)
			     (+ 3 p-addr3))
			    ((the (unsigned-byte #.*physical-address-size+1*) 4+p-addr3)
			     (+ 4 p-addr3))
			    ...
			    (byte2 (the (unsigned-byte 8) (memi  2+p-addr x86)))
			    (byte3 (the (unsigned-byte 8) (memi   p-addr3 x86)))
			    (byte4 (the (unsigned-byte 8) (memi 1+p-addr3 x86)))
			    (byte5 (the (unsigned-byte 8) (memi 2+p-addr3 x86)))
			    (byte6 (the (unsigned-byte 8) (memi 3+p-addr3 x86)))
			    (byte7 (the (unsigned-byte 8) (memi 4+p-addr3 x86)))
		      ...
		      ((equal low-bits (+ -2 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    ((the (unsigned-byte #.*physical-address-size+1*) 1+p-addr2)
			     (+ 1 p-addr2))
			    ((the (unsigned-byte #.*physical-address-size+1*) 2+p-addr2)
			     (+ 2 p-addr2))
			    ((the (unsigned-byte #.*physical-address-size+1*) 3+p-addr2)
			     (+ 3 p-addr2))
			    ((the (unsigned-byte #.*physical-address-size+1*) 4+p-addr2)
			     (+ 4 p-addr2))
			    ((the (unsigned-byte #.*physical-address-size+1*) 5+p-addr2)
			     (+ 5 p-addr2))
			    ...
			    (byte2 (the (unsigned-byte 8) (memi   p-addr2 x86)))
			    (byte3 (the (unsigned-byte 8) (memi 1+p-addr2 x86)))
			    (byte4 (the (unsigned-byte 8) (memi 2+p-addr2 x86)))
			    (byte5 (the (unsigned-byte 8) (memi 3+p-addr2 x86)))
			    (byte6 (the (unsigned-byte 8) (memi 4+p-addr2 x86)))
			    (byte7 (the (unsigned-byte 8) (memi 5+p-addr2 x86)))
		      ...
		      (t
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    ((the (unsigned-byte #.*physical-address-size+1*) 2+p-addr1)
			     (+ 2 p-addr1))
			    ((the (unsigned-byte #.*physical-address-size+1*) 3+p-addr1)
			     (+ 3 p-addr1))
			    ((the (unsigned-byte #.*physical-address-size+1*) 4+p-addr1)
			     (+ 4 p-addr1))
			    ((the (unsigned-byte #.*physical-address-size+1*) 5+p-addr1)
			     (+ 5 p-addr1))
			    ((the (unsigned-byte #.*physical-address-size+1*) 6+p-addr1)
			     (+ 6 p-addr1))
			    ...
			    (byte2 (the (unsigned-byte 8) (memi 1+p-addr1 x86)))
			    (byte3 (the (unsigned-byte 8) (memi 2+p-addr1 x86)))
			    (byte4 (the (unsigned-byte 8) (memi 3+p-addr1 x86)))
			    (byte5 (the (unsigned-byte 8) (memi 4+p-addr1 x86)))
			    (byte6 (the (unsigned-byte 8) (memi 5+p-addr1 x86)))
			    (byte7 (the (unsigned-byte 8) (memi 6+p-addr1 x86)))
	...
	(if (mbe :logic (canonical-address-p 7+lin-addr)
		...
		(mbe
		 ...
		 (wvm64 lin-addr val x86))
	      ...
	      (let* ((cs-segment (the (unsigned-byte 64) (seg-visiblei *cs* x86)))
		     (cpl (the (unsigned-byte 2) (seg-sel-layout-slice :rpl cs-segment)))
		     (low-bits (mbe :logic (loghead 12 lin-addr)
				    :exec (the (unsigned-byte 12) (logand #.*2^12-1* lin-addr)))))
		...
		(cond ((< low-bits (+ -7 #.*2^12*))
		       ...
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr :w cpl x86))
			    ((when flag) (mv (list 'wm64 flag) x86))
			    ...
			    (x86 (!memi 2+p-addr byte2 x86))
			    (x86 (!memi 3+p-addr byte3 x86))
			    (x86 (!memi 4+p-addr byte4 x86))
			    (x86 (!memi 5+p-addr byte5 x86))
			    (x86 (!memi 6+p-addr byte6 x86))
			    (x86 (!memi 7+p-addr byte7 x86)))
		      ...
		      ((equal low-bits (+ -5 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    (x86 (!memi 1+p-addr5 byte6 x86))
			    (x86 (!memi 2+p-addr5 byte7 x86)))
		      ...
		      ((equal low-bits (+ -4 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    (x86 (!memi 1+p-addr4 byte5 x86))
			    (x86 (!memi 2+p-addr4 byte6 x86))
			    (x86 (!memi 3+p-addr4 byte7 x86)))
			   (mv nil x86)))
		      ...
		      ((equal low-bits (+ -3 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    (x86 (!memi  2+p-addr byte2 x86))
			    (x86 (!memi   p-addr3 byte3 x86))
			    (x86 (!memi 1+p-addr3 byte4 x86))
			    (x86 (!memi 2+p-addr3 byte5 x86))
			    (x86 (!memi 3+p-addr3 byte6 x86))
			    (x86 (!memi 4+p-addr3 byte7 x86)))
		      ...
		      ((equal low-bits (+ -2 #.*2^12*))
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    (x86 (!memi   p-addr2 byte2 x86))
			    (x86 (!memi 1+p-addr2 byte3 x86))
			    (x86 (!memi 2+p-addr2 byte4 x86))
			    (x86 (!memi 3+p-addr2 byte5 x86))
			    (x86 (!memi 4+p-addr2 byte6 x86))
			    (x86 (!memi 5+p-addr2 byte7 x86)))
		      ...
		      (t
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			    ...
			    (x86 (!memi 1+p-addr1 byte2 x86))
			    (x86 (!memi 2+p-addr1 byte3 x86))
			    (x86 (!memi 3+p-addr1 byte4 x86))
			    (x86 (!memi 4+p-addr1 byte5 x86))
			    (x86 (!memi 5+p-addr1 byte6 x86))
			    (x86 (!memi 6+p-addr1 byte7 x86)))
	...
	(if (mbe :logic (canonical-address-p 15+lin-addr)
		...
		(cond ((< low-bits (+ -15 #.*2^12*))
		       ...
		       (b* (((mv flag
			    ...
			    (qword0 (the (unsigned-byte 64) (rm-low-64 p-addr x86)))
			    (qword1 (the (unsigned-byte 64) (rm-low-64 (+ 8 p-addr) x86)))

			    (oword (the (unsigned-byte 128) (logior (the (unsigned-byte 128) (ash qword1 64))
								    qword0))))

			   (mv nil oword x86)))
		      (t
		       (b* (((mv flag0
				 (the (unsigned-byte 64) qword0)
				 x86)
			     (rm64 lin-addr r-w-x x86))
			    ((when flag0)
			     (mv (list 'rm128 flag0)
  ...
  (if (mbt (canonical-address-p lin-addr))
	...
	(if (mbe :logic (canonical-address-p 15+lin-addr)
		...
		(cond ((< low-bits (+ -15 #.*2^12*))
		       ...
		       (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			     (la-to-pa lin-addr :w cpl x86))
			    ((when flag) (mv (list 'wm128 flag) x86))
			    ...
			    (x86 (!memi (+ 6 p-addr) byte6 x86))
			    (x86 (!memi (+ 7 p-addr) byte7 x86))
			    (x86 (!memi (+ 8 p-addr) byte8 x86))
			    (x86 (!memi (+ 9 p-addr) byte9 x86))
			    (x86 (!memi (+ 10 p-addr) byte10 x86))
			    (x86 (!memi (+ 11 p-addr) byte11 x86))
			    (x86 (!memi (+ 12 p-addr) byte12 x86))
			    (x86 (!memi (+ 13 p-addr) byte13 x86))
			    (x86 (!memi (+ 14 p-addr) byte14 x86))
			    (x86 (!memi (+ 15 p-addr) byte15 x86)))
	    ...
	    (cond ((< low-bits (+ -7 #.*2^12*))
		   ...
		   (b* (((mv flag (the (unsigned-byte #.*physical-address-size*) p-addr) x86)
			...
			((the (signed-byte 32) canonical-address-high-int)
			 (mbe
			  :logic
			  (n16-to-i16 (part-select canonical-address
						   :low 32 :high 47))
			  :exec
			  (the (signed-byte 16) (ash canonical-address -32))))
...
(define program-at (addresses bytes x86)
  ...
  :parents (x86-top-level-memory reasoning-about-memory-regions)
  :non-executable t
