...
(include-book "x86-concrete-memory")
...
(defun define-abstract-stobj-indices ()
  ...
  ;; To add an index, add the proper constant name in the defconsts
  ;; args below, and increment the last arg of gl-int.

  `(defconsts (*rgfi* *rflags* *rip* *seg-visiblei* *seg-hiddeni*
                      *stri* *ssr-visiblei* *ssr-hiddeni* *ctri* *dbgi*
                      *fp-datai* *fp-ctrl* *fp-status* *fp-tag* *fp-last-inst*
                      *fp-last-data* *fp-opcode* *xmmi* *mxcsr* *cpl* *msri*
                      *ms* *fault* *env* *undef* *programmer-level-mode* *memi*
                      *x86-abs-stobj-len*)
...
(defsection field-recognizers
  ...
  ;; (x86$a-recognizers-2 *pruned-x86-model*)
...
(defsection abstract-stobj-recognizer
  ...
  ;; (append-elements (create-x86-abs-stobj-recognizer-2 *pruned-x86-model*) nil)
...
(defsection field-accessors-and-updaters
  ...
  ;; (x86$a-accessors-and-updaters-2 *pruned-x86-model*)
...
(DEFTHML !MEMI{PRESERVED}
  (IMPLIES (AND (X86$AP X86) (N52P I) (N08P V))
           (X86$AP (!MEM$AI I V X86)))
  :RULE-CLASSES NIL)
