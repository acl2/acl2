...
(def-inst x86-syscall
  ...
  (b* ((ctx 'x86-syscall)
       ;; Halt if SYSCALL is called when we are not in programmer-level
       ;; mode.
       ((when (not (programmer-level-mode x86)))
	(!!ms-fresh :syscall-unsupported-in-system-mode))
	...
	(!!ms-fresh :ia32-efer-sce-or-lma=0 (cons 'ia32_efer-slice ia32-efer-lma)))
