...
(include-book "instructions-spec/add-adc-sub-sbb-or-and-xor-cmp-test"
              :ttags (:include-raw :syscall-exec :other-non-det :undef-flg))
(include-book "instructions-spec/sal-sar-shl-shr-rcl-rcr-rol-ror"
              :ttags (:include-raw :syscall-exec :other-non-det :undef-flg))
(include-book "instructions-spec/mul-imul"
              :ttags (:include-raw :syscall-exec :other-non-det :undef-flg))
(include-book "instructions-spec/div-idiv"
              :ttags (:include-raw :syscall-exec :other-non-det :undef-flg))
...
(local
 (def-gl-thm x86-syscall-guard-helper-1
   :hyp (unsigned-byte-p 64 msri)
   :concl (equal (logand 65532
                         (bitops::logsquash
                          2
                          (loghead 16 (logtail 32 msri))))
                 (logand 65532
                         (loghead 16 (logtail 32 msri))))
   :g-bindings (gl::auto-bindings
                (:nat msri    64))))
...
(local
 (def-gl-thm x86-syscall-guard-helper-2
   :hyp (unsigned-byte-p 112 seg-hiddeni)
   :concl (and
           (equal (logior 19342794667089993085747200
                          (logand -79228162495817593519834398721
                                  seg-hiddeni))
                  (logior 19342794667089993085747200
                          (logand 5192217630372331810936976494821375
                                  seg-hiddeni)))
           (equal (logior 19342794667089993085747200
                          (logand 5192217630372313364192902785269760
                                  (bitops::logsquash 64 seg-hiddeni)))
                  (logand 5192296858534809181786422619668480
                          (logior 19342794667089993085747200
                                  (logand 5192217630372331810936976494821375
                                          seg-hiddeni)))))
...
(local
 (def-gl-thm x86-syscall-guard-helper-3
   :hyp (unsigned-byte-p 112 seg-hiddeni)
   :concl
   (and
    (equal
     (logior
      (logand 79228162495817593519834398720
              (logior 19342794667089993085747200
                      (logand 5192217630372331810936976494821375
                              seg-hiddeni)))
      (ash
            ...
            (logand
             ...
             (logior
              ...
              (logand 63999
                      (logior 2048
                              (logand 63487
                                      (logtail 96 seg-hiddeni)))))))))))
       96))
     (logior
      (logand 5192296858534809181786422619668480
              (logior 19342794667089993085747200
                      (loghead 64 seg-hiddeni)))
      (ash
       (logior
        11
        (logand
...
(local
 (def-gl-thm x86-sysret-guard-helper-3
   :hyp (unsigned-byte-p 64 msri)
   :concl (and
...
(local
 (defthm logand-of-two-equishifted-numbers-helper-1
   (implies (natp i)
            (equal (loghead i (logand (ash a i) (ash x i)))
                   0))
   :hints (("Goal" :in-theory (e/d () (unsigned-byte-p))))))

(local
 (defthm logand-of-two-equishifted-numbers-helper-2
   (implies (natp i)
            (equal (logtail i (logand (ash a i) (ash x i)))
                   (logand a x)))
   :hints (("Goal" :in-theory (e/d () (unsigned-byte-p))))))
...
(encapsulate
 ...
 (defthmd joining-loghead-and-logtail
   (implies (and (natp i)
                 (natp x))
            (equal (logior (ash (logtail i x) i)
                           (loghead i x))
                   x))
   :hints (("Goal" :in-theory (e/d (logtail loghead)
                                   ())))))

(defthm logand-of-two-equishifted-numbers
  (implies (and (natp x)
                (natp i))
           (equal (logand (ash a i) (ash x i))
                  (ash (logand a x) i)))
  :hints (("Goal" :in-theory (e/d () ())
           :use ((:instance joining-loghead-and-logtail
                            (i i)
                            (x (logand (ash a i) (ash x i))))))))

(local
 (defthm logand-of-two-equishifted-numbers-restricted-to-64-bits
   (implies (and (equal y (ash (1- (ash 1 64)) i))
                 (unsigned-byte-p 64 x)
                 (natp i))
            (equal (logand y (ash x i))
                   (ash x i)))))
...
;;  ;; The following is useful for MBE kind of proofs.  Be careful
;;  ;; when using it though; sometimes, it might cause loops when
;;  ;; enabled alongside logand-with-negated-bitmask and
;;  ;; logand-with-bitmask.

;;  (defthmd big-logand-to-small-logand
