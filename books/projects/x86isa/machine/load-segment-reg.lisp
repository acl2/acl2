  ...
  (defun load-segment-reg (seg-reg selector x86)
    (declare (xargs :stobjs (x86)
                    :guard (and (integerp seg-reg)
                                ...
                                (>= selector 0)
                                (< selector #x10000))))
    (b* (((mv & descriptor x86) (rml64 (+ (i64 (gdtr/idtrBits->base-addr (stri (if (logtest selector #x4)
                                                                                 *ldtr*
                                                                                 *gdtr*)
                                                                               x86)))
         ...
         (base (logior (logand (ash descriptor -16) #xFFFFFF)
                       (logand (ash descriptor (- 24 56)) #xFF000000)))
  ...
  (defun load-system-segment-reg (seg-reg selector x86)
    (declare (xargs :stobjs (x86)
                    :guard (and (integerp seg-reg)
                                ...
                                (>= selector 0)
                                (< selector #x10000))))
    (b* ((x86 (!ssr-visiblei seg-reg selector x86))
         ((mv & descriptor x86) (rml64 (+ (i64 (gdtr/idtrBits->base-addr (stri (if (logtest selector #x4)
                                                                                 *ldtr*
                                                                                 *gdtr*)
                                                                               x86)))
                                          (logand selector #xFFF8))
