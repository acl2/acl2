...
x86_model_state *load_state_from_file(char *filename) {
  ...
  state->mem = mem_create();
  for (mem_file_element *current =
           (mem_file_element *)((uintptr_t)file_contents + sizeof(state_file));
       (uintptr_t)current + (1 << 20) <= (uintptr_t)file_contents + st.st_size;
       current++) {
    memcpy(l2_ptr(state->mem, current->pg_addr)->page, current->contents,
           1 << 20);
  }
...
void vm_load_model_state(vm *v, x86_model_state *state) {
  ...
  // We use mmap instead of malloc to get a page aligned chunk
  // of memory (which is required to pass it to the VM)
  v->guest_mem = mmap(NULL, 4UL * (1 << 30), PROT_READ | PROT_WRITE,
                      MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);

  for (size_t i1 = 0; i1 < 1 << 22; i1++)
    if (state->mem->l1s[i1] != NULL)
      for (size_t i2 = 0; i2 < 1 << 22; i2++)
        if (state->mem->l1s[i1]->l2s[i2] != NULL) {
          memcpy((void *)((uintptr_t)v->guest_mem + (i1 << 42UL | i2 << 20UL)),
                 state->mem->l1s[i1]->l2s[i2]->page, 1 << 20);

          // state->mem->l1s[i1]->l2s[i2]->slot = state->mem->min_inused_slot++;
          // vm_link_mem(v, state->mem->l1s[i1]->l2s[i2]->slot,
          //             i1 << 42 | i2 << 20,
          //             state->mem->l1s[i1]->l2s[i2]->page, 1 << 20);
        }
  // We skip the first page because that will be used for MMIO
  // TODO make it so that first page isn't allocated
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    vm_run_outside_hole_single_instruction(v);
    gprs_file real_gprs;
