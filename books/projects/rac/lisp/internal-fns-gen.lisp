...
;; function's local variables.  This book implements tools that assist such an
;; approach for functions generated by the RAC compiler.  In particular, the
;; presented tools extract terms that compute values of local variables bound
;; by LET/LET*/B*/MV-LET/MV?-LET bindings, which are declared in function
;; definitions.  Two tools are provided: CONST-FNS-GEN and LOOP-FNS-GEN.
;; Depending on whether the definition of the input function is recursive, the
;; appropriate tool may be applied: CONST-FNS-GEN for functions with
;; non-recursive definitions, and LOOP-FNS-GEN for functions with recursive
;; definitions.  The application of LOOP-FNS-GEN is fairly restrictive; it
;; requires that one of the arguments of the recursive input function is the
;; counter that increases/decreases by one at each recursive call, and the
;; termination of the input function is based on this argument.
