...
tuple<ui13, bool, ui105, bool, bool, bool, bool> rightShft(si14 expBiased, bool hugeNegScale, ui106 prod) {
  ...
  if (hugeNegScale || expDeficit.slc<8>(6) != 0) {
    shift.set_slc(1, ui5(0x1F));
  }
  ...
  stkMask.set_slc(52, stkShftMask.slc<55>(0));
  ...
  return tuple<ui13, bool, ui105, bool, bool, bool, bool>(expShft, expInc, frac105, stkShft, lsb, grd, stk);
}

// Perform left shift if leading zero count is positive and biased sum of exponents is greater than 1:

tuple<ui13, bool, ui105, bool, bool, bool, bool> leftShft(si14 expBiased, ui106 prod, ui6 clz) {
    
  si14 expDiff = expBiased - clz;

  // Shift amount:
  ui6 shift = expDiff == 0 ? ui6(clz - 1) : expDiff > 0 ? clz : ui6(expBiased - 1);

  // Shifted product and adjusted exponent:
  ui106 prodShft = prod << shift;
  ui13 expShft = expDiff > 0 ? expDiff : si14(0);
  ...
  return tuple<ui13, bool, ui105, bool, bool, bool, bool>(expShft, expInc, frac105, 0, lsb, grd, stk);
}
...
tuple<ui117, ui8, bool, bool, bool> fmul64(ui64 opa, ui64 opb, si64 scale, bool fz, bool dn, ui2 rmode, bool fused, bool fscale) {
  ...
  if (classa == ZERO || classa == INF || classa == SNAN || classa == QNAN ||
    ...
    si14 expbUnbiased = fscale ? si14(scale) : expb == 0 ? si14(-0x3FE) : si14(expb - 0x3FF);
    si14 expaBiased = expa == 0 ? si14(1) : si14(expa);
    ...
    ui13 expShft;        // expShft + expInc is biased exponent of shifted product
    bool expInc;
    ui105 frac105;       // fraction to be returned for FMA
    bool stkShft;        // sticky bit for FMA
    bool lsb, grd, stk;  // lsb, guard, and sticky bits for FMUL/FSCALE 

    if (expBiased <= 0 || hugeNegScale) {
      tie(expShft, expInc, frac105, stkShft, lsb, grd, stk) = rightShft(expBiased, hugeNegScale, prod);
    }
    else {
      ...
      tie(expShft, expInc, frac105, stkShft, lsb, grd, stk) = leftShft(expBiased, prod, clz);
    }
    ...
    if (fused) { // FMA case
      ...
      bool rndUp = rmode == RNE && grd && (lsb || stk) ||
...
SC_MODULE(fmul) {
  ...
  void doit() {
    ...
    ui8 excps;
    bool piz, inz, egti;
    tie(data, excps, piz, inz, egti) = fmul64(opa, fscale ? ui64(0x3FF0000000000000) : opb, scale, fz, dn, rmode, !fscale && fused, fscale);
...
ui64 opa = 0x0008000000000000;
ui64 scale = 1;
ui2 rmode = 0;
bool dn = 1, fz = 0. fma = 0, fscale = 1;

ui117 D;
ui8 flags;
bool prodInfZero, infNanZero, expOvfl;
tie(D, flags, prodInfZero, infNanZero, expOvfl) = fmul64(opa, 0x3FF0000000000000, scale, fz, dn, rmode, fma, fscale);
ui64 Dlo = D.slc<64>(0);

printf("opa = %s\n", opa.to_string(AC_HEX, false).c_str());
printf("scale = %s\n", scale.to_string(AC_HEX, false).c_str());
printf("D[63:0] = %s\n", Dlo.to_string(AC_HEX, false).c_str());
