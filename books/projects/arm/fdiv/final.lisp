...
(local-defund ovf-flags ()
  (setbitn (setbitn (flags-b) 8 (ofc) 1) 8 (ixc) 1))

(local-defund unf-flags ()
  (if1 (fzp)
       (setbitn (flags-b) 8 (ufc) 1)
       (let ((flags (setbitn (flags-b) 8 (ixc) (logior1 (bitn (flags-b) (ixc)) (inxden)))))
         (setbitn flags 8 (ufc) (logior1 (bitn flags (ufc)) (inxden))))))

(local-defund norm-flags ()
  (setbitn (flags-b) 8 (ixc) (logior1 (bitn (flags-b) (ixc)) (inx))))
  ...
  (let* ((f (case (fnum) (0 (hp)) (1 (sp)) (2 (dp))))
    ...
    (mv-let (data flags) (execute (opa) (opb) (fnum) fzp dnp rmode)
      (mv-let (data-spec r-spec)
        ...
        (and (equal data data-spec)
             (equal (logior (rin) flags) r-spec)))))
  :hints (("Goal" :use (fdiv64-main execute-lemma)
