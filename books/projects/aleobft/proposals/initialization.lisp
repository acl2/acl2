  ...
  (xdoc::topstring
   ...
   (xdoc::p
    "There is a unique possible initial state
     ...
     that returns that initial validator state.")
   (xdoc::p
  ...
  (xdoc::topstring
   (xdoc::p
    "Initially, each correct validator
     is in round 1,
     has empty DAG,
     has proposed nothing,
     has endorsed nothing,
...
(define system-init ((correct-vals address-setp))
  ...
  ((define system-init-loop ((correct-vals address-setp)
                             (valmap address-validator-state-mapp))
     :returns (new-valmap address-validator-state-mapp
                          :hyp (address-validator-state-mapp valmap))
     (b* (((when (set::emptyp (address-set-fix correct-vals)))
           (address-validator-state-map-fix valmap))
          (valmap (omap::update (set::head correct-vals)
                                (validator-init)
                                valmap)))
       (system-init-loop (set::tail correct-vals) valmap))
