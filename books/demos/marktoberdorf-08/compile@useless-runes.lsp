(M1::COLLECT-AT-END)
(M1::COLLECT-VARS-IN-EXPR (500 231 (:REWRITE DEFAULT-+-2))
                          (322 231 (:REWRITE DEFAULT-+-1))
                          (230 46 (:DEFINITION INTEGER-ABS))
                          (184 46 (:REWRITE COMMUTATIVITY-OF-+))
                          (184 23 (:DEFINITION LENGTH))
                          (115 23 (:DEFINITION LEN))
                          (73 73 (:REWRITE DEFAULT-CDR))
                          (64 54 (:REWRITE DEFAULT-<-2))
                          (60 54 (:REWRITE DEFAULT-<-1))
                          (46 46 (:REWRITE DEFAULT-UNARY-MINUS))
                          (42 42 (:REWRITE DEFAULT-CAR))
                          (23 23 (:TYPE-PRESCRIPTION LEN))
                          (23 23 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                          (23 23 (:REWRITE INTEGERP==>DENOMINATOR=1))
                          (23 23 (:REWRITE DEFAULT-REALPART))
                          (23 23 (:REWRITE DEFAULT-NUMERATOR))
                          (23 23 (:REWRITE DEFAULT-IMAGPART))
                          (23 23 (:REWRITE DEFAULT-DENOMINATOR))
                          (23 23 (:REWRITE DEFAULT-COERCE-2))
                          (23 23 (:REWRITE DEFAULT-COERCE-1))
                          (2 2 (:META CANCEL_TIMES-EQUAL-CORRECT))
                          (2 2 (:META CANCEL_PLUS-EQUAL-CORRECT)))
(M1::COLLECT-VARS-IN-STMT* (215 107 (:REWRITE DEFAULT-+-2))
                           (143 107 (:REWRITE DEFAULT-+-1))
                           (120 24 (:DEFINITION INTEGER-ABS))
                           (96 24 (:REWRITE COMMUTATIVITY-OF-+))
                           (96 12 (:DEFINITION LENGTH))
                           (60 12 (:DEFINITION LEN))
                           (36 30 (:REWRITE DEFAULT-<-2))
                           (33 30 (:REWRITE DEFAULT-<-1))
                           (24 24 (:REWRITE DEFAULT-UNARY-MINUS))
                           (12 12 (:TYPE-PRESCRIPTION LEN))
                           (12 12 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                           (12 12 (:REWRITE INTEGERP==>DENOMINATOR=1))
                           (12 12 (:REWRITE DEFAULT-REALPART))
                           (12 12 (:REWRITE DEFAULT-NUMERATOR))
                           (12 12 (:REWRITE DEFAULT-IMAGPART))
                           (12 12 (:REWRITE DEFAULT-DENOMINATOR))
                           (12 12 (:REWRITE DEFAULT-COERCE-2))
                           (12 12 (:REWRITE DEFAULT-COERCE-1))
                           (8 8 (:META CANCEL_TIMES-EQUAL-CORRECT))
                           (8 8 (:META CANCEL_PLUS-EQUAL-CORRECT))
                           (1 1
                              (:TYPE-PRESCRIPTION M1::COLLECT-AT-END)))
(M1::INDEX)
(M1::OP!)
(M1::LOAD!)
(M1::PUSH!)
(M1::EXPR! (500 231 (:REWRITE DEFAULT-+-2))
           (322 231 (:REWRITE DEFAULT-+-1))
           (230 46 (:DEFINITION INTEGER-ABS))
           (184 46 (:REWRITE COMMUTATIVITY-OF-+))
           (184 23 (:DEFINITION LENGTH))
           (115 23 (:DEFINITION LEN))
           (73 73 (:REWRITE DEFAULT-CDR))
           (64 54 (:REWRITE DEFAULT-<-2))
           (60 54 (:REWRITE DEFAULT-<-1))
           (46 46 (:REWRITE DEFAULT-UNARY-MINUS))
           (42 42 (:REWRITE DEFAULT-CAR))
           (23 23 (:TYPE-PRESCRIPTION LEN))
           (23 23 (:REWRITE NUMERATOR-WHEN-INTEGERP))
           (23 23 (:REWRITE INTEGERP==>DENOMINATOR=1))
           (23 23 (:REWRITE DEFAULT-REALPART))
           (23 23 (:REWRITE DEFAULT-NUMERATOR))
           (23 23 (:REWRITE DEFAULT-IMAGPART))
           (23 23 (:REWRITE DEFAULT-DENOMINATOR))
           (23 23 (:REWRITE DEFAULT-COERCE-2))
           (23 23 (:REWRITE DEFAULT-COERCE-1))
           (2 2 (:META CANCEL_TIMES-EQUAL-CORRECT))
           (2 2 (:META CANCEL_PLUS-EQUAL-CORRECT)))
(M1::IFLE!)
(M1::GOTO!)
(M1::WHILE!)
(M1::TEST!)
(M1::STORE!)
(M1::STMT*! (215 107 (:REWRITE DEFAULT-+-2))
            (143 107 (:REWRITE DEFAULT-+-1))
            (120 24 (:DEFINITION INTEGER-ABS))
            (96 24 (:REWRITE COMMUTATIVITY-OF-+))
            (96 12 (:DEFINITION LENGTH))
            (60 12 (:DEFINITION LEN))
            (36 30 (:REWRITE DEFAULT-<-2))
            (33 30 (:REWRITE DEFAULT-<-1))
            (24 24 (:REWRITE DEFAULT-UNARY-MINUS))
            (12 12 (:TYPE-PRESCRIPTION LEN))
            (12 12 (:REWRITE NUMERATOR-WHEN-INTEGERP))
            (12 12 (:REWRITE INTEGERP==>DENOMINATOR=1))
            (12 12 (:REWRITE DEFAULT-REALPART))
            (12 12 (:REWRITE DEFAULT-NUMERATOR))
            (12 12 (:REWRITE DEFAULT-IMAGPART))
            (12 12 (:REWRITE DEFAULT-DENOMINATOR))
            (12 12 (:REWRITE DEFAULT-COERCE-2))
            (12 12 (:REWRITE DEFAULT-COERCE-1))
            (8 8 (:META CANCEL_TIMES-EQUAL-CORRECT))
            (8 8 (:META CANCEL_PLUS-EQUAL-CORRECT))
            (1 1 (:TYPE-PRESCRIPTION TRUE-LISTP)))
(M1::COMPILE)
(M1::EXAMPLE-COMPILATION-1)
