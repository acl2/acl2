; Defabsstobj Example 1 Modified to use DFs

; Copyright (C) 2023, ForrestHunt, Inc.
; Written by Matt Kaufmann
; License: A 3-clause BSD license.  See the LICENSE file distributed with ACL2.

; Based on Defabsstobj Example 1
; Copyright (C) 2012, Regents of the University of Texas
; Written by Matt Kaufmann, July, 2012 (updated Nov. and Dec., 2012)
; License: A 3-clause BSD license.  See the LICENSE file distributed with ACL2.

;;; This is a slight modification of defabsstobj-example-1.lisp, to illustrate
;;; the use of dfs (see :DOC df) with abstract stobjs.
...
; the abstract stobj.
  ...
  (misc$c :type double-float :initially 0))
...
; This is of logical interest only, so no guard is given.
  ...
  (and (st$cp+ st$c)
       (st$ap st$a)
       (= (from-df (misc$c st$c)) (misc$a st$a))
...
(defthm update{preserved}-lemma
  (implies (and (integerp i) (<= 0 i) (<= i 49)
                (integerp v) (<= 0 v)
                (map$ap map)
                (mem$c-entryp v))
           (map$ap (put-assoc-equal i v map)))) 
...
(assert-event (df= (misc st) 0))

(assert-event (let ((st (update-misc (to-df 3) st)))
                (mv (df= (misc st) 3)
                    st))
              :stobjs-out '(nil st))
