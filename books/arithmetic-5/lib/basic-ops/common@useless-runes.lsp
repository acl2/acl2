(COLLECT-+)
(COLLECT-*)
(BUBBLE-DOWN)
(ADDEND-PATTERN)
(MATCHING-ADDEND-PATTERNS-P)
(MATCHING-ADDEND-P)
(FACTOR-PATTERN-BASE)
(MATCHING-EXPONENTS)
(FACTOR-PATTERN-GATHER-EXPONENTS (145 70 (:REWRITE DEFAULT-+-2))
                                 (95 70 (:REWRITE DEFAULT-+-1))
                                 (56 14 (:REWRITE COMMUTATIVITY-OF-+))
                                 (56 14 (:DEFINITION INTEGER-ABS))
                                 (56 7 (:DEFINITION LENGTH))
                                 (35 7 (:DEFINITION LEN))
                                 (27 27 (:REWRITE DEFAULT-CDR))
                                 (23 18 (:REWRITE DEFAULT-<-2))
                                 (22 18 (:REWRITE DEFAULT-<-1))
                                 (17 17 (:REWRITE DEFAULT-CAR))
                                 (14 14 (:REWRITE DEFAULT-UNARY-MINUS))
                                 (7 7 (:TYPE-PRESCRIPTION LEN))
                                 (7 7 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                                 (7 7 (:REWRITE INTEGERP==>DENOMINATOR=1))
                                 (7 7 (:REWRITE DEFAULT-REALPART))
                                 (7 7 (:REWRITE DEFAULT-NUMERATOR))
                                 (7 7 (:REWRITE DEFAULT-IMAGPART))
                                 (7 7 (:REWRITE DEFAULT-DENOMINATOR))
                                 (7 7 (:REWRITE DEFAULT-COERCE-2))
                                 (7 7 (:REWRITE DEFAULT-COERCE-1)))
(MATCHING-FACTOR-GATHER-EXPONENTS-PATTERNS-P)
(MATCHING-FACTOR-GATHER-EXPONENTS-P)
(FACTOR-PATTERN-SCATTER-EXPONENTS (171 83 (:REWRITE DEFAULT-+-2))
                                  (111 83 (:REWRITE DEFAULT-+-1))
                                  (64 16 (:REWRITE COMMUTATIVITY-OF-+))
                                  (64 16 (:DEFINITION INTEGER-ABS))
                                  (64 8 (:DEFINITION LENGTH))
                                  (40 8 (:DEFINITION LEN))
                                  (34 34 (:REWRITE DEFAULT-CDR))
                                  (27 21 (:REWRITE DEFAULT-<-2))
                                  (26 21 (:REWRITE DEFAULT-<-1))
                                  (22 22 (:REWRITE DEFAULT-CAR))
                                  (16 16 (:REWRITE DEFAULT-UNARY-MINUS))
                                  (8 8 (:TYPE-PRESCRIPTION LEN))
                                  (8 8 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                                  (8 8 (:REWRITE INTEGERP==>DENOMINATOR=1))
                                  (8 8 (:REWRITE DEFAULT-REALPART))
                                  (8 8 (:REWRITE DEFAULT-NUMERATOR))
                                  (8 8 (:REWRITE DEFAULT-IMAGPART))
                                  (8 8 (:REWRITE DEFAULT-DENOMINATOR))
                                  (8 8 (:REWRITE DEFAULT-COERCE-2))
                                  (8 8 (:REWRITE DEFAULT-COERCE-1)))
(MATCHING-FACTOR-SCATTER-EXPONENTS-PATTERNS-P)
(MATCHING-FACTOR-SCATTER-EXPONENTS-P)
(GOOD-VAL-TRIPLE-P)
(VAL-<)
(INFO-ENTRY-P)
(INFO-LIST-P)
(FACTOR-VAL-GATHER-EXPONENTS1 (397 190 (:REWRITE DEFAULT-+-2))
                              (254 190 (:REWRITE DEFAULT-+-1))
                              (144 36 (:REWRITE COMMUTATIVITY-OF-+))
                              (144 36 (:DEFINITION INTEGER-ABS))
                              (144 18 (:DEFINITION LENGTH))
                              (90 18 (:DEFINITION LEN))
                              (87 87 (:REWRITE DEFAULT-CDR))
                              (59 46 (:REWRITE DEFAULT-<-2))
                              (56 46 (:REWRITE DEFAULT-<-1))
                              (50 50 (:REWRITE DEFAULT-CAR))
                              (36 36 (:REWRITE DEFAULT-UNARY-MINUS))
                              (18 18 (:TYPE-PRESCRIPTION LEN))
                              (18 18 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                              (18 18 (:REWRITE INTEGERP==>DENOMINATOR=1))
                              (18 18 (:REWRITE DEFAULT-REALPART))
                              (18 18 (:REWRITE DEFAULT-NUMERATOR))
                              (18 18 (:REWRITE DEFAULT-IMAGPART))
                              (18 18 (:REWRITE DEFAULT-DENOMINATOR))
                              (18 18 (:REWRITE DEFAULT-COERCE-2))
                              (18 18 (:REWRITE DEFAULT-COERCE-1)))
(FACTOR-VAL-GATHER-EXPONENTS1-THM
     (81 81 (:REWRITE DEFAULT-CAR))
     (73 73 (:REWRITE DEFAULT-CDR))
     (14 2 (:DEFINITION ARG2))
     (8 8
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (7 7 (:REWRITE DEFAULT-UNARY-MINUS))
     (7 7 (:REWRITE DEFAULT-<-2))
     (7 7 (:REWRITE DEFAULT-<-1))
     (5 3 (:REWRITE DEFAULT-+-2))
     (5 3 (:REWRITE DEFAULT-+-1))
     (1 1 (:TYPE-PRESCRIPTION QUOTEP)))
(FACTOR-VAL-GATHER-EXPONENTS1)
(FACTOR-VAL-GATHER-EXPONENTS (342 166 (:REWRITE DEFAULT-+-2))
                             (222 166 (:REWRITE DEFAULT-+-1))
                             (128 32 (:REWRITE COMMUTATIVITY-OF-+))
                             (128 32 (:DEFINITION INTEGER-ABS))
                             (128 16 (:DEFINITION LENGTH))
                             (80 16 (:DEFINITION LEN))
                             (68 68 (:REWRITE DEFAULT-CDR))
                             (54 42 (:REWRITE DEFAULT-<-2))
                             (52 42 (:REWRITE DEFAULT-<-1))
                             (46 46 (:REWRITE DEFAULT-CAR))
                             (32 32 (:REWRITE DEFAULT-UNARY-MINUS))
                             (16 16 (:TYPE-PRESCRIPTION LEN))
                             (16 16 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                             (16 16 (:REWRITE INTEGERP==>DENOMINATOR=1))
                             (16 16 (:REWRITE DEFAULT-REALPART))
                             (16 16 (:REWRITE DEFAULT-NUMERATOR))
                             (16 16 (:REWRITE DEFAULT-IMAGPART))
                             (16 16 (:REWRITE DEFAULT-DENOMINATOR))
                             (16 16 (:REWRITE DEFAULT-COERCE-2))
                             (16 16 (:REWRITE DEFAULT-COERCE-1)))
(FACTOR-VAL-SCATTER-EXPONENTS1 (397 190 (:REWRITE DEFAULT-+-2))
                               (254 190 (:REWRITE DEFAULT-+-1))
                               (144 36 (:REWRITE COMMUTATIVITY-OF-+))
                               (144 36 (:DEFINITION INTEGER-ABS))
                               (144 18 (:DEFINITION LENGTH))
                               (90 18 (:DEFINITION LEN))
                               (87 87 (:REWRITE DEFAULT-CDR))
                               (59 46 (:REWRITE DEFAULT-<-2))
                               (56 46 (:REWRITE DEFAULT-<-1))
                               (50 50 (:REWRITE DEFAULT-CAR))
                               (36 36 (:REWRITE DEFAULT-UNARY-MINUS))
                               (18 18 (:TYPE-PRESCRIPTION LEN))
                               (18 18 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                               (18 18 (:REWRITE INTEGERP==>DENOMINATOR=1))
                               (18 18 (:REWRITE DEFAULT-REALPART))
                               (18 18 (:REWRITE DEFAULT-NUMERATOR))
                               (18 18 (:REWRITE DEFAULT-IMAGPART))
                               (18 18 (:REWRITE DEFAULT-DENOMINATOR))
                               (18 18 (:REWRITE DEFAULT-COERCE-2))
                               (18 18 (:REWRITE DEFAULT-COERCE-1)))
(FACTOR-VAL-SCATTER-EXPONENTS1-THM
     (81 81 (:REWRITE DEFAULT-CAR))
     (73 73 (:REWRITE DEFAULT-CDR))
     (14 2 (:DEFINITION ARG2))
     (8 8
        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (7 7 (:REWRITE DEFAULT-UNARY-MINUS))
     (7 7 (:REWRITE DEFAULT-<-2))
     (7 7 (:REWRITE DEFAULT-<-1))
     (5 3 (:REWRITE DEFAULT-+-2))
     (5 3 (:REWRITE DEFAULT-+-1))
     (1 1 (:TYPE-PRESCRIPTION QUOTEP)))
(FACTOR-VAL-SCATTER-EXPONENTS1)
(FACTOR-VAL-SCATTER-EXPONENTS (342 166 (:REWRITE DEFAULT-+-2))
                              (222 166 (:REWRITE DEFAULT-+-1))
                              (128 32 (:REWRITE COMMUTATIVITY-OF-+))
                              (128 32 (:DEFINITION INTEGER-ABS))
                              (128 16 (:DEFINITION LENGTH))
                              (80 16 (:DEFINITION LEN))
                              (68 68 (:REWRITE DEFAULT-CDR))
                              (54 42 (:REWRITE DEFAULT-<-2))
                              (52 42 (:REWRITE DEFAULT-<-1))
                              (46 46 (:REWRITE DEFAULT-CAR))
                              (32 32 (:REWRITE DEFAULT-UNARY-MINUS))
                              (16 16 (:TYPE-PRESCRIPTION LEN))
                              (16 16 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                              (16 16 (:REWRITE INTEGERP==>DENOMINATOR=1))
                              (16 16 (:REWRITE DEFAULT-REALPART))
                              (16 16 (:REWRITE DEFAULT-NUMERATOR))
                              (16 16 (:REWRITE DEFAULT-IMAGPART))
                              (16 16 (:REWRITE DEFAULT-DENOMINATOR))
                              (16 16 (:REWRITE DEFAULT-COERCE-2))
                              (16 16 (:REWRITE DEFAULT-COERCE-1)))
(FACTOR-GATHER-EXPONENTS-INFO-ENTRY)
(ASSOC-FACTOR-GATHER-EXPONENTS (9 9 (:REWRITE DEFAULT-CDR))
                               (6 6 (:REWRITE DEFAULT-CAR))
                               (2 1 (:DEFINITION TRUE-LISTP)))
(FACTOR-GATHER-EXPONENTS-INTERSECT-INFO-LISTS
     (25608 25608 (:REWRITE DEFAULT-CDR))
     (21400 21400 (:REWRITE DEFAULT-CAR))
     (162 162
          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (60 30 (:REWRITE DEFAULT-<-2))
     (60 30 (:REWRITE DEFAULT-<-1)))
(FACTOR-GATHER-EXPONENTS-INFO-LIST
     (815 391 (:REWRITE DEFAULT-+-2))
     (520 391 (:REWRITE DEFAULT-+-1))
     (296 74 (:REWRITE COMMUTATIVITY-OF-+))
     (296 74 (:DEFINITION INTEGER-ABS))
     (296 37 (:DEFINITION LENGTH))
     (195 195 (:REWRITE DEFAULT-CDR))
     (185 37 (:DEFINITION LEN))
     (121 94 (:REWRITE DEFAULT-<-2))
     (113 94 (:REWRITE DEFAULT-<-1))
     (105 105 (:REWRITE DEFAULT-CAR))
     (74 74 (:REWRITE DEFAULT-UNARY-MINUS))
     (37 37 (:TYPE-PRESCRIPTION LEN))
     (37 37 (:REWRITE NUMERATOR-WHEN-INTEGERP))
     (37 37 (:REWRITE INTEGERP==>DENOMINATOR=1))
     (37 37 (:REWRITE DEFAULT-REALPART))
     (37 37 (:REWRITE DEFAULT-NUMERATOR))
     (37 37 (:REWRITE DEFAULT-IMAGPART))
     (37 37 (:REWRITE DEFAULT-DENOMINATOR))
     (37 37 (:REWRITE DEFAULT-COERCE-2))
     (37 37 (:REWRITE DEFAULT-COERCE-1)))
(TEMP-1 (4767 4767 (:REWRITE DEFAULT-CDR))
        (3628 3628 (:REWRITE DEFAULT-CAR))
        (16 16
            (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(TEMP-2 (1053 1047 (:REWRITE DEFAULT-CDR))
        (891 885 (:REWRITE DEFAULT-CAR))
        (107 107
             (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
        (92 50 (:REWRITE DEFAULT-<-1))
        (88 50 (:REWRITE DEFAULT-<-2)))
(TEMP-3 (81 81 (:REWRITE DEFAULT-CAR))
        (73 73 (:REWRITE DEFAULT-CDR))
        (14 2 (:DEFINITION ARG2))
        (7 7 (:REWRITE DEFAULT-UNARY-MINUS))
        (7 7 (:REWRITE DEFAULT-<-2))
        (7 7 (:REWRITE DEFAULT-<-1))
        (5 3 (:REWRITE DEFAULT-+-2))
        (5 3 (:REWRITE DEFAULT-+-1))
        (4 4
           (:REWRITE FACTOR-VAL-GATHER-EXPONENTS1-THM))
        (1 1 (:TYPE-PRESCRIPTION QUOTEP)))
(TEMP-4 (250 5
             (:DEFINITION FACTOR-VAL-GATHER-EXPONENTS1))
        (198 180 (:REWRITE DEFAULT-CDR))
        (155 149 (:REWRITE DEFAULT-CAR))
        (92 23 (:DEFINITION ARG1))
        (60 10 (:DEFINITION RATIONAL-CONSTANT-P))
        (15 15 (:REWRITE DEFAULT-UNARY-MINUS))
        (15 15 (:REWRITE DEFAULT-<-2))
        (15 15 (:REWRITE DEFAULT-<-1))
        (10 10
            (:REWRITE FACTOR-VAL-GATHER-EXPONENTS1-THM))
        (10 5 (:REWRITE DEFAULT-+-2))
        (10 5 (:REWRITE DEFAULT-+-1)))
(FACTOR-GATHER-EXPONENTS-INFO-LIST-THM
     (1503 9 (:DEFINITION FACTOR-PATTERN-BASE))
     (1260 42 (:DEFINITION EXPT))
     (1224 9 (:DEFINITION POWER-OF-2))
     (1020 1019 (:REWRITE DEFAULT-CDR))
     (915 914 (:REWRITE DEFAULT-CAR))
     (711 42 (:DEFINITION POWER-OF-2-HELPER))
     (515 5
          (:DEFINITION FACTOR-GATHER-EXPONENTS-INTERSECT-INFO-LISTS))
     (486 150 (:REWRITE DEFAULT-*-2))
     (450 9
          (:DEFINITION FACTOR-VAL-GATHER-EXPONENTS1))
     (442 64
          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (318 243 (:REWRITE DEFAULT-+-2))
     (310 285 (:REWRITE DEFAULT-<-2))
     (310 285 (:REWRITE DEFAULT-<-1))
     (287 41 (:DEFINITION ARG2))
     (260 5
          (:DEFINITION ASSOC-FACTOR-GATHER-EXPONENTS))
     (252 243 (:REWRITE DEFAULT-+-1))
     (252 84 (:REWRITE COMMUTATIVITY-OF-+))
     (225 5
          (:DEFINITION MATCHING-FACTOR-GATHER-EXPONENTS-PATTERNS-P))
     (195 5 (:DEFINITION VAL-<))
     (150 150 (:REWRITE DEFAULT-*-1))
     (132 33
          (:REWRITE FUNCTIONAL-COMMUTATIVITY-OF-MINUS-*-RIGHT))
     (126 42 (:REWRITE ZIP-OPEN))
     (125 125 (:REWRITE DEFAULT-UNARY-MINUS))
     (63 21 (:REWRITE <-0-MINUS))
     (54 9 (:DEFINITION INTEGER-CONSTANT-P))
     (18 18
         (:REWRITE FACTOR-VAL-GATHER-EXPONENTS1-THM))
     (16 8 (:DEFINITION TRUE-LISTP))
     (2 2
        (:TYPE-PRESCRIPTION FACTOR-GATHER-EXPONENTS-INTERSECT-INFO-LISTS)))
(FACTOR-GATHER-EXPONENTS-INFO-LIST)
(FIRST-MATCH-IN-FACTOR-GATHER-EXPONENTS-INFO-LISTS
     (24129 24129 (:REWRITE DEFAULT-CDR))
     (21110 21110 (:REWRITE DEFAULT-CAR))
     (4518 1506
           (:REWRITE STATE-P-IMPLIES-AND-FORWARD-TO-STATE-P1))
     (170 170
          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(FIND-MATCHING-FACTORS-GATHER-EXPONENTS)
(FIND-MATCHING-FACTORS-GATHER-EXPONENTS)
(FIRST-RATIONAL-MATCH-IN-FACTOR-GATHER-EXPONENTS-INFO-LISTS
     (10554 10554 (:REWRITE DEFAULT-CDR))
     (9554 9554 (:REWRITE DEFAULT-CAR))
     (1944 648
           (:REWRITE STATE-P-IMPLIES-AND-FORWARD-TO-STATE-P1))
     (88 88
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(FIND-RATIONAL-MATCHING-FACTORS-GATHER-EXPONENTS)
(FIND-RATIONAL-MATCHING-FACTORS-GATHER-EXPONENTS)
(FIRST-NON-ZERO-RATIONAL-MATCH-IN-FACTOR-GATHER-EXPONENTS-INFO-LISTS
     (10554 10554 (:REWRITE DEFAULT-CDR))
     (9554 9554 (:REWRITE DEFAULT-CAR))
     (1944 648
           (:REWRITE STATE-P-IMPLIES-AND-FORWARD-TO-STATE-P1))
     (88 88
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(FIND-NON-ZERO-RATIONAL-MATCHING-FACTORS-GATHER-EXPONENTS)
(FIND-NON-ZERO-RATIONAL-MATCHING-FACTORS-GATHER-EXPONENTS)
(FACTOR-SCATTER-EXPONENTS-INFO-ENTRY)
(ASSOC-FACTOR-SCATTER-EXPONENTS (9 9 (:REWRITE DEFAULT-CDR))
                                (6 6 (:REWRITE DEFAULT-CAR))
                                (2 1 (:DEFINITION TRUE-LISTP)))
(FACTOR-SCATTER-EXPONENTS-INTERSECT-INFO-LISTS
     (2162 2162 (:REWRITE DEFAULT-CDR))
     (1960 1960 (:REWRITE DEFAULT-CAR))
     (60 60
         (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (60 30 (:REWRITE DEFAULT-<-2))
     (60 30 (:REWRITE DEFAULT-<-1)))
(FACTOR-SCATTER-EXPONENTS-INFO-LIST
     (644 308 (:REWRITE DEFAULT-+-2))
     (409 308 (:REWRITE DEFAULT-+-1))
     (232 58 (:REWRITE COMMUTATIVITY-OF-+))
     (232 58 (:DEFINITION INTEGER-ABS))
     (232 29 (:DEFINITION LENGTH))
     (161 161 (:REWRITE DEFAULT-CDR))
     (145 29 (:DEFINITION LEN))
     (94 73 (:REWRITE DEFAULT-<-2))
     (87 73 (:REWRITE DEFAULT-<-1))
     (82 82 (:REWRITE DEFAULT-CAR))
     (58 58 (:REWRITE DEFAULT-UNARY-MINUS))
     (29 29 (:TYPE-PRESCRIPTION LEN))
     (29 29 (:REWRITE NUMERATOR-WHEN-INTEGERP))
     (29 29 (:REWRITE INTEGERP==>DENOMINATOR=1))
     (29 29 (:REWRITE DEFAULT-REALPART))
     (29 29 (:REWRITE DEFAULT-NUMERATOR))
     (29 29 (:REWRITE DEFAULT-IMAGPART))
     (29 29 (:REWRITE DEFAULT-DENOMINATOR))
     (29 29 (:REWRITE DEFAULT-COERCE-2))
     (29 29 (:REWRITE DEFAULT-COERCE-1)))
(TEMP-1 (131 131 (:REWRITE DEFAULT-CDR))
        (116 116 (:REWRITE DEFAULT-CAR))
        (28 14 (:DEFINITION TRUE-LISTP)))
(TEMP-2 (540 534 (:REWRITE DEFAULT-CAR))
        (513 507 (:REWRITE DEFAULT-CDR))
        (92 50 (:REWRITE DEFAULT-<-1))
        (88 50 (:REWRITE DEFAULT-<-2))
        (80 80
            (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(TEMP-3 (81 81 (:REWRITE DEFAULT-CAR))
        (73 73 (:REWRITE DEFAULT-CDR))
        (14 2 (:DEFINITION ARG2))
        (7 7 (:REWRITE DEFAULT-UNARY-MINUS))
        (7 7 (:REWRITE DEFAULT-<-2))
        (7 7 (:REWRITE DEFAULT-<-1))
        (5 3 (:REWRITE DEFAULT-+-2))
        (5 3 (:REWRITE DEFAULT-+-1))
        (4 4
           (:REWRITE FACTOR-VAL-SCATTER-EXPONENTS1-THM))
        (1 1 (:TYPE-PRESCRIPTION QUOTEP)))
(TEMP-4 (250 5
             (:DEFINITION FACTOR-VAL-SCATTER-EXPONENTS1))
        (198 180 (:REWRITE DEFAULT-CDR))
        (155 149 (:REWRITE DEFAULT-CAR))
        (92 23 (:DEFINITION ARG1))
        (60 10 (:DEFINITION RATIONAL-CONSTANT-P))
        (15 15 (:REWRITE DEFAULT-UNARY-MINUS))
        (15 15 (:REWRITE DEFAULT-<-2))
        (15 15 (:REWRITE DEFAULT-<-1))
        (10 10
            (:REWRITE FACTOR-VAL-SCATTER-EXPONENTS1-THM))
        (10 5 (:REWRITE DEFAULT-+-2))
        (10 5 (:REWRITE DEFAULT-+-1)))
(FACTOR-SCATTER-EXPONENTS-INFO-LIST-THM
     (6160 5
           (:DEFINITION FACTOR-GATHER-EXPONENTS-INFO-LIST))
     (5460 10
           (:DEFINITION FACTOR-GATHER-EXPONENTS-INFO-ENTRY))
     (4660 10
           (:DEFINITION FACTOR-PATTERN-GATHER-EXPONENTS))
     (3173 19 (:DEFINITION FACTOR-PATTERN-BASE))
     (2584 19 (:DEFINITION POWER-OF-2))
     (1740 58 (:DEFINITION EXPT))
     (1690 1689 (:REWRITE DEFAULT-CDR))
     (1491 1490 (:REWRITE DEFAULT-CAR))
     (1480 10 (:DEFINITION POWER-OF-2-GENERALIZED))
     (922 922
          (:TYPE-PRESCRIPTION POWER-OF-2-HELPER))
     (922 124
          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
     (903 58 (:DEFINITION POWER-OF-2-HELPER))
     (844 480
          (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
     (790 10
          (:DEFINITION FACTOR-VAL-GATHER-EXPONENTS))
     (658 194 (:REWRITE DEFAULT-*-2))
     (567 81 (:DEFINITION ARG2))
     (515 5
          (:DEFINITION FACTOR-GATHER-EXPONENTS-INTERSECT-INFO-LISTS))
     (500 10
          (:DEFINITION FACTOR-VAL-GATHER-EXPONENTS1))
     (480 480
          (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
     (480 480
          (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
     (480 430 (:REWRITE DEFAULT-<-2))
     (480 430 (:REWRITE DEFAULT-<-1))
     (468 78 (:DEFINITION RATIONAL-CONSTANT-P))
     (450 9
          (:DEFINITION FACTOR-VAL-SCATTER-EXPONENTS1))
     (426 329 (:REWRITE DEFAULT-+-2))
     (422 422
          (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
     (422 422
          (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
     (390 10 (:DEFINITION VAL-<))
     (348 329 (:REWRITE DEFAULT-+-1))
     (348 116 (:REWRITE COMMUTATIVITY-OF-+))
     (295 5
          (:DEFINITION FACTOR-SCATTER-EXPONENTS-INTERSECT-INFO-LISTS))
     (260 5
          (:DEFINITION ASSOC-FACTOR-GATHER-EXPONENTS))
     (225 5
          (:DEFINITION MATCHING-FACTOR-GATHER-EXPONENTS-PATTERNS-P))
     (194 194 (:REWRITE DEFAULT-*-1))
     (187 187 (:REWRITE DEFAULT-UNARY-MINUS))
     (174 58 (:REWRITE ZIP-OPEN))
     (156 39
          (:REWRITE FUNCTIONAL-COMMUTATIVITY-OF-MINUS-*-RIGHT))
     (114 19 (:DEFINITION INTEGER-CONSTANT-P))
     (87 29 (:REWRITE <-0-MINUS))
     (40 5
         (:DEFINITION ASSOC-FACTOR-SCATTER-EXPONENTS))
     (20 20
         (:REWRITE FACTOR-VAL-GATHER-EXPONENTS1-THM))
     (18 18
         (:REWRITE FACTOR-VAL-SCATTER-EXPONENTS1-THM))
     (17 17
         (:TYPE-PRESCRIPTION FACTOR-GATHER-EXPONENTS-INFO-LIST))
     (12 6 (:DEFINITION TRUE-LISTP))
     (5 5
        (:TYPE-PRESCRIPTION MATCHING-FACTOR-SCATTER-EXPONENTS-PATTERNS-P))
     (2 2
        (:TYPE-PRESCRIPTION FACTOR-SCATTER-EXPONENTS-INTERSECT-INFO-LISTS)))
(FACTOR-SCATTER-EXPONENTS-INFO-LIST)
(FIRST-MATCH-IN-FACTOR-SCATTER-EXPONENTS-INFO-LISTS
     (1946 1946 (:REWRITE DEFAULT-CAR))
     (1616 1616 (:REWRITE DEFAULT-CDR))
     (480 160
          (:REWRITE STATE-P-IMPLIES-AND-FORWARD-TO-STATE-P1)))
(FIND-MATCHING-FACTORS-SCATTER-EXPONENTS)
(FIND-MATCHING-FACTORS-SCATTER-EXPONENTS)
(FIRST-RATIONAL-MATCH-IN-FACTOR-SCATTER-EXPONENTS-INFO-LISTS
     (1226 1226 (:REWRITE DEFAULT-CAR))
     (1003 1003 (:REWRITE DEFAULT-CDR))
     (216 72
          (:REWRITE STATE-P-IMPLIES-AND-FORWARD-TO-STATE-P1)))
(FIND-RATIONAL-MATCHING-FACTORS-SCATTER-EXPONENTS)
(FIND-RATIONAL-MATCHING-FACTORS-SCATTER-EXPONENTS)
(FIRST-NON-ZERO-RATIONAL-MATCH-IN-FACTOR-SCATTER-EXPONENTS-INFO-LISTS
     (1226 1226 (:REWRITE DEFAULT-CAR))
     (1003 1003 (:REWRITE DEFAULT-CDR))
     (216 72
          (:REWRITE STATE-P-IMPLIES-AND-FORWARD-TO-STATE-P1)))
(FIND-NON-ZERO-RATIONAL-MATCHING-FACTORS-SCATTER-EXPONENTS)
(FIND-NON-ZERO-RATIONAL-MATCHING-FACTORS-SCATTER-EXPONENTS)
(ARITH-FACTOR-PATTERN-SCATTER-EXPONENTS
     (171 83 (:REWRITE DEFAULT-+-2))
     (111 83 (:REWRITE DEFAULT-+-1))
     (64 16 (:REWRITE COMMUTATIVITY-OF-+))
     (64 16 (:DEFINITION INTEGER-ABS))
     (64 8 (:DEFINITION LENGTH))
     (40 8 (:DEFINITION LEN))
     (34 34 (:REWRITE DEFAULT-CDR))
     (27 21 (:REWRITE DEFAULT-<-2))
     (26 21 (:REWRITE DEFAULT-<-1))
     (22 22 (:REWRITE DEFAULT-CAR))
     (16 16 (:REWRITE DEFAULT-UNARY-MINUS))
     (8 8 (:TYPE-PRESCRIPTION LEN))
     (8 8 (:REWRITE NUMERATOR-WHEN-INTEGERP))
     (8 8 (:REWRITE INTEGERP==>DENOMINATOR=1))
     (8 8 (:REWRITE DEFAULT-REALPART))
     (8 8 (:REWRITE DEFAULT-NUMERATOR))
     (8 8 (:REWRITE DEFAULT-IMAGPART))
     (8 8 (:REWRITE DEFAULT-DENOMINATOR))
     (8 8 (:REWRITE DEFAULT-COERCE-2))
     (8 8 (:REWRITE DEFAULT-COERCE-1))
     (1 1
        (:TYPE-PRESCRIPTION ARITH-FACTOR-PATTERN-SCATTER-EXPONENTS)))
(ARITH-MATCHING-FACTOR-SCATTER-EXPONENTS-PATTERNS-P)
(ARITH-MATCHING-FACTOR-SCATTER-EXPONENTS-P)
(FOO-0 (5 5 (:REWRITE DEFAULT-<-2))
       (5 5 (:REWRITE DEFAULT-<-1))
       (2 2 (:REWRITE DEFAULT-*-2))
       (2 2 (:REWRITE DEFAULT-*-1))
       (1 1 (:REWRITE ZP-OPEN))
       (1 1 (:REWRITE DEFAULT-UNARY-/)))
(PRIME-FACTORS-2 (19 17 (:REWRITE DEFAULT-<-2))
                 (17 17 (:REWRITE DEFAULT-<-1))
                 (12 11 (:REWRITE DEFAULT-+-2))
                 (11 11 (:REWRITE DEFAULT-+-1))
                 (10 10 (:REWRITE DEFAULT-*-2))
                 (10 10 (:REWRITE DEFAULT-*-1))
                 (6 5 (:REWRITE DEFAULT-UNARY-MINUS))
                 (4 4 (:REWRITE DEFAULT-UNARY-/))
                 (2 2 (:REWRITE ZP-OPEN))
                 (2 2
                    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(BUT-NTH)
(HOW-MANY-FACTORS)
(FOO-1 (46 46 (:REWRITE DEFAULT-+-2))
       (46 46 (:REWRITE DEFAULT-+-1))
       (26 14 (:REWRITE DEFAULT-<-1))
       (25 25 (:REWRITE DEFAULT-CDR))
       (14 14 (:REWRITE DEFAULT-<-2)))
(FOO-2 (898 447 (:REWRITE DEFAULT-+-2))
       (606 447 (:REWRITE DEFAULT-+-1))
       (392 98 (:REWRITE COMMUTATIVITY-OF-+))
       (392 98 (:DEFINITION INTEGER-ABS))
       (392 49 (:DEFINITION LENGTH))
       (245 49 (:DEFINITION LEN))
       (241 183 (:REWRITE DEFAULT-<-2))
       (219 183 (:REWRITE DEFAULT-<-1))
       (148 148 (:REWRITE DEFAULT-CDR))
       (98 98 (:REWRITE DEFAULT-UNARY-MINUS))
       (73 73 (:REWRITE DEFAULT-CAR))
       (49 49 (:TYPE-PRESCRIPTION LEN))
       (49 49 (:REWRITE NUMERATOR-WHEN-INTEGERP))
       (49 49 (:REWRITE INTEGERP==>DENOMINATOR=1))
       (49 49 (:REWRITE DEFAULT-REALPART))
       (49 49 (:REWRITE DEFAULT-NUMERATOR))
       (49 49 (:REWRITE DEFAULT-IMAGPART))
       (49 49 (:REWRITE DEFAULT-DENOMINATOR))
       (49 49 (:REWRITE DEFAULT-COERCE-2))
       (49 49 (:REWRITE DEFAULT-COERCE-1))
       (32 32
           (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(FOO-3 (10 10 (:REWRITE DEFAULT-CAR))
       (7 7 (:REWRITE DEFAULT-CDR))
       (7 7 (:REWRITE DEFAULT-+-2))
       (7 7 (:REWRITE DEFAULT-+-1))
       (6 2 (:REWRITE FOLD-CONSTS-IN-+)))
(CONDENSE-FACTORS (65 2 (:DEFINITION ACL2-COUNT))
                  (24 12 (:REWRITE DEFAULT-+-2))
                  (16 12 (:REWRITE DEFAULT-+-1))
                  (12 1 (:DEFINITION BUT-NTH))
                  (8 2 (:REWRITE COMMUTATIVITY-OF-+))
                  (8 2 (:DEFINITION INTEGER-ABS))
                  (8 1 (:DEFINITION LENGTH))
                  (6 6 (:REWRITE DEFAULT-CDR))
                  (6 4 (:REWRITE FOLD-CONSTS-IN-+))
                  (5 1 (:DEFINITION LEN))
                  (4 4 (:REWRITE DEFAULT-CAR))
                  (4 3 (:REWRITE DEFAULT-<-2))
                  (4 1 (:REWRITE <-0-+-NEGATIVE-1))
                  (3 3 (:REWRITE DEFAULT-<-1))
                  (3 1 (:DEFINITION HOW-MANY-FACTORS))
                  (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
                  (1 1 (:TYPE-PRESCRIPTION LEN))
                  (1 1 (:REWRITE NUMERATOR-WHEN-INTEGERP))
                  (1 1 (:REWRITE INTEGERP==>DENOMINATOR=1))
                  (1 1 (:REWRITE DEFAULT-REALPART))
                  (1 1 (:REWRITE DEFAULT-NUMERATOR))
                  (1 1 (:REWRITE DEFAULT-IMAGPART))
                  (1 1 (:REWRITE DEFAULT-DENOMINATOR))
                  (1 1 (:REWRITE DEFAULT-COERCE-2))
                  (1 1 (:REWRITE DEFAULT-COERCE-1)))
(PRIME-FACTORS-1)
(PRIME-FACTORS)
