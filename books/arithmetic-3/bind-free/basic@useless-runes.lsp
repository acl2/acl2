(DEFAULT-EXPT-1
 (25 25
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (4 4
    (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (4 4
    (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (4 4
    (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (4 4
    (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (2 2 (:REWRITE ZIP-OPEN))
 (2 2 (:LINEAR EXPT->-1-2))
 (2 2 (:LINEAR EXPT->-1-1))
 (2 2 (:LINEAR EXPT-<-1-2))
 (2 2 (:LINEAR EXPT-<-1-1))
 (1 1
    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(DEFAULT-EXPT-2
 (44 24 (:REWRITE DEFAULT-<-2))
 (28 24 (:REWRITE DEFAULT-<-1))
 (25 25
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (25 25
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (25 25
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (16 4
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (16 4
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (16 4
     (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (16 4
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (8 2 (:LINEAR EXPT->-1-2))
 (8 2 (:LINEAR EXPT->-1-1))
 (8 2 (:LINEAR EXPT-<-1-2))
 (8 2 (:LINEAR EXPT-<-1-1))
 (2 1 (:REWRITE DEFAULT-EXPT-1)))
(|(+ y x)|)
(|(+ y (+ x z))|)
(|(+ (+ x y) z)|)
(|(+ 0 x)| (1 1
              (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(|(- (- x))| (1 1
                (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(|(- (+ x y))|)
(|(* y x)| (5 3 (:REWRITE DEFAULT-*-2))
           (5 3 (:REWRITE DEFAULT-*-1))
           (4 4
              (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(|(* y (* x z))| (7 5 (:REWRITE DEFAULT-*-1))
                 (6 5 (:REWRITE DEFAULT-*-2))
                 (3 3
                    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                 (3 3 (:REWRITE FOLD-CONSTS-IN-*)))
(|(* (* x y) z)|)
(|(* 1 x)| (1 1
              (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(|(* 0 x)|)
(|(* -1 x)|)
(|(/ (/ x))| (1 1
                (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(|(/ (* x y))|)
(|(* x (+ y z))| (7 5 (:REWRITE DEFAULT-*-2))
                 (6 5 (:REWRITE DEFAULT-*-1))
                 (5 5
                    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                 (4 3 (:REWRITE DEFAULT-+-2))
                 (4 3 (:REWRITE DEFAULT-+-1)))
(|(* (+ x y) z)| (6 4 (:REWRITE DEFAULT-*-1))
                 (5 5
                    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                 (5 4 (:REWRITE DEFAULT-*-2))
                 (4 3 (:REWRITE DEFAULT-+-2))
                 (4 3 (:REWRITE DEFAULT-+-1)))
(|(* a (/ a))| (2 2
                  (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
               (2 2 (:REWRITE DEFAULT-UNARY-/))
               (2 2 (:REWRITE DEFAULT-*-2))
               (2 2 (:REWRITE DEFAULT-*-1)))
(|(* a (/ a) b)| (2 2
                    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                 (1 1 (:REWRITE FOLD-CONSTS-IN-*))
                 (1 1 (:REWRITE DEFAULT-UNARY-/))
                 (1 1 (:REWRITE DEFAULT-*-2))
                 (1 1 (:REWRITE DEFAULT-*-1)))
(|(* x (- y))| (4 3 (:REWRITE DEFAULT-UNARY-MINUS))
               (3 3
                  (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
               (3 2 (:REWRITE DEFAULT-*-2))
               (3 2 (:REWRITE DEFAULT-*-1)))
(|(* (- x) y)| (4 3 (:REWRITE DEFAULT-UNARY-MINUS))
               (3 3
                  (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
               (3 2 (:REWRITE DEFAULT-*-2))
               (3 2 (:REWRITE DEFAULT-*-1)))
(|(- (* c x))| (4 3 (:REWRITE DEFAULT-UNARY-MINUS))
               (3 3
                  (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
               (3 2 (:REWRITE DEFAULT-*-2))
               (3 2 (:REWRITE DEFAULT-*-1)))
(|(/ (- x))|)
(EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP)))
(EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE
 (1 1
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (1 1
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (1 1
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (1 1
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (1 1
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (1 1
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT)))
(EXPT-TYPE-PRESCRIPTION-NON-0-BASE
  (1 1
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP)))
(EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP)))
(EXPT-TYPE-PRESCRIPTION-NONNEGATIVE-BASE
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (1 1
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP)))
(|(expt x 0)|
  (3 3
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONNEGATIVE-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP)))
(|(expt 0 n)|
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (4 4
    (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (4 4
    (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (4 4
    (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (4 4
    (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (2 2 (:REWRITE ZIP-OPEN))
 (2 2 (:LINEAR EXPT->-1-2))
 (2 2 (:LINEAR EXPT->-1-1))
 (2 2 (:LINEAR EXPT-<-1-2))
 (2 2 (:LINEAR EXPT-<-1-1))
 (1 1 (:REWRITE DEFAULT-EXPT-2))
 (1 1 (:REWRITE DEFAULT-EXPT-1)))
(|(expt x 1)|
  (3 3
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONNEGATIVE-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (3 3
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
  (1 1
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(|(expt 1 n)|
   (3 3
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
   (3 3
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
   (3 3
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP)))
(|(expt x -1)|
 (44 24 (:REWRITE DEFAULT-<-2))
 (33 33
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (28 24 (:REWRITE DEFAULT-<-1))
 (16 4
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (16 4
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (16 4
     (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (16 4
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (15 15
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONNEGATIVE-BASE))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (15 15
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (9 3 (:REWRITE DEFAULT-UNARY-/))
 (8 2 (:LINEAR EXPT->-1-2))
 (8 2 (:LINEAR EXPT->-1-1))
 (8 2 (:LINEAR EXPT-<-1-2))
 (8 2 (:LINEAR EXPT-<-1-1))
 (2 1 (:REWRITE DEFAULT-EXPT-1))
 (2 1 (:REWRITE DEFAULT-*-1))
 (1 1 (:REWRITE DEFAULT-EXPT-2))
 (1 1 (:REWRITE DEFAULT-*-2)))
(|(equal (expt x n) 0)|
 (46 1 (:DEFINITION EXPT))
 (28 2 (:REWRITE DEFAULT-*-2))
 (24 24
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (24 24
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
 (24 24
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (24 24
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (24 24
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (24 24
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (24 24
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONNEGATIVE-BASE))
 (24 24
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (24 24
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (24 24
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (24 24
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (24 24
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (22 22
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (22 22
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (6 2 (:REWRITE |(+ y x)|))
 (3 3 (:REWRITE DEFAULT-EXPT-2))
 (3 3 (:REWRITE DEFAULT-EXPT-1))
 (2 2 (:REWRITE DEFAULT-+-2))
 (2 2 (:REWRITE DEFAULT-+-1))
 (2 2 (:REWRITE DEFAULT-*-1))
 (1 1 (:REWRITE ZIP-OPEN))
 (1 1
    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (1 1 (:REWRITE DEFAULT-UNARY-/))
 (1 1 (:REWRITE DEFAULT-<-2))
 (1 1 (:REWRITE DEFAULT-<-1)))
(|(expt (+ x y) 2)|
 (57 57
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (57 57
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (30 30
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (24 24 (:REWRITE DEFAULT-<-2))
 (24 24 (:REWRITE DEFAULT-<-1))
 (19 11 (:REWRITE DEFAULT-+-2))
 (17 11 (:REWRITE DEFAULT-+-1))
 (12 4
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (12 4
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (12 4
     (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (12 4
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (9 5 (:REWRITE DEFAULT-EXPT-1))
 (6 2 (:LINEAR EXPT->-1-2))
 (6 2 (:LINEAR EXPT->-1-1))
 (6 2 (:LINEAR EXPT-<-1-2))
 (6 2 (:LINEAR EXPT-<-1-1))
 (5 5 (:REWRITE DEFAULT-EXPT-2))
 (4 4 (:REWRITE FOLD-CONSTS-IN-+))
 (3 3 (:REWRITE FOLD-CONSTS-IN-*))
 (1 1
    (:REWRITE FUNCTIONAL-COMMUTATIVITY-OF-MINUS-*-LEFT)))
(|(expt (+ x y) 3)|
 (116 39 (:REWRITE DEFAULT-+-2))
 (93 93
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (93 93
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (72 72
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (61 61 (:REWRITE FOLD-CONSTS-IN-*))
 (58 39 (:REWRITE DEFAULT-+-1))
 (27 27 (:REWRITE FOLD-CONSTS-IN-+))
 (24 24 (:REWRITE DEFAULT-<-2))
 (24 24 (:REWRITE DEFAULT-<-1))
 (22 14 (:REWRITE DEFAULT-EXPT-1))
 (14 14 (:REWRITE DEFAULT-EXPT-2))
 (12 4
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (12 4
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (12 4
     (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (12 4
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (6 2 (:LINEAR EXPT->-1-2))
 (6 2 (:LINEAR EXPT->-1-1))
 (6 2 (:LINEAR EXPT-<-1-2))
 (6 2 (:LINEAR EXPT-<-1-1))
 (2 2
    (:REWRITE FUNCTIONAL-COMMUTATIVITY-OF-MINUS-*-LEFT)))
(|(expt x c)|
 (314 314
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (314 314
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (314 314
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (314
     314
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (312 312
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (312 312
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (304 304
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (282 282
      (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (282 282
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (282 282
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (131 10 (:REWRITE DEFAULT-*-2))
 (28 28
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (26 26
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (26 26
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (26 26
     (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (26 26
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (26 10 (:REWRITE DEFAULT-*-1))
 (17 17 (:REWRITE DEFAULT-EXPT-2))
 (13 13 (:LINEAR EXPT->-1-2))
 (13 13 (:LINEAR EXPT->-1-1))
 (13 13 (:LINEAR EXPT-<-1-2))
 (13 13 (:LINEAR EXPT-<-1-1))
 (10 10 (:REWRITE DEFAULT-<-2))
 (10 10 (:REWRITE DEFAULT-<-1))
 (5 5 (:REWRITE DEFAULT-UNARY-MINUS))
 (4 4 (:REWRITE DEFAULT-+-2))
 (4 4 (:REWRITE DEFAULT-+-1))
 (2 2
    (:REWRITE EXPONENTS-ADD-FOR-NONPOS-EXPONENTS))
 (2 2
    (:REWRITE EXPONENTS-ADD-FOR-NONNEG-EXPONENTS))
 (2 2 (:REWRITE EXPONENTS-ADD-2))
 (2 1 (:REWRITE |(expt x 1)|))
 (1 1 (:REWRITE FOLD-CONSTS-IN-*)))
(|(expt x (- n))|
 (1085 1085
       (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (1085 1085
       (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (1085
      1085
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (1085
     1085
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (1076 1076
       (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (1070 1070
       (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (1070 1070
       (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (1070 1070
       (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (946 64 (:REWRITE DEFAULT-*-2))
 (837 35 (:REWRITE DEFAULT-UNARY-/))
 (646 646
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (646 646
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (229 64 (:REWRITE DEFAULT-*-1))
 (187 127 (:REWRITE DEFAULT-<-2))
 (143 143
      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (140 127 (:REWRITE DEFAULT-<-1))
 (68 24
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (68 24
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (68 24
     (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (68 24
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (46 15 (:REWRITE <-0-MINUS))
 (44 44 (:REWRITE DEFAULT-+-2))
 (44 44 (:REWRITE DEFAULT-+-1))
 (37 22 (:REWRITE ZIP-OPEN))
 (34 12 (:LINEAR EXPT->-1-2))
 (34 12 (:LINEAR EXPT->-1-1))
 (34 12 (:LINEAR EXPT-<-1-2))
 (34 12 (:LINEAR EXPT-<-1-1))
 (13 8 (:REWRITE |(equal (expt x n) 0)|))
 (2 2 (:REWRITE FOLD-CONSTS-IN-*)))
(|(expt (/ x) n)|
  (6 6
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (6 6
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP)))
(|(expt (- (/ x)) n)|
 (558 558
      (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (558 558
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
 (558 558
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (558 558
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (558 558
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (558 558
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (558 558
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (558
     558
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (558 558
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (558 558
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (342 33 (:REWRITE DEFAULT-*-2))
 (259 41 (:REWRITE DEFAULT-UNARY-MINUS))
 (230 15 (:REWRITE DEFAULT-UNARY-/))
 (170 170
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (170 170
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (127 33 (:REWRITE DEFAULT-*-1))
 (48 36 (:REWRITE DEFAULT-<-1))
 (48 12 (:REWRITE <-0-MINUS))
 (36 36 (:REWRITE DEFAULT-<-2))
 (36 12 (:REWRITE |(+ y x)|))
 (33 33
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (22 22 (:REWRITE DEFAULT-EXPT-1))
 (20 10
     (:REWRITE EXPT-NEGATIVE-BASE-ODD-EXPONENT))
 (20 10
     (:REWRITE EXPT-NEGATIVE-BASE-EVEN-EXPONENT))
 (20 4
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (20 4
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (16 16 (:REWRITE |(- (* c x))|))
 (14 14 (:REWRITE |(expt x c)|))
 (14 14 (:REWRITE |(expt x (- n))|))
 (12 12 (:REWRITE DEFAULT-+-2))
 (12 12 (:REWRITE DEFAULT-+-1))
 (12 4
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (12 4
     (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (10 2 (:LINEAR EXPT->-1-2))
 (10 2 (:LINEAR EXPT-<-1-1))
 (9 9 (:REWRITE ZIP-OPEN))
 (6 2 (:LINEAR EXPT->-1-1))
 (6 2 (:LINEAR EXPT-<-1-2))
 (3 1
    (:REWRITE FUNCTIONAL-SELF-INVERSION-OF-/))
 (1 1 (:REWRITE |(* 0 x)|)))
(|(expt 1/c n)|
 (114 2 (:DEFINITION EXPT))
 (64 4 (:REWRITE DEFAULT-*-2))
 (35 35
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (35 35
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (13 13
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (12 4 (:REWRITE |(+ y x)|))
 (11 6 (:REWRITE DEFAULT-EXPT-1))
 (8 2 (:REWRITE DEFAULT-UNARY-/))
 (6 4 (:REWRITE DEFAULT-*-1))
 (4 4 (:REWRITE DEFAULT-+-2))
 (4 4 (:REWRITE DEFAULT-+-1))
 (4 4 (:REWRITE |(expt x c)|))
 (4 4 (:REWRITE |(expt x (- n))|))
 (2 2 (:REWRITE ZIP-OPEN))
 (2 2 (:REWRITE DEFAULT-<-2))
 (2 2 (:REWRITE DEFAULT-<-1)))
(|(expt (- x) n)|
 (458 38 (:REWRITE DEFAULT-*-2))
 (218 218
      (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (218 218
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
 (218 218
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (218 218
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (218 218
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (218 218
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (218 218
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (218 218
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (218 218
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (218 218
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (218
     218
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (218 218
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (218 218
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (176 19 (:REWRITE DEFAULT-UNARY-MINUS))
 (95 95
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (79 47 (:REWRITE DEFAULT-EXPT-1))
 (56 14 (:REWRITE DEFAULT-UNARY-/))
 (52 38 (:REWRITE DEFAULT-*-1))
 (47 47 (:REWRITE DEFAULT-EXPT-2))
 (28 28 (:REWRITE DEFAULT-+-2))
 (28 28 (:REWRITE DEFAULT-+-1))
 (19 19 (:REWRITE |(expt x c)|))
 (19 19 (:REWRITE |(expt x (- n))|))
 (19 19 (:REWRITE |(expt 1/c n)|))
 (14 14 (:REWRITE ZIP-OPEN))
 (14 14 (:REWRITE DEFAULT-<-2))
 (14 14 (:REWRITE DEFAULT-<-1))
 (4 4 (:REWRITE |(- (* c x))|)))
(POWER-OF-2-HELPER)
(POWER-OF-2)
(|(expt 2^i n)|
 (154 154
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (154 154
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (153 153
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (134 30
      (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (134 30
      (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (122 30
      (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (122 30
      (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (112 15 (:LINEAR EXPT->-1-1))
 (112 15 (:LINEAR EXPT-<-1-2))
 (106 3 (:DEFINITION EXPT))
 (99 63 (:REWRITE DEFAULT-<-2))
 (99 63 (:REWRITE DEFAULT-<-1))
 (68 15 (:LINEAR EXPT->-1-2))
 (68 15 (:LINEAR EXPT-<-1-1))
 (44 4 (:REWRITE 0-<-*))
 (42 8 (:REWRITE DEFAULT-*-2))
 (34 6 (:DEFINITION IFF))
 (28 2 (:REWRITE <-*-0))
 (18 6 (:REWRITE |(+ y x)|))
 (16 10 (:REWRITE DEFAULT-EXPT-1))
 (10 10 (:REWRITE DEFAULT-EXPT-2))
 (9 3 (:REWRITE ZIP-OPEN))
 (8 8 (:REWRITE DEFAULT-*-1))
 (6 6 (:REWRITE DEFAULT-+-2))
 (6 6 (:REWRITE DEFAULT-+-1))
 (6 2 (:REWRITE EQUAL-*-X-Y-0))
 (4 4 (:REWRITE |(expt x c)|))
 (4 4 (:REWRITE |(expt x (- n))|))
 (4 4 (:REWRITE |(expt 1/c n)|))
 (4 4 (:REWRITE |(expt (- x) n)|))
 (4 4 (:DEFINITION FIX)))
(|(expt (* x y) n)|
 (228 4 (:DEFINITION EXPT))
 (164 12 (:REWRITE DEFAULT-*-2))
 (91 91
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (91 91
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (48 12 (:REWRITE DEFAULT-*-1))
 (30 30
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (24 8 (:REWRITE |(+ y x)|))
 (23 13 (:REWRITE DEFAULT-EXPT-1))
 (16 4 (:REWRITE DEFAULT-UNARY-/))
 (8 8 (:REWRITE DEFAULT-+-2))
 (8 8 (:REWRITE DEFAULT-+-1))
 (8 8 (:REWRITE |(expt x c)|))
 (8 8 (:REWRITE |(expt x (- n))|))
 (8 8 (:REWRITE |(expt 2^i n)|))
 (8 8 (:REWRITE |(expt 1/c n)|))
 (8 8 (:REWRITE |(expt (- x) n)|))
 (8 4 (:DEFINITION FIX))
 (4 4 (:REWRITE ZIP-OPEN))
 (4 4 (:REWRITE DEFAULT-<-2))
 (4 4 (:REWRITE DEFAULT-<-1)))
(|(expt (expt x m) n)|
 (859 319 (:REWRITE DEFAULT-<-2))
 (664 52
      (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (664 52
      (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (664 52
      (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (664 52
      (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (427 319 (:REWRITE DEFAULT-<-1))
 (340 26 (:LINEAR EXPT->-1-2))
 (340 26 (:LINEAR EXPT->-1-1))
 (340 26 (:LINEAR EXPT-<-1-2))
 (340 26 (:LINEAR EXPT-<-1-1))
 (307 307
      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (270 214
      (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (270 214
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
 (270 214
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (214 214
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (214 214
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (214 214
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (214 214
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (214 214
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (214 214
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (214 214
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (214
     214
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (214 214
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (214 214
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (189 3 (:DEFINITION EXPT))
 (98 8 (:REWRITE DEFAULT-*-2))
 (32 10 (:REWRITE DEFAULT-EXPT-1))
 (18 6 (:REWRITE |(+ y x)|))
 (16 2 (:REWRITE 0-<-*))
 (12 3 (:REWRITE DEFAULT-UNARY-/))
 (11 8 (:REWRITE DEFAULT-*-1))
 (10 10 (:REWRITE DEFAULT-EXPT-2))
 (10 7 (:DEFINITION FIX))
 (9 3 (:REWRITE ZIP-OPEN))
 (6 6 (:REWRITE DEFAULT-+-2))
 (6 6 (:REWRITE DEFAULT-+-1))
 (6 2 (:REWRITE EQUAL-*-X-Y-0))
 (6 2 (:DEFINITION IFF))
 (4 4 (:REWRITE |(expt x c)|))
 (4 4 (:REWRITE |(expt x (- n))|))
 (4 4 (:REWRITE |(expt 2^i n)|))
 (4 4 (:REWRITE |(expt 1/c n)|))
 (4 4 (:REWRITE |(expt (- x) n)|)))
(|(expt x (+ m n))|
 (228 4 (:DEFINITION EXPT))
 (160 10 (:REWRITE DEFAULT-*-2))
 (87 87
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (87 87
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (44 10 (:REWRITE DEFAULT-*-1))
 (27 27
     (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
 (24 13 (:REWRITE DEFAULT-EXPT-1))
 (24 8 (:REWRITE |(+ y x)|))
 (16 4 (:REWRITE DEFAULT-UNARY-/))
 (13 13 (:REWRITE DEFAULT-EXPT-2))
 (11 11 (:REWRITE DEFAULT-+-2))
 (11 11 (:REWRITE DEFAULT-+-1))
 (6 6 (:REWRITE DEFAULT-<-2))
 (6 6 (:REWRITE DEFAULT-<-1))
 (5 5 (:REWRITE |(expt x c)|))
 (5 5 (:REWRITE |(expt x (- n))|))
 (5 5 (:REWRITE |(expt 2^i n)|))
 (5 5 (:REWRITE |(expt 1/c n)|))
 (5 5 (:REWRITE |(expt (- x) n)|))
 (4 4 (:REWRITE ZIP-OPEN))
 (4 4 (:REWRITE DEFAULT-UNARY-MINUS))
 (3 1
    (:REWRITE EXPONENTS-ADD-FOR-NONPOS-EXPONENTS))
 (3 1
    (:REWRITE EXPONENTS-ADD-FOR-NONNEG-EXPONENTS))
 (1 1 (:REWRITE EXPONENTS-ADD-2)))
(|(expt x (+ m n)) non-zero x|
 (276 6 (:DEFINITION EXPT))
 (196 14 (:REWRITE DEFAULT-*-2))
 (155 155
      (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (155 155
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
 (155 155
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (155 155
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
 (155 155
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (155 155
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (155 155
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (155
     155
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (155 155
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (155 155
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (40 14 (:REWRITE DEFAULT-*-1))
 (36 12 (:REWRITE |(+ y x)|))
 (33 7 (:REWRITE DEFAULT-UNARY-/))
 (20 20
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (20 20
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (18 18 (:REWRITE DEFAULT-EXPT-2))
 (18 18 (:REWRITE DEFAULT-EXPT-1))
 (14 14 (:REWRITE DEFAULT-+-2))
 (14 14 (:REWRITE DEFAULT-+-1))
 (7 7 (:REWRITE |(expt 2^i n)|))
 (7 7 (:REWRITE |(expt 1/c n)|))
 (7 7 (:REWRITE |(expt (- x) n)|))
 (6 6 (:REWRITE ZIP-OPEN))
 (6 6 (:REWRITE DEFAULT-<-2))
 (6 6 (:REWRITE DEFAULT-<-1))
 (6 6 (:REWRITE |(expt x c)|))
 (5 5 (:REWRITE DEFAULT-UNARY-MINUS))
 (2 2
    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(|(equal (/ x) 0)| (1 1
                      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(|(equal (- x) 0)| (1 1
                      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(|(equal (/ x) (/ y))| (2 2
                          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(|(equal (- x) (- y))| (2 2
                          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP)))
(|(< (/ x) 0)| (3 1 (:REWRITE DEFAULT-UNARY-/))
               (2 2 (:REWRITE DEFAULT-<-2))
               (2 2 (:REWRITE DEFAULT-<-1))
               (1 1 (:REWRITE |(equal (- x) 0)|))
               (1 1 (:REWRITE |(equal (- x) (- y))|)))
(|(< 0 (/ x))| (3 1 (:REWRITE DEFAULT-UNARY-/))
               (2 2 (:REWRITE DEFAULT-<-2))
               (2 2 (:REWRITE DEFAULT-<-1))
               (1 1 (:REWRITE |(equal (- x) 0)|))
               (1 1 (:REWRITE |(equal (- x) (- y))|)))
(|(< (- x) 0)|)
(|(< 0 (- x))|)
(|(< (- x) (- y))|)
(|(equal (+ c x) d)| (5 5
                        (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                     (5 5 (:REWRITE DEFAULT-+-2))
                     (5 5 (:REWRITE DEFAULT-+-1))
                     (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
                     (2 2 (:REWRITE |(equal (- x) (- y))|)))
(|(equal (+ c x y) d)| (13 10 (:REWRITE DEFAULT-+-2))
                       (10 10 (:REWRITE DEFAULT-+-1))
                       (8 8
                          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                       (4 4 (:REWRITE FOLD-CONSTS-IN-+))
                       (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
                       (2 2 (:REWRITE |(equal (- x) (- y))|))
                       (2 2 (:REWRITE |(equal (+ c x) d)|)))
(|(equal (+ c x) (+ d y))| (9 9 (:REWRITE DEFAULT-+-2))
                           (9 9 (:REWRITE DEFAULT-+-1))
                           (8 8
                              (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                           (3 3 (:REWRITE |(equal (- x) (- y))|))
                           (3 3 (:REWRITE |(equal (+ c x) d)|))
                           (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
                           (2 2 (:REWRITE DEFAULT-<-2))
                           (2 2 (:REWRITE DEFAULT-<-1))
                           (2 2 (:REWRITE |(< (- x) (- y))|))
                           (1 1 (:REWRITE FOLD-CONSTS-IN-+))
                           (1 1 (:REWRITE |(equal (+ c x y) d)|)))
(|(equal (+ d x) (+ c y))| (9 9 (:REWRITE DEFAULT-+-2))
                           (9 9 (:REWRITE DEFAULT-+-1))
                           (8 8
                              (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                           (3 3 (:REWRITE |(equal (- x) (- y))|))
                           (3 3 (:REWRITE |(equal (+ c x) d)|))
                           (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
                           (2 2 (:REWRITE DEFAULT-<-2))
                           (2 2 (:REWRITE DEFAULT-<-1))
                           (2 2 (:REWRITE |(equal (+ c x) (+ d y))|))
                           (2 2 (:REWRITE |(< (- x) (- y))|))
                           (1 1 (:REWRITE FOLD-CONSTS-IN-+))
                           (1 1 (:REWRITE |(equal (+ c x y) d)|)))
(EQUAL-TO-IFF (10 6 (:REWRITE DEFAULT-<-2))
              (10 6 (:REWRITE DEFAULT-<-1))
              (8 8
                 (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
              (6 6 (:REWRITE |(< (- x) (- y))|))
              (1 1 (:REWRITE |(equal (- x) (- y))|)))
(|(< (+ c x) d)| (3 3
                    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                 (3 3 (:REWRITE DEFAULT-<-2))
                 (3 3 (:REWRITE DEFAULT-<-1))
                 (3 3 (:REWRITE DEFAULT-+-2))
                 (3 3 (:REWRITE DEFAULT-+-1))
                 (3 3 (:REWRITE |(< (- x) (- y))|))
                 (2 2 (:REWRITE DEFAULT-UNARY-MINUS)))
(|(< (+ c x y) d)| (9 6 (:REWRITE DEFAULT-+-2))
                   (6 6
                      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                   (6 6 (:REWRITE DEFAULT-+-1))
                   (3 3 (:REWRITE DEFAULT-<-2))
                   (3 3 (:REWRITE DEFAULT-<-1))
                   (3 3 (:REWRITE |(< (- x) (- y))|))
                   (3 3 (:REWRITE |(< (+ c x) d)|))
                   (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
                   (1 1 (:REWRITE FOLD-CONSTS-IN-+)))
(|(< (+ c x) (+ d y))| (8 8 (:REWRITE DEFAULT-+-2))
                       (8 8 (:REWRITE DEFAULT-+-1))
                       (4 4
                          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                       (4 4 (:REWRITE DEFAULT-<-2))
                       (4 4 (:REWRITE DEFAULT-<-1))
                       (4 4 (:REWRITE |(< (- x) (- y))|))
                       (2 2 (:REWRITE FOLD-CONSTS-IN-+))
                       (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
                       (1 1 (:REWRITE |(< (+ c x) d)|)))
(|(< (+ d x) (+ c y))| (8 8 (:REWRITE DEFAULT-+-2))
                       (8 8 (:REWRITE DEFAULT-+-1))
                       (4 4
                          (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                       (4 4 (:REWRITE DEFAULT-<-2))
                       (4 4 (:REWRITE DEFAULT-<-1))
                       (4 4 (:REWRITE |(< (- x) (- y))|))
                       (3 3 (:REWRITE |(< (+ c x) d)|))
                       (2 2 (:REWRITE FOLD-CONSTS-IN-+))
                       (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
                       (2 2 (:REWRITE |(< (+ c x y) d)|))
                       (1 1 (:REWRITE |(< (+ c x) (+ d y))|)))
(|(< d (+ c x))| (3 3
                    (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                 (3 3 (:REWRITE DEFAULT-<-2))
                 (3 3 (:REWRITE DEFAULT-<-1))
                 (3 3 (:REWRITE DEFAULT-+-2))
                 (3 3 (:REWRITE DEFAULT-+-1))
                 (3 3 (:REWRITE |(< (- x) (- y))|))
                 (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
                 (2 2 (:REWRITE |(< (+ c x) d)|)))
(|(< d (+ c x y))| (9 6 (:REWRITE DEFAULT-+-2))
                   (6 6
                      (:REWRITE RATIONALP-IMPLIES-ACL2-NUMBERP))
                   (6 6 (:REWRITE DEFAULT-+-1))
                   (3 3 (:REWRITE DEFAULT-<-2))
                   (3 3 (:REWRITE DEFAULT-<-1))
                   (3 3 (:REWRITE |(< d (+ c x))|))
                   (3 3 (:REWRITE |(< (- x) (- y))|))
                   (2 2 (:REWRITE DEFAULT-UNARY-MINUS))
                   (2 2 (:REWRITE |(< (+ d x) (+ c y))|))
                   (2 2 (:REWRITE |(< (+ c x) d)|))
                   (2 2 (:REWRITE |(< (+ c x) (+ d y))|))
                   (1 1 (:REWRITE FOLD-CONSTS-IN-+)))
(|(equal (* x y) 0)| (5 5 (:REWRITE |(equal (- x) 0)|))
                     (5 5 (:REWRITE |(equal (- x) (- y))|))
                     (1 1 (:REWRITE DEFAULT-*-2))
                     (1 1 (:REWRITE DEFAULT-*-1)))
(|(< (* x y) 0)| (8 8 (:REWRITE |(< (- x) (- y))|))
                 (7 7 (:REWRITE DEFAULT-<-2))
                 (7 7 (:REWRITE DEFAULT-<-1))
                 (5 5 (:REWRITE |(equal (- x) (- y))|))
                 (4 4 (:REWRITE |(equal (- x) 0)|))
                 (4 4 (:REWRITE |(< 0 (- x))|))
                 (4 4 (:REWRITE |(< (- x) 0)|))
                 (1 1 (:REWRITE DEFAULT-*-2))
                 (1 1 (:REWRITE DEFAULT-*-1)))
(|(< 0 (* x y))| (8 8 (:REWRITE |(< (- x) (- y))|))
                 (7 7 (:REWRITE DEFAULT-<-2))
                 (7 7 (:REWRITE DEFAULT-<-1))
                 (6 6 (:REWRITE |(< 0 (- x))|))
                 (5 5 (:REWRITE |(equal (- x) (- y))|))
                 (4 4 (:REWRITE |(equal (- x) 0)|))
                 (2 2 (:REWRITE |(< (- x) 0)|))
                 (1 1 (:REWRITE DEFAULT-*-2))
                 (1 1 (:REWRITE DEFAULT-*-1)))
(EXPT-X->-X
  (38 38
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (38 38
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (38 38
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
  (38 2 (:DEFINITION EXPT))
  (14 2 (:REWRITE DEFAULT-*-2))
  (6 2 (:REWRITE |(+ y x)|))
  (4 4
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (4 4 (:REWRITE DEFAULT-EXPT-2))
  (4 4 (:REWRITE DEFAULT-EXPT-1))
  (4 4 (:REWRITE |(expt x (- n))|))
  (4 4 (:REWRITE |(expt 2^i n)|))
  (4 4 (:REWRITE |(expt 1/c n)|))
  (4 4 (:REWRITE |(expt (- x) n)|))
  (2 2 (:REWRITE DEFAULT-<-2))
  (2 2 (:REWRITE DEFAULT-<-1))
  (2 2 (:REWRITE DEFAULT-+-2))
  (2 2 (:REWRITE DEFAULT-+-1))
  (2 2 (:REWRITE DEFAULT-*-1))
  (2 2 (:REWRITE |(< 0 (- x))|))
  (2 2 (:REWRITE |(< (- x) (- y))|))
  (2 2
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
  (2 2
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
  (2 2
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
  (2 2 (:DEFINITION FIX))
  (1 1 (:LINEAR EXPT->-1-2))
  (1 1 (:LINEAR EXPT-<-1-2))
  (1 1 (:LINEAR EXPT-<-1-1)))
(EXPT-X->=-X
 (324 324
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (324 324
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (324 324
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (91 67 (:REWRITE DEFAULT-<-1))
 (81 81
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (81 81
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (81 81
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
 (69 69 (:REWRITE |(< (- x) (- y))|))
 (67 67 (:REWRITE DEFAULT-<-2))
 (64 64
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (64 64
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (64 64
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (64 64
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (56 14
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
 (56 14
     (:LINEAR EXPT-IS-INCREASING-FOR-BASE>1))
 (28 7 (:LINEAR EXPT-X->-X))
 (28 7 (:LINEAR EXPT->-1-1))
 (28 7 (:LINEAR EXPT-<-1-2))
 (22 4 (:REWRITE DEFAULT-*-2))
 (14 14
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
 (14 14
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
 (7 7 (:LINEAR EXPT->-1-2))
 (7 7 (:LINEAR EXPT-<-1-1))
 (6 6 (:REWRITE |(expt x (- n))|))
 (6 6 (:REWRITE |(expt 2^i n)|))
 (6 6 (:REWRITE |(expt 1/c n)|))
 (6 6 (:REWRITE |(expt (- x) n)|))
 (6 6 (:REWRITE |(< 0 (- x))|))
 (6 2 (:REWRITE |(+ y x)|))
 (5 5 (:REWRITE DEFAULT-EXPT-2))
 (5 5 (:REWRITE DEFAULT-EXPT-1))
 (4 4 (:REWRITE DEFAULT-*-1))
 (2 2 (:REWRITE DEFAULT-+-2))
 (2 2 (:REWRITE DEFAULT-+-1)))
(EXPT-IS-INCREASING-FOR-BASE->-1
  (72 72
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (72 72
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (72 72
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
  (68 2 (:DEFINITION EXPT))
  (28 4 (:REWRITE DEFAULT-*-2))
  (12 4 (:REWRITE |(+ y x)|))
  (8 8
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (6 6 (:REWRITE DEFAULT-EXPT-2))
  (6 6 (:REWRITE DEFAULT-EXPT-1))
  (6 6 (:REWRITE DEFAULT-<-2))
  (6 6 (:REWRITE DEFAULT-<-1))
  (6 6 (:REWRITE |(< (- x) (- y))|))
  (5 1 (:LINEAR EXPT->-1-1))
  (5 1 (:LINEAR EXPT-<-1-2))
  (4 4 (:REWRITE DEFAULT-+-2))
  (4 4 (:REWRITE DEFAULT-+-1))
  (4 4 (:REWRITE DEFAULT-*-1))
  (4 1 (:LINEAR EXPT-X->=-X))
  (4 1 (:LINEAR EXPT-X->-X))
  (3 3 (:REWRITE |(< 0 (- x))|))
  (2 2 (:REWRITE ZIP-OPEN))
  (2 2 (:REWRITE DEFAULT-UNARY-/))
  (2 2 (:REWRITE |(expt x c)|))
  (2 2 (:REWRITE |(expt x (- n))|))
  (2 2 (:REWRITE |(expt 2^i n)|))
  (2 2 (:REWRITE |(expt 1/c n)|))
  (2 2 (:REWRITE |(expt (- x) n)|))
  (2 2
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
  (2 2
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
  (2 2
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE<1))
  (2 2 (:DEFINITION FIX))
  (1 1 (:REWRITE |(< (- x) 0)|))
  (1 1 (:LINEAR EXPT->-1-2))
  (1 1 (:LINEAR EXPT-<-1-1)))
(EXPT-IS-DECREASING-FOR-POS-BASE-<-1
  (69 69
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (69 69
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (69 69
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
  (68 2 (:DEFINITION EXPT))
  (28 4 (:REWRITE DEFAULT-*-2))
  (12 4 (:REWRITE |(+ y x)|))
  (8 8
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
  (8 8
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (6 6 (:REWRITE DEFAULT-EXPT-2))
  (6 6 (:REWRITE DEFAULT-EXPT-1))
  (5 1 (:LINEAR EXPT->-1-2))
  (5 1 (:LINEAR EXPT-<-1-1))
  (4 4 (:REWRITE DEFAULT-<-2))
  (4 4 (:REWRITE DEFAULT-<-1))
  (4 4 (:REWRITE DEFAULT-+-2))
  (4 4 (:REWRITE DEFAULT-+-1))
  (4 4 (:REWRITE DEFAULT-*-1))
  (4 4 (:REWRITE |(< (- x) (- y))|))
  (3 3 (:REWRITE |(< 0 (- x))|))
  (2 2 (:REWRITE ZIP-OPEN))
  (2 2 (:REWRITE DEFAULT-UNARY-/))
  (2 2 (:REWRITE |(expt x c)|))
  (2 2 (:REWRITE |(expt x (- n))|))
  (2 2 (:REWRITE |(expt 2^i n)|))
  (2 2 (:REWRITE |(expt 1/c n)|))
  (2 2 (:REWRITE |(expt (- x) n)|))
  (2 2
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE>1))
  (2 2
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
  (2 2
     (:LINEAR EXPT-IS-INCREASING-FOR-BASE->-1))
  (2 2 (:DEFINITION FIX))
  (1 1 (:REWRITE |(< (- x) 0)|))
  (1 1 (:LINEAR EXPT-X->=-X))
  (1 1 (:LINEAR EXPT-X->-X))
  (1 1 (:LINEAR EXPT->-1-1))
  (1 1 (:LINEAR EXPT-<-1-2)))
(EXPT-IS-WEAKLY-INCREASING-FOR-BASE->-1
  (68 68
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (68 68
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (68 68
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
  (68 2 (:DEFINITION EXPT))
  (28 4 (:REWRITE DEFAULT-*-2))
  (12 4 (:REWRITE |(+ y x)|))
  (8 8 (:REWRITE DEFAULT-<-2))
  (8 8 (:REWRITE DEFAULT-<-1))
  (8 8 (:REWRITE |(< (- x) (- y))|))
  (8 2
     (:LINEAR EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE<1))
  (6 6 (:REWRITE DEFAULT-EXPT-2))
  (6 6 (:REWRITE DEFAULT-EXPT-1))
  (4 4
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (4 4 (:REWRITE DEFAULT-+-2))
  (4 4 (:REWRITE DEFAULT-+-1))
  (4 4 (:REWRITE DEFAULT-*-1))
  (4 1 (:LINEAR EXPT-X->=-X))
  (4 1 (:LINEAR EXPT-X->-X))
  (4 1 (:LINEAR EXPT->-1-1))
  (4 1 (:LINEAR EXPT-<-1-2))
  (2 2 (:REWRITE ZIP-OPEN))
  (2 2 (:REWRITE DEFAULT-UNARY-/))
  (2 2 (:REWRITE |(expt x c)|))
  (2 2 (:REWRITE |(expt x (- n))|))
  (2 2 (:REWRITE |(expt 2^i n)|))
  (2 2 (:REWRITE |(expt 1/c n)|))
  (2 2 (:REWRITE |(expt (- x) n)|))
  (2 2 (:REWRITE |(< 0 (- x))|))
  (2 2
     (:LINEAR EXPT-IS-INCREASING-FOR-BASE->-1))
  (2 2
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE-<-1))
  (2 2 (:DEFINITION FIX))
  (1 1 (:LINEAR EXPT->-1-2))
  (1 1 (:LINEAR EXPT-<-1-1)))
(EXPT-IS-WEAKLY-DECREASING-FOR-POS-BASE-<-1
  (68 2 (:DEFINITION EXPT))
  (65 65
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (65 65
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (65 65
      (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
  (28 4 (:REWRITE DEFAULT-*-2))
  (12 4 (:REWRITE |(+ y x)|))
  (6 6 (:REWRITE DEFAULT-EXPT-2))
  (6 6 (:REWRITE DEFAULT-EXPT-1))
  (6 6 (:REWRITE DEFAULT-<-2))
  (6 6 (:REWRITE DEFAULT-<-1))
  (6 6 (:REWRITE |(< (- x) (- y))|))
  (5 2
     (:LINEAR EXPT-IS-WEAKLY-INCREASING-FOR-BASE->-1))
  (4 4
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
  (4 4
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (4 4 (:REWRITE DEFAULT-+-2))
  (4 4 (:REWRITE DEFAULT-+-1))
  (4 4 (:REWRITE DEFAULT-*-1))
  (4 1 (:LINEAR EXPT-X->=-X))
  (4 1 (:LINEAR EXPT->-1-2))
  (4 1 (:LINEAR EXPT-<-1-1))
  (2 2 (:REWRITE ZIP-OPEN))
  (2 2 (:REWRITE DEFAULT-UNARY-/))
  (2 2 (:REWRITE |(expt x c)|))
  (2 2 (:REWRITE |(expt x (- n))|))
  (2 2 (:REWRITE |(expt 2^i n)|))
  (2 2 (:REWRITE |(expt 1/c n)|))
  (2 2 (:REWRITE |(expt (- x) n)|))
  (2 2 (:REWRITE |(< 0 (- x))|))
  (2 2
     (:LINEAR EXPT-IS-INCREASING-FOR-BASE->-1))
  (2 2
     (:LINEAR EXPT-IS-DECREASING-FOR-POS-BASE-<-1))
  (2 2 (:DEFINITION FIX))
  (1 1 (:LINEAR EXPT-X->-X))
  (1 1 (:LINEAR EXPT->-1-1))
  (1 1 (:LINEAR EXPT-<-1-2)))
(EXPT->-1-ONE
 (11 11
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (11 11
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (11 11
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (2 2
    (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT)))
(EXPT->-1-TWO
 (11 11
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (11 11
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (11 11
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (2 2
    (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT)))
(EXPT-<-1-ONE
 (13 13
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (13 13
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (13 13
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (2 2
    (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT)))
(EXPT-<-1-TWO
 (13 13
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
 (13 13
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
 (13 13
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP))
 (2 2
    (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
 (2 2
    (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT)))
(|(+ (if a b c) x)|)
(|(+ x (if a b c))|)
(|(- (if a b c))|)
(|(* (if a b c) x)|)
(|(* x (if a b c))|)
(|(/ (if a b c))|)
(|(expt (if a b c) x)|
  (9 9
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONNEGATIVE-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP)))
(|(expt x (if a b c))|
  (9 9
     (:TYPE-PRESCRIPTION RATIONALP-EXPT-TYPE-PRESCRIPTION))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-RATIONALP))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-2))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-POSITIVE-1))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONZERO))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NONNEGATIVE-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-ZERO-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NON-0-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-ODD-EXPONENT))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-NEGATIVE-BASE-EVEN-EXPONENT))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP-BASE))
  (9 9
     (:TYPE-PRESCRIPTION EXPT-TYPE-PRESCRIPTION-INTEGERP)))
(|(equal x (if a b c))|)
(|(equal (if a b c) x)|)
(|(< x (if a b c))|)
(|(< (if a b c) x)|)
