...
; BOZO Jared thinks this is not used for anything.
...
Under the hood, we try to write an object into a file.  Any failures (e.g.,
file not openable) will result in a hard Lisp error.
...
The ~c[obj] is the object to write.
...
(defun serialize-read-fn (filename hons-mode verbosep state)
  (declare (xargs :guard (and (stringp filename)
                              ...
                              (state-p state))
                  :stobjs state)
           (ignore filename hons-mode verbosep))
  (mv-let (erp val state)
    (read-acl2-oracle state)
    (declare (ignore erp))
    (mv val state)))
...
contents of the file.  Any failures (e.g., file not found, bad file format,
...
As general rules, if you do not care about honses then you should use
~c[:never] because it will load the file as fast as possible.  But if you are
later going to ~ilc[hons-copy] some part of the file's contents, then it is
likely faster to use ~c[:smart] or ~c[:always] instead of first creating
normal conses and then copying them to build honses.
