...
(defun make-value (value type-key &aux type )
  ...
  (cond ((and
	 ...
	 (t  (let (chk)
		   ...
		   (or (typep ,(or chk value) ',type) (type-error))
		   ,(or chk value)))))))
	(t (list value))))
  ...
  (when (and type(or force-type (null *no-declare*)))
    (dolist (v *loop-bindings*)
      (cond ((assoc var (car v)) (setf found t)
	     (do ((decs (cdr v) (cdr decs)))
		 ((null decs) (push nil (cdr v))(setf tem (cdr v)))
		 (when (or (and (eq (caar decs) 'type)
				(eq (third (car decs)) var))
			   (eql (second (car decs)) var))
		       (setf tem decs) (return 'done)))
	     (setf (car tem)
		   (cond ((and (consp type) (eq (car type) 'type))
			  (list 'type (second type) var))
			 (t (list type var))))

	     (and  found (return 'done)))))
...
(defun parse-loop-for ( &aux direction inc)
  ...
  (let* ((var (loop-pop)) test incr)
    ...
    (let (type)
      ...
      (cond ((and direction (not *no-declare*)
		  (not test)
		  *type-check*
