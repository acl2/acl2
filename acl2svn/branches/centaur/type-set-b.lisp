...
; See term-order1 for comments.
  ...
  ACL2 must occasionally choose which of two terms is syntactically smaller.
  The need for such a choice arises, for example, when using equality
  ...
  (~pl[loop-stopper]), and in choosing the order in which to try to cancel the
  addends in linear arithmetic inequalities.  When this notion of syntactic
  ...
  (b) the numbers of variable occurrences in the two terms are equal but the
  number of function applications in ~c[term1] is less than that in ~c[term2],
  or

  (c) the numbers of variable occurrences in the two terms are equal, the
  numbers of functions applications in the two terms are equal, but
  pseudo-function application count for ~c[term1] is less than that for
  ~c[term2], or
  ...
  comes before ~c[term2] in a lexicographic ordering, ~ilc[lexorder], based
  their structure as Lisp objects: ~pl[lexorder].
  ...
  ~c[(Cons x x)] has two variable occurrences, not one.  Thus, perhaps
  counterintuitively, a large term that contains only one variable occurrence,
  e.g., ~c[(standard-char-p (car (reverse x)))] comes before ~c[(cons x x)] in
  the term order.

  Since constants contain no variable occurrences and non-constant expressions
  must contain at least one variable occurrence, constants come before
  non-constants in the term order, no matter how large the constants.  For
  example, the list constant
  ~bv[]
  '(monday tuesday wednesday thursday friday)
  ~ev[]
  comes before ~c[x] in the term order.  Because term order is involved in the
  control of permutative rewrite rules and used to shift smaller terms to the
  left, a set of permutative rules designed to allow the permutation of any two
  tips in a tree representing the nested application of some function will
  always move the constants into the left-most tips.  Thus,
  ~bv[]
  ...
  lexicographic means, based on their structures.  ~l[lexorder].  Thus, if two
  terms ~c[(member ...)] and ~c[(reverse ...)] contain the same numbers of
  variable occurrences and function applications, and no quoted constants, then
  the ~ilc[member] term is first in the term order because ~ilc[member] comes
  before ~ilc[reverse] in the term order (which is here reduced to alphabetic
  ordering).

  It remains to discuss the notion of ~em[pseudo-function application count].
  ...
  built by primitive constructor functions.  These constructor functions are
  not actually defined in ACL2 but merely imagined for the purposes of term
  order.  We here use suggestive names for these imagined functions, ignoring
  entirely the prior use of these names within ACL2.  The imagined applications
  of these functions are what we refer to as ~em[pseudo-function] applications.
  ...
  ~c[(s (s (z)))] and contains three function applications.  ~c[100] contains
  one hundred and one applications.  Negative integers are constructed from
  their positive counterparts by ~ilc[-].  Thus, ~c[-2] is ~c[(- (s (s (z))))]
  and has four applications.  Ratios are constructed by the dyadic function
  ~ilc[/].  Thus, ~c[-1/2] is
  ~bv[]
  (/ (- (s (z))) (s (s (z))))
  ~ev[]
  and contains seven applications.  Complex rationals are similarly constructed
  from rationals.  All character objects are considered primitive and are
  constructed by constant functions of the same name.  Thus ~c[#\\a] and
  ~c[#\\b] both contain one application.  Strings are built from the empty
  string, ~c[(o)] by the ``string-cons'' function written ~c[cs].  Thus
  ~c[\"AB\"] is ~c[(cs (#\\a) (cs (#\\b) (o)))] and contains five applications.
  Symbols are obtained from strings by ``packing'' the ~ilc[symbol-name] with
  the unary function ~c[p].  Thus ~c['ab] is
  ~bv[]
  (p (cs (#\\a) (cs (#\\b) (o))))
  ~ev[]
  and has six applications.  Note that packages are here ignored and thus
  ~c['acl2::ab] and ~c['my-package::ab] each contain just six applications.
  Finally, ~il[cons]es are built with ~ilc[cons], as usual.  So ~c['(1 . 2)] is
  ~c[(cons '1 '2)] and contains six applications, since ~c['1] contains two and
  ~c['2] contains three.  This, for better or worse, answers the question ``How
  ...
  bound this number by the constant ~c[(fn-count-evg-max-val)].  (This bound is
  important for efficiency, so that constants that are very large ~c[cons]
...
(defmacro accumulated-persistence (flg)
  ...
  For other proof debugging utilities, ~pl[break-rewrite] and ~pl[dmr].

  Accumulated persistence tracking can be turned on or off.  It is generally
  off.  When on, proofs may take perhaps 50% more time than otherwise!  But
  some useful numbers are collected.  When it is turned on, by
  ~bv[]
  ...
  ~c[:]~ilc[rewrite] rule is tried but its hypotheses are not successfully
  relieved, then that rule application and all work done on behalf of those
  hypotheses is ``useless'' work.  In general, an attempt to apply a ~il[rune]
  is viewed as ``useful'' unless the attempt fails or the attempt is on the
  stack (as described above) for a ~il[rune] application that ultimately fails.
  A large number of ``useless'' ~c[:frames] or ~c[:tries] along with
  ...
  ~c[show-accumulated-persistence].  Suffixes ``s'', ``f'', and ``a'' are
  ...
  (respectively, ``useless'').  We illustrate with the following example.
  ~bv[]
  (progn
    (defstub hyp (x) t)
    (defstub concl (x) t)
  ...
  ~ev[]
  To prove the ~ilc[thm] form, ACL2 attempts to rewrite ~c[(concl x)] to true
  by applying rule ~c[hyp-implies-concl].  It then attempts to establish
  ~c[(hyp x)] first by trying rule ~c[bad-ax], which fails, and second by
  trying rule ~c[good-ax], which succeeds.  As expected, the report labels as
  ...
  ~bv[]
     ...
     --------------------------------
  ~ev[]
  Now consider the top-level application of rule ~c[hyp-implies-concl].  Even
  though the above report shows the application of ~c[bad-ax] as ``useless'',
  note that this rule was applied on behalf of the successful (``useful'')
  application of ~c[hyp-implies-concl], and hence is incorporated into the
  ``useful'' line for ~c[hyp-implies-concl], as follows.
  ~bv[]
     --------------------------------
     ...
     --------------------------------
  ~ev[]
  In summary: categorization of ~c[:frames] as ``useful'' or ``useless'' is
  based on whether they support ``useful'' or ``useless'' ~c[:tries].
  ...
  If a proof is aborted, then in general, ~ilc[show-accumulated-persistence]
  ...
  ~ilc[show-accumulated-persistence] when such incomplete information is
  omitted, you can get this information by using argument ~c[:frames-a] or
  ~c[:tries-a].
...
(defdoc accumulated-persistence-subtleties
  ...
  As of this writing, we consider every ~c[:]~ilc[forward-chaining] rule
...
(defun type-set (x force-flg dwp type-alist ens w ttree pot-lst pt)
  ...
  ~c[(type-set 'term nil nil nil (ens state) (w state) nil nil nil)] will
