...
(deflabel extended-metafunctions
  ...
  ~c[(mfc-unify-subst mfc)]: returns ~c[nil] except when evaluating
  a ~ilc[syntaxp] or ~ilc[bind-free] hypothesis, in which case, returns the
  ...
  relieved (proved) under the giving variable bindings, ~c[alist].  Note that
  this function returns ~c[nil] if ~c[hyp] has free variables
  (~pl[free-variables]).  Here, ~c[hyp] is the hypothesis of the indicated
  ~ilc[rune] at (one-based) position ~c[bkptr], and ~c[target] is an
  instantiated term to which ~c[rune] is being applied.  Note that no
  indication is returned for whether any assumptions have been generated by
  ~ilc[force] or ~ilc[case-split].  (If you need such a feature, feel free to
  request it of the ACL2 implementors.)
