...
; one in stobj-creatorp.
  ...
  (when (null defun-mode) ; non-executable, generated by intro-udf-lst2
    (return-from oneify-cltl-code
                 (case-match def
...
#+mswindows
(defun set-mswindows-drive (state)
  (let* ((str (namestring (truename "")))
         (posn (position #\: str)))
    (when (null posn)
      (error
       "We are unable to determine the drive using ~s, which evaluates to ~s."
       '(namestring (truename ""))
       str))
...
; defined (needed by pathname-os-to-unix).  First, though, set mswindows-drive,
; which is used by pathname-os-to-unix (which is called by unix-full-pathname,
; which is called below).

  #+mswindows
  (set-mswindows-drive *the-live-state*)
...
; This function lds all of the *acl2-files* (except "*-raw.lisp") in
...
; For Windows, we set the drive before we set the distributed-books-dir.  Note
; that we initially set both of these during the build; but we want to support
; use of the image on a machine with a different drive and/or
; distributed-books-dir than are present at build time.  Because of the way
; unix-full-pathname (called by unix-full-pathname) works, if ACL2_SYSTEM_BOOKS
; has the drive (e.g. "C:") as a prefix, then that won't change below even if
; we set the drive differently (based on the user's start-up directory).

          #+mswindows (set-mswindows-drive state)
          (f-put-global 'distributed-books-dir
