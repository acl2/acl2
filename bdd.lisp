...
; that
  ...
  `(the-fixnum
    (let ((x ,x))
      (declare (type (signed-byte #.*fixnum-bits*) x))
...
;    (declare (type (signed-byte #.*fixnum-bits*) new-mx-id))
...
; "turnaround" here earlier.  But we have not yet checked this claim carefully.
  ...
  (declare (type (signed-byte #.*fixnum-bits*) i acc)
...
; fixnum for it in GCL.
  ...
  (declare (type (signed-byte #.*fixnum-bits*) op-code))
  (the-mv
   2
   (signed-byte #.*fixnum-bits*)
   (let ((n (op-hash-index op-code args)))
     (declare (type (signed-byte #.*fixnum-bits*) n))
...
(defun op-hash-index-string (index acc string)
  (declare (type (signed-byte #.*fixnum-bits*) index acc))
  (the-fixnum
   (cond
    ((= index 0) acc)
    (t (let ((index (1- (the-fixnum index))))
         (declare (type (signed-byte #.*fixnum-bits*) index))
...
(defun bdd-quotep (term op-ht mx-id)
  (declare (type (signed-byte #.*fixnum-bits*) mx-id))
  (the-mv
   3
   (signed-byte #.*fixnum-bits*)
   (cond
    ...
    (t
     (mv-let (hash-index ans)
             ...
             (declare (type (signed-byte #.*fixnum-bits*) hash-index))
             (cond
                                               ...
                                               (bdd-quotep
                                                (kwote (- (cadr term)
                                                          (fix (cadr (fargn pat
                                                                            1)))))
                                                op-ht mx-id)
                                               (declare (type (signed-byte #.*fixnum-bits*)
                                                              mx-id))
                                               (one-way-unify1-cst
                                                mx-id (fargn pat 2)
                                                cst alist op-ht)))
...
; two hash arrays.
  ...
  (declare (type (signed-byte #.*fixnum-bits*) n mx-id))
  (the-mv
   4
   (signed-byte #.*fixnum-bits*)
...
; version of the code.
         ...
         (t (let ((m (if-hash-index x y z)))
              (declare (type (signed-byte #.*fixnum-bits*) m))
...
; terms as false-branch-args.
  ...
  (declare (type (signed-byte #.*fixnum-bits*) var-id))
  (if (endp args)
      (mv nil nil)
    (mv-let (x y)
...
; that op is not a member of bdd-constructors.
  ...
  (declare (type (signed-byte #.*fixnum-bits*) hash-index mx-id))
  (the-mv
   5
   (signed-byte #.*fixnum-bits*)
   (let ((new-mx-id (1+mx-id mx-id))
...
; Keep this case in sync with make-if.
    ...
    ((let ((true-split-var (split-var true-cst))
       ...
       (declare (type (signed-byte #.*fixnum-bits*) test-id))
...
; combine-op-csts-comm.
  ...
  (declare (type (signed-byte #.*fixnum-bits*) op-code mx-id))
  (the-mv
   5
   (signed-byte #.*fixnum-bits*)
   (mv-let
...
; are all the bdd rules that rewrite calls of the function symbol op.
  ...
  (declare (type (signed-byte #.*fixnum-bits*) op-code mx-id hash-index))
  (the-mv
   5
   (signed-byte #.*fixnum-bits*)
   (mv-let
...
(defun bdd (term alist op-ht if-ht mx-id ttree bddspv state)
  (declare (xargs :measure (acl2-count term)
           ...
           (type (signed-byte #.*fixnum-bits*) mx-id))
  (the-mv
   5
   (signed-byte #.*fixnum-bits*)
   (cond
...
; We need a cst next, though we don't care about it.
    ...
    (t (mv-let
        ...
        (declare (type (signed-byte #.*fixnum-bits*) opcode))
        (cond
         (comm-p
          (bdd-mv-let
