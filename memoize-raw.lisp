...
; Fn is the name of a function.  Min-siz is the minimum size of the table we we
; will create --- perhaps a size that the user says to use.  We want to create
; and return a new hash table for this function's memoization table.  One
; possible implementation of this function would just be:
;
...
; rate (.83 MB instead of 1 MB) hurts us and so we have left it.
  ...
  (let* ((max-sizes ; previously recorded sizes of this table, if any exist
    ...
    (if (not max-sizes)
        ...
        ;; anything to add.  Fall through to the default.
        min-size
      (let* ((nclears       (access memo-max-sizes-entry max-sizes :num-clears))
        ...
        (when (and (< (integer-length (- nclears 1))
                   ...
                   *memoize-verbose*)
          (format t "; ~s memo table: ~s clears, avg size ~s, guess ~s, final ~s~%"
...
; memoize-fn only affects the memoize table and not the initial pons table.
      ...
      (let* ((nclears       (access memo-max-sizes-entry max-sizes :num-clears))
        ...
        (when (and (< (integer-length (- nclears 1))
                      (integer-length nclears))
                   *memoize-verbose*)
          (format t "; ~s pons table: ~s clears, avg size ~s, guess ~s, final ~s~%"
...
(defun memoize-fn-outer-body (inner-body fn fn-col-base start-ticks forget prog1-fn)
             ...
             `((safe-incf
                ...
                (the-mfixnum
                 (fix-ticks (the-mfixnum (- (internal-real-ticks)
                            ...
                            ',fn)))))
      ,@(and forget
             ;; Clear the hash table or set it to NIL, using the usual
             ;; heuristics.  Historically we always set them to NIL, but
             ;; it seems better to use clrhash when it's cheap, to avoid
...
;  (1) has the advantage of not allocating new memory/making new garbage.
;  However, it has the following disadvantages:
;     - it takes runtime proportional to the table size (we think).  On CCL, it
;       seems that clrhash on a full hash table is slightly slower than
;       reallocating that table (~1.2x), whereas clrhash on an empty hash table
;       is significantly faster (~0.4x) than reallocating.
;     - it prevents memory from being freed, if there is a GC before the table
;       would have been recreated,
;     - it prevents us from shrinking the table to a more reasonable size, if
;       the current table is exceptionally large.
