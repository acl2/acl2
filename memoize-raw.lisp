...
; PONSING ---------------------------------------------------------------------
;
; [Jared]: dealt with this in books/centaur/memoize/pons.lsp.

; 8/27-29/2014: Mod for soundness fix.  Much of what's below, before the
; definition of pist*, comes from the above file.
...
; We might want to think about creating a static cons here.  But in order to
; benefit from that, we would need to find a good way to use its index.  Even
; if we find such a use, we would need to hang on to that (static) cons so that
; it isn't garbage collected; so no obvious benefit is obvious.
